/*
 * settings.h - BaM Read/Write Benchmark 설정 파일
 *
 * 커맨드라인 인자를 파싱하여 벤치마크 실행 설정을 관리한다.
 * getopt_long 기반의 옵션 파서 프레임워크(OptionIface/Option/Range)를 포함한다.
 *
 * 주요 설정:
 *   -f: 입력 파일 경로 (NVMe에 쓸 원본 데이터)
 *   -i: 입력 파일 내 읽기 시작 오프셋 (ifileoffset)
 *   -l: NVMe SSD 내 쓰기 시작 오프셋 (ofileoffset)
 *   -o: 접근 타입 (0=READ, 1=WRITE, 2=MIXED, 3=VERIFY)
 *   -g: CUDA GPU 디바이스 번호
 *   -k: NVMe 컨트롤러 수
 *   -p: 페이지 캐시 내 페이지 수
 *   -P: 페이지 크기 (바이트)
...
/* I/O 접근 타입 상수 */
#define READ 0       /* NVMe에서 읽기 */
#define WRITE 1      /* NVMe에 쓰기 */
#define MIXED 2      /* 읽기/쓰기 혼합 (ratio로 비율 지정) */
#define VERIFY 3     /* 원본 파일과 NVMe 데이터 비교 검증 */

struct Settings
{
    uint32_t        cudaDevice;       /* 사용할 CUDA GPU 디바이스 번호 (기본: 0) */
    uint64_t        cudaDeviceId;     /* DIS 클러스터에서의 CUDA 디바이스 FDID */
    const char*     blockDevicePath;  /* 블록 디바이스 경로 (미사용) */
    const char*     controllerPath;   /* NVMe 컨트롤러 디바이스 경로 (미사용, ctrls_paths[] 사용) */
    uint64_t        controllerId;     /* DIS 클러스터에서의 컨트롤러 FDID */
    ...
    size_t          numReqs;          /* 스레드당 반복 요청 수 (기본: 1) */
    size_t          numPages;         /* 페이지 캐시 내 페이지 수 (기본: 1024) */
    size_t          startBlock;       /* 시작 블록 오프셋 (미사용) */
    bool            stats;            /* 통계 출력 여부 */
    const char*     input;            /* 입력 파일 경로 (-f 옵션) */
    const char*     output;           /* 출력 파일 경로 (미사용) */
    size_t          ifileoffset;      /* 입력 파일 내 시작 오프셋 (바이트, -i 옵션) */
    size_t          ofileoffset;      /* NVMe SSD 내 쓰기 시작 오프셋 (바이트, -l 옵션) */
    ...
    size_t blkSize;                   /* CUDA 블록당 스레드 수 (기본: 64) */
    size_t queueDepth;                /* NVMe SQ/CQ 큐 깊이 (기본: 16) */
    size_t numQueues;                 /* 컨트롤러당 I/O 큐 수 (기본: 1) */
    size_t pageSize;                  /* 페이지 캐시 페이지 크기 (바이트, 기본: 4096) */
    uint64_t numBlks;                 /* 백킹 배열 페이지 수 (기본: 2097152 = 2M) */
    bool random;                      /* true=랜덤 접근, false=순차 접근 */
    uint64_t accessType;              /* 접근 타입: READ(0)/WRITE(1)/MIXED(2)/VERIFY(3) */
    uint64_t ratio;                   /* MIXED 모드에서 읽기 비율 (0~100%, 기본: 100) */
    Settings();
