/*
 * BaM Connected Components (CC) Benchmark (cc/main.cu)
 *
 * 그래프의 연결 요소(Connected Components)를 GPU에서 계산하는 벤치마크.
 * Shiloach-Vishkin 알고리즘 기반의 label propagation 방식을 사용한다.
 *
 * 알고리즘:
 *   1. 각 정점의 컴포넌트 라벨을 자기 자신으로 초기화
 *   2. 반복적으로 간선을 순회하며, 인접 정점 중 더 작은 라벨로 atomicMin 업데이트
 *   3. 라벨 변경이 없을 때까지 반복 (수렴)
 *
 * 커널 변형: BASELINE, COALESCE, COALESCE_CHUNK, *_PC(페이지캐시), *_HASH(해싱) 등
 * GPU 메모리 모드: GPUMEM, UVM_READONLY, UVM_DIRECT, BAFS_DIRECT
 *
 * CSR(Compressed Sparse Row) 그래프 포맷:
 *   - offsets[]: 각 정점의 인접 리스트 시작 인덱스 (크기: |V|+1)
 *   - edges[]: 간선 목적지 정점 배열 (크기: |E|)
 *
 * BaM I/O: page_cache를 통해 SSD의 CSR 데이터에 GPU에서 직접 접근
 *
 * References:
 *      Coalesce
 *          Hong, Sungpack, et al.
...
/* cc_compute: CC 핵심 디바이스 함수. Shiloach-Vishkin 스타일 label propagation.
 * 현재 정점(cid)과 이웃 정점(next)의 컴포넌트 라벨을 비교하여 더 작은 값으로 갱신한다.
 * @param cid: 현재 정점 ID
 * @param comp: 컴포넌트 라벨 배열 (comp[v] = v가 속한 컴포넌트의 대표 정점 ID)
 * @param next: 이웃 정점 ID (edgeList에서 읽은 값)
 * @param next_visit: 다음 반복에서 방문할 정점 표시 배열
 * @param changed: 이번 반복에서 라벨 변경이 있었는지 표시하는 플래그
 */
__device__ void cc_compute(uint64_t cid, unsigned long long *comp, EdgeT next, bool *next_visit, bool *changed){

    unsigned long long comp_src = comp[cid];    // 현재 정점의 컴포넌트 라벨
    unsigned long long comp_next = comp[next];  // 이웃 정점의 컴포넌트 라벨
    unsigned long long comp_target;              // 갱신할 대상 라벨 값
    EdgeT next_target;                           // 갱신할 대상 정점 ID

    if (comp_next != comp_src) {                 // 두 정점의 라벨이 다르면 = 아직 같은 컴포넌트로 병합되지 않았으면
       if (comp_src < comp_next) {               // 현재 정점의 라벨이 더 작으면
          next_target = next;                    // 이웃 정점의 라벨을 갱신 대상으로
          comp_target = comp_src;                // 현재 정점의 (더 작은) 라벨로 갱신
       }
       else {                                    // 이웃 정점의 라벨이 더 작으면
          next_target = cid;                     // 현재 정점의 라벨을 갱신 대상으로
          comp_target = comp_next;               // 이웃 정점의 (더 작은) 라벨로 갱신
       }

       atomicMin(&comp[next_target], comp_target); // 원자적으로 최솟값 갱신 (경합 안전)
       next_visit[next_target] = true;             // 갱신된 정점을 다음 반복에서 재방문하도록 표시
       *changed = true;                            // 이번 반복에서 변경이 있었음을 표시 (수렴 검사용)
    }
}


/* kernel_baseline: BASELINE CC 커널. 정점당 1스레드가 인접 리스트를 순차 순회한다.
 * 프로파일링 코드 포함: 방문 카운트(vertexVisitCount_d), 이웃 크기 히스토그램(neigBin) */
__global__
void kernel_baseline(bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed, unsigned long long int *vertexVisitCount_d, uint64_t largebin, uint64_t binelems, unsigned long long int *neigBin) {
    const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x; // 2D 그리드에서 전역 스레드 ID 계산

    if(tid < vertex_count && curr_visit[tid] == true){  // 유효한 정점이고 이번 반복에서 활성(방문 대상)인 경우만 처리
        const uint64_t start = vertexList[tid];          // CSR: 정점 tid의 인접 리스트 시작 인덱스
        const uint64_t shift_start = start & 0xFFFFFFFFFFFFFFF0; // 16-원소(128B) 경계 정렬 (GPU 메모리 coalescing)
        const uint64_t end = vertexList[tid+1];          // CSR: 정점 tid의 인접 리스트 끝 인덱스 (exclusive)

        atomicAdd(&vertexVisitCount_d[tid], (end-start)); // 프로파일링: 이 정점이 처리한 간선 수 누적

        uint64_t diff = end - start;                      // 이웃(차수) 수
        uint64_t diffidx = diff/ binelems;                // 히스토그램 빈 인덱스 계산 (binelems 단위)
        if(diffidx>largebin){                             // 빈 범위 초과 시 마지막 빈에 넣음
                diffidx = largebin;
        }
        atomicAdd(&neigBin[diffidx], 1);                  // 프로파일링: 이웃 크기 히스토그램 업데이트

        for(uint64_t i = shift_start; i < end; i++){      // 정렬된 시작부터 끝까지 순회
        //for(uint64_t i = start; i < end; i++){
            if(i >= start){                                // shift_start로 앞당긴 부분은 건너뛰기 (정렬 패딩)
                const EdgeT next = edgeList[i];            // GPU 메모리에서 이웃 정점 ID 읽기
                cc_compute(tid, comp, next, next_visit, changed); // 라벨 전파 (더 작은 라벨로 갱신)
            }
        }
    }
}
...
/* kernel_baseline_pc: BASELINE + page_cache(BaM) 변형. da->seq_read(i)로 SSD에서 간선을 직접 읽는다. */
...
void kernel_baseline_pc(array_d_t<uint64_t>* da, bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed) {
    ...
    if(tid < vertex_count && curr_visit[tid] == true){   // 활성 정점만 처리
        const uint64_t start = vertexList[tid];           // 인접 리스트 시작
        const uint64_t shift_start = start & 0xFFFFFFFFFFFFFFF0; // 16-원소 정렬
        const uint64_t end = vertexList[tid+1];           // 인접 리스트 끝

        for(uint64_t i = shift_start; i < end; i++){
            if(i >= start){
                const EdgeT next = da->seq_read(i);       // BaM page_cache를 통한 SSD 직접 읽기 (캐시 미스 시 NVMe I/O 발생)
                cc_compute(tid, comp, next, next_visit, changed); // 라벨 전파
             }
        }
    }
}
...
/* kernel_baseline_hash: BASELINE + 해시 인터리빙. sm_count(SM 수) 기반으로 정점 ID를 인터리빙하여
 * 인접한 스레드가 서로 다른 SSD 페이지의 정점에 접근하도록 분산시킨다. */
__global__
void kernel_baseline_hash(bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed, int sm_count) {
    const uint64_t oldtid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x; // 원래 스레드 ID
    uint64_t STRIDE = sm_count;                   // 인터리빙 stride = SM 수

    if(oldtid < vertex_count){
        uint64_t tid;                              // 해싱 후 실제 정점 ID
        const uint64_t nep = vertex_count/STRIDE;  // STRIDE당 정점 수
        if(oldtid <(STRIDE*nep)){
                tid = (oldtid/nep) + ((oldtid % nep)*STRIDE); // 해시 재배치: oldtid → 분산된 tid
        }
        else{
                tid = oldtid;                      // 나머지 정점은 그대로
        }
...
/* kernel_baseline_hash_pc: BASELINE + 해시 인터리빙 + page_cache(BaM) 변형 */
__global__ //__launch_bounds__(64,32)
void kernel_baseline_hash_pc(array_d_t<uint64_t>* da, bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed, int sm_count) {
   const uint64_t oldtid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x; // 원래 스레드 ID
    uint64_t STRIDE = sm_count;                    // 인터리빙 stride
...
/* kernel_coalesce: 워프(32스레드) 단위 CC. warpIdx가 정점 ID이고, 32개 레인이 인접 리스트를 stride 접근.
 * shift_start로 16-원소 경계 정렬하여 GPU 메모리 coalescing을 달성한다. */
__global__
void kernel_coalesce(bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed) {
    ...
    const uint64_t laneIdx = tid & ((1 << WARP_SHIFT) - 1); // 워프 내 레인 번호 (0~31)

    if (warpIdx < vertex_count && curr_visit[warpIdx] == true) { // 유효한 활성 정점만 처리
       const uint64_t start = vertexList[warpIdx];               // 정점 warpIdx의 인접 리스트 시작
       const uint64_t shift_start = start & 0xFFFFFFFFFFFFFFF0;  // 16-원소(128B) 경계 정렬
       const uint64_t end = vertexList[warpIdx+1];               // 인접 리스트 끝

       for(uint64_t i = shift_start + laneIdx; i < end; i += WARP_SIZE) { // 레인별 stride 접근 (coalesced)
           if (i >= start) {                                      // 정렬 패딩 영역은 건너뛰기
...
/* kernel_coalesce_pc: COALESCE + page_cache(BaM). da->seq_read()로 SSD에서 간선을 직접 읽는다. */
...
void kernel_coalesce_pc(array_d_t<uint64_t>* da, bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed) {
    const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x; // 전역 스레드 ID
    const uint64_t warpIdx = tid >> WARP_SHIFT;    // 워프 인덱스 = 담당 정점
    const uint64_t laneIdx = tid & ((1 << WARP_SHIFT) - 1); // 레인 번호

    if (warpIdx < vertex_count && curr_visit[warpIdx] == true) { // 활성 정점만 처리
        ...
        for(uint64_t i = shift_start + laneIdx; i < end; i += WARP_SIZE) { // 워프 stride 접근
            if (i >= start) {
                EdgeT next = da->seq_read(i);      // BaM page_cache 경유 SSD 읽기
                cc_compute(warpIdx, comp, next, next_visit, changed); // 라벨 전파
            }
        }
    }
...
/* kernel_coalesce_ptr_pc: COALESCE + bam_ptr(BaM). ptr[i]로 SSD 직접 접근. 연속 접근 시 페이지 경계 추적으로 효율적. */
__global__ //__launch_bounds__(128,16)
void kernel_coalesce_ptr_pc(array_d_t<uint64_t>* da, bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed) {
    const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x; // 전역 스레드 ID
    const uint64_t warpIdx = tid >> WARP_SHIFT;    // 담당 정점 ID
    const uint64_t laneIdx = tid & ((1 << WARP_SHIFT) - 1); // 레인 번호

    if (warpIdx < vertex_count && curr_visit[warpIdx] == true) { // 활성 정점만 처리
        bam_ptr<uint64_t> ptr(da);                 // BaM 스마트 포인터 생성 (페이지 경계 추적)
        const uint64_t start = vertexList[warpIdx]; // 인접 리스트 시작
        const uint64_t shift_start = start & 0xFFFFFFFFFFFFFFF0; // 128B 정렬
        const uint64_t end = vertexList[warpIdx+1]; // 인접 리스트 끝
        ...
        for(uint64_t i = shift_start + laneIdx; i < end; i += WARP_SIZE) { // 워프 stride 접근
            if (i >= start) {
                EdgeT next = ptr[i];               // bam_ptr로 SSD 직접 읽기 (warp 내 coalesced I/O)
                cc_compute(warpIdx, comp, next, next_visit, changed); // 라벨 전파
            }
        }
    }
...
/* kernel_coalesce_coarse: 코어스닝 최적화. 워프 1개가 coarse개 연속 정점을 처리하여 커널 launch 비용 절감. */
__global__ //__launch_bounds__(128,16)
void kernel_coalesce_coarse(bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed, uint64_t coarse) {
    ...
    for(uint64_t j = 0; j < coarse; j++){          // coarse개 정점을 순차 처리
        uint64_t cwarpIdx = warpIdx*coarse+j;       // 실제 정점 ID = 워프 인덱스 * coarse + j
        if (cwarpIdx < vertex_count && curr_visit[cwarpIdx] == true) { // 유효하고 활성인 정점
...
/* kernel_coalesce_coarse_ptr_pc: 코어스닝 + bam_ptr(BaM). 연속 정점의 간선을 SSD에서 직접 읽는다. */
__global__ //__launch_bounds__(128,16)
void kernel_coalesce_coarse_ptr_pc(array_d_t<uint64_t>* da, bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed, uint64_t coarse) {
    ...
    const uint64_t warpIdx = tid >> WARP_SHIFT;    // 워프 인덱스
    const uint64_t laneIdx = tid & ((1 << WARP_SHIFT) - 1); // 레인 번호
    bam_ptr<uint64_t> ptr(da);                     // BaM 스마트 포인터 (SSD 직접 접근)
...
/* kernel_coalesce_hash: COALESCE + 해시 인터리빙. 워프-정점 매핑을 stride 간격으로 분산하여
 * 인접 워프가 서로 다른 SSD 페이지의 정점에 접근하게 하여 캐시 경합을 줄인다. */
__global__
void kernel_coalesce_hash(bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed, int sm_count) {
    const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x; // 전역 스레드 ID
    const uint64_t oldwarpIdx = tid >> WARP_SHIFT; // 원래 워프 인덱스
    const uint64_t laneIdx = tid & ((1 << WARP_SHIFT) - 1); // 레인 번호
    uint64_t STRIDE = sm_count ;                   // 해시 stride = SM 수

    if (oldwarpIdx < vertex_count){                // 유효 범위 내
       uint64_t warpIdx;                           // 해싱 후 실제 정점 ID
       const uint64_t nep = vertex_count/STRIDE;   // STRIDE당 정점 수
       if(oldwarpIdx <(STRIDE*nep)){
               warpIdx = (oldwarpIdx/nep) + ((oldwarpIdx % nep)*STRIDE); // 인터리빙 해시
       }
       else{
               warpIdx = oldwarpIdx;               // 나머지는 그대로
       }
...
/* preprocess_kernel: 프로파일링용 전처리 커널. 각 정점의 인접 리스트 시작 오프셋이 속하는
 * SSD 페이지 번호를 계산하여 히스토그램(outarray)에 집계한다. 페이지 재사용 패턴 분석에 사용. */
__global__
void preprocess_kernel(uint64_t* vertices, uint64_t vertex_count, uint64_t num_elems_per_page, uint64_t n_pages, unsigned long long int* outarray){
    ...
    if(tid<vertex_count){                          // 유효한 정점만 처리
        unsigned long long int val = vertices[tid] / (num_elems_per_page); // 정점 tid의 이웃 시작이 속하는 페이지 번호
        unsigned long long int update = atomicAdd(&outarray[val], 1); // 해당 페이지의 정점 수 원자적으로 증가
    }
}
 ...
 * =============================================================================
 * 전체 흐름:
 * (1) Settings 파싱 (커널 타입, 메모리 모드, NVMe 설정 등)
 * (2) CSR 그래프 파일(.col, .dst) 읽기 → vertexList, edgeList
 * (3) 메모리 모드별 edgeList 준비 (GPUMEM/UVM/BAFS_DIRECT)
 * (4) BAFS_DIRECT: Controller → page_cache_t → range_t → array_t 생성
 * (5) CC 반복 실행: do-while 루프로 커널 호출, 수렴 시 종료
 * (6) 결과 출력 (컴포넌트 수, 반복 횟수, 실행 시간)
 * ============================================================================= */
int main(int argc, char *argv[]) {
    using namespace std::chrono;

    Settings settings;                             // 벤치마크 설정 구조체
    try
    {
        settings.parseArguments(argc, argv);       // 커맨드라인 인자 파싱
    }
    ...
    cudaDeviceProp properties;                     // GPU 속성 (SM 수 등을 해싱에 사용)
    ...
    std::string vertex_file, edge_file;            // .col(정점 오프셋), .dst(간선) 파일 경로
    std::string filename;                          // 기본 파일명 (.bel 경로)
    ...
    uint32_t *pad;                                 // GPU 메모리 스로틀링용 패딩
    uint32_t iter, comp_total = 0;                 // CC 반복 카운터, 총 컴포넌트 수
    unsigned long long *comp_d, *comp_h;           // 컴포넌트 라벨 배열 (디바이스/호스트)
    uint64_t *vertexList_h, *vertexList_d;         // CSR 정점 오프셋 배열 (호스트/디바이스)
    EdgeT *edgeList_h, *edgeList_d;                // CSR 간선 배열 (호스트/디바이스)
    uint64_t vertex_count, edge_count, vertex_size, edge_size; // 그래프 크기 정보
    uint64_t typeT;                                // 파일 헤더의 타입 필드 (미사용)
    ...
    uint64_t pc_pages;                             // BaM page cache 페이지 수

    try{

        filename = std::string(settings.input);    // 입력 그래프 파일 경로 
        ...
        */
        ...
        /* BAFS_DIRECT 모드: NVMe 컨트롤러 초기화. /dev/libnvm* 디바이스를 열어 GPU에서 직접 NVMe 커맨드를 발행할 수 있게 한다. */
        std::vector<Controller*> ctrls(settings.n_ctrls);
        if(mem == BAFS_DIRECT){
            cuda_err_chk(cudaSetDevice(settings.cudaDevice));
            for (size_t i = 0 ; i < settings.n_ctrls; i++)
                ctrls[i] = new Controller(settings.ssdtype == 0 ? sam_ctrls_paths[i] : intel_ctrls_paths[i], settings.nvmNamespace, settings.cudaDevice, settings.queueDepth, settings.numQueues); // SSD 타입에 따라 경로 선택
            printf("Controllers Created\n");
        }
        printf("Initialization done\n");
        fflush(stdout);

        /* BaM page cache 관련 객체 선언:
         * - page_cache_t: GPU VRAM에 페이지 캐시 할당 (NVMe I/O의 버퍼 역할)
         * - range_t: SSD의 edgeList 데이터 영역을 페이지 캐시에 매핑
         * - array_t: 논리적 배열 인터페이스 제공 (커널에서 d_array_ptr로 SSD 접근) */
        page_cache_t* h_pc;
        ...
        /* CC 실행: 2회 반복 (워밍업 + 본 측정). 매 반복마다 curr_visit/next_visit를 스왑하며
         * 라벨 변경이 없을 때(changed_h == false)까지 반복한다. */
        for(int titr=0; titr<2; titr+=1){          // 2회 반복: 첫 번째는 워밍업, 두 번째가 실제 측정
            iter = 0;                               // CC 반복 카운터 초기화
           ...
           // printf("Hash Stride: %llu Coarse: %llu\n", (settings.stride), settings.coarse);
            ...
            do {
                ...
                cuda_err_chk(cudaMemset(curr_visit_d, 0x00, vertex_count * sizeof(bool))); // 현재 방문 배열 초기화
                bool *temp = curr_visit_d;         // curr_visit과 next_visit를 스왑 (더블 버퍼링)
                curr_visit_d = next_visit_d;       // 다음 반복의 활성 정점 = 이번에 갱신된 정점
                next_visit_d = temp;               // 초기화된 배열이 다음 반복의 next_visit이 됨

                iter++;                             // 반복 카운터 증가
                cuda_err_chk(cudaMemcpy(&changed_h, changed_d, sizeof(bool), cudaMemcpyDeviceToHost)); // 수렴 검사: 변경 여부를 호스트로 복사
