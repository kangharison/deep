...
/*
 * =============================================================================
 * BaM Pattern/Scan 벤치마크 (benchmarks/pattern/scan.cu)
 * =============================================================================
 *
 * [목적]
 * 이 파일은 두 가지 기능을 포함한다:
 ...
 *    GPU 메모리, UVM 등 다양한 I/O 경로에서의 성능을 비교한다.
 *
...
#include <stdint.h>                // 정수 타입 정의
#include <stdio.h>                 // C 표준 I/O
#include <iostream>                // C++ 스트림 I/O
#include <string.h>                // 문자열 처리
#include <getopt.h>                // 커맨드라인 옵션 파싱
#include <unistd.h>                // POSIX API (close, getopt 등)
#include "helper_cuda.h"           // checkCudaErrors 매크로
#include <algorithm>               // STL 알고리즘
#include <vector>                  // STL 벡터 컨테이너
#include <numeric>                 // 수치 연산
#include <fstream>                 // 파일 스트림 (중복 포함)
...
#include <fcntl.h>                 // 파일 제어 (O_RDONLY, O_DIRECT)
#include <unistd.h>                // POSIX API

#include <page_cache.h>            // BaM page_cache_t, range_t, array_t
#include <cuMemcpy.h>              // BaM CUDA 메모리 복사 유틸리티

#define WARP_SHIFT 5       // log2(32) = 5, 워프 내 lane 인덱스 비트 시프트
#define WARP_SIZE 32        // NVIDIA GPU 워프 크기: 32 스레드가 SIMT로 동시 실행

#define CHUNK_SHIFT 3       // log2(8) = 3, chunk 기반 접근 단위
...
//#define COARSE 4
typedef uint64_t EdgeT;     // 에지(인접 vertex ID) 타입: 8바이트 unsigned integer

/*
 * impl_type: CC 커널 구현 방식 + SCAN 전처리 모드 (-t 옵션)
 *
 * [CC 커널 변형]
 * - COALESCE(0):              워프 단위 coalesced edge 탐색 (기본)
 * - COALESCE_COARSE(1):       coarsening: 한 워프가 coarse개 vertex를 순차 처리
 * - COALESCE_CHUNK(2):        chunk 기반 edge 탐색 (legacy)
 ...
 *
 * [Scan 전처리]
 * - SCAN(6):                  prefix sum 전처리 (NVMe 페이지별 vertex 히스토그램 + 누적합)
 */
typedef enum {
    COALESCE = 0,
    COALESCE_COARSE = 1,
...
/*
 * mem_type: edge list 메모리 할당 방식 (-m 옵션)
 * - GPUMEM(0):              cudaMalloc + cudaMemcpy (GPU 전용 VRAM)
 * - UVM_READONLY(1):        UVM + SetReadMostly (GPU 읽기 전용 캐싱)
 * - UVM_DIRECT(2):          UVM + O_DIRECT + SetAccessedBy
 * - UVM_READONLY_NVLINK(3): UVM + NVLink 원격 GPU 프리페치
 ...
 *
 * [알고리즘]
 * - vertices[tid]는 CSR 포맷에서 vertex tid의 edge list 시작 오프셋
 * - vertices[tid] / num_elems_per_page = 해당 vertex의 edge가 시작하는 NVMe 페이지 번호
 * - atomicAdd로 outarray[page_num]을 증가 -> 페이지별 vertex 수 히스토그램
 * - 이 히스토그램에 prefix sum을 적용하면 페이지별 누적 vertex 수를 알 수 있음
 *   -> BaM page cache의 페이지 재사용 패턴을 분석하는 데 활용
 */
__global__
void preprocess_kernel(VertT* vertices, uint64_t vertex_count, uint64_t num_elems_per_page, uint64_t n_pages, VertT* outarray){
    ...
    if(tid<vertex_count){
        ...
        unsigned long long int val = vertices[tid] / (num_elems_per_page);   // vertex의 edge가 속한 페이지 번호
        //if(val>=n_pages)
        //    printf("val: %llu \t update: %llu\n", val, 1);
        unsigned long long int update = atomicAdd((unsigned long long int*)&outarray[val], 1);  // 해당 페이지 카운트 증가
    }
}


/*
 * finalsum: multi-block scan의 마지막 단계. 각 블록의 scan 결과에 이전 블록들의 총합을 더한다.
 *
 * [알고리즘]
 ...
 *
 * [스레드 매핑]
 * - 각 블록이 2*BLOCKSIZE개 원소를 담당 (스레드당 2개: idx와 idx+BLOCKSIZE)
 */
__global__ void finalsum (VertT *input, VertT* output, uint64_t len){
   if(blockIdx.x == 0) return;  // 첫 블록은 보정 불필요

   uint64_t idx = 2*blockIdx.x *BLOCKSIZE + threadIdx.x;  // 이 블록이 담당하는 원소 인덱스

   if(idx < len)
       output[idx] += input[blockIdx.x -1];           // 이전 블록 합을 첫 번째 원소에 더함
   if((idx + BLOCKSIZE) < len)
       output[idx+BLOCKSIZE] += input[blockIdx.x -1]; // 이전 블록 합을 두 번째 원소에 더함
}



/*
 * scan: Blelloch의 work-efficient parallel inclusive prefix sum 커널
 *
 * [GPU 스레드 구조]
...
__global__ void scan(VertT *input, VertT *output, VertT *intermediate,  uint64_t len){
    ...
    __shared__ VertT sharedMem[BLOCKSIZE*2];   // 블록당 2*512 = 1024개 원소를 shared memory에 저장

    uint64_t idx = 2*blockIdx.x * BLOCKSIZE + threadIdx.x;  // 이 스레드가 담당하는 첫 번째 원소의 글로벌 인덱스

    /* 각 스레드가 2개 원소를 global memory에서 shared memory로 로드 */
    if(idx < len)
        sharedMem[threadIdx.x] = input[idx];           // 첫 번째 원소 로드
    else
        sharedMem[threadIdx.x] = 0;                    // 범위 초과 시 0으로 패딩

    if((idx + BLOCKSIZE) < len)
        sharedMem[threadIdx.x + BLOCKSIZE] = input[idx + BLOCKSIZE];  // 두 번째 원소 로드
    else
        sharedMem[threadIdx.x + BLOCKSIZE] = 0;

    __syncthreads();   // 모든 스레드가 데이터 로드를 완료할 때까지 대기

    /* Phase 1: Up-sweep (Reduce) - 트리 구조로 부분합을 상위로 전파 */
    //first pass
    for(uint64_t stride = 1; stride <= BLOCKSIZE; stride*=2){
        uint64_t index = (threadIdx.x+1)*2*stride -1;     // 합산할 위치 계산
        if(index <2 * BLOCKSIZE)
            sharedMem[index] += sharedMem[index-stride];   // stride만큼 떨어진 원소를 더함
        __syncthreads();   // 각 단계 완료 후 동기화
    }

    /* Phase 2: Down-sweep (Distribution) - 부분합을 하위로 전파하여 inclusive scan 완성 */
    //second pass
    for(uint64_t stride = BLOCKSIZE/2; stride>0; stride/=2){
        uint64_t index = (threadIdx.x+1)*stride*2 - 1;
        if(index + stride < (2*BLOCKSIZE))
            sharedMem[index+stride] += sharedMem[index];   // 상위 노드의 합을 하위로 전파
        __syncthreads();
    }

    /* scan 결과를 global memory에 기록 */
    ...
    if(intermediate !=NULL && threadIdx.x == 0)
        intermediate[blockIdx.x] = sharedMem[2*BLOCKSIZE-1];  // 블록의 마지막 원소 = 블록 내 전체 합
}
 ...
 * - comp_src(cid의 comp) != comp_next(next의 comp)이면:
 *   - comp_src < comp_next: next의 comp를 comp_src로 갱신 (더 작은 ID 전파)
 *   - comp_next < comp_src: cid의 comp를 comp_next로 갱신
 * - atomicMin으로 원자적으로 갱신하여 경쟁 조건 방지
...
__device__ void cc_compute(uint64_t cid, unsigned long long *comp, EdgeT next, bool *next_visit, bool *changed){
    ...
    unsigned long long comp_src = comp[cid];       // 현재 vertex의 component ID
    unsigned long long comp_next = comp[next];     // 이웃 vertex의 component ID
    unsigned long long comp_target;
    EdgeT next_target;

    if (comp_next != comp_src) {                   // 두 vertex가 다른 component에 속하면
       if (comp_src < comp_next) {
          next_target = next;                      // 이웃의 comp를 더 작은 값으로 갱신
          comp_target = comp_src;
       }
       else {
          next_target = cid;                       // 현재의 comp를 더 작은 값으로 갱신
          comp_target = comp_next;
       }

       atomicMin(&comp[next_target], comp_target); // 원자적 최솟값 갱신 (경쟁 조건 안전)
       next_visit[next_target] = true;             // 다음 반복에서 재방문 필요
       *changed = true;                            // 수렴하지 않았음을 표시
    }
}


/*
 * kernel_coalesce_coarse: coarsening을 적용한 워프 단위 coalesced CC 커널
 *
 * [GPU 스레드 구조]
 * - 워프 단위: 32개 lane이 하나의 vertex의 edge list를 coalesced하게 탐색
 * - coarsening: 한 워프가 coarse개의 연속 vertex를 순차 처리 (루프)
 * - cwarpIdx = warpIdx * coarse + j (j = 0..coarse-1)
 *
 * [데이터 접근 패턴]
 * - shift_start = start & 0xFFFFFFFFFFFFFFF0: edge list 시작을 16 원소(128바이트) 경계에 정렬
 *   -> GPU 캐시 라인에 맞춰 memory coalescing 효율 극대화
 * - 워프 내 32개 lane이 stride=WARP_SIZE로 edge list를 순회
 * - coarsening으로 워프당 처리량을 coarse배 증가시켜 커널 launch 오버헤드를 줄임
 */
__global__ void kernel_coalesce_coarse(bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed, uint64_t coarse) {
    const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t warpIdx = tid >> (WARP_SHIFT);         // 전역 워프 ID (tid / 32)
    const uint64_t laneIdx = tid & ((1 << WARP_SHIFT) - 1);  // 워프 내 lane 인덱스 (tid % 32)

    for(uint64_t j=0; j< coarse; j++){                    // coarsening 루프: coarse개 vertex 순차 처리
        uint64_t cwarpIdx = warpIdx*coarse+j;             // 실제 처리할 vertex ID
        if ( (cwarpIdx) < vertex_count) {
            if(curr_visit[cwarpIdx] == true) {             // 현재 반복에서 방문 대상인 vertex만 처리
               const uint64_t start = vertexList[cwarpIdx];           // CSR: vertex의 edge 시작 인덱스
               const uint64_t shift_start = start & 0xFFFFFFFFFFFFFFF0;  // 16 원소 정렬
               const uint64_t end = vertexList[cwarpIdx+1];           // CSR: vertex의 edge 끝 인덱스

               for(uint64_t i = shift_start + laneIdx; i < end; i += WARP_SIZE) {  // coalesced 순회
                   if (i >= start) {                       // shift_start ~ start 사이의 패딩은 무시
                       const EdgeT next = edgeList[i];     // 이웃 vertex ID 읽기
                       cc_compute(cwarpIdx, comp, next, next_visit, changed);  // CC 연산
                   }
               }
            }
        }
...
__global__ void kernel_coalesce(bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed) {
    const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t warpIdx = tid >> WARP_SHIFT;            // 전역 워프 ID = 담당 vertex
    const uint64_t laneIdx = tid & ((1 << WARP_SHIFT) - 1);  // 워프 내 lane 번호


        if ((warpIdx) < vertex_count) {
            if(curr_visit[warpIdx] == true) {              // 방문 대상 vertex만 처리
...
void kernel_coalesce_hash(bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed, int sm_count) {
    const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t oldwarpIdx = tid >> WARP_SHIFT;         // 원래 워프 ID
    // const uint64_t warpIdx = tid >> WARP_SHIFT;
    const uint64_t laneIdx = tid & ((1 << WARP_SHIFT) - 1);
    uint64_t STRIDE = sm_count * MAXWARP;                  // 총 동시 워프 수 (SM 수 * SM당 최대 워프)

    uint64_t warpIdx;
    const uint64_t nep = (vertex_count+STRIDE)/STRIDE;     // 재매핑 단위
    warpIdx = (oldwarpIdx/nep) + ((oldwarpIdx % nep)*STRIDE);  // 재매핑된 워프 ID
 ...
 * - shift_start 정렬이 0xFFFFFFFFFFFFFFFC (4 원소 단위)로 더 세밀
 * - STRIDE = sm_count (full warp 대비 절반)
...
void kernel_coalesce_hash_half(bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed, int sm_count) {
    const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t oldwarpIdx = tid >> 4;                  // half-warp ID (16 스레드 단위)
    // const uint64_t warpIdx = tid >> WARP_SHIFT;
    const uint64_t laneIdx = tid & ((1 << 4) - 1);        // half-warp 내 lane (0~15)
    uint64_t STRIDE = sm_count;                            // half-warp 기준 stride
    ...
    if (warpIdx < vertex_count){
            if(curr_visit[warpIdx] == true){
                const uint64_t start = vertexList[warpIdx];
                const uint64_t shift_start = start & 0xFFFFFFFFFFFFFFFC;  // 4 원소 정렬 (32바이트)
 ...
 *
 * [결합 전략]
 * - hash로 워프를 NVMe 채널에 분산 (인접 워프가 멀리 떨어진 페이지 접근)
 * - coarsening으로 워프당 처리량 증가 (커널 launch 오버헤드 감소)
 *
 * [워프 ID 재매핑 + coarsening 공식]
 * STRIDE = stride (커맨드라인 -s 옵션으로 직접 제어)
...
void kernel_coalesce_coarse_hash(bool *curr_visit, bool *next_visit, uint64_t vertex_count, uint64_t *vertexList, EdgeT *edgeList, unsigned long long *comp, bool *changed, int sm_count, uint64_t coarse, uint64_t stride) {
    ...
    uint64_t STRIDE = stride;//sm_count * MAXWARP;         // stride를 직접 제어 (sm_count*MAXWARP 대신)
    ...
    for(uint64_t j=0; j<coarse; j++){                     // coarsening 루프
...
/* throttle_memory: GPU 메모리를 의도적으로 점유하여 가용 메모리를 줄이는 헬퍼 커널.
 * UVM 스래싱(thrashing) 테스트에서 GPU VRAM을 제한하는 데 사용된다.
 * pad[1] = pad[0]은 의미 없는 연산이지만 커널이 메모리를 사용하게 만든다. */
 ...
 *
 * [실행 흐름]
 * 1. 커맨드라인 파싱 (-f 파일명, -t 타입, -m 메모리, -p 페이지크기, -n 캐시엔트리, -s stride, -l coarse)
 * 2. vertex list(.col)와 edge list(.dst) 로드
 * 3. 메모리 모드에 따라 edge list를 GPU/UVM에 배치
 * 4. CC 초기화: 각 vertex의 component = 자기 자신, 모든 vertex를 방문 대상으로 설정
 * 5. CC 반복: changed == false 될 때까지 커널 실행 + 방문 버퍼 스왑
 *    - SCAN 모드: CC 대신 페이지 히스토그램 + prefix sum을 1회 수행
 * 6. component 수 카운트 및 실행 시간 출력
 */
int main(int argc, char *argv[]) {
    ...
    bool changed_h, *changed_d;            // 호스트/디바이스 변경 플래그
    bool *curr_visit_d, *next_visit_d, *comp_check;  // 방문 마스크, component 확인 배열
    int c, arg_num = 0;
    impl_type type;                        // 커널 구현 타입
    mem_type mem;                          // 메모리 할당 모드
    uint32_t *pad;                         // throttle_memory용 패딩 메모리
    uint32_t iter, comp_total = 0;         // CC 반복 횟수, 총 component 수
    unsigned long long *comp_d, *comp_h;   // 디바이스/호스트 component ID 배열
    uint64_t *vertexList_h, *vertexList_d; // 호스트/디바이스 vertex list (CSR row pointer)
    EdgeT *edgeList_h, *edgeList_d;        // 호스트/디바이스 edge list (CSR column index)
    uint64_t vertex_count, edge_count, vertex_size, edge_size;
    uint64_t typeT;                        // 파일 헤더의 타입 필드
    uint64_t numblocks, numthreads;        // CUDA 그리드/블록 크기
    size_t freebyte, totalbyte;            // GPU 메모리 사용량 확인용
    EdgeT *edgeList_dtmp;                  // Dragon MAP용 임시 포인터

    float milliseconds;                    // 커널 실행 시간

    Settings settings;
    uint64_t pc_page_size = 4096;          // BaM page cache 페이지 크기 (바이트). -p 옵션으로 변경 가능
    uint64_t pc_pages = 2*1024*1024;       // 페이지 캐시 엔트리 수. 2M * 4096B = 8GB. -n 옵션으로 변경 가능

    uint64_t stride;   // hash 워프 재매핑 stride 팩터
    uint64_t coarse;   // coarsening 팩터 (워프당 처리할 vertex 수)
    ...
    file.read((char*)(&vertex_count), 8);   // 파일 헤더: vertex 수 + 1 (row pointer 수)
    file.read((char*)(&typeT), 8);          // 파일 헤더: 타입 정보
    ...
    file.read((char*)vertexList_h, vertex_size);   // vertex list(row pointer) 읽기
    file.close();
    ...
    file.read((char*)(&edge_count), 8);    // edge 수
     ...
     * - UVM_READONLY: UVM에 직접 읽고 SetReadMostly 힌트 (GPU 읽기 캐싱)
     * - UVM_DIRECT: O_DIRECT로 UVM에 직접 fread + SetAccessedBy + vertex list 오프셋 보정(+2)
    ...
    switch (mem) {
        ...
        case UVM_READONLY_NVLINK:
            /* NVLink 경유 UVM: device 0에서 할당 -> device 2로 프리페치 -> device 0에서 접근 */
        ...
        case UVM_DIRECT_NVLINK:
            /* NVLink Direct: device 2에서 할당 -> 프리페치 -> device 0에서 AccessedBy */
            checkCudaErrors(cudaSetDevice(2));
            checkCudaErrors(cudaMallocManaged((void**)&edgeList_d, edge_size));
            file.read((char*)edgeList_d, edge_size);
            checkCudaErrors(cudaMemPrefetchAsync(edgeList_d, edge_size, 2, 0));
    ...
    checkCudaErrors(cudaMalloc((void**)&curr_visit_d, vertex_count * sizeof(bool)));   // 현재 방문 마스크
    checkCudaErrors(cudaMalloc((void**)&next_visit_d, vertex_count * sizeof(bool)));   // 다음 방문 마스크
    checkCudaErrors(cudaMalloc((void**)&comp_d, vertex_count * sizeof(unsigned long long)));  // component ID 배열
    checkCudaErrors(cudaMalloc((void**)&changed_d, sizeof(bool)));   // 변경 플래그

    printf("Allocation finished\n");
    fflush(stdout);

    /* CC 초기화: 각 vertex의 component ID를 자기 자신의 ID로 설정 */
    // Initialize values
    ...
    /* 첫 반복: 모든 vertex를 방문 대상으로 설정 (curr_visit = 0x01 = true) */
    ...
    numthreads = BLOCKSIZE;   // CC 커널은 BLOCKSIZE(512) 스레드/블록 고정

    /*
     * 커널 타입별 그리드 크기 계산:
     * - SCAN: 블록당 2*numthreads개 원소 처리 -> numblocks = ceil(vertex_count / (2*numthreads))
     * - COALESCE_COARSE/COARSE_HASH/HASH_HALF: coarsening 적용 -> 워프 수 감소
     * - COALESCE/COALESCE_PC/COALESCE_HASH: 워프당 1 vertex -> numblocks = ceil(vertex_count * 32 / numthreads)
     * - COALESCE_CHUNK/CHUNK_PC: chunk 단위 -> 워프당 CHUNK_SIZE개 vertex
     */
    switch (type) {
        case SCAN:
    ...
    /* 2D 그리드: x=BLOCK_NUM, y=ceil(numblocks/BLOCK_NUM) */
    ...
    uint64_t n_pages = ceil(((float)edge_size)/pc_page_size);  // edge list가 차지하는 NVMe 페이지 수
    if((type == COALESCE_PC) ||(type == COALESCE_CHUNK_PC)){
...
//     range_t<uint64_t>* d_range = (range_t<uint64_t>*) h_range.d_range_ptr;
    ...
    do {
        ...
        switch (type) {
            ...
            {
                /*
                 * SCAN 모드: CC 대신 NVMe 페이지 재사용 분석을 수행
                 * 1. preprocess_kernel: vertex별 edge 시작 오프셋이 속한 페이지의 히스토그램 생성
                 * 2. scan: 히스토그램에 Blelloch prefix sum 적용 (multi-block)
                 * 3. finalsum: 블록 간 합산으로 전역 prefix sum 완성
                 * 4. 결과 출력: 처음 64개 페이지의 vertex 분포와 누적 합
                 */
                uint64_t k = pc_page_size/sizeof(VertT);   // 한 페이지의 원소 수
                VertT* outarray_d;
                ...
                preprocess_kernel<<<blockDim, numthreads>>>(vertexList_d, vertex_count, k,n_pages, outarray_d);  // 히스토그램 생성
                cuda_err_chk(cudaMemcpy(outarray_h, outarray_d, n_pages*sizeof(unsigned long long int), cudaMemcpyDeviceToHost));
                scan<<<blockDim, numthreads>>>(outarray_d, (&result_d[1]), intermediate_d, n_pages);  // 블록 내 scan
                scan<<<dim3(1,1,1), numthreads>>>(intermediate_d, dev2out_d, NULL, BLOCKSIZE*2);      // intermediate scan
                finalsum<<<blockDim, numthreads>>>(dev2out_d, (&result_d[1]), n_pages);               // 블록 간 합산
                checkCudaErrors(cudaMemcpy(result_h, (result_d), (n_pages+1)*sizeof(VertT), cudaMemcpyDeviceToHost));

                /* 결과 출력: vertex list, 페이지별 히스토그램, prefix sum 결과의 처음 64개 */
                printf("\n******\n");
                fflush(stdout);
                printf("Vertex list::");
        ...
        /* 변경 플래그 확인: true이면 아직 수렴하지 않았으므로 다음 반복 실행 */
    ...
    /* 결과: component ID를 호스트로 복사하고 고유 component 수를 카운트 */
