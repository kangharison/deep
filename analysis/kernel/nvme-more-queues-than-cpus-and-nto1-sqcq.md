...
**최종 결론 요약** (양방향 실측: §11.1.1): 큐를 **(클래스 × CPU) 격자**로 배치한다. 클래스 = blk-mq의 map(hctx type)이며, 지금 3개(default/read/poll)로 고정된 arity를 K개로 일반화한다. **클래스당 큐 수 = ncpus**를 불변식으로 유지하면 (CPU, 클래스)가 큐를 유일하게 결정하므로 **라운드로빈이 필요 없고 배칭이 100% 보존**된다. 실측(§11.1)에서 CPU 2개짜리 게스트가 hctx 6개(=3×2)로 정상 동작함을 확인했으므로, 이는 신규 메커니즘이 아니라 **shipping 메커니즘의 arity 확장**이다.
...
#### 11.1.1 양방향 실측 매트릭스 (CPU ≷ 디바이스 SQ)

CPU 수와 디바이스가 광고하는 queue pair 수의 **양쪽 방향**을 모두 확인했다. (동일 환경: qemu-8.2.2 tcg, 게스트 6.1.4)

| # | 구성 | 생성된 I/O 큐 | hctx → cpu_list | nvme IRQ 라인 |
|---|------|--------------|-----------------|---------------|
| **A1** | `smp=8`, `max_ioqpairs=2` | `2/0/0` (2개) | `0=[0,1,2,3]` `1=[4,5,6,7]` | 3 |
| **A2** | `smp=8`, `max_ioqpairs=1` | `1/0/0` (1개) | `0=[0..7]` | 2 |
| **A3** | `smp=8`, `max_ioqpairs=2`, `w2 p2` | `1/0/1` (2개) | `0=[0..7]` `1=[0..7]` | 2 |
| **B1** | `smp=4`, `max_ioqpairs=64`, `w4 p4` | `4/4/4` (**12개**) | `0=[0] 1=[1] 2=[2] 3=[3]` / `4=[0] 5=[1] 6=[2] 7=[3]` / `8=[0] 9=[1] 10=[2] 11=[3]` | 9 |
| **B2** | `smp=4`, `max_ioqpairs=5` | `4/0/0` (4개) | `0=[0] 1=[1] 2=[2] 3=[3]` | 5 |

**A 방향 (CPU > 디바이스 SQ) — blk-mq가 네이티브로 처리한다**

- A1: 8 CPU에 큐 2개 → `group_cpus_evenly(2)`가 4개씩 균등 분할. 정상.
- A2: 극단(큐 1개) → 전 CPU가 hctx 하나 공유. 정상.
- A3: **큐가 희소할 때 클래스 분할이 우아하게 축소된다.** 요청은 default+write+poll이었으나 디바이스가 2개만 주자 `1/0/1`로 줄었고, **각 클래스의 유일한 큐가 전 CPU를 담당**(`0=[0..7]`, `1=[0..7]`)한다. 즉 **"클래스당 큐 수 ≤ ncpus" 불변식이 1까지 축소돼도 성립**한다.
- A3의 IRQ가 2인 것도 정합적이다 — admin 1 + default 1, poll 큐는 IRQ 없음.

> A3의 배분 정책은 주목할 만하다. vanilla는 큐가 부족할 때 **poll을 먼저 챙기고 default를 1로 줄인다**(`nvme_setup_irqs`의 `poll_queues = min(nr_poll_queues, nr_io_queues - 1)`, pci.c:2908 → `nvme_calc_irq_sets`가 남은 1개를 default에 배정). 클래스 모델에서는 **희소 상황의 클래스 우선순위 정책**을 명시적으로 정해야 한다 (§11.6의 URGENT를 먼저 살리는 식).

**B 방향 (CPU < 디바이스 SQ) — 여기가 목표 영역**

- **B1이 Class-Major Queue Model의 완전한 실증이다.** 4 CPU에 **12큐 = 3 클래스 × 4 CPU**, 각 hctx가 **CPU 정확히 1개**를 전담하고 각 CPU는 3개 hctx에 소속된다. §11.2의 격자가 그대로 나타났다. IRQ 9개 = admin 1 + default 4 + read 4 → **CPU 4개에 I/O 벡터 8개**로, 벡터도 CPU 수의 2배까지 정상 분배된다.
- B2는 **G1 문제의 축소판**이다. 디바이스가 5개를 주는데 CPU 캡 때문에 4개만 쓰고 **1개를 버린다**.

**완전성 논증**: A3(클래스당 1큐)과 B1(클래스당 ncpus큐)이 함께 성립하므로, 임의의 디바이스 큐 수 Q는

```
K = ceil(Q / ncpus) 개 클래스,  마지막 클래스는 (Q mod ncpus) 개 큐
```

로 **남김 없이 소비할 수 있다.** B2의 Q=5, ncpus=4라면 K=2 (4개 + 1개)로 5개를 전부 쓴다. 즉 Class-Major 모델은 **격자에 딱 떨어지지 않는 큐 수도 수용**한다.
...
디바이스 전체 큐를 쓰려면  K = ceil(device_queues / ncpus)
                          (마지막 클래스는 device_queues mod ncpus 개 — §11.1.1 A3가 부분 폭 클래스의 동작을 실증)
```
...
┌────────────────────────────────────────────────────────────────────────────────┐
│ 애플리케이션 / 파일시스템                                                       │
│   read()   write()   fsync()   O_DIRECT   readahead                            │
└─────────────────────────────────────┬──────────────────────────────────────────┘
                                      │  bio
                                      │  blk_opf_t = REQ_OP_* | REQ_META | REQ_FUA
                                      │              | REQ_SYNC | REQ_POLLED | ...
┌─────────────────────────────────────▼──────────────────────────────────────────┐
│ blk-mq 제출 경로            __blk_mq_alloc_requests()   block/blk-mq.c:553      │
│                                                                                │
│   ┌──────────────────────────────┐      ┌──────────────────────────────┐       │
│   │ ① 클래스 결정  (무엇을)       │      │ ② CPU 결정  (어디서)          │       │
│   │                              │      │                              │       │
│   │ blk_mq_get_hctx_type(opf)    │      │ blk_mq_get_ctx(q)            │       │
│   │ block/blk-mq.h    ★변경       │      │ = 현재 실행 중인 CPU          │       │
│   └──────────────┬───────────────┘      └───────────────┬──────────────┘       │
│                  │ class c                              │ cpu p                │
│                  └───────────────┬──────────────────────┘                      │
│                                  ▼                                             │
│                    hctx = ctx->hctxs[c]                                        │
│                                                                                │
│         ★ (c, p) 가 큐를 유일하게 결정한다 → 라운드로빈 없음                     │
│           같은 성격의 IO는 항상 같은 큐 → 배칭·도어벨 amortization 보존           │
└─────────────────────────────────────┬──────────────────────────────────────────┘
                                      ▼
┌────────────────────────────────────────────────────────────────────────────────┐
│ (클래스 × CPU) 하드웨어 큐 격자     ← §13.2                                      │
│   hctx 마다 독립: tags / depth / 스케줄러 / sysfs 엔트리 / IRQ 벡터              │
└─────────────────────────────────────┬──────────────────────────────────────────┘
                                      │ nvme_queue_rq()
                                      ▼
┌────────────────────────────────────────────────────────────────────────────────┐
│ NVMe 드라이버        hctx_idx ──(+1)──> qid ──> dev->queues[qid]                │
│                      drivers/nvme/host/pci.c   ★변경                            │
└─────────────────────────────────────┬──────────────────────────────────────────┘
                                      ▼
┌────────────────────────────────────────────────────────────────────────────────┐
│ NVMe 컨트롤러        SQ/CQ 쌍 + MSI-X 벡터                                      │
└────────────────────────────────────────────────────────────────────────────────┘
```

### 13.2 큐 격자 — 실측 T1 (`-smp 4`, `nvme.io_classes=6`)

```
                    cpu0        cpu1        cpu2        cpu3
                 ┌───────────┬───────────┬───────────┬───────────┐
   class 0       │  hctx3    │  hctx0    │  hctx2    │  hctx1    │  qid  1.. 4
   default       │           │           │           │           │  vec  1.. 4
                 ├───────────┼───────────┼───────────┼───────────┤
   class 1       │  hctx7    │  hctx4    │  hctx6    │  hctx5    │  qid  5.. 8
   read          │           │           │           │           │  vec  5.. 8
                 ├───────────┼───────────┼───────────┼───────────┤
   class 2       │  hctx11   │  hctx8    │  hctx10   │  hctx9    │  qid  9..12
   urgent        │           │           │           │           │  vec  9..12
                 ├───────────┼───────────┼───────────┼───────────┤
   class 3       │  hctx15   │  hctx12   │  hctx14   │  hctx13   │  qid 13..16
   fua           │           │           │           │           │  vec 13..16
                 ├───────────┼───────────┼───────────┼───────────┤
   class 4       │  hctx19   │  hctx16   │  hctx18   │  hctx17   │  qid 17..20
   sync          │           │           │           │           │  vec 17..20
                 ├───────────┼───────────┼───────────┼───────────┤
   class 5       │  hctx23   │  hctx20   │  hctx22   │  hctx21   │  qid 21..24
   bulk          │           │           │           │           │  vec 21..24
                 └───────────┴───────────┴───────────┴───────────┘
                       │
                       └── 한 칸 = hctx 1개 = NVMe SQ/CQ 1쌍 = MSI-X 벡터 1개
                                  = CPU 정확히 1개가 전담 (cpu_list 원소 1개)

   총 hctx = 6 × 4 = 24          CPU 수(4)의 6배
   총 벡터 = 24 (+ admin 1)      IRQ 라인 25개로 실측 확인
```

**불변식**: 가로 폭(클래스당 큐 수) ≤ ncpus. 넘으면 `blk_mq_map_queues()`의 `queue % nr_masks`가 되감기며 앞쪽 큐가 어떤 CPU에도 매핑되지 않는다(§1.2 B2).

### 13.3 vanilla vs 패치 — 같은 하드웨어에서의 차이

```
  [ vanilla v7.2 ]  -smp 4,  디바이스 max_ioqpairs=64

                    cpu0    cpu1    cpu2    cpu3
                 ┌───────┬───────┬───────┬───────┐
   default       │ hctx0 │ hctx1 │ hctx2 │ hctx3 │   ← 큐 4개만 생성
                 └───────┴───────┴───────┴───────┘
                 ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
                 ░  디바이스가 제공한 나머지 60개 큐 = 미사용 낭비  ░
                 ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░

  [ patched, io_classes=6 ]

                    cpu0    cpu1    cpu2    cpu3
                 ┌───────┬───────┬───────┬───────┐
   default       │ hctx3 │ hctx0 │ hctx2 │ hctx1 │  ┐
                 ├───────┼───────┼───────┼───────┤  │
   read          │ hctx7 │ hctx4 │ hctx6 │ hctx5 │  │
                 ├───────┼───────┼───────┼───────┤  │  클래스를 K로 늘린 만큼
   urgent        │ hctx11│ hctx8 │hctx10 │ hctx9 │  ├─ 큐가 늘어난다
                 ├───────┼───────┼───────┼───────┤  │  (K × ncpus)
   fua           │ hctx15│ hctx12│hctx14 │hctx13 │  │
                 ├───────┼───────┼───────┼───────┤  │
   sync          │ hctx19│ hctx16│hctx18 │hctx17 │  │
                 ├───────┼───────┼───────┼───────┤  │
   bulk          │ hctx23│ hctx20│hctx22 │hctx21 │  ┘
                 └───────┴───────┴───────┴───────┘
```

세로(클래스)로 늘리지 가로(CPU당 큐 수)로 늘리지 않는 것이 핵심이다. 가로로 늘리면 한 CPU가 여러 큐 중 하나를 골라야 하고 → 라운드로빈 → 배칭 손실.

### 13.4 NVMe 큐 ID 공간 배치 — poll 은 항상 꼬리

```
   nvme_class_order[]  (drivers/nvme/host/pci.c)
   ────────────────────────────────────────────────────────────────────────
     DEFAULT → READ → URGENT → FUA → SYNC → BULK → SPARE → POLL
        (0)     (1)     (3)     (4)    (5)    (6)     (7)    (2)
        └──────────── 인터럽트 구동 클래스 ────────────┘      └ IRQ 없음

   큐 ID 공간
   ┌─────┬──────────────────────────────────────────┬───────────────────┐
   │ qid │  인터럽트 구동 큐 (rw_queues)             │  poll 큐          │
   │  0  │  qid 1 .. rw_queues                      │  rw_queues+1 ..   │
   │admin│                                          │                   │
   └─────┴──────────────────────────────────────────┴───────────────────┘
            │                                          │
            ...
            │                                              "rw_queues 초과 = polled"
            │                                              → poll 이 꼬리여야 성립
            └── MSI-X 벡터 1..rw_queues 와 1:1
                affinity set 도 이 순서로 연속 배치

   ★ HCTX_TYPE_POLL 의 enum 값은 2로 고정 (blk_mq_alloc_queue() 의
     nr_maps > HCTX_TYPE_POLL 판정, virtio_blk/rnbd 호환).
     "enum 값"과 "큐 ID 배치 순서"를 분리한 것이 nvme_class_order[] 의 역할.
```
...
┌──────────────────────────────────────────────────────────────────────────────┐
│ 자료구조                                                                      │
│   include/linux/blk-mq.h        enum hctx_type + 5클래스, HCTX_MAX_TYPES 3→8  │
│        └─ struct blk_mq_ctx 의 rq_lists[] / index_hw[] / hctxs[] 폭 결정      │
│           = "CPU 하나가 속할 수 있는 hctx 수" = 격자의 세로 길이                │
└──────────────────────────────────────────────────────────────────────────────┘
┌──────────────────────────────────────────────────────────────────────────────┐
│ 라우팅                                                                        │
│   block/blk-mq.h                blk_mq_get_hctx_type(opf)                    │
│        └─ 격자에서 어느 행(클래스)으로 갈지 결정. 분류만, 분산 금지             │
└──────────────────────────────────────────────────────────────────────────────┘
┌──────────────────────────────────────────────────────────────────────────────┐
│ 인터럽트                                                                      │
│   include/linux/interrupt.h     IRQ_AFFINITY_MAX_SETS 4→8                    │
│   drivers/nvme/host/pci.c       nvme_calc_irq_sets()                         │
│        └─ set = 클래스. 위치 기반·연속·비영 필수                               │
│           (크기 0 → group_cpus_evenly() NULL → 벡터 할당 전체 실패)            │
└──────────────────────────────────────────────────────────────────────────────┘
┌──────────────────────────────────────────────────────────────────────────────┐
│ 큐 개수 · 매핑                                                                │
│   drivers/nvme/host/pci.c       io_classes 모듈 파라미터                      │
│                                 nvme_max_io_queues()   ← CPU 상한 해제 지점    │
│                                 nvme_class_order[]                           │
│                                 nvme_pci_map_queues()  ← 격자를 mq_map 에 기록 │
│                                 nvme_pci_nr_maps()                           │
│                                 nvme_create_io_queues() ← rw_queues 재정의     │
│                                 nvme_setup_io_queues()  ← 리셋 시 클래스 클램프 │
└──────────────────────────────────────────────────────────────────────────────┘
┌──────────────────────────────────────────────────────────────────────────────┐
│ 관측                                                                          │
│   block/blk-mq-debugfs.c        hctx_types[] 이름 5개                        │
│        └─ /sys/kernel/debug/block/nvme0n1/hctxN/type 로 클래스 확인            │
│   block/blk-mq.c                BLK_FEAT_POLL 게이트 정확화                   │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
```
   dd if=/dev/nvme0n1 iflag=direct                REQ_OP_READ
        └─────────────────────────────────────────────┴──> class read  ──> q3  +200 ✅

   dd of=/dev/nvme0n1 oflag=direct                REQ_OP_WRITE|REQ_SYNC|REQ_IDLE
        │                                             │
        │  [수정 전]  REQ_IDLE 을 BULK 조건에 넣음 ────┴──> class bulk  ──> q12 +200 ❌
        │                                                   ★ 전 direct write 강등
        │  [수정 후]  REQ_IDLE 제거 (idling 힌트일 뿐) ─────> class sync  ──> q9  +200 ✅
        │
   dd of=/dev/nvme0n1 conv=fsync
        ├── flush 명령      REQ_PREFLUSH ─────────────────> class fua   ──> q7  +1   ✅
        └── writeback 쓰기  REQ_SYNC ─────────────────────> class sync  ──> q9  +25  ✅
```
