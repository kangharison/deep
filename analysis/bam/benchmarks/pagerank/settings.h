/*
 * [BaM PageRank 벤치마크 - 설정 헤더]
 *
 * PageRank(페이지랭크) 그래프 알고리즘 벤치마크의 설정 파일.
 * PageRank는 각 정점의 중요도를 반복적으로 계산하는 알고리즘이다.
 *   - 각 정점은 자신의 delta 값을 이웃 정점들에게 균등 분배한다.
 *   - residual이 tolerance를 초과하는 정점만 활성화하여 반복한다.
 *   - 수렴 조건: 더 이상 tolerance를 초과하는 residual이 없거나 최대 100회 반복.
 *
 * 그래프는 CSR(Compressed Sparse Row) 포맷으로 저장됨:
 *   - .col 파일: vertexList (각 정점의 인접 리스트 시작 오프셋)
 *   - .dst 파일: edgeList (간선의 목적지 정점 ID)
 *
 * Settings 구조체는 cc/settings.h와 유사하나,
 * stride/coarse/largebin/binelems/ssdtype 필드가 없다.
 *
 * 메모리 할당 방식:
 *   GPUMEM(0)       - 간선 리스트를 GPU 메모리에 전부 적재
 *   UVM_READONLY(1) - CUDA Unified Memory + ReadMostly 힌트
 *   UVM_DIRECT(2)   - CUDA Unified Memory + AccessedBy 힌트 (CPU에서 직접 fread)
 *   BAFS_DIRECT(6)  - BaM 페이지 캐시를 통해 GPU가 SSD에서 직접 I/O
 */
...
struct Settings
{
    ...
    bool            stats;           // 통계 출력 여부
    const char*     input;           // 입력 그래프 파일 경로 (.bel)
    const char*     output;          // 출력 파일 경로
    size_t          ofileoffset;     // NVMe SSD 내 파일 오프셋 (바이트)
    size_t          type;            // 커널 구현 타입 (BASELINE=0 ~ COALESCE_HASH_PC=10)
...
/* setBDF(): GPU PCIe BDF 정보 자동 감지. BaM P2P DMA 설정에 필요. */
...
/* parseArguments(): PageRank 커맨드라인 옵션 파싱.
 * CC settings.h와 유사하나 stride/coarse/largebin/binelems/ssdtype 옵션이 없다. */
