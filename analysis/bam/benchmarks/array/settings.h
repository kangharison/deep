/*
 * settings.h - BaM Array Access Benchmark 설정 파일
 *
 * 주요 설정:
 *   -g: CUDA GPU 디바이스 번호
 *   -k: NVMe 컨트롤러 수
 *   -e: 배열 원소(uint64_t) 수 (기본: 2147483648 = 2G개 = 16GB)
 *   -r: 랜덤 접근 여부 (기본: true)
 *   -S: SSD 타입 (0=Samsung, 1=Intel)
 *   -p/-P: 페이지 캐시 페이지 수/크기
 *   -t/-b: CUDA 스레드 수/블록 크기
 *   -d/-q: NVMe 큐 깊이/큐 수
 */
...
struct Settings
{
    ...
    uint64_t        cudaDeviceId;     /* DIS 클러스터용 FDID */
    ...
    size_t          numPages;         /* 페이지 캐시 페이지 수 (기본: 1024) */
    ...
    size_t pageSize;                  /* 페이지 크기 (기본: 4096) */
    uint64_t numElems;                /* 배열 원소 수 (기본: 2G = 16GB) */
    bool random;                      /* 랜덤 접근 여부 (기본: true) */
    uint64_t ssdtype;                 /* SSD 타입: 0=Samsung, 1=Intel */
    Settings();
