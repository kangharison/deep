/*
 * settings.h - BaM Cache Access Benchmark 설정 파일
 *
 * 주요 설정:
 *   -G: 접근 단위 (0=WARP, 1=BLK) - 현재 WARP만 사용
 *   -T: 접근 타입 (0=ORIG 개별접근, 1=COAL bam_ptr coalesced 접근)
 *   -r: 랜덤(1) vs 순차(0) 접근
 *   -S: stride 해싱 팩터 (캐시 경합 분산)
 *   -e: 배열 원소 수 (기본: 2G)
 *   -n: warp당 접근 페이지 수
 */
...
#define WARP 0    /* warp(32 스레드) 단위 접근 */
#define BLK 1     /* 블록 단위 접근 (미사용) */

#define ORIG 0    /* 원본 접근: array_t::operator[]로 매번 개별 acquire/release */
#define COAL 1    /* Coalesced 접근: bam_ptr로 warp 내 같은 페이지 접근을 자동 병합 */

struct Settings
{
    uint32_t        cudaDevice;       /* CUDA GPU 번호 */
    uint64_t        cudaDeviceId;     /* DIS FDID */
    const char*     blockDevicePath;  /* (미사용) */
    const char*     controllerPath;   /* (미사용) */
    uint64_t        controllerId;     /* DIS 컨트롤러 FDID */
    uint32_t        adapter;          /* DIS 어댑터 */
    uint32_t        segmentId;        /* DIS 세그먼트 */
    uint32_t        nvmNamespace;     /* NVMe NS ID (기본:1) */
    bool            doubleBuffered;   /* (미사용) */
    size_t          numReqs;          /* warp당 접근 페이지 수 (기본:1) */
    size_t          numPages;         /* 캐시 페이지 수 (기본:1024) */
    size_t          startBlock;       /* (미사용) */
    bool            stats;            /* 통계 출력 */
    const char*     output;           /* (미사용) */
    size_t          numThreads;       /* 총 CUDA 스레드 수 */
    uint32_t        domain;           /* GPU PCIe 도메인 */
    uint32_t        bus;              /* GPU PCIe 버스 */
    uint32_t        devfn;            /* GPU PCIe 디바이스 */
    uint32_t        gran;             /* 접근 단위: WARP(0) 또는 BLK(1) */
    uint32_t       type;              /* 접근 타입: ORIG(0) 또는 COAL(1) */
    uint32_t n_ctrls;                 /* NVMe 컨트롤러 수 */
    size_t blkSize;                   /* CUDA 블록 크기 */
    size_t queueDepth;                /* NVMe 큐 깊이 */
    size_t numQueues;                 /* 컨트롤러당 큐 수 */
    size_t pageSize;                  /* 페이지 크기(바이트) */
    uint64_t numElems;                /* 배열 원소 수 (기본:2G) */
    uint32_t random;                  /* 랜덤(1)/순차(0) */
    uint64_t stride;                  /* warp해싱 스트라이드 */
    Settings();
