/*
 * =============================================================================
 * BaM SSSP (정수 가중치) 벤치마크 - 설정(Settings) 헤더
 * =============================================================================
 *
 * SSSP(Single Source Shortest Path) 벤치마크의 커맨드라인 옵션 파싱 및 저장 구조체.
 * BFS settings.h와 동일한 프레임워크(Option/Range)를 사용하되, SSSP 고유 옵션이 추가됨.
 *
 * [BFS settings.h 대비 차이점]
 * - wfileoffset (-w): SSD 내 가중치(weight) 파일의 오프셋 위치 (기본 34359738368 = 32GB 지점)
 *   SSSP는 edgeList(간선)과 weightList(가중치) 두 개의 SSD 영역이 필요하다.
 *   ofileoffset은 간선 파일 오프셋, wfileoffset은 가중치 파일 오프셋이다.
 * - stride/coarse/tsize/ssdtype: SSSP에는 포함되지 않음 (BFS 전용 최적화 옵션)
 *
 * [주요 설정 항목]
 * - cudaDevice (-g): GPU 디바이스 번호
 * - input (-f): 그래프 파일 경로 (.bel 형식 -> .col/.dst/.val 파일)
 * - type (-v): 커널 구현 타입 (COALESCE=1, COALESCE_CHUNK=2, COALESCE_PC=4, COALESCE_CHUNK_PC=5)
 * - memalloc (-m): 메모리 모드 (GPUMEM=0, UVM_READONLY=1, UVM_DIRECT=2, BAFS_DIRECT=6)
 * - pageSize (-p): BaM page cache 페이지 크기 (기본 4096)
 * - maxPageCacheSize (-M): BaM page cache 최대 크기 (기본 8GB)
 * - n_ctrls (-k): NVMe 컨트롤러 수
 * - queueDepth (-d): NVMe 큐 depth
 * - numQueues (-q): 컨트롤러당 큐 수
 * - ofileoffset (-l): SSD 내 간선 데이터 오프셋
 * - wfileoffset (-w): SSD 내 가중치 데이터 오프셋
 * =============================================================================
 */
...
struct Settings
{
    ...
    uint32_t        adapter;         // DIS 어댑터 번호 (미사용)
    uint32_t        segmentId;       // DIS 세그먼트 ID (미사용)
    ...
    size_t          ofileoffset;     // SSD 내 간선 데이터 오프셋 (-l)
    size_t          wfileoffset;     // SSD 내 가중치 데이터 오프셋 (-w, 기본 32GB 지점)
    size_t          type;            // 커널 구현 타입 (-v, impl_type 참조)
    size_t          memalloc;        // 메모리 할당 방식 (-m, mem_type 참조)
    ...
    size_t pageSize;                 // page cache 페이지 크기 (-p, 바이트)
    uint64_t numElems;               // backing array 원소 수 (미사용)
    size_t repeat;                   // 반복 횟수 (-r)
    size_t src;                      // 시작 정점 (-s)
    uint64_t maxPageCacheSize;       // page cache 최대 크기 (-M, 바이트)
    Settings();
...
/* OptionIface / Option<T> / Range: 커맨드라인 옵션 파싱 프레임워크 (BFS와 동일) */
struct OptionIface;
using std::string;
using std::vector;
...
/* setBDF: GPU PCIe BDF 정보를 cudaDeviceProp에서 읽어 Settings에 저장 */
...
/* parseArguments: SSSP 고유 옵션인 -w (wfileoffset)이 추가된 것 외에는 BFS와 동일 */
...
/* Settings 기본 생성자: wfileoffset 기본값 34359738368 (32GB) = SSD에서 가중치 파일이 저장되는 위치 */
Settings::Settings()
{
    cudaDevice = 0;
    cudaDeviceId = 0;
