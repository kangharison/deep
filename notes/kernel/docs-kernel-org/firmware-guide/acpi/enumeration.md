...
> 출처(원문): https://docs.kernel.org/firmware-guide/acpi/enumeration.html
...
>   are represented as [`struct spi_device`](../../driver-api/spi.html#c.spi_device "spi_device") or [`struct i2c_client`](../../driver-api/i2c.html#c.i2c_client "i2c_client"). Note
...
This means that when ACPI\_HANDLE(dev) returns non-NULL the device was
...
There are also devm\_\* versions of these functions which release the
descriptors once the device is released.

See [\_DSD Device Properties Related to GPIO](gpio-properties.html) for more information
...
Refer to [\_DSD Device Properties Usage Rules](DSD-properties-rules.html) for more
information.
...
16 GPIOs and we want to add the property `gpio-line-names` [[1]](#id4) to these pins.
...
in particular the DSDT (see also [[2]](#id5)):
...
and the \_ADR method [[3]](#id6) returns exactly the device/function couple that
...
[\_DSD Device Properties Related to GPIO](gpio-properties.html)


[[2](#id2)]

[Upgrading ACPI tables via initrd](../../admin-guide/acpi/initrd_table_override.html)
