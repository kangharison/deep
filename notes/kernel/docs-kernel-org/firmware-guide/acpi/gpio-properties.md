...
> 출처(원문): https://docs.kernel.org/firmware-guide/acpi/gpio-properties.html
...
active-low or active-high, the “active\_low” argument can be used here.
Setting it to 1 marks the GPIO as active-low.

Note, active\_low in \_DSD does not make sense for `GpioInt()` resource and
...
crs\_entry\_index, line\_index, active\_low, representing the index of the target
...
the `GpioIo()` in \_CRS the desc will hold ERR\_PTR(-ENOENT).
...
Be aware that [`gpiod_get_index()`](../../driver-api/gpio/index.html#c.gpiod_get_index "gpiod_get_index") in cases 1 and 2, assuming that there
