...
> 출처(원문): https://docs.kernel.org/firmware-guide/acpi/apei/einj.html
...
* available\_error\_type
  ...
  | V2\_0x00000001 | EINJV2 Processor Error |
  | V2\_0x00000002 | EINJV2 Memory Error |
  | V2\_0x00000004 | EINJV2 PCI Express Error |
...
* error\_type
  ...
  are defined in the file available\_error\_type above.
* error\_inject
...
* flags
  ...
  SET\_ERROR\_TYPE\_WITH\_ADDRESS data structure:
...
* param1
  ...
  depends on the error type specified in error\_type. For example, if
...
* component\_id0 .. component\_idN, component\_syndrome0 .. component\_syndromeN
...
* einj\_types:
  ...
  Provides the same functionality as available\_error\_types above, but
  for CXL error types
* $dport\_dev/einj\_inject:

  Injects a CXL error type into the CXL port represented by $dport\_dev,
  where $dport\_dev is the name of the CXL port (usually a PCIe device name).
...
extension (enabled with the param\_extension=1 module parameter, or boot
command line einj.param\_extension=1). This allows the address and mask
...
error\_type. A file vendor\_flags controls the interpretation of param1
...
A CXL error injection example with $dport\_dev=0000:e0:01.1:
