...
> 출처(원문): https://docs.kernel.org/mm/page_migration.html
...
Also see [Heterogeneous Memory Management (HMM)](hmm.html) for migrating pages to or from device
private memory.
...
pages are located through the MF\_MOVE and MF\_MOVE\_ALL options while setting
...
pages of a process are located. See also the numa\_maps documentation in the
proc(5) man page.
...
“move\_pages” allows the moving of individual pages within a process.
...
[CPUSETS](../admin-guide/cgroup-v1/cpusets.html#cpusets)).
...
## In kernel use of migrate\_pages()
...
1. Remove folios from the LRU.
   ...
   calling [`folio_isolate_lru()`](../core-api/mm-api.html#c.folio_isolate_lru "folio_isolate_lru").
   Calling [`folio_isolate_lru()`](../core-api/mm-api.html#c.folio_isolate_lru "folio_isolate_lru") increases the references to the folio
   so that it cannot vanish while the folio migration occurs.
   It also prevents the swapper or other scans from encountering
   the folio.
2. We need to have a function of type new\_folio\_t that can be
...
already been removed from the LRU via [`folio_isolate_lru()`](../core-api/mm-api.html#c.folio_isolate_lru "folio_isolate_lru") and the refcount
...
5. The i\_pages lock is taken. This will cause all processes trying
...
11. The i\_pages lock is dropped. With that lookups in the mapping
...
## movable\_ops page migration

Selected typed, non-folio pages (e.g., pages inflated in a memory balloon,
zsmalloc pages) can be migrated using the movable\_ops migration framework.

The “[`struct movable_operations`](#c.movable_operations "movable_operations")” provide callbacks specific to a page type
for isolating, migrating and un-isolating (putback) these pages.

Once a page is indicated as having movable\_ops, that condition must not
...
PG\_movable\_ops page flag.

Arbitrary drivers cannot currently make use of this framework, as it
requires:

1. a page type
2. indicating them as possibly having movable\_ops in [`page_has_movable_ops()`](../core-api/mm-api.html#c.page_has_movable_ops "page_has_movable_ops")
   based on the page type
3. returning the movable\_ops from `page_movable_ops()` based on the page
   type
4. not reusing the PG\_movable\_ops and PG\_movable\_ops\_isolated page flags
   for other purposes
...
1. PGMIGRATE\_SUCCESS: Normal page migration success. Each count means that a
...
2. PGMIGRATE\_FAIL: Normal page migration failure. Same counting rules as for
   PGMIGRATE\_SUCCESS, above: this will be increased by the number of subpages,
   if it was a THP or hugetlb.
3. THP\_MIGRATION\_SUCCESS: A THP was migrated without being split.
4. THP\_MIGRATION\_FAIL: A THP could not be migrated nor it could be split.
5. THP\_MIGRATION\_SPLIT: A THP was migrated, but not as such: first, the THP had
   to be split. After splitting, a migration retry was used for its sub-pages.

THP\_MIGRATION\_\* events also update the appropriate PGMIGRATE\_SUCCESS or
PGMIGRATE\_FAIL events. For example, a THP migration failure will cause both
THP\_MIGRATION\_FAIL and PGMIGRATE\_FAIL to increase.

Christoph Lameter, May 8, 2006.
Minchan Kim, Mar 28, 2016.

struct movable\_operations
