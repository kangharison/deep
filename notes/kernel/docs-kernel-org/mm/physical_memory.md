...
> 출처(원문): https://docs.kernel.org/mm/physical_memory.html
...
be discussed further in Section [Nodes](#nodes)
...
* `ZONE_DMA` and `ZONE_DMA32` historically represented memory suitable for
  ...
  memory with DMA specific requirements ([Dynamic DMA mapping using the generic device](../core-api/dma-api.html)),
...
* `ZONE_MOVABLE` is for normal accessible memory, just like `ZONE_NORMAL`.
  ...
  [Page migration](page_migration.html) and
  [Memory Hot(Un)Plug](../admin-guide/mm/memory-hotplug.html) for additional details.
* `ZONE_DEVICE` represents memory residing on devices such as PMEM and GPU.
  It has different characteristics than RAM zone types and it exists to provide
  [struct page](#pages) and memory map services for device driver
...
discussed further in Section [Zones](#zones).
...
[NUMA Memory Policy](../admin-guide/mm/numa_memory_policy.html).
...
:   The zones for this node. Not all of the zones may be populated, but it is
    the full list. It is referenced by this node’s node\_zonelists as well as
    other node’s node\_zonelists.
...
See also [Page Reclaim](page_reclaim.html).
...
The page allocator uses the GFP flags, see [Memory Allocation Controls](../core-api/mm-api.html#mm-api-gfp-flags), specified by
