...
> 출처(원문): https://docs.kernel.org/mm/vmemmap_dedup.html
...
architectures. See [HugeTLB Pages](../admin-guide/mm/hugetlbpage.html) for more
...
is the compound\_info field, and this field is the same for all tail pages.
...
Where HugeTLB\_Size is the size of the HugeTLB page. We know that the size
of the HugeTLB page is always n times PAGE\_SIZE. So we can get the following
relationship:
...
n is (PAGE\_SIZE / sizeof(pte\_t)).

This optimization only supports 64-bit system, so the value of sizeof(pte\_t)
...
Where the struct\_size(pmd) is the size of the `struct page` structs of a
HugeTLB page of the pmd level mapping.

E.g.: A 2MB HugeTLB page on x86\_64 consists in 8 page frames while 1GB
...
We also can use this approach to free (PAGE\_SIZE - 1) vmemmap pages.
...
The following page sizes are supported in DAX: PAGE\_SIZE (4K on x86\_64),
PMD\_SIZE (2M on x86\_64) and PUD\_SIZE (1G on x86\_64).
For powerpc equivalent details see [Device DAX](../arch/powerpc/vmemmap_dedup.html)
