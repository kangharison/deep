...
> 출처(원문): https://docs.kernel.org/mm/zsmalloc.html
...
any object of size PAGE\_SIZE/2 or larger would occupy an entire page.
...
For simplicity, zsmalloc can only allocate objects of size up to PAGE\_SIZE
since this satisfies the requirements of all its current users (in the
worst case, page is incompressible and is thus stored “as-is” i.e. in
uncompressed form). For allocation requests larger than this size, failure
is returned (see zs\_malloc).

Additionally, [`zs_malloc()`](#c.zs_malloc "zs_malloc") does not return a dereferenceable pointer.
...
With CONFIG\_ZSMALLOC\_STAT, we could see zsmalloc internal information via
...
the total number of objects the zspage can hold (objs\_per\_zspage). The
closer the inuse counter is to objs\_per\_zspage, the better.

## Internals

zsmalloc has 255 size classes, each of which can hold a number of zspages.
Each zspage can contain up to ZSMALLOC\_CHAIN\_SIZE physical (0-order) pages.
...
* CONFIG\_ZSMALLOC\_CHAIN\_SIZE=4
  ...
  ```
* CONFIG\_ZSMALLOC\_CHAIN\_SIZE=8
...
CONFIG\_ZSMALLOC\_CHAIN\_SIZE option).

## Functions

void obj\_to\_location(unsigned long obj, struct [zpdesc](#c.obj_to_location "zpdesc") \*\*zpdesc, unsigned int \*obj\_idx)
:   get (<zpdesc>, <obj\_idx>) from encoded object value
...
unsigned long location\_to\_obj(struct [zpdesc](#c.location_to_obj "zpdesc") \*zpdesc, unsigned int obj\_idx)
:   get obj value encoded from (<zpdesc>, <obj\_idx>)
...
unsigned int zs\_lookup\_class\_index(struct zs\_pool \*pool, unsigned int size)
...
size\_t zs\_huge\_class\_size(struct zs\_pool \*pool)
...
unsigned long zs\_malloc(struct zs\_pool \*pool, size\_t size, gfp\_t gfp, const int nid)
...
otherwise an [`ERR_PTR()`](../core-api/kernel-api.html#c.ERR_PTR "ERR_PTR").
Allocation requests with size > ZS\_MAX\_ALLOC\_SIZE will fail.

struct zs\_pool \*zs\_create\_pool(const char \*name)
