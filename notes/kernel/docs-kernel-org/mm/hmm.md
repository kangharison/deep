...
> 출처(원문): https://docs.kernel.org/mm/hmm.html
...
## [Problems of using a device specific memory allocator](#id1)
...
## [I/O bus, device memory characteristics](#id2)
...
## [Shared address space and migration](#id3)
...
The second mechanism HMM provides is a new kind of ZONE\_DEVICE memory that
...
mechanisms. Here again, HMM only provides helpers, first to hotplug new ZONE\_DEVICE
...
## [Address space mirroring implementation and API](#id4)

Address space mirroring’s main objective is to allow duplication of a range of
CPU page table into a device page table; HMM helps keep both synchronized. A
device driver that wants to mirror a process address space must start with the
registration of a mmu\_interval\_notifier:
...
## [Leverage default\_flags and pfn\_flags\_mask](#id5)

The hmm\_range `struct has` 2 fields, default\_flags and pfn\_flags\_mask, that specify
...
address == range->start + (index\_of\_write << PAGE\_SHIFT) it will fault with
write permission i.e., if the CPU pte does not have write permission set then HMM
will call `handle_mm_fault()`.

After hmm\_range\_fault completes the flag bits are set to the current state of
the page tables, ie HMM\_PFN\_VALID | HMM\_PFN\_WRITE will be set if the page is
writable.

## [Represent and manage device memory from core kernel point of view](#id6)
...
## [Migration to and from device memory](#id7)
...
The [`migrate_vma_setup()`](../core-api/mm-api.html#c.migrate_vma_setup "migrate_vma_setup"), [`migrate_vma_pages()`](../core-api/mm-api.html#c.migrate_vma_pages "migrate_vma_pages"), and [`migrate_vma_finalize()`](../core-api/mm-api.html#c.migrate_vma_finalize "migrate_vma_finalize")
...
There are also [`devm_request_free_mem_region()`](../core-api/kernel-api.html#c.devm_request_free_mem_region "devm_request_free_mem_region"), [`devm_memremap_pages()`](../core-api/mm-api.html#c.devm_memremap_pages "devm_memremap_pages"),
...
memory (see [Page migration](page_migration.html)) but the steps are split
between device driver specific code and shared common code:

1. `mmap_read_lock()`

   The device driver has to pass a `struct vm_area_struct` to
   [`migrate_vma_setup()`](../core-api/mm-api.html#c.migrate_vma_setup "migrate_vma_setup") so the `mmap_read_lock()` or `mmap_write_lock()` needs to
   be held for the duration of the migration.
2. `migrate_vma_setup(struct migrate_vma *args)`

   The device driver initializes the `struct migrate_vma` fields and passes
   the pointer to [`migrate_vma_setup()`](../core-api/mm-api.html#c.migrate_vma_setup "migrate_vma_setup"). The `args->flags` field is used to
   ...
   One of the first steps [`migrate_vma_setup()`](../core-api/mm-api.html#c.migrate_vma_setup "migrate_vma_setup") does is to invalidate other
   ...
   the `args->pgmap_owner` field passed to [`migrate_vma_setup()`](../core-api/mm-api.html#c.migrate_vma_setup "migrate_vma_setup"). This
   ...
   [`migrate_vma_setup()`](../core-api/mm-api.html#c.migrate_vma_setup "migrate_vma_setup") also clears the `args->dst` array.
...
## [Memory cgroup (memcg) and rss accounting](#id9)
