...
> 출처(원문): https://docs.kernel.org/sound/kernel-api/writing-an-alsa-driver.html
...
`core/seq/oss` directory (see [below](#core-seq-oss)).
...
* define the PCI ID table (see the section [PCI Entries](#pci-entries)).
* create `probe` callback.
* create `remove` callback.
* create a [`struct pci_driver`](../../PCI/pci.html#c.pci_driver "pci_driver") structure
  containing the three pointers above.
* create an `init` function just calling the
  `pci_register_driver()` to register the pci\_driver
  table defined above.
* create an `exit` function to call the
  [`pci_unregister_driver()`](../../driver-api/pci/pci.html#c.pci_unregister_driver "pci_unregister_driver") function.
...
return. dev will be incremented also later ([step 7](#set-the-pci-driver-data-and-return-zero)).
...
Components](#management-of-cards-and-components).
...
Management](#pci-resource-management).
...
[`snd_card_free()`](alsa-driver-api.html#c.snd_card_free "snd_card_free") call should suffice in most cases.
...
Here you define the basic components such as [PCM](#pcm-interface),
mixer (e.g. [AC97](#api-for-ac97-codec)), MIDI (e.g.
[MPU-401](#midi-mpu401-uart-interface)), and other interfaces.
Also, if you want a [proc file](#proc-interface), define it here,
too.
...
Components](#management-of-cards-and-components), too.
...
It would be typically just calling [`snd_card_free()`](alsa-driver-api.html#c.snd_card_free "snd_card_free"):
...
[`mdelay()`](../../timers/delay_sleep_functions.html#c.mdelay "mdelay") or [`udelay()`](../../timers/delay_sleep_functions.html#c.udelay "udelay") functions, you’ll need
...
[`snd_card_new()`](alsa-driver-api.html#c.snd_card_new "snd_card_new"):
...
card->private\_data for the chip-specific data. Note that these data are
allocated by [`snd_card_new()`](alsa-driver-api.html#c.snd_card_new "snd_card_new").

The first argument, the pointer of [`struct device`](../../driver-api/infrastructure.html#c.device "device"), specifies the parent
...
A component can be created via the [`snd_device_new()`](alsa-driver-api.html#c.snd_device_new "snd_device_new")
function:
...
[`snd_device_new()`](alsa-driver-api.html#c.snd_device_new "snd_device_new") inside its constructor. The destructor
...
function to the dev\_free callback in the `ops`, so that it can be
released automatically via [`snd_card_free()`](alsa-driver-api.html#c.snd_card_free "snd_card_free"). The next
...
#### 1. Allocating via [`snd_card_new()`](alsa-driver-api.html#c.snd_card_new "snd_card_new").

As mentioned above, you can pass the extra-data-length to the 5th
argument of [`snd_card_new()`](alsa-driver-api.html#c.snd_card_new "snd_card_new"), e.g.:
...
After allocating a card instance via [`snd_card_new()`](alsa-driver-api.html#c.snd_card_new "snd_card_new")
(with `0` on the 4th arg), call [`kzalloc()`](../../core-api/mm-api.html#c.kzalloc "kzalloc"):
...
registering and disconnecting the card via a setting in snd\_device\_ops.
...
[`snd_card_register()`](alsa-driver-api.html#c.snd_card_register "snd_card_register"). Access to the device files is
enabled at this point. That is, before
[`snd_card_register()`](alsa-driver-api.html#c.snd_card_register "snd_card_register") is called, the components are safely
inaccessible from external side. If this call fails, exit the probe
function after releasing the card via [`snd_card_free()`](alsa-driver-api.html#c.snd_card_free "snd_card_free").

For releasing the card instance, you can call simply
[`snd_card_free()`](alsa-driver-api.html#c.snd_card_free "snd_card_free"). As mentioned earlier, all components
are released automatically by this call.

For a device which allows hotplugging, you can use
[`snd_card_free_when_closed()`](alsa-driver-api.html#c.snd_card_free_when_closed "snd_card_free_when_closed"). This one will postpone
...
[`pci_enable_device()`](../../driver-api/pci/pci.html#c.pci_enable_device "pci_enable_device") function before allocating
...
[`pci_set_master()`](../../driver-api/pci/pci.html#c.pci_set_master "pci_set_master") function, too.
...
[`kzalloc()`](../../core-api/mm-api.html#c.kzalloc "kzalloc") automatically, so you don’t have to take care of
resetting them.
...
[`kmalloc()`](../../core-api/mm-api.html#c.kmalloc "kmalloc") by `request_region()`. The pointer
must be released via [`kfree()`](../../core-api/mm-api.html#c.kfree "kfree"), but there is a problem with
...
defined [later](#pcm-interrupt-handler). Note that
`chip->irq` should be defined only when [`request_irq()`](../../core-api/genericirq.html#c.request_irq "request_irq")
succeeded.

On the PCI bus, interrupts can be shared. Thus, `IRQF_SHARED` is used
as the interrupt flag of [`request_irq()`](../../core-api/genericirq.html#c.request_irq "request_irq").

The last argument of [`request_irq()`](../../core-api/genericirq.html#c.request_irq "request_irq") is the data pointer
...
[`synchronize_irq()`](../../core-api/genericirq.html#c.synchronize_irq "synchronize_irq") at the right time, like before `hw_free`.
See the later section [sync\_stop callback](#sync-stop-callback) for details.
...
[`pci_request_region()`](../../driver-api/pci/pci.html#c.pci_request_region "pci_request_region") or
[`pci_request_regions()`](../../driver-api/pci/pci.html#c.pci_request_regions "pci_request_regions") like in this example, release the
resource(s) using the corresponding function,
[`pci_release_region()`](../../driver-api/pci/pci.html#c.pci_release_region "pci_release_region") or
[`pci_release_regions()`](../../driver-api/pci/pci.html#c.pci_release_regions "pci_release_regions"):
...
[`release_resource()`](../../core-api/kernel-api.html#c.release_resource "release_resource"). Suppose that you keep the resource
pointer returned from `request_region()` in
chip->res\_port, the release procedure looks like:

```
release_and_free_resource(chip->res_port);
```

Don’t forget to call [`pci_disable_device()`](../../driver-api/pci/pci.html#c.pci_disable_device "pci_disable_device") before the
end.
...
[`snd_device_new()`](alsa-driver-api.html#c.snd_device_new "snd_device_new") with `SNDRV_DEV_LOWLELVEL`, its
...
Of course, a modern way with [`pci_iomap()`](../../driver-api/pci/pci.html#c.pci_iomap "pci_iomap") will make things a
bit easier, too:
...
[`struct pci_device_id`](../../PCI/pci.html#c.pci_device_id "pci_device_id") table for
...
The first and second fields of the [`struct pci_device_id`](../../PCI/pci.html#c.pci_device_id "pci_device_id") are the vendor
...
[`struct pci_device_id`](../../PCI/pci.html#c.pci_device_id "pci_device_id") contains private data for this entry. You can specify
...
Then, prepare the [`struct pci_driver`](../../PCI/pci.html#c.pci_driver "pci_driver")
record:
...
access some functions related with hw\_param.
...
A PCM instance is allocated by the [`snd_pcm_new()`](alsa-driver-api.html#c.snd_pcm_new "snd_pcm_new")
...
The [`snd_pcm_new()`](alsa-driver-api.html#c.snd_pcm_new "snd_pcm_new") function takes six arguments. The
...
All the callbacks are described in the [Operators](#operators) subsection.
...
Management](#buffer-and-memory-management).
...
need to control the PCM: a copy of hw\_params and sw\_params
...
in the [PCM open callback](#pcm-open-callback). Note that the runtime instance holds a copy of
...
* The `info` field contains the type and capabilities of this
  ...
  flag. See the [Power Management](#power-management) section for details.
...
you can use the [`frames_to_bytes()`](alsa-driver-api.html#c.frames_to_bytes "frames_to_bytes") and
[`bytes_to_frames()`](alsa-driver-api.html#c.bytes_to_frames "bytes_to_frames") helper functions:
...
[`memcpy()`](../../core-api/kernel-api.html#c.memcpy "memcpy") from/to this pointer. Meanwhile, `dma_addr` holds
...
API function [`snd_pcm_lib_malloc_pages()`](alsa-driver-api.html#c.snd_pcm_lib_malloc_pages "snd_pcm_lib_malloc_pages") for allocating the buffer,
these fields are set by the ALSA middle layer, and you should *not*
change them by yourself. You can read them but not write them. On the
other hand, if you want to allocate the buffer by yourself, you’ll
need to manage it in the hw\_params callback. At least, `dma_bytes` is
mandatory. `dma_area` is necessary when the buffer is mmapped. If
your driver doesn’t support mmap, this field is not
necessary. `dma_addr` is also optional. You can use dma\_private as
you like, too.
...
callback](#pcm-open-callback). Don’t mix this with `pcm->private_data`. The
...
The allocated object must be released in the [close callback](#close-callback).
...
[Private Data](#private-data) section.

If the hardware configuration needs more constraints, set the hardware
constraints here, too. See [Constraints](#constraints) for more details.
...
function [`snd_pcm_lib_ioctl()`](alsa-driver-api.html#c.snd_pcm_lib_ioctl "snd_pcm_lib_ioctl"). If you need to deal with a
...
[`snd_pcm_lib_malloc_pages()`](alsa-driver-api.html#c.snd_pcm_lib_malloc_pages "snd_pcm_lib_malloc_pages") is available only when the
DMA buffers have been pre-allocated. See the section [Buffer Types](#buffer-types)
for more details.
...
`trigger` callback. Please see the subsection [Atomicity](#atomicity) for
details.
...
[`snd_pcm_lib_malloc_pages()`](alsa-driver-api.html#c.snd_pcm_lib_malloc_pages "snd_pcm_lib_malloc_pages") like:
...
[`snd_pcm_prepare()`](alsa-driver-api.html#c.snd_pcm_prepare "snd_pcm_prepare") is called, i.e. when recovering after
underruns, etc.
...
See the [Power Management](#power-management) section for details.
...
like [`synchronize_irq()`](../../core-api/genericirq.html#c.synchronize_irq "synchronize_irq") here.

For the majority of drivers that need only a call of
[`synchronize_irq()`](../../core-api/genericirq.html#c.synchronize_irq "synchronize_irq"), there is a simpler setup, too.
...
[`synchronize_irq()`](../../core-api/genericirq.html#c.synchronize_irq "synchronize_irq") with the given IRQ appropriately.
...
middle layer, which is invoked when [`snd_pcm_period_elapsed()`](alsa-driver-api.html#c.snd_pcm_period_elapsed "snd_pcm_period_elapsed")
...
[`snd_pcm_period_elapsed()`](alsa-driver-api.html#c.snd_pcm_period_elapsed "snd_pcm_period_elapsed") function.
...
[`snd_pcm_period_elapsed()`](alsa-driver-api.html#c.snd_pcm_period_elapsed "snd_pcm_period_elapsed") at each interrupt.

[`snd_pcm_period_elapsed()`](alsa-driver-api.html#c.snd_pcm_period_elapsed "snd_pcm_period_elapsed") takes the substream pointer as
...
calling [`snd_pcm_period_elapsed()`](alsa-driver-api.html#c.snd_pcm_period_elapsed "snd_pcm_period_elapsed"), because
[`snd_pcm_period_elapsed()`](alsa-driver-api.html#c.snd_pcm_period_elapsed "snd_pcm_period_elapsed") calls other PCM callbacks
inside.
...
[`snd_pcm_stop_xrun()`](alsa-driver-api.html#c.snd_pcm_stop_xrun "snd_pcm_stop_xrun"). This function stops the stream and sets
...
[`snd_pcm_period_elapsed()`](alsa-driver-api.html#c.snd_pcm_period_elapsed "snd_pcm_period_elapsed") and reset the accumulator.
...
#### On calling [`snd_pcm_period_elapsed()`](alsa-driver-api.html#c.snd_pcm_period_elapsed "snd_pcm_period_elapsed")

In both cases, even if more than one period has elapsed, you don’t have
to call [`snd_pcm_period_elapsed()`](alsa-driver-api.html#c.snd_pcm_period_elapsed "snd_pcm_period_elapsed") many times. Call only
...
callback, please use [`udelay()`](../../timers/delay_sleep_functions.html#c.udelay "udelay") or [`mdelay()`](../../timers/delay_sleep_functions.html#c.mdelay "mdelay").
...
[`snd_pcm_period_elapsed()`](alsa-driver-api.html#c.snd_pcm_period_elapsed "snd_pcm_period_elapsed") is called typically from the
...
[`snd_pcm_period_elapsed()`](alsa-driver-api.html#c.snd_pcm_period_elapsed "snd_pcm_period_elapsed") in the atomic context (e.g. the
period gets elapsed during `ack` or other callback). There is a
variant that can be called inside the PCM stream lock
[`snd_pcm_period_elapsed_under_stream_lock()`](alsa-driver-api.html#c.snd_pcm_period_elapsed_under_stream_lock "snd_pcm_period_elapsed_under_stream_lock") for that purpose,
too.
...
values, use [`snd_pcm_hw_constraint_list()`](alsa-driver-api.html#c.snd_pcm_hw_constraint_list "snd_pcm_hw_constraint_list"). You need to
...
[`snd_pcm_hw_constraint_integer()`](alsa-driver-api.html#c.snd_pcm_hw_constraint_integer "snd_pcm_hw_constraint_integer") for
...
are described in the [Control Names](#control-names) subsection.
...
there. The details will be explained in the [Access Flags](#access-flags)
subsection.
...
see the [Metadata](#metadata) subsection.

The other three are [Control Callbacks](#control-callbacks).
...
changed without [Change notification](#change-notification). Applications should poll such
...
[`snd_ctl_enum_info()`](alsa-driver-api.html#c.snd_ctl_enum_info "snd_ctl_enum_info"). The final code looks like below.
...
[`snd_ctl_boolean_mono_info()`](alsa-driver-api.html#c.snd_ctl_boolean_mono_info "snd_ctl_boolean_mono_info") and
[`snd_ctl_boolean_stereo_info()`](alsa-driver-api.html#c.snd_ctl_boolean_stereo_info "snd_ctl_boolean_stereo_info"). Obviously, the former
...
[`snd_ctl_new1()`](alsa-driver-api.html#c.snd_ctl_new1 "snd_ctl_new1") and [`snd_ctl_add()`](alsa-driver-api.html#c.snd_ctl_add "snd_ctl_add").
...
and chip is the object pointer to be passed to kcontrol->private\_data which
can be referred to in callbacks.

[`snd_ctl_new1()`](alsa-driver-api.html#c.snd_ctl_new1 "snd_ctl_new1") allocates a new `struct snd_kcontrol` instance, and
[`snd_ctl_add()`](alsa-driver-api.html#c.snd_ctl_add "snd_ctl_add") assigns the given control component to the
card.

### Change Notification

If you need to change and update a control in the interrupt routine, you
can call [`snd_ctl_notify()`](alsa-driver-api.html#c.snd_ctl_notify "snd_ctl_notify"). For example:
...
To create an ac97 instance, first call [`snd_ac97_bus()`](alsa-driver-api.html#c.snd_ac97_bus "snd_ac97_bus")
...
And then call [`snd_ac97_mixer()`](alsa-driver-api.html#c.snd_ac97_mixer "snd_ac97_mixer") with a `struct snd_ac97_template`
...
following functions: [`snd_ac97_write()`](alsa-driver-api.html#c.snd_ac97_write "snd_ac97_write"),
[`snd_ac97_read()`](alsa-driver-api.html#c.snd_ac97_read "snd_ac97_read"), [`snd_ac97_update()`](alsa-driver-api.html#c.snd_ac97_update "snd_ac97_update") and
[`snd_ac97_update_bits()`](alsa-driver-api.html#c.snd_ac97_update_bits "snd_ac97_update_bits").

Both [`snd_ac97_write()`](alsa-driver-api.html#c.snd_ac97_write "snd_ac97_write") and
[`snd_ac97_update()`](alsa-driver-api.html#c.snd_ac97_update "snd_ac97_update") functions are used to set a value to
the given register (`AC97_XXX`). The difference between them is that
[`snd_ac97_update()`](alsa-driver-api.html#c.snd_ac97_update "snd_ac97_update") doesn’t write a value if the given
value has been already set, while [`snd_ac97_write()`](alsa-driver-api.html#c.snd_ac97_write "snd_ac97_write")
...
[`snd_ac97_read()`](alsa-driver-api.html#c.snd_ac97_read "snd_ac97_read") is used to read the value of the given
...
[`snd_ac97_update_bits()`](alsa-driver-api.html#c.snd_ac97_update_bits "snd_ac97_update_bits") is used to update some bits in
the given register:
...
codec: [`snd_ac97_set_rate()`](alsa-driver-api.html#c.snd_ac97_set_rate "snd_ac97_set_rate"):
...
[`snd_ac97_mixer()`](alsa-driver-api.html#c.snd_ac97_mixer "snd_ac97_mixer") multiple times with `ac97.num=1` or
...
To create a rawmidi object, call [`snd_mpu401_uart_new()`](alsa-driver-api.html#c.snd_mpu401_uart_new "snd_mpu401_uart_new"):
...
to pass the iomapped address to [`snd_mpu401_uart_new()`](alsa-driver-api.html#c.snd_mpu401_uart_new "snd_mpu401_uart_new").

When `MPU401_INFO_TX_IRQ` is set, the output stream isn’t checked in
the default interrupt handler. The driver needs to call
[`snd_mpu401_uart_interrupt_tx()`](alsa-driver-api.html#c.snd_mpu401_uart_interrupt_tx "snd_mpu401_uart_interrupt_tx") by itself to start
processing the output stream in the irq handler.

If the MPU-401 interface shares its interrupt with the other logical
devices on the card, set `MPU401_INFO_IRQ_HOOK` (see
[below](#midi-interrupt-handler)).

Usually, the port address corresponds to the command port and port + 1
corresponds to the data port. If not, you may change the `cport`
field of `struct snd_mpu401` manually afterward.
However, `struct snd_mpu401` pointer is
not returned explicitly by [`snd_mpu401_uart_new()`](alsa-driver-api.html#c.snd_mpu401_uart_new "snd_mpu401_uart_new"). You
...
[`snd_mpu401_uart_new()`](alsa-driver-api.html#c.snd_mpu401_uart_new "snd_mpu401_uart_new"), an exclusive ISA interrupt
...
[`snd_mpu401_uart_interrupt()`](alsa-driver-api.html#c.snd_mpu401_uart_interrupt "snd_mpu401_uart_interrupt") explicitly from your own
...
object from [`snd_mpu401_uart_new()`](alsa-driver-api.html#c.snd_mpu401_uart_new "snd_mpu401_uart_new") as the second
argument of [`snd_mpu401_uart_interrupt()`](alsa-driver-api.html#c.snd_mpu401_uart_interrupt "snd_mpu401_uart_interrupt"):
...
To create a rawmidi device, call the [`snd_rawmidi_new()`](alsa-driver-api.html#c.snd_rawmidi_new "snd_rawmidi_new")
function:
...
These callbacks are explained in the [RawMIDI Callbacks](#rawmidi-callbacks) section.
...
[`snd_rawmidi_transmit_peek()`](alsa-driver-api.html#c.snd_rawmidi_transmit_peek "snd_rawmidi_transmit_peek"). It will return the number
...
[`snd_rawmidi_transmit_ack()`](alsa-driver-api.html#c.snd_rawmidi_transmit_ack "snd_rawmidi_transmit_ack") to remove the data from the
substream buffer:
...
the [`snd_rawmidi_transmit()`](alsa-driver-api.html#c.snd_rawmidi_transmit "snd_rawmidi_transmit") function which reads some
...
[`snd_rawmidi_receive()`](alsa-driver-api.html#c.snd_rawmidi_receive "snd_rawmidi_receive") for all received data:
...
[`snd_hwdep_new()`](alsa-driver-api.html#c.snd_hwdep_new "snd_hwdep_new"):
...
[`snd_dma_alloc_pages_fallback()`](alsa-driver-api.html#c.snd_dma_alloc_pages_fallback "snd_dma_alloc_pages_fallback"). This function tries
...
To release the pages, call the [`snd_dma_free_pages()`](alsa-driver-api.html#c.snd_dma_free_pages "snd_dma_free_pages")
function.
...
device pointer (see the [Non-Contiguous Buffers](#non-contiguous-buffers) section).
...
This is done by calling [`snd_pcm_set_managed_buffer_all()`](alsa-driver-api.html#c.snd_pcm_set_managed_buffer_all "snd_pcm_set_managed_buffer_all")
instead of [`snd_pcm_lib_preallocate_pages_for_all()`](alsa-driver-api.html#c.snd_pcm_lib_preallocate_pages_for_all "snd_pcm_lib_preallocate_pages_for_all"):
...
[`snd_pcm_lib_malloc_pages()`](alsa-driver-api.html#c.snd_pcm_lib_malloc_pages "snd_pcm_lib_malloc_pages") internally already before calling
the PCM `hw_params` callback, and call [`snd_pcm_lib_free_pages()`](alsa-driver-api.html#c.snd_pcm_lib_free_pages "snd_pcm_lib_free_pages")
...
[`snd_pcm_set_managed_buffer()`](alsa-driver-api.html#c.snd_pcm_set_managed_buffer "snd_pcm_set_managed_buffer") or
[`snd_pcm_set_managed_buffer_all()`](alsa-driver-api.html#c.snd_pcm_set_managed_buffer_all "snd_pcm_set_managed_buffer_all") with
...
Then in the [`snd_pcm_lib_malloc_pages()`](alsa-driver-api.html#c.snd_pcm_lib_malloc_pages "snd_pcm_lib_malloc_pages") call, the common SG-buffer
...
is addressed via runtime->dma\_area. The physical address
...
via [`snd_pcm_sgbuf_get_addr()`](alsa-driver-api.html#c.snd_pcm_sgbuf_get_addr "snd_pcm_sgbuf_get_addr").

If you need to release the SG-buffer data explicitly, call the
standard API function [`snd_pcm_lib_free_pages()`](alsa-driver-api.html#c.snd_pcm_lib_free_pages "snd_pcm_lib_free_pages") as usual.

### Vmalloc’ed Buffers

It’s possible to use a buffer allocated via [`vmalloc()`](../../core-api/mm-api.html#c.vmalloc "vmalloc"), for
example, for an intermediate buffer.
You can simply allocate it via the standard
[`snd_pcm_lib_malloc_pages()`](alsa-driver-api.html#c.snd_pcm_lib_malloc_pages "snd_pcm_lib_malloc_pages") and co. after setting up the
...
that default pages (GFP\_KERNEL and GFP\_HIGHMEM) will be
allocated.
...
In the write callback, you can use [`snd_info_get_line()`](alsa-driver-api.html#c.snd_info_get_line "snd_info_get_line")
to get a text line, and [`snd_info_get_str()`](alsa-driver-api.html#c.snd_info_get_str "snd_info_get_str") to retrieve
...
with \_\_maybe\_unused attribute; otherwise the compiler will complain.
...
calling [`snd_pcm_prepare()`](alsa-driver-api.html#c.snd_pcm_prepare "snd_pcm_prepare") and restart the stream
...
[`snd_pcm_suspend_all()`](alsa-driver-api.html#c.snd_pcm_suspend_all "snd_pcm_suspend_all") is called, regardless of the
...
3. If AC97 codecs are used, call [`snd_ac97_suspend()`](alsa-driver-api.html#c.snd_ac97_suspend "snd_ac97_suspend") for
   each codec.
...
4. Resume the mixer, e.g. by calling [`snd_ac97_resume()`](alsa-driver-api.html#c.snd_ac97_resume "snd_ac97_resume").
...
[`snd_pcm_suspend_all()`](alsa-driver-api.html#c.snd_pcm_suspend_all "snd_pcm_suspend_all") internally.
...
When you created the chip data with [`snd_card_new()`](alsa-driver-api.html#c.snd_card_new "snd_card_new"), it’s
...
example, an object allocated via [`devm_kmalloc()`](../../driver-api/basics.html#c.devm_kmalloc "devm_kmalloc") will be
freed automatically at unbinding the device.

ALSA core provides also the device-managed helper, namely,
[`snd_devm_card_new()`](alsa-driver-api.html#c.snd_devm_card_new "snd_devm_card_new") for creating a card object.
Call this functions instead of the normal [`snd_card_new()`](alsa-driver-api.html#c.snd_card_new "snd_card_new"),
and you can forget the explicit [`snd_card_free()`](alsa-driver-api.html#c.snd_card_free "snd_card_free") call, as
it’s called automagically at error and removal paths.

One caveat is that the call of [`snd_card_free()`](alsa-driver-api.html#c.snd_card_free "snd_card_free") would be put
at the beginning of the call chain only after you call
[`snd_card_register()`](alsa-driver-api.html#c.snd_card_register "snd_card_register").
...
[`snd_card_register()`](alsa-driver-api.html#c.snd_card_register "snd_card_register") call succeeds.
...
In addition to SND\_PCM, the following components are supported for
select command: SND\_RAWMIDI, SND\_TIMER, SND\_HWDEP, SND\_MPU401\_UART,
SND\_OPL3\_LIB, SND\_OPL4\_LIB, SND\_VX\_LIB, SND\_AC97\_CODEC.
...
### [`snd_BUG()`](alsa-driver-api.html#c.snd_BUG "snd_BUG")

It shows the `BUG?` message and stack trace as well as
[`snd_BUG_ON()`](alsa-driver-api.html#c.snd_BUG_ON "snd_BUG_ON") at the point. It’s useful to show that a
fatal error happens there.

When no debug flag is set, this macro is ignored.

### [`snd_BUG_ON()`](alsa-driver-api.html#c.snd_BUG_ON "snd_BUG_ON")

[`snd_BUG_ON()`](alsa-driver-api.html#c.snd_BUG_ON "snd_BUG_ON") macro is similar with
`WARN_ON()` macro. For example, snd\_BUG\_ON(!pointer); or
