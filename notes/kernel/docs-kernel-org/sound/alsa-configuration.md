...
> 출처(원문): https://docs.kernel.org/sound/alsa-configuration.html
...
:   Reserve the slot index for the given driver;
    ...
    See [Module Autoloading Support](#module-autoloading-support) section for details.
...
ac97\_quirk
:   AC’97 workaround for strange hardware;
    See [AC97 Quirk Option](#ac97-quirk-option) section below.
...
:   Specifies the early “patch” files to modify the HD-audio setup
    ...
    is set. See [More Notes on HD-Audio Driver](hd-audio/notes.html) for details.
...
See [More Notes on HD-Audio Driver](hd-audio/notes.html) for more details about HD-audio driver.
...
is found in [HD-Audio Codec-Specific Models](hd-audio/models.html).
...
ML (see the section [Links and Addresses](#links-and-addresses)).

`power_save` and `power_save_controller` options are for power-saving
mode. See powersave.rst for details.

Note 2: If you get click noises on output, try the module option
`position_fix=1` or `2`. `position_fix=1` will use the SD\_LPIB
...
stage of boot sequence. See [Early Buffer Allocation](#early-buffer-allocation)
section.
...
See [Alsa driver for Digigram miXart8 and miXart8AES/EBU soundcards](cards/mixart.html) for details.
...
:   Apply the generic implicit feedback sync mode. When this is set
    ...
    source. This is equivalent with quirk\_flags bit 17.

use\_vmalloc
:   Use [`vmalloc()`](../core-api/mm-api.html#c.vmalloc "vmalloc") for allocations of the PCM buffers (default: yes).
...
:   The option provides a refined and flexible control for applying quirk
    ...
    E.g., `quirk_flags=0x01,0x02` applies get\_sample\_rate to the first
    device, and share\_media\_device to the second device.
    ...
    > * bit 15: `dsd_raw`
    >   Support generic DSD raw U32\_BE format
    ...
    >   Skip the probe-time interface setup (usb\_set\_interface,
    >   init\_pitch, init\_sample\_rate); redundant with
...
subsystem vendor/device ids. See [Links and Addresses](#links-and-addresses)
below.
...
but only `Headphone` does, pass ac97\_quirk=hp\_only module option.
