...
> 출처(원문): https://docs.kernel.org/sound/soc/usb.html
...
The snd\_soc\_usb structure can be referenced using the ASoC platform card
...
Returns pointer to priv\_data on success, NULL on failure.
...
> **usb\_audio\_probe()**
> :   --> USB audio streams allocated and saved to usb\_chip[]
>
>     --> Propagate connect event to USB offload driver in USB SND
>
>     --> **snd\_soc\_usb\_connect()** exits as USB BE DAI link is not ready
>
...
> :   --> DAI link is probed and SoC USB port is allocated
>
>     --> SoC USB device added, and **snd\_usb\_rediscover\_devices()** runs
>
> **snd\_usb\_rediscover\_devices()**
> :   --> Traverses through usb\_chip[] and for non-NULL entries issue
>
>     **connection\_status\_cb()**
...
> :   --> **qc\_usb\_audio\_offload\_init()**
>
>     --> Calls **snd\_usb\_rediscover\_devices()** to notify of devices
