...
> 출처(원문): https://docs.kernel.org/sound/designs/compress-accel.html
...
A new direction SND\_COMPRESS\_ACCEL is introduced to identify
the passthrough API.
...
for given device. The fragment\_size limits the input buffer size for the given
...
fragment\_size. Allocates unique seqno.

The hardware drivers allocate internal ‘[`struct dma_buf`](../../driver-api/dma-buf.html#c.dma_buf "dma_buf")’ for both input and
output buffers (using ‘[`dma_buf_export()`](../../driver-api/dma-buf.html#c.dma_buf_export "dma_buf_export")’ function). The anonymous
...
the task is created. In this case, origin\_seqno must be zero.
The second case is for reusing of already finished task. The origin\_seqno
...
new source data and set input\_size to pass the real data size to the driver.
...
the user space may set SND\_COMPRESS\_TFLG\_NEW\_STREAM flag to mark the
