...
> 출처(원문): https://docs.kernel.org/sound/cards/pcmtest.html
...
> * fill\_mode (bool) - Buffer fill mode (see below)
> * inject\_delay (int)
> * inject\_hwpars\_err (bool)
> * inject\_prepare\_err (bool)
> * inject\_trigger\_err (bool)

### Capture Data Generation

The driver has two modes of data generation: the first (0 in the fill\_mode parameter)
means random data generation, the second (1 in the fill\_mode) - pattern-based
...
> * /sys/kernel/debug/pcmtest/fill\_pattern[0-3]
> * /sys/kernel/debug/pcmtest/fill\_pattern[0-3]\_len
...
The driver has ‘inject\_delay’ parameter, which has very self-descriptive name and
...
If the ‘inject\_delay’ value is positive, the buffer will be filled slower, if it is
...
short period of time (with the negative ‘inject\_delay’ value).
...
For example, you can make all ‘hw\_params’ PCM callback calls return EBUSY error by
writing ‘1’ to the ‘inject\_hwpars\_err’ module parameter:
...
buffer for containing the looped pattern (which is specified in the fill\_pattern
debugfs file for each channel). If the playback buffer content represents the looped
pattern, ‘pc\_test’ debugfs entry is set into ‘1’. Otherwise, the driver sets it to ‘0’.

### ioctl redefinition test

The driver redefines the ‘reset’ ioctl, which is default for all PCM devices. To test
this functionality, we can trigger the reset ioctl and check the ‘ioctl\_test’ debugfs
entry:
