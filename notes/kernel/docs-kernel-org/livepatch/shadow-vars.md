...
> 출처(원문): https://docs.kernel.org/livepatch/shadow-vars.html
...
* The klp\_shadow variable data structure encapsulates both tracking
...
It is important to note that the [`klp_shadow_alloc()`](api.html#c.klp_shadow_alloc "klp_shadow_alloc") and
[`klp_shadow_get_or_alloc()`](api.html#c.klp_shadow_get_or_alloc "klp_shadow_get_or_alloc") are zeroing the variable by default.
...
Note that the constructor is called under klp\_shadow\_lock spinlock. It allows
to do actions that can be done only once when a new variable is allocated.

* [`klp_shadow_get()`](api.html#c.klp_shadow_get "klp_shadow_get") - retrieve a shadow variable data pointer
  - search hashtable for <obj, id> pair
* [`klp_shadow_alloc()`](api.html#c.klp_shadow_alloc "klp_shadow_alloc") - allocate and add a new shadow variable
  - search hashtable for <obj, id> pair
...
* [`klp_shadow_get_or_alloc()`](api.html#c.klp_shadow_get_or_alloc "klp_shadow_get_or_alloc") - get existing or alloc a new shadow variable
  - search hashtable for <obj, id> pair
...
* [`klp_shadow_free()`](api.html#c.klp_shadow_free "klp_shadow_free") - detach and free a <obj, id> shadow variable
...
* [`klp_shadow_free_all()`](api.html#c.klp_shadow_free_all "klp_shadow_free_all") - detach and free all <\_, id> shadow variables
...
spinlock to net/mac80211/sta\_info.h :: [`struct sta_info`](../driver-api/80211/mac80211-advanced.html#c.sta_info "sta_info"). Each use-case
...
For [commit 1d147bfa6429](https://git.kernel.org/torvalds/c/1d147bfa6429), when a parent sta\_info structure is allocated,
allocate a shadow copy of the ps\_lock pointer, then initialize it:
...
When requiring a ps\_lock, query the shadow variable API to retrieve one
for a specific [`struct sta_info`](../driver-api/80211/mac80211-advanced.html#c.sta_info "sta_info")::
...
When the parent sta\_info structure is freed, first free the shadow
variable:
...
In these cases, the [`klp_shadow_get_or_alloc()`](api.html#c.klp_shadow_get_or_alloc "klp_shadow_get_or_alloc") call can be used to attach
