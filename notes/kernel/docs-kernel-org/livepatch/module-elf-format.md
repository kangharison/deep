...
> 출처(원문): https://docs.kernel.org/livepatch/module-elf-format.html
...
## [1. Background and motivation](#id2)
...
### [Why does livepatch need to write its own relocations?](#id3)
...
these relocations. In the new format, livepatch manages its own SHT\_RELA
...
## [3. Livepatch relocation sections](#id6)
...
## [3.1 Livepatch relocation section format](#id7)

Livepatch relocation sections must be marked with the SHF\_RELA\_LIVEPATCH
...
at patch module load time. These sections must also be marked with SHF\_ALLOC,
so that the module loader doesn’t discard them on module load (i.e. they will
be copied into memory along with the other SHF\_ALLOC sections).
...
[\*]
:   Livepatch relocation sections are SHT\_RELA sections but with a few special
    characteristics. Notice that they are marked SHF\_ALLOC (“A”) so that they will
    not be discarded when the module is loaded into memory, as well as with the
    SHF\_RELA\_LIVEPATCH flag (“o” - for OS-specific).
...
Livepatch symbols must be marked with SHN\_LIVEPATCH so that the module
...
## [4.1 A livepatch module’s symbol table](#id10)
...
## [4.2 Livepatch symbol format](#id11)

Livepatch symbols must have their section index marked as SHN\_LIVEPATCH, so
...
[\*]
:   Note that the ‘Ndx’ (Section index) for these symbols is SHN\_LIVEPATCH (0xff20).
    “OS” means OS-specific.

## [5. Symbol table and ELF section access](#id13)
...
module->klp\_info, which is a [`klp_modinfo`](../driver-api/basics.html#c.klp_modinfo "klp_modinfo") struct. When a livepatch module
