...
> 출처(원문): https://docs.kernel.org/livepatch/livepatch.html
...
## [2. Kprobes, Ftrace, Livepatching](#id4)
...
1. The first and most effective approach is stack checking of sleeping
   ...
   reliable stacks (HAVE\_RELIABLE\_STACKTRACE).
...
Architectures which don’t have HAVE\_RELIABLE\_STACKTRACE solely rely on
...
without HAVE\_RELIABLE\_STACKTRACE are not considered fully supported by
...
There’s also a /proc/<pid>/patch\_state file which can be used to
...
TIF\_PATCH\_PENDING flag of all tasks and thus forces the tasks to the patched
...
### [3.1 Adding consistency model support to new architectures](#id6)

For adding consistency model support to new architectures, there are a
few options:

1. Add CONFIG\_HAVE\_RELIABLE\_STACKTRACE. This means porting objtool, and
...
2. Alternatively, ensure that every kthread has a call to
   ...
   In that case, arches without HAVE\_RELIABLE\_STACKTRACE would still be
...
[Livepatch module ELF format](module-elf-format.html) for more details.
...
> * [`struct klp_func`](api.html#c.klp_func "klp_func") is defined for each patched function. It describes
...
> * [`struct klp_object`](api.html#c.klp_object "klp_object") defines an array of patched functions ([`struct
>   klp_func`](api.html#c.klp_func "klp_func")) in the same object. Where the object is either vmlinux
...
> * [`struct klp_patch`](api.html#c.klp_patch "klp_patch") defines an array of patched objects ([`struct
>   klp_object`](api.html#c.klp_object "klp_object")).
>
...
module is being loaded. For this, [`klp_enable_patch()`](api.html#c.klp_enable_patch "klp_enable_patch") has to be called
in the [`module_init()`](../driver-api/basics.html#c.module_init "module_init") callback. There are two main reasons:

First, only the module has an easy access to the related [`struct klp_patch`](api.html#c.klp_patch "klp_patch").
...
The livepatch gets enabled by calling [`klp_enable_patch()`](api.html#c.klp_enable_patch "klp_enable_patch") from
the [`module_init()`](../driver-api/basics.html#c.module_init "module_init") callback. The system will start using the new
...
ftrace handler is registered[[1]](#id2). This stage is indicated by a value of ‘1’
...
an entry to the list (see field func\_stack) of the `struct klp_ops`.
...
all the functions ([`struct klp_func`](api.html#c.klp_func "klp_func")) associated with the replaced
patches are removed from the corresponding `struct klp_ops`. Also
the ftrace handler is unregistered and the `struct klp_ops` is
freed when the related function is not modified by the new patch
and func\_stack list becomes empty.

See [Atomic Replace & Cumulative Patches](cumulative-patches.html) for more details.
...
to ‘0’. All the functions ([`struct klp_func`](api.html#c.klp_func "klp_func")) associated with the to-be-disabled
patch are removed from the corresponding `struct klp_ops`. The ftrace handler
is unregistered and the `struct klp_ops` is freed when the func\_stack list
becomes empty.
...
See [ABI file testing/sysfs-kernel-livepatch](../admin-guide/abi-testing-files.html#abi-file-testing-sysfs-kernel-livepatch) for more details.
...
>   using -fentry gcc compiler option on x86\_64.
>
