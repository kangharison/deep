...
> 출처(원문): https://docs.kernel.org/arch/x86/amd-memory-encryption.html
...
If support for SME is present, MSR 0xc00100010 (MSR\_AMD64\_SYSCFG) can be used to
...
If SEV is supported, MSR 0xc0010131 (MSR\_AMD64\_SEV) can be used to determine if
SEV is active:
...
> * Enabled:
>   Supported and bit 23 of MSR\_AMD64\_SYSCFG is set.
> * Active:
...
If the BIOS merely enables SME (sets bit 23 of the MSR\_AMD64\_SYSCFG),
then memory encryption can be enabled by supplying mem\_encrypt=on on the
kernel command line. However, if BIOS does not enable SME, then Linux
will not be able to activate memory encryption, even if configured to do
so by default or the mem\_encrypt=on command line parameter is specified.

## 19.1. Secure Nested Paging (SNP)

SEV-SNP introduces new features (SEV\_FEATURES[1:63]) which can be enabled
...
More details in AMD64 APM[1] Vol 2: 15.34.10 SEV\_STATUS MSR
...
Hardware requires that RMP\_BASE and (RPM\_END + 1) be 8KB aligned, but SEV
...
the RMP and to set RMP\_BASE and RMP\_END appropriately. Linux uses the MSR
...
The RMP segment size defined in the RMP\_CFG MSR applies to all segments
of the RMP. Therefore each RMP segment covers a specific range of system
physical addresses. For example, if the RMP\_CFG MSR value is 0x2401, then
the RMP segment coverage value is 0x24 => 36, meaning the size of memory
covered by an RMP segment is 64GB (1 << 36). So the first RMP segment
covers physical addresses from 0 to 0xF\_FFFF\_FFFF, the second RMP segment
covers physical addresses from 0x10\_0000\_0000 to 0x1F\_FFFF\_FFFF, etc.

When a segmented RMP is enabled, RMP\_BASE points to the RMP bookkeeping
...
of cacheable RMP segments (CPUID 0x80000025\_EBX[9:0]) if the number of cacheable
RMP segments is a hard limit (CPUID 0x80000025\_EBX[10]).

The current Linux support relies on BIOS to allocate/reserve the memory for
the segmented RMP (the bookkeeping area, RST, and all segments), build the RST
and to set RMP\_BASE, RMP\_END, and RMP\_CFG appropriately. Linux uses the MSR
