...
> 출처(원문): https://docs.kernel.org/arch/x86/shstk.html
...
The kernel Kconfig option is X86\_USER\_SHADOW\_STACK. When compiled in, shadow
...
CET. “user\_shstk” means that userspace shadow stack is supported on the current
kernel and HW.
...
## 15.4. Enabling arch\_prctl()’s

Elf features should be enabled by the loader using the below arch\_prctl’s. They
...
arch\_prctl(ARCH\_SHSTK\_ENABLE, unsigned long feature)
:   Enable a single feature specified in ‘feature’. Can only operate on
    one feature at a time.

arch\_prctl(ARCH\_SHSTK\_DISABLE, unsigned long feature)
:   Disable a single feature specified in ‘feature’. Can only operate on
    one feature at a time.

arch\_prctl(ARCH\_SHSTK\_LOCK, unsigned long features)
...
arch\_prctl(ARCH\_SHSTK\_UNLOCK, unsigned long features)
:   Unlock features. ‘features’ is a mask of all features to unlock. All
    bits set are processed, unset bits are ignored. Only works via ptrace.

arch\_prctl(ARCH\_SHSTK\_STATUS, unsigned long addr)
...
MIN(RLIMIT\_STACK, 4 GB). In other words, the shadow stack is allocated to
...
The shadow stack’s vma has VM\_SHADOW\_STACK flag set; its PTEs are required
