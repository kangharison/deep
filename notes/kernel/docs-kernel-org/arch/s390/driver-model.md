...
> 출처(원문): https://docs.kernel.org/arch/s390/driver-model.html
...
The basic [`struct ccw_device`](../../driver-api/s390-drivers.html#c.ccw_device "ccw_device") and [`struct ccw_driver`](../../driver-api/s390-drivers.html#c.ccw_driver "ccw_driver") data structures can be found
...
Each driver should declare in a MODULE\_DEVICE\_TABLE into which CU types/models
...
The functions in ccw\_driver should be used in the following way:
...
:   This function is called by the device layer for each device the driver
    ...
    to put in dev->driver\_data and create attributes (if needed). Also,
...
set\_offline: This function is called by the common I/O layer when the device is
...
:   This function is called by the common I/O layer for some state changes
    ...
    * In online state, device detached (CIO\_GONE) or last path gone
      (CIO\_NO\_PATH). The driver must return !0 to keep the device; for
      return code 0, the device will be deleted as usual (also when no
      notify function is registered). If the driver wants to keep the
      device, it is moved into disconnected state.
    * In disconnected state, device operational again (CIO\_OPER). The
...
:   cdev
    ...
    event
    :   * the event that happened. This can be one of CIO\_GONE,
          CIO\_NO\_PATH or CIO\_OPER.

The handler field of the [`struct ccw_device`](../../driver-api/s390-drivers.html#c.ccw_device "ccw_device") is meant to be set to the interrupt
handler for the device. In order to accommodate drivers which use several
distinct handlers (e.g. multi subchannel devices), this is a member of ccw\_device
instead of ccw\_driver.
...
The device driver is called from the common ccw\_device layer and can retrieve
...
ccwgroup device in the driver\_data of its device struct. This field must not be
touched by the driver - it should use the ccwgroup device’s driver\_data for its
private data.
