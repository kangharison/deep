...
> 출처(원문): https://docs.kernel.org/arch/riscv/cmodx.html
...
is limited to +-2K from the predetermined target, ftrace\_caller, due to the lack
of immediate encoding space in RISC-V. To address the issue, we introduce
CALL\_OPS, where an 8B naturally align metadata is added in front of each
...
remaining arguments will be delegated to the riscv\_set\_icache\_flush\_ctx
function detailed below.

int riscv\_set\_icache\_flush\_ctx(unsigned long ctx, unsigned long scope)
