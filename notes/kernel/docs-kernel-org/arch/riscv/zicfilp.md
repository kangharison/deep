...
> 출처(원문): https://docs.kernel.org/arch/riscv/zicfilp.html
...
* Indirect control transfers must land on a landing pad instruction `lpad`.
  ...
  + rs1 = x1 or rs1 = x5, i.e. a return from a function and returns are
    protected using shadow stack (see [Shadow stack to protect function returns on RISC-V Linux](zicfiss.html))
...
The toolchain sets up `GNU_PROPERTY_RISCV_FEATURE_1_FCFI` for
...
controlled via the `PR_GET_CFI` and `PR_SET_CFI`
`prctl()` arguments (respectively), by supplying
`PR_CFI_BRANCH_LANDING_PADS` as the second argument. These
...
If `PR_CFI_ENABLE` is set in arg, and the CPU supports
...
setting the bit `PR_CFI_LOCK` in arg. Either indirect branch
tracking must already be enabled for the task, or the bit
`PR_CFI_ENABLE` must also be set in arg. This is intended
...
`PR_CFI_DISABLE`. Disabling indirect branch tracking for the
...
have the `PR_CFI_ENABLE` bit set if indirect branch tracking
is currently enabled for the task, and if it is locked, will
additionally have the `PR_CFI_LOCK` bit set. If indirect
branch tracking is currently disabled for the task, the
`PR_CFI_DISABLE` bit will be set.
...
The kernel will treat this as `SIGSEGV` with code =
