...
> 출처(원문): https://docs.kernel.org/arch/powerpc/pci_iov_resource_on_powernv.html
...
>       0x8000\_0000..0xffff\_ffff. (Note: The top 64KB are actually
...
>
> But doing so introduces another problem: total\_VFs is usually smaller
...
> total\_VFs is less than 256, we have the situation in Figure 1.0, where
> segments [total\_VFs, 255] of the M64 window may map to some MMIO range on
> other devices:
>
> ```
> 0      1                     total_VFs - 1
...
> reserved in software; there are still only total\_VFs VFs, and they only
> respond to segments [0, total\_VFs - 1]. There’s nothing in hardware that
> responds to segments [total\_VFs, 255].
