...
> 출처(원문): https://docs.kernel.org/arch/powerpc/firmware-assisted-dump.html
...
* Unlike phyp dump, FADump allows user to release all the memory reserved
  for dump, with a single operation of echo 1 > /sys/kernel/fadump\_release\_mem.
* Once enabled through kernel boot parameter, FADump can be
  started/stopped through /sys/kernel/fadump\_registered interface (see
...
* When system crashes, the Power firmware will copy the registered
  ...
  :   The term ‘boot memory’ means size of the low memory chunk
      ...
      refer to [Documentation for Kdump - The kexec-based Crash Dumping Solution](../../admin-guide/kdump/kdump.html). If any
...
* Once the userspace tool is done saving dump, it will echo
  ‘1’ to /sys/kernel/fadump\_release\_mem to release the reserved
...
option CONFIG\_PRESERVE\_FA\_DUMP has to be enabled on such kernel
to ensure that crash data is preserved to process later.

-- On OPAL based machines (PowerNV), if the kernel is build with
:   CONFIG\_OPAL\_CORE=y, OPAL memory at the time of crash is also
    exported as /sys/firmware/opal/mpipl/core file. This procfs file is
    helpful in debugging OPAL crashes with GDB. The kernel memory
    used for exporting this procfs file can be released by echo’ing
    ‘1’ to /sys/firmware/opal/mpipl/release\_core node.

    e.g.
    :   # echo 1 > /sys/firmware/opal/mpipl/release\_core
...
:   > Fadump has a feature that allows passing additional kernel arguments
    ...
    e.g.
    # echo “nr\_cpus=16” > /sys/kernel/fadump/bootargs\_append
    ...
    e.g.
    # cat /sys/kernel/fadump/bootargs\_append
...
:   supported if the RMA size is greater than 768 MB. If the RMA
    ...
    /sys/kernel/fadump/bootargs\_append sysfs node.
...
/sys/kernel/fadump\_release\_mem file is created, and the reserved
memory is held.
...
1. Set config option CONFIG\_FA\_DUMP=y and build kernel.
...
NOTE:
:   1. ‘fadump\_reserve\_mem=’ parameter has been deprecated. Instead
...
>
>     To register FADump echo 1 > /sys/kernel/fadump\_registered and
>     echo 0 > /sys/kernel/fadump\_registered for un-register and stop the
>     FADump. Once the FADump is un-registered, the system crash will not
>     be handled and vmcore will not be captured. This interface can be
>     easily integrated with kdump service start/stop.
>
> /sys/kernel/fadump/mem\_reserved
>
...
>     ```
>
>     After echo 1, the content of the /sys/kernel/debug/powerpc/fadump\_region
...
Note: /sys/kernel/fadump\_release\_opalcore sysfs has moved to
:   > /sys/firmware/opal/mpipl/release\_core
...
| /sys/kernel/fadump\_enabled | /sys/kernel/fadump/enabled |
| /sys/kernel/fadump\_registered | /sys/kernel/fadump/registered |
| /sys/kernel/fadump\_release\_mem | /sys/kernel/fadump/release\_mem |

Here is the list of files under powerpc debugfs:
(Assuming debugfs is mounted on /sys/kernel/debug directory.)

> /sys/kernel/debug/powerpc/fadump\_region
...
NOTE:
:   Please refer to [DebugFS](../../filesystems/debugfs.html) on
