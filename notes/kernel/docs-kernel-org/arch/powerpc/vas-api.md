...
> 출처(원문): https://docs.kernel.org/arch/powerpc/vas-api.html
...
descriptor (fd). Then should issue VAS\_TX\_WIN\_OPEN ioctl with this fd to
...
(aka paste\_address) returned by mmap(). User space can close the
...
> * issue VAS\_TX\_WIN\_OPEN ioctl
> * mmap() the engine’s request queue into application’s virtual
>   address space (i.e. get a paste\_address for the co-processor
>   engine).
...
the vas\_id field in the VAS\_TX\_WIN\_OPEN ioctl as detailed below.
...
Applications should use the VAS\_TX\_WIN\_OPEN ioctl as follows to establish
...
>
> The attributes attr for the VAS\_TX\_WIN\_OPEN ioctl are defined as
> follows:
>
> ```
> #define VAS_MAGIC 'v'
...
> ```
>
> The VAS\_TX\_WIN\_OPEN ioctl returns 0 on success. On errors, it
...
The mmap() system call for a NX-GZIP device fd returns a paste\_address
...
> > |  |  |
> > | --- | --- |
> > | EINVAL | fd is not associated with an open window (i.e mmap() does not follow a successful call to the VAS\_TX\_WIN\_OPEN ioctl). |
