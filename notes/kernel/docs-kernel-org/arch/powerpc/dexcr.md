...
> 출처(원문): https://docs.kernel.org/arch/powerpc/dexcr.html
...
*[prctl(2)](https://man7.org/linux/man-pages/man2/prctl.2.html)* commands. These calls have the form:
...
| `prctl()` ctrl | Meaning |
| --- | --- |
| `PR_PPC_DEXCR_CTRL_EDITABLE` | This aspect can be configured with PR\_PPC\_SET\_DEXCR (get only) |
...
* The aspect state when starting a process is copied from the parent’s state on
  *[fork(2)](https://man7.org/linux/man-pages/man2/fork.2.html)*. The state is reset to a fixed value on
  *[execve(2)](https://man7.org/linux/man-pages/man2/execve.2.html)*. The PR\_PPC\_SET\_DEXCR `prctl()` can control both of these
  values.
...
| `EPERM` | This aspect cannot be modified with `prctl()` (check for the PR\_PPC\_DEXCR\_CTRL\_EDITABLE flag with PR\_PPC\_GET\_DEXCR). |
