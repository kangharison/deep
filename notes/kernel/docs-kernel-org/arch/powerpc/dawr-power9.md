...
> 출처(원문): https://docs.kernel.org/arch/powerpc/dawr-power9.html
...
PPC\_PTRACE\_GETHWDBGINFO call. This results in GDB falling back to
software emulation of the watchpoint (which is slow).

h\_set\_mode(DAWR) and `h_set_dabr()` will now return an error to the
...
PTRACE\_GETHWDBGINFO) and setting the breakpoint, userspace will get an
inconsistent view of what’s available. Similarly for guests.

For the DAWR to be enabled in a KVM guest, the DAWR needs to be force
enabled in the host AND the guest. For this reason, this won’t work on
POWERVM as it doesn’t allow the HCALL to work. Writes of ‘Y’ to the
dawr\_enable\_dangerous file will fail if the hypervisor doesn’t support
writing the DAWR.
