...
> 출처(원문): https://docs.kernel.org/arch/powerpc/papr_hcalls.html
...
specification [[1]](#id9) which describes the run-time environment for a guest
...
* **Qemu/KVM**: Supports PPC64 linux guests running on a PPC64 linux host.
  Though it only implements a subset of PAPR specification called LoPAPR [[2]](#id10).

On PPC64 arch a guest kernel running on top of a PAPR hypervisor is called
a *pSeries guest*. A pseries guest runs in a supervisor mode (HV=0) and must
issue hypercalls to the hypervisor whenever it needs to perform an action
that is hypervisor privileged [[3]](#id11) or for other services managed by the
hypervisor.
...
is covered in section 14.5.3 of ref [[2]](#id10). Switch to the Hypervisor context is
...
Powerpc arch code provides convenient wrappers named **plpar\_hcall\_xxx** defined
in a arch specific header [[4]](#id12) to issue hcalls from the linux kernel
running as pseries guest.

## Register Conventions

Any hcall should follow same register convention as described in section 2.2.1.1
of “64-Bit ELF V2 ABI Specification: Power Architecture”[[5]](#id13). Table below
...
return value of *H\_CONTINUE* indicates that hypervisor hasn’t still finished
...
for each subsequent hcall until hypervisor returns a non *H\_CONTINUE*
return value.

## HCALL Op-codes

Below is a partial list of HCALLs that are supported by PHYP. For the
corresponding opcode values please look into the arch specific header [[4]](#id12):
...
Return Value: *H\_Success, H\_Parameter, H\_P2, H\_P3, H\_P4, H\_Overlap,*

*H\_Too\_Big, H\_P5, H\_Busy*
...
case *targetLogicalMemoryAddress == 0xFFFFFFFF\_FFFFFFFF* then hypervisor
...
| Return Value: *H\_Success, H\_Parameter, H\_P2, H\_P3, H\_In\_Use, H\_Overlap,*
| *H\_Busy, H\_LongBusyOrder1mSec, H\_LongBusyOrder10mSec*
...
The hcall returns H\_BUSY when the flush takes longer time and the hcall needs
to be issued multiple times in order to be completely serviced. The
*continue-token* from the output to be passed in the argument list of
subsequent hcalls to the hypervisor until the hcall is completely serviced
at which point H\_SUCCESS or other error is returned by the hypervisor.
...
Return Value: *H\_Success,H\_Busy,H\_LongBusyOrder,H\_Partial,H\_Parameter,
H\_P2,H\_P3,H\_P4,H\_P5,H\_P6,H\_State,H\_Not\_Available,H\_Authority*

H\_HTM supports setup, configuration, control and dumping of Hardware Trace
...
Return Value: *H\_SUCCESS, H\_Function, H\_State, H\_R\_State, H\_Parameter, H\_P2,
H\_P3, H\_P4, H\_P5, H\_P6, H\_Authority, H\_Nomem, H\_Busy, H\_Resource,
H\_Aborted*

H\_PKS\_GEN\_KEY is used to have the hypervisor generate a new random key.
...
Return Value: *H\_SUCCESS, H\_Function, H\_State, H\_R\_State, H\_Parameter, H\_P2,
H\_P3, H\_P4, H\_P5, H\_P6, H\_P7, H\_P8, H\_P9, H\_Authority, H\_Invalid\_Key,
H\_NOT\_FOUND, H\_Busy, H\_LongBusy, H\_Aborted*

H\_PKS\_WRAP\_OBJECT is used to wrap an object using a wrapping key stored in the
...
which must have been previously created with H\_PKS\_GEN\_KEY. The provided object
is then encrypted with the wrapping key and additional metadata and the result
is returned to the caller.

**H\_PKS\_UNWRAP\_OBJECT**
...
Return Value: *H\_SUCCESS, H\_Function, H\_State, H\_R\_State, H\_Parameter, H\_P2,
H\_P3, H\_P4, H\_P5, H\_P6, H\_P7, H\_Authority, H\_Unsupported, H\_Bad\_Data,
H\_NOT\_FOUND, H\_Invalid\_Key, H\_Busy, H\_LongBusy, H\_Aborted*

H\_PKS\_UNWRAP\_OBJECT is used to unwrap an object that was previously warapped with
H\_PKS\_WRAP\_OBJECT.
