...
> 출처(원문): https://docs.kernel.org/arch/arm/interrupts.html
...
Unfortunately, this means that machine types that touch the irq\_desc[]
...
rerun
:   * optional. Not required if you’re using do\_level\_IRQ for all
...
> * “disable” depth (number of [`disable_irq()`](../../core-api/genericirq.html#c.disable_irq "disable_irq")s without [`enable_irq()`](../../core-api/genericirq.html#c.enable_irq "enable_irq")s)
...
2. New functions to manipulate the irqdesc array. The first 4 are expected
   ...
   > set\_irq\_chip(irq,chip)
   > :   Set the mask/unmask methods for handling this IRQ
   >
   > set\_irq\_handler(irq,handler)
   > :   Set the handler for this IRQ (level, edge, simple)
   >
   > set\_irq\_chained\_handler(irq,handler)
   > :   Set a “chained” handler for this IRQ - automatically
   >     enables this IRQ (eg, Neponset and SA1111 handlers).
   >
   > set\_irq\_flags(irq,flags)
   > :   Set the valid/probe/noautoenable flags.
   >
   > set\_irq\_type(irq,type)
   > :   Set active the IRQ edge(s)/level. This replaces the
   >     SA1111 INTPOL manipulation, and the `set_GPIO_IRQ_edge()`
   >     function. Type should be one of IRQ\_TYPE\_xxx defined in
   >     <linux/irq.h>
3. `set_GPIO_IRQ_edge()` is obsolete, and should be replaced by set\_irq\_type.
4. Direct access to SA1111 INTPOL is deprecated. Use set\_irq\_type instead.
