...
> 출처(원문): https://docs.kernel.org/arch/m68k/features.html
...
| core | eBPF-JIT | HAVE\_EBPF\_JIT | TODO | arch supports eBPF JIT optimizations |
| core | generic-idle-thread | GENERIC\_SMP\_IDLE\_THREAD | TODO | arch makes use of the generic SMP idle thread facility |
| core | jump-labels | HAVE\_ARCH\_JUMP\_LABEL | TODO | arch supports live patched, high efficiency branches |
...
| core | tracehook | HAVE\_ARCH\_TRACEHOOK | TODO | arch supports tracehook (ptrace) register handling APIs |
...
| debug | kgdb | HAVE\_ARCH\_KGDB | TODO | arch supports the kGDB kernel debugger |
| debug | kmemleak | HAVE\_DEBUG\_KMEMLEAK | TODO | arch supports the kernel memory leak detector |
| debug | kprobes | HAVE\_KPROBES | TODO | arch supports live patched kernel probe |
| debug | kprobes-on-ftrace | HAVE\_KPROBES\_ON\_FTRACE | TODO | arch supports combined kprobes and ftrace live patching |
| debug | kretprobes | HAVE\_KRETPROBES | TODO | arch supports kernel function-return probes |
...
| locking | lockdep | LOCKDEP\_SUPPORT | TODO | arch supports the runtime locking correctness debug facility |
...
| perf | kprobes-event | HAVE\_REGS\_AND\_STACK\_ACCESS\_API | TODO | arch supports kprobes with perf events |
...
| time | clockevents | !LEGACY\_TIMER\_TICK | TODO | arch support generic clock events |
...
| vm | batch-unmap-tlb-flush | ARCH\_WANT\_BATCHED\_UNMAP\_TLB\_FLUSH | --- | arch supports deferral of TLB flush until multiple pages are unmapped |
...
| vm | ioremap\_prot | HAVE\_IOREMAP\_PROT | TODO | arch has `ioremap_prot()` |
| vm | pte\_special | ARCH\_HAS\_PTE\_SPECIAL | TODO | arch supports the `pte_special()`/`pte_mkspecial()` VM APIs |
| vm | THP | HAVE\_ARCH\_TRANSPARENT\_HUGEPAGE | --- | arch supports transparent hugepages |
