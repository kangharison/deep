...
> 출처(원문): https://docs.kernel.org/arch/sparc/oradax/oracle-dax.html
...
The immediate commands are CCB\_DEQUEUE, CCB\_KILL, and CCB\_INFO. For
...
CCB\_KILL must be invoked on that CCB. Upon success, read() must be
...
accepted, and status\_data will provide additional data in some cases.
...
user process, and the mmap call must not specify PROT\_WRITE.
...
(ldxa, lduba, etc.) with ASI 0x84 (ASI\_MONITOR\_PRIMARY). Second, a
...
>   (CCB\_KILL or CCB\_INFO may be used any time as necessary)
> * perform a CCB\_DEQUEUE
...
See libdax/common/sys/dax1/dax1\_ccb.h for a detailed description of
...
ccb\_submit call are documented in the DAX HV API in section 36.3.1/
