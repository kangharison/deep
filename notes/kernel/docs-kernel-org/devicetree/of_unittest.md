...
> 출처(원문): https://docs.kernel.org/devicetree/of_unittest.html
...
1. [Linux and the Devicetree](usage-model.html)
...
to read. The script scripts/dtc/of\_unittest\_expect was created to filter
...
from ‘scripts/dtc/of\_unittest\_expect --help’.
...
When the kernel is built with CONFIG\_OF\_UNITTEST enabled, then the following make
rule:
...
Un-flattened device tree consists of connected device\_node(s) in form of a tree
...
Secondly, it calls [`of_fdt_unflatten_tree()`](kernel-api.html#c.of_fdt_unflatten_tree "of_fdt_unflatten_tree") to unflatten the flattened
...
If a duplicate node is found (i.e. if a node with same full\_name property is
...
Once the test case execution is complete, selftest\_data\_remove is called in
order to remove the device nodes attached initially (first the leaf nodes are
detached and then moving up the parent nodes are removed, and eventually the
whole tree). `selftest_data_remove()` calls `detach_node_and_children()` that uses
[`of_detach_node()`](kernel-api.html#c.of_detach_node "of_detach_node") to detach the nodes from the live device tree.

To detach a node, [`of_detach_node()`](kernel-api.html#c.of_detach_node "of_detach_node") either updates the child pointer of given
