...
> 출처(원문): https://docs.kernel.org/locking/robust-futex-ABI.html
...
Robust\_futexes provide a mechanism that is used in addition to normal
...
kernel intervention required for robust\_futexes above and beyond what is
required for futexes is:

> 1. a one time call, per thread, to tell the kernel where its list of
>    held robust\_futexes begins, and
...
threads in the kernel. Options on the sys\_futex(2) system call support
waiting on a particular futex, and waking up the next waiter on a
particular futex.

For robust\_futexes to work, the user code (typically in a library such
...
A thread that anticipates possibly using robust\_futexes should first
...
>
> The third word, called ‘list\_op\_pending’, contains transient copy of
...
robust\_futexes. The kernel will only be able to wakeup the next thread
...
robust\_futex support for exit cleanup of that lock, it should have one
...
anticipates using robust\_futexes, the kernel stores the passed in ‘head’
...
It is anticipated that threads will use robust\_futexes embedded in
larger, user level locking structures, one per lock. The kernel
robust\_futex mechanism doesn’t care what else is in that structure, so
long as the ‘offset’ to the ‘lock word’ is the same for all
robust\_futexes used by that thread. The thread should link those locks
...
essential service available for robust\_futexes, which is to help clean
...
only essential involvement in robust\_futexes is to remember where the
...
that thread’s robust\_futex linked lock list a given time.
...
> 1. set the ‘list\_op\_pending’ word to the address of the ‘lock entry’
>    to be inserted,
> 2. acquire the futex lock,
> 3. add the lock entry, with its thread id (TID) in the bottom 30 bits
>    of the ‘lock word’, to the linked list starting at ‘head’, and
> 4. clear the ‘list\_op\_pending’ word.
...
> 4. clear the ‘lock\_op\_pending’ word.

On exit, the kernel will consider the address stored in
‘list\_op\_pending’ and the address of each ‘lock word’ found by walking
