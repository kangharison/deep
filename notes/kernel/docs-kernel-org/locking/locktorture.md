...
> 출처(원문): https://docs.kernel.org/locking/locktorture.html
...
The CONFIG\_LOCK\_TORTURE\_TEST config option provides a kernel module
that runs torture tests on core kernel locking primitives. The kernel
module, ‘locktorture’, may be built after the fact on the running
kernel to be tested, if desired. The tests periodically output status
messages via [`printk()`](../core-api/printk-basics.html#c.printk "printk"), which can be examined via the dmesg (perhaps
...
:   Number of kernel threads that will stress shared lock
    ...
    locks. If the user did not specify nwriters\_stress, then
...
:   Type of lock to torture. By default, only spinlocks will
    ...
    >       rwlock pairs.
    > * “mutex\_lock”:
    >   :   [`mutex_lock()`](../kernel-hacking/locking.html#c.mutex_lock "mutex_lock") and [`mutex_unlock()`](../kernel-hacking/locking.html#c.mutex_unlock "mutex_unlock") pairs.
    > * “rtmutex\_lock”:
    >   :   `rtmutex_lock()` and `rtmutex_unlock()` pairs.
    >       Kernel must have CONFIG\_RT\_MUTEXES=y.
    > * “rwsem\_lock”:
...
:   The number of seconds between each attempt to execute a
    ...
    CONFIG\_HOTPLUG\_CPU=n kernels, locktorture will silently
    refuse to do any CPU-hotplug operations regardless of
    what value is specified for onoff\_interval.
...
:   The number of seconds to wait until starting CPU-hotplug
    ...
    CONFIG\_HOTPLUG\_CPU is enabled.

stat\_interval
:   Number of seconds between statistics-related [`printk()`](../core-api/printk-basics.html#c.printk "printk")s.
...
:   The number of seconds to keep the test threads affinitized
    ...
    Used in conjunction with test\_no\_idle\_hz.

verbose
:   Enable verbose debugging printing, via [`printk()`](../core-api/printk-basics.html#c.printk "printk"). Enabled
...
Also see: [RCU Torture Test Operation](../RCU/torture.html)
