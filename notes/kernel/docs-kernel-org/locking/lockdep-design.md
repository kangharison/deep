...
> 출처(원문): https://docs.kernel.org/locking/lockdep-design.html
...
where the n STATEs are coded in kernel/locking/lockdep\_states.h and as of
now they include:
...
could interrupt \_any\_ of the irq-unsafe or hardirq-unsafe locks, which
...
must be held: lockdep\_assert\_held\*(&lock) and lockdep\_\*pin\_lock(&lock).

As the name suggests, lockdep\_assert\_held\* family of macros assert that a
...
The other family of macros is lockdep\_\*`pin_lock()`, which is admittedly only
...
The validator tracks a maximum of MAX\_LOCKDEP\_KEYS number of lock classes.
...
By default, MAX\_LOCKDEP\_KEYS is currently set to 8191, and typical
...
2. Using structures such as arrays that have large numbers of
   ...
   spinlock\_t will consume 8192 lock classes -unless- each spinlock
...
`down_read()`) and recursive readers (recursive shared lockers, like [`rcu_read_lock()`](../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock")).
