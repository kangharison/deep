...
> 출처(원문): https://docs.kernel.org/translations/it_IT/process/adding-syscalls.html
...
Original:
:   [Documentation/process/adding-syscalls.rst](../../../process/adding-syscalls.html#addsyscalls)
...
[Documentation/translations/it\_IT/process/submitting-patches.rst](submitting-patches.html#it-submittingpatches).
...
>   >   *[read(2)](https://man7.org/linux/man-pages/man2/read.2.html)*/*[write(2)](https://man7.org/linux/man-pages/man2/write.2.html)* dovrebbero essere implementate
>   >   come chiamate *[ioctl(2)](https://man7.org/linux/man-pages/man2/ioctl.2.html)*, il che potrebbe portare ad un’API in
...
>   potrebbe essere appropriata l’aggiunta di un comando *[fcntl(2)](https://man7.org/linux/man-pages/man2/fcntl.2.html)*.
>   Tuttavia, *[fcntl(2)](https://man7.org/linux/man-pages/man2/fcntl.2.html)* è una chiamata di sistema multiplatrice che
>   nasconde una notevole complessità, quindi è ottima solo quando la nuova
>   funzione assomiglia a quelle già esistenti in *[fcntl(2)](https://man7.org/linux/man-pages/man2/fcntl.2.html)*, oppure
...
>   potrebbe essere appropriata l’aggiunta di un comando *[prctl(2)](https://man7.org/linux/man-pages/man2/prctl.2.html)*.
>   Come per *[fcntl(2)](https://man7.org/linux/man-pages/man2/fcntl.2.html)*, questa chiamata di sistema è un complesso
...
Vedere *[perf\_event\_open(2)](https://man7.org/linux/man-pages/man2/perf_event_open.2.html)* e la funzione `perf_copy_attr()` (in
...
della famiglia di *[poll(2)](https://man7.org/linux/man-pages/man2/poll.2.html)*. Rendere un descrittore di file pronto
...
man *[openat(2)](https://man7.org/linux/man-pages/man2/openat.2.html)*; per un esempio di AT\_EMPTY\_PATH, leggere la pagina
man *[fstatat(2)](https://man7.org/linux/man-pages/man2/fstatat.2.html)*).
...
*[capabilities(7)](https://man7.org/linux/man-pages/man7/capabilities.7.html)*. Scegliete un bit di privilegio già esistente per
...
>   solitamente x86 (ovvero tutti: x86\_64, x86\_32 e x32).
...
Aggiornate anche il contatore \_\_NR\_syscalls di modo che sia coerente con
...
dovete aggiungere un elemento *common* (per x86\_64 e x32) in
...
*[splice(2)](https://man7.org/linux/man-pages/man2/splice.2.html)* l’argomento di tipo `loff_t __user *` non necessita
...
`COMPAT_SYSCALL_DEFINEn()` (simile a SYSCALL\_DEFINEn). Questa versione
...
arch/x86/entry/syscalls/syscall\_64.tbl rimane immutata).
...
Per l’architettura x86\_64, questo è implementato come un punto d’accesso
...
se l’implementazione dell’ABI x32 è diversa da quella x86\_64, allora la sua
...
farà riferimento a stub\_xyzzy, ma UML non include l’implementazione
...
funzioni quando viene compilato per x86\_64 (-m64), x86\_32 (-m32) e x32 (-mx32).
...
> * Articolo di Michael Kerris su LWN sull’evitare nuovi usi di CAP\_SYS\_ADMIN:
...
>   le estensioni *[prctl(2)](https://man7.org/linux/man-pages/man2/prctl.2.html)*: <https://lore.kernel.org/r/CAHO5Pa3F2MjfTtfNxa8LbnkeeU8=YJ+9tDqxZpw7Gz59E-4AUg@mail.gmail.com>
...
>
>   > + [commit 75069f2b5bfb](https://git.kernel.org/torvalds/c/75069f2b5bfb) (“vfs: renumber FMODE\_NONOTIFY and add to uniqueness
>   >   check”)
>   > + [commit 12ed2e36c98a](https://git.kernel.org/torvalds/c/12ed2e36c98a) (“fanotify: FMODE\_NONOTIFY and \_\_O\_SYNC in sparc
>   >   conflict”)
>   > + [commit bb458c644a59](https://git.kernel.org/torvalds/c/bb458c644a59) (“Safer ABI for O\_TMPFILE”)
