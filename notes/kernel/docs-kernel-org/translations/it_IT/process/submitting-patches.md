...
> 출처(원문): https://docs.kernel.org/translations/it_IT/process/submitting-patches.html
...
[Una guida al processo di sviluppo del Kernel](development-process.html). Leggete anche
[Lista delle verifiche da fare prima di inviare una patch per il kernel Linux](submit-checklist.html) per una lista di
...
[Documentation/translations/it\_IT/process/maintainer-handbooks.rst](maintainer-handbooks.html#it-maintainer-handbooks-main)
...
`git`, come un “commit log”. Leggete [Il formato canonico delle patch](#it-the-canonical-patch-format).
...
divisa. Leggete [Separate le vostre modifiche](#it-split-changes).
...
dettagli sono disponibili in [Stile del codice per il kernel Linux](coding-style.html).
...
codice. Lo script scripts/get\_maintainer.pl può esservi d’aiuto (passategli il
...
Leggete [Informazioni sui programmi di posta elettronica per Linux](email-clients.html)
...
[Il formato canonico delle patch](#it-the-canonical-patch-format)). Aggiungete a CC tutte le persone che
vi hanno fornito dei commenti per notificarle di eventuali nuove versioni.

Leggete [Informazioni sui programmi di posta elettronica per Linux](email-clients.html) per
...
patch. Per maggiori dettagli leggete [Descrivete le vostre modifiche](#it-describe-changes)
...
includere proprio \_tutto\_ quello che è uscito dal compilatore;
...
Kernel [Stile del codice per il kernel Linux](coding-style.html).
