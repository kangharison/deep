# Le operazioni RCU per le verifichetorture

> 출처(원문): https://docs.kernel.org/translations/it_IT/RCU/torture.html
...
L’opzione CONFIG\_RCU\_TORTURE\_TEST è disponibile per tutte le implementazione di
RCU. L’opzione creerà un modulo rcutorture che potrete caricare per avviare le
verifiche. La verifica userà [`printk()`](../../../core-api/printk-basics.html#c.printk "printk") per riportare lo stato, dunque potrete
...
usare altri comandi per visualizzare i messaggi di [`printk()`](../../../core-api/printk-basics.html#c.printk "printk"). La funzione
[`printk()`](../../../core-api/printk-basics.html#c.printk "printk") usa KERN\_ALERT, dunque i messaggi dovrebbero essere ben visibili. ;-)
...
* “rtmbe”: Un valore diverso da zero indica che rcutorture crede che
  [`rcu_assign_pointer()`](../../../core-api/kernel-api.html#c.rcu_assign_pointer "rcu_assign_pointer") e [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") non funzionino correttamente. Il
  valore dovrebbe essere zero.
* “rtbe”: un valore diverso da zero indica che le funzioni della famiglia
  [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") non funzionano correttamente.
...
* “rtb”: Il numero di volte che rcutorture ha provato a forzare l’inversione di
  ...
  promozione della priorità col parametro “test\_bootst”.
...
* “Reader Pipe”: un istogramma dell’età delle strutture viste dai lettori. RCU
  ...
  passata per (RCU\_TORTURE\_PIPE\_LEN-2) periodi di grazia.
...
*Tree SRCU*, usando un’allocazione dinamica di srcu\_struct (dunque “srcud-”
...
dev’essere compilato con CONFIG\_RCU\_TORTURE\_TEST=m, cosicché le verifiche possano
...
“SUCCESS” (successo), “FAILURE” (fallimento), o “RCU\_HOTPLUG”. I primi due sono
