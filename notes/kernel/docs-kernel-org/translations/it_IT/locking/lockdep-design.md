...
> 출처(원문): https://docs.kernel.org/translations/it_IT/locking/lockdep-design.html
...
gli n STATI sono codificati in kernel/locking/lockdep\_states.h, ad oggi
includono:
...
devono essere trattenuti: lockdep\_assert\_held\*(&lock) e
lockdep\_\*pin\_lock(&lock).

Come suggerito dal nome, la famiglia di macro lockdep\_assert\_held\* asseriscono
...
L’altra famiglia di macro è lockdep\_\*`pin_lock()`, che a dire il vero viene usata
...
una qualsiasi classe di blocco. [[1]](#id2)
...
MAX\_LOCKDEP\_KEYS. Oltrepassare questo limite indurrà lokdep a generare il
seguente avviso:
...
2. Usare array con un gran numero di blocchi che non vengono esplicitamente
   ...
   il proprio spinlock\_t consumerà 8192 classi di blocco a meno che non vengano
...
[`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock")). D’ora in poi, per questi tipi di bloccatori, useremo la
seguente notazione:
