...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/power/energy-model.html
...
Original:
:   [Energy Model of devices](../../../power/energy-model.html)
...
[Energy Aware Scheduling](../../../scheduler/sched-energy.html)。对于一些子系统，比如热能或
powercap，用“抽象刻度”描述功率值可能会导致问题。这些子系统对过去使用的功率的
估算值更感兴趣，因此可能需要真实的微瓦。这些要求的一个例子可以在智能功率分配
[Power allocator governor tunables](../../../driver-api/thermal/power_allocator.html)文档中找到。
...
必须使能CONFIG\_ENERGY\_MODEL才能使用EM框架。
...
框架会在注册过程中正确处理这样的平台。这种平台会被设置EM\_PERF\_DOMAIN\_ARTIFICIAL
...
有两个API函数提供对能量模型的访问。[`em_cpu_get()`](../../../power/energy-model.html#c.em_cpu_get "em_cpu_get")以CPU id为参数，[`em_pd_get()`](../../../power/energy-model.html#c.em_pd_get "em_pd_get")
以设备指针为参数。使用哪个接口取决于子系统，但对于CPU设备来说，这两个函数都返
回相同的性能域。

对CPU的能量模型感兴趣的子系统可以通过[`em_cpu_get()`](../../../power/energy-model.html#c.em_cpu_get "em_cpu_get") API检索它。在创建性能域时
分配一次能量模型表，它保存在内存中不被修改。

一个性能域所消耗的能量可以使用[`em_cpu_energy()`](../../../power/energy-model.html#c.em_cpu_energy "em_cpu_energy") API来估算。该估算假定CPU设备
...
参见 include/linux/energy\_model.h 和 kernel/power/energy\_model.c 的kernel doc。

## 3. 驱动示例

CPUFreq框架支持专用的回调函数，用于为指定的CPU（们）注册EM：
cpufreq\_driver::`register_em()`。这个回调必须为每个特定的驱动程序正确实现，
