...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/mm/numa.html
...
可以作为一种内存资源管理机制。[见 [CPUSETS](../../../admin-guide/cgroup-v1/cpusets.html)]

对于每个有内存的节点，Linux构建了一个独立的内存管理子系统，有自己的空闲页列表、使用中页列表、
使用统计和锁来调解访问。此外，Linux为每个内存区[DMA、DMA32、NORMAL、HIGH\_MEMORY、MOVABLE
...
中[见 [Scheduler Domains](../../../scheduler/sched-domains.html)]——并且调度器试图尽量减少任务迁移到遥
...
序接口，如sched\_setaffinity(2)，来限制任务的迁移，以改善NUMA定位。此外，人们可以使用
...
的内存。 [见 [CPUSETS](../../../admin-guide/cgroup-v1/cpusets.html)]
