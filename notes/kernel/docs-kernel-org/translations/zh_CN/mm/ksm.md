...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/mm/ksm.html
...
Original:
:   [Kernel Samepage Merging](../../../mm/ksm.html)
...
KSM 是一种节省内存的数据去重功能，由CONFIG\_KSM=y启用，并在2.6.32版本时被添加
到Linux内核。详见 `mm/ksm.c` 的实现，以及http://lwn.net/Articles/306704和
<https://lwn.net/Articles/330589>

KSM的用户空间的接口在Documentation/translations/zh\_CN/admin-guide/mm/ksm.rst
文档中有描述。
...
KSM页的稳定树其中的节点指向了一个ksm\_rmap\_item结构体类型的列表。同时，这个KSM页
...
rmap链表变得过大。rmap的遍历具有O(N)的复杂度，其中N是共享页面的rmap\_项（即
...
内核代码请见mm/ksm.c。
涉及的函数(mm\_slot ksm\_scan stable\_node rmap\_item)。
