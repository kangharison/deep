...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/mm/hmm.html
...
Original:
:   [Heterogeneous Memory Management (HMM)](../../../mm/hmm.html)
...
HMM 提供的第二种机制是一种新的 ZONE\_DEVICE 内存，它允许为设备内存的每个页面分配一个
...
首先为设备内存热插拔新的 ZONE\_DEVICE 内存，然后执行迁移。迁移内容和时间的策略决定留
给设备驱动程序。
...
保持两者同步。想要镜像进程地址空间的设备驱动程序必须从注册 mmu\_interval\_notifier
开始:
...
## [利用 default\_flags 和 pfn\_flags\_mask](#id10)

hmm\_range 结构有 2 个字段，default\_flags 和 pfn\_flags\_mask，它们指定整个范围
...
== range->start + (index\_of\_write << PAGE\_SHIFT) 它将异常写入权限，即，如果
CPU pte 没有设置写权限，那么HMM将调用`handle_mm_fault()`。

hmm\_range\_fault 完成后，标志位被设置为页表的当前状态，即 HMM\_PFN\_VALID | 如果页
面可写，将设置 HMM\_PFN\_WRITE。

## [从核心内核的角度表示和管理设备内存](#id11)
...
储指令来迁移数据。[`migrate_vma_setup()`](../../../core-api/mm-api.html#c.migrate_vma_setup "migrate_vma_setup")、[`migrate_vma_pages()`](../../../core-api/mm-api.html#c.migrate_vma_pages "migrate_vma_pages") 和
[`migrate_vma_finalize()`](../../../core-api/mm-api.html#c.migrate_vma_finalize "migrate_vma_finalize") 函数旨在使驱动程序更易于编写并集中跨驱动程序的通用代码。
...
还有[`devm_request_free_mem_region()`](../../../core-api/kernel-api.html#c.devm_request_free_mem_region "devm_request_free_mem_region"), [`devm_memremap_pages()`](../../../core-api/mm-api.html#c.devm_memremap_pages "devm_memremap_pages"),
`devm_memunmap_pages()` 和 `devm_release_mem_region()` 当资源可以绑定到 `struct device`.

整体迁移步骤类似于在系统内存中迁移 NUMA 页面(see [Page migration](../../../mm/page_migration.html)) ，
但这些步骤分为设备驱动程序特定代码和共享公共代码:

1. `mmap_read_lock()`

   设备驱动程序必须将 `struct vm_area_struct` 传递给[`migrate_vma_setup()`](../../../core-api/mm-api.html#c.migrate_vma_setup "migrate_vma_setup")，
   因此需要在迁移期间保留 `mmap_read_lock()` 或 `mmap_write_lock()`。
2. `migrate_vma_setup(struct migrate_vma *args)`

   设备驱动初始化了 `struct migrate_vma` 的字段，并将该指针传递给
   [`migrate_vma_setup()`](../../../core-api/mm-api.html#c.migrate_vma_setup "migrate_vma_setup")。`args->flags` 字段是用来过滤哪些源页面应该被迁移。
   ...
   [`migrate_vma_setup()`](../../../core-api/mm-api.html#c.migrate_vma_setup "migrate_vma_setup")所做的第一步是用 `mmu_notifier_invalidate_range_start()`
   ...
   其 `event` 字段设置为MMU\_NOTIFY\_MIGRATE， `owner` 字段设置为传递给
   [`migrate_vma_setup()`](../../../core-api/mm-api.html#c.migrate_vma_setup "migrate_vma_setup")的 `args->pgmap_owner` 字段。这允许设备驱动跳过无
