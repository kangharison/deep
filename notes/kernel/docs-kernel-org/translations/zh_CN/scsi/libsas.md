...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/scsi/libsas.html
...
sas\_ha\_struct 结构体用于向 SAS 层描述一个 SAS LLDD。该结构的
...
以启动实际的 OOB（带外）过程；此时驱动将开始调用 notify\_\* 系
列事件回调函数。
...
随后，在主机适配器（HA）的结构体中，所有的 PHY 通常以 my\_phy
数组的形式存在（如下文所示）。

在初始化各个 PHY 时，除了初始化驱动自定义的 PHY 结构体外，还
需要同时初始化其中的 sas\_phy 结构体。
...
:   * 当接收到 IDENTIFY 或 FIS 帧时，将该帧复制到此处。正确的
      操作流程是获取锁 → 复制数据 → 设置 frame\_rcvd\_size → 释
...
port
:   * 如果该 PHY 属于某个端口（port），此字段指向对应的 sas\_port
...
:   * LLDD 应将此字段设置为指向自身定义的 PHY 结构体，这样当 SAS
      层调用某个回调并传入 sas\_phy 时，驱动可以快速定位自身的 PHY
      结构体。如果 sas\_phy 是嵌入式成员，也可以使用 [`container_of()`](../../../driver-api/basics.html#c.container_of "container_of")
      宏进行访问——两种方式均可。
...
phy\_mask 为 32 位，目前这一长度已足够使用，因为尚未听说有主机适配
器拥有超过8 个 PHY。
...
sas\_phy
:   * 指向结构体的指针数组（参见上文关于 sas\_addr 的说明）。
      这些指针必须设置。更多细节见下文说明。

num\_phys
:   * 表示 sas\_phy 数组中 PHY 的数量，同时也表示 sas\_port
      数组中的端口数量。一个端口最多对应一个 PHY，因此最大端口数
      等于 num\_phys。因此，结构中不再单独使用 num\_ports 字段，
      而仅使用 num\_phys。
...
lldd\_ha
:   * 应设置为指向驱动的主机适配器（HA）结构体的指针。如果 sas\_ha\_struct
      被嵌入到更大的结构体中，也可以通过 [`container_of()`](../../../driver-api/basics.html#c.container_of "container_of") 宏来获取。
...
2. SAS 1.1 未定义 I\_T Nexus Reset TMF（任务管理功能）。
...
:   * 定时器超时、信号丢失、丢失 DWS 等情况。 [[1]](#id7)

PORTE\_TIMER\_EVENT
:   * DWS 复位超时定时器到期时。[[1]](#id7)
...
PHYE\_OOB\_DONE
:   * OOB 过程成功完成，oob\_mode 有效。

PHYE\_OOB\_ERROR
:   * 执行 OOB 过程中出现错误，设备可能已断开。 [[1]](#id7)

PHYE\_SPINUP\_HOLD
:   * 检测到 SATA 设备，但未发送 COMWAKE 信号。

[1]
([1](#id3),[2](#id4),[3](#id5),[4](#id6))

应设置或清除 phy 中相应的字段，或者从 tasklet 中调用
...
用于将任务排队提交给 SAS LLDD，@task 为要执行的任务，@gfp\_mask
为定义调用者上下文的 gfp 掩码。
...
结构体 domain\_device 描述了 SAS 域中的任意设备。它完全由 SAS
层管理。一个任务会指向某个域设备，SAS LLDD 就是通过这种方式知
道任务应发送到何处。SAS LLDD 只读取 domain\_device 结构的内容，
但不会创建或销毁它。

## 用户空间中的扩展器管理

在 sysfs 中的每个扩展器目录下，都有一个名为 “smp\_portal” 的
...
整个过程在 “expander\_conf.c” 文件中的函数 `do_smp_func()`
及其调用者中有详细展示。

对应的内核实现位于 “sas\_expander.c” 文件中。

程序 “expander\_conf.c” 实现了上述逻辑。它接收一个参数——扩展器
