...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/PCI/sysfs-pci.html
...
Original:
:   [Accessing PCI device resources through sysfs](../../../PCI/sysfs-pci.html)
...
> | irq | IRQ编号 (ascii, ro) |
> | local\_cpus | 临近CPU掩码(cpumask, ro) |
> | remove | 从内核的列表中删除设备 (ascii, wo) |
> | resource | PCI资源主机地址 (ascii, ro) |
> | resource0..N | PCI资源N，如果存在的话 (binary, mmap, rw[[1]](#id2)) |
> | resource0\_wc..N\_wc | PCI WC映射资源N，如果可预取的话 (binary, mmap) |
> | revision | PCI修订版 (ascii, ro) |
> | rom | PCI ROM资源，如果存在的话 (binary, ro) |
> | subsystem\_device | PCI子系统设备 (ascii, ro) |
> | subsystem\_vendor | PCI子系统供应商 (ascii, ro) |
...
rw 仅适用于 IORESOURCE\_IO（I/O 端口）区域
...
legacy\_io文件是一个读/写文件，可以被应用程序用来做传统的端口I/O。应用程序应该打开该文件，寻
找所需的端口（例如0x3e8），并进行1、2或4字节的读或写。legacy\_mem文件应该被mmapped，其偏移
量与所需的内存偏移量相对应，例如0xa0000用于VGA帧缓冲器。然后，应用程序可以简单地解除引用返回
的指针（当然是在检查了错误之后）来访问遗留内存空间。

## 5.2. 支持新平台上的PCI访问

为了支持上述的PCI资源映射，Linux平台代码最好定义ARCH\_GENERIC\_PCI\_MMAP\_RESOURCE并使用该
功能的通用实现。为了支持通过/proc/bus/pci中的文件实现mmapmmap()的历史接口，平台也可以设置
HAVE\_PCI\_MMAP。

另外，设置了 HAVE\_PCI\_MMAP 的平台可以提供他们自己的 `pci_mmap_page_range()` 实现，而不是定
义 ARCH\_GENERIC\_PCI\_MMAP\_RESOURCE。

支持PCI资源的写组合映射的平台必须定义`arch_can_pci_mmap_wc()`，当写组合被允许时，在运行时应
评估为非零。支持I/O资源映射的平台同样定义`arch_can_pci_mmap_io()`。

遗留资源由HAVE\_PCI\_LEGACY定义保护。希望支持遗留功能的平台应该定义它并提供 pci\_legacy\_read,
pci\_legacy\_write 和 pci\_mmap\_legacy\_page\_range 函数。
