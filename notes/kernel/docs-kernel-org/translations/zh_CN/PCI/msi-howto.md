...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/PCI/msi-howto.html
...
Original:
:   [The MSI Driver Guide HOWTO](../../../PCI/msi-howto.html)
...
为了支持MSI或MSI-X，内核在构建时必须启用CONFIG\_PCI\_MSI选项。这个选项只在某些架
构上可用，而且它可能取决于其他一些选项的设置。例如，在x86上，你必须同时启用X86\_UP\_APIC
或SMP，才能看到CONFIG\_PCI\_MSI选项。
...
它为一个PCI设备分配最多至max\_vecs的中断向量。它返回分配的向量数量或一个负的错误。
如果设备对最小数量的向量有要求，驱动程序可以传递一个min\_vecs参数，设置为这个限制，
如果PCI核不能满足最小数量的向量，将返回-ENOSPC。

flags参数用来指定设备和驱动程序可以使用哪种类型的中断（PCI\_IRQ\_INTX, PCI\_IRQ\_MSI,
PCI\_IRQ\_MSIX）。一个方便的短语（PCI\_IRQ\_ALL\_TYPES）也可以用来要求任何可能的中断类型。
如果PCI\_IRQ\_AFFINITY标志被设置，[`pci_alloc_irq_vectors()`](../../../PCI/msi-howto.html#c.pci_alloc_irq_vectors "pci_alloc_irq_vectors")将把中断分散到可用的CPU上。

要获得传递给`require_irq()`和[`free_irq()`](../../../core-api/genericirq.html#c.free_irq "free_irq")的Linux IRQ号码和向量，请使用以下函数:
...
数量作为“min\_vecs“和“max\_vecs“参数传递给[`pci_alloc_irq_vectors()`](../../../PCI/msi-howto.html#c.pci_alloc_irq_vectors "pci_alloc_irq_vectors")函数。:
...
此外，还有一些API来提供支持的MSI或MSI-X向量的数量：[`pci_msi_vec_count()`](../../../driver-api/pci/pci.html#c.pci_msi_vec_count "pci_msi_vec_count")和
[`pci_msix_vec_count()`](../../../PCI/msi-howto.html#c.pci_msix_vec_count "pci_msix_vec_count")。一般来说，应该避免使用这些方法，而是让[`pci_alloc_irq_vectors()`](../../../PCI/msi-howto.html#c.pci_alloc_irq_vectors "pci_alloc_irq_vectors")
...
将告诉你MSI是否被启用（1）或禁用（0）。如果在任何属于PCI根和设备之间的桥的msi\_bus
文件中发现0，说明MSI被禁用。

也需要检查设备驱动程序，看它是否支持MSI。例如，它可能包含对带有PCI\_IRQ\_MSI或
PCI\_IRQ\_MSIX标志的pci\_alloc\_irq\_vectors（）的调用。
