...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/dev-tools/kasan.html
...
Original:
:   [Kernel Address Sanitizer (KASAN)](../../../dev-tools/kasan.html)
...
用CONFIG\_KASAN\_GENERIC启用的通用KASAN，是用于调试的模式，类似于用户空
间的ASan。这种模式在许多CPU架构上都被支持，但它有明显的性能和内存开销。

基于软件标签的KASAN或SW\_TAGS KASAN，通过CONFIG\_KASAN\_SW\_TAGS启用，
可以用于调试和自我测试，类似于用户空间HWASan。这种模式只支持arm64，但其
适度的内存开销允许在内存受限的设备上用真实的工作负载进行测试。

基于硬件标签的KASAN或HW\_TAGS KASAN，用CONFIG\_KASAN\_HW\_TAGS启用，被
...
在x86\_64、arm、arm64、powerpc、riscv、s390、xtensa和loongarch上支持通用KASAN，
...
通用KASAN支持在所有的slab、page\_alloc、vmap、vmalloc、堆栈和全局内存
中查找错误。

基于软件标签的KASAN支持slab、page\_alloc、vmalloc和堆栈内存。

基于硬件标签的KASAN支持slab、page\_alloc和不可执行的vmalloc内存。
...
* `kasan.mode=sync`, `=async` or `=asymm` 控制KASAN是否
  ...
  件中（对于arm64来说是在TFSR\_EL1寄存器中）。内核周期性地检查硬件，并且只在这些检查中报告标签异常。
...
出现在错误访问堆栈跟踪中的位置。目前，这包括 [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") 和排队的工作队列。

### CONFIG\_KASAN\_EXTRA\_INFO

启用 CONFIG\_KASAN\_EXTRA\_INFO 选项允许 KASAN 记录和报告更多信息。目前支持的
额外信息包括分配和释放时的 CPU 编号和时间戳。更多的信息可以帮助找到内核错误的原因，
并将错误与其他系统事件关联起来，但代价是用额外的内存来记录更多信息（有关更多
开销的细节，请参见 CONFIG\_KASAN\_EXTRA\_INFO 选项的帮助文本）。

以下为 CONFIG\_KASAN\_EXTRA\_INFO 开启后的报告（仅显示不同部分）:
...
通用KASAN将1/8的内核内存专用于其影子内存（16TB以覆盖x86\_64上的128TB），并使用
...
请注意，启用CONFIG\_KASAN\_HW\_TAGS始终会导致启用内核中的TBI。即使提供了
