...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/dev-tools/kcsan.html
...
Original:
:   [Kernel Concurrency Sanitizer (KCSAN)](../../../dev-tools/kcsan.html)
...
点的采样方法来检测竞争。KCSAN 的主要目的是检测 [数据竞争](#id5)。
...
* 使用函数属性 `__no_kcsan` 可以对整个函数禁用数据竞争检测:
  ...
  为了动态限制该为哪些函数生成报告，查阅 [Debug 文件系统接口](#debug) 黑名单/白名单特性。
...
宏定义可以结合原文（[Kernel Concurrency Sanitizer (KCSAN)](../../../dev-tools/kcsan.html)）阅读。
...
后发生顺序，这可以用来确定 [数据竞争](#id5) 中定义的数据竞争。
