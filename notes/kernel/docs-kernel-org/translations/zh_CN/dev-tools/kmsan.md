...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/dev-tools/kmsan.html
...
Original:
:   [Kernel Memory Sanitizer (KMSAN)](../../../dev-tools/kmsan.html)
...
将其应用到文件或目录中的每个函数。大多数用户不会需要 KMSAN\_SANITIZE，
...
每个 task\_struct 都有一个关联的 KMSAN 任务状态，它保存 KMSAN
