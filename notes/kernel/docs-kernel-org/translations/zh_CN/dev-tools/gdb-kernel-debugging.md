...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/dev-tools/gdb-kernel-debugging.html
...
Original:
:   [Debugging kernel and modules via gdb](../../../process/debugging/gdb-kernel-debugging.html)
...
* 创建一个QEMU/KVM的linux虚拟机（详情请参考 www.linux-kvm.org 和 www.qemu.org ）。
  ...
  可以帮助搭建交叉开发环境。
* 编译内核时开启CONFIG\_GDB\_SCRIPTS，关闭CONFIG\_DEBUG\_INFO\_REDUCED。
  如果架构支持CONFIG\_FRAME\_POINTER，请保持开启。
* 在guest环境上安装该内核。如有必要，通过在内核command line中添加“nokaslr”来关闭KASLR。
  ...
  在这种情况下，如果架构支持KASLR，应该在禁用CONFIG\_RANDOMIZE\_BASE的情况下构建内核。
