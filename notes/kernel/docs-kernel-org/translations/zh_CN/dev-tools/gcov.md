...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/dev-tools/gcov.html
...
Original:
:   [Using gcov with the Linux kernel](../../../dev-tools/gcov.html)
...
如果要在整个内核的覆盖率统计（开启CONFIG\_GCOV\_PROFILE\_ALL）中单独排除
某一个文件或者路径，可以使用如下的方法:
...
可以通过对内核参数gcov\_persist的修改来停用gcov对模块的备份机制:
...
问题原因
:   由于seq\_file的工作方式，某些工具（例如cp或tar）可能无法正确地从
    sysfs复制文件。
...
## 附录A：collect\_on\_build.sh

用于在编译机上收集覆盖率元文件的示例脚本
（见 [编译机和测试机分离 a.](#gcov-test-zh-cn) ）
...
## 附录B：collect\_on\_test.sh

用于在测试机上收集覆盖率数据文件的示例脚本
（见 [编译机和测试机分离 b.](#gcov-build-zh-cn) ）
