...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/dev-tools/testing-overview.html
...
存在些许差异，因此建议读者结合原文
（[Kernel Testing Guide](../../../dev-tools/testing-overview.html)）辅助阅读。
如对部分翻译有异议或有更好的翻译意见，欢迎联系译者进行修订。

KUnit（[KUnit - Linux Kernel Unit Testing](../../../dev-tools/kunit/index.html)）是用于“白箱”测
...
[Test Style and Nomenclature](../../../dev-tools/kunit/style.html)

kselftest（[Linux Kernel Selftests](../../../dev-tools/kselftest.html)），相对来说，大量用
...
[在Linux内核里使用gcov做代码覆盖率检查](gcov.html) 是GCC的覆盖率测试
工具，能用于获取内核的全局或每个模块的覆盖率。与KCOV不同的是，这个工具
不记录每个任务的覆盖率。覆盖率数据可以通过debugfs读取，并通过常规的
gcov工具进行解释。

[KCOV: code coverage for fuzzing](../../../dev-tools/kcov.html) 是能够构建在内核之中，用于在每个任务
...
* UBSAN检测C标准中未定义的行为，如整型溢出。参阅
  [Undefined Behavior Sanitizer - UBSAN](../../../dev-tools/ubsan.html)
* KCSAN检测数据竞争。参阅 [Kernel Concurrency Sanitizer (KCSAN)](../../../dev-tools/kcsan.html)
* KFENCE是一个低开销的内存问题检测器，比KASAN更快且能被用于批量构建。
  参阅 [Kernel Electric-Fence (KFENCE)](../../../dev-tools/kfence.html)
...
* 运行时确认（Runtime Verification）支持检查给定子系统的特定行为。参阅
  [Runtime Verification](../../../trace/rv/runtime-verification.html)。
