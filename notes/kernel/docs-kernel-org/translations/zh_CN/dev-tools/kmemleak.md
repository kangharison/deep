...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/dev-tools/kmemleak.html
...
Original:
:   [Kernel Memory Leak Detector](../../../dev-tools/kmemleak.html)
...
“Kernel hacking” 中的 CONFIG\_DEBUG\_KMEMLEAK 必须被启用。一个内核线程每10分钟
...
冲区中。缓冲区的大小通过 CONFIG\_DEBUG\_KMEMLEAK\_MEM\_POOL\_SIZE 选项配置。

如果 CONFIG\_DEBUG\_KMEMLEAK\_DEFAULT\_OFF 被启用，则 kmemleak 默认被禁用。在内核命
令行中传递 `kmemleak=on` 参数来开启这个功能。

如果出现 “Error while writing to stdout” 或 “write\_loop: Invalid argument” 这样
的错误，请确认 kmemleak 被正确启用。

## 基础算法

通过 [`kmalloc()`](../../../core-api/mm-api.html#c.kmalloc "kmalloc"), [`vmalloc()`](../../../core-api/mm-api.html#c.vmalloc "vmalloc"), [`kmem_cache_alloc()`](../../../core-api/mm-api.html#c.kmem_cache_alloc "kmem_cache_alloc") 以及同类
...
* `kmemleak_vmalloc` - 通知一个使用 [`vmalloc()`](../../../core-api/mm-api.html#c.vmalloc "vmalloc") 的内存分配
...
* `kmemleak_alloc_recursive` - 和 kmemleak\_alloc 效果相同但会检查是否有递归的
  :   内存分配
* `kmemleak_free_recursive` - 和 kmemleak\_free 效果相同但会检查是否有递归的
  :   内存释放
...
泄露（孤立对象）。为了减少假阴性的出现次数，kmemleak 提供了 kmemleak\_ignore，
kmemleak\_scan\_area，kmemleak\_no\_scan 和 kmemleak\_erase 函数（见上）。
任务栈也会增加假阴性的数量并且默认不开启对它们的扫描。

假阳性是对象被误报为内存泄露（孤立对象）。对于已知未泄露的对象，kmemleak
提供了 kmemleak\_not\_leak 函数。同时 kmemleak\_ignore 可以用于标记已知不包含任何
其他指针的内存块，标记后该内存块不会再被扫描。

一些被报告的泄露仅仅是暂时的，尤其是在 SMP（对称多处理）系统中，因为其指针
暂存在 CPU 寄存器或栈中。Kmemleak 定义了 MSECS\_MIN\_AGE（默认值为 1000）
...
（某些 init\_call 函数中），指针的计算是通过其他方法而不是常规的 container\_of 宏
...
kmemleak-test。设置 CONFIG\_SAMPLE\_KMEMLEAK 为模块（不能作为内建模块使用）
