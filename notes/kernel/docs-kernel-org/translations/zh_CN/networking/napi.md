...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/networking/napi.html
...
NAPI 是 Linux 网络堆栈中使用的事件处理机制。NAPI 的名称现在不再代表任何特定含义 [[1]](#id13)。

在基本操作中，设备通过中断通知主机有新事件发生。主机随后调度 NAPI 实例来处理这些事件。
该设备也可以通过 NAPI 进行事件轮询，而无需先接收中断信号（[忙轮询](#poll-zh-cn)）。

NAPI 处理通常发生在软中断上下文中，但有一个选项，可以使用 [单独的内核线程](#threaded-zh-cn)
来进行 NAPI 处理。
...
[`netif_napi_add()`](../../../networking/kapi.html#c.netif_napi_add "netif_napi_add") 和 [`netif_napi_del()`](../../../networking/kapi.html#c.netif_napi_del "netif_napi_del") 用于向系统中添加/删除一个 NAPI 实例。实例会被
附加到作为参数传递的 netdevice上（并在 netdevice 注销时自动删除）。实例在添加时处于禁
用状态。

[`napi_enable()`](../../../networking/kapi.html#c.napi_enable "napi_enable") 和 [`napi_disable()`](../../../networking/kapi.html#c.napi_disable "napi_disable") 管理禁用状态。禁用的 NAPI 不会被调度，并且保证其
poll 方法不会被调用。[`napi_disable()`](../../../networking/kapi.html#c.napi_disable "napi_disable") 会等待 NAPI 实例的所有权被释放。

这些控制 API 并非幂等的。控制 API 调用在面对数据路径 API 的并发使用时是安全的，但控制
API 调用顺序错误可能会导致系统崩溃、死锁或竞态条件。例如，连续多次调用 [`napi_disable()`](../../../networking/kapi.html#c.napi_disable "napi_disable")
会造成死锁。

### 数据路径API

[`napi_schedule()`](../../../networking/kapi.html#c.napi_schedule "napi_schedule") 是调度 NAPI 轮询的基本方法。驱动程序应在其中断处理程序中调用此函数
（更多信息请参见 [调度与IRQ屏蔽](#drv-sched-zh-cn)）。成功的 [`napi_schedule()`](../../../networking/kapi.html#c.napi_schedule "napi_schedule") 调用将获得 NAPI 实例
的所有权。
...
如果事件处理已完成（所有未处理的数据包都已处理完毕），轮询方法在返回之前应调用 [`napi_complete_done()`](../../../networking/kapi.html#c.napi_complete_done "napi_complete_done")。
[`napi_complete_done()`](../../../networking/kapi.html#c.napi_complete_done "napi_complete_done") 会释放实例的所有权。

Warning

当出现既完成了所有事件处理，又恰好达到了 `budget` 数量的情况时，必须谨慎处理。因为没
有办法将这种（很少出现的）情况报告给协议栈，所以驱动程序要么不调用 [`napi_complete_done()`](../../../networking/kapi.html#c.napi_complete_done "napi_complete_done")
并等待再次被调用，要么返回 `budget - 1`。

当 `budget` 为 0 时，[`napi_complete_done()`](../../../networking/kapi.html#c.napi_complete_done "napi_complete_done") 绝对不能被调用。

### 调用序列

驱动程序不应假定调用的顺序是固定不变的。即使驱动程序没有调度该实例，轮询方法也可能会被调用
（除非该实例处于禁用状态）。同样，即便 [`napi_schedule()`](../../../networking/kapi.html#c.napi_schedule "napi_schedule") 调用成功，也不能保证轮询方法一定
会被调用（例如，如果该实例被禁用）。

正如在 [控制API](#drv-ctrl-zh-cn) 部分所提到的，[`napi_disable()`](../../../networking/kapi.html#c.napi_disable "napi_disable") 以及后续对轮询方法的调用，
仅会等待该实例的所有权被释放，而不会等待轮询方法退出。这意味着，驱动程序在调用 [`napi_complete_done()`](../../../networking/kapi.html#c.napi_complete_done "napi_complete_done")
之后，应避免访问任何数据结构。
...
显式屏蔽中断的驱动程序（而非设备自动屏蔽 IRQ）应使用 [`napi_schedule_prep()`](../../../networking/kapi.html#c.napi_schedule_prep "napi_schedule_prep") 和
[`__napi_schedule()`](../../../networking/kapi.html#c.__napi_schedule "__napi_schedule") 调用：
...
IRQ 仅应在成功调用 [`napi_complete_done()`](../../../networking/kapi.html#c.napi_complete_done "napi_complete_done") 后取消屏蔽：
...
[`napi_schedule_irqoff()`](../../../networking/kapi.html#c.napi_schedule_irqoff "napi_schedule_irqoff") 是 [`napi_schedule()`](../../../networking/kapi.html#c.napi_schedule "napi_schedule") 的一个变体，它利用了在中断请求（IRQ）上下文
环境中调用所带来的特性（无需屏蔽中断）。如果中断请求（IRQ）是通过线程处理的（例如启用了
`PREEMPT_RT` 时的情况），[`napi_schedule_irqoff()`](../../../networking/kapi.html#c.napi_schedule_irqoff "napi_schedule_irqoff") 会回退为使用 [`napi_schedule()`](../../../networking/kapi.html#c.napi_schedule "napi_schedule") 。
...
的用户配置就会丢失。[`netif_napi_add_config()`](../../../networking/kapi.html#c.netif_napi_add_config "netif_napi_add_config") API接口通过将每个 NAPI 实例与基于驱动程序定义
...
驱动程序应尽可能尝试使用 [`netif_napi_add_config()`](../../../networking/kapi.html#c.netif_napi_add_config "netif_napi_add_config")。
...
`irq-suspend-timeout` 用于确定应用程序可以完全挂起 IRQ 的时长。与 SO\_PREFER\_BUSY\_POLL
...
等 sysctls 启用忙轮询。还存在基于 io\_uring 的 NAPI 忙轮询 API 可使用。
...
SO\_INCOMING\_NAPI\_ID）然后将该文件描述符分发给工作线程。工作线程将该文件描述符添加到其
epoll 上下文。这确保了每个工作线程的 epoll 上下文中所包含的文件描述符具有相同的 NAPI ID。

或者，如果应用程序使用 SO\_REUSEPORT，可以插入 bpf 或 ebpf 程序来分发传入连接，使得每个
线程只接收具有相同 NAPI ID 的连接。但是必须谨慎处理系统中可能存在多个网卡的情况。

为了启用忙轮询，有两种选择：

1. `/proc/sys/net/core/busy_poll` 可以设置为微秒数以在忙循环中等待事件。这是一个系统
   范围的设置，将导致所有基于 epoll 的应用程序在调用 epoll\_wait 时忙轮询。这可能不是理想
...
每秒高请求的应用程序（尤其是路由/转发应用程序和特别使用 AF\_XDP 套接字的应用程序）
...
只要应用程序对 epoll\_wait 的调用成功获取事件，内核就会推迟 IRQ 挂起定时器。如果
...
>    为覆盖用户应用程序调用 epoll\_wait 处理数据所需的时间，需注意的是，应用程
>    序可通过在调用 epoll\_wait 时设置 `max_events` 来控制获取的数据量。
...
如上所述，只要后续对 epoll\_wait 的调用向用户空间返回事件，`irq-suspend-timeout`
就会被推迟并且 IRQ 会被禁用。这允许应用程序在无干扰的情况下处理数据。

一旦 epoll\_wait 的调用没有找到任何事件，IRQ 挂起会被自动禁用，并且 `gro_flush_timeout`
