...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/devicetree/of_unittest.html
...
Original:
:   [Open Firmware Devicetree Unittest](../../../devicetree/of_unittest.html)
...
1. [Linux and the Devicetree](../../../devicetree/usage-model.html)
...
当内核在启用CONFIG\_OF\_UNITTEST的情况下被构建时，那么下面的make规则:
...
其次，它调用[`of_fdt_unflatten_tree()`](../../../devicetree/kernel-api.html#c.of_fdt_unflatten_tree "of_fdt_unflatten_tree")来解除扁平化的blob。最后，如果机器的设备树
...
如果发现一个重复的节点（即如果一个具有相同full\_name属性的节点已经存在于实时树中），
那么该节点不会被附加，而是通过调用函数`update_node_properties()`将其属性更新到活
树的节点中。

### 2.2. 删除测试数据

一旦测试用例执行完，selftest\_data\_remove被调用，以移除最初连接的设备节点（首先是
叶子节点被分离，然后向上移动父节点被移除，最后是整个树）。`selftest_data_remove()`
调用`detach_node_and_children()`，使用[`of_detach_node()`](../../../devicetree/kernel-api.html#c.of_detach_node "of_detach_node")将节点从实时设备树上分离。

为了分离一个节点，[`of_detach_node()`](../../../devicetree/kernel-api.html#c.of_detach_node "of_detach_node")要么将给定节点的父节点的子节点指针更新为其同级节
