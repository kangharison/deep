...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/doc-guide/sphinx.html
...
Original:
:   [Using Sphinx for kernel documentation](../../../doc-guide/sphinx.html)
...
[检查Sphinx依赖项](#sphinx-pre-install-zh) 。
...
*targets* 例外（例如引用 `` :ref:`最后一行 <last row_zh>` `` / [最后一行](#last-row-zh) ）。
...
[Writing kernel-doc comments](../../../doc-guide/kernel-doc.html) 。

# 图形图片

如果要添加图片，应该使用 `kernel-figure` 和 `kernel-image` 指令。例如，
要插入具有可缩放图像格式的图形，请使用SVG（[SVG 图片示例](#svg-image-example-zh) ）:
...
一个简单的例子（[DOT 示例](#hello-dot-file-zh) ）:
...
标记将作为 *文字块* 插入（[嵌入式 DOT (Graphviz) 代码](#hello-dot-render-zh) ）。
...
如果您想引用它，还需要一个 `caption` （[嵌入式 SVG 标记](#hello-svg-render-zh) ）。
