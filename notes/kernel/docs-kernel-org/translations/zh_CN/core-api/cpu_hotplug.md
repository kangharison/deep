...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/core-api/cpu_hotplug.html
...
Original:
:   [CPU hotplug in the Kernel](../../../core-api/cpu_hotplug.html)
...
`cpu_possible_mask`
:   系统中可能可用CPU的位图。这是用来为per\_cpu变量分配一些启动时的内存，这些变量
...
内核选项 *CONFIG\_HOTPLUG\_CPU* 需要被启用。它目前可用于多种架构，包括ARM、MIPS、
...
在X86上，内核选项 *CONFIG\_BOOTPARAM\_HOTPLUG\_CPU0* 必须被启用，以便能够关闭CPU0。
或者，可以使用内核命令选项 *cpu0\_hotplug* 。CPU0的一些已知的依赖性:
...
* 如果任务因暂停操作而被冻结，那么 *cpuhp\_tasks\_frozen* 将被设置为true。
...
CPU热插拔使用一个从CPUHP\_OFFLINE到CPUHP\_ONLINE的线性状态空间的普通状态机。每个状态都
有一个startup和teardown的回调。

当一个CPU上线时，将按顺序调用startup回调，直到达到CPUHP\_ONLINE状态。当设置状态的回调
或将实例添加到多实例状态时，也可以调用它们。

当一个CPU下线时，将按相反的顺序依次调用teardown回调，直到达到CPUHP\_OFFLINE状态。当删
...
* PREPARE阶段
  ...
  PREPARE阶段涵盖了从CPUHP\_OFFLINE到CPUHP\_BRINGUP\_CPU之间的状态空间。
...
* STARTING阶段
  ...
  STARTING阶段涵盖了CPUHP\_BRINGUP\_CPU + 1到CPUHP\_AP\_ONLINE之间的状态空间。
...
* ONLINE阶段
  ...
  ONLINE阶段涵盖了CPUHP\_AP\_ONLINE + 1到CPUHP\_ONLINE之间的状态空间。
...
* 静态分配
  ...
  cpuhp\_state枚举中的常量描述，可以在include/linux/cpuhotplug.h中找到。
...
@state参数要么是静态分配的状态，要么是动态分配状态（PUHP\_PREPARE\_DYN，CPUHP\_ONLINE\_DYN）
...
> * [`cpuhp_setup_state_nocalls()`](../../../core-api/cpu_hotplug.html#c.cpuhp_setup_state_nocalls "cpuhp_setup_state_nocalls"), [`cpuhp_setup_state_nocalls_cpuslocked()`](../../../core-api/cpu_hotplug.html#c.cpuhp_setup_state_nocalls_cpuslocked "cpuhp_setup_state_nocalls_cpuslocked")和
>   [`cpuhp_setup_state_multi()`](../../../core-api/cpu_hotplug.html#c.cpuhp_setup_state_multi "cpuhp_setup_state_multi")只注册回调。
> * [`cpuhp_setup_state()`](../../../core-api/cpu_hotplug.html#c.cpuhp_setup_state "cpuhp_setup_state")和[`cpuhp_setup_state_cpuslocked()`](../../../core-api/cpu_hotplug.html#c.cpuhp_setup_state_cpuslocked "cpuhp_setup_state_cpuslocked")注册回调，并对当前状态大于新
...
@state参数要么是静态分配的状态，要么是由cpuhp\_setup\_state\*()在动态范围内分配
的状态编号。如果状态在动态范围内，则状态编号被释放，可再次进行动态分配。

这些函数在处理已注册回调的方式上有所不同:

> * [`cpuhp_remove_state_nocalls()`](../../../core-api/cpu_hotplug.html#c.cpuhp_remove_state_nocalls "cpuhp_remove_state_nocalls"), [`cpuhp_remove_state_nocalls_cpuslocked()`](../../../core-api/cpu_hotplug.html#c.cpuhp_remove_state_nocalls_cpuslocked "cpuhp_remove_state_nocalls_cpuslocked")
>   和 [`cpuhp_remove_multi_state()`](../../../core-api/cpu_hotplug.html#c.cpuhp_remove_multi_state "cpuhp_remove_multi_state")只删除回调。
> * [`cpuhp_remove_state()`](../../../core-api/cpu_hotplug.html#c.cpuhp_remove_state "cpuhp_remove_state")删除回调，并调用所有当前状态大于被删除状态的在线CPU的
...
@state参数是一个静态分配的状态或由[`cpuhp_setup_state_multi()`](../../../core-api/cpu_hotplug.html#c.cpuhp_setup_state_multi "cpuhp_setup_state_multi")在动态范围内分配的状
态编号。

@node参数是一个指向hlist\_node的指针，它被嵌入到实例的数据结构中。这个指针被交给
多实例状态的回调，可以被回调用来通过[`container_of()`](../../../driver-api/basics.html#c.container_of "container_of")检索到实例。

这些函数在处理已注册回调的方式上有所不同:

> * [`cpuhp_state_add_instance_nocalls()`](../../../core-api/cpu_hotplug.html#c.cpuhp_state_add_instance_nocalls "cpuhp_state_add_instance_nocalls")只将实例添加到多实例状态的节点列表中。
> * [`cpuhp_state_add_instance()`](../../../core-api/cpu_hotplug.html#c.cpuhp_state_add_instance "cpuhp_state_add_instance")为所有当前状态大于@state的在线CPU添加实例并调用与
...
参数与上述cpuhp\_state\_add\_instance\*()变体相同。

这些函数在处理已注册回调的方式上有所不同:

> * [`cpuhp_state_remove_instance_nocalls()`](../../../core-api/cpu_hotplug.html#c.cpuhp_state_remove_instance_nocalls "cpuhp_state_remove_instance_nocalls")只从状态的节点列表中删除实例。
> * [`cpuhp_state_remove_instance()`](../../../core-api/cpu_hotplug.html#c.cpuhp_state_remove_instance "cpuhp_state_remove_instance")删除实例并调用与@state相关的回调（如果不是NULL），
...
:   这实际上是为了确保CPU的死亡。实际上，看看其他架构中实现CPU热拔插的一些示例代
    ...
    通常会等待一些per\_cpu状态的设置，以确保处理器的死亡例程被调用来保持活跃。
