...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/core-api/protection-keys.html
...
Original:
:   [Memory Protection Keys](../../../core-api/protection-keys.html)
...
`pkey_set()`是RDPKRU和WRPKRU指令的一个封装器。在tools/testing/selftests/x86/protection\_keys.c中可以找到一个实现实例。
tools/testing/selftests/x86/protection\_keys.c.
...
在这两种情况下，内核都会发送一个SIGSEGV，但当违反保护密钥时，si\_code
将被设置为SEGV\_PKERR，而当违反普通的`mprotect()`权限时，则是SEGV\_ACCERR。
