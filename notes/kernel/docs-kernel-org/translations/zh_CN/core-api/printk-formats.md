...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/core-api/printk-formats.html
...
Original:
:   [How to get printk format specifiers right](../../../core-api/printk-formats.html)
...
如果 <type> 的大小依赖于配置选项 (例如 sector\_t, blkcnt\_t) 或其大小依赖于架构
(例如 tcflag\_t)，则使用其可能的最大类型的格式占位符并显式强制转换为它。
...
提醒：sizeof()返回类型为size\_t。

内核的printf不支持%n。显而易见，浮点格式(%e, %f, %g, %a)也不被识别。使用任何不
支持的占位符或长度限定符都会导致一个WARN并且终止[`vsnprintf()`](../../../core-api/kernel-api.html#c.vsnprintf "vsnprintf")执行。
...
`seq_printf()`，而不是[`printk()`](../../../core-api/printk-basics.html#c.printk "printk")）由用户空间进程读取，使用下面描述的%pK修饰符，不
要用%p或%px。
...
用于打印错误指针(即[`IS_ERR()`](../../../core-api/kernel-api.html#c.IS_ERR "IS_ERR")为真的指针)的符号错误名。不知道符号名的错误值会以十进制打印，
而作为%pe参数传递的非ERR\_PTR会被视为普通的%p。
...
定符的结果是打印一个字符串。对于直接在常规的[`vsnprintf()`](../../../core-api/kernel-api.html#c.vsnprintf "vsnprintf")中使用时，(k)和(u)注释被忽略，但是，当
...
用于打印应该对非特权用户隐藏的内核指针。%pK的行为取决于kptr\_restrict sysctl——详见
[Documentation for /proc/sys/kernel/](../../../admin-guide/sysctl/kernel.html)。
...
在使用%px之前，请考虑使用%p并在调试过程中启用’ ‘ no\_hash\_pointer ‘ ‘内核参数是否足
够(参见上面的%p描述)。%px的一个有效场景可能是在panic发生之前立即打印消息，这样无论如何
都可以防止任何敏感消息被利用，使用%px就不需要用no\_hash\_pointer来重现panic。
...
用于打印phys\_addr\_t类型（以及它的衍生物，如resource\_size\_t），该类型可以根据构建选项而
...
用于打印dma\_addr\_t类型，该类型可以根据构建选项而变化，而不考虑CPU数据路径的宽度。
...
ESCAPE\_ANY\_NP是许多情况下的明智选择，特别是对于打印SSID。
...
用于打印一个IP地址，不需要区分它的类型是AF\_INET还是AF\_INET6。一个指向有效结构
...
不会oops。 %pd dentry比较安全，其相当于我们以前用的%s dentry->d\_name.name，%pd<n>打
...
用于打印结构体va\_format。这些结构包含一个格式字符串
和va\_list如下
...
如果没有一些机制来验证格式字符串和va\_list参数的正确性，请不要使用这个功能。
...
用于打印fwnode\_handles的消息。默认情况下是打印完整的节点名称，包括路径。
这些修饰符在功能上等同于上面的%pOF。
...
是[p]age flags， [v]ma\_flags(都期望 `unsigned long *` )和
[g]fp\_flags(期望 `gfp_t *` )。标志名称和打印顺序取决于特定的类型。

注意，这种格式不应该直接用于跟踪点的:c:func:`TP_printk()` 部分。相反，应使
用 <trace/events/mmflags.h>中的show\_\*`_flags()`函数。
...
如果您添加了其他%p扩展，请在可行的情况下，用一个或多个测试用例扩展<lib/test\_printf.c>。
