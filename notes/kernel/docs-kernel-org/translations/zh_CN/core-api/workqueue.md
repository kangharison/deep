...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/core-api/workqueue.html
...
Original:
:   [Workqueue](../../../core-api/workqueue.html)
...
* 除非有特殊需要，建议使用0作为@max\_active。在大多数使用情
  况下，并发水平通常保持在默认限制之下。
...
使用 tools/workqueue/wq\_dump.py（drgn脚本） 来检查未
...
使用 tools/workqueue/wq\_monitor.py 来监视工作队列的运行：
