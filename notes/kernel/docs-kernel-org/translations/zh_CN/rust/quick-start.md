...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/rust/quick-start.html
...
另一种方式是使用 [kernel.org](https://kernel.org/pub/tools/llvm/rust/) 上提
供的预构建稳定版本的 LLVM+Rust。这些与 [获取 LLVM](../kbuild/llvm.html#zh-cn-getting-llvm) 中的精
...
请参阅 [Building Linux with Clang/LLVM](../../../kbuild/llvm.html) 了解更多信息，以及获取预构建版本和发行包
的进一步方法。
...
传递 `CLIPPY=1` 来运行（关于细节，详见 [基本信息](general-information.html) ）。
...
`rustdoc` 是Rust的文档工具。它为Rust代码生成漂亮的HTML文档（详情请见 [基本信息](general-information.html) ）。
...
Rust支持（CONFIG\_RUST）需要在 `General setup` 菜单中启用。在其他要求得到满足的情
