...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/userspace-api/sysfs-platform_profile.html
...
Original:
:   [Platform Profile Selection (e.g. /sys/firmware/acpi/platform\_profile)](../../../userspace-api/sysfs-platform_profile.html)

翻译:
:   李睿 Rui Li <[me@lirui.org](mailto:me%40lirui.org)>

# 平台配置文件选择（如 /sys/firmware/acpi/platform\_profile）
...
platform\_profile属性的目的是提供一个通用的sysfs API来选择这些平台自动配置
机制的配置文件。
...
sysfs-platform\_profile ABI 文档定义了一个固定的配置文件名集合。驱动程序
...
>    sysfs-platform\_profile ABI文档中。
