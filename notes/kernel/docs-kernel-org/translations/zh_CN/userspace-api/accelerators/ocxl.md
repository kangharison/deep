...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/userspace-api/accelerators/ocxl.html
...
Original:
:   [OpenCAPI (Open Coherent Accelerator Processor Interface)](../../../../userspace-api/accelerators/ocxl.html)
...
OpenCAPI拥有AFU向主机进程发送中断的可能性。它通过定义在传输层的“intrp\_req”
...
[ABI file testing/sysfs-class-ocxl](../../../../admin-guide/abi-testing-files.html#abi-file-testing-sysfs-class-ocxl) 中描述。
...
OCXL\_IOCTL\_IRQ\_ALLOC:

> 分配AFU中断，返回标识符。

OCXL\_IOCTL\_IRQ\_FREE:

> 释放之前分配的AFU中断。

OCXL\_IOCTL\_IRQ\_SET\_FD:

> 将一个事件文件描述符和AFU中断关联，因此用户进程可以在AFU发送中断时收到通
> 知。

OCXL\_IOCTL\_GET\_METADATA:

> 从卡中获取配置信息，比如内存映射IO区域的大小、AFU版本和当前上下文的进程
> 地址空间ID（PASID）。

OCXL\_IOCTL\_ENABLE\_P9\_WAIT:

> 允许AFU唤醒执行“等待”的用户空间进程。返回信息给用户空间，允许其配置AFU。
> 注意这只在POWER9上可用。

OCXL\_IOCTL\_GET\_FEATURES:
