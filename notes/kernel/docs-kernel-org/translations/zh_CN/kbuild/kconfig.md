...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/kbuild/kconfig.html
...
:   allyesconfig/allmodconfig/allnoconfig/randconfig 这些变体也可以使用环境
    变量 KCONFIG\_ALLCONFIG 作为标志或包含用户要求设置为特定值的配置符号的文件名。
    如果 KCONFIG\_ALLCONFIG 未指定文件名，即 KCONFIG\_ALLCONFIG == “” 或
    KCONFIG\_ALLCONFIG == “1”，则 `make *config` 将查找名为
...
:   该变量可用于倾斜概率分布。此变量可不设置或设置为空，或设置为以下三种不同格式：
    ...
    | KCONFIG\_PROBABILITY | y:n 分配 | y:m:n 分配 |
    | --- | --- | --- |
...
`MENUCONFIG_COLOR`
:   可以使用变量 MENUCONFIG\_COLOR 选择不同的配色主题。使用以下命令选择主题:
