...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/kernel-hacking/hacking.html
...
Original:
:   [Unreliable Guide To Hacking The Linux Kernel](../../../kernel-hacking/hacking.html)
...
注意，如果持有 [bottom half lock](#local-bh-disable-zh) 锁，这将返回
假阳性。
...
[`printk()`](../../../core-api/printk-basics.html#c.printk "printk") 将内核消息提供给控制台、dmesg和syslog守护进程。它对于调
...
[`printk()`](../../../core-api/printk-basics.html#c.printk "printk") 内部使用的1K缓冲区，不捕获溢出。请确保足够使用。
...
[`put_user()`](../../../core-api/mm-api.html#c.put_user "put_user") 和 [`get_user()`](../../../core-api/mm-api.html#c.get_user "get_user") 用于从用户空间中获取和向用户空
...
与 [`put_user()`](../../../core-api/mm-api.html#c.put_user "put_user") 和 [`get_user()`](../../../core-api/mm-api.html#c.get_user "get_user") 不同，它们返回未复制的
数据量（即0仍然意味着成功）。
...
[`kmalloc()`](../../../core-api/mm-api.html#c.kmalloc "kmalloc") 需要额外的标志词。重要的值：
...
如果分配的字节数超过一页，可以使用 [`vmalloc()`](../../../core-api/mm-api.html#c.vmalloc "vmalloc") 。它将在内核映射中分
...
[`udelay()`](../../../timers/delay_sleep_functions.html#c.udelay "udelay") 和 [`ndelay()`](../../../timers/delay_sleep_functions.html#c.ndelay "ndelay") 函数可被用于小暂停。不要对它们使用
大的值，因为这样会导致溢出——帮助函数 [`mdelay()`](../../../timers/delay_sleep_functions.html#c.mdelay "mdelay") 在这里很有用，或者
考虑 [`msleep()`](../../../timers/delay_sleep_functions.html#c.msleep "msleep")。
...
[`module_init()`](../../../driver-api/basics.html#c.module_init "module_init") 和 [`module_exit()`](../../../driver-api/basics.html#c.module_exit "module_exit") 宏可以简化代码编写，无需
`#ifdef` ，即可以作为模块运行或内置在内核中。

[`module_init()`](../../../driver-api/basics.html#c.module_init "module_init") 宏定义在模块插入时（如果文件编译为模块）或在引导时
调用哪个函数：如果文件未编译为模块，[`module_init()`](../../../driver-api/basics.html#c.module_init "module_init") 宏将等效于
...
[`try_module_get()`](../../../driver-api/basics.html#c.try_module_get "try_module_get") ：若失败，那么该模块将被删除，您应该将其视为不存在。
若成功，您就可以安全地进入模块，并在完成后调用模块 [`module_put()`](../../../driver-api/basics.html#c.module_put "module_put") 。
...
个宏可以来执行此操作： [`wait_event_interruptible()`](../../../driver-api/basics.html#c.wait_event_interruptible "wait_event_interruptible")
（ `include/linux/wait.h` ）第一个参数是等待队列头，第二个参数是计算的表达
式；当该表达式为true时宏返回0，或者在接收到信号时返回 `-ERESTARTSYS` 。
[`wait_event()`](../../../driver-api/basics.html#c.wait_event "wait_event") 版本会忽略信号。
...
[`atomic_read()`](../../../driver-api/basics.html#c.atomic_read "atomic_read") 和 [`atomic_set()`](../../../driver-api/basics.html#c.atomic_set "atomic_set") 获取并设置计数器，还有
[`atomic_add()`](../../../driver-api/basics.html#c.atomic_add "atomic_add") ，[`atomic_sub()`](../../../driver-api/basics.html#c.atomic_sub "atomic_sub") ，[`atomic_inc()`](../../../driver-api/basics.html#c.atomic_inc "atomic_inc") ，
[`atomic_dec()`](../../../driver-api/basics.html#c.atomic_dec "atomic_dec") 和 [`atomic_dec_and_test()`](../../../driver-api/basics.html#c.atomic_dec_and_test "atomic_dec_and_test") （如果递减为零，
则返回true）。
...
位。[`set_bit()`](../../../core-api/kernel-api.html#c.set_bit "set_bit")，[`clear_bit()`](../../../core-api/kernel-api.html#c.clear_bit "clear_bit") 和 [`change_bit()`](../../../core-api/kernel-api.html#c.change_bit "change_bit") 设置、
清除和更改给定位。[`test_and_set_bit()`](../../../core-api/kernel-api.html#c.test_and_set_bit "test_and_set_bit") ，[`test_and_clear_bit()`](../../../core-api/kernel-api.html#c.test_and_clear_bit "test_and_clear_bit")
和 [`test_and_change_bit()`](../../../core-api/kernel-api.html#c.test_and_change_bit "test_and_change_bit") 执行相同的操作，但如果之前设置了位，则返回
true；这些对于原子设置标志特别有用。
...
[Symbol Namespaces](../../../core-api/symbol-namespaces.html) 。
...
通常 [`list_for_each_entry()`](../../../core-api/list.html#c.list_for_each_entry "list_for_each_entry") 很有用。
...
使用 [`ERR_PTR()`](../../../core-api/kernel-api.html#c.ERR_PTR "ERR_PTR") （ `include/linux/err.h` ）将负错误值编码到指针中，
然后使用 [`IS_ERR()`](../../../core-api/kernel-api.html#c.IS_ERR "IS_ERR") 和 [`PTR_ERR()`](../../../core-api/kernel-api.html#c.PTR_ERR "PTR_ERR") 将其再取出：避免为错误值
...
* 声明函数/变量/类型的属性（\_\_attribute\_\_）
* typeof
* 零长度数组
* 宏变量
* 空指针运算
...
* 通常你需要一个配置选项来支持你的内核编程。在适当的目录中编辑 `Kconfig` 。
  配置语言很容易通过剪切和粘贴来使用，在
  [Kconfig Language](../../../kbuild/kconfig-language.html) 中有完整的文档。

  在您对选项的描述中，请确保同时照顾到了专家用户和对此功能一无所知的用户。
  在此说明任何不兼容和问题。结尾一定要写上“如有疑问，就选N”（或者是“Y”）；
  这是针对那些看不懂你在说什么的人的。
* 编辑 `Makefile` ：配置变量在这里导出，因此通常你只需添加一行
  “obj-$(CONFIG\_xxx) += xxx.o”。语法记录在
  [Linux Kernel Makefiles](../../../kbuild/makefiles.html) 。
* 如果你认为自己做了一些有意义的事情，可以把自己放进 `CREDITS` ，通常不
  止一个文件（无论如何你的名字都应该在源文件的顶部）。 `MAINTAINERS`
  意味着您希望在对子系统进行更改时得到询问，并了解缺陷；这意味着对某部分
  代码做出更多承诺。
* 最后，别忘记去阅读 [Submitting patches: the essential guide to getting your code into the kernel](../../../process/submitting-patches.html)。
...
感谢Werner Almesberger对 [`disable_irq()`](../../../core-api/genericirq.html#c.disable_irq "disable_irq") 做了一个很好的总结，
