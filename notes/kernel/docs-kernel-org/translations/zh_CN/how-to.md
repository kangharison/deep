...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/how-to.html
...
详细的贡献指南：[与Linux 内核社区一起工作](process/index.html)。
...
关于邮件客户端的配置，请查阅 [Linux邮件客户端配置信息](process/email-clients.html)。

## 开始翻译文档

### 文档索引结构

目前中文文档是在 Documentation/translations/zh\_CN/ 目录下进行，该
目录结构最终会与 Documentation/ 结构一致，所以您只需要将您感兴趣的英文
文档文件和对应的 [中文翻译](index.html) 复制到 zh\_CN 目录下对应的位置，然后修改更
上一级的 index 即可开始您的翻译。
...
更加明确的示例，请对 zh\_CN 目录执行 git log。
...
关于详细操作说明，请参考：[工作原理](doc-guide/checktransupdate.html)。
