...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/scheduler/schedutil.html
...
Original:
:   [Schedutil](../../../scheduler/schedutil.html)
...
r\_cpu被定义为当前CPU的最高性能水平与系统中任何其它CPU的最高性能水平的比率。
...
> * [算力感知调度](sched-capacity.html):”1. CPU Capacity + 2. Task utilization”
...
为了缓解这个问题，（一个默认使能的编译选项）UTIL\_EST驱动一个无限脉冲响应
（Infinite Impulse Response，IIR）的EWMA，“运行”值在出队时是最高的。
UTIL\_EST滤波使其在遇到更高值时立刻增加，而遇到低值时会缓慢衰减。
...
可以在每个CFS或RT任务上设置有效的u\_min和u\_max clamp值（译注：clamp可以理解
...
>   时间流逝将自动修正。另注，由于没有空闲时间，我们还能保证f\_max值吗？
