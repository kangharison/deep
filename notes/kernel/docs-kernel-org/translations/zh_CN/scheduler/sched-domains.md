...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/scheduler/sched-domains.html
...
Original:
:   [Scheduler Domains](../../../scheduler/sched-domains.html)
...
CPU共享。任意两个组的CPU掩码的交集不一定为空，如果是这种情况，对应调度域的SD\_OVERLAP
...
实现者需要阅读include/linux/sched/sd\_flags.h的注释：读SD\_\*来了解具体情况以及调度域的
SD标志位调节了哪些东西。

体系结构可以把指定的拓扑层级的通用调度域构建器和默认的SD标志位覆盖掉，方法是创建一个
sched\_domain\_topology\_level数组，并以该数组作为入参调用`set_sched_topology()`。

调度域调试基础设施可以通过CONFIG\_SCHED\_DEBUG开启，并在开机启动命令行中增加
“sched\_verbose”。如果你忘记调整开机启动命令行了，也可以打开
