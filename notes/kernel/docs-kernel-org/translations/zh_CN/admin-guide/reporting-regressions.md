...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/admin-guide/reporting-regressions.html
...
的回归；关于内核开发者如何处理的方面参见 [Handling regressions](../../../process/handling-regressions.html) 。
...
1. 如果某程序在原先的Linux内核上运行良好，但在较新版本上效果更差、或者根本不
   ...
   节参见下方。
2. 按照 [报告问题](reporting-issues.html) 中
...
只需按照 [报告问题](reporting-issues.html) 中
...
如 [报告问题](reporting-issues.html) （简要）
和 [二分（bisect）缺陷](bug-bisect.html) （详细）中所
...
多发言，其中一些在 [Handling regressions](../../../process/handling-regressions.html) 中被引用。
