...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/admin-guide/mm/damon/usage.html
...
* *sysfs接口。*
  [这](../../../../../admin-guide/mm/damon/usage.html#sysfs-interface) 是为那些希望更高级的使用DAMON的特权用户空间程序员准备的。
  ...
  空间的监测。
* *内核空间编程接口。*
  [这](../../../../../mm/damon/api.html) 这是为内核空间程序员准备的。使用它，用户可以通过为你编写内
  核空间的DAMON应用程序，最灵活有效地利用DAMON的每一个功能。你甚至可以为各种地址空间扩展DAMON。
  详细情况请参考接口 [文件](../../../../../mm/damon/api.html)。
...
[stats section](../../../../../admin-guide/mm/damon/usage.html#sysfs-schemes-stats). 将 `update_schemes_tried_regions` 写到
`state` 文件，为kdamond的每个基于DAMON的操作方案，更新基于DAMON的操作方案动作的尝试区域目录。
将`clear\_schemes\_tried\_regions`写入`state`文件，清除kdamond的每个基于DAMON的操作方案的动作
尝试区域目录。 关于基于DAMON的操作方案动作尝试区域目录的细节，请参考:ref:tried\_regions 部分
...
关于间隔和监测区域范围的更多细节，请参考设计文件 ([Design](../../../../../mm/damon/design.html))。
...
内存区域的详细信息，在下一个 [聚集区间](../../../../../admin-guide/mm/damon/usage.html#sysfs-monitoring-attrs)，相应的方案的 `动作`
