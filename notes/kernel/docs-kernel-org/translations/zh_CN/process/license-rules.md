...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/process/license-rules.html
...
:   数文件，这是第一行，除了那些在第一行中需要’#!PATH\_TO\_INTERPRETER’的脚本。
    对于这些脚本，SPDX标识符进入第二行。
...
3. 句法:
   ...
   的标记中。有关详细信息，请参阅 [许可标识符](#id2) 一章中的 [例外](#id5) 。
...
1. 优先许可:
   ...
   此目录中的文件包含完整的许可证文本和 [元标记](#id4) 。文件名与SPDX许可证标识
   符相同，后者应用于源文件中的许可证。
...
3. 例外:
   ...
   此目录中的文件包含完整的例外文本和所需的 [例外元标记](#id6) 。
...
> 可加载内核模块还需要MODULE\_LICENSE（）标记。此标记既不替代正确的源代码
...
> > | “Proprietary” | 该模块属于专有许可。此字符串仅用于专有的第三 方模块，不能用于在内核树中具有源代码的模块。 以这种方式标记的模块在加载时会使用’P’标记污 染内核，并且内核模块加载器拒绝将这些模块链接 到使用EXPORT\_SYMBOL\_GPL（）导出的符号。 |
