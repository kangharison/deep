...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/process/submit-checklist.html
...
这些都是在 [提交补丁：如何让你的改动进入内核](submitting-patches.html)
...
4. PPC64是一种很好的交叉编译检查体系结构，因为它倾向于对64位的数使用无符号
   长整型。
5. 按 [Linux 内核代码风格](coding-style.html) 所述检查您的
...
8. 已仔细审查了相关的 `Kconfig` 组合。这很难用测试来纠正——脑力在这里是有
   回报的。
9. 通过 sparse 清查。
   （参见 [Sparse](../dev-tools/sparse.html) ）
10. 使用 `make checkstack` 并修复他们发现的任何问题。
    ...
    字节的函数都可以进行更改。
11. 包括 [kernel-doc](../doc-guide/kernel-doc.html#kernel-doc-zh) 内核文档以记录全局内核API。（静态
    函数不需要，但也可以。）使用 `make htmldocs` 或 `make pdfdocs` 检查
    [kernel-doc](../doc-guide/kernel-doc.html#kernel-doc-zh) 并修复任何问题。
...
18. 所有新的用户空间接口都记录在 `Documentation/ABI/` 中。有关详细信息，
    请参阅 [Linux ABI description](../../../admin-guide/abi.html) (或 `Documentation/ABI/README`)。
