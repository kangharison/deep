...
> 출처(원문): https://docs.kernel.org/translations/zh_CN/security/landlock.html
...
对被移除链接的inode并无影响。这就是“LANDLOCK\_ACCESS\_FS\_REMOVE\_FILE”
（文件系统移除文件访问权限）或“LANDLOCK\_ACCESS\_FS\_REFER” （文件系
...
以LANDLOCK\_ACCESS\_FS\_TRUNCATE权限为例，如果相关的文件层次结构没有
...
与文件访问模式类似（例如O\_RDWR），即使文件描述符在进程之间传递
