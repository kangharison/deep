...
> 출처(원문): https://docs.kernel.org/translations/zh_TW/dev-tools/testing-overview.html
...
存在些許差異，因此建議讀者結合原文
（[Kernel Testing Guide](../../../dev-tools/testing-overview.html)）輔助閱讀。
如對部分翻譯有異議或有更好的翻譯意見，歡迎聯繫譯者進行修訂。

KUnit（[KUnit - Linux Kernel Unit Testing](../../../dev-tools/kunit/index.html)）是用於“白箱”測
...
kselftest（[Linux Kernel Selftests](../../../dev-tools/kselftest.html)），相對來說，大量用
...
[在Linux内核里使用gcov做代码覆盖率检查](../../zh_CN/dev-tools/gcov.html) 是GCC的覆蓋率測試
工具，能用於獲取內核的全局或每個模塊的覆蓋率。與KCOV不同的是，這個工具
不記錄每個任務的覆蓋率。覆蓋率數據可以通過debugfs讀取，並通過常規的
gcov工具進行解釋。

[KCOV: code coverage for fuzzing](../../../dev-tools/kcov.html) 是能夠構建在內核之中，用於在每個任務
...
* KCSAN檢測數據競爭。參閱 [Kernel Concurrency Sanitizer (KCSAN)](../../../dev-tools/kcsan.html)
* KFENCE是一個低開銷的內存問題檢測器，比KASAN更快且能被用於批量構建。
  參閱 [Kernel Electric-Fence (KFENCE)](../../../dev-tools/kfence.html)
...
> [Sparse](../../zh_CN/dev-tools/sparse.html)。
>
