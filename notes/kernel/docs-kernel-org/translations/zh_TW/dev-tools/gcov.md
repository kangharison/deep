...
> 출처(원문): https://docs.kernel.org/translations/zh_TW/dev-tools/gcov.html
...
如果要在整個內核的覆蓋率統計（開啓CONFIG\_GCOV\_PROFILE\_ALL）中單獨排除
某一個文件或者路徑，可以使用如下的方法:
...
可以通過對內核參數gcov\_persist的修改來停用gcov對模塊的備份機制:
...
問題原因
:   由於seq\_file的工作方式，某些工具（例如cp或tar）可能無法正確地從
    sysfs複製文件。
...
## 附錄A：collect\_on\_build.sh

用於在編譯機上收集覆蓋率元文件的示例腳本
（見 [編譯機和測試機分離 a.](#gcov-test-zh-tw) ）
...
## 附錄B：collect\_on\_test.sh

用於在測試機上收集覆蓋率數據文件的示例腳本
（見 [編譯機和測試機分離 b.](#gcov-build-zh-tw) ）
