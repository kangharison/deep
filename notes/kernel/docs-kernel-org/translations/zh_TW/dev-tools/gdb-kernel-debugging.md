...
> 출처(원문): https://docs.kernel.org/translations/zh_TW/dev-tools/gdb-kernel-debugging.html
...
* 創建一個QEMU/KVM的linux虛擬機（詳情請參考 www.linux-kvm.org 和 www.qemu.org ）。
  ...
  可以幫助搭建交叉開發環境。
* 編譯內核時開啓CONFIG\_GDB\_SCRIPTS，關閉CONFIG\_DEBUG\_INFO\_REDUCED。
  如果架構支持CONFIG\_FRAME\_POINTER，請保持開啓。
* 在guest環境上安裝該內核。如有必要，通過在內核command line中添加“nokaslr”來關閉KASLR。
  ...
  在這種情況下，如果架構支持KASLR，應該在禁用CONFIG\_RANDOMIZE\_BASE的情況下構建內核。
