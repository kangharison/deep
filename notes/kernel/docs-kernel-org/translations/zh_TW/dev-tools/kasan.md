...
> 출처(원문): https://docs.kernel.org/translations/zh_TW/dev-tools/kasan.html
...
用CONFIG\_KASAN\_GENERIC啓用的通用KASAN，是用於調試的模式，類似於用戶空
間的ASan。這種模式在許多CPU架構上都被支持，但它有明顯的性能和內存開銷。

基於軟件標籤的KASAN或SW\_TAGS KASAN，通過CONFIG\_KASAN\_SW\_TAGS啓用，
可以用於調試和自我測試，類似於用戶空間HWASan。這種模式只支持arm64，但其
適度的內存開銷允許在內存受限的設備上用真實的工作負載進行測試。

基於硬件標籤的KASAN或HW\_TAGS KASAN，用CONFIG\_KASAN\_HW\_TAGS啓用，被
...
通用KASAN支持在所有的slab、page\_alloc、vmap、vmalloc、堆棧和全局內存
中查找錯誤。

基於軟件標籤的KASAN支持slab、page\_alloc、vmalloc和堆棧內存。

基於硬件標籤的KASAN支持slab、page\_alloc和不可執行的vmalloc內存。
...
* `kasan.mode=sync`, `=async` or `=asymm` 控制KASAN是否
  ...
  件中（對於arm64來說是在TFSR\_EL1寄存器中）。內核週期性地檢查硬件，並且只在這些檢查中報告標籤異常。
...
出現在錯誤訪問堆棧跟蹤中的位置。目前，這包括 [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") 和排隊的工作隊列。
...
通用KASAN將1/8的內核內存專用於其影子內存（16TB以覆蓋x86\_64上的128TB），並使用
...
請注意，啓用CONFIG\_KASAN\_HW\_TAGS始終會導致啓用內核中的TBI。即使提供了
