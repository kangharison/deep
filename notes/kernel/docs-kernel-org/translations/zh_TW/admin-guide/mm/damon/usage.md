...
> 출처(원문): https://docs.kernel.org/translations/zh_TW/admin-guide/mm/damon/usage.html
...
Original:
:   [Detailed Usages](../../../../../admin-guide/mm/damon/usage.html)
...
* *sysfs接口。*
  [這](../../../../../admin-guide/mm/damon/usage.html#sysfs-interface) 是爲那些希望更高級的使用DAMON的特權用戶空間程序員準備的。
  ...
  空間的監測。
* *內核空間編程接口。*
  [這](../../../../../mm/damon/api.html) 這是爲內核空間程序員準備的。使用它，用戶可以通過爲你編寫內
  核空間的DAMON應用程序，最靈活有效地利用DAMON的每一個功能。你甚至可以爲各種地址空間擴展DAMON。
  詳細情況請參考接口 [文件](../../../../../mm/damon/api.html)。
...
[stats section](../../../../../admin-guide/mm/damon/usage.html#sysfs-schemes-stats). 將 `update_schemes_tried_regions` 寫到
`state` 文件，爲kdamond的每個基於DAMON的操作方案，更新基於DAMON的操作方案動作的嘗試區域目錄。
將`clear\_schemes\_tried\_regions`寫入`state`文件，清除kdamond的每個基於DAMON的操作方案的動作
嘗試區域目錄。 關於基於DAMON的操作方案動作嘗試區域目錄的細節，請參考:ref:tried\_regions 部分
<sysfs\_schemes\_tried\_regions>`。
...
關於間隔和監測區域範圍的更多細節，請參考設計文件 ([Design](../../../../../mm/damon/design.html))。
...
內存區域的詳細信息，在下一個 [聚集區間](../../../../../admin-guide/mm/damon/usage.html#sysfs-monitoring-attrs)，相應的方案的 `動作`
