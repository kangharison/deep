...
> 출처(원문): https://docs.kernel.org/translations/zh_TW/process/submit-checklist.html
...
這些都是在 [提交补丁：如何让你的改动进入内核](../../zh_CN/process/submitting-patches.html)
...
4. PPC64是一種很好的交叉編譯檢查體系結構，因爲它傾向於對64位的數使用無符號
   長整型。
5. 按 [Linux 内核代码风格](../../zh_CN/process/coding-style.html) 所述檢查您的
...
8. 已仔細審查了相關的 `Kconfig` 組合。這很難用測試來糾正——腦力在這裏是有
   回報的。
9. 通過 sparse 清查。
   （參見 [Sparse](../../zh_CN/dev-tools/sparse.html) ）
10. 使用 `make checkstack` 並修復他們發現的任何問題。
    ...
    字節的函數都可以進行更改。
11. 包括 [kernel-doc](../../zh_CN/doc-guide/kernel-doc.html#kernel-doc-zh) 內核文檔以記錄全局內核API。（靜態
    函數不需要，但也可以。）使用 `make htmldocs` 或 `make pdfdocs` 檢查
    [kernel-doc](../../zh_CN/doc-guide/kernel-doc.html#kernel-doc-zh) 並修復任何問題。
...
16. 所有新的內核引導參數都記錄在
    [The kernel’s command-line parameters](../../../admin-guide/kernel-parameters.html) 中。
17. 所有新的模塊參數都記錄在 `MODULE_PARM_DESC()`
18. 所有新的用戶空間接口都記錄在 `Documentation/ABI/` 中。有關詳細信息，
    請參閱 [Linux ABI description](../../../admin-guide/abi.html) (或 `Documentation/ABI/README`)。
