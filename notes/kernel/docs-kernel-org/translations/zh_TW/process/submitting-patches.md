...
> 출처(원문): https://docs.kernel.org/translations/zh_TW/process/submitting-patches.html
...
參見： [内核开发过程指南](../../zh_CN/process/development-process.html) 。
[Linux内核补丁提交检查单](../../zh_CN/process/submit-checklist.html) 給出了一系列
...
參見 [标准补丁格式](../../zh_CN/process/submitting-patches.html#zh-the-canonical-patch-format) 。

每個補丁只解決一個問題。如果你的描述開始變長，這就表明你可能需要拆分你的補丁。
請見 [拆分你的改动](../../zh_CN/process/submitting-patches.html#zh-split-changes) 。
...
[Linux 内核代码风格](../../zh_CN/process/coding-style.html)
...
scripts/get\_maintainer.pl在這個步驟中非常有用。如果您找不到正在工作的子系統
...
顯然不應將補丁發送到任何公共列表。
參見 [安全缺陷](../../zh_CN/process/security-bugs.html) 。
...
除了本文件之外，您還應該閱讀
[所有你想知道的事情 - 关于linux稳定版发布](../../zh_CN/process/stable-kernel-rules.html) 。
...
請參閱 [Linux邮件客户端配置信息](../../zh_CN/process/email-clients.html)
...
前一版本的不同之處（參見 [标准补丁格式](../../zh_CN/process/submitting-patches.html#zh-the-canonical-patch-format) ）。
...
指示補丁修復的錯誤的首選方法。請參閱 [描述你的改动](../../zh_CN/process/submitting-patches.html#zh-describe-changes) 瞭解更多信息。
...
手動添加回復補丁的的郵件頭（In-Reply\_To:）是有用的（例如，使用 `git send-email` ），
...
內核 [Linux 内核代码风格](../../zh_CN/process/coding-style.html)
