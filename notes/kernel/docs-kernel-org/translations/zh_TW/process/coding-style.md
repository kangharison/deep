...
> 출처(원문): https://docs.kernel.org/translations/zh_TW/process/coding-style.html
...
Original:
:   [Linux kernel coding style](../../../process/coding-style.html)
...
後要加一個空格。值得注意的例外是 sizeof, typeof, alignof 和 \_\_attribute\_\_，這
...
但是不要在 sizeof, typeof, alignof 或者 \_\_attribute\_\_ 這些關鍵字之後放空格。
例如，
...
>    Note
>
>    不透明性和“訪問函數”本身是不好的。我們使用 pte\_t 等類型的原因在於真
>    的是完全沒有任何共用的可訪問信息。
...
>    `u32` 類型。因此，我們在與用戶空間共享的所有結構體中使用 \_\_u32 和類似
>    的類型。
...
這樣的GW\_BASIC 名稱，因爲一旦你添加或刪除了 (函數的) 退出路徑，你就必須對它們
重新編號，這樣會難以去檢驗正確性。
...
[如何编写内核文档](../../zh_CN/doc-guide/index.html) 和 tools/docs/kernel-doc 。
...
詳見 [clang-format](../../../dev-tools/clang-format.html) 。
...
要查看配置文件的完整文檔，請看 [Kconfig Language](../../../kbuild/kconfig-language.html) 。
...
這種 `多級引用計數` 的例子可以在內存管理 (`struct mm_struct`: mm\_users 和
mm\_count)，和文件系統 (`struct super_block`: s\_count 和 s\_active) 中找到。
...
ret 是本地變量的通用名字—— \_\_foo\_ret 更不容易與一個已存在的變量衝突。
...
了 [`pr_notice()`](../../../core-api/printk-basics.html#c.pr_notice "pr_notice"), [`pr_info()`](../../../core-api/printk-basics.html#c.pr_info "pr_info"), [`pr_warn()`](../../../core-api/printk-basics.html#c.pr_warn "pr_warn"), [`pr_err()`](../../../core-api/printk-basics.html#c.pr_err "pr_err") 和其他。

寫出好的調試信息可以是一個很大的挑戰；一旦你寫出後，這些信息在遠程除錯時能提
供極大的幫助。然而打印調試信息的處理方式同打印非調試信息不同。其他 `pr_XXX()`
函數能無條件地打印，[`pr_debug()`](../../../core-api/printk-basics.html#c.pr_debug "pr_debug") 卻不；默認情況下它不會被編譯，除非定義了 DEBUG
或設定了 CONFIG\_DYNAMIC\_DEBUG。實際這同樣是爲了 `dev_dbg()`，一個相關約定是在一
個已經開啓了 DEBUG 時，使用 VERBOSE\_DEBUG 來添加 `dev_vdbg()`。

許多子系統擁有 Kconfig 調試選項來開啓對應 Makefile 裏面的 -DDEBUG；在其他
情況下，特殊文件使用 #define DEBUG。當一條調試信息需要被無條件打印時，例如，
如果已經包含一個調試相關的 #ifdef 條件，printk(KERN\_DEBUG ...) 就可被使用。

## 14) 分配內存

內核提供了下面的一般用途的內存分配函數：
[`kmalloc()`](../../../core-api/mm-api.html#c.kmalloc "kmalloc"), [`kzalloc()`](../../../core-api/mm-api.html#c.kzalloc "kzalloc"), [`kmalloc_array()`](../../../core-api/mm-api.html#c.kmalloc_array "kmalloc_array"), [`kcalloc()`](../../../core-api/mm-api.html#c.kcalloc "kcalloc"), [`vmalloc()`](../../../core-api/mm-api.html#c.vmalloc "vmalloc") 和 [`vzalloc()`](../../../core-api/mm-api.html#c.vzalloc "vzalloc")。
請參考 API 文檔以獲取有關它們的詳細信息：
[内存分配指南](../../zh_CN/core-api/memory-allocation.html) 。
...
在沒有 \_\_GFP\_NOWARN 的情況下使用時，這些通用分配函數都會在失敗時發起堆棧轉儲，
...
[`kmalloc()`](../../../core-api/mm-api.html#c.kmalloc "kmalloc") 內聯函數就是一個很好的例子。
...
-EBUSY。類似的，因爲 `PCI device present` 是一個判斷，所以 [`pci_dev_present()`](../../../driver-api/pci/pci.html#c.pci_dev_present "pci_dev_present")
...
未使用，請把它標記爲 \_\_maybe\_unused 而不是將它包含在一個預處理條件中。(然而，
如果一個函數或變量總是未使用，就直接刪除它。)

在代碼中，儘可能地使用 IS\_ENABLED 宏來轉化某個 Kconfig 標記爲 C 的布爾
表達式，並在一般的 C 條件中使用它：
...
內核文檔 [Linux kernel coding style](../../../process/coding-style.html)，
