...
> 출처(원문): https://docs.kernel.org/translations/sp_SP/process/coding-style.html
...
Original:
:   [Documentation/process/coding-style.rst](../../../process/submitting-patches.html#submittingpatches)
...
typeof, alignof y \_\_attribute\_\_, que parecen algo así como funciones (y
...
pero no con sizeof, typeof, alignof, o \_\_attribute\_\_. Por ejemplo,
...
>    mismas. La razón por la que los tenemos para cosas como pte\_t, etc.
...
>
>    > typedef unsigned long mis\_flags\_t;
>
...
>    tanto, usamos \_\_u32 y tipos similares en todas las estructuras que se
...
kernel-doc. Consulte los archivos en [Documentation/doc-guide/](../../../doc-guide/index.html#doc-guide)
...
[Documentation/dev-tools/clang-format.rst](../../../dev-tools/clang-format.html#clangformat) para más
detalles.
...
consulte el archivo [Kconfig Language](../../../kbuild/kconfig-language.html).
...
mm\_users y mm\_count), y en código del sistema de archivos
(`struct super_block`: s\_count y s\_active).
...
\_\_foo\_ret colisione (coincida) con una variable existente.
...
Hay varias modelos de macros de diagnóstico de driver en <linux/dev\_printk.h>
...
[`pr_notice()`](../../../core-api/printk-basics.html#c.pr_notice "pr_notice"), [`pr_info()`](../../../core-api/printk-basics.html#c.pr_info "pr_info"), [`pr_warn()`](../../../core-api/printk-basics.html#c.pr_warn "pr_warn"), [`pr_err()`](../../../core-api/printk-basics.html#c.pr_err "pr_err"), etc.
...
[`pr_debug()`](../../../core-api/printk-basics.html#c.pr_debug "pr_debug") no lo hace; se compila fuera por defecto, a menos que DEBUG sea
definido o se establezca CONFIG\_DYNAMIC\_DEBUG. Eso es cierto para `dev_dbg()`
también, y una convención relacionada usa VERBOSE\_DEBUG para agregar
...
relacionada con la depuración, printk(KERN\_DEBUG ...) puede ser usado.

## 14) Reservando memoria

El kernel proporciona los siguientes asignadores de memoria de propósito
general: [`kmalloc()`](../../../core-api/mm-api.html#c.kmalloc "kmalloc"), [`kzalloc()`](../../../core-api/mm-api.html#c.kzalloc "kzalloc"), [`kmalloc_array()`](../../../core-api/mm-api.html#c.kmalloc_array "kmalloc_array"), [`kcalloc()`](../../../core-api/mm-api.html#c.kcalloc "kcalloc"), [`vmalloc()`](../../../core-api/mm-api.html#c.vmalloc "vmalloc") y
[`vzalloc()`](../../../core-api/mm-api.html#c.vzalloc "vzalloc"). Consulte la documentación de la API para obtener más información.
a cerca de ellos. [Documentation/core-api/memory-allocation.rst](../../../core-api/memory-allocation.html#memory-allocation)
...
(” stack dump”) en caso de fallo cuando se usan sin \_\_GFP\_NOWARN, por lo
...
este último caso, véase la función en línea [`kmalloc()`](../../../core-api/mm-api.html#c.kmalloc "kmalloc").
...
función [`pci_dev_present()`](../../../driver-api/pci/pci.html#c.pci_dev_present "pci_dev_present") devuelve 1 si tiene éxito en encontrar un
...
punteros; estos usan NULL o el mecanismo ERR\_PTR para informar de fallos.
...
definición sin usar, marque la definición como \_\_maybe\_unused en lugar de
envolverla en un preprocesador condicional. (Sin embargo, si una función o
variable *siempre* acaba sin ser usada, bórrela.)

Dentro del código, cuando sea posible, use la macro IS\_ENABLED para
...
### Use WARN\_ON\_ONCE() en lugar de WARN() o WARN\_ON()
...
### No se preocupe sobre panic\_on\_warn de usuarios

Algunas palabras más sobre panic\_on\_warn: Recuerde que `panic_on_warn` es
...
ligera”, arriba. Sin embargo, la existencia de panic\_on\_warn de usuarios no
es una razón válida para evitar el uso juicioso de WARN\*(). Esto se debe a
que quien habilita panic\_on\_warn, explícitamente pidió al kernel que
...
### Use BUILD\_BUG\_ON() para aserciones en tiempo de compilación
...
[process/coding-style.rst](../../../process/coding-style.html#codingstyle) del kernel, por [greg@kroah.com](mailto:greg%40kroah.com) at OLS 2002:
