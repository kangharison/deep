...
> 출처(원문): https://docs.kernel.org/translations/sp_SP/scheduler/sched-bwc.html
...
para SCHED\_NORMAL. El caso de SCHED\_RT se trata en [Real-Time group scheduling](../../../scheduler/sched-rt-group.html)

El control de ancho de banda es una extensión CONFIG\_FAIR\_GROUP\_SCHED que
...
restablecidas cfs\_quota unidades al final de cada periodo. Según los
...
consuma totalmente la cuota; esto permite que se pueda describir u\_i
como una distribución estadística.

Por ejemplo, se tiene u\_i = {x,e}\_i, donde x es el p(95) y x+e p(100)
...
tiempo límite de ejecución será Sum e\_i; esto es una retraso acotado
...
v1. Para cgroup v2, ver [Documentation/admin-guide/cgroup-v2.rst](../../../admin-guide/cgroup-v2.html#cgroup-v2-cpu).

* cpu.cfs\_quota\_us: tiempo de ejecución que se refresca cada periodo (en microsegundos)
* cpu.cfs\_period\_us: la duración del periodo (en microsegundos)
* cpu.stat: exporta las estadísticas de limitación [explicado a continuación]
* cpu.cfs\_burst\_us: el máximo tiempo de ejecución acumulado (en microsegundos)
...
Un valor de -1 para cpu.cfs\_quota\_us indica que el grupo no tiene ninguna
...
Asignar cualquier valor (válido) y positivo no menor que cpu.cfs\_burst\_us
...
Asignar cualquier valor negativo a cpu.cfs\_quota\_us eliminará el límite de
ancho de banda y devolverá de nuevo al grupo a un estado sin restricciones.

Un valor de 0 para cpu.cfs\_burst\_us indica que el grupo no puede acumular
...
(válido) positivo no mayor que cpu.cfs\_quota\_us en cpu.cgs\_burst\_us definirá
...
* nr\_periods: Número de intervalos aplicados que han pasado.
* nr\_throttled: Número de veces que el grupo ha sido restringido/limitado.
* throttled\_time: La duración de tiempo total (en nanosegundos) en las
  que las entidades del grupo han sido limitadas.
* nr\_bursts: Número de periodos en que ha ocurrido una ráfaga.
* burst\_time: Tiempo acumulado (en nanosegundos) en la que una CPU ha
...
sea siempre factible, esto es: max(c\_i) <= C. De todas maneras,
...
> e.g. Sum (c\_i) puede superar C

[ Donde C es el ancho de banda de el padre, y c\_i el de su hijo ]
...
en el tiempo de compilación por la variable min\_cfs\_rq\_runtime. Esto es un
...
periodo. Como resultado se espera que nr\_periods sea aproximadamente igual
a nr\_throttled, y que cpuacct.usage se incremente aproximadamente igual
a cfs\_quota\_us en cada periodo.
...
min\_cfs\_rq\_runtime). Este pequeño sobreuso únicamente tiene lugar si
...
[Gestor de tareas CFS](sched-design-CFS.html)) el que decida qué aplicación es la elegida para
