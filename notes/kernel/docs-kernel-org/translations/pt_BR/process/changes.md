...
> 출처(원문): https://docs.kernel.org/translations/pt_BR/process/changes.html
...
| Sphinx[[1]](#f1) | 3.4.3 | sphinx-build --version |
...
O Sphinx é necessário apenas para gerar a documentação do Kernel.
...
ref:Building Linux with Clang/LLVM <kbuild\_llvm>.

#### Rust (opcional)

É necessária uma versão recente do compilador Rust.

Por favor, consulte [Quick Start](../../../rust/quick-start.html) para obter instruções
...
Desde o Linux 5.2, se CONFIG\_DEBUG\_INFO\_BTF estiver selecionado, o sistema de
...
padrão (defconfigs) de arm/arm64, CONFIG\_LTO\_CLANG, algumas configurações
...
kernel via sysfs (CONFIG\_IKHEADERS).
...
módulos integrados (CONFIG\_BUILTIN\_MODULE\_RANGES).
...
necessário. É geralmente preferível compilar o kernel com CONFIG\_KALLSYMS para
...
Se por algum motivo o seu kernel não for compilado com CONFIG\_KALLSYMS e você
...
* Para o seu arquivo [Requisitos mínimos para compilar o Kernel](#), a tradução técnica adequada é:
...
mas as opções de montagem direct\_io e kernel\_cache não funcionarão.
...
eventos de machine check quando opção CONFIG\_X86\_MCE está ativada. Eventos de
...
Por favor, consulte [Using Sphinx for kernel documentation](../../../doc-guide/sphinx.html) para detalhes sobre os
requisitos do Sphinx.

#### rustdoc

O rustdoc é utilizado para gerar a documentação para código Rust. Por favor,
consulte [General Information](../../../rust/general-information.html) para mais informações.
...
* [Getting LLVM](../../../kbuild/llvm.html#getting-llvm).

#### Rust

* [Quick Start](../../../rust/quick-start.html).
