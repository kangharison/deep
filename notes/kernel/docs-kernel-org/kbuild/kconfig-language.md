...
> 출처(원문): https://docs.kernel.org/kbuild/kconfig-language.html
...
>     4. Hardware or infrastructure that everybody expects, such as CONFIG\_NET
>        or CONFIG\_BLOCK. These are rare exceptions.
...
* transitional attribute: “transitional”
  ...
  Example migration from OLD\_NAME to NEW\_NAME:
  ...
  With this setup, existing .config files with “CONFIG\_OLD\_NAME=y” will
  result in “CONFIG\_NEW\_NAME=y” being set, while CONFIG\_OLD\_NAME will be
...
The recommended way to do so is to use a config variable named HAVE\_\*
...
config variable to select HAVE\_GENERIC\_IOMAP.

Note: the use of the internal config variable HAVE\_GENERIC\_IOMAP, it is
introduced to overcome the limitation of select which will force a
config option to ‘y’ no matter the dependencies.
The dependencies are moved to the symbol GENERIC\_IOMAP and we avoid the
...
CC\_HAS\_ is the recommended prefix for the config option:
...
increase build coverage by adding an “|| COMPILE\_TEST” clause to the
...
platform (e.g. ARCH\_OMAP4) dependency. This makes life simpler not only for
...
>     depends on ARCH\_FOO\_VENDOR || COMPILE\_TEST
...
discarded, when CONFIG\_BAR=m and this code is built-in. This is not what users
...
of the “select CORE” from CORE\_BELL\_A\_ADVANCED as that is implicit already
since CORE\_BELL\_A depends on CORE. At times it may not be possible to remove
...
one study has completed a broad analysis of Kconfig use in 12 projects [[0]](#id10).
Despite its widespread use, and although this document does a reasonable job
in documenting basic Kconfig syntax a more precise definition of Kconfig
semantics is welcomed. One project deduced Kconfig semantics through
the use of the xconfig configurator [[1]](#id11). Work should be done to confirm if
the deduced semantics matches our intended Kconfig design goals.
Another project formalized a denotational semantics of a core subset of
the Kconfig language [[10]](#id26).
...
Linux using this methodology [[1]](#id11) (Section 8: Threats to validity).
The kismet tool, based on the semantics in [[10]](#id26), finds abuses of reverse
dependencies and has led to dozens of committed fixes to Linux Kconfig files [[11]](#id27).

Confirming this could prove useful as Kconfig stands as one of the leading
industrial variability modeling languages [[1]](#id11) [[2]](#id12). Its study would help
evaluate practical uses of such languages, their use was only theoretical
and real world requirements were not well understood. As it stands though
only reverse engineering techniques have been used to deduce semantics from
variability modeling languages such as Kconfig [[3]](#id13).
...
Although SAT solvers [[4]](#id20) haven’t yet been used by Kconfig directly, as noted
in the previous subsection, work has been done however to express in boolean
abstraction the inferred semantics of Kconfig to translate Kconfig logic into
boolean formulas and run a SAT solver on it [[5]](#id21). Another known related project
is CADOS [[6]](#id22) (former VAMOS [[7]](#id23)) and the tools, mainly undertaker [[8]](#id24), which
has been introduced first with [[9]](#id25). The basic concept of undertaker is to
