...
> 출처(원문): https://docs.kernel.org/kbuild/gendwarfksyms.html
...
When CONFIG\_MODVERSIONS is enabled, symbol versions for modules
...
the resulting ABI. With CONFIG\_GENDWARFKSYMS (and CONFIG\_DEBUG\_INFO)
...
\_\_gendwarfksyms\_ptr\_, followed by the name of the exported symbol.
...
symtypes files during a kernel build, set KBUILD\_SYMTYPES=1.
...
the traditional #ifndef \_\_GENKSYMS\_\_ to hide these changes from symbol
...
build, set KBUILD\_GENDWARFKSYMS\_STABLE=1.
...
.discard.gendwarfksyms.kabi\_rules section of the object file and
...
Using the \_\_KABI\_RULE macro, this rule can be defined as:
...
enumerators without changing symbol versions. The enumerator\_ignore
rule allows us to hide named enumerators from the input.

The rule fields are expected to be as follows:

* type: “enumerator\_ignore”
...
enumerator when calculating versions. The enumerator\_value rule allows
us to override the value of an enumerator for version calculation:

* type: “enumerator\_value”
* target: The fully qualified name of the target enum
  (as shown in **--dump-dies** output) and the name of the
  enumerator field separated by a space.
...
described in section [Hiding members](#hiding-members), but we can’t
hide the increase in structure size. The byte\_size rule allows us to
...
targeted kABI rules can become tedious. The type\_string rule allows us
...
The type\_string rule should be used only as a last resort if maintaining
...
start with \_\_kabi\_ are left out when calculating symbol versions:
...
If the \_\_kabi\_ naming scheme was used when reserving space, the name
of the first member of the `union must` start with \_\_kabi\_reserved. This
...
\_\_kabi\_renamed followed by the original name.

The examples include KABI\_(RESERVE|USE|REPLACE)\* macros that help
...
examples include a KABI\_IGNORE macro to simplify the code.
