...
> 출처(원문): https://docs.kernel.org/process/threat-model.html
...
([security-related ones](security-bugs.html) vs [non-security ones](../admin-guide/reporting-issues.html)), and can complicate security enforcement
...
* **Capability-based protection**:
  ...
  + users not having elevated capabilities (including but not limited to
    CAP\_SYS\_ADMIN) may not alter the
...
* **Configuration**:
  ...
  + issues triggered only when using features intended for development or
    debugging (e.g., LOCKDEP, KASAN, FAULT\_INJECTION): these features are known
    to introduce overhead and potential instability and are not intended for
    production use.
  + issues affecting drivers exposed under CONFIG\_STAGING, as well as features
