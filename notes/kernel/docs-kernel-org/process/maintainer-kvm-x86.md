...
> 출처(원문): https://docs.kernel.org/process/maintainer-kvm-x86.html
...
[Coding style notes](maintainer-tip.html#maintainer-tip-coding-style), as patches/series often touch both KVM and
...
See [The canonical patch format](submitting-patches.html#the-canonical-patch-format) for more information, with one amendment:
...
See [Describe your changes](submitting-patches.html#describe-changes) for more information, with one amendment: lead with
...
that primarily target arch/x86 code that is \_NOT\_ KVM code.
...
At a bare minimum, *all* patches in a series must build cleanly for KVM\_INTEL=m
KVM\_AMD=m, and KVM\_WERROR=y. Building every possible combination of Kconfigs
isn’t feasible, but the more the merrier. KVM\_SMM, KVM\_XEN, PROVE\_LOCKING, and
X86\_64 are particularly interesting knobs to turn.
...
feature via KVM\_GET\_SUPPORTED\_CPUID, i.e. for instructions/features that KVM
...
[Security bugs](security-bugs.html#securitybugs) if you suspect a bug can lead to an escape, data leak, etc.
