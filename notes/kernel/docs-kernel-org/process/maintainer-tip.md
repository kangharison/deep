...
> 출처(원문): https://docs.kernel.org/process/maintainer-tip.html
...
The general rules about changelogs in the [Submitting patches guide](submitting-patches.html#describe-changes), apply.
...
See [Backtraces in commit messages](submitting-patches.html#backtraces).
...
See [Don’t get discouraged - or impatient](submitting-patches.html#resend-reminders).
...
So called \_urgent\_ branches will be merged into mainline during the
...
These debugging options can be found in kernel/configs/x86\_debug.config
and can be added to an existing kernel config by running:

> make x86\_debug.config
...
> ```
>
> In PROVE\_LOCKING kernels, `lockdep_assert_held()` emits a warning
...
combine the subsystem and the component name such as ‘x86\_comp\_’,
...
Please note that ‘xxx\_vendor\_’ and ‘vendor\_xxx\_` prefixes are not
