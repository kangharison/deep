...
> 출처(원문): https://docs.kernel.org/process/debugging/kgdb.html
...
KGDB\_KDB: include kdb frontend for kgdb in the config menu.
...
`CONFIG_KDB_KEYBOARD` which is called KGDB\_KDB: keyboard as
...
The `kgdbcon` feature allows you to see [`printk()`](../../core-api/printk-basics.html#c.printk "printk") messages inside gdb
...
int kgdb\_skipexception(int exception, struct pt\_regs \*regs)
...
void pt\_regs\_to\_gdb\_regs(unsigned long \*gdb\_regs, struct pt\_regs \*regs)
...
> Convert the pt\_regs in **regs** into the format for registers that
> GDB expects, stored in **gdb\_regs**.

void sleeping\_thread\_to\_gdb\_regs(unsigned long \*gdb\_regs, struct task\_struct \*p)
:   Convert ptrace regs to GDB regs
...
> **gdb\_regs** with what has been saved in `struct thread_struct`
> thread field during switch\_to.

void gdb\_regs\_to\_pt\_regs(unsigned long \*gdb\_regs, struct pt\_regs \*regs)
...
> Convert the GDB regs in **gdb\_regs** into the pt\_regs, and store them
> in **regs**.

int kgdb\_arch\_handle\_exception(int vector, int signo, int err\_code, char \*remcom\_in\_buffer, char \*remcom\_out\_buffer, struct pt\_regs \*regs)
...
void kgdb\_arch\_handle\_qxfer\_pkt(char \*remcom\_in\_buffer, char \*remcom\_out\_buffer)
...
void kgdb\_call\_nmi\_hook(void \*ignored)
...
> If you’re using the default implementation of [`kgdb_roundup_cpus()`](#c.kgdb_roundup_cpus "kgdb_roundup_cpus")
> this function will be called per CPU. If you don’t implement
> [`kgdb_call_nmi_hook()`](#c.kgdb_call_nmi_hook "kgdb_call_nmi_hook") a default will be used.

void kgdb\_roundup\_cpus(void)
...
void kgdb\_arch\_set\_pc(struct pt\_regs \*regs, unsigned long pc)
...
callbacks in the [`struct uart_ops`](../../driver-api/serial/driver.html#c.uart_ops "uart_ops").
...
kdb\_poll\_funcs[]. The `kdb_get_kbd_char()` is the top-level
...
`kgdboc_pre_exp_handler()` which in turn calls [`con_debug_enter()`](../../driver-api/tty/console.html#c.con_debug_enter "con_debug_enter")
in the virtual console layer. On resuming kernel execution, the kernel
debugger calls `kgdboc_post_exp_handler()` which in turn calls
[`con_debug_leave()`](../../driver-api/tty/console.html#c.con_debug_leave "con_debug_leave").
