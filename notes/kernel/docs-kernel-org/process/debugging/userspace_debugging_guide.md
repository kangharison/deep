...
> 출처(원문): https://docs.kernel.org/process/debugging/userspace_debugging_guide.html
> 자동 미러링: docs.kernel.org · 정본은 출처 URL (영문 원문 자동 변환본)

---

# [Userspace debugging advice](#id1)

This document provides a brief overview of common tools to debug the Linux
Kernel from userspace.
For debugging advice aimed at driver developers go [here](driver_development_debugging_guide.html).
For general debugging advice, see [general advice document](index.html).
...
* [`pr_debug()`](../../core-api/printk-basics.html#c.pr_debug "pr_debug")
* `dev_dbg()`
...
Here is one example, that enables all available [`pr_debug()`](../../core-api/printk-basics.html#c.pr_debug "pr_debug")’s within the file:
...
* When the code contains one of the valid print statements (see above) or when
  you have added multiple [`pr_debug()`](../../core-api/printk-basics.html#c.pr_debug "pr_debug") statements during development
* When timing is not an issue, meaning if multiple [`pr_debug()`](../../core-api/printk-basics.html#c.pr_debug "pr_debug") statements in
...
For the full documentation see [Dynamic debug](../../admin-guide/dynamic-debug-howto.html)
...
* You can perform a function trace by adding a function name to the
  ...
  at: [dynamic ftrace](../../trace/ftrace.html#dynamic-ftrace)).
...
* Tracing the children of a function call and showing the return values are
  ...
  [Dynamic ftrace with the function graph tracer](../../trace/ftrace.html#dynamic-ftrace-with-the-function-graph-tracer).

For the full Ftrace documentation see [ftrace - Function Tracer](../../trace/ftrace.html)

Or you could also trace for specific events by [using event tracing](../../trace/events.html#using-event-tracing), which can be defined as described here:
[Creating a custom Ftrace tracepoint](driver_development_debugging_guide.html#ftrace).

For the full Ftrace event tracing documentation see [Event Tracing](../../trace/events.html)

### [Reading the ftrace log](#id4)
...
[trace\_pipe](../../trace/ftrace.html#trace-pipe) will behave similarly to the `trace` file, but
...
### [Why should you do a performance analysis?](#id7)
...
### [How to do a simple analysis with linux tools?](#id8)
...
## [Kernel panic analysis tools](#id11)

> To capture the crash dump please use `Kdump` & `Kexec`. Below you can find
> some advice for analysing the data.
>
> For the full documentation see the [Documentation for Kdump - The kexec-based Crash Dumping Solution](../../admin-guide/kdump/kdump.html)
>
