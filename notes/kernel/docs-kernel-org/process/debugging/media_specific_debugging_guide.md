...
> 출처(원문): https://docs.kernel.org/process/debugging/media_specific_debugging_guide.html
> 자동 미러링: docs.kernel.org · 정본은 출처 URL (영문 원문 자동 변환본)

---

# [Debugging and tracing in the media subsystem](#id1)
...
## [General debugging advice](#id2)

For general advice see the [general advice document](index.html).

The following sections show you some of the available tools.

## [dev\_debug module parameter](#id3)
...
For the full documentation see [video device debugging](../../driver-api/media/v4l2-dev.html#video-device-debugging)

## [dev\_dbg() / v4l2\_dbg()](#id4)
...
[printk() & friends](driver_development_debugging_guide.html#printk-friends)
guide.

* Difference between both?

  + `v4l2_dbg()` utilizes `v4l2_printk()` under the hood, which further uses
    [`printk()`](../../core-api/printk-basics.html#c.printk "printk") directly, thus it cannot be targeted by dynamic debug
  + `dev_dbg()` can be targeted by dynamic debug
  + `v4l2_dbg()` has a more specific prefix format for the media subsystem, while
    dev\_dbg only highlights the driver name and the location of the log
...
[Dynamic debug](userspace_debugging_guide.html#dynamic-debug) guide.
...
For general advice see the
[Ftrace](userspace_debugging_guide.html#ftrace) guide.
...
For general advice see the
[DebugFS](driver_development_debugging_guide.html#debugfs) guide.

## [Perf & alternatives](#id8)

Tools to measure the various stats on a running system to diagnose issues.

For general advice see the
[Perf & alternatives](userspace_debugging_guide.html#perf-alternatives) guide.
...
## [Error checking & panic analysis](#id9)

Various Kernel configuration options to enhance error detection of the Linux
Kernel with the cost of lowering performance.

For general advice see the
[KASAN, UBSAN, lockdep and other error checkers](driver_development_debugging_guide.html#kasan-ubsan-lockdep-and-other-error-checkers) guide.

## [Driver verification with v4l2-compliance](#id10)

To verify, that a driver adheres to the v4l2 API, the tool v4l2-compliance is
used, which is part of the [v4l\_utils](https://git.linuxtv.org/v4l-utils.git), a suite of userspace tools to work
...
## [Debugging problems with receiving video](#id11)

Implementing vidioc\_log\_status in the driver: this can log the current status
