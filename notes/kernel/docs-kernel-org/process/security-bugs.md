...
> 출처(원문): https://docs.kernel.org/process/security-bugs.html
...
[Reporting issues](../admin-guide/reporting-issues.html) if you are unclear about what
...
>   immediately merged (see [Submitting patches: the essential guide to getting your code into the kernel](submitting-patches.html)).
...
[The Linux Kernel threat model](threat-model.html), and ought to have been sent through
the normal channels described in [Reporting issues](../admin-guide/reporting-issues.html)
instead.
...
security officers use) is to rely on the get\_maintainer.pl script, tuned to
...
>   of the kernel’s threat model (see [The Linux Kernel threat model](threat-model.html))
...
>   “this bug permits any user to gain CAP\_NET\_ADMIN”) without enumerating
...
>   [Submitting patches: the essential guide to getting your code into the kernel](submitting-patches.html) and include a ‘Fixes:’ tag
...
[regular patch submission](submitting-patches.html)
...
text by default, please consult [Email clients info for Linux](email-clients.html) for
more info.
...
assignment team](cve.html) to obtain one.
