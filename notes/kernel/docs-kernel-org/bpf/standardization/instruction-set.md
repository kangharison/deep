# 1BPF Instruction Set Architecture (ISA)

> 출처(원문): https://docs.kernel.org/bpf/standardization/instruction-set.html
> 자동 미러링: docs.kernel.org · 정본은 출처 URL (영문 원문 자동 변환본)

---

# [1 BPF Instruction Set Architecture (ISA)](#id13)
...
## [1.1 Documentation conventions](#id14)
...
* atomic32: includes 32-bit atomic operation instructions (see [Atomic operations](#atomic-operations)).
...
### [1.2.1 Basic instruction encoding](#id20)
...
:   operation to perform, encoded as follows:
    ...
    **class**
    :   The instruction class (see [Instruction classes](#instruction-classes))
...
:   The source and destination register numbers, encoded as follows
    ...
    **src\_reg**
    :   the source register number (0-10), except where otherwise specified
        ([64-bit immediate instructions](#bit-immediate-instructions) reuse this field for other purposes)
...
### [1.2.2 Wide instruction encoding](#id21)
...
with ‘opcode’, ‘dst\_reg’, ‘src\_reg’, and ‘offset’ all set to zero.
...
### [1.2.3 Instruction classes](#id22)
...
| --- | --- | --- | --- |
| LD | 0x0 | non-standard load operations | [Load and store instructions](#load-and-store-instructions) |
| LDX | 0x1 | load into register operations | [Load and store instructions](#load-and-store-instructions) |
| ST | 0x2 | store from immediate operations | [Load and store instructions](#load-and-store-instructions) |
| STX | 0x3 | store from register operations | [Load and store instructions](#load-and-store-instructions) |
| ALU | 0x4 | 32-bit arithmetic operations | [Arithmetic and jump instructions](#arithmetic-and-jump-instructions) |
| JMP | 0x5 | 64-bit jump operations | [Arithmetic and jump instructions](#arithmetic-and-jump-instructions) |
| JMP32 | 0x6 | 32-bit jump operations | [Arithmetic and jump instructions](#arithmetic-and-jump-instructions) |
| ALU64 | 0x7 | 64-bit arithmetic operations | [Arithmetic and jump instructions](#arithmetic-and-jump-instructions) |

## [1.3 Arithmetic and jump instructions](#id23)
...
:   the source operand location, which unless otherwise specified is one of:
    ...
    | X | 1 | use ‘src\_reg’ register value as source operand |

**instruction class**
:   the instruction class (see [Instruction classes](#instruction-classes))

### [1.3.1 Arithmetic instructions](#id24)
...
| SDIV | 0x3 | 1 | dst = (src == 0) ? 0 : ((src == -1 && dst == LLONG\_MIN) ? LLONG\_MIN : (dst s/ src)) |
| OR | 0x4 | 0 | dst |= src |
| AND | 0x5 | 0 | dst &= src |
...
| SMOD | 0x9 | 1 | dst = (src == 0) ? dst : ((src == -1 && dst == LLONG\_MIN) ? 0: (dst s% src)) |
| XOR | 0xa | 0 | dst ^= src |
| MOV | 0xb | 0 | dst = src |
| MOVSX | 0xb | 8/16/32 | dst = (s8,s16,s32)src |
| ARSH | 0xc | 0 | [sign extending](#term-Sign-Extend) dst >>= (src & mask) |
| END | 0xd | 0 | byte swap operations (see [Byte swap instructions](#byte-swap-instructions) below) |
...
‘imm’ is first [sign extended](#term-Sign-Extend) from 32 to 64 bits, and then
interpreted as a 64-bit unsigned value.

For signed operations (`SDIV` and `SMOD`), for `ALU`,
‘imm’ is interpreted as a 32-bit signed value. For `ALU64`, ‘imm’
is first [sign extended](#term-Sign-Extend) from 32 to 64 bits, and then
...
`{MOVSX, X, ALU}` [sign extends](#term-Sign-Extend) 8-bit and 16-bit operands into
32-bit operands, and zeroes the remaining upper 32 bits.
`{MOVSX, X, ALU64}` [sign extends](#term-Sign-Extend) 8-bit, 16-bit, and 32-bit
...
### [1.3.2 Byte swap instructions](#id25)
...
| code | value | src\_reg | description | notes |
| --- | --- | --- | --- | --- |
...
| CALL | 0x8 | 0x0 | call helper function by static ID | {CALL, K, JMP} only, see [Helper functions](#helper-functions) |
| CALL | 0x8 | 0x1 | call PC += imm | {CALL, K, JMP} only, see [Program-local functions](#program-local-functions) |
| CALL | 0x8 | 0x2 | call helper function by BTF ID | {CALL, K, JMP} only, see [Helper functions](#helper-functions) |
...
#### [1.3.3.2 Program-local functions](#id28)
...
## [1.4 Load and store instructions](#id29)
...
:   The mode modifier is one of:
    ...
    | --- | --- | --- | --- |
    | IMM | 0 | 64-bit immediate instructions | [64-bit immediate instructions](#bit-immediate-instructions) |
    | ABS | 1 | legacy BPF packet access (absolute) | [Legacy BPF Packet access instructions](#legacy-bpf-packet-access-instructions) |
    | IND | 2 | legacy BPF packet access (indirect) | [Legacy BPF Packet access instructions](#legacy-bpf-packet-access-instructions) |
    | MEM | 3 | regular load and store operations | [Regular load and store operations](#regular-load-and-store-operations) |
    | MEMSX | 4 | sign-extension load operations | [Sign-extension load operations](#sign-extension-load-operations) |
    | ATOMIC | 6 | atomic operations | [Atomic operations](#atomic-operations) |
...
### [1.4.1 Regular load and store operations](#id30)
...
### [1.4.2 Sign-extension load operations](#id31)

The `MEMSX` mode modifier is used to encode [sign-extension](#term-Sign-Extend) load
instructions that transfer data between a register and memory.
...
### [1.4.4 64-bit immediate instructions](#id33)

Instructions with the `IMM` ‘mode’ modifier use the wide instruction
encoding defined in [Instruction encoding](#instruction-encoding), and use the ‘src\_reg’ field of the
basic instruction to hold an opcode subtype.

The following table defines a set of `{IMM, DW, LD}` instructions
with opcode subtypes in the ‘src\_reg’ field, using new terms such as “map”
defined further below:

64-bit immediate instructions

| src\_reg | pseudocode | imm type | dst type |
| --- | --- | --- | --- |
| 0x0 | dst = (next\_imm << 32) | imm | integer | integer |
| 0x1 | dst = map\_by\_fd(imm) | map fd | map |
| 0x2 | dst = map\_val(map\_by\_fd(imm)) + next\_imm | map fd | data address |
| 0x3 | dst = var\_addr(imm) | variable id | data address |
| 0x4 | dst = code\_addr(imm) | integer | code address |
| 0x5 | dst = map\_by\_idx(imm) | map index | map |
| 0x6 | dst = map\_val(map\_by\_idx(imm)) + next\_imm | map index | data address |

where

* map\_by\_fd(imm) means to convert a 32-bit file descriptor into an address of a map (see [Maps](#maps))
...
* var\_addr(imm) gets the address of a platform variable (see [Platform Variables](#platform-variables)) with a given id
...
may not have a single contiguous memory region, but the ‘map\_val(map)’ is
currently only defined for maps that do have a single contiguous memory region.

Each map can have a file descriptor (fd) if supported by the platform, where
‘map\_by\_fd(imm)’ means to get the map with the specified file descriptor. Each
BPF program can also be defined to use a set of maps associated with the
program at load time, and ‘map\_by\_idx(imm)’ means to get the map with the given
index in the set associated with the BPF program containing the instruction.

#### [1.4.4.2 Platform Variables](#id35)

Platform variables are memory regions, identified by integer ids, exposed by
the runtime and accessible by BPF programs on some platforms. The
‘var\_addr(imm)’ operation means to get the address of the memory region
identified by the given id.

### [1.4.5 Legacy BPF Packet access instructions](#id36)

BPF previously introduced special instructions for access to packet data that were
carried over from classic BPF. These instructions used an instruction
class of `LD`, a size modifier of `W`, `H`, or `B`, and a
mode modifier of `ABS` or `IND`. The ‘dst\_reg’ and ‘offset’ fields were
set to zero, and ‘src\_reg’ was set to zero for `ABS`. However, these
