...
> 출처(원문): https://docs.kernel.org/bpf/prog_sk_lookup.html
> 자동 미러링: docs.kernel.org · 정본은 출처 URL (영문 원문 자동 변환본)

---

# BPF sk\_lookup program

BPF sk\_lookup program type (`BPF_PROG_TYPE_SK_LOOKUP`) introduces programmability
into the socket lookup performed by the transport layer when a packet is to be
delivered locally.

When invoked BPF sk\_lookup program can select a socket that will receive the
...
BPF sk\_lookup program type was introduced to address setup scenarios where
...
BPF sk\_lookup program can be attached to a network namespace with
...
The attached BPF sk\_lookup programs run whenever the transport layer needs to
...
as usual without triggering the BPF sk\_lookup hook.
...
A BPF sk\_lookup program can also select a socket to receive the packet by
...
In its context, an instance of `struct bpf_sk_lookup`, BPF sk\_lookup program
