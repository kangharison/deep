...
> 출처(원문): https://docs.kernel.org/bpf/graph_ds_impl.html
...
structures (linked\_list, rbtree), with particular focus on the verifier’s
...
Two such data structures - linked\_list and rbtree - have many verification
details in common. Because both have “root”s (“head” for linked\_list) and
“node”s, the verifier code and this document refer to common functionality
as “graph\_api”, “graph\_root”, “graph\_node”, etc.
...
The “root” type for both linked\_list and rbtree expects to be in a map\_value
...
spin\_lock to be associated with the `bpf_rb_root` by virtue of both being in
the same map\_value and will enforce that the correct lock is held when
...
between spin\_lock and spin\_unlock. Why? After spin\_unlock another program
can do arbitrary operations on the data structure like removing and `free`-ing
via bpf\_obj\_drop. A non-owning ref to some chunk of memory that was remove’d,
`free`’d, and reused via bpf\_obj\_new would point to an entirely different thing.
...
Any reference to a node that is in an rbtree \_must\_ be non-owning, since
the tree has control of the pointee’s lifetime. Similarly, any ref to a node
that isn’t in rbtree \_must\_ be owning. This results in a nice property:
