...
> 출처(원문): https://docs.kernel.org/bpf/bpf_design_QA.html
...
## [Questions and Answers](#id1)

### [Q: Is BPF a generic instruction set similar to x64 and arm64?](#id2)

A: NO.

### [Q: Is BPF a generic virtual machine ?](#id3)

A: NO.

### [BPF is generic instruction set *with* C calling convention.](#id4)

#### [Q: Why C calling convention was chosen?](#id5)
...
#### [Q: Can multiple return values be supported in the future?](#id6)

A: NO. BPF allows only register R0 to be used as return value.

#### [Q: Can more than 5 function arguments be supported in the future?](#id7)

A: NO. BPF calling convention only allows registers R1-R5 to be used
as arguments. BPF is not a standalone instruction set.
(unlike x64 ISA that allows msft, cdecl and other conventions)

### [Q: Can BPF programs access instruction pointer or return address?](#id8)

A: NO.

### [Q: Can BPF programs access stack pointer ?](#id9)
...
### [Q: Does C-calling convention diminishes possible use cases?](#id10)
...
### [Q: Does it mean that ‘innovative’ extensions to BPF code are disallowed?](#id11)
...
### [Q: Can loops be supported in a safe way?](#id12)
...
### [Q: What are the verifier limits?](#id13)

A: The only limit known to the user space is BPF\_MAXINSNS (4096).
...
expressions. Now it can recognize pointer + bounded\_register.
bpf\_lookup\_map\_elem(key) had a requirement that ‘key’ must be
...
### [Instruction level questions](#id14)

#### [Q: LD\_ABS and LD\_IND instructions vs C code](#id15)

Q: How come LD\_ABS and LD\_IND instruction are present in BPF whereas
...
#### [Q: BPF instructions mapping not one-to-one to native CPU](#id16)

Q: It seems not all BPF instructions are one-to-one to native CPU.
For example why BPF\_JNE and other compare and jumps are not cpu-like?

A: This was necessary to avoid introducing flags into ISA which are
impossible to make generic and efficient across CPU architectures.

#### [Q: Why BPF\_DIV instruction doesn’t map to x64 div?](#id17)

A: Because if we picked one-to-one relationship to x64 it would have made
it more complicated to support on arm64 and other archs. Also it
needs div-by-zero runtime check.

#### [Q: Why BPF has implicit prologue and epilogue?](#id18)

A: Because architectures like sparc have register windows and in general
there are enough subtle differences between architectures, so naive
store return address into stack won’t work. Another reason is BPF has
to be safe from division by zero (and legacy exception path
of LD\_ABS insn). Those instructions need to invoke epilogue and
return implicitly.

#### [Q: Why BPF\_JLT and BPF\_JLE instructions were not introduced in the beginning?](#id19)
...
#### [Q: BPF 32-bit subregister requirements](#id20)
...
### [Q: Does BPF have a stable ABI?](#id21)
...
lifecycle expectations as described in [3. kfunc lifecycle expectations](kfuncs.html#bpf-kfunc-lifecycle-expectations).

### [Q: Are tracepoints part of the stable ABI?](#id22)

A: NO. Tracepoints are tied to internal implementation details hence they are
subject to change and can break with newer kernels. BPF programs need to change
accordingly when this happens.

### [Q: Are places where kprobes can attach part of the stable ABI?](#id23)
...
### [Q: How much stack space a BPF program uses?](#id24)

A: Currently all program types are limited to 512 bytes of stack
space, but the verifier computes the actual amount of stack used
and both interpreter and most JITed code consume necessary amount.

### [Q: Can BPF be offloaded to HW?](#id25)

A: YES. BPF HW offload is supported by NFP driver.

### [Q: Does classic BPF interpreter still exist?](#id26)

A: NO. Classic BPF programs are converted into extend BPF instructions.

### [Q: Can BPF call arbitrary kernel functions?](#id27)
...
### [Q: Can BPF overwrite arbitrary kernel memory?](#id28)
...
### [Q: Can BPF overwrite arbitrary user memory?](#id29)
...
### [Q: New functionality via kernel modules?](#id30)
...
### [Q: Directly calling kernel function is an ABI?](#id31)

Q: Some kernel functions (e.g. tcp\_slow\_start) can be called
...
[3. kfunc lifecycle expectations](kfuncs.html#bpf-kfunc-lifecycle-expectations) for details.

### [Q: Attaching to arbitrary kernel functions is an ABI?](#id32)
...
### [Q: Marking a function with BTF\_ID makes that function an ABI?](#id33)

A: NO.

The BTF\_ID macro does not cause a function to become part of the ABI
any more than does the EXPORT\_SYMBOL\_GPL macro.

### [Q: What is the compatibility story for special BPF types in map values?](#id34)

Q: Users are allowed to embed bpf\_spin\_lock, bpf\_timer fields in their BPF map
values (when using BTF support for BPF maps). This allows to use helpers for
such objects on these fields inside map values. Users are also allowed to embed
pointers to some kernel types (with \_\_kptr\_untrusted and \_\_kptr BTF tags). Will the
kernel preserve backwards compatibility for these features?

A: It depends. For bpf\_spin\_lock, bpf\_timer: YES, for kptr and everything else:
NO, but see below.

For `struct types` that have been added already, like bpf\_spin\_lock and bpf\_timer,
the kernel will preserve backwards compatibility, as they are part of UAPI.

For kptrs, they are also part of UAPI, but only with respect to the kptr
mechanism. The types that you can use with a \_\_kptr\_untrusted and \_\_kptr tagged
...
### [Q: What is the compatibility story for special BPF types in allocated objects?](#id35)

Q: Same as above, but for allocated objects (i.e. objects allocated using
bpf\_obj\_new for user defined types). Will the kernel preserve backwards
