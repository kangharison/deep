...
> 출처(원문): https://docs.kernel.org/block/bfq-iosched.html
...
max\_budget. The other performance-related parameters have been
...
In particular, the tunables back\_seek-max, back\_seek\_penalty,
...
weight = (IOPRIO\_BE\_NR - ioprio) \* 10.
...
slice\_idle is a non-zero value. Idling has a double purpose: boosting
...
flash-based fast storage, setting slice\_idle=0 might end up in better
...
strict\_guarantees tunable, short-term service guarantees may be
...
on a non-rotational device, setting slice\_idle to 0 may be needed too
...
The default value is 0, which enables auto-tuning: BFQ sets max\_budget
...
[Block IO Controller](../admin-guide/cgroup-v1/blkio-controller.html). If, instead,
...
>     weight.
>
>     For cgroup v1, it is set by writing the value to blkio.bfq.weight\_device.
>
