...
> 출처(원문): https://docs.kernel.org/block/ublk.html
...
program. `ublksrv` [[1]](#userspace) is one of such implementation. It
provides `libublksrv` [[2]](#userspace-lib) library for developing specific
user block device conveniently, while also generic type block device is
included, such as loop and null. Richard W.M. Jones wrote userspace nbd device
`nbdublk` [[3]](#userspace-nbdublk) based on `libublksrv` [[2]](#userspace-lib).
...
`io_uring` passthrough command; that is why ublk is also one io\_uring based
...
See usage details in README of `ublksrv` [[4]](#userspace-readme).
...
* `UBLK_CMD_START_DEV`
  ...
  threads & io\_uring for handling ublk IO), this command is sent to the
...
* `UBLK_CMD_GET_DEV_INFO2`
  ...
  > UBLK\_F\_UNPRIVILEGED\_DEV isn’t available for user
...
thread should have its own io\_uring through which it is notified of new
...
* `UBLK_U_IO_NEED_GET_DATA`
  ...
  should not enable UBLK\_F\_NEED\_GET\_DATA. ublk server pre-allocates IO
...
#### Batch I/O Commands (UBLK\_F\_BATCH\_IO)
...
* **Multishot commands**: Use io\_uring multishot for reduced submission overhead
...
* `UBLK_U_IO_FETCH_IO_CMDS`
  ...
  **Multishot command** for fetching I/O commands in batch. This is the key
  ...
  + Uses io\_uring multishot capability for reduced submission overhead
  ...
  It is one typical multishot io\_uring request with provided buffer, and it
...
ublk zero copy relies on io\_uring’s fixed kernel buffer, which provides
two APIs: `io_buffer_register_bvec()` and io\_buffer\_unregister\_bvec.

ublk adds IO command of UBLK\_IO\_REGISTER\_IO\_BUF to call
`io_buffer_register_bvec()` for ublk server to register client request
buffer into io\_uring buffer table, then ublk server can submit io\_uring
IOs with the registered buffer index. IO command of UBLK\_IO\_UNREGISTER\_IO\_BUF
...
`io_buffer_unregister_bvec()`. Any io\_uring operation which supports this
kind of kernel buffer will grab one reference of the buffer until the
operation is completed.

ublk server implementing zero copy or user copy has to be CAP\_SYS\_ADMIN and
...
unnecessary IO split, which usually hurts io\_uring performance.
...
This feature automatically registers request buffers to the io\_uring context
before delivering I/O commands to the ublk server and unregisters them when
completing I/O commands. This eliminates the need for manual buffer
registration/unregistration via `UBLK_IO_REGISTER_IO_BUF` and
`UBLK_IO_UNREGISTER_IO_BUF` commands, then IO handling in ublk server
can avoid dependency on the two uring\_cmd operations.

IOs can’t be issued concurrently to io\_uring if there is any dependency
...
1. The ublk server must create a sparse buffer table on the same `io_ring_ctx`
   ...
   uring\_cmd is issued on a different `io_ring_ctx`, manual buffer
   unregistration is required.
2. Buffer registration data must be passed via uring\_cmd’s `sqe->addr` with the
   following structure:
...
2. If fallback is not enabled:
   ...
   * The uring\_cmd won’t be completed

#### Limitations

* Requires same `io_ring_ctx` for all operations
* May require manual buffer management in fallback cases
* io\_ring\_ctx buffer table has a max size of 16K, which may not be enough
  in case that too many ublk devices are handled by this single io\_ring\_ctx
  and each one has very large queue depth

### Shared Memory Zero Copy (UBLK\_F\_SHMEM\_ZC)
...
and the ublk server. Unlike the io\_uring fixed buffer approach above,
shared memory zero copy does not require io\_uring buffer registration
...
buffer directly, which is unlikely for the io\_uring fixed buffer
approach.
...
* **Database engines** that use pre-allocated buffer pools with O\_DIRECT
...
* **Direct buffer access**: the ublk server can read and write the shared
  buffer directly via its own mmap, without going through io\_uring fixed
...
* **Fast**: PFN matching is a single maple tree lookup per bvec. No
  io\_uring command round-trips for buffer management.
