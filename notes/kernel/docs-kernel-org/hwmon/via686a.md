...
> 출처(원문): https://docs.kernel.org/hwmon/via686a.html
...
|  |  |
| --- | --- |
| force\_addr=0xaddr | Set the I/O base address. Useful for boards that don’t set the address in the BIOS. Look for a BIOS upgrade before resorting to this. Does not do a PCI force; the via686a must still be present in lspci. Don’t use this unless the driver complains that the base address is not set. Example: ‘modprobe via686a force\_addr=0x6000’ |
...
So, if you need the force\_addr parameter, and end up with values which
