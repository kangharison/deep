...
> 출처(원문): https://docs.kernel.org/hwmon/max16065.html
...
the devices explicitly. Please see [How to instantiate I2C devices](../i2c/instantiating-devices.html) for
details.
...
| in12\_input | Voltage on CSP (Current Sense Positive) pin. Only if the chip supports current sensing and if current sensing is enabled. |
...
| curr1\_input | Current sense input; only if the chip supports current sensing and if current sensing is enabled. Displayed current assumes 0.001 Ohm current sense resistor. |
| curr1\_alarm | Overcurrent alarm; only if the chip supports current sensing and if current sensing is enabled. |
