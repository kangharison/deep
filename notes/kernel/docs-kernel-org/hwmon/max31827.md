...
> 출처(원문): https://docs.kernel.org/hwmon/max31827.html
...
below the TH\_HYST value or when shutdown mode is entered. Similarly, UT status
returns to 0 when the temperature rises above TL\_HYST value or when shutdown
mode is entered.
...
below TH\_HYST. The output remains asserted until it is reset by a read. It is
...
The conversion rate can be modified with update\_interval attribute of the chip.
Conversion/second = 1/update\_interval. Thus, the available options according to
the data sheet are:
...
There is a temp1\_resolution attribute which indicates the unit change in the
