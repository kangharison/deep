...
> 출처(원문): https://docs.kernel.org/hwmon/adt7410.html
...
maximum and critical temperature, it can only be set for temp#\_max\_hyst.
However, temp#\_min\_hyst and temp#\_crit\_hyst show their corresponding
hysteresis.
...
| temp#\_min\_hyst | hysteresis for temperature minimum (read-only) |
| temp#\_max\_hyst | hysteresis for temperature maximum (read/write) |
| temp#\_crit\_hyst | hysteresis for critical temperature (read-only) |
| temp#\_min\_alarm | temperature minimum alarm flag |
| temp#\_max\_alarm | temperature maximum alarm flag |
| temp#\_crit\_alarm | critical temperature alarm flag |
