...
> 출처(원문): https://docs.kernel.org/hwmon/emc1403.html
...
The chips implement three limits for each sensor: low (tempX\_min), high
(tempX\_max) and critical (tempX\_crit.) The chips also implement an
...
* The hysteresis values can’t be set independently. We decided to make
  only temp1\_crit\_hyst writable, while all other hysteresis attributes
  are read-only. Setting temp1\_crit\_hyst writes the difference between
  temp1\_crit\_hyst and temp1\_crit into the chip, and the same relative
