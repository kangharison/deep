...
> 출처(원문): https://docs.kernel.org/hwmon/sis5595.html
...
|  |  |
| --- | --- |
| force\_addr=0xaddr | Set the I/O base address. Useful for boards that don’t set the address in the BIOS. Does not do a PCI force; the device must still be present in lspci. Don’t use this unless the driver complains that the base address is not set.  Example: ‘modprobe sis5595 force\_addr=0x290’ |
