...
> 출처(원문): https://docs.kernel.org/hwmon/smsc47m192.html
...
The +12V analog voltage input channel (in4\_input) is multiplexed with
...
|  |  |
| --- | --- |
| in0\_input | +2.5V voltage input |
| in1\_input | CPU voltage input (nominal 2.25V) |
| in2\_input | +3.3V voltage input |
| in3\_input | +5V voltage input |
| in4\_input | +12V voltage input (may be missing if used as VID4) |
| in5\_input | Vcc voltage input (nominal 3.3V) This is the supply voltage of the sensor chip itself. |
| in6\_input | +1.5V voltage input |
| in7\_input | +1.8V voltage input |
| in[0-7]\_min, |  |
...
| temp1\_input | chip temperature measured by on-chip diode |
...
| temp[2-3]\_input\_fault | diode fault flags for temperature inputs 2 and 3. A fault is detected if the two pins for the corresponding sensor are open or shorted, or any of the two is shorted to ground or Vcc. ‘1’ indicates a diode fault. |
| cpu0\_vid | CPU voltage as received from the CPU |
