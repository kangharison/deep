...
> 출처(원문): https://docs.kernel.org/hwmon/abituguru-datasheet.html
...
Next CMD \_must\_ be read and should hold 0xAC, usually CMD will hold 0xAC the
...
Bit 0:
:   Give an alarm if measured temp is over the warning threshold (RW) [[1]](#id4)

Bit 1:
:   Give an alarm if measured volt is over the max threshold (RW) [[2]](#id5)

Bit 2:
:   Give an alarm if measured volt is under the min threshold (RW) [[2]](#id5)
...
resulted in a \_permanent\_ reprogramming of the voltages, luckily I had the
