...
> 출처(원문): https://docs.kernel.org/filesystems/gfs2/uevents.html
...
for (by gfs\_controld in gfs2-utils).
...
This is used as a signal by gfs\_controld that it is then ok for other
...
process or as the result of gfs\_controld requesting a specific journal
recovery via the /sys/fs/gfs2/<fsname>/lock\_module/recovery file.

Because the CHANGE uevent was used (in early versions of gfs\_controld)
...
as well as providing the information for a lock\_dlm mount to be
...
lock\_nolock or lock\_dlm. In the future other lock managers
may be supported.
