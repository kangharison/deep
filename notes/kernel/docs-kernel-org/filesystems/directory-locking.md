...
> 출처(원문): https://docs.kernel.org/filesystems/directory-locking.html
...
kinds of locks - per-inode (->i\_rwsem) and per-filesystem
(->s\_vfs\_rename\_mutex).

When taking the i\_rwsem on multiple non-directory objects, we
...
5. rename that is \_not\_ cross-directory. Locking rules:
   ...
   >   allowed only with RENAME\_EXCHANGE, and that won’t be removing the target).
...
> * rank ->i\_rwsem of non-directories on given filesystem in inode pointer
>   order.
> * put ->i\_rwsem of all directories on a filesystem at the same rank,
>   lower than ->i\_rwsem of any non-directory on the same filesystem.
> * put ->s\_vfs\_rename\_mutex at rank lower than that of any ->i\_rwsem
...
> 1. ->s\_vfs\_rename\_mutex of NFS filesystem
> 2. ->i\_rwsem of directories on that NFS filesystem, same rank for all
> 3. ->i\_rwsem of non-directories on that filesystem, in order of
>    increasing address of inode
> 4. ->s\_vfs\_rename\_mutex of local filesystem
> 5. ->i\_rwsem of directories on the local filesystem, same rank for all
> 6. ->i\_rwsem of non-directories on local filesystem, in order of
>    increasing address of inode.
...
i.e. they all will be ->i\_rwsem of directories on the same filesystem.
...
in [`is_subdir()`](api-summary.html#c.is_subdir "is_subdir") that would make it work even in presence of such beasts.
