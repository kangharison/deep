...
> 출처(원문): https://docs.kernel.org/filesystems/nfs/client-identifier.html
...
> * co\_ownerid: An arbitrary but fixed string.
...
“nfs\_client\_id4”. The NFSv4.1 specification refers to these two
items as a “client\_owner4”.
...
“co\_ownerid” string has several important properties:

> * The “co\_ownerid” string identifies the client during reboot
>   recovery, therefore the string is persistent across client
>   reboots.
> * The “co\_ownerid” string helps servers distinguish the client
>   from others, therefore the string is globally unique. Note
>   that there is no central authority that assigns “co\_ownerid”
>   strings.
> * Because it often appears on the network in the clear, the
>   “co\_ownerid” string does not reveal private information about
>   the client itself.
> * The content of the “co\_ownerid” string is set and unchanging
...
NFSv4 servers utilize the “client\_owner4” as described above to
...
If distinct clients present the same “co\_ownerid” string and use
the same principal (for example, AUTH\_SYS and UID 0), a server is
...
If a client’s “co\_ownerid” string or principal are not stable,
...
“co\_ownerid” string or principal to the server, the server orphans
the client’s previous open and lock state. This blocks access to
locked files until the server removes the orphaned state.

If the server restarts and a client presents a changed “co\_ownerid”
...
“co\_ownerid” string starting with the words “Linux NFS” followed by
the client’s UTS node name (the same node name, incidentally, that
is used as the “machine name” in an AUTH\_SYS credential). In small
...
Linux provides two mechanisms to add uniqueness to its “co\_ownerid”
string:
...
>     loaded.
>
> /sys/fs/nfs/net/nfs\_client/identifier
...
AUTH\_SYS and UID 0 as the principal part of its client identity.
...
“co\_ownerid” string uniqueness and persistence is critical in this
case.
...
Additionally, the Linux NFS client uses the RPCSEC\_GSS security
...
migration by presenting the same “client\_owner4” to all servers it
encounters.
