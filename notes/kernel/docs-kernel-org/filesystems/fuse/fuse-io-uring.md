...
> 출처(원문): https://docs.kernel.org/filesystems/fuse/fuse-io-uring.html
...
and works. For generic details about FUSE see [FUSE Overview](fuse.html).
...
needs to submit SQEs (opcode = IORING\_OP\_URING\_CMD) to the /dev/fuse
connection file descriptor. Initial submit is with the sub command
FUSE\_URING\_REQ\_REGISTER, which will just register entries to be
...
subcommand FUSE\_URING\_REQ\_COMMIT\_AND\_FETCH - kernel completes
