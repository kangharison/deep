...
> 출처(원문): https://docs.kernel.org/filesystems/fsverity.html
...
* Trusted userspace code. Often, the userspace code that accesses
  ...
  retrieving its fs-verity digest using [FS\_IOC\_MEASURE\_VERITY](#fs-ioc-measure-verity), then
...
* Integrity Policy Enforcement (IPE). IPE supports enforcing access
  ...
  modes, please refer to [IPE admin guide](../admin-guide/LSM/ipe.html).
* Trusted userspace code in combination with [Built-in signature
  verification](#built-in-signature-verification). This approach should be used only with great care.

## User API

### FS\_IOC\_ENABLE\_VERITY

The FS\_IOC\_ENABLE\_VERITY ioctl enables fs-verity on a file. It takes
...
* `hash_algorithm` must be the identifier for the hash algorithm to
  use for the Merkle tree, such as FS\_VERITY\_HASH\_ALG\_SHA256. See
...
* `sig_ptr` is the pointer to the builtin signature, or NULL if no
  ...
  if the [Built-in signature verification](#built-in-signature-verification) feature is being used. It
  is not needed for IMA appraisal, and it is not needed if the file
  signature is being handled entirely in userspace.
* All reserved fields must be zeroed.

FS\_IOC\_ENABLE\_VERITY causes the filesystem to build a Merkle tree for
...
FS\_IOC\_ENABLE\_VERITY checks for write access to the inode. However,
it must be executed on an O\_RDONLY file descriptor and no processes
...
On success, FS\_IOC\_ENABLE\_VERITY returns 0, and the file becomes a
verity file. On failure (including the case of interruption by a
fatal signal), no changes are made to the file.

FS\_IOC\_ENABLE\_VERITY can fail with the following errors:
...
* `EOPNOTSUPP`: the kernel was not configured with fs-verity
  ...
  on this file. (See [Filesystem support](#filesystem-support).)
...
### FS\_IOC\_MEASURE\_VERITY

The FS\_IOC\_MEASURE\_VERITY ioctl retrieves the digest of a verity file.
...
FS\_IOC\_MEASURE\_VERITY is guaranteed to execute in constant time,
regardless of the size of the file.

FS\_IOC\_MEASURE\_VERITY can fail with the following errors:
...
* `EOPNOTSUPP`: the kernel was not configured with fs-verity
  support, or the filesystem superblock has not had the ‘verity’
  feature enabled on it. (See [Filesystem support](#filesystem-support).)
* `EOVERFLOW`: the digest is longer than the specified
  `digest_size` bytes. Try providing a larger buffer.

### FS\_IOC\_READ\_VERITY\_METADATA

The FS\_IOC\_READ\_VERITY\_METADATA ioctl reads verity metadata from a
...
* `FS_VERITY_METADATA_TYPE_MERKLE_TREE` reads the blocks of the
  ...
  See [Merkle tree](#merkle-tree) for more information.
* `FS_VERITY_METADATA_TYPE_DESCRIPTOR` reads the fs-verity
  descriptor. See [fs-verity descriptor](#fs-verity-descriptor).
* `FS_VERITY_METADATA_TYPE_SIGNATURE` reads the builtin signature
  which was passed to FS\_IOC\_ENABLE\_VERITY, if any. See [Built-in
...
The metadata returned by FS\_IOC\_READ\_VERITY\_METADATA isn’t guaranteed
to be authenticated against the file digest that would be returned by
[FS\_IOC\_MEASURE\_VERITY](#fs-ioc-measure-verity), as the metadata is expected to be used to
...
FS\_IOC\_READ\_VERITY\_METADATA can fail with the following errors:
...
* `ENODATA`: the file is not a verity file, or
  FS\_VERITY\_METADATA\_TYPE\_SIGNATURE was requested but the file doesn’t
...
The existing ioctl FS\_IOC\_GETFLAGS (which isn’t specific to fs-verity)
can also be used to check whether a file has fs-verity enabled or not.
To do so, check for FS\_VERITY\_FL (0x00100000) in the returned flags.

The verity flag is not settable via FS\_IOC\_SETFLAGS. You must use
FS\_IOC\_ENABLE\_VERITY instead, since parameters must be provided.

### statx

Since Linux v5.5, the `statx()` system call sets STATX\_ATTR\_VERITY if
the file has fs-verity enabled. This can perform better than
FS\_IOC\_GETFLAGS and FS\_IOC\_MEASURE\_VERITY because it doesn’t require
opening the file, and opening verity files can be expensive.

### FS\_IOC\_FSGETXATTR

Since Linux v7.0, the FS\_IOC\_FSGETXATTR ioctl sets FS\_XFLAG\_VERITY (0x00020000)
in the returned flags when the file has verity enabled. Note that this attribute
cannot be set with FS\_IOC\_FSSETXATTR as enabling verity requires input
parameters. See FS\_IOC\_ENABLE\_VERITY.

### file\_getattr

Since Linux v7.0, the `file_getattr()` syscall sets FS\_XFLAG\_VERITY (0x00020000)
in the returned flags when the file has verity enabled. Note that this attribute
cannot be set with `file_setattr()` as enabling verity requires input parameters.
See FS\_IOC\_ENABLE\_VERITY.
...
* If the sysctl “fs.verity.require\_signatures” is set to 1 and the
  file is not signed by a key in the “.fs-verity” keyring, then
  opening the file will fail. See [Built-in signature verification](#built-in-signature-verification).
...
CONFIG\_FS\_VERITY\_BUILTIN\_SIGNATURES=y adds supports for in-kernel
...
2. [FS\_IOC\_ENABLE\_VERITY](#fs-ioc-enable-verity) accepts a pointer to a PKCS#7 formatted
   ...
   “fs.verity.require\_signatures” described in the next item. The IPE LSM
   relies on this behavior to recognize and label fsverity files
   that contain a verified built-in fsverity signature.
3. A new sysctl “fs.verity.require\_signatures” is made available.
...
[Use cases](#use-cases) for an overview of ways in which fs-verity can be used.
...
* Builtin signature verification does *not* make the kernel enforce
  ...
  fs.verity.require\_signatures=1, just checking whether fs-verity is
...
* Another approach is to utilize fs-verity builtin signature
  ...
  fs.verity.require\_signatures=1.
  Please refer to [IPE admin guide](../admin-guide/LSM/ipe.html) for
  more details.
...
* The sysctl fs.verity.require\_signatures applies system-wide.
...
* fs-verity builtin signatures are in PKCS#7 format, and the public
  ...
  eliminates the need to enable CONFIG\_FS\_VERITY\_BUILTIN\_SIGNATURES
...
CONFIG\_FS\_VERITY kconfig option must be enabled to use fs-verity on
...
pages have been read into the pagecache. (See [Verifying data](#verifying-data).)
...
it. “verity” is an RO\_COMPAT filesystem feature, so once set, old
...
ext4 sets the EXT4\_VERITY\_FL on-disk inode flag on verity files. It
can only be set by [FS\_IOC\_ENABLE\_VERITY](#fs-ioc-enable-verity), and it cannot be cleared.
...
ext4 stores the verity metadata (Merkle tree and fsverity\_descriptor)
past the end of the file, starting at the first 64K boundary beyond
i\_size. This approach works because (a) verity files are readonly,
and (b) pages fully beyond i\_size aren’t visible to userspace but can
...
EA\_INODE feature and on rearchitecturing ext4’s xattr support to
...
f2fs sets the FADVISE\_VERITY\_BIT on-disk inode flag on verity files.
It can only be set by [FS\_IOC\_ENABLE\_VERITY](#fs-ioc-enable-verity), and it cannot be
cleared.

Like ext4, f2fs stores the verity metadata (Merkle tree and
fsverity\_descriptor) past the end of the file, starting at the first
64K boundary beyond i\_size. See explanation for ext4 above.
...
marked with a RO\_COMPAT inode flag, and the verity metadata is stored
...
hash blocks via fsverity\_operations::`read_merkle_tree_page()`.
...
postprocessing step the filesystem enqueues the bio\_post\_read\_ctx on a
...
:   To verify the authenticity of an fs-verity file you must verify
    ...
    incorporates the root hash of the Merkle tree. See [Use cases](#use-cases).
...
A:
:   See [Use cases](#use-cases). In the initial use case, it’s really trusted
...
A:
:   The existing “immutable” bit (FS\_IMMUTABLE\_FL) already has a
...
:   So far all filesystems that have implemented fs-verity support are
    ...
    by manipulating i\_size. The data verification functions provided
...
:   There are many reasons why this is not possible or would be very
    ...
    * It would require defining a filesystem-independent way to store
      ...
      filesystems’ notion of i\_size were divorced from the VFS’s,
      which would be complex and require changes to all filesystems.
    * It’s desirable that FS\_IOC\_ENABLE\_VERITY uses the filesystem’s
