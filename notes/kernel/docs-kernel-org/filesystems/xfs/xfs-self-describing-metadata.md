...
> 출처(원문): https://docs.kernel.org/filesystems/xfs/xfs-self-describing-metadata.html
...
thing that resets it is running xfs\_repair on the filesystem. Further, by use of
...
inode is already extensively verified during writeback in xfs\_iflush\_int, so the
