...
> 출처(원문): https://docs.kernel.org/filesystems/debugfs.html
...
files exported there. The real world is not always so simple, though [[1]](#id2);
...
clean it up at the end). An ERR\_PTR(-ERROR) return value indicates that
something went wrong. If ERR\_PTR(-ENODEV) is returned, that is an
...
should hold the file, data will be stored in the i\_private field of the
resulting inode structure, and fops is a set of file operations which
implement the file’s behavior. At a minimum, the read() and/or write()
operations should be provided; others can be included as needed. Again,
the return value will be a dentry pointer to the created file,
ERR\_PTR(-ERROR) on error, or ERR\_PTR(-ENODEV) if debugfs support is
missing.
...
file\_size is the initial file size. The other parameters are the same
as the function debugfs\_create\_file.
...
Also, atomic\_t values can be placed in debugfs with:
...
A read of this file will get atomic\_t values, and a write of this file
will set atomic\_t values.
...
debugfs\_blob\_wrapper structure. Some drivers use “blobs” as a simple way
...
[`debugfs_create_blob()`](api-summary.html#c.debugfs_create_blob "debugfs_create_blob") are read-only.
...
using \_\_stringify, and a number of register names (macros) are actually
...
There is a helper function to create a device-related seq\_file:
...
the “read\_fn” is a function pointer which to be called to print the
seq\_file content.
...
A call to [`debugfs_change_name()`](api-summary.html#c.debugfs_change_name "debugfs_change_name") will give a new name to an existing debugfs
file, always in the same directory. The new\_name must not exist prior
to the call; the return value is 0 on success and -E... on failure.
Symbolic links can be created with [`debugfs_create_symlink()`](api-summary.html#c.debugfs_create_symlink "debugfs_create_symlink").
...
[`debugfs_remove()`](api-summary.html#c.debugfs_remove "debugfs_remove"). `debugfs_remove_recursive()` should be considered
deprecated.
