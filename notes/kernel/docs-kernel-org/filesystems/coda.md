...
> 출처(원문): https://docs.kernel.org/filesystems/coda.html
...
> driver allow Venus to retrieve the message with a getmsg\_from\_kernel
...
> passed the contents of the data buffer. The getmsg\_from\_kernel call
> now returns and Venus processes the request.
>
> At some later point the FS driver receives a message from Venus,
> namely when Venus calls sendmsg\_to\_kernel. At this moment the Coda FS
...
>   mode context of Venus) and the sendmsg\_to\_kernel call returns to
...
> sendmsg\_to\_kernel call. In the normal case, the upcall routine will
...
> extra field “handle\_signals” could be added in the message structure
...
> from user memory to kernel memory with OPCODES. The sendmsg\_to\_kernel
> is issued as a synchronous call, while the getmsg\_from\_kernel call is
...
> they are set for the calling process. The vuid\_t and vgid\_t are 32 bit
...
> directory in the Coda filesystem within a cell [[1]](#id2):
>
> ```
...
>
> Each of the constituent fields: VolumeId, VnodeId and Unique\_t are
...
>     the Coda filesystem. If the result is zero, the cfs\_root structure
...
>     and Venus will search the directory identified by cfs\_lookup\_in.VFid.
...
>     If the result is zero, the field cfs\_lookup\_out.VFid contains the
>     targets ViceFid and cfs\_lookup\_out.vtype the coda\_vtype giving the
>     type of object the name designates.
>
> The name of the object is an 8 bit character string of maximum length
> CFS\_MAXNAMLEN, currently set to 256 (including a 0 terminator.)
>
> It is extremely important to realize that Venus bitwise ors the field
> cfs\_lookup.vtype with CFS\_NOCACHE to indicate that the object should
> not be put in the kernel name cache.
>
> Note
>
> The type of the vtype is currently wrong. It should be
> coda\_vtype. Linux does not take note of CFS\_NOCACHE. It should.
...
>
> Description
> :   Remove file named cfs\_remove\_in.name in directory
>     identified by VFid.
>
> Note
>
...
>     to hold any name up to CFS\_MAXNAMLEN (PATH or NAM??).
>
> Errors
> :   No unusual errors.
...
> Note
>
> Currently the cfs\_open\_out structure is not properly adapted to
...
> fetching the data in Venus vproc\_vfscalls. This seems silly. If a
...
>     have support for it. (ODY\_PREFETCH is not a defined operation).
>
> Note
>
