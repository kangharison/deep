...
> 출처(원문): https://docs.kernel.org/filesystems/sharedsubtree.html
...
1. A **shared mount** can be replicated to as many mountpoints and all the
...
2. A **slave mount** is like a shared mount except that mount and umount events
   ...
   /mnt
3. A **private mount** does not forward or receive propagation.

   This is the mount we are familiar with. Its the default type.
4. An **unbindable mount** is, as the name suggests, an unbindable private
   mount.
...
1. A process wants to clone its own namespace, but still wants to
   ...
   namespaces.
2. A process wants its mounts invisible to any other process, but
...
1. Datastructure
   ...
   ‘E’, ‘K’, ‘F’, ‘G’ have their ->mnt\_master point to `struct vfsmount` of ‘A’
