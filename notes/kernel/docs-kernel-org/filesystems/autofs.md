...
> 출처(원문): https://docs.kernel.org/filesystems/autofs.html
...
If a dentry has the DCACHE\_NEED\_AUTOMOUNT flag set (which gets set if
the inode has S\_AUTOMOUNT set, or can be set directly) then it is
(potentially) a mount trap. Any access to this directory beyond a
“stat” will (normally) cause the d\_op->`d_automount()` dentry operation
...
autofs d\_automount method then waits for the daemon to report that
...
If a dentry has DCACHE\_MANAGE\_TRANSIT set then two very different but
related behaviours are invoked, both using the d\_op->`d_manage()`
dentry operation.

Firstly, before checking to see if any filesystem is mounted on the
directory, `d_manage()` will be called with the rcu\_walk parameter set
...
* A return value of -EISDIR tells the VFS to ignore any mounts
  ...
  This effectively disables the **DCACHE\_NEED\_AUTOMOUNT** flag
  ...
  first. This means that d\_manage cannot *always* return -EISDIR for
  the automount daemon. It must only return it when a mount has
  been requested, but has not yet completed.

  d\_manage also returns -EISDIR if the dentry shouldn’t be a
...
The second use case only occurs during an “RCU-walk” and so rcu\_walk
will be set.
...
In the rcu\_walk case, `d_manage()` cannot return -EISDIR to tell the
VFS that this is a directory that doesn’t require d\_automount. If
rcu\_walk sees a dentry with DCACHE\_NEED\_AUTOMOUNT set but nothing
...
So `d_manage()`, when called with rcu\_walk set, should either return
...
This is guided by the MNT\_SHRINKABLE flag. This only applies to
...
The VFS also supports “expiry” of mounts using the MNT\_EXPIRE flag to
the umount system call. Unmounting with MNT\_EXPIRE will fail unless
...
used for a while. For this purpose autofs maintains a “last\_used”
...
can be used to avoid the “last\_used” update on path walk thereby
...
When autofs considers a directory it checks the last\_used time and
...
only the last\_used time is ever considered.
...
use the **AUTOFS\_IOC\_EXPIRE** ioctl. This only works for indirect
...
The second mechanism uses either the **AUTOFS\_DEV\_IOCTL\_EXPIRE\_CMD** or
the **AUTOFS\_IOC\_EXPIRE\_MULTI** ioctl. This will work for both direct and
...
d\_manage will block until the daemon affirms that the unmount has
...
O\_DIRECT) to \_pipe2(2)\_ so that a read from the pipe will return at
most one packet, and any unread portion of a packet will be discarded.

The wait\_queue\_token is a unique number which can identify a
...
ioctls. The process issuing the ioctl must have the CAP\_SYS\_ADMIN
...
* **AUTOFS\_IOC\_READY**:
  :   a notification has been handled. The argument
      to the ioctl command is the “wait\_queue\_token” number
...
* **AUTOFS\_IOC\_EXPIRE**:
  :   as described above, this asks if there is
      ...
      errno is set to EAGAIN. Even though a wait\_queue\_token
      is present in the structure, no “wait queue” is established
      and no acknowledgment is needed.
* **AUTOFS\_IOC\_EXPIRE\_MULTI**:
  :   This is similar to
      **AUTOFS\_IOC\_EXPIRE** except that it causes notification to be
      ...
      **AUTOFS\_EXP\_IMMEDIATE** causes last\_used time to be ignored
      and objects are expired if the are not in use.

      **AUTOFS\_EXP\_FORCED** causes the in use status to be ignored
      and objects are expired even if they are in use. This assumes
      that the daemon has requested this because it is capable of
      performing the umount.

      **AUTOFS\_EXP\_LEAVES** will select a leaf rather than a top-level
...
It requires CAP\_SYS\_ADMIN for access.

The ‘ioctl’s that can be used on this device are described in a separate
document [Miscellaneous Device control operations for the autofs kernel module](autofs-mount-control.html), and are summarised briefly here.
Each ioctl is passed a pointer to an autofs\_dev\_ioctl structure:
...
For the **OPEN\_MOUNT** and **IS\_MOUNTPOINT** commands, the target
...
root, and which can be returned by **OPEN\_MOUNT**.

The ver\_major and ver\_minor are in/out parameters which check that
...
function as the similarly named **AUTOFS\_IOC** ioctls, except
that **FAIL** can be given an explicit error number in fail.status
instead of assuming ENOENT, and this **EXPIRE** command
corresponds to **AUTOFS\_IOC\_EXPIRE\_MULTI**.
...
When the filesystem is mounted a \_uid\_ and \_gid\_ can be given which
...
**AUTOFS\_DEV\_IOCTL\_OPENMOUNT\_CMD** ioctl on the /dev/autofs.
