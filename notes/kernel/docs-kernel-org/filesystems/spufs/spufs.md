...
> 출처(원문): https://docs.kernel.org/filesystems/spufs/spufs.html
...
> can use spu\_create(2) to establish SPU contexts in the spufs root.
>
...
> > only the st\_mode, st\_nlink, st\_uid and st\_gid fields of `struct stat`
...
>         MAP\_SHARED mappings are allowed.
>
> `/mbox`
...
>     :   If a count smaller than four is requested, read returns -1 and
>         sets errno to EINVAL. If there is no data available in the mail
>         box and the file descriptor has been opened with O\_NONBLOCK, the
>         return value is set to -1 and errno becomes EAGAIN.
>
>         If there is no data available in the mail box and the file
>         descriptor has been opened without O\_NONBLOCK, the call will
...
>     opened with O\_NONBLOCK, the return value is set to -1 and errno becomes
>     EAGAIN.
>
>     If there is no space available in the mail box and the file descriptor
>     has been opened without O\_NONBLOCK, the call will block until the SPU
...
>         read from (for mbox\_stat and ibox\_stat) or written to (for
>         wbox\_stat) the respective mail box without blocking or resulting
>         in EAGAIN.
>
...
>
>     The possible operations on an open npc, decr, decr\_status,
>     spu\_tag\_mask, event\_mask or srr0 file are:
>
>     read(2)
...
>         contents of the signal1\_type, or signal2\_type respectively,
>         file.
>
...
>     quently written to it. The possible operations on an open signal1\_type
>     or signal2\_type file are:
>
>     read(2)
>     :   When the count supplied to the read call is shorter than the
...
> capabilities(7), close(2), spu\_create(2), spu\_run(2), spufs(7)
