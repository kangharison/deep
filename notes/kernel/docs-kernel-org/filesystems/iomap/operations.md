...
> 출처(원문): https://docs.kernel.org/filesystems/iomap/operations.html
...
### [2.1.1. `struct address_space_operations`](#id2)
...
### [2.1.2. `struct iomap_write_ops`](#id3)
...
### [2.1.3. `struct iomap_read_ops`](#id4)
...
### [2.1.4. Internal per-Folio State](#id5)
...
### [2.1.5. Buffered Readahead and Reads](#id6)
...
#### [2.1.6.2. Buffered Write Failures](#id9)
...
#### [2.1.6.3. Zeroing for File Operations](#id10)
...
#### [2.1.6.4. Unsharing Reflinked File Data](#id11)
...
### [2.1.8. Pagecache Writeback](#id13)
...
#### [2.1.8.1. `struct iomap_writeback_ops`](#id14)
...
>   Block based file systems should use the iomap\_ioend\_writeback\_submit
...
#### [2.1.8.2. Pagecache Writeback Completion](#id15)
...
extent conversions) should set their own bi\_end\_io on the bios
...
The `iomap_dio_rw` function implements O\_DIRECT (direct I/O) reads and
writes for files.
...
>   the write offset.
>   The filesystems must set IOMAP\_F\_ATOMIC\_BIO to inform iomap core of an
...
### [2.2.4. `struct iomap_dio_ops:`](#id20)
...
> * `bio_set`: This allows the filesystem to provide a custom bio\_set
...
#### [2.3.2.1. fsdax mmap Faults](#id24)
...
### [2.3.3. fsdax Truncation, fallocate, and Unsharing](#id25)
...
### [2.3.4. fsdax Deduplication](#id26)
...
## [2.4. Seeking Files](#id27)

iomap implements the two iterating whence modes of the `llseek` system
call.

### [2.4.1. SEEK\_DATA](#id28)

The `iomap_seek_data` function implements the SEEK\_DATA “whence” value
for llseek.
...
### [2.4.2. SEEK\_HOLE](#id29)

The `iomap_seek_hole` function implements the SEEK\_HOLE “whence” value
for llseek.
`IOMAP_REPORT` will be passed as the `flags` argument to
`->iomap_begin`.
...
## [2.6. File Space Mapping Reporting](#id31)

iomap implements two of the file space mapping system calls.

### [2.6.1. FS\_IOC\_FIEMAP](#id32)
