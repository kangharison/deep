...
> 출처(원문): https://docs.kernel.org/filesystems/vfat.html
...
:   This option controls the permission check of mtime/atime.
    ...
    the file, or it has CAP\_FOWNER capability. But FAT
...
:   Sets the codepage number for converting to shortname
    ...
    By default, FAT\_DEFAULT\_CODEPAGE setting is used.
...
:   Character set to use for converting between the
    ...
    By default, FAT\_DEFAULT\_IOCHARSET setting is used.
...
:   UTF-8 is the filesystem safe version of Unicode that
    ...
    If ‘uni\_xlate’ gets set, UTF-8 gets disabled.
    By default, FAT\_DEFAULT\_UTF8 setting is used.
...
**sys\_immutable**
:   If set, ATTR\_SYS attribute on FAT is handled as
...
**rodir**
:   FAT has the ATTR\_RO (read-only) attribute. On Windows,
    the ATTR\_RO of the directory will just be ignored,
    and is used only by applications as a flag (e.g. it’s set
    for the customized folder).

    If you want to use ATTR\_RO as read-only flag even for
...
:   Enable this only if you want to export the FAT filesystem
    ...
    > **stale\_rw**: This option maintains an index (cache) of directory
    > *inodes* by *i\_logstart* which is used by the nfs-related code to
    > improve look-ups. Full file operations (read/write) over NFS is
    > supported but with cache eviction at NFS server, this could
    > result in ESTALE issues.
    >
    > **nostale\_ro**: This option bases the *inode* number and filehandle
...
when using fallocate with FALLOC\_FL\_KEEP\_SIZE.
...
* vfat\_valid\_longname does not properly checked reserved names.
...
> 2. Checksum. Each slot has an alias\_checksum value. The
