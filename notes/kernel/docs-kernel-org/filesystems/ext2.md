...
> 출처(원문): https://docs.kernel.org/filesystems/ext2.html
...
| dax |  | Use direct access (no page cache). See [Direct Access for files](dax.html). |
...
| user\_xattr |  | Enable “user.” POSIX Extended Attributes (requires CONFIG\_EXT2\_FS\_XATTR). |
| nouser\_xattr |  | Don’t support “user.” extended attributes. |
| acl |  | Enable POSIX Access Control Lists support (requires CONFIG\_EXT2\_FS\_POSIX\_ACL). |
| noacl |  | Don’t support POSIX ACLs. |
| quota, usrquota |  | Enable user disk quota support (requires CONFIG\_QUOTA). |
| grpquota |  | Enable group disk quota support (requires CONFIG\_QUOTA). |
...
the original revision 0 (EXT2\_GOOD\_OLD\_REV) of ext2, but was introduced in
revision 1. There are three 32-bit fields, one for compatible features
(COMPAT), one for read-only compatible (RO\_COMPAT) features and one for
...
HAS\_JOURNAL feature is a COMPAT flag because the ext3 journal is simply
...
An RO\_COMPAT flag indicates that the on-disk format is 100% compatible
...
most common such feature, SPARSE\_SUPER, is an RO\_COMPAT feature because
...
boundary, but this is a legitimate layout in a SPARSE\_SUPER filesystem.
...
RO\_COMPAT, or INCOMPAT flags it will refuse to check the filesystem,
...
* per-file if you have the program source: use the O\_SYNC flag to open()
...
| Compression [[1]](#id5) | <http://e2compr.sourceforge.net/> |
...
| Windows 95 [[1]](#id5) | <http://www.yipton.net/content/fsdext2/> |
| DOS client [[1]](#id5) | <ftp://metalab.unc.edu/pub/Linux/system/filesystems/ext2/> |
| OS/2 [[2]](#id6) | <ftp://metalab.unc.edu/pub/Linux/system/filesystems/ext2/> |
