...
> 출처(원문): https://docs.kernel.org/filesystems/multigrain-ts.html
...
points. For io\_uring or nfsd operations, that’s the request submission and
...
It uses an unused bit in the i\_ctime\_nsec field to indicate whether the mtime
...
monotonic ktime\_t, and the values are converted to realtime clock values as
needed.
...
For most filesystems, it’s sufficient to just set the FS\_MGTIME flag in the
fstype->fs\_flags in order to opt-in, providing the ctime is only ever set via
[`inode_set_ctime_current()`](api-summary.html#c.inode_set_ctime_current "inode_set_ctime_current"). If the filesystem has a ->getattr routine that
doesn’t call generic\_fillattr, then it should call [`fill_mg_cmtime()`](api-summary.html#c.fill_mg_cmtime "fill_mg_cmtime") to
fill those values. For setattr, it should use [`setattr_copy()`](api-summary.html#c.setattr_copy "setattr_copy") to update the
