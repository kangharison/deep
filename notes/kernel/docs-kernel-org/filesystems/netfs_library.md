...
> 출처(원문): https://docs.kernel.org/filesystems/netfs_library.html
...
Note that local caching used to use the PG\_private\_2 (aliased as PG\_fscache) to
keep track of a page that was being written to the cache, but this is now
deprecated as PG\_private\_2 will be removed.
...
> * `remote_i_size`
>
>   The size of the file on the server. This differs from inode->i\_size if
...
A number of functions are provided to manage the locking of i\_rwsem for I/O and
...
>
>    These should just access i\_rwsem directly.
> 4. mmap().
>
...
> 3. If `->unpinned_netfs_wb` is now set, the write\_inode procedure is forced.
...
Further, if an inode is deleted, the filesystem’s write\_inode method may not
get called, so:
...
filesystem is called through the standard VFS read/write\_iter methods:
...
used. The writeback function requires the buffer to be of ITER\_FOLIOQ type.
...
It uses the deprecated PG\_private\_2 flag and so should not be used.
...
> * `origin`
>
>   The origin of the request (readahead, read\_folio, DIO read, writeback, ...).
> * `inode`
> * `mapping`
>
>   The inode and the address space of the file being read from. The mapping
>   may or may not point to inode->i\_data.
> * `group`
>
...
>
>     The filesystem can set this if it wants to use PG\_private\_2 to track
>     whether a folio is being written to the cache. This is deprecated as
>     PG\_private\_2 is going to go away.
...
>     from transferred to len, should be cleared. Do not set if HIT\_EOF is set.
...
>   data should be read from the server and ->io\_iter indicates the buffer to be
>   used.
>
...
>   given, netfslib will set i\_size and i\_blocks and update the local cache
>   cookie.
> * `post_modify()`
>
...
>   finds a dirty page that isn’t simply marked NETFS\_FOLIO\_COPY\_TO\_CACHE,
...
>   should be written to the server and ->io\_iter indicates the buffer to be
>   used.
>
...
Firstly, the netfs\_io\_request object contains a place for the cache to hang its
state:
...
>   In the subrequest, ->start, ->len and ->transferred indicate what data
>   should be written to the cache and ->io\_iter indicates the buffer to be
>   used.
>
>   There is no return value; the `netfs_write_subreq_terminated()` function
>   should be called to indicate that the subrequest completed either way.
...
void folio\_start\_private\_2(struct [folio](#c.folio_start_private_2 "folio") \*folio)
...
struct [netfs\_inode](#c.netfs_inode "netfs_inode") \*netfs\_inode(struct [inode](#c.netfs_inode "inode") \*inode)
...
unsigned long long netfs\_read\_remote\_i\_size(const struct [inode](#c.netfs_read_remote_i_size "inode") \*inode)
:   Read remote\_i\_size safely
...
Read remote\_i\_size safely without the potential for tearing on 32-bit
arches.

**NOTE**

in a 32bit arch with a preemptable kernel and an UP compile the
i\_size\_read/write must be atomic with respect to the local cpu (unlike with
...
unsigned long long netfs\_read\_zero\_point(const struct [inode](#c.netfs_read_zero_point "inode") \*inode)
:   Read zero\_point safely
...
Read zero\_point safely without the potential for tearing on 32-bit
arches.
...
void netfs\_read\_sizes(const struct [inode](#c.netfs_read_sizes "inode") \*inode, unsigned long long \*i\_size, unsigned long long \*remote\_i\_size, unsigned long long \*zero\_point)
:   Read remote\_i\_size and zero\_point safely
...
`unsigned long long *i_size`
:   Where to return the local file size.

`unsigned long long *remote_i_size`
:   Where to return the size of the file on the server

`unsigned long long *zero_point`
:   Where to return the the point beyond which the server has no data

**Description**

Read remote\_i\_size and zero\_point safely without the potential for tearing
on 32-bit arches.
...
void netfs\_inode\_init(struct [netfs\_inode](#c.netfs_inode "netfs_inode") \*ctx, const struct netfs\_request\_ops \*ops, bool use\_zero\_point)
...
`bool use_zero_point`
:   True to use the zero\_point read optimisation
...
void netfs\_resize\_file(struct [netfs\_inode](#c.netfs_inode "netfs_inode") \*ictx, unsigned long long new\_i\_size, bool changed\_on\_server)
...
struct fscache\_cookie \*netfs\_i\_cookie(struct [netfs\_inode](#c.netfs_inode "netfs_inode") \*ctx)
...
void netfs\_wait\_for\_outstanding\_io(struct [inode](#c.netfs_wait_for_outstanding_io "inode") \*inode)
...
void netfs\_readahead(struct [readahead\_control](../core-api/mm-api.html#c.readahead_control "readahead_control") \*ractl)
...
int netfs\_read\_folio(struct [file](#c.netfs_read_folio "file") \*file, struct [folio](#c.netfs_read_folio "folio") \*folio)
:   Helper to manage a read\_folio request
...
Fulfil a read\_folio request by drawing data from the cache if
...
int netfs\_write\_begin(struct [netfs\_inode](#c.netfs_inode "netfs_inode") \*ctx, struct [file](#c.netfs_write_begin "file") \*file, struct [address\_space](api-summary.html#c.address_space "address_space") \*mapping, loff\_t pos, unsigned int len, struct [folio](../core-api/mm-api.html#c.folio "folio") \*\*\_folio, void \*\*\_fsdata)
...
issue\_read, is mandatory.

The `check_write_begin()` operation can be provided to check for and flush
conflicting writes once the folio is grabbed and locked. It is passed a
pointer to the fsdata cookie that gets returned to the VM to be passed to
write\_end. It is permitted to sleep. It should return 0 if the request
...
ssize\_t netfs\_buffered\_read\_iter(struct kiocb \*iocb, struct iov\_iter \*iter)
...
The IOCB\_NOWAIT flag in iocb->ki\_flags indicates that -EAGAIN shall be
returned when no data can be read without waiting for I/O requests to
complete; it doesn’t prevent readahead.

The IOCB\_NOIO flag in iocb->ki\_flags indicates that no new I/O requests
...
ssize\_t netfs\_file\_read\_iter(struct kiocb \*iocb, struct iov\_iter \*iter)
