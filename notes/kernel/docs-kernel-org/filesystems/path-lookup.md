...
> 출처(원문): https://docs.kernel.org/filesystems/path-lookup.html
...
conceptual sequence “lock; inc\_ref; unlock;” can often be performed
...
`DCACHE_PAR_LOOKUP` to be cleared, using a wait\_queue that was passed
...
`path_lookupat()` will unset LOOKUP\_JUMPED in nameidata so that in the
...
complexity needed to handle the different subtleties of O\_CREAT (with
or without O\_EXCL), final “`/`” characters, and trailing symbolic
...
covered in greater detail in [autofs - how it works](autofs.html) in the Linux documentation
...
data structures - dentries, inodes, super\_blocks, and mounts - will
...
rename\_lock would bring no significant value.
...
involves using [READ\_ONCE()](https://lwn.net/Articles/624126/) to access fields, and verifying the
...
file systems can set their own put\_link function and argument through
...
3. An open with O\_CREAT **does** follow a symlink in the final component,
   ...
   `O_EXCL` is set but otherwise is handled for an O\_CREAT open much
