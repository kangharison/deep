...
> 출처(원문): https://docs.kernel.org/filesystems/fscrypt.html
...
full 255 bytes (NAME\_MAX). Finally, unlike eCryptfs, the fscrypt API
...
Unless [hardware-wrapped keys](#hardware-wrapped-keys) are used, an attacker who gains the
...
In more detail, the FS\_IOC\_REMOVE\_ENCRYPTION\_KEY ioctl (or the
FS\_IOC\_REMOVE\_ENCRYPTION\_KEY\_ALL\_USERS ioctl) can wipe a master
...
* The kernel cannot magically wipe copies of the master key(s) that
  ...
  be done immediately after FS\_IOC\_ADD\_ENCRYPTION\_KEY, without waiting
  for FS\_IOC\_REMOVE\_ENCRYPTION\_KEY. Naturally, the same also applies
...
* In general, decrypted contents and filenames in the kernel VFS
  ...
  were wiped. To partially solve this, you can add init\_on\_free=1 to
...
However, if [hardware-wrapped keys](#hardware-wrapped-keys) are used, such attackers will be
...
key hierarchy slightly. For details, see [Hardware-wrapped keys](#hardware-wrapped-keys).
...
derivation function](#key-derivation-function)) to derive the file’s key from the master key
and nonce.
...
### DIRECT\_KEY policies

The Adiantum encryption mode (see [Encryption modes and usage](#encryption-modes-and-usage)) is
...
this by setting FSCRYPT\_POLICY\_FLAG\_DIRECT\_KEY in the fscrypt policy,
...
### IV\_INO\_LBLK\_64 policies

When FSCRYPT\_POLICY\_FLAG\_IV\_INO\_LBLK\_64 is set in the fscrypt policy,
...
### IV\_INO\_LBLK\_32 policies

IV\_INO\_LBLK\_32 policies work like IV\_INO\_LBLK\_64, except that for
IV\_INO\_LBLK\_32, the inode number is hashed with SipHash-2-4 (where the
...
So, for example, FSCRYPT\_MODE\_AES\_256\_CTS means AES-256-CBC-CTS.
...
if the kernel supports fscrypt at all; see [Kernel config options](#kernel-config-options).
...
entire result.) As described in [Filenames encryption](#filenames-encryption), a wide-block
...
Enabling fscrypt support (CONFIG\_FS\_ENCRYPTION) automatically pulls in
...
kernel crypto API (see [Inline encryption support](#inline-encryption-support)); in that case,
...
* AES-256-XTS and AES-256-CBC-CTS
  :   + Recommended:
        :   - arm64: CONFIG\_CRYPTO\_AES\_ARM64\_CE\_BLK
            - x86: CONFIG\_CRYPTO\_AES\_NI\_INTEL
* AES-256-HCTR2
  :   + Mandatory:
        :   - CONFIG\_CRYPTO\_HCTR2
      + Recommended:
        :   - arm64: CONFIG\_CRYPTO\_AES\_ARM64\_CE\_BLK
            - x86: CONFIG\_CRYPTO\_AES\_NI\_INTEL
* Adiantum
  :   + Mandatory:
        :   - CONFIG\_CRYPTO\_ADIANTUM
* AES-128-CBC-ESSIV and AES-128-CBC-CTS:
  :   + Mandatory:
        :   - CONFIG\_CRYPTO\_ESSIV
            - CONFIG\_CRYPTO\_SHA256 or another SHA-256 implementation
      + Recommended:
...
* Fixed-size data units. This is how all filesystems other than UBIFS
  ...
  the encryption policy; see [FS\_IOC\_SET\_ENCRYPTION\_POLICY](#fs-ioc-set-encryption-policy).
...
As mentioned in [Key hierarchy](#key-hierarchy), the default encryption setting uses
...
* With [DIRECT\_KEY policies](#direct-key-policies), the data unit index is placed in bits
  0-63 of the IV, and the file’s nonce is placed in bits 64-191.
* With [IV\_INO\_LBLK\_64 policies](#iv-ino-lblk-64-policies), the data unit index is placed in
...
* With [IV\_INO\_LBLK\_32 policies](#iv-ino-lblk-32-policies), the file’s inode number is hashed
...
If the user selects FSCRYPT\_MODE\_AES\_128\_CBC for the contents mode, an
...
alternatively has the file’s nonce (for [DIRECT\_KEY policies](#direct-key-policies)) or
inode number (for [IV\_INO\_LBLK\_64 policies](#iv-ino-lblk-64-policies)) included in the IVs.
...
#### FS\_IOC\_SET\_ENCRYPTION\_POLICY
...
* `version` must be FSCRYPT\_POLICY\_V1 (0) if
  `struct fscrypt_policy_v1` is used or FSCRYPT\_POLICY\_V2 (2) if
...
* `contents_encryption_mode` and `filenames_encryption_mode` must
  ...
  encryption modes to use. If unsure, use FSCRYPT\_MODE\_AES\_256\_XTS
  (1) for `contents_encryption_mode` and FSCRYPT\_MODE\_AES\_256\_CTS
  (4) for `filenames_encryption_mode`. For details, see [Encryption
  modes and usage](#encryption-modes-and-usage).

  v1 encryption policies only support three combinations of modes:
  (FSCRYPT\_MODE\_AES\_256\_XTS, FSCRYPT\_MODE\_AES\_256\_CTS),
  (FSCRYPT\_MODE\_AES\_128\_CBC, FSCRYPT\_MODE\_AES\_128\_CTS), and
  (FSCRYPT\_MODE\_ADIANTUM, FSCRYPT\_MODE\_ADIANTUM). v2 policies support
  all combinations documented in [Supported modes](#supported-modes).
* `flags` contains optional flags from `<linux/fscrypt.h>`:

  + FSCRYPT\_POLICY\_FLAGS\_PAD\_\*: The amount of NUL padding to use when
    encrypting filenames. If unsure, use FSCRYPT\_POLICY\_FLAGS\_PAD\_32
    (0x3).
  + FSCRYPT\_POLICY\_FLAG\_DIRECT\_KEY: See [DIRECT\_KEY policies](#direct-key-policies).
  + FSCRYPT\_POLICY\_FLAG\_IV\_INO\_LBLK\_64: See [IV\_INO\_LBLK\_64
    policies](#iv-ino-lblk-64-policies).
  + FSCRYPT\_POLICY\_FLAG\_IV\_INO\_LBLK\_32: See [IV\_INO\_LBLK\_32
    policies](#iv-ino-lblk-32-policies).

  v1 encryption policies only support the PAD\_\* and DIRECT\_KEY flags.
  The other flags are only supported by v2 encryption policies.

  The DIRECT\_KEY, IV\_INO\_LBLK\_64, and IV\_INO\_LBLK\_32 flags are
  mutually exclusive.
...
* For v1 encryption policies, `master_key_descriptor` specifies how
  to find the master key in a keyring; see [Adding keys](#adding-keys). It is up
  ...
  [FS\_IOC\_ADD\_ENCRYPTION\_KEY](#fs-ioc-add-encryption-key). Then, the `key_spec.u.identifier`
...
corresponding master key as described in [Adding keys](#adding-keys), all regular
...
user or that the caller has CAP\_FOWNER in the initial user namespace.
(This is needed to prevent a user from encrypting their data with
another user’s key.) The key must remain added while
FS\_IOC\_SET\_ENCRYPTION\_POLICY is executing. However, if the new
...
* `ENOKEY`: a v2 encryption policy was specified, but the key with
  ...
  the process have the CAP\_FOWNER capability in the initial user
  namespace
...
* `EOPNOTSUPP`: the kernel was not configured with encryption
  ...
  ext4 filesystem, CONFIG\_FS\_ENCRYPTION must be enabled in the
...
* [FS\_IOC\_GET\_ENCRYPTION\_POLICY\_EX](#fs-ioc-get-encryption-policy-ex)
* [FS\_IOC\_GET\_ENCRYPTION\_POLICY](#fs-ioc-get-encryption-policy)
...
#### FS\_IOC\_GET\_ENCRYPTION\_POLICY\_EX

The FS\_IOC\_GET\_ENCRYPTION\_POLICY\_EX ioctl retrieves the encryption
...
version code for the “v1” policy is actually 0 (FSCRYPT\_POLICY\_V1).

FS\_IOC\_GET\_ENCRYPTION\_POLICY\_EX can fail with the following errors:

* `EINVAL`: the file is encrypted, but it uses an unrecognized
  encryption policy version
* `ENODATA`: the file is not encrypted
* `ENOTTY`: this type of filesystem does not implement encryption,
  or this kernel is too old to support FS\_IOC\_GET\_ENCRYPTION\_POLICY\_EX
  (try FS\_IOC\_GET\_ENCRYPTION\_POLICY instead)
* `EOPNOTSUPP`: the kernel was not configured with encryption
...
#### FS\_IOC\_GET\_ENCRYPTION\_POLICY

The FS\_IOC\_GET\_ENCRYPTION\_POLICY ioctl can also retrieve the
encryption policy, if any, for a directory or regular file. However,
unlike [FS\_IOC\_GET\_ENCRYPTION\_POLICY\_EX](#fs-ioc-get-encryption-policy-ex),
FS\_IOC\_GET\_ENCRYPTION\_POLICY only supports the original policy
version. It takes in a pointer directly to `struct fscrypt_policy_v1`
rather than `struct fscrypt_get_policy_ex_arg`.

The error codes for FS\_IOC\_GET\_ENCRYPTION\_POLICY are the same as those
for FS\_IOC\_GET\_ENCRYPTION\_POLICY\_EX, except that
FS\_IOC\_GET\_ENCRYPTION\_POLICY also returns `EINVAL` if the file is
...
Since Linux v5.7, the ioctl FS\_IOC\_GET\_ENCRYPTION\_NONCE is supported.
...
#### FS\_IOC\_ADD\_ENCRYPTION\_KEY

The FS\_IOC\_ADD\_ENCRYPTION\_KEY ioctl adds a master encryption key to
...
* If the key is being added for use by v1 encryption policies, then
  `key_spec.type` must contain FSCRYPT\_KEY\_SPEC\_TYPE\_DESCRIPTOR, and
  ...
  CAP\_SYS\_ADMIN capability in the initial user namespace.

  Alternatively, if the key is being added for use by v2 encryption
  policies, then `key_spec.type` must contain
  FSCRYPT\_KEY\_SPEC\_TYPE\_IDENTIFIER, and `key_spec.u.identifier` is
...
* `flags` contains optional flags from `<linux/fscrypt.h>`:
  ...
  + FSCRYPT\_ADD\_KEY\_FLAG\_HW\_WRAPPED: This denotes that the key is a
    hardware-wrapped key. See [Hardware-wrapped keys](#hardware-wrapped-keys). This flag
    can’t be used if FSCRYPT\_KEY\_SPEC\_TYPE\_DESCRIPTOR is used.
* `raw` is a variable-length field which must contain the actual
  key, `raw_size` bytes long. Alternatively, if `key_id` is
  nonzero, then this field is unused. Note that despite being named
  `raw`, if FSCRYPT\_ADD\_KEY\_FLAG\_HW\_WRAPPED is specified then it
  will contain a wrapped key, not a raw key.

For v2 policy keys, the kernel keeps track of which user (identified
by effective user ID) added the key, and only allows the key to be
removed by that user --- or by “root”, if they use
[FS\_IOC\_REMOVE\_ENCRYPTION\_KEY\_ALL\_USERS](#fs-ioc-remove-encryption-key-all-users).
...
FS\_IOC\_ADD\_ENCRYPTION\_KEY may also be used to add a v2 policy key
*again*, even if it’s already added by other user(s). In this case,
FS\_IOC\_ADD\_ENCRYPTION\_KEY will just install a claim to the key for the
current user, rather than actually add the key again (but the key must
still be provided, as a proof of knowledge).

FS\_IOC\_ADD\_ENCRYPTION\_KEY returns 0 if either the key or a claim to
the key was either added or already exists.

FS\_IOC\_ADD\_ENCRYPTION\_KEY can fail with the following errors:

* `EACCES`: FSCRYPT\_KEY\_SPEC\_TYPE\_DESCRIPTOR was specified, but the
  caller does not have the CAP\_SYS\_ADMIN capability in the initial
...
combination with FS\_IOC\_REMOVE\_ENCRYPTION\_KEY (see [Removing keys](#removing-keys)),
...
[FS\_IOC\_ADD\_ENCRYPTION\_KEY](#fs-ioc-add-encryption-key):

* [FS\_IOC\_REMOVE\_ENCRYPTION\_KEY](#fs-ioc-remove-encryption-key)
* [FS\_IOC\_REMOVE\_ENCRYPTION\_KEY\_ALL\_USERS](#fs-ioc-remove-encryption-key-all-users)
...
Before using these ioctls, read the [Online attacks](#online-attacks) section for a
discussion of the security goals and limitations of these ioctls.

#### FS\_IOC\_REMOVE\_ENCRYPTION\_KEY

The FS\_IOC\_REMOVE\_ENCRYPTION\_KEY ioctl removes a claim to a master
...
* The key to remove is specified by `key_spec`:
  ...
  >   `key_spec.type` to FSCRYPT\_KEY\_SPEC\_TYPE\_DESCRIPTOR and fill
  >   in `key_spec.u.descriptor`. To remove this type of key, the
  >   calling process must have the CAP\_SYS\_ADMIN capability in the
  >   initial user namespace.
  > + To remove a key used by v2 encryption policies, set
  >   `key_spec.type` to FSCRYPT\_KEY\_SPEC\_TYPE\_IDENTIFIER and fill
  >   in `key_spec.u.identifier`.

For v2 policy keys, this ioctl is usable by non-root users. However,
to make this possible, it actually just removes the current user’s
claim to the key, undoing a single call to FS\_IOC\_ADD\_ENCRYPTION\_KEY.
Only after all claims are removed is the key really removed.

For example, if FS\_IOC\_ADD\_ENCRYPTION\_KEY was called with uid 1000,
then the key will be “claimed” by uid 1000, and
FS\_IOC\_REMOVE\_ENCRYPTION\_KEY will only succeed as uid 1000. Or, if
both uids 1000 and 2000 added the key, then for each uid
FS\_IOC\_REMOVE\_ENCRYPTION\_KEY will only remove their own claim. Only
once *both* are removed is the key really removed. (Think of it like
unlinking a file that may have hard links.)

If FS\_IOC\_REMOVE\_ENCRYPTION\_KEY really removes the key, it will also
...
FS\_IOC\_REMOVE\_ENCRYPTION\_KEY returns 0 if either the key was removed
...
FS\_IOC\_REMOVE\_ENCRYPTION\_KEY can fail with the following errors:

* `EACCES`: The FSCRYPT\_KEY\_SPEC\_TYPE\_DESCRIPTOR key specifier type
  was specified, but the caller does not have the CAP\_SYS\_ADMIN
...
#### FS\_IOC\_REMOVE\_ENCRYPTION\_KEY\_ALL\_USERS

FS\_IOC\_REMOVE\_ENCRYPTION\_KEY\_ALL\_USERS is exactly the same as
[FS\_IOC\_REMOVE\_ENCRYPTION\_KEY](#fs-ioc-remove-encryption-key), except that for v2 policy keys, the
ALL\_USERS version of the ioctl will remove all users’ claims to the
key, not just the current user’s. I.e., the key itself will always be
removed, no matter how many users have added it. This difference is
only meaningful if non-root users are adding and removing keys.

Because of this, FS\_IOC\_REMOVE\_ENCRYPTION\_KEY\_ALL\_USERS also requires
“root”, namely the CAP\_SYS\_ADMIN capability in the initial user
namespace. Otherwise it will fail with EACCES.

### Getting key status

#### FS\_IOC\_GET\_ENCRYPTION\_KEY\_STATUS

The FS\_IOC\_GET\_ENCRYPTION\_KEY\_STATUS ioctl retrieves the status of a
...
* `status` indicates whether the key is absent, present, or
  ...
  [FS\_IOC\_REMOVE\_ENCRYPTION\_KEY](#fs-ioc-remove-encryption-key) returned 0 but set the informational
  status flag FSCRYPT\_KEY\_REMOVAL\_STATUS\_FLAG\_FILES\_BUSY.
...
FS\_IOC\_GET\_ENCRYPTION\_KEY\_STATUS can fail with the following errors:
...
Among other use cases, FS\_IOC\_GET\_ENCRYPTION\_KEY\_STATUS can be useful
...
FS\_IOC\_GET\_ENCRYPTION\_KEY\_STATUS can only get the status of keys in
the filesystem-level keyring, i.e. the keyring managed by
[FS\_IOC\_ADD\_ENCRYPTION\_KEY](#fs-ioc-add-encryption-key) and [FS\_IOC\_REMOVE\_ENCRYPTION\_KEY](#fs-ioc-remove-encryption-key). It
...
* Unencrypted files, or files encrypted with a different encryption
  ...
  enforcement](#encryption-policy-enforcement). Attempts to do so will fail with EXDEV. However,
...
* Direct I/O is supported on encrypted files only under some
  circumstances. For details, see [Direct I/O support](#direct-i-o-support).
* The fallocate operations FALLOC\_FL\_COLLAPSE\_RANGE and
  FALLOC\_FL\_INSERT\_RANGE are not supported on encrypted files and will
...
* Directories may be listed, in which case the filenames will be
  ...
  encoding](#filename-hashing-and-encoding). The algorithm is subject to change, but it is
  guaranteed that the presented filenames will be no longer than
  NAME\_MAX bytes, will not contain the `/` or `\0` characters, and
...
[Documentation/block/inline-encryption.rst](../block/inline-encryption.html#inline-encryption).

On supported filesystems (currently ext4 and f2fs), fscrypt can use
blk-crypto instead of the kernel crypto API to encrypt/decrypt file
contents. To enable this, set CONFIG\_FS\_ENCRYPTION\_INLINE\_CRYPT=y in
...
CONFIG\_BLK\_INLINE\_ENCRYPTION\_FALLBACK=y, and the file must be
...
[Documentation/block/inline-encryption.rst](../block/inline-encryption.html#hardware-wrapped-keys). The rest of this section just focuses on
...
add a hardware-wrapped key with [FS\_IOC\_ADD\_ENCRYPTION\_KEY](#fs-ioc-add-encryption-key),
userspace must specify FSCRYPT\_ADD\_KEY\_FLAG\_HW\_WRAPPED in the
...
[IV\_INO\_LBLK\_64 policies](#iv-ino-lblk-64-policies) and [IV\_INO\_LBLK\_32 policies](#iv-ino-lblk-32-policies), as it
...
* The file must be using inline encryption. Usually this means that
  ...
  is also available. For details, see [Inline encryption support](#inline-encryption-support).
...
policy structs (see [Setting an encryption policy](#setting-an-encryption-policy)), except that the
context structs also contain a nonce. The nonce is randomly generated
by the kernel and is used as KDF input or as a tweak to cause
different files to be encrypted differently; see [Per-file encryption
keys](#per-file-encryption-keys) and [DIRECT\_KEY policies](#direct-key-policies).
...
possible to run most xfstests with the “test\_dummy\_encryption” mount
