...
> 출처(원문): https://docs.kernel.org/scsi/st.html
...
ST\_NBR\_MODE\_BITS in st.h. Mode 0 corresponds to the defaults discussed
...
4. If the open option O\_NONBLOCK is used, open succeeds even if the
drive is not ready. If O\_NONBLOCK is not used, the driver waits for
the drive to become ready. If this does not happen in ST\_BLOCK\_SECONDS
seconds, open fails with the errno value EIO. With O\_NONBLOCK the
...
The driver creates the directory /sys/class/scsi\_tape and populates it with
...
Each directory contains the entries: default\_blksize default\_compression
default\_density defined dev device driver. The file ‘defined’ contains 1
if the mode is defined and zero if not defined. The files ‘default\_\*’ contain
...
Each directory contains the entry ‘position\_lost\_in\_reset’. If this value is
...
1. Use opendir(3) on the directory /sys/class/scsi\_tape
...
4. Access the statistics from the /sys/class/scsi\_tape/<match>/stats
   directory (where <match> is a directory entry from /sys/class/scsi\_tape
...
3. other\_cnt
   :   * The number of I/Os issued to the tape drive other than read or
         ...
         following calculation io\_ms-read\_ms-write\_ms.
4. read\_byte\_cnt
...
The in\_flight value is incremented when an I/O starts the I/O
itself is not added to the statistics until it completes.

The total of read\_cnt, write\_cnt, and other\_cnt may not total to the same
value as iodone\_cnt at the device level. The tape statistics only count
...
The value shown in in\_flight is incremented before any statstics are
updated and decremented when an I/O completes after updating statistics.
The value of in\_flight is 0 when there are no I/Os outstanding that are
...
defining the value of the symbol ST\_SYSV. The semantics differ when a
...
ST\_FIXED\_BUFFER\_BLOCKS. With small block size this allows buffering of
...
allowed if ST\_BUFFER\_WRITES is non-zero and direct i/o is not used.
...
Read ahead for fixed block mode (ST\_READ\_AHEAD). Filling the buffer is
...
1. The initial segment that is used for all SCSI adapters including
   ...
   (PAGE\_SIZE << ST\_FIRST\_ORDER) bytes if the system can give a chunk of
   this size (and it is not larger than the buffer size specified by
   ST\_BUFFER\_BLOCKS). If this size is not available, the driver halves
   the size and tries again until the size of one page. The default
   settings in st\_options.h make the driver to try to allocate all of the
   buffer as one chunk.
2. The scatter/gather segments to fill the specified buffer size are
   ...
   of segments does not exceed ST\_FIRST\_SG.
3. The remaining segments between ST\_MAX\_SG (or the module parameter
   max\_sg\_segs) and the number of segments used in phases 1 and 2
...
|  |  |
| --- | --- |
| buffer\_kbs=xxx | the buffer size for fixed block mode is set to xxx kilobytes |
| write\_threshold\_kbs=xxx | the write threshold in kilobytes set to xxx |
| max\_sg\_segs=xxx | the maximum number of scatter/gather segments |
| try\_direct\_io=x | try direct transfer between user buffer and tape drive if this is non-zero |
...
segments, the parameter ‘st.max\_sg\_segs=xx’ should be used (xx is the
...
:   Explicitly load and unload the tape. If the
    command argument x is between MT\_ST\_HPLOADER\_OFFSET + 1 and
    MT\_ST\_HPLOADER\_OFFSET + 6, the number x is used sent to the
...
:   Is used for several purposes. The command is obtained from count
    with mask MT\_SET\_OPTIONS, the low order bits are used as argument.
    ...
    * MT\_ST\_BOOLEANS
      :   Sets the buffering options. The bits are the new states
          ...
          > MT\_ST\_BUFFER\_WRITES
          > :   write buffering (mode)
          >
          > MT\_ST\_ASYNC\_WRITES
          ...
          >     variable) (global)
          >
          > MT\_ST\_CAN\_PARTITIONS
          > :   enables support for partitioned
          >     tapes (global)
          >
          > MT\_ST\_SCSI2LOGICAL
    ...
    * MT\_ST\_SETBOOLEANS, MT\_ST\_CLEARBOOLEANS
    ...
    * MT\_ST\_DEF\_DENSITY, MT\_ST\_DEF\_DRVBUFFER
      :   Used to set or clear the density (8 bits), and drive buffer
          state (3 bits). If the value is MT\_ST\_CLEAR\_DEFAULT
    ...
    * MT\_ST\_DEF\_COMPRESSION
      :   The compression default will not be used if the value of
          ...
          MT\_ST\_CLEAR\_DEFAULT can be used to clear the compression
          default.
    ...
    * MT\_ST\_SET\_CLN
      :   Set the cleaning request interpretation parameters using
          ...
          generic status bit GMT\_CLN if a cleaning request bit pattern
...
:   Returns some status information.
    ...
    is stored in the lower word of the field mt\_erreg.
    The current block size and the density code are stored in the field
    mt\_dsreg (shifts for the subfields are MT\_ST\_BLKSIZE\_SHIFT and
    MT\_ST\_DENSITY\_SHIFT).
    The GMT\_xxx status bits reflect the drive status. GMT\_DR\_OPEN
    is set if there is no tape in the drive. GMT\_EOD means either
    end of recorded data or end of tape. GMT\_EOT means end of tape.

## Miscellaneous Compile Options

The recovered write errors are considered fatal if ST\_RECOVERED\_WRITE\_FATAL
is defined.

The maximum number of tape devices is determined by the define
ST\_MAX\_TAPES. If more tapes are detected at driver initialization, the
maximum is adjusted accordingly.

Immediate return from tape positioning SCSI commands can be enabled by
defining ST\_NOWAIT. If this is defined, the user should take care that
...
ST\_FAST\_EOM 1 (or using the MT\_ST\_OPTIONS ioctl). In this case the file
...
within file can be obtained if ST\_IN\_FILE\_POS is defined at compile
time or the MT\_ST\_CAN\_BSR bit is set for the drive with an ioctl.
...
with the kernel module parameter debug\_flag defaulting to 0. Debugging
can still be switched on and off with an ioctl. To enable debug at
module load time add debug\_flag=1 to the module load options, the
...
file /sys/bus/scsi/drivers/st/debug\_flag.
