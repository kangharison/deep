...
> 출처(원문): https://docs.kernel.org/scsi/qlogicfas.html
...
function. If there are any problems, try setting SLOW\_CABLE to 1, and
then try changing USE\_IRQ and TURBO\_PDMA to zero. If you are familiar
with SCSI, there are other settings which can tune the bus.

It may be a good idea to enable RESET\_AT\_START, especially if the
