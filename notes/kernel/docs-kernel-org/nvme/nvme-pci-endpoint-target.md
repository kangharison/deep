...
> 출처(원문): https://docs.kernel.org/nvme/nvme-pci-endpoint-target.html
...
The kernel must be compiled with the configuration options CONFIG\_PCI\_ENDPOINT,
CONFIG\_PCI\_ENDPOINT\_CONFIGFS, and CONFIG\_NVME\_TARGET\_PCI\_EPF enabled.
CONFIG\_PCI, CONFIG\_BLK\_DEV\_NVME and CONFIG\_NVME\_TARGET must also be enabled
(obviously).

In addition to this, at least one PCI endpoint controller driver should be
available for the endpoint hardware used.

To facilitate testing, enabling the null-blk driver (CONFIG\_BLK\_DEV\_NULL\_BLK)
is also recommended. With this, a simple setup using a null\_blk block device
...
and subsystem with a single namespace backed by a null\_blk device.
...
Next, create a null\_blk device (default settings give a 250 GB device without
...
Next, create and enable the subsystem namespace using the null\_blk block
device:
...
| deviceid | Anything is OK (e.g. PCI\_ANY\_ID) |
| revid | Do not care |
| progif\_code | Must be 0x02 (NVM Express) |
| baseclass\_code | Must be 0x01 (PCI\_BASE\_CLASS\_STORAGE) |
| subclass\_code | Must be 0x08 (Non-Volatile Memory controller) |
| cache\_line\_size | Do not care |
| subsys\_vendor\_id | Ignored (the subsystem vendor id of the NVMe target subsystem is used) |
| subsys\_id | Anything is OK (e.g. PCI\_ANY\_ID) |
| msi\_interrupts | At least equal to the number of queue pairs desired |
| msix\_interrupts | At least equal to the number of queue pairs desired |
| interrupt\_pin | Interrupt PIN to use if MSI and MSI-X are not supported |
...
|  |  |
| --- | --- |
| mdts\_kb | Maximum data transfer size in KiB (default: 512) |
