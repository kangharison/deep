...
> 출처(원문): https://docs.kernel.org/fb/framebuffer.html
...
hardware are organized. XF68\_FBDev is a general X server for bitmapped,
...
a driver just has to call [`register_framebuffer()`](../driver-api/frame-buffer.html#c.register_framebuffer "register_framebuffer") and supply some functions.
...
The X server (XF68\_FBDev) is the most notable application program for the frame
...
work 100% with XF68\_FBDev: the reported clock values are always incorrect.
...
> * left\_margin: time from sync to picture
> * right\_margin: time from picture to sync
> * upper\_margin: time from sync to picture
> * lower\_margin: time from picture to sync
> * hsync\_len: length of horizontal sync
> * vsync\_len: length of vertical sync
...
2. horizontal timings:
   ...
   left\_margin = HFL - SH2

   right\_margin = SH1 - HR

   hsync\_len = SH2 - SH1
3. vertical timings:

   upper\_margin = VFL - SV2

   lower\_margin = SV1 - VR
...
> * The manual pages for XFree86: XF68\_FBDev(1), XF86Config(4/5)
