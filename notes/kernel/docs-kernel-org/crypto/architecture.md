...
> 출처(원문): https://docs.kernel.org/crypto/architecture.html
...
* CRYPTO\_ALG\_TYPE\_KPP Key-agreement Protocol Primitive (KPP) such as
  an ECDH or DH implementation
* CRYPTO\_ALG\_TYPE\_HASH Raw message digest
...
* CRYPTO\_ALG\_TYPE\_SIG Asymmetric signature
...
1. `esp_output()` invokes [`crypto_aead_encrypt()`](api-aead.html#c.crypto_aead_encrypt "crypto_aead_encrypt") to trigger an
