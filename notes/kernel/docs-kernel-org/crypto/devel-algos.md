...
> 출처(원문): https://docs.kernel.org/crypto/devel-algos.html
...
filled with each, [`struct crypto_alg`](api-skcipher.html#c.crypto_alg "crypto_alg"), must be considered -- see below
...
crypto_alg`](api-skcipher.html#c.crypto_alg "crypto_alg") field .cra\_type is empty. The .cra\_u.cipher has to be
filled in with proper callbacks to implement this transformation.

See [`struct cipher_alg`](api-skcipher.html#c.cipher_alg "cipher_alg") below.
...
### Cipher Definition With struct skcipher\_alg

Struct skcipher\_alg defines a multi-block cipher, or more generally, a
...
- \_not\_ call any of .`final()`, .`finup()` or .`export()` at any point in future
