...
> 출처(원문): https://docs.kernel.org/admin-guide/device-mapper/cache.html
...
“invalidate\_cblocks” message, the cache device can be transitioned to
...
For the time being, a message “migration\_threshold <#sectors>”
...
messages are used. Refer to [Guidance for writing policies](cache-policies.html).
...
> | policy args | key/value pairs passed to the policy E.g. ‘sequential\_threshold 1024’ See [Guidance for writing policies](cache-policies.html) for details. |
...
> | no\_discard\_passdown | disable passing down discards from the cache to the origin’s data device. |
...
| core args | Key/value pairs for tuning the core e.g. migration\_threshold |
| policy name | Name of the policy |
| #policy args | Number of policy arguments to follow (must be even) |
| policy args | Key/value pairs e.g. sequential\_threshold |
...
| needs\_check | ‘needs\_check’ if set, ‘-’ if not set A metadata operation has failed, resulting in the needs\_check flag being set in the metadata’s superblock. The metadata device must be deactivated and checked/repaired before the cache can be made fully operational again. ‘-’ indicates needs\_check is not set. |
...
back. Cache blocks can be invalidated via the invalidate\_cblocks
...
when invalidate\_cblocks is used:
