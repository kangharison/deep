...
> 출처(원문): https://docs.kernel.org/admin-guide/device-mapper/vdo-design.html
...
dm-vdo. For usage, see [dm-vdo](vdo.html) in the same directory as this file.
...
request object (the “data\_vio”) which will be added to a work queue when
...
### The vio and data\_vio Structures
...
called a data\_vio to track information about their progress. A `struct
data_vio` contain a `struct vio` and also includes several other fields
related to deduplication and other vdo features. The data\_vio is the
primary unit of application work in vdo. Each data\_vio proceeds through a
set of steps to handle the application data, after which it is reset and
returned to a pool of data\_vios for reuse.

There is a fixed pool of 2048 data\_vios. This number was chosen to bound
...
that are required for a data\_vio. The recovery journal issues a flush
...
1. A data\_vio is obtained from the data\_vio pool and associated with the
   application bio. If there are no data\_vios available, the incoming bio
   will block until a data\_vio is available. This provides back pressure
   to the application. The data\_vio pool is protected by a spin lock.

   The newly acquired data\_vio is reset and the bio’s data is copied into
   the data\_vio if it is a write and the data is not all zeroes. The data
   must be copied because the application bio can be acknowledged before
   the data\_vio processing is complete, which means later processing steps
   will no longer have access to the application bio. The application bio
   may also be smaller than 4K, in which case the data\_vio will have
   already read the underlying block and the data is instead copied over
   the relevant portion of the larger block.
2. The data\_vio places a claim (the “logical lock”) on the logical address
   ...
   the logical address and the value is a pointer to the data\_vio
   currently handling that address.

   If a data\_vio looks in the hashtable and finds that another data\_vio is
   ...
   lock holder that it is waiting. Most notably, a new data\_vio waiting
   ...
   This stage requires the data\_vio to get an implicit lock on the
   ...
   described above.
3. The data\_vio traverses the block map tree to ensure that all the
   ...
   1. If any page-node in the tree has not yet been allocated, it must be
      ...
      data\_vio to lock the page-node that needs to be allocated. This
      lock, like the logical block lock in step 2, is a hashtable entry
      that causes other data\_vios to wait for the allocation process to
      complete.
      ...
      The data\_vio journals the intent to add the new node to the block
      ...
      updated, the data\_vio proceeds down the tree. Any other data\_vios
      waiting on this allocation also proceed.
   2. In the steady-state case, the block map tree nodes will already be
      allocated, so the data\_vio just traverses the tree until it finds
      the required leaf node. The location of the mapping (the “block map
      slot”) is recorded in the data\_vio so that later steps do not need
      to traverse the tree again. The data\_vio then releases the implicit
      logical zone lock.
4. If the block is a zero block, skip to step 9. Otherwise, an attempt is
   ...
   data\_vio can write its data somewhere even if deduplication and
   ...
   The data\_vio will search each slab in a zone until it finds a free
   ...
   free block or runs out of zones to search. The data\_vio will acquire a
   ...
   claims that data\_vios can have on physical blocks. The pbn\_lock is
   ...
   data\_vio from considering it free. The reference counters are a
   sub-component of the slab and are thus also covered by the implicit
   physical zone lock.
5. If an allocation was obtained, the data\_vio has all the resources it
   ...
   thread to prevent the application callback from blocking other data\_vio
   operations.

   If an allocation could not be obtained, the data\_vio continues to
...
6. At this point vdo must determine where to store the application data.
   The data\_vio’s data is hashed and the hash (the “record name”) is
   recorded in the data\_vio.
7. The data\_vio reserves or joins a `struct hash_lock`, which manages all of
   the data\_vios currently writing the same data. Active hash locks are
   ...
   If there is no existing hash lock for this data\_vio’s record\_name, the
   data\_vio obtains a hash lock from the pool, adds it to the hashtable,
   and sets itself as the new hash lock’s “agent.” The hash\_lock pool is
   ...
   written. If a hash lock for the data\_vio’s record\_name already exists,
   and the data\_vio’s data is the same as the agent’s data, the new
   data\_vio will wait for the agent to complete its work and then share
   its result.

   In the rare case that a hash lock exists for the data\_vio’s hash but
   the data does not match the hash lock’s agent, the data\_vio skips to
...
8. The hash lock agent attempts to deduplicate or compress its data with
   ...
   1. The agent initializes and sends its embedded deduplication request
      ...
      require the data\_vio to get any locks because the index components
      manage their own locking. The data\_vio waits until it either gets a
      response from the index or times out.
   2. If the deduplication index returns advice, the data\_vio attempts to
      ...
      data\_vio’s data, and that it can accept more references. If the
      physical address is already locked by another data\_vio, the data at
   ...
   3. If the data matches and the physical block can add references, the
      agent and any other data\_vios waiting on it will record this
      physical block as their new physical address and proceed to step 9
      to record their new mapping. If there are more data\_vios in the hash
      lock than there are references available, one of the remaining
      data\_vios becomes the new agent and continues to step 8d as if no
   ...
   4. If no usable duplicate block was found, the agent first checks that
      ...
      to. If the agent does not have an allocation, some other data\_vio in
      the hash lock that does have an allocation takes over as agent. If
      none of the data\_vios have an allocated physical block, these writes
      are out of space, so they proceed to step 13 for cleanup.
   5. The agent attempts to compress its data. If the data does not
      compress, the data\_vio will continue to step 8h to write its data
      directly.
      ...
      data\_vios. All compression operations require the implicit lock on
      the packer zone.
      ...
      data\_vios into a single data block. This means that a data\_vio may
      wait in the packer for an arbitrarily long time for other data\_vios
      to fill out the compressed block. There is a mechanism for vdo to
      evict waiting data\_vios when continuing to wait would cause
      problems. Circumstances causing an eviction include an application
      flush, device shutdown, or a subsequent data\_vio trying to overwrite
      the same logical block address. A data\_vio may also be evicted from
      ...
      data\_vio will proceed to step 8h to write its data directly.
   6. If the agent fills a packer bin, either because all 14 of its slots
      ...
      using the allocated physical block from one of its data\_vios. Step
      8d has already ensured that an allocation is available.
   7. Each data\_vio sets the compressed block as its new physical address.
      The data\_vio obtains an implicit lock on the physical zone and
   ...
   8. Any data\_vio evicted from the packer will have an allocation from
      step 3. It will write its data to that allocated physical block.
   9. After the data is written, if the data\_vio is the agent of a hash
      lock, it will reacquire the implicit hash zone lock and share its
      physical address with as many other data\_vios in the hash lock as
      possible. Each data\_vio will then proceed to step 9 to record its
      new mapping.
...
9. The data\_vio determines the previous mapping of the logical address.
   ...
   data\_vio will reserve a slot in the cache and load the desired page
   into it, possibly evicting an older cached page. The data\_vio then
   ...
   lock.
10. The data\_vio makes an entry in the recovery journal containing the
    ...
    recovery journal lock. The data\_vio will wait in the journal until all
...
11. Once the recovery journal entry is stable, the data\_vio makes two slab
...
12. Once both of the reference count updates are done, the data\_vio
...
13. If the data\_vio has a hash lock, it acquires the implicit hash zone
    lock and releases its hash lock to the pool.

    The data\_vio then acquires the implicit physical zone lock and releases
    ...
    that block back to zero to free it for use by subsequent data\_vios.

    The data\_vio then acquires the implicit logical zone lock and releases
    the logical block lock acquired in step 2.

    The application bio is then acknowledged if it has not previously been
    acknowledged, and the data\_vio is returned to the pool.

*Read Path*

An application read bio follows a much simpler set of steps. It does steps
1 and 2 in the write path to obtain a data\_vio and lock its logical
address. If there is already a write data\_vio in progress for that logical
address that is guaranteed to complete, the read data\_vio will copy the
data from the write data\_vio and return it. Otherwise, it will look up the
...
physical block address. A read data\_vio will not allocate block map tree
...
data\_vio will return all zeroes. A read data\_vio handles cleanup and
...
operations, and a single data\_vio is used throughout this operation.
