...
> 출처(원문): https://docs.kernel.org/admin-guide/device-mapper/dm-ima.html
...
be queried from userspace by setting DM\_IMA\_MEASUREMENT\_FLAG with
DM\_TABLE\_STATUS\_CMD.

*NOTE #2:*

The Kernel configuration CONFIG\_IMA\_DISABLE\_HTABLE allows measurement of duplicate records.

To support recording duplicate IMA events in the IMA log, the Kernel needs to be configured with
CONFIG\_IMA\_DISABLE\_HTABLE=y.
...
The IMA measurement log has the following format for ‘dm\_table\_load’:
...
The IMA measurement log has the following format for ‘dm\_device\_resume’:
...
The IMA measurement log has the following format for ‘dm\_device\_remove’:
...
The IMA measurement log has the following format for ‘dm\_table\_clear’:
...
The IMA measurement log has the following format for ‘dm\_device\_rename’:
...
The ‘target\_attributes’ (described as part of EVENT\_DATA in ‘Table load’
