...
> 출처(원문): https://docs.kernel.org/admin-guide/device-mapper/dm-zoned.html
...
> 0 <size> zoned <nr\_zones> zones <nr\_unmap\_rnd>/<nr\_rnd> random <nr\_unmap\_seq>/<nr\_seq> sequential

where <nr\_zones> is the total number of zones, <nr\_unmap\_rnd> is the number
of unmapped (ie free) random zones, <nr\_rnd> the total number of zones,
<nr\_unmap\_seq> the number of unmapped sequential zones, and <nr\_seq> the
