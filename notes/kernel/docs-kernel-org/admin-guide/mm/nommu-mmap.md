...
> 출처(원문): https://docs.kernel.org/admin-guide/mm/nommu-mmap.html
...
> 1. Anonymous mapping, MAP\_PRIVATE
>
...
>    > pages.
> 2. Anonymous mapping, MAP\_SHARED
>
>    > These behave very much like private mappings, except that they’re
>    > shared across fork() or clone() without CLONE\_VM in the MMU case. Since
>    > the no-MMU case doesn’t support these, behaviour is identical to
>    > MAP\_PRIVATE there.
> 3. File, MAP\_PRIVATE, PROT\_READ / PROT\_EXEC, !PROT\_WRITE
>
...
>    > >   if the backing device has the NOMMU\_MAP\_DIRECT capability and
>    > >   appropriate mapping protection capabilities. Ramfs, romfs, cramfs
>    > >   and mtd might all permit this.
>    > > * If the backing device can’t or won’t permit direct sharing,
>    > >   but does have the NOMMU\_MAP\_COPY capability, then a copy of the
>    > >   appropriate bit of the file will be read into a contiguous bit of
>    > >   memory and any extraneous space beyond the EOF will be cleared
>    > > * Writes to the file do not affect the mapping; writes to the mapping
>    > >   are visible in other processes (no MMU protection), but should not
>    > >   happen.
> 4. File, MAP\_PRIVATE, PROT\_READ / PROT\_EXEC, PROT\_WRITE
>
>    > In the MMU case: like the non-PROT\_WRITE case, except that the pages in
...
>    >
>    > In the no-MMU case: works much like the non-PROT\_WRITE case, except
>    > that a copy is always taken and never shared.
> 5. Regular file / blockdev, MAP\_SHARED, PROT\_READ / PROT\_EXEC / PROT\_WRITE
>
>    > In the MMU case: VM regions backed by pages read from file; changes to
...
> 6. Memory backed regular file, MAP\_SHARED, PROT\_READ / PROT\_EXEC / PROT\_WRITE
>
...
> 7. Memory backed blockdev, MAP\_SHARED, PROT\_READ / PROT\_EXEC / PROT\_WRITE
>
>    > In the MMU case: As for ordinary regular files.
>    >
...
> 8. Memory backed chardev, MAP\_SHARED, PROT\_READ / PROT\_EXEC / PROT\_WRITE
>
>    > In the MMU case: As for ordinary regular files.
>    >
...
>    returned by `malloc()` - mmap() can take a MAP\_UNINITIALIZED flag to
>    indicate to the kernel that it shouldn’t bother clearing the memory before
>    returning it. Note that CONFIG\_MMAP\_ALLOW\_UNINITIALIZED must be enabled
...
> 6. Supplying MAP\_FIXED or a requesting a particular mapping address will
>    result in an error.
...
mapping, and may move it [[1]](#id2) if MREMAP\_MAYMOVE is specified and if the new size
of the mapping exceeds the size of the slab object currently occupied by the
memory to which the mapping refers, or if a smaller slab object could be used.

MREMAP\_FIXED is not supported, though it is ignored if there’s no change of
...
file->f\_op->`get_unmapped_area()` operation. The mmap() routines will call this
...
The file->f\_op->mmap() operation will be called to actually inaugurate the
mapping. It can be rejected at that point. Returning the ENOSYS error will
cause the mapping to be copied instead if NOMMU\_MAP\_COPY is specified.

The vm\_ops->close() routine will be invoked when the last mapping on a chardev
is removed. An existing mapping will be shared, partially or not, if possible
without notifying the driver.

It is permitted also for the file->f\_op->`get_unmapped_area()` operation to
...
mapping request will be rejected if NOMMU\_MAP\_COPY is not specified, and a
