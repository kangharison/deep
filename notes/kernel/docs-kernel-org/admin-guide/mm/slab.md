...
> 출처(원문): https://docs.kernel.org/admin-guide/mm/slab.html
...
CONFIG\_SLUB\_DEBUG=y) but it is off by default (unless built with
CONFIG\_SLUB\_DEBUG\_ON=y). You can enable debugging only for selected
...
## Some more sophisticated uses of slab\_debug:

Parameters may be given to `slab_debug`. If none is specified then full
debugging is enabled. Format:

slab\_debug=<Debug-Options>
:   Enable options for all slabs

slab\_debug=<Debug-Options>,<slab name1>,<slab name2>,...
...
a result of storing the metadata (for example, caches with PAGE\_SIZE object
...
SLUB can validate all object if the kernel was booted with slab\_debug. In
...
list\_lock once in a while to deal with partial slabs. That overhead is
...
:   allows to specify how many objects must at least fit into one
    ...
    (list\_lock) where contention may occur.
...
to be booted with slab\_debug) then the following output will be dumped
into the syslog:
...
1. Description of the problem encountered
   ...
   (Object allocation / free information is only available if SLAB\_STORE\_USER is
   set for the slab. slab\_debug sets that option)
2. The object contents if an object was involved.
   ...
   :   The Redzone following the object. The Redzone is used to detect
       ...
       (Redzone information is only available if SLAB\_RED\_ZONE is set.
       slab\_debug sets that option)
...
2. pass stats file(-s) to `slabinfo-gnuplot.sh` script:
   ...
   file:
   - Slabcache Totals: FOO\_STATS-totals.png
   - Slabs sorted by size: FOO\_STATS-slabs-by-size.png
   - Slabs sorted by loss: FOO\_STATS-slabs-by-loss.png
