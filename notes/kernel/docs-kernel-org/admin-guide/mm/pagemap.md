...
> 출처(원문): https://docs.kernel.org/admin-guide/mm/pagemap.html
...
>   `fs/proc/task_mmu.c`, above pagemap\_read):
>
...
>   > + Bit 63 page present
>
>   Since Linux 4.0 only users with the CAP\_SYS\_ADMIN capability can get PFNs.
>   In 4.0 and 4.1 opens by unprivileged fail with -EPERM. Starting from
>   4.2 the PFN field is zeroed if the user does not have CAP\_SYS\_ADMIN.
...
>   page, indexed by PFN.
>
>   The flags are (from `fs/proc/page.c`, above kpageflags\_read):
>
>   > 0. LOCKED
>   > 1. ERROR
>   > 2. REFERENCED
...
:   A compound page with order N consists of 2^N physically contiguous pages.
    ...
    pages are hugeTLB pages ([HugeTLB Pages](hugetlbpage.html)),
...
24 - ZERO\_PAGE
:   Zero page for pfn\_zero or huge\_zero page.
...
:   The page is in the unevictable (non-)LRU list It is somehow pinned and
    ...
    shmctl(SHM\_LOCK) and `mlock()` memory segments.
...
page is swapped out (i.e. SOFT\_DIRTY).
...
holes (none/non-allocated) by specifying the SEEK\_DATA flag on the file where
...
>    can get full etc. The walk ending address is specified in``end\_walk``.
...
“PAGE\_IS\_WRITTEN” category is used with uffd write protect-enabled ranges to
