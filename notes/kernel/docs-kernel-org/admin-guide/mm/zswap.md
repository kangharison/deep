...
> 출처(원문): https://docs.kernel.org/admin-guide/mm/zswap.html
...
being swapped out. However, it will \_not\_ immediately write out or fault
...
in the swap\_map goes to 0) the swap code calls the zswap invalidate function
to free the compressed entry.

Zswap seeks to be simple in its policies. Sysfs attributes allow for one user
controlled policy:

* max\_pool\_percent - The maximum percentage of memory that the compressed
  pool can occupy.
