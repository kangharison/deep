...
> 출처(원문): https://docs.kernel.org/admin-guide/syscall-user-dispatch.html
...
> prctl(PR\_SET\_SYSCALL\_USER\_DISPATCH, <op>, <offset>, <length>, [selector])

<op> is either PR\_SYS\_DISPATCH\_EXCLUSIVE\_ON/PR\_SYS\_DISPATCH\_INCLUSIVE\_ON
or PR\_SYS\_DISPATCH\_OFF, to enable and disable the mechanism globally for
that thread. When PR\_SYS\_DISPATCH\_OFF is used, the other fields must be zero.

For PR\_SYS\_DISPATCH\_EXCLUSIVE\_ON [<offset>, <offset>+<length>) delimit
...
For PR\_SYS\_DISPATCH\_INCLUSIVE\_ON [<offset>, <offset>+<length>) delimit
...
can be set to SYSCALL\_DISPATCH\_FILTER\_ALLOW or SYSCALL\_DISPATCH\_FILTER\_BLOCK.
Any other value should terminate the program with a SIGSYS.

Additionally, a tasks syscall user dispatch configuration can be peeked
and poked via the PTRACE\_(GET|SET)\_SYSCALL\_USER\_DISPATCH\_CONFIG ptrace
