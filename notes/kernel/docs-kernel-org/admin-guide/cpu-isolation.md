...
> 출처(원문): https://docs.kernel.org/admin-guide/cpu-isolation.html
...
* Bound work, where “bound” means tied to a specific CPU, usually
  ...
  >   This is the case of RCU with CONFIG\_RCU\_NOCB\_CPU.
  ...
  >   replacement. E.g.: the timer tick is shut down on nohz\_full
...
* CONFIG\_CPUSETS=y for the cpusets-based interface
...
* [Control Group v2](cgroup-v2.html) cpuset isolated partitions are recommended
...
* The file /proc/irq/\*/smp\_affinity as explained in detail in
  [SMP IRQ affinity](../core-api/irq/irq-affinity.html) page.
* The “irqaffinity=” kernel boot parameter for a default setting.
* The “managed\_irq” flag in the “isolcpus=” kernel boot parameter
  tries a best effort affinity override for managed IRQs.

### Full Dynticks (aka nohz\_full)
...
Global timer callbacks are also isolated from the nohz\_full CPUs.
...
* RCU adds more overhead due to the locked, offloaded and threaded
  callbacks processing (the same that would be obtained with “rcu\_nocbs”
  boot parameter).
...
* Deep C-states may result in latency issues upon wake-up. If this
  ...
  parameters such as processor.max\_cstate or intel\_idle.max\_cstate.
  More finegrained tunings are described in
  [CPU Idle Time Management](pm/cpuidle.html) page
...
* Full dynticks: nohz\_full=7
* IRQs isolation: irqaffinity=0-6
* Managed IRQs isolation: isolcpus=managed\_irq,7
* Prevent SMT: nosmt

The full command line is then:

> nohz\_full=7 irqaffinity=0-6 isolcpus=managed\_irq,7 nosmt
...
second during 10 seconds when user\_loop was running.
...
events: workqueue, timer, irq\_vector, etc... It also can be
interesting to enable the tick\_stop event to diagnose why the tick is
retained when that happens.

Some tools may also be useful for higher level analysis:

* [rtla](../tools/rtla/rtla.html) provides a suite of tools to analyze
  latency and noise in the system. For example [rtla-osnoise](../tools/rtla/rtla-osnoise.html)
