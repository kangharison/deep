...
> 출처(원문): https://docs.kernel.org/admin-guide/pm/cpufreq.html
...
algorithms. That is done by the [intel\_pstate](intel_pstate.html) scaling driver.
...
As mentioned before, the [intel\_pstate](intel_pstate.html) scaling driver bypasses the scaling
governor layer of `CPUFreq` and provides its own P-state selection algorithms.
Consequently, if [intel\_pstate](intel_pstate.html) is used, scaling governors are not attached to
new policy objects. Instead, the driver’s `->setpolicy()` callback is invoked
to register per-CPU utilization update callbacks for each policy. These
callbacks are invoked by the CPU scheduler in the same way as for scaling
governors, but in the [intel\_pstate](intel_pstate.html) case they both determine the P-state to
...
:   List of `CPUFreq` scaling governors present in the kernel that can
    be attached to this policy or (if the [intel\_pstate](intel_pstate.html) scaling driver is
...
:   The scaling governor currently attached to this policy or (if the
    [intel\_pstate](intel_pstate.html) scaling driver is in use) the scaling algorithm
    provided by the driver that is currently applied to this policy.

    This attribute is read-write and writing to it will cause a new scaling
    governor to be attached to this policy or a new scaling algorithm
    provided by the scaling driver to be applied to it (in the
    [intel\_pstate](intel_pstate.html) case), as indicated by the string written to this
...
`scaling_setspeed`
:   This attribute is functional only if the [userspace](#userspace) scaling governor
...
LWN.net article [[1]](#id2) for a description of the PELT mechanism). Then, the new
...
This governor generally is regarded as a replacement for the older [ondemand](#ondemand)
and [conservative](#conservative) governors (described below), as it is simpler and more
...
It estimates the CPU load in the same way as the [ondemand](#ondemand) governor described
...
[intel\_pstate](intel_pstate.html)).
