...
> 출처(원문): https://docs.kernel.org/admin-guide/pm/sleep-states.html
...
This state can be used on platforms without support for [standby](#standby)
or [suspend-to-RAM](#s2ram), or it can be used in addition to any of the
...
I/O devices into low-power states, which is done for [suspend-to-idle](#s2idle) too, nonboot CPUs are taken offline and all low-level system functions
are suspended during transitions into this state. For this reason, it should
allow more energy to be saved relative to [suspend-to-idle](#s2idle), but
...
reduced relative to [suspend-to-idle](#s2idle) and it may be necessary to
...
contents. All of the steps carried out when entering [standby](#standby)
...
relative to [suspend-to-idle](#s2idle) and [standby](#standby) and it
...
:   This file contains a list of strings representing sleep states supported
    ...
    [hibernation](#hibernation), [suspend-to-idle](#s2idle) and
    [standby](#standby) sleep states, respectively. The “mem” string
...
:   This file contains a list of strings representing supported system
    ...
    and “deep”. The “s2idle” string always represents [suspend-to-idle](#s2idle) and, by convention, “shallow” and “deep” represent
    [standby](#standby) and [suspend-to-RAM](#s2ram),
    respectively.
...
[suspend-to-idle](#s2idle) state. The first one is to write “freeze”
...
into the [standby](#standby) state (the strings to write to the control
...
system go into the [suspend-to-RAM](#s2ram) state (write “deep” into
...
supporting [suspend-to-RAM](#s2ram)) or “s2idle”, but it can be overridden
by the value of the `mem_sleep_default` parameter in the kernel command line.
On some systems with ACPI, depending on the information in the ACPI tables, the
default may be “s2idle” even if [suspend-to-RAM](#s2ram) is supported in
principle.
