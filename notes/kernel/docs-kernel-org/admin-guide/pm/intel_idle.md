# intel_idleCPU Idle Time Management Driver

> 출처(원문): https://docs.kernel.org/admin-guide/pm/intel_idle.html
...
[CPU idle time management subsystem](cpuidle.html) in the Linux kernel
...
[CPU Idle Time Management](cpuidle.html) if you have not done that yet.]
...
Manual [[1]](#id3)). Accordingly, `intel_idle` refuses to work with processors in
...
subsystem (see [Representation of Idle States](cpuidle.html#idle-states-representation) in
...
configured to ignore the ACPI tables; see [below](#intel-idle-parameters).]
...
[Representation of Idle States](cpuidle.html#idle-states-representation) in
[CPU Idle Time Management](cpuidle.html)). This basically means that
...
“CX\_ACPI”, where X is the index of that idle state in the final list (note that
...
[above](#intel-idle-enumeration-of-states)), and whether or not the processor
...
[below](#intel-idle-parameters)), the idle states information provided by the
...
[above](#intel-idle-enumeration-of-states).
...
for configuring individual CPUs is registered via [`cpuhp_setup_state()`](../../core-api/cpu_hotplug.html#c.cpuhp_setup_state "cpuhp_setup_state"), which
...
even if they have been enumerated (see [Power Management Quality of Service for CPUs](cpuidle.html#cpu-pm-qos) in
[CPU Idle Time Management](cpuidle.html)).
...
idle state; see [Representation of Idle States](cpuidle.html#idle-states-representation) in
[CPU Idle Time Management](cpuidle.html)).
...
[PM QoS](cpuidle.html#cpu-pm-qos) or the `max_cstate` module parameter of
`intel_idle` described [above](#intel-idle-parameters) must be used to
