# intel_pstateCPU Performance Scaling Driver

> 출처(원문): https://docs.kernel.org/admin-guide/pm/intel_pstate.html
...
[CPU performance scaling subsystem](cpufreq.html) in the Linux kernel
(`CPUFreq`). It is a scaling driver for the Sandy Bridge and later
generations of Intel processors. Note, however, that some of those processors
may not be supported. [To understand `intel_pstate` it is necessary to know
how `CPUFreq` works in general, so this is the time to read
[CPU Performance Scaling](cpufreq.html) if you have not done that yet.]

For the processors supported by `intel_pstate`, the P-state concept is broader
than just an operating frequency or an operating performance point (see the
LinuxCon Europe 2015 presentation by Kristen Accardi [[1]](#id8) for more
information about that). For this reason, the representation of P-states used
by `intel_pstate` internally follows the hardware specification (for details
refer to Intel Software Developer’s Manual [[2]](#id9)). However, the `CPUFreq` core
...
registered (see [below](#status-attr)).
...
contains the string “intel\_pstate”.
...
(see [Energy vs Performance Hints](#energy-performance-hints) below). Moreover, any attempts to change
...
`sysfs` for all `CPUFreq` policies contains the string “intel\_cpufreq”.
...
[below](#acpi-cpufreq) for more information).

Moreover, since `intel_pstate` always knows what the real turbo threshold is
(even if the Configurable TDP feature is enabled in the processor), its
`no_turbo` attribute in `sysfs` (described [below](#no-turbo-attr)) should
...
> * The maximum supported [non-turbo P-state](#turbo).
> * Whether or not turbo P-states are supported at all.
> * The maximum supported [one-core turbo P-state](#turbo) (if turbo
...
[Capacity-Aware Scheduling Support](#cas) it registers an Energy Model for the processor. This allows the
Energy-Aware Scheduling (EAS) support to be enabled in the CPU scheduler if
`schedutil` is used as the `CPUFreq` governor which requires `intel_pstate`
to operate in the [passive mode](#passive-mode).
...
:   Number of P-states supported by the processor (between 0 and 255
    ...
    [Turbo P-states Support](#turbo)).

    This attribute is present only if the value exposed by it is the same
    for all of the CPUs in the system.

    The value of this attribute is not affected by the `no_turbo`
    setting described [below](#no-turbo-attr).

    This attribute is read-only.

`turbo_pct`
:   Ratio of the [turbo range](#turbo) size to the size of the entire
...
:   If set (equal to 1), the driver is not allowed to set any turbo P-states
    (see [Turbo P-states Support](#turbo)). If unset (equal to 0, which is the
    ...
    (see [Interpretation of Policy Attributes](#policy-attributes-interpretation) below for details).

`hwp_dynamic_boost`
:   This attribute is only present if `intel_pstate` works in the
    [active mode with the HWP feature enabled](#active-mode-hwp) in
...
:   Operation mode of the driver: “active”, “passive” or “off”.
    ...
    “active”
    :   The driver is functional and in the [active mode](#active-mode).

    “passive”
    :   The driver is functional and in the [passive mode](#passive-mode).
...
[CPU Performance Scaling](cpufreq.html) is special with `intel_pstate`
...
If the `no_turbo` [global attribute](#no-turbo-attr) is set, the driver
...
`intel_pstate` works in the [active mode](#active-mode):
...
The meaning of these attributes in the [passive mode](#passive-mode) is the
same as for other scaling drivers.

Additionally, the value of the `scaling_driver` attribute for `intel_pstate`
depends on the operation mode of the driver. Namely, it is either
“intel\_pstate” (in the [active mode](#active-mode)) or “intel\_cpufreq”
(in the [passive mode](#passive-mode)).

### Coordination of P-State Limits

`intel_pstate` allows P-state limits to be set in two ways: with the help of
the `max_perf_pct` and `min_perf_pct` [global attributes](#global-attributes) or via the `scaling_max_freq` and `scaling_min_freq`
...
In the [active mode with the HWP feature enabled](#active-mode-hwp), the
resulting effective values are written into hardware registers whenever the
limits change in order to request its internal P-state selection logic to always
set P-states within these limits. Otherwise, the limits are taken into account
by scaling governors (in the [passive mode](#passive-mode)) and by the
...
“/sys/devices/system/cpu/cpu\*/power/energy\_perf\_bias” interface.
...
[[3]](#id10) for details on the `_PSS` objects and the format of the information
returned by them).
...
[turbo range](#turbo) is represented by one item in it (the topmost one).
...
[Configurable TDP feature](#turbo) allowing the platform firmware to set
...
[passive mode](#passive-mode), except that the number of P-states it can
...
`active`
:   Register `intel_pstate` in the [active mode](#active-mode) to
    start with.

`passive`
:   Register `intel_pstate` in the [passive mode](#passive-mode) to
    start with.
...
:   Use per-logical-CPU P-State limits (see
    [Coordination of P-State Limits](#pstate-limits-coordination) for details).

`no_cas`
:   Do not enable [capacity-aware scheduling](#cas) which is enabled
...
it works in the [active mode](#active-mode).
...
If `intel_pstate` works in the [passive mode](#passive-mode), the
