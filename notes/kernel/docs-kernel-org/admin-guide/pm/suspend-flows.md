...
> 출처(원문): https://docs.kernel.org/admin-guide/pm/suspend-flows.html
...
[sleep states](sleep-states.html). Hibernation requires more than one
...
significant differences between the [suspend-to-idle](sleep-states.html#s2idle) code flows
and the code flows related to the [suspend-to-RAM](sleep-states.html#s2ram) and
[standby](sleep-states.html#standby) sleep states.

The [suspend-to-RAM](sleep-states.html#s2ram) and [standby](sleep-states.html#standby) sleep states
...
state to the [suspend-to-idle](sleep-states.html#s2idle) sleep state:
...
>    *late suspend* and *noirq suspend* (see [Device Power Management Basics](../../driver-api/pm/devices.html#driverapi-pm-devices) for more
...
[suspend-to-idle](sleep-states.html#s2idle) sleep state into the working state:
...
>    *resume* and *complete* (see [Device Power Management Basics](../../driver-api/pm/devices.html#driverapi-pm-devices) for more
>    information on what exactly happens in each phase).
>
>    Every device is visited in each phase, but typically it is not physically
>    accessed in more than two of them.
...
> 3. Thawing tasks.
>
>    Tasks frozen in step 2 of the preceding [suspend](#s2idle-suspend)
...
>
>    This is analogous to step 1 of the [suspend](#s2idle-suspend) transition
...
>    described [above](#s2idle-suspend).
> 2. Freezing tasks.
>
...
>    described [above](#s2idle-suspend), but the arming of IRQs for system
