...
> 출처(원문): https://docs.kernel.org/admin-guide/pm/intel-speed-select.html
...
offline CPUs which are not present in the enable\_cpu\_mask for this performance
level.

Now if the base\_frequency is checked:
...
“CONFIG\_INTEL\_HFI\_THERMAL” is required. The minimum version of the tool
...
frequency\_weight, which can be specified per CLOS group. The goal of
...
Each CLOS group has its own attributes including min, max, freq\_weight and
