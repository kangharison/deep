# 7.27.The Virtual Video Test Driver (vivid)

> 출처(원문): https://docs.kernel.org/admin-guide/media/vivid.html
...
* no\_error\_inj:
  ...
  > removed. Unless overridden by ccs\_cap\_mode and/or ccs\_out\_mode the
...
* cache\_hints:
  ...
  > consistency hint capability (V4L2\_BUF\_CAP\_SUPPORTS\_MMAP\_CACHE\_HINTS).
...
## 7.27.2. Video Capture

This is probably the most frequently used feature. The video capture device
can be configured by using the module options num\_inputs, input\_types and
ccs\_cap\_mode (see “Configuring the driver” for more detailed information),
...
### 7.27.2.2. TV and S-Video Inputs
...
When you start capturing in V4L2\_FIELD\_ALTERNATE mode the first buffer will
...
+/- 1 MHz around the channel. Beyond that it is just noise. The VIDIOC\_G\_TUNER
...
Finally, for these inputs the v4l2\_timecode `struct is` filled in the
dequeued v4l2\_buffer struct.
...
mode for interlaced formats is always V4L2\_FIELD\_ALTERNATE. For HDMI the
...
num\_outputs, output\_types and ccs\_out\_mode (see “Configuring the driver”
...
### 7.27.3.2. HDMI Output

The HDMI output supports all CEA-861 and DMT timings, both progressive and
interlaced, for pixelclock frequencies between 25 and 600 MHz. The field
mode for interlaced formats is always V4L2\_FIELD\_ALTERNATE.
...
An HDMI output has a valid EDID which can be obtained through VIDIOC\_G\_EDID.
...
support both. This is determined by the node\_types module option. In all
...
(undecoded) VBI, those that only support sliced (decoded) VBI and those that
support both. This is determined by the node\_types module option.
...
## 7.27.8. Software Defined Radio Receiver
...
### 7.27.12.1. User Controls - Test Controls
...
meaning that one or more menu items return EINVAL when VIDIOC\_QUERYMENU is called.
...
### 7.27.12.2. User Controls - Video Capture
...
cause a control event to be sent with the V4L2\_EVENT\_CTRL\_CH\_RANGE flag set.
...
#### 7.27.12.4.2. Capture Feature Selection Controls
...
* Sensor Flipped Horizontally:
  ...
  > V4L2\_IN\_ST\_HFLIP input status flag is set. This emulates the case where
  > a sensor is for example mounted upside down.
* Sensor Flipped Vertically:

  > the image is flipped vertically and the
  > V4L2\_IN\_ST\_VFLIP input status flag is set. This emulates the case where
  > a sensor is for example mounted upside down.
...
* Colorspace:
  ...
  >
  > Changing the colorspace will result in the V4L2\_EVENT\_SOURCE\_CHANGE
...
* Transfer Function:
  ...
  > by colorspace conversions.
  >
  > Changing the transfer function will result in the V4L2\_EVENT\_SOURCE\_CHANGE
  > to be sent since it emulates a detected colorspace change.
* Y’CbCr Encoding:
  ...
  >
  > Changing the Y’CbCr encoding will result in the V4L2\_EVENT\_SOURCE\_CHANGE
  > to be sent since it emulates a detected colorspace change.
* Quantization:
  ...
  >
  > Changing the quantization will result in the V4L2\_EVENT\_SOURCE\_CHANGE
  > to be sent since it emulates a detected colorspace change.
...
* Enable Capture Cropping:
  ...
  > the ccs\_cap\_mode module option is set to the default value of -1 and if
  > the no\_error\_inj module option is set to 0 (the default).
* Enable Capture Composing:

  > enables composing support. This control is only
  > present if the ccs\_cap\_mode module option is set to the default value of
  > -1 and if the no\_error\_inj module option is set to 0 (the default).
* Enable Capture Scaler:

  > enables support for a scaler (maximum 4 times upscaling
  > and downscaling). This control is only present if the ccs\_cap\_mode
  > module option is set to the default value of -1 and if the no\_error\_inj
...
#### 7.27.12.4.3. Output Feature Selection Controls
...
* Enable Output Cropping:
  ...
  > enables crop support. This control is only present if
  > the ccs\_out\_mode module option is set to the default value of -1 and if
  > the no\_error\_inj module option is set to 0 (the default).
* Enable Output Composing:

  > enables composing support. This control is only
  > present if the ccs\_out\_mode module option is set to the default value of
  > -1 and if the no\_error\_inj module option is set to 0 (the default).
* Enable Output Scaler:

  > enables support for a scaler (maximum 4 times upscaling
  > and downscaling). This control is only present if the ccs\_out\_mode
  > module option is set to the default value of -1 and if the no\_error\_inj
  > module option is set to 0 (the default).

#### 7.27.12.4.4. Error Injection Controls

The following two controls are only valid for video and vbi capture.

* Standard Signal Mode:

  > selects the behavior of VIDIOC\_QUERYSTD: what should it return?
  >
  > Changing this control will result in the V4L2\_EVENT\_SOURCE\_CHANGE
  > to be sent since it emulates a changed input condition (e.g. a cable
  > was plugged in or out).
* Standard:

  > selects the standard that VIDIOC\_QUERYSTD should return if the
...
* DV Timings Signal Mode:
  ...
  > selects the behavior of VIDIOC\_QUERY\_DV\_TIMINGS: what
  > should it return?
  >
  > Changing this control will result in the V4L2\_EVENT\_SOURCE\_CHANGE
  > to be sent since it emulates a changed input condition (e.g. a cable
...
* DV Timings:
  ...
  > selects the timings the VIDIOC\_QUERY\_DV\_TIMINGS should return
  > if the previous control is set to “Selected DV Timings”.
  >
  > Changing this control will result in the V4L2\_EVENT\_SOURCE\_CHANGE
  > to be sent since it emulates changed input timings.

The following controls are only present if the no\_error\_inj module option
...
* Inject Fatal Streaming Error:
  ...
  > [`vb2_queue_error()`](../../driver-api/media/v4l2-videobuf2.html#c.vb2_queue_error "vb2_queue_error") function is called.

#### 7.27.12.4.5. VBI Raw Capture Controls
...
### 7.27.12.5. Digital Video Controls
...
* Transmit Mode:
  ...
  > affects the reported colorspace since DVI\_D outputs will always use
  > sRGB.

### 7.27.12.6. FM Radio Receiver Controls
...
* Radio HW Seek Mode:
  ...
  > determines if VIDIOC\_S\_HW\_FREQ\_SEEK will be bounded by the frequency
...
### 7.27.12.7. FM Radio Modulator Controls
...
### 7.27.12.8. Metadata Capture Controls
...
## 7.27.13. Video, Sliced VBI and HDMI CEC Looping
...
(see module option n\_devs). If there is only one vivid instance (the default), then
...
* the field settings must be identical on both sides. Same reason as above:
  ...
  be 100% accurate.
* field settings V4L2\_FIELD\_SEQ\_TB/BT are not supported. While it is possible to
...
VIDIOC\_G\_TUNER). However, it will also look at the frequency set by the radio
...
## 7.27.15. Cropping, Composing, Scaling
...
ccs\_cap\_mode and ccs\_out\_mode module options. See “Configuring the driver” on
...
VIDIOC\_ENUM\_FRAMESIZES to list a set of discrete framesizes that it supports.
...
VIDIOC\_S\_FMT allows but that few drivers implement.
...
will have a plane that has a non-zero data\_offset of 128 bytes. It is rare for
data\_offset to be non-zero, so this is a useful feature for testing applications.

Video output will also honor any data\_offset that the application set.

## 7.27.17. Output Overlay
...
same for the overlay to work. Otherwise VIDIOC\_OVERLAY will return an error.

Output overlays only work if the driver has been configured to create a
framebuffer by setting flag 0x10000 in the node\_types module option. The
...
## 7.27.18. CEC (Consumer Electronics Control)
...
## 7.27.19. Some Future Improvements
...
* Add support for V4L2\_CID\_BG\_COLOR for video outputs
* Add ARGB888 overlay support: better testing of the alpha channel
* Improve pixel aspect support in the tpg code by passing a real v4l2\_fract
