# 7.12.The mgb4 driver

> 출처(원문): https://docs.kernel.org/admin-guide/media/mgb4.html
...
### 7.12.1.1. Global (PCI card) parameters
...
### 7.12.1.2. Common FPDL3/GMSL input parameters

**input\_id** (R):
:   Input number ID, zero based.

**oldi\_lane\_width** (RW):
...
:   Video link status. If the link is locked, chips are properly connected and
    ...
    A value of 0 is equivalent to the V4L2\_IN\_ST\_NO\_SYNC flag of the V4L2
    VIDIOC\_ENUMINPUT status bits.
...
:   Video stream status. A stream is detected if the link is locked, the input
    ...
    A value of 0 is equivalent to the V4L2\_IN\_ST\_NO\_SIGNAL flag of the V4L2
    VIDIOC\_ENUMINPUT status bits.
...
:   Video stream width. This is the actual width as detected by the HW.
    ...
    The value is identical to what VIDIOC\_QUERY\_DV\_TIMINGS returns in the width
    field of the v4l2\_bt\_timings struct.

**video\_height** (R):
:   Video stream height. This is the actual height as detected by the HW.

    The value is identical to what VIDIOC\_QUERY\_DV\_TIMINGS returns in the height
    field of the v4l2\_bt\_timings struct.

**vsync\_status** (R):
:   The type of VSYNC pulses as detected by the video format detector.

    The value is equivalent to the flags returned by VIDIOC\_QUERY\_DV\_TIMINGS in
    the polarities field of the v4l2\_bt\_timings struct.
...
:   Input pixel clock frequency in kHz.
    ...
    The value is identical to what VIDIOC\_QUERY\_DV\_TIMINGS returns in
    the pixelclock field of the v4l2\_bt\_timings struct.

    *Note: The frequency\_range parameter must be set properly first to get
...
:   Width of the HSYNC signal in PCLK clock ticks.
    ...
    The value is identical to what VIDIOC\_QUERY\_DV\_TIMINGS returns in
    the hsync field of the v4l2\_bt\_timings struct.

**vsync\_width** (R):
:   Width of the VSYNC signal in PCLK clock ticks.

    The value is identical to what VIDIOC\_QUERY\_DV\_TIMINGS returns in
    the vsync field of the v4l2\_bt\_timings struct.
...
:   Number of PCLK pulses between deassertion of the HSYNC signal and the first
    ...
    The value is identical to what VIDIOC\_QUERY\_DV\_TIMINGS returns in
    the hbackporch field of the v4l2\_bt\_timings struct.
...
:   Number of PCLK pulses between the end of the last valid pixel in the video
    ...
    The value is identical to what VIDIOC\_QUERY\_DV\_TIMINGS returns in
    the hfrontporch field of the v4l2\_bt\_timings struct.
...
:   Number of video lines between deassertion of the VSYNC signal and the video
    ...
    The value is identical to what VIDIOC\_QUERY\_DV\_TIMINGS returns in
    the vbackporch field of the v4l2\_bt\_timings struct.
...
:   Number of video lines between the end of the last valid pixel line (marked
    ...
    The value is identical to what VIDIOC\_QUERY\_DV\_TIMINGS returns in
    the vfrontporch field of the v4l2\_bt\_timings struct.
...
:   PLL frequency range of the OLDI input clock generator. The PLL frequency is
    ...
    oldi\_lane\_width is set to “single” and PCLK/2 if oldi\_lane\_width is set to
    “dual”.
...
### 7.12.1.3. Common FPDL3/GMSL output parameters
...
### 7.12.1.4. FPDL3 specific input parameters

**fpdl3\_input\_width** (RW):
...
### 7.12.1.5. FPDL3 specific output parameters

**fpdl3\_output\_width** (RW):
...
### 7.12.1.6. GMSL specific input parameters
...
Depending on the CONFIG\_MTD\_PARTITIONED\_MASTER kernel configuration, you may
