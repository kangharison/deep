...
> 출처(원문): https://docs.kernel.org/admin-guide/ramoops.html
...
>   power of two.
> * `mem_type` to specify if the memory type (default is pgprot\_writecombine).
> * `mem_name` to specify a memory region defined by `reserve_mem` command
>   line parameter.

Typically the default value of `mem_type=0` should be used as that sets the pstore
mapping to pgprot\_writecombine. Setting `mem_type=1` attempts to use
...
>
>    > reserve\_mem=2M:4096:oops ramoops.mem\_name=oops
