...
> 출처(원문): https://docs.kernel.org/admin-guide/reporting-issues.html
...
issue. Check the [MAINTAINERS](../process/maintainers.html#maintainers) file for how its developers
...
* If you never reported an issue to a FLOSS project before, consider skimming
  guides like [How to ask good questions](https://jvns.ca/blog/good-questions/), [How To Ask Questions The Smart Way](http://www.catb.org/esr/faqs/smart-questions.html), and [How to Report
  Bugs Effectively](https://www.chiark.greenend.org.uk/~sgtatham/bugs.html),.
...
[Reporting regressions](reporting-regressions.html) explains this in more
...
[Security bugs](../process/security-bugs.html) before proceeding, as it
...
If your kernel is tainted, study [Tainted kernels](tainted-kernels.html)
...
To illustrate how to use the [MAINTAINERS](../process/maintainers.html#maintainers) file, let’s assume
...
MAINTAINERS file. In the case of ‘ath10k\_pci’ you won’t find anything, as the
...
the proper place to report: the script ‘scripts/get\_maintainer.pl’ which tries
...
please try to enable CONFIG\_KALLSYMS when configuring your kernel.
Additionally, enable CONFIG\_DEBUG\_KERNEL and CONFIG\_DEBUG\_INFO, too; the
latter is the relevant one of those two, but can only be reached if you enable
the former. Be aware CONFIG\_DEBUG\_INFO increases the storage space required to
...
works if you enabled CONFIG\_DEBUG\_INFO and CONFIG\_KALLSYMS when configuring
...
[Bisecting a regression](bug-bisect.html) describes in detail. That process
...
[Reporting regressions](reporting-regressions.html); that document also
...
Now send or file the report like the [MAINTAINERS](../process/maintainers.html#maintainers) file told
...
See [Security bugs](../process/security-bugs.html) for more information.
...
the document [Bisecting a regression](bug-bisect.html) for details how to
...
within rules outlined in [Everything you ever wanted to know about Linux -stable releases](../process/stable-kernel-rules.html).
