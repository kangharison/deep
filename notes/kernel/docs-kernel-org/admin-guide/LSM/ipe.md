...
> 출처(원문): https://docs.kernel.org/admin-guide/LSM/ipe.html
...
documentation about IPE please see [the design docs](../../security/ipe.html).
...
The digest\_cache LSM [[1]](#digest-cache-lsm) is a system that when combined with IPE,
...
`AT_EXECVE_CHECK` flag ([AT\_EXECVE\_CHECK](../../userspace-api/check_exec.html)),
...
IPE policy is a plain-text [[2]](#devdoc) policy composed of multiple statements
...
| --- | --- | --- | --- |
| ipe\_op | string | No | The IPE operation name associated with the log |
| ipe\_hook | string | No | The name of the LSM hook that triggered the IPE event |
...
#### 1421 AUDIT\_IPE\_CONFIG\_CHANGE
...
| Field | Value Type | Optional? | Description of Value |
| --- | --- | --- | --- |
| old\_active\_pol\_name | string | Yes | The name of previous active policy |
| old\_active\_pol\_version | string | Yes | The version of previous active policy |
| old\_policy\_digest | string | Yes | The hash of previous active policy |
| new\_active\_pol\_name | string | No | The name of current active policy |
| new\_active\_pol\_version | string | No | The version of current active policy |
| new\_policy\_digest | string | No | The hash of current active policy |
| auid | integer | No | The login user ID |
| ses | integer | No | The login session ID |
| lsm | string | No | The lsm name associated with the event |
| res | integer | No | The result of the audited operation(success/fail) |

#### 1422 AUDIT\_IPE\_POLICY\_LOAD
...
| Field | Value Type | Optional? | Description of Value |
| --- | --- | --- | --- |
| policy\_name | string | Yes | The policy\_name |
| policy\_version | string | Yes | The policy\_version |
| policy\_digest | string | Yes | The policy hash |
| auid | integer | No | The login user ID |
| ses | integer | No | The login session ID |
| lsm | string | No | The lsm name associated with the event |
| res | integer | No | The result of the audited operation(success/fail) |
...
| old\_enforcing | integer | No | The enforcing state IPE is being switched from, 1 is in enforcing mode, 0 is in permissive mode |
| auid | integer | No | The login user ID |
| ses | integer | No | The login session ID |
...
> `FIRMWARE`:
>
> > Pertains to firmware being loaded via the firmware\_class interface.
...
> implemented by using switch\_root(8) [[3]](#switch-root). Therefore the
...
> dependency on the DM\_VERITY module. This property is controlled by
...
> ```
>
> The supported DigestNames for dmverity\_roothash are [[4]](#dmveritydigests)
>
> > * blake2b-512
> > * blake2s-256
> > * sha256
...
> ```
>
> The supported DigestNames for fsverity\_digest are [[5]](#fsveritydigest)
>
> > * sha256
> > * sha512
...
* [Developer and design docs for IPE](../../security/ipe.html)
...
> the two is the policy. [[2]](#devdoc)
>
...
Please see [the design docs](../../security/ipe.html) for more on
this topic.
