...
> 출처(원문): https://docs.kernel.org/admin-guide/cgroup-v2.html
...
v1 is available under [Documentation/admin-guide/cgroup-v1/index.rst](cgroup-v1/index.html#cgroup-v1).
...
>     controllers, and then seeding it with CLONE\_INTO\_CGROUP is
...
“cgroup.procs”, “cgroup.threads”, “cgroup.subtree\_control”, etc.).
...
>     [Documentation/accounting/psi.rst](../accounting/psi.html#psi) for details.
...
realtime processes. For a kernel built with the CONFIG\_RT\_GROUP\_SCHED option
...
CONFIG\_RT\_GROUP\_SCHED enabled kernel.

With CONFIG\_RT\_GROUP\_SCHED disabled, this limitation does not apply and some of
...
CONFIG\_RT\_GROUP\_SCHED. Other controllers can be used for the resource control of
realtime processes irrespective of CONFIG\_RT\_GROUP\_SCHED.
...
check out [Documentation/scheduler/sched-ext.rst](../scheduler/sched-ext.html#sched-ext).
...
>     range [1, 10000].
>
>     If the cgroup has been configured to be SCHED\_IDLE (cpu.idle = 1),
...
>     values similar to the sched\_setattr(2). This minimum utilization
...
>     values similar to the sched\_setattr(2). This maximum utilization
...
>     The default is 0.
>
>     This is the cgroup analog of the per-task SCHED\_IDLE sched policy.
>     Setting this value to a 1 will make the scheduling policy of the
>     cgroup SCHED\_IDLE. The threads inside the cgroup will retain their
...
>     pressure.
>
>     If memory.high is opened with O\_NONBLOCK then the synchronous
...
>     next charge request.
>
>     Please note that with O\_NONBLOCK, there is a chance that the
...
>
>     If memory.max is opened with O\_NONBLOCK, then the synchronous
...
>
>     Tasks with the OOM protection (oom\_score\_adj set to -1000)
...
>     memory.numa\_stat). We use ‘npn’ (non-per-node) as the tag
>     to indicate that it will not show in the memory.numa\_stat.
>
>     > anon
>     > :   Amount of memory used in anonymous mappings such as
>     >     `brk()`, `sbrk()`, and mmap(MAP\_ANONYMOUS). Note that
...
>     >     (kernel\_stack, pagetables, percpu, vmalloc, slab) in
...
>     >     memory management lists), inactive\_foo + active\_foo may not be equal to
...
>     >     structures.
>     >
>     > workingset\_refault\_anon
>     > :   Number of refaults of previously evicted anonymous pages.
>     >
>     > workingset\_refault\_file
...
>     >
>     > workingset\_restore\_anon
>     > :   Number of restored anonymous pages which have been detected as
>     >     an active workingset before they got reclaimed.
>     >
>     > workingset\_restore\_file
...
>     >
>     > pgscan\_khugepaged (npn)
>     > :   Amount of scanned pages by khugepaged (in an inactive LRU list)
>     >
>     > pgscan\_proactive (npn)
...
>     >
>     > pgsteal\_khugepaged (npn)
>     > :   Amount of reclaimed pages by khugepaged
>     >
>     > pgsteal\_proactive (npn)
...
>     >     a size smaller than PAGE\_SIZE, so they are stored as-is.
>     >
>     > thp\_fault\_alloc (npn)
>     > :   Number of transparent hugepages which were allocated to satisfy
>     >     a page fault. This counter is not present when CONFIG\_TRANSPARENT\_HUGEPAGE
>     >     is not set.
>     >
>     > thp\_collapse\_alloc (npn)
>     > :   Number of transparent hugepages which were allocated to allow
>     >     collapsing an existing range of pages. This counter is not
>     >     present when CONFIG\_TRANSPARENT\_HUGEPAGE is not set.
>     >
>     > thp\_swpout (npn)
>     > :   Number of transparent hugepages which are swapout in one piece
>     >     without splitting.
>     >
>     > thp\_swpout\_fallback (npn)
>     > :   Number of transparent hugepages which were split before swapout.
>     >     Usually because failed to allocate some continuous swap space
>     >     for the huge page.
>     >
>     > numa\_pages\_migrated (npn)
>     > :   Number of pages migrated by NUMA balancing.
>     >
>     > numa\_pte\_updates (npn)
>     > :   Number of pages whose page table entries are modified by
>     >     NUMA balancing to produce NUMA hinting faults on access.
>     >
>     > numa\_hint\_faults (npn)
...
>     >     cgroup is mounted with the memory\_hugetlb\_accounting option).
>
> memory.numa\_stat
...
use memory\_recursiveprot. Configuring all descendants of a parent with finite
...
>     model based controller (CONFIG\_BLK\_CGROUP\_IOCOST) which
...
>     controller (CONFIG\_BLK\_CGROUP\_IOCOST) which currently
...
>
>     If needed, tools/cgroup/iocost\_coef\_gen.py can be used to
...
avg\_lat value in io.stat for your workload group to get an idea of the
latency you see during normal operation. Use the avg\_lat value as a basis for
...
* Artificial delay induction. There are certain types of IO that cannot be
  ...
  originating group is being throttled you will see the use\_delay and delay
...
* If I/O priority class policy is promote-to-rt, change the request I/O
  priority class to IOPRIO\_CLASS\_RT and change the request I/O priority
  level to 4.
...
>     >     limit (see also pids\_localevents).
>
> pids.events.local
...
create bpf programs of type BPF\_PROG\_TYPE\_CGROUP\_DEVICE and attach
them to cgroups with BPF\_CGROUP\_DEVICE flag. On an attempt to access a
device file, corresponding BPF programs will be executed, and depending
on the return value the attempt will succeed or fail with -EPERM.

A BPF\_PROG\_TYPE\_CGROUP\_DEVICE program takes a pointer to the
bpf\_cgroup\_dev\_ctx structure, which describes the device access attempt:
access type (mknod/read/write) and device (type, major and minor numbers).
If the program returns 0, the attempt fails with -EPERM, otherwise it
succeeds.

An example of BPF\_PROG\_TYPE\_CGROUP\_DEVICE program may be found in
tools/testing/selftests/bpf/progs/dev\_cgroup.c in the kernel source tree.
...
>     generated on this file reflects only the local events.
>
> hugetlb.<hugepagesize>.numa\_stat
> :   Similar to memory.numa\_stat, it shows the numa information of the
...
cgroup resources. Controller is enabled by the CONFIG\_CGROUP\_MISC config
option.

A resource can be added to the controller via `enum misc_res_type`{} in the
include/linux/misc\_cgroup.h file and the corresponding name via misc\_res\_name[]
...
include/linux/misc\_cgroup.h.

#### Misc Interface Files

Miscellaneous controller provides 3 interface files. If two misc resources (res\_a and res\_b) are registered then:
...
2. the process has CAP\_SYS\_ADMIN against the target cgroup
   namespace’s userns
...
address\_space\_operations->`writepages()` to annotate bio’s using the
...
>     before submission.
>
> wbc\_account\_cgroup\_owner(@wbc, @folio, @bytes)
