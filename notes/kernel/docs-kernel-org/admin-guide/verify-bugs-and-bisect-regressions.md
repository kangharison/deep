...
> 출처(원문): https://docs.kernel.org/admin-guide/verify-bugs-and-bisect-regressions.html
...
over to the* ‘[step-by-step guide](#introguide-bissbs)’ *below. It utilizes
...
> [Preparations: set up everything to build your own kernels](#introprep-bissbs).
>
> [Segment 1: try to reproduce the problem with the latest codebase](#introlatestcheck-bissbs).
>
> [Segment 2: check if the kernels you build work fine](#introworkingcheck-bissbs).
>
> [Segment 3: perform a bisection and validate the result](#introbisect-bissbs).
>
> [Complementary tasks: cleanup during and after following this guide](#introclosure-bissbs).
>
> [Optional tasks: test reverts, patches, or later versions](#introoptional-bissbs).
...
out [Reporting issues](reporting-issues.html), which works in conjunction
...
as a regression check out [Reporting regressions](reporting-regressions.html).

If you run into any problems while following this guide or have ideas how to
improve it, [please let the kernel developers know](#submit-improvements-vbbr).
...
[Build kernels on a different machine](#buildhost-bis) below.

* Create a fresh backup and put system repair and restore tools at hand, just
  to be prepared for the unlikely case of something going sideways.

  [[details](#backup-bisref)]
...
* Retrieve the mainline Linux sources; then change into the directory holding
  ...
  *Note, the following describe how to retrieve the sources using a full
  ...
  [reference section describes two alternatives](#sources-bisref) *:
...
* Start preparing a kernel build configuration (the ‘.config’ file).
  ...
  like ‘arch/x86/configs/x86\_64\_defconfig’, then the build infra failed to find
...
* Check if you may want or need to adjust some other kernel configuration
  ...
  + Are you running Debian? Then you want to avoid known problems by performing
    ...
    [[details](#configmods-distros-bisref)].
  + If you want to influence other aspects of the configuration, do so now using
    ...
    [[details](#configmods-individual-bisref)].
...
be a waste of time. [[details](#introlatestcheck-bisref)]
...
* Did you just built a stable or longterm kernel? And were you able to reproduce
  ...
  stable team. See [Reporting issues](reporting-issues.html) for details.

  [[details](#recheckstablebroken-bisref)]
...
otherwise would be a waste of time. [[details](#introworkingcheck-bisref)]
...
or a patch to Thorsten Leemhuis <[linux@leemhuis.info](mailto:linux%40leemhuis.info)> while ideally CCing the
public Linux docs mailing list <[linux-doc@vger.kernel.org](mailto:linux-doc%40vger.kernel.org)>. Such feedback is
...
[[back to step-by-step guide](#introprep-bissbs)].
...
[[back to step-by-step guide](#backup-bissbs)]

#### Remove anything related to externally maintained kernel modules

> *Remove all software that depends on externally developed kernel drivers or
> builds them automatically.* [[...](#vanilla-bissbs)]
...
considered ‘vanilla’ anymore, as [Reporting issues](reporting-issues.html)
explains in more detail.

[[back to step-by-step guide](#vanilla-bissbs)]
...
its purpose; ‘[Kernel module signing facility](module-signing.html)’ and various web
...
debug symbols’](#debugsymbols-bissbs) and its [accompanying reference
section’](#debugsymbols-bisref), as disabling then will reduce the consumed disk
space by quite a few gigabytes.

[[back to step-by-step guide](#diskspace-bissbs)]

#### Bisection range

> *Determine the kernel versions considered ‘good’ and ‘bad’ throughout this
> guide.* [[...](#rangecheck-bissbs)]
...
[[back to step-by-step guide](#rangecheck-bissbs)]

#### Install build requirements

> *Install all software required to build a Linux kernel.*
> [[...](#buildrequires-bissbs)]
...
[[back to step-by-step guide](#buildrequires-bissbs)]
...
[[back to step-by-step guide](#sources-bissbs)]
[[back to section intro](#sources-bisref)]
...
[[back to step-by-step guide](#localmodconfig-bissbs)]

#### Tag the kernels about to be build

> *Ensure all the kernels you will build are clearly identifiable using a
> special tag and a unique version identifier.* [[...](#tagging-bissbs)]
...
[[back to step-by-step guide](#tagging-bissbs)]

#### Decide to enable or disable debug symbols

> *Decide how to handle debug symbols.* [[...](#debugsymbols-bissbs)]
...
‘Decode failure messages’ in [Reporting issues](reporting-issues.html)
explains this process in more detail.

[[back to step-by-step guide](#debugsymbols-bissbs)]
...
> *Are you running* [[...](#configmods-bissbs)]
...
[[back to step-by-step guide](#configmods-bissbs)]

##### Individual adjustments

> *If you want to influence the other aspects of the configuration, do so
> now.* [[...](#configmods-bissbs)]
...
[[back to step-by-step guide](#saveconfig-bissbs)]
...
> still occurs with the latest codebase.* [[...](#introlatestcheck-bissbs)]
...
* These steps will rule out if your problem is caused by some change in the
  ...
  [Reporting regressions](reporting-regressions.html) explains in more detail.
...
[[back to step-by-step guide](#introlatestcheck-bissbs)]

#### Check out the latest Linux codebase

> *Check out the latest Linux codebase.*
> [[...](#checkoutmaster-bissbs)]
...
[[back to step-by-step guide](#build-bissbs)]

#### Put the kernel in place

> *Install the kernel you just built.* [[...](#install-bissbs)]
...
* On almost all Linux distributions installkernel will store your kernel’s
  image in /boot/, usually as ‘/boot/vmlinuz-<kernelrelease\_id>’; often it will
  put a ‘System.map-<kernelrelease\_id>’ alongside it.
* On most distributions installkernel will then generate an ‘initramfs’
  ...
  ‘/boot/initramfs-<kernelrelease\_id>.img’ or
  ‘/boot/initrd-<kernelrelease\_id>’. Commodity distributions rely on this file
  for booting, hence ensure to execute the make target ‘modules\_install’ first,
...
[[back to step-by-step guide](#storagespace-bissbs)]
...
[[back to step-by-step guide](#recheckbroken-bissbs)]
...
[[back to step-by-step guide](#introworkingcheck-bissbs)]

#### Build your own version of the ‘good’ kernel

> *Build your own variant of the working kernel and check if the feature that
> regressed works as expected with it.* [[...](#recheckworking-bissbs)]
...
[[back to step-by-step guide](#bisectbuild-bissbs)]
...
[[back to step-by-step guide](#bisecttest-bissbs)]
...
[[back to step-by-step guide](#revert-bissbs)]
...
> of the kernels you installed.* [[...](#introclosure-bissbs)]

The steps in this section describe clean-up procedures.

[[back to step-by-step guide](#introclosure-bissbs)].

#### Cleaning up during the bisection

> *To remove one of the kernels you installed, look up its ‘kernelrelease’
> identifier.* [[...](#makeroom-bissbs)]
...
[[back to step-by-step guide](#makeroom-bissbs)]
...
tasks: test reverts, patches, or later versions](#introoptional-bissbs).

Additional tests are also the reason why you want to keep the
~/kernel-config-working file around for a few weeks.

[[back to step-by-step guide](#finishingtouch-bissbs)]
...
[[back to step-by-step guide](#introoptional-bissbs)].
...
* After executing ‘[Boot into the working kernel and briefly use the
  apparently broken feature](#bootworking-bissbs)’, save the list of loaded
  ...
  build configuration for your kernel](#oldconfig-bisref)’ for hints on where
...
* Continue the guide on the build host (e.g. with ‘[Ensure to have enough
  free space for building [...]](#diskspace-bissbs)’).
* When you reach ‘[Start preparing a kernel build configuration[...]](#oldconfig-bissbs)’: before running `make olddefconfig` for the first time,
...
* During the next step to ‘[disable any apparently superfluous kernel
  modules](#localmodconfig-bissbs)’ use the following command instead:
...
* Switch to the test machine to check if you have enough space to hold another
  ...
  loader’s configuration](#install-bisref); on some distributions the following
