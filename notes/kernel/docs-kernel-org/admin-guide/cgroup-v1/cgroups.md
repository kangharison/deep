...
> 출처(원문): https://docs.kernel.org/admin-guide/cgroup-v1/cgroups.html
...
[CPUSETS](cpusets.html)

Original copyright statements from [CPUSETS](cpusets.html):
...
access. For example, cpusets (see [CPUSETS](cpusets.html)) allow
...
css\_set is allocated. The appropriate existing css\_set is located by
...
a single cgroup on its cgrp\_link\_list field, and a list of
cg\_cgroup\_links for a single css\_set on its cg\_link\_list.

Thus the set of tasks in a cgroup can be listed by iterating over
each css\_set that references the cgroup, and sub-iterating over
each css\_set’s task set.
...
value of their parents’ notify\_on\_release settings. The default value of
a cgroup hierarchy’s release\_agent path is empty.

### 1.5 What does clone\_children do ?

This flag only affects the cpuset controller. If the clone\_children
...
release\_agent. Rebinding is hardly useful as it only works when the
hierarchy is empty and release\_agent itself should be replaced with
...
system needs to create a cgroup\_subsys object. This contains
various methods, which are callbacks from the cgroup system, along
with a subsystem ID which will be assigned by the cgroup system.

Other fields in the cgroup\_subsys object include:

* subsys\_id: a unique array index for the subsystem, indicating which
  entry in cgroup->subsys[] this subsystem should be managing.
* name: should be initialized to a unique subsystem name. Should be
  no longer than MAX\_CGROUP\_TYPE\_NAMELEN.
* early\_init: indicate if the subsystem needs early initialization
  at system boot.
...
There is a global mutex, cgroup\_mutex, used by the cgroup
...
- while holding the task’s alloc\_lock (via `task_lock()`)
- inside an [`rcu_read_lock()`](../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") section via [`rcu_dereference()`](../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference")
...
* define a cgroup\_subsys object called <name>\_cgrp\_subsys

Each subsystem may export the following methods. The only mandatory
methods are css\_alloc/free. Any others that are null are presumed to
be successful no-ops.

`struct cgroup_subsys_state *css_alloc(struct cgroup *cgrp)`
(cgroup\_mutex held by caller)

Called to allocate a subsystem state object for a cgroup. The
subsystem should allocate its subsystem state object for the passed
cgroup, returning a pointer to the new object on success or a
[`ERR_PTR()`](../../core-api/kernel-api.html#c.ERR_PTR "ERR_PTR") value. On success, the subsystem pointer should point to
...
visible to cgroup\_for\_each\_child/descendant\_\*() iterators. The
...
while the caller holds cgroup\_mutex and it is ensured that either
...
“cgroup.subtree\_control” but should remain enabled because other
...
Called when the task\_struct is freed.
