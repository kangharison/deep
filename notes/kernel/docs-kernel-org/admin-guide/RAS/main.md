...
> 출처(원문): https://docs.kernel.org/admin-guide/RAS/main.html
...
as “DIMM\_A1”, as given by the *locator* field. Please notice that this
...
are referred to as the *syndrome*[[1]](#f1)[[2]](#f2).
...
please read [Configurable sysfs parameters for the x86-64 machine check code](../../arch/x86/x86_64/machinecheck.html) at the Kernel tree.
...
reports it, then an edac\_device device probably can be constructed to
...
Each `mc` device controls a set of memory modules [[4]](#f4). These modules
...
ranked DIMMs are placed in slots DIMM\_A0 and DIMM\_B0, then they will
...
> [ABI file testing/sysfs-devices-edac](../abi-testing-files.html#abi-file-testing-sysfs-devices-edac)

### `dimmX` or `rankX` directories

The recommended way to use the EDAC subsystem is to look at the information
provided by the `dimmX` or `rankX` directories [[5]](#f5).

A typical EDAC system has the following structure under
`/sys/devices/system/edac/`[[6]](#f6):
...
At the location `/sys/devices/system/edac` (sysfs) new edac\_device devices
will appear.
...
> |  |  |
> | --- | --- |
> | log\_ce | boolean to log CE events |
> | log\_ue | boolean to log UE events |
> | panic\_on\_ue | boolean to `panic` the system if an UE is encountered (default off, can be set true via startup script) |
> | poll\_msec | time period between POLL cycles for events |

The test\_device\_edac device adds at least one of its own custom control:

> |  |  |
> | --- | --- |
> | test\_bits | which in the current test driver does nothing but show how it is installed. A ported driver can add one or more such controls and/or attributes for specific uses. One out-of-tree driver uses controls here to allow for ERROR INJECTION operations to hardware injection registers |
...
> |  |  |
> | --- | --- |
> | ce\_count | total of CE events of subdirectories |
> | ue\_count | total of UE events of subdirectories |
...
> |  |  |
> | --- | --- |
> | ce\_count | which is counter of CE events for this `block` of hardware being monitored |
> | ue\_count | which is counter of UE events for this `block` of hardware being monitored |
