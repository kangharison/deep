...
> 출처(원문): https://docs.kernel.org/admin-guide/cifs/usage.html
...
mount.cifs with the following flag: CIFS\_ALLOW\_USR\_SUID
...
>     same domain (e.g. running winbind or nss\_ldap) and
...
>     can be enabled by specifying file\_mode and dir\_mode on
>     the client. (default)
>
> forcegid
...
>     not specified then the nls\_default specified
...
>
> perm
> :   Client does permission checks (vfs\_permission check of uid
...
>
> locallease
> :   This option is rarely needed. Fcntl F\_SETLEASE is
...
| open\_files | List all the open file handles on all active SMB sessions. |
| mount\_params | List of all mount parameters available for the module |
...
Setting it to 4 requires CONFIG\_CIFS\_STATS2 to be set in kernel configuration
...
Additional information is available if CONFIG\_CIFS\_STATS2 is enabled in the
...
|  |  |
| --- | --- |
| 1. enable\_oplocks | Enable or disable oplocks. Oplocks are enabled by default. [Y/y/1]. To disable use any of [N/n/0]. |
