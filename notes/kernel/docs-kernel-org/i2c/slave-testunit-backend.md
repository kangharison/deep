...
> 출처(원문): https://docs.kernel.org/i2c/slave-testunit-backend.html
...
### 0x02 SMBUS\_HOST\_NOTIFY

| CMD | DATAL | DATAH | DELAY |
| --- | --- | --- | --- |
| 0x02 | low byte of the status word to send | high byte of the status word to send | n \* 10ms |

Also needs master mode. This test will send an SMBUS\_HOST\_NOTIFY message to the
...
support the I2C\_M\_RECV\_LEN flag of an i2c\_msg. This is a good testcase for it.
...
### 0x04 GET\_VERSION\_WITH\_REP\_START
...
message with a NUL terminated version string based on UTS\_RELEASE. The first
