...
> 출처(원문): https://docs.kernel.org/misc-devices/uacce.html
...
According to the IOMMU capability, uacce\_interface flags can be:
...
uacce\_register results can be:

1. If uacce module is not compiled, ERR\_PTR(-ENODEV)
2. Succeed with the desired flags
3. Succeed with the negotiated flags, for example

> uacce\_interface.flags = UACCE\_DEV\_SVA but uacce->flags = ~UACCE\_DEV\_SVA
>
...
More details in [ABI file testing/sysfs-driver-uacce](../admin-guide/abi-testing-files.html#abi-file-testing-sysfs-driver-uacce).
