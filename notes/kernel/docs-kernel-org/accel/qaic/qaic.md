...
> 출처(원문): https://docs.kernel.org/accel/qaic/qaic.html
...
all times. QAIC will trigger KOBJ\_ONLINE/OFFLINE uevents to advertise when the
...
:   This IOCTL allows userspace to allocate a buffer object (BO) which can send
    ...
    sliced (see DRM\_IOCTL\_QAIC\_ATTACH\_SLICE\_BO).
...
:   This IOCTL operates like DRM\_IOCTL\_QAIC\_EXECUTE\_BO, but it allows userspace
...
DRM\_IOCTL\_QAIC\_DETACH\_SLICE\_BO
:   This IOCTL allows userspace to remove the slicing information from a BO that
    was originally provided by a call to DRM\_IOCTL\_QAIC\_ATTACH\_SLICE\_BO. This
    is the inverse of DRM\_IOCTL\_QAIC\_ATTACH\_SLICE\_BO. The BO must be idle for
    DRM\_IOCTL\_QAIC\_DETACH\_SLICE\_BO to be called. After a successful detach slice
    operation the BO may have new slicing information attached with a new call
    to DRM\_IOCTL\_QAIC\_ATTACH\_SLICE\_BO. After detach slice, the BO cannot be
...
**control\_resp\_timeout\_s (unsigned int)**
...
**wait\_exec\_default\_timeout\_ms (unsigned int)**

Sets the default timeout for the wait\_exec ioctl in milliseconds (ms). Must be
set prior to the waic\_exec ioctl call. A value specified in the ioctl call
overrides this for that call. Default is 5000 (5 seconds).

**datapath\_poll\_interval\_us (unsigned int)**

Sets the polling interval in microseconds (us) when datapath polling is active.
Takes effect at the next polling interval. Default is 100 (100 us).

**timesync\_delay\_ms (unsigned int)**
