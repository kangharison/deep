...
> 출처(원문): https://docs.kernel.org/accel/qaic/aic100.html
...
[MHI](../../mhi/index.html) MHI is the mechanism the host uses to communicate
...
| --- | --- | --- | --- |
| QAIC\_LOOPBACK | 0 & 1 | AMSS | Any data sent to the device on this channel is sent back to the host. |
| QAIC\_SAHARA | 2 & 3 | SBL | Used by SBL to obtain the runtime firmware from the host. |
| QAIC\_DIAG | 4 & 5 | AMSS | Used to communicate with QSM via the DIAG protocol. |
| QAIC\_SSR | 6 & 7 | AMSS | Used to notify the host of subsystem restart events, and to offload SSR crashdumps. |
| QAIC\_QDSS | 8 & 9 | AMSS | Used for the Qualcomm Debug Subsystem. |
| QAIC\_CONTROL | 10 & 11 | AMSS | Used for the Neural Network Control (NNC) protocol. This is the primary channel between host and QSM for managing workloads. |
| QAIC\_LOGGING | 12 & 13 | SBL | Used by the SBL to send the bootlog to the host. |
| QAIC\_STATUS | 14 & 15 | AMSS | Used to notify the host of Reliability, Accessibility, Serviceability (RAS) events. |
| QAIC\_TELEMETRY | 16 & 17 | AMSS | Used to get/set power/thermal/etc attributes. |
| QAIC\_DEBUG | 18 & 19 | AMSS | Not used. |
| QAIC\_TIMESYNC | 20 & 21 | SBL | Used to synchronize timestamps in the device side logs with the host time source. |
| QAIC\_TIMESYNC \_PERIODIC | 22 & 23 | AMSS | Used to periodically synchronize timestamps in the device side logs with the host time source. |
| IPCR | 24 & 25 | AMSS | AF\_QIPCRTR clients and servers. |
...
specified in pcie\_dma\_cmd. The structure of a response FIFO element:
...
req\_id
:   matches the req\_id of the request that generated this element.
...
It uses the QAIC\_CONTROL MHI channel.
...
If a particular workload crashes, QSM notifies the host via the QAIC\_SSR MHI
...
the dev\_coredump subsystem. The host can also decline the crashdump collection
...
occurs, QSM will report the event with appropriate details via the QAIC\_STATUS
