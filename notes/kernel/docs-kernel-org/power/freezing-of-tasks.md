...
> 출처(원문): https://docs.kernel.org/power/freezing-of-tasks.html
...
There is one per-task flag (PF\_NOFREEZE) and three per-task states
(TASK\_FROZEN, TASK\_FREEZABLE and \_\_TASK\_FREEZABLE\_UNSAFE) used for that.
The tasks that have PF\_NOFREEZE unset (all user space tasks and some kernel
...
static key freezer\_active (as opposed to a per-task flag or state) is used to
...
in kernel/freezer.c), which changes the task’s state to TASK\_FROZEN, and makes
it loop until it is woken by an explicit TASK\_FROZEN wakeup. Then, that task
...
explicitly in suitable places or use the [`wait_event_freezable()`](../driver-api/basics.html#c.wait_event_freezable "wait_event_freezable") or
`wait_event_freezable_timeout()` macros (defined in include/linux/wait.h)
that put the task to sleep (TASK\_INTERRUPTIBLE) or freeze it (TASK\_FROZEN) if
freezer\_active is set. The main loop of a freezable kernel thread may look
...
(from mm/oom\_kill.c::`oom_reaper()`).
...
[`wait_event_freezable()`](../driver-api/basics.html#c.wait_event_freezable "wait_event_freezable") and `wait_event_freezable_timeout()` macros.
...
freeze\_kernel\_threads():
:   * freezes all tasks (including kernel threads) because we can’t freeze
      kernel threads without freezing userspace tasks

thaw\_kernel\_threads():
...
PF\_NOFREEZE for itself by calling `set_freezable()` (the resetting of PF\_NOFREEZE
directly is not allowed). From this point it is regarded as freezable
and must call `try_to_freeze()` or variants of [`wait_event_freezable()`](../driver-api/basics.html#c.wait_event_freezable "wait_event_freezable") in a
suitable place.
...
TASK\_UNINTERRUPTIBLE state) that needs to be done by freezable kernel thread B
...
they are not directly related to it. For example, if [`request_firmware()`](../driver-api/firmware/request_firmware.html#c.request_firmware "request_firmware") is
called from a device driver’s .`resume()` routine, it will timeout and eventually
fail, because the user land process that should respond to the request is frozen
at this point. So, seemingly, the failure is due to the freezing of tasks.
Suppose, however, that the firmware file is located on a filesystem accessible
only through another device that hasn’t been resumed yet. In that case,
[`request_firmware()`](../driver-api/firmware/request_firmware.html#c.request_firmware "request_firmware") will fail regardless of whether or not the freezing of tasks
...
[Suspend/Hibernation Notifiers](../driver-api/pm/notifiers.html).

## VI. Are there any precautions to be taken to prevent freezing failures?

Yes, there are.

First of all, grabbing the ‘system\_transition\_mutex’ lock to mutually exclude a
...
However, if that is not feasible, and grabbing ‘system\_transition\_mutex’ is
deemed necessary, it is strongly discouraged to directly call
mutex\_[un]lock(&system\_transition\_mutex) since that could lead to freezing
failures, because if the suspend/hibernate code successfully acquired the
‘system\_transition\_mutex’ lock, and hence that other entity failed to acquire
the lock, then that task would get blocked in TASK\_UNINTERRUPTIBLE state. As a
consequence, the freezer would not be able to freeze that task, leading to
freezing failure.

However, the [un]`lock_system_sleep()` APIs are safe to use in this scenario,
since they ask the freezer to skip freezing this task, since it is anyway
“frozen enough” as it is blocked on ‘system\_transition\_mutex’, which will be
...
mutex\_[un]lock(&system\_transition\_mutex). That would prevent freezing failures.

## V. Miscellaneous

/sys/power/pm\_freeze\_timeout controls how long it will cost at most to freeze
