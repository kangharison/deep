...
> 출처(원문): https://docs.kernel.org/power/suspend-and-cpuhotplug.html
...
* `thaw_secondary_cpus()` which involves:
  ...
  ```
* thaw tasks
* send PM\_POST\_SUSPEND notifications
* Release system\_transition\_mutex lock.

It is to be noted here that the system\_transition\_mutex lock is acquired at the
...
in that during regular CPU hotplug, 0 is passed for the ‘tasks\_frozen’
argument. But during suspend, since the tasks are already frozen by the time
the non-boot CPUs are offlined or onlined, the \_cpu\_\*() functions are called
with the ‘tasks\_frozen’ argument set to 1.
...
* kernel/cpu.c: cpu\_[up|down](), \_cpu\_[up|down](),
...
userspace, using the [`request_firmware()`](../driver-api/firmware/request_firmware.html#c.request_firmware "request_firmware") function defined in
drivers/base/firmware\_loader/main.c]
...
1. When all the CPUs are identical:
   ...
   arch/x86/kernel/microcode\_core.c helps in discovering the type of the CPU
...
3. When a CPU is physically hot-unplugged and a new (and possibly different
   ...
   a regular CPU hotplug operation, upon receiving the CPU\_DEAD notification
...
4. Handling microcode update during suspend/hibernate:
   ...
   as part of the suspend/hibernate cycle (cpuhp\_tasks\_frozen is set),
...
1. When invoking regular CPU hotplug, the ‘tasks\_frozen’ argument passed to
   the `_cpu_down()` and `_cpu_up()` functions is *always* 0.
   This might not reflect the true current state of the system, since the
   tasks could have been frozen by an out-of-band event such as a suspend
   operation in progress. Hence, the cpuhp\_tasks\_frozen variable will not
...
2. If a regular CPU hotplug stress test happens to race with the freezer due
   ...
   >   TASK\_UNINTERRUPTIBLE state, in order to get the microcode image.
