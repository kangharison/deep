...
> 출처(원문): https://docs.kernel.org/power/pci.html
...
power management refer to [Device Power Management Basics](../driver-api/pm/devices.html) and
[Runtime Power Management Framework for I/O Devices](runtime_pm.html).
...
Specifically, the pm field of the PCI subsystem’s [`struct bus_type`](../driver-api/infrastructure.html#c.bus_type "bus_type") object,
pci\_bus\_type, points to a [`struct dev_pm_ops`](../driver-api/pm/types.html#c.dev_pm_ops "dev_pm_ops") object, pci\_dev\_pm\_ops, containing
...
They also indirectly use some fields of the [`struct device`](../driver-api/infrastructure.html#c.device "device") that is embedded in
`struct pci_dev`.
...
in the configuration space is stored in the pm\_cap field of the device’s `struct
...
the [`struct device`](../driver-api/infrastructure.html#c.device "device") embedded in it are updated accordingly and the generation of
...
the function updates the wakeup fields in [`struct device`](../driver-api/infrastructure.html#c.device "device") embedded in the
...
(runtime PM) framework described in [Runtime Power Management Framework for I/O Devices](runtime_pm.html).
...
[Device Power Management Basics](../driver-api/pm/devices.html). Each of them requires devices to be
...
> prepare, suspend, suspend\_noirq.
...
dev_pm_ops`](../driver-api/pm/types.html#c.dev_pm_ops "dev_pm_ops") object is present and the prepare pointer in that object is valid).
...
the device’s driver doesn’t provide a [`struct dev_pm_ops`](../driver-api/pm/types.html#c.dev_pm_ops "dev_pm_ops") object (containing
...
done that). Second, if the device driver’s [`struct dev_pm_ops`](../driver-api/pm/types.html#c.dev_pm_ops "dev_pm_ops") object is not
...
> prepare, poweroff, poweroff\_noirq

where the prepare phase is exactly the same as for system suspend. The other
two phases are analogous to the suspend and suspend\_noirq phases, respectively.
...
As described in [Device Power Management Basics](../driver-api/pm/devices.html), the hibernation image
...
thaw\_noirq, thaw, and complete phases (that will only affect the devices having
...
> restore\_noirq, restore, complete

The first two of these are analogous to the resume\_noirq and resume phases
...
dev\_pm\_ops structure described in [Device Power Management Basics](../driver-api/pm/devices.html), and
the “legacy” one, in which the .`suspend()` and .`resume()` callbacks from [`struct
pci_driver`](../PCI/pci.html#c.pci_driver "pci_driver") are used. The legacy approach, however, doesn’t allow one to define
...
It is recommended that all PCI device drivers define a [`struct dev_pm_ops`](../driver-api/pm/types.html#c.dev_pm_ops "dev_pm_ops") object
...
driver’s [`struct dev_pm_ops`](../driver-api/pm/types.html#c.dev_pm_ops "dev_pm_ops") object has to be assigned to the driver.pm field in
its [`struct pci_driver`](../PCI/pci.html#c.pci_driver "pci_driver") object. Once that has happened, the “legacy” PM callbacks
in [`struct pci_driver`](../PCI/pci.html#c.pci_driver "pci_driver") are ignored (even if they are not NULL).

The PM callbacks in [`struct dev_pm_ops`](../driver-api/pm/types.html#c.dev_pm_ops "dev_pm_ops") are not mandatory and if they are not
defined (i.e. the respective fields of [`struct dev_pm_ops`](../driver-api/pm/types.html#c.dev_pm_ops "dev_pm_ops") are unset) the PCI
...
in [Suspend/Hibernation Notifiers](../driver-api/pm/notifiers.html)).
...
a PCI driver. Then, [`pci_save_state()`](../driver-api/pci/pci.html#c.pci_save_state "pci_save_state"), [`pci_prepare_to_sleep()`](../driver-api/pci/pci.html#c.pci_prepare_to_sleep "pci_prepare_to_sleep"), and
[`pci_set_power_state()`](../driver-api/pci/pci.html#c.pci_set_power_state "pci_set_power_state") should be used to save the device’s standard configuration
registers, to prepare it for system wakeup (if necessary), and to put it into a
low-power state, respectively. Moreover, if the driver calls [`pci_save_state()`](../driver-api/pci/pci.html#c.pci_save_state "pci_save_state"),
the PCI subsystem will not execute either [`pci_prepare_to_sleep()`](../driver-api/pci/pci.html#c.pci_prepare_to_sleep "pci_prepare_to_sleep"), or
[`pci_set_power_state()`](../driver-api/pci/pci.html#c.pci_set_power_state "pci_set_power_state") for its device, so the driver is then responsible for
...
save the device’s standard configuration registers using [`pci_save_state()`](../driver-api/pci/pci.html#c.pci_save_state "pci_save_state").
...
the `poweroff()` callback should use [`pci_prepare_to_sleep()`](../driver-api/pci/pci.html#c.pci_prepare_to_sleep "pci_prepare_to_sleep") and
[`pci_set_power_state()`](../driver-api/pci/pci.html#c.pci_set_power_state "pci_set_power_state") to prepare the device for system wakeup and to put it
into a low-power state, respectively, but it need not save the device’s standard
configuration registers.

#### 3.1.7. poweroff\_noirq()
...
state in the resume\_noirq phase of system resume and restores their standard
...
core, in the thaw\_noirq phase of hibernation. It also may be executed if the
...
#### 3.1.12. restore\_noirq()

The `restore_noirq()` callback is hibernation-specific. It is executed in the
restore\_noirq phase of hibernation, when the boot kernel has passed control to
...
point two or more members of [`struct dev_pm_ops`](../driver-api/pm/types.html#c.dev_pm_ops "dev_pm_ops") to the same routine. There are
a few convenience macros that can be used for this purpose.

The `DEFINE_SIMPLE_DEV_PM_OPS()` declares a [`struct dev_pm_ops`](../driver-api/pm/types.html#c.dev_pm_ops "dev_pm_ops") object with one
...
.`restore()` members. The other function pointers in this [`struct dev_pm_ops`](../driver-api/pm/types.html#c.dev_pm_ops "dev_pm_ops") are
unset.
...
dev_pm_ops`](../driver-api/pm/types.html#c.dev_pm_ops "dev_pm_ops") to indicate that one suspend routine is to be pointed to by the
...
The DPM\_FLAG\_NO\_DIRECT\_COMPLETE flag prevents the PM core from using the
...
The DPM\_FLAG\_SMART\_PREPARE flag causes the PCI bus type to return a positive
...
DPM\_FLAG\_NO\_DIRECT\_COMPLETE means permanent opt-out).

The DPM\_FLAG\_SMART\_SUSPEND flag tells the PCI bus type that from the driver’s
perspective the device can be safely left in runtime suspend during system
suspend. That causes `pci_pm_suspend()`, `pci_pm_freeze()` and `pci_pm_poweroff()`
to avoid resuming the device from runtime suspend unless there are PCI-specific
reasons for doing that. Also, it causes pci\_pm\_suspend\_late/`noirq()` and
pci\_pm\_poweroff\_late/`noirq()` to return early if the device remains in runtime
...
Setting the DPM\_FLAG\_MAY\_SKIP\_RESUME flag means that the driver allows its
...
taken into consideration by the PM core along with the power.may\_skip\_resume
...
by work items put into the power management workqueue, pm\_wq. Although there
...
request into pm\_wq. In the majority of cases this also is done by their
...
[Device Power Management Basics](../driver-api/pm/devices.html)
