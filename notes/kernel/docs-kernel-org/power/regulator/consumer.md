...
> 출처(원문): https://docs.kernel.org/power/regulator/consumer.html
...
Please see [Linux voltage and current regulator framework](overview.html) for a description of the terms used in this text.
...
The consumer passes in its [`struct device`](../../driver-api/infrastructure.html#c.device "device") pointer and power supply ID. The core
...
The regulator access functions [`regulator_get()`](../../driver-api/regulator.html#c.regulator_get "regulator_get") and [`regulator_put()`](../../driver-api/regulator.html#c.regulator_put "regulator_put") will
...
NOTE:
:   The supply may already be enabled before [`regulator_enable()`](../../driver-api/regulator.html#c.regulator_enable "regulator_enable") is called.
...
Where min\_uV and max\_uV are the minimum and maximum acceptable voltages in
microvolts.
...
Where min\_uA and max\_uA are the minimum and maximum acceptable current limit in
microamps.
...
The load\_uA value can be determined from the consumer’s datasheet. e.g. most
...
regulator\_list\_voltage) to a hardware-specific voltage selector that can be
...
use [`regulator_get_exclusive()`](../../driver-api/regulator.html#c.regulator_get_exclusive "regulator_get_exclusive"), as it can’t work if there’s more than one
