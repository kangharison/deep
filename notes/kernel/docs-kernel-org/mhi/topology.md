...
> 출처(원문): https://docs.kernel.org/mhi/topology.html
...
* Allocates `struct mhi_controller` and registers with the MHI bus framework
  with channel and event configurations using mhi\_register\_controller.
...
* Registers the driver with the MHI bus framework using mhi\_driver\_register.
* Prepares the device for transfer by calling mhi\_prepare\_for\_transfer.
* Initiates data transfer by calling mhi\_queue\_transfer.
* Once the data transfer is finished, calls mhi\_unprepare\_from\_transfer to
  end data transfer.
