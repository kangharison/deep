...
> 출처(원문): https://docs.kernel.org/scheduler/sched-ext.html
...
sched\_ext is a scheduler class whose behavior can be defined by a set of BPF
programs - the BPF scheduler.

* sched\_ext exports a full scheduling interface so that any scheduling
...
* When the BPF scheduler triggers an error, debug information is dumped to
  ...
  sched\_ext\_dump tracepoint. The SysRq key sequence SysRq-D
  triggers a debug dump. This doesn’t terminate the BPF scheduler and can
  only be read through the tracepoint.

## Switching to and from sched\_ext

`CONFIG_SCHED_CLASS_EXT` is the config option to enable sched\_ext and
`tools/sched_ext` contains the example schedulers. The following config
options should be enabled to use sched\_ext:
...
sched\_ext is used only when the BPF scheduler is loaded and running.
...
`SCHED_EXT` tasks are scheduled by sched\_ext.

However, when the BPF scheduler is loaded and `SCX_OPS_SWITCH_PARTIAL` is
set in `ops->flags`, only tasks with the `SCHED_EXT` policy are scheduled
by sched\_ext, while tasks with `SCHED_NORMAL`, `SCHED_BATCH` and
`SCHED_IDLE` policies are scheduled by the fair-class scheduler which has
higher sched\_class precedence than `SCHED_EXT`.

Terminating the sched\_ext scheduler program, triggering SysRq-S, or
...
Whether a given task is on sched\_ext can be determined as follows:
...
sched\_ext uses DSQs (dispatch queues) which can operate as both a FIFO and a
...
of a task managed by a sched\_ext scheduler:
...
* A sched\_ext task can be preempted by a task from a higher-priority scheduling
...
* `include/linux/sched/ext.h` defines the core data structures, ops table
  and constants.
* `kernel/sched/ext.c` contains sched\_ext core implementation and helpers.
...
sched\_ext exposes two module parameters under the `sched_ext.` prefix that
...
The APIs provided by sched\_ext to BPF schedulers programs have no stability
