...
> 출처(원문): https://docs.kernel.org/scheduler/schedutil.html
...
> ewma\_sum(u) := u\_0 + u\_1\*y + u\_2\*y^2 + ...
>
...
r\_cpu is determined as the ratio of highest performance level of the current
...
> * [Capacity Aware Scheduling](sched-capacity.html):”1. CPU Capacity + 2. Task utilization”
...
To alleviate this (a default enabled option) UTIL\_EST drives an Infinite
Impulse Response (IIR) EWMA with the ‘running’ value on dequeue -- when it is
highest. UTIL\_EST filters to instantly increase and only decay on decrease.

A further runqueue wide sum (of runnable tasks) is maintained of:

> util\_est := Sum\_t max( t\_running, t\_util\_est\_ewma )

For more detail see: kernel/sched/fair.c:`util_est_dequeue()`

## UCLAMP

It is possible to set effective u\_min and u\_max clamps on each CFS or RT task;
...
XXX: deadline tasks (Sporadic Task Model) allows us to calculate a hard f\_min
...
>   correct this. XXX do we still guarantee f\_max due to no idle-time?
