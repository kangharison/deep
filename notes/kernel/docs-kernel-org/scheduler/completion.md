...
> 출처(원문): https://docs.kernel.org/scheduler/completion.html
...
you probably want to look into using one of the wait\_for\_completion\*()
...
does not happen until all related activities (`complete()` or [`reinit_completion()`](../driver-api/basics.html#c.reinit_completion "reinit_completion"))
...
[`init_completion()`](../driver-api/basics.html#c.init_completion "init_completion"):
...
The re-initialization function, [`reinit_completion()`](../driver-api/basics.html#c.reinit_completion "reinit_completion"), simply resets the
...
Calling [`init_completion()`](../driver-api/basics.html#c.init_completion "init_completion") on the same completion object twice is
most likely a bug as it re-initializes the queue to an empty queue and
enqueued tasks could get “lost” - use [`reinit_completion()`](../driver-api/basics.html#c.reinit_completion "reinit_completion") in that case,
...
[`DECLARE_COMPLETION()`](../driver-api/basics.html#c.DECLARE_COMPLETION "DECLARE_COMPLETION"):
...
initialized to ‘not done’ and doesn’t require an [`init_completion()`](../driver-api/basics.html#c.init_completion "init_completion") call.

When a completion is declared as a local variable within a function,
then the initialization should always use [`DECLARE_COMPLETION_ONSTACK()`](../driver-api/basics.html#c.DECLARE_COMPLETION_ONSTACK "DECLARE_COMPLETION_ONSTACK")
...
from the wait\_on\_completion\*() caller function will deallocate the function
...
A naive [`DECLARE_COMPLETION()`](../driver-api/basics.html#c.DECLARE_COMPLETION "DECLARE_COMPLETION") on the stack triggers a lockdep warning.
...
## wait\_for\_completion\*() variants available:
...
This function marks the task TASK\_INTERRUPTIBLE while it is waiting.
...
The task is marked as TASK\_UNINTERRUPTIBLE and will wait at most ‘timeout’
jiffies. If a timeout occurs it returns 0, else the remaining time in
jiffies (but at least 1).

Timeouts are preferably calculated with [`msecs_to_jiffies()`](../driver-api/basics.html#c.msecs_to_jiffies "msecs_to_jiffies") or [`usecs_to_jiffies()`](../driver-api/basics.html#c.usecs_to_jiffies "usecs_to_jiffies"),
...
TASK\_INTERRUPTIBLE. If a signal was received it will return -ERESTARTSYS;
otherwise it returns 0 if the completion timed out, or the remaining time in
jiffies if completion occurred.

Further variants include \_killable which uses TASK\_KILLABLE as the
