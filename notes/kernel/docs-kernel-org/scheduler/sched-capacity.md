...
> 출처(원문): https://docs.kernel.org/scheduler/sched-capacity.html
...
* work\_per\_hz(CPU0) = W
* work\_per\_hz(CPU1) = W/2
...
This yields duty\_cycle(p) == 50%, despite the task having the exact same
...
* duty\_cycle(p) == 25% if p runs on CPU0 at its maximum frequency
* duty\_cycle(p) == 75% if p runs on CPU1 at its maximum frequency
...
The arm, arm64, and RISC-V architectures directly map this to the arch\_topology driver
...
utilization. Architectures must define arch\_scale\_freq\_capacity(cpu) for that
purpose.
...
* The sched\_asym\_cpucapacity static key will be enabled.
* The SD\_ASYM\_CPUCAPACITY\_FULL flag will be set at the lowest sched\_domain
  level that spans all unique CPU capacity values.
* The SD\_ASYM\_CPUCAPACITY flag will be set for any sched\_domain that spans
  CPUs with any range of asymmetry.

The sched\_asym\_cpucapacity static key is intended to guard sections of code that
...
sched\_asym\_cpucapacity static key will be enabled. However, the sched\_domain
hierarchy of CPUs 0-1 spans a single capacity value: SD\_ASYM\_CPUCAPACITY isn’t
...
* Check the sched\_asym\_cpucapacity static key
* If it is enabled, then also check for the presence of SD\_ASYM\_CPUCAPACITY in
  the sched\_domain hierarchy (if relevant, i.e. the codepath targets a specific
...
[Control Group v2](../admin-guide/cgroup-v2.html)). As its name imply, this can be used to
...
(EAS), which is described in [Energy Aware Scheduling](sched-energy.html).
