...
> 출처(원문): https://docs.kernel.org/scheduler/sched-energy.html
...
The actual EM used by EAS is \_not\_ maintained by the scheduler, but by a
dedicated framework. For details about this framework and what it provides,
please refer to its documentation (see [Energy Model of devices](../power/energy-model.html)).
...
per ‘performance domain’ in the system (see [Energy Model of devices](../power/energy-model.html)
...
em_perf_domain`](../power/energy-model.html#c.em_perf_domain "em_perf_domain") as provided by the EM framework.
...
EAS also maintains a static key (sched\_energy\_present) which is enabled when at
...
save energy compared to leaving it on prev\_cpu, i.e. the CPU where the task ran
...
:   Let us consider a (fake) platform with 2 independent performance domains
    ...
    The scheduler must decide where to place a task P whose util\_avg = 200
    and prev\_cpu = 0.

    The current utilization landscape of the CPUs is depicted on the graph
    below. CPUs 0-3 have a util\_avg of 400, 100, 600 and 500 respectively
    ...
    the Energy Model table. The util\_avg of P is shown on the figures
    below as ‘PP’:
    ...
    **Case 3. P stays on prev\_cpu / CPU 0**:
...
looking for the presence of the SD\_ASYM\_CPUCAPACITY\_FULL flag when the scheduling
domains are built.

See [Capacity Aware Scheduling](sched-capacity.html) for requirements to be met for this
flag to be set in the sched\_domain hierarchy.
...
independent EM framework in [Energy Model of devices](../power/energy-model.html).
...
restricts the number of CPUs to EM\_MAX\_NUM\_CPUS to prevent overflows during
...
least \_requests\_ frequencies calculated using the utilization signals.
...
be obtained using the architecture-defined arch\_scale{cpu,freq}`_capacity()`
callbacks.
