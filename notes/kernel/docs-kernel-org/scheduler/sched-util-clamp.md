...
> 출처(원문): https://docs.kernel.org/scheduler/sched-util-clamp.html
...
Since the scheduler and schedutil are both driven by PELT (util\_avg) signals,
...
> * UCLAMP\_MIN, which sets the lower bound.
> * UCLAMP\_MAX, which sets the upper bound.

These two bounds will ensure a task will operate within this performance range
of the system. UCLAMP\_MIN implies boosting a task, while UCLAMP\_MAX implies
capping a task.
...
UCLAMP\_MIN=1024 will ensure such tasks will always see the highest performance
...
Both SCHED\_NORMAL/OTHER and SCHED\_FIFO/RR honour uclamp requests/hints.

In the SCHED\_FIFO/RR case, uclamp gives the option to run RT tasks at any
...
See [section 3.4](#uclamp-default-values) for default values and
[3.4.1](#sched-util-clamp-min-rt-default) on how to change RT tasks
default value.
...
a task requests to run at UTIL\_MIN = 512, then the util signal of the rq needs
...
uclamp value of the rq. See [section 2.1](#uclamp-buckets) for details on
how this works.
...
CONFIG\_UCLAMP\_BUCKETS\_COUNT. By default it is set to 5.

The rq has a bucket for each uclamp\_id tunables: [UCLAMP\_MIN, UCLAMP\_MAX].
...
is enqueued into the rq, bucket 1 will be incremented for UCLAMP\_MIN and bucket
4 will be incremented for UCLAMP\_MAX to reflect the fact the rq has a task in
this range.
...
See [section 3.4](#uclamp-default-values) for details on default values.
...
then assuming both p0 and p1 are enqueued to the same rq, both UCLAMP\_MIN
and UCLAMP\_MAX become:
...
As we shall see in [section 5.1](#uclamp-capping-fail), this max
aggregation is the cause of one of limitations when using util clamp, in
particular for UCLAMP\_MAX hint when user space would like to save power.
...
[Section 3](#uclamp-interfaces) discusses the interfaces and will expand
further on that.
...
* sched\_util\_min: requests the minimum performance point the system should run
  at when this task is running. Or lower performance bound.
* sched\_util\_max: requests the maximum performance point the system should run
...
RT could be changed. SCHED\_NORMAL/OTHER might gain similar knobs too in the
future.
...
* cpu.uclamp.min is a protection as described in [section 3-3 of cgroup
  v2 documentation](../admin-guide/cgroup-v2.html#cgroupv2-protections-distributor).

  If a task uclamp\_min value is lower than cpu.uclamp.min, then the task will
...
* cpu.uclamp.max is a limit as described in [section 3-2 of cgroup v2
  documentation](../admin-guide/cgroup-v2.html#cgroupv2-limits-distributor).

  If a task uclamp\_max value is higher than cpu.uclamp.max, then the task will
...
### 3.3.1 sched\_util\_clamp\_min

System wide limit of allowed UCLAMP\_MIN range. By default it is set to 1024,
which means that permitted effective UCLAMP\_MIN range for tasks is [0:1024].
...
they won’t be satisfied until it is more than p->uclamp[UCLAMP\_MIN].

The value must be smaller than or equal to sched\_util\_clamp\_max.

### 3.3.2 sched\_util\_clamp\_max

System wide limit of allowed UCLAMP\_MAX range. By default it is set to 1024,
which means that permitted effective UCLAMP\_MAX range for tasks is [0:1024].
...
won’t be satisfied until it is more than p->uclamp[UCLAMP\_MAX].

The value must be greater than or equal to sched\_util\_clamp\_min.

### 3.4. Default values

By default all SCHED\_NORMAL/SCHED\_OTHER tasks are initialized to:
...
For SCHED\_FIFO/SCHED\_RR tasks:
...
RT tasks default uclamp\_min value can be modified at boot or runtime via
sysctl. See below section.

### 3.4.1 sched\_util\_clamp\_min\_rt\_default
...
This task can increase its UCLAMP\_MIN value every time it misses the deadline
to ensure on next wake up it runs at a higher performance point. It should try
to approach the lowest UCLAMP\_MIN value that allows to meet its deadline on any
...
UCLAMP\_MAX for some background tasks that don’t care about performance but
could end up being busy and consume unnecessary system resources on the system.

### 4.3. Powersave mode

sched\_util\_clamp\_max system wide interface can be used to limit all tasks from
...
Middleware/Utility can provide the user an option to set UCLAMP\_MIN/MAX for an
...
### 5.1. Capping frequency with uclamp\_max fails under certain conditions
...
Assuming both p0 and p1 have UCLAMP\_MIN = 0, then the frequency selection for
...
### 5.2. UCLAMP\_MAX can break PELT (util\_avg) signal

PELT assumes that frequency will always increase as the signals grow to ensure
there’s always some idle time on the CPU. But with UCLAMP\_MAX, this frequency
...
which would result in util\_avg being 1024.
...
\_actual\_ util\_avg will not be 900 though, but somewhere between 300 and 900. As
long as there’s idle time, p->util\_avg updates will be off by a some margin,
...
this happens, then the \_actual\_ util\_avg will become:
...
then the effective UCLAMP\_MAX for the CPU will be 1024 according to max
aggregation rule. But since the capped p0 task was running and throttled
severely, then the rq->util\_avg will be:
...
> 3. schedutil rate\_limit\_us drops any requests during this rate\_limit\_us
>    window.
...
There is room to improve the behavior of schedutil rate\_limit\_us, but not much
