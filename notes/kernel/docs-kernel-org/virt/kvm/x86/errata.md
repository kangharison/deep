...
> 출처(원문): https://docs.kernel.org/virt/kvm/x86/errata.html
...
(FDP\_EXCPTN\_ONLY) and CPUID[EAX=7,ECX=0]:EBX]13] (ZERO\_FCS\_FDS) are
...
Invalid KVM\_SET\_VCPU\_EVENTS input with respect to error codes *may* result in
...
IA32\_VMX\_BASIC[56] as ‘1’ relax VMX’s consistency checks, and AMD CPUs have no
restrictions whatsoever. KVM\_SET\_VCPU\_EVENTS doesn’t sanity check the vector
versus “has\_error\_code”, i.e. KVM’s ABI follows AMD behavior.
...
When KVM\_X2APIC\_API\_USE\_32BIT\_IDS is enabled, KVM activates a hack/quirk that
...
only to the vCPU with the lowest vCPU ID. If KVM\_X2APIC\_API\_USE\_32BIT\_IDS is
