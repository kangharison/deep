...
> 출처(원문): https://docs.kernel.org/virt/kvm/halt-polling.html
...
> kvm\_vcpu->halt\_poll\_ns

or in the case of powerpc kvm-hv, in the vcore struct:

> kvmppc\_vcore->halt\_poll\_ns
...
is multiplied by the module parameters halt\_poll\_ns\_grow and
halt\_poll\_ns\_grow\_start.
...
halt\_poll\_ns\_shrink, or set to 0 iff halt\_poll\_ns\_shrink == 0.
...
include/linux/kvm\_host.h and as module parameters in virt/kvm/kvm\_main.c, or
arch/powerpc/kvm/book3s\_hv.c in the powerpc kvm-hv case.

|  |  |  |
| --- | --- | --- |
| Module Parameter | Description | Default Value |
| halt\_poll\_ns | The global max polling interval which defines the ceiling value of the polling interval for each vcpu. | KVM\_HALT\_POLL\_NS\_DEFAULT  (per arch value) |
| halt\_poll\_ns\_grow | The value by which the halt polling interval is multiplied in the `grow_halt_poll_ns()` function. | 2 |
| halt\_poll\_ns\_grow\_start | The initial value to grow to from zero in the `grow_halt_poll_ns()` function. | 10000 |
| halt\_poll\_ns\_shrink | The value by which the halt polling interval is divided in the `shrink_halt_poll_ns()` function. | 2 |
...
next time they halt, with the notable exception of VMs using KVM\_CAP\_HALT\_POLL
(see next section).

## KVM\_CAP\_HALT\_POLL

KVM\_CAP\_HALT\_POLL is a VM capability that allows userspace to override halt\_poll\_ns
on a per-VM basis. VMs using KVM\_CAP\_HALT\_POLL ignore halt\_poll\_ns completely (but
still obey halt\_poll\_ns\_grow, halt\_poll\_ns\_grow\_start, and halt\_poll\_ns\_shrink).

See [The Definitive KVM (Kernel-based Virtual Machine) API Documentation](api.html) for more information on this capability.

## Further Notes

* Care should be taken when setting the halt\_poll\_ns module parameter as a large value
  ...
  global max polling interval (halt\_poll\_ns) then the host will always poll for the
