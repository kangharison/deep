...
> 출처(원문): https://docs.kernel.org/virt/kvm/review-checklist.html
...
1. The patch must follow [Linux kernel coding style](../../process/coding-style.html) and
...
3. If the patch introduces or modifies a new userspace API:
   - the API must be documented in [The Definitive KVM (Kernel-based Virtual Machine) API Documentation](api.html)
   - the API must be discoverable using KVM\_CHECK\_EXTENSION
4. New state must include support for save/restore.
5. New features must default to off (userspace should explicitly request them).
   Performance improvements can and should default to on.
6. New cpu features should be exposed via KVM\_GET\_SUPPORTED\_CPUID2,
