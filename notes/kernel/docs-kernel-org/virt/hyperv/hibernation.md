...
> 출처(원문): https://docs.kernel.org/virt/hyperv/hibernation.html
...
[System Sleep States](../../admin-guide/pm/sleep-states.html).
...
[VMBus](vmbus.html).) Upon resume from hibernation,
...
RESCIND\_CHANNELOFFER message, which Linux processes by freeing the
...
[Device Power Management Basics](../../driver-api/pm/devices.html) for the sequencing of the
phases.
...
Virtual Functions (VF) devices. See [PCI pass-thru devices](vpci.html).
...
driver (uio\_hv\_generic.c) so that a user space driver can control and
