...
> 출처(원문): https://docs.kernel.org/virt/hyperv/vpci.html
...
irq\_chip.irq\_compose\_msi\_msg function as `hv_compose_msi_msg()`.
...
irq\_chip.irq\_compose\_msi\_msg can be called with IRQ locks
...
the x86\_vector IRQ domain has picked for the interrupt. This
...
smp\_affinity for an unmanaged IRQ is updated via the /proc/irq
interface, the Hyper-V virtual PCI driver is called to tell
the Hyper-V host to change the interrupt targeting and
everything works properly. However, on x86 if the x86\_vector
...
operations as part of dma\_map/unmap\_\*() calls.
...
these hv\_pcifront\_\*() functions directly access the PCI config
...
access, so the hv\_pcifront\_\*() functions must invoke
...
(pci-hyperv-intf.c, under CONFIG\_PCI\_HYPERV\_INTERFACE) that
