...
> 출처(원문): https://docs.kernel.org/virt/hyperv/coco.html
...
objectives described in [Confidential Computing in Linux for x86 virtualization](../../security/snp-tdx-threat-model.html). Note
...
* Device emulation. In paravisor mode, the Hyper-V paravisor provides
  ...
  (private). See the “is\_private\_mmio” callback.
...
acting cooperatively. Such drivers are marked as “allowed\_in\_isolated” in the
vmbus\_devs[] table. Other drivers for VMBus devices that are not needed in a
...
“use\_calls” flag indicating to use hypercalls.
...
a PCI pass-thru device, see [PCI pass-thru devices](vpci.html)) that is directly assigned to VTL2
...
Confidential VMBus is supported by the current paravisor. The x86\_64-specific
