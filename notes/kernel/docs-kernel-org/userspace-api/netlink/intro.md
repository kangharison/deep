...
> 출처(원문): https://docs.kernel.org/userspace-api/netlink/intro.html
...
file, they include among others - general networking (NETLINK\_ROUTE),
iSCSI (NETLINK\_ISCSI), and audit (NETLINK\_AUDIT).
...
A Netlink message always starts with [`struct nlmsghdr`](#c.nlmsghdr "nlmsghdr"), which is followed
...
the operation instead. (See [Resolving the Family ID](#res-fam) for
...
See [Netlink message types](#nl-msg-type) for more details.
...
See [nlmsg\_pid](#nlmsg-pid) for the (uncommon) uses of the field.
...
There are two instances of [`struct nlmsghdr`](#c.nlmsghdr "nlmsghdr") here, first of the response
...
attributes. See [Extended ACK](#ext-ack) for more details. The presence
...
Types of extended ack attributes are defined in [`enum nlmsgerr_attrs`](#c.nlmsgerr_attrs "nlmsgerr_attrs").
...
object as reported in [Resolving the Family ID](#res-fam). User can query information about
...
> 1. AF\_NETLINK is a bona fide socket protocol family
> 2. AF\_NETLINK’s documentation refers to what comes after its own
>    header ([`struct nlmsghdr`](#c.nlmsghdr "nlmsghdr")) in a message as a “Family Header”
> 3. Generic Netlink is a family for AF\_NETLINK (`struct genlmsghdr` follows
>    [`struct nlmsghdr`](#c.nlmsghdr "nlmsghdr")), yet it also calls its users “Families”.
...
a considerable number of fields after [`struct nlmsghdr`](#c.nlmsghdr "nlmsghdr"). It was also
...
and `DEL` requests in the upper byte of nlmsg\_flags in [`struct nlmsghdr`](#c.nlmsghdr "nlmsghdr").
...
enum netlink\_policy\_type\_attr
...
:   type of the attribute,
    [`enum netlink_attribute_type`](#c.netlink_attribute_type "netlink_attribute_type") (U32)
