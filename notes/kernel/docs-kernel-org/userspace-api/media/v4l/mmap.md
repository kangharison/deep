...
> 출처(원문): https://docs.kernel.org/userspace-api/media/v4l/mmap.html
...
[ioctl VIDIOC\_QUERYCAP](vidioc-querycap.html#vidioc-querycap) ioctl is set. There are two
streaming methods, to determine if the memory mapping flavor is
supported applications must call the [ioctl VIDIOC\_REQBUFS](vidioc-reqbufs.html#vidioc-reqbufs) ioctl
...
different file descriptors must be used. [[1]](#f1)

To allocate device buffers applications call the
[ioctl VIDIOC\_REQBUFS](vidioc-reqbufs.html#vidioc-reqbufs) ioctl with the desired number
...
address space with the [`mmap()`](func-mmap.html#c.V4L.mmap "mmap") function. The
location of the buffers in device memory can be determined with the
[ioctl VIDIOC\_QUERYBUF](vidioc-querybuf.html#vidioc-querybuf) ioctl. In the single-planar
API case, the `m.offset` and `length` returned in a struct
[`v4l2_buffer`](buffer.html#c.V4L.v4l2_buffer "v4l2_buffer") are passed as sixth and second
parameter to the [`mmap()`](func-mmap.html#c.V4L.mmap "mmap") function. When using the
multi-planar API, struct [`v4l2_buffer`](buffer.html#c.V4L.v4l2_buffer "v4l2_buffer") contains an
array of struct [`v4l2_plane`](buffer.html#c.V4L.v4l2_plane "v4l2_plane") structures, each
containing its own `m.offset` and `length`. When using the
multi-planar API, every plane of every buffer has to be mapped
separately, so the number of calls to [`mmap()`](func-mmap.html#c.V4L.mmap "mmap") should
...
possible with the [`munmap()`](func-munmap.html#c.V4L.munmap "munmap") function.
...
the driver can *fill* enqueued *empty* buffers in any order. [[2]](#f2) The
index number of a buffer (struct [`v4l2_buffer`](buffer.html#c.V4L.v4l2_buffer "v4l2_buffer")
...
up the output is started with [VIDIOC\_STREAMON](vidioc-streamon.html#vidioc-streamon).
...
[VIDIOC\_QBUF](vidioc-qbuf.html#vidioc-qbuf) and [VIDIOC\_DQBUF](vidioc-qbuf.html#vidioc-qbuf)
ioctl. The status of a buffer being mapped, enqueued, full or empty can
be determined at any time using the [ioctl VIDIOC\_QUERYBUF](vidioc-querybuf.html#vidioc-querybuf) ioctl. Two
methods exist to suspend execution of the application until one or more
buffers can be dequeued. By default [VIDIOC\_DQBUF](vidioc-qbuf.html#vidioc-qbuf)
blocks when no buffer is in the outgoing queue. When the `O_NONBLOCK`
flag was given to the [`open()`](func-open.html#c.V4L.open "open") function,
[VIDIOC\_DQBUF](vidioc-qbuf.html#vidioc-qbuf) returns immediately with an `EAGAIN`
error code when no buffer is available. The [`select()`](func-select.html#c.V4L.select "select")
or [`poll()`](func-poll.html#c.V4L.poll "poll") functions are always available.

To start and stop capturing or output applications call the
[VIDIOC\_STREAMON](vidioc-streamon.html#vidioc-streamon) and [VIDIOC\_STREAMOFF](vidioc-streamon.html#vidioc-streamon) ioctl.

Drivers implementing memory mapping I/O must support the
[VIDIOC\_REQBUFS](vidioc-reqbufs.html#vidioc-reqbufs), [VIDIOC\_QUERYBUF](vidioc-querybuf.html#vidioc-querybuf), [VIDIOC\_QBUF](vidioc-qbuf.html#vidioc-qbuf), [VIDIOC\_DQBUF](vidioc-qbuf.html#vidioc-qbuf), [VIDIOC\_STREAMON](vidioc-streamon.html#vidioc-streamon)
and [VIDIOC\_STREAMOFF](vidioc-streamon.html#vidioc-streamon) ioctls, the [mmap()](func-mmap.html#func-mmap), [`munmap()`](func-munmap.html#c.V4L.munmap "munmap"), [select()](func-select.html#func-select) and [`poll()`](func-poll.html#c.V4L.poll "poll") function. [[3]](#f3)
...
accordingly when calling [ioctl VIDIOC\_QBUF, VIDIOC\_DQBUF](vidioc-qbuf.html#vidioc-qbuf) etc.,
but it makes the [`select()`](func-select.html#c.V4L.select "select") function ambiguous. We also
