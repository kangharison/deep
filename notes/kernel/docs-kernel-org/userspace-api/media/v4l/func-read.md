...
> 출처(원문): https://docs.kernel.org/userspace-api/media/v4l/func-read.html
...
[`read()`](#c.V4L.read "read") attempts to read up to `count` bytes from file
...
section, see ##. If `count` is zero, [`read()`](#c.V4L.read "read") returns zero
and has no other results. If `count` is greater than `SSIZE_MAX`,
the result is unspecified. Regardless of the `count` value each
[`read()`](#c.V4L.read "read") call will provide at most one frame (two fields)
worth of data.

By default [`read()`](#c.V4L.read "read") blocks until data becomes available. When
the `O_NONBLOCK` flag was given to the [`open()`](func-open.html#c.V4L.open "open")
function it returns immediately with an `EAGAIN` error code when no data
is available. The [`select()`](func-select.html#c.V4L.select "select") or
[`poll()`](func-poll.html#c.V4L.poll "poll") functions can always be used to suspend
execution until data becomes available. All drivers supporting the
[`read()`](#c.V4L.read "read") function must also support [`select()`](func-select.html#c.V4L.select "select") and
[`poll()`](func-poll.html#c.V4L.poll "poll").
...
[`read()`](#c.V4L.read "read") never returns a “snapshot” of a buffer being filled.
...
The behavior of [`read()`](#c.V4L.read "read") when called during the active picture
...
time of the [`read()`](#c.V4L.read "read") call as soon as it is complete.

A driver discarding the newest frames stops capturing until the next
[`read()`](#c.V4L.read "read") call. The frame being received at [`read()`](#c.V4L.read "read")
...
the bttv driver, initiating a DMA to user memory when [`read()`](#c.V4L.read "read")
...
the driver with the [VIDIOC\_G\_PARM](vidioc-g-parm.html#vidioc-g-parm) and
[VIDIOC\_S\_PARM](vidioc-g-parm.html#vidioc-g-parm) ioctls. They are optional,
however. The discarding policy is not reported and cannot be changed.
For minimum requirements see [Interfaces](devices.html#devices).
...
[`read()`](#c.V4L.read "read") was interrupted by a signal. On error, -1 is
...
EAGAIN
:   Non-blocking I/O has been selected using O\_NONBLOCK and no data was
...
EINVAL
:   The [`read()`](#c.V4L.read "read") function is not supported by this driver, not
