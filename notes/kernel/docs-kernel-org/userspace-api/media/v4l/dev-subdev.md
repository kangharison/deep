...
> 출처(원문): https://docs.kernel.org/userspace-api/media/v4l/dev-subdev.html
...
node with the V4L2 control API described in [User Controls](control.html#control). The ioctls
...
[Event Interface](dev-event.html#event). The API behaves identically as when used on V4L2 device
...
[selection](vidioc-subdev-g-selection.html#vidioc-subdev-g-selection) ioctls. The driver is
...
[Image Format Negotiation on Pipelines](#pipeline-scaling), where image scaling can be performed on both
...
Drivers that implement the [media API](../mediactl/media-controller-intro.html#media-controller-intro)
can expose pad-level image format configuration to applications. When
they do, applications can use the
[VIDIOC\_SUBDEV\_G\_FMT](vidioc-subdev-g-fmt.html#vidioc-subdev-g-fmt) and
[VIDIOC\_SUBDEV\_S\_FMT](vidioc-subdev-g-fmt.html#vidioc-subdev-g-fmt) ioctls. to
...
[VIDIOC\_STREAMON](vidioc-streamon.html#vidioc-streamon) time, and an `EPIPE` error
...
the [ioctl VIDIOC\_SUBDEV\_G\_FMT, VIDIOC\_SUBDEV\_S\_FMT](vidioc-subdev-g-fmt.html#vidioc-subdev-g-fmt) ioctl on pad
...
[V4L2\_SUBDEV\_FORMAT\_TRY](vidioc-subdev-g-fmt.html#vidioc-subdev-g-fmt), the
[VIDIOC\_SUBDEV\_G\_FMT](vidioc-subdev-g-fmt.html#vidioc-subdev-g-fmt) and
[VIDIOC\_SUBDEV\_S\_FMT](vidioc-subdev-g-fmt.html#vidioc-subdev-g-fmt) ioctls operate on
...
[VIDIOC\_SUBDEV\_G\_FMT](vidioc-subdev-g-fmt.html#vidioc-subdev-g-fmt) call will return
...
[VIDIOC\_SUBDEV\_S\_FMT](vidioc-subdev-g-fmt.html#vidioc-subdev-g-fmt) ioctl. Drivers
...
to an [VIDIOC\_SUBDEV\_S\_FMT](vidioc-subdev-g-fmt.html#vidioc-subdev-g-fmt) call as-is
...
[Sample Pipeline Configuration](#sample-pipeline-config) shows a sample configuration sequence
for the pipeline described in [Image Format Negotiation on Pipelines](#pipeline-scaling) (table columns
...
| --- | --- | --- | --- | --- | --- | --- |
| Initial state | 2048x1536  SGRBG8\_1X8 | (default) | (default) | (default) | (default) | (default) |
| Configure frontend sink format | 2048x1536  SGRBG8\_1X8 | *2048x1536*  *SGRBG8\_1X8* | *2046x1534*  *SGRBG8\_1X8* | (default) | (default) | (default) |
| Configure scaler sink format | 2048x1536  SGRBG8\_1X8 | 2048x1536  SGRBG8\_1X8 | 2046x1534  SGRBG8\_1X8 | *2046x1534*  *SGRBG8\_1X8* | *0,0/2046x1534* | *2046x1534*  *SGRBG8\_1X8* |
| Configure scaler sink compose selection | 2048x1536  SGRBG8\_1X8 | 2048x1536  SGRBG8\_1X8 | 2046x1534  SGRBG8\_1X8 | 2046x1534  SGRBG8\_1X8 | *0,0/1280x960* | *1280x960*  *SGRBG8\_1X8* |

1. Initial state. The sensor source pad format is set to its native 3MP
   size and V4L2\_MBUS\_FMT\_SGRBG8\_1X8 media bus code. Formats on the
...
2. The application configures the frontend sink pad format’s size to
   2048x1536 and its media bus code to V4L2\_MBUS\_FMT\_SGRBG\_1X8. The
   driver propagates the format to the frontend source pad.
3. The application configures the scaler sink pad format’s size to
   2046x1534 and the media bus code to V4L2\_MBUS\_FMT\_SGRBG\_1X8 to
   match the frontend source size and media bus code. The media bus code
   on the sink pad is set to V4L2\_MBUS\_FMT\_SGRBG\_1X8. The driver
...
struct [`v4l2_rect`](dev-overlay.html#c.V4L.v4l2_rect "v4l2_rect") by the coordinates of the top
...
selection targets [Common selection definitions](selections-common.html#v4l2-selections-common).
...
struct [`v4l2_rect`](dev-overlay.html#c.V4L.v4l2_rect "v4l2_rect").

Scaling support is optional. When supported by a subdev, the crop
rectangle on the subdev’s sink pad is scaled to the size configured
using the
[VIDIOC\_SUBDEV\_S\_SELECTION](vidioc-subdev-g-selection.html#vidioc-subdev-g-selection) IOCTL
...
the image size either up or down. [Selection flags](v4l2-selection-flags.html#v4l2-selection-flags)
...
### 4.13.4.1. Understanding streams
...
### 4.13.4.2. Interaction between routes, streams, formats and selections
...
the same as without streams (see [Order of configuration and format propagation](#format-propagation)).
...
### 4.13.4.3. Device types and routing setup
...
1. Set up links. Connect the pads between sub-devices using the
   [Media Controller API](../mediactl/media-controller.html#media-controller)
2. Streams. Streams are declared and their routing is configured by setting the
   routing table for the sub-device using [VIDIOC\_SUBDEV\_S\_ROUTING](vidioc-subdev-g-routing.html#vidioc-subdev-g-routing) ioctl. Note that setting the routing table will
...
3. Configure formats and selections. Formats and selections of each stream are
   ...
   [Order of configuration and format propagation](#format-propagation). The stream ID is set to the same stream ID
   associated with either sink or source pads of routes configured using the
   [VIDIOC\_SUBDEV\_S\_ROUTING](vidioc-subdev-g-routing.html#vidioc-subdev-g-routing) ioctl.

### 4.13.4.5. Multiplexed streams setup example
...
| --- | --- | --- | --- |
| 0/0 | 2/0 | V4L2\_SUBDEV\_ROUTE\_FL\_ACTIVE | Pixel data stream from Sensor A |
| 1/0 | 2/1 | V4L2\_SUBDEV\_ROUTE\_FL\_ACTIVE | Pixel data stream from Sensor B |

Receiver routing table

| Sink Pad/Stream | Source Pad/Stream | Routing Flags | Comments |
| --- | --- | --- | --- |
| 0/0 | 1/0 | V4L2\_SUBDEV\_ROUTE\_FL\_ACTIVE | Pixel data stream from Sensor A |
| 0/1 | 2/0 | V4L2\_SUBDEV\_ROUTE\_FL\_ACTIVE | Pixel data stream from Sensor B |
...
3. Configure formats and selections
   ...
   [VIDIOC\_SUBDEV\_S\_FMT](vidioc-subdev-g-fmt.html#vidioc-subdev-g-fmt) ioctls to configure each
