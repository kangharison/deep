...
> 출처(원문): https://docs.kernel.org/userspace-api/media/v4l/dev-stateless-decoder.html
...
Stateless decoders make use of the [Request API](../mediactl/request-api.html#media-request-api). A stateless
decoder must expose the `V4L2_BUF_CAP_SUPPORTS_REQUESTS` capability on its
`OUTPUT` queue when [`VIDIOC_REQBUFS()`](vidioc-reqbufs.html#c.V4L.VIDIOC_REQBUFS "VIDIOC_REQBUFS") or [`VIDIOC_CREATE_BUFS()`](vidioc-create-bufs.html#c.V4L.VIDIOC_CREATE_BUFS "VIDIOC_CREATE_BUFS")
are invoked.
...
1. To enumerate the set of coded formats supported by the decoder, the client
   calls [`VIDIOC_ENUM_FMT()`](vidioc-enum-fmt.html#c.V4L.VIDIOC_ENUM_FMT "VIDIOC_ENUM_FMT") on the `OUTPUT` queue.
...
2. To enumerate the set of supported raw formats, the client calls
   [`VIDIOC_ENUM_FMT()`](vidioc-enum-fmt.html#c.V4L.VIDIOC_ENUM_FMT "VIDIOC_ENUM_FMT") on the `CAPTURE` queue.
...
3. The client may use [`VIDIOC_ENUM_FRAMESIZES()`](vidioc-enum-framesizes.html#c.V4L.VIDIOC_ENUM_FRAMESIZES "VIDIOC_ENUM_FRAMESIZES") to detect supported
   ...
   [`v4l2_frmsizeenum`](vidioc-enum-framesizes.html#c.V4L.v4l2_frmsizeenum "v4l2_frmsizeenum")’s `pixel_format`.
...
1. Set the coded format on the `OUTPUT` queue via [`VIDIOC_S_FMT()`](vidioc-g-fmt.html#c.V4L.VIDIOC_S_FMT "VIDIOC_S_FMT").
...
2. Call [`VIDIOC_S_EXT_CTRLS()`](vidioc-g-ext-ctrls.html#c.V4L.VIDIOC_S_EXT_CTRLS "VIDIOC_S_EXT_CTRLS") to set all the controls (parsed headers,
   etc.) required by the `OUTPUT` format to enumerate the `CAPTURE` formats.
3. Call [`VIDIOC_G_FMT()`](vidioc-g-fmt.html#c.V4L.VIDIOC_G_FMT "VIDIOC_G_FMT") for `CAPTURE` queue to get the format for the
...
4. *[optional]* Enumerate `CAPTURE` formats via [`VIDIOC_ENUM_FMT()`](vidioc-enum-fmt.html#c.V4L.VIDIOC_ENUM_FMT "VIDIOC_ENUM_FMT") on
   ...
   select one of them via [`VIDIOC_S_FMT()`](vidioc-g-fmt.html#c.V4L.VIDIOC_S_FMT "VIDIOC_S_FMT").
   ...
   as the `OUTPUT` format, [`VIDIOC_ENUM_FMT()`](vidioc-enum-fmt.html#c.V4L.VIDIOC_ENUM_FMT "VIDIOC_ENUM_FMT") may return a set of
...
5. *[optional]* Choose a different `CAPTURE` format than suggested via
   [`VIDIOC_S_FMT()`](vidioc-g-fmt.html#c.V4L.VIDIOC_S_FMT "VIDIOC_S_FMT") on `CAPTURE` queue. It is possible for the client to
   choose a different format than selected/suggested by the driver in
   [`VIDIOC_G_FMT()`](vidioc-g-fmt.html#c.V4L.VIDIOC_G_FMT "VIDIOC_G_FMT").
...
6. Allocate source (bytestream) buffers via [`VIDIOC_REQBUFS()`](vidioc-reqbufs.html#c.V4L.VIDIOC_REQBUFS "VIDIOC_REQBUFS") on
   `OUTPUT` queue.
...
7. Allocate destination (raw format) buffers via [`VIDIOC_REQBUFS()`](vidioc-reqbufs.html#c.V4L.VIDIOC_REQBUFS "VIDIOC_REQBUFS") on the
   `CAPTURE` queue.
...
9. Start streaming on both `OUTPUT` and `CAPTURE` queues via
   :   [`VIDIOC_STREAMON()`](vidioc-streamon.html#c.V4L.VIDIOC_STREAMON "VIDIOC_STREAMON").
...
1. Queue an `OUTPUT` buffer containing one unit of encoded bytestream data for
   the decoding request, using [`VIDIOC_QBUF()`](vidioc-qbuf.html#c.V4L.VIDIOC_QBUF "VIDIOC_QBUF").
...
2. Set the codec-specific controls for the decoding request, using
   [`VIDIOC_S_EXT_CTRLS()`](vidioc-g-ext-ctrls.html#c.V4L.VIDIOC_S_EXT_CTRLS "VIDIOC_S_EXT_CTRLS").
   ...
   [`VIDIOC_S_EXT_CTRLS()`](vidioc-g-ext-ctrls.html#c.V4L.VIDIOC_S_EXT_CTRLS "VIDIOC_S_EXT_CTRLS"), or to overwrite a previously set control, as
...
3. Call [`VIDIOC_STREAMOFF()`](vidioc-streamon.html#c.V4L.VIDIOC_STREAMOFF "VIDIOC_STREAMOFF") on both the `OUTPUT` and `CAPTURE`
   queues.
4. Free all `CAPTURE` buffers by calling [`VIDIOC_REQBUFS()`](vidioc-reqbufs.html#c.V4L.VIDIOC_REQBUFS "VIDIOC_REQBUFS") on the
