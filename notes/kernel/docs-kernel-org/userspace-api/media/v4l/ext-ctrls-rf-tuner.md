...
> 출처(원문): https://docs.kernel.org/userspace-api/media/v4l/ext-ctrls-rf-tuner.html
...
The RF Tuner (RF\_TUNER) class includes controls for common features of
...
## 1.19.1. RF\_TUNER Control IDs

`V4L2_CID_RF_TUNER_CLASS (class)`
:   The RF\_TUNER class descriptor. Calling
...
:   Filter(s) on tuner signal path are used to filter signal according
    ...
    V4L2\_CID\_RF\_TUNER\_BANDWIDTH\_AUTO is not set. Unit is in Hz. The
