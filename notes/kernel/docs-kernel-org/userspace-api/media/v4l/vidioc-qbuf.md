...
> 출처(원문): https://docs.kernel.org/userspace-api/media/v4l/vidioc-qbuf.html
> 자동 미러링: docs.kernel.org · 정본은 출처 URL (영문 원문 자동 변환본)

---

# 7.46. ioctl VIDIOC\_QBUF, VIDIOC\_DQBUF
...
one. The contents of the struct [`v4l2_buffer`](buffer.html#c.V4L.v4l2_buffer "v4l2_buffer") returned
by a [ioctl VIDIOC\_QUERYBUF](vidioc-querybuf.html#vidioc-querybuf) ioctl will do as well.
When the buffer is intended for output (`type` is
`V4L2_BUF_TYPE_VIDEO_OUTPUT`, `V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE`,
or `V4L2_BUF_TYPE_VBI_OUTPUT`) applications must also initialize the
`bytesused`, `field` and `timestamp` fields, see [Buffers](buffer.html#buffer)
for details. Applications must also set `flags` to 0. The
`reserved2` and `reserved` fields must be set to 0. When using the
[multi-planar API](planar-apis.html#planar-apis), the `m.planes` field must
contain a userspace pointer to a filled-in array of struct
[`v4l2_plane`](buffer.html#c.V4L.v4l2_plane "v4l2_plane") and the `length` field must be set
to the number of elements in that array.

To enqueue a [memory mapped](mmap.html#mmap) buffer applications set the
...
To enqueue a [user pointer](userp.html#userp) buffer applications set the
...
passed array of struct [`v4l2_plane`](buffer.html#c.V4L.v4l2_plane "v4l2_plane") have to be used
...
[VIDIOC\_STREAMOFF](vidioc-streamon.html#vidioc-streamon) or
[ioctl VIDIOC\_REQBUFS](vidioc-reqbufs.html#vidioc-reqbufs) ioctl is called, or until the
device is closed.

To enqueue a [DMABUF](dmabuf.html#dmabuf) buffer applications set the
...
[`v4l2_plane`](buffer.html#c.V4L.v4l2_plane "v4l2_plane") have to be used instead. When
...
dequeued, until the [VIDIOC\_STREAMOFF](vidioc-streamon.html#vidioc-streamon) or
[ioctl VIDIOC\_REQBUFS](vidioc-reqbufs.html#vidioc-reqbufs) ioctl is called, or until the
device is closed.

The `request_fd` field can be used with the `VIDIOC_QBUF` ioctl to specify
the file descriptor of a [request](../mediactl/request-api.html#media-request-api), if requests are
...
[VIDIOC\_STREAMOFF](vidioc-streamon.html#vidioc-streamon) or calling [ioctl VIDIOC\_REQBUFS](vidioc-reqbufs.html#vidioc-reqbufs)
the check for this will be reset.

For [memory-to-memory devices](dev-mem2mem.html#mem2mem) you can specify the
...
a struct [`v4l2_buffer`](buffer.html#c.V4L.v4l2_buffer "v4l2_buffer") as above, when
...
dequeue a [DMABUF](dmabuf.html#dmabuf) buffer, the driver fills the `m.fd` field
...
API is used the `m.fd` fields of the passed array of struct
[`v4l2_plane`](buffer.html#c.V4L.v4l2_plane "v4l2_plane") are filled instead.

By default `VIDIOC_DQBUF` blocks when no buffer is in the outgoing
queue. When the `O_NONBLOCK` flag was given to the
[`open()`](func-open.html#c.V4L.open "open") function, `VIDIOC_DQBUF` returns
