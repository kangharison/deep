...
> 출처(원문): https://docs.kernel.org/userspace-api/media/v4l/control.html
...
controls using `V4L2_CID_PRIVATE_BASE` [[1]](#f1) and higher values. The
pre-defined control IDs have the prefix `V4L2_CID_`, and are listed in
[Control IDs](#control-id). The ID is used when querying the attributes of a
...
change value (see
[ioctl VIDIOC\_SUBSCRIBE\_EVENT, VIDIOC\_UNSUBSCRIBE\_EVENT](vidioc-subscribe-event.html#vidioc-subscribe-event), event
...
:   Rotates the image by specified angle. Common angles are 90, 270 and
    ...
    [VIDIOC\_S\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) ioctl according to the
...
:   Sets the alpha color component. When a capture device (or capture
    ...
    [packed RGB image formats](pixfmt-rgb.html#pixfmt-rgb)) and the alpha value
...
:   ID of the first custom (driver specific) control. Applications
    ...
    and version, see [Querying Capabilities](querycap.html#querycap).

Applications can enumerate the available controls with the
[ioctls VIDIOC\_QUERYCTRL, VIDIOC\_QUERY\_EXT\_CTRL and VIDIOC\_QUERYMENU](vidioc-queryctrl.html#vidioc-queryctrl) and
[VIDIOC\_QUERYMENU](vidioc-queryctrl.html#vidioc-queryctrl) ioctls, get and set a
control value with the [VIDIOC\_G\_CTRL](vidioc-g-ctrl.html#vidioc-g-ctrl) and
[VIDIOC\_S\_CTRL](vidioc-g-ctrl.html#vidioc-g-ctrl) ioctls. Drivers must implement
...
instead of using [ioctls VIDIOC\_QUERYCTRL, VIDIOC\_QUERY\_EXT\_CTRL and VIDIOC\_QUERYMENU](vidioc-queryctrl.html#vidioc-queryctrl) with
