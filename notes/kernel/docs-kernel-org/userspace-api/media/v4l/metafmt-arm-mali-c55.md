...
> 출처(원문): https://docs.kernel.org/userspace-api/media/v4l/metafmt-arm-mali-c55.html
> 자동 미러링: docs.kernel.org · 정본은 출처 URL (영문 원문 자동 변환본)

---

# 2.13.1. V4L2\_META\_FMT\_MALI\_C55\_STATS (‘C55S’), V4L2\_META\_FMT\_MALI\_C55\_PARAMS (‘C55P’)
...
[mali-c55 3a stats](../../../admin-guide/media/mali-c55.html#mali-c55-3a-stats) metadata capture video node, using
the `v4l2_meta_format` interface. The buffer contains a single instance
of the C structure [`mali_c55_stats_buffer`](#c.mali_c55_stats_buffer "mali_c55_stats_buffer") defined in
...
For details of the statistics see [`mali_c55_stats_buffer`](#c.mali_c55_stats_buffer "mali_c55_stats_buffer").

## 2.13.1.2. Configuration Parameters

The configuration parameters are passed to the [mali-c55 3a params](../../../admin-guide/media/mali-c55.html#mali-c55-3a-params) metadata output video node, using the
...
[`v4l2_isp_params_buffer`](v4l2-isp.html#c.v4l2_isp_params_buffer "v4l2_isp_params_buffer"). Userspace is responsible for populating the
data member with the blocks that need to be configured by the driver. Each
block-specific `struct embeds` [`v4l2_isp_params_block_header`](v4l2-isp.html#c.v4l2_isp_params_block_header "v4l2_isp_params_block_header") as its first
member and userspace must populate the type member with a value from
[`mali_c55_param_block_type`](#c.mali_c55_param_block_type "mali_c55_param_block_type").
...
struct mali\_c55\_ae\_1024bin\_hist
...
struct mali\_c55\_ae\_5bin\_hist
...
struct mali\_c55\_awb\_average\_ratios
...
struct mali\_c55\_af\_statistics
...
struct mali\_c55\_stats\_buffer
...
“unknown” and may not be 0, but should not be used. The **ae\_5bin\_hists**,
**awb\_ratios** and **af\_statistics** members are arrays of statistics per-zone.
The zones are arranged in the array in raster order starting from the top
left corner of the image.

enum mali\_c55\_param\_block\_type
...
:   Auto-exposure 1024-bin histogram
    configuration

`MALI_C55_PARAM_BLOCK_AEXP_IHIST`
:   Post-Iridix auto-exposure 1024-bin
...
struct mali\_c55\_params\_sensor\_off\_preshading
...
header.type should be set to MALI\_C55\_PARAM\_BLOCK\_SENSOR\_OFFS from
[`mali_c55_param_block_type`](#c.mali_c55_param_block_type "mali_c55_param_block_type") for this block.

enum mali\_c55\_aexp\_hist\_tap\_points
...
enum mali\_c55\_aexp\_row\_column\_offset
...
enum mali\_c55\_aexp\_hist\_plane\_mode
...
struct mali\_c55\_params\_aexp\_hist
...
`skip_x`
:   Horizontal decimation. See [`enum mali_c55_aexp_skip_x`](#c.mali_c55_aexp_skip_x "mali_c55_aexp_skip_x")

`offset_x`
:   Skip the first column, or not. See [`enum mali_c55_aexp_row_column_offset`](#c.mali_c55_aexp_row_column_offset "mali_c55_aexp_row_column_offset")

`skip_y`
:   Vertical decimation. See [`enum mali_c55_aexp_skip_y`](#c.mali_c55_aexp_skip_y "mali_c55_aexp_skip_y")

`offset_y`
:   Skip the first row, or not. See [`enum mali_c55_aexp_row_column_offset`](#c.mali_c55_aexp_row_column_offset "mali_c55_aexp_row_column_offset")
...
`plane_mode`
:   Plane separation mode. See [`enum mali_c55_aexp_hist_plane_mode`](#c.mali_c55_aexp_hist_plane_mode "mali_c55_aexp_hist_plane_mode")

`tap_point`
:   Tap point for histogram from [`enum mali_c55_aexp_hist_tap_points`](#c.mali_c55_aexp_hist_tap_points "mali_c55_aexp_hist_tap_points").
...
The skip\_x, offset\_x, skip\_y and offset\_y fields allow users to ignore or
mask pixels in the frame by their position relative to the top left pixel.
First, the skip\_y, offset\_x and offset\_y fields define which of the pixels
within each 2x2 region will be counted in the statistics.

If skip\_y == 0 then two pixels from each covered region will be counted. If
both offset\_x and offset\_y are zero, then the two left-most pixels in each
2x2 pixel region will be counted. Setting offset\_x = 1 will discount the top
left pixel and count the top right pixel. Setting offset\_y = 1 will discount
the bottom left pixel and count the bottom right pixel.

If skip\_y != 0 then only a single pixel from each region covered by the
pattern will be counted. In this case offset\_x controls whether the pixel
that’s counted is in the left (if offset\_x == 0) or right (if offset\_x == 1)
column and offset\_y controls whether the pixel that’s counted is in the top
(if offset\_y == 0) or bottom (if offset\_y == 1) row.

The skip\_x and skip\_y fields control how the 2x2 pixel region is repeated
across the image data. The first instance of the region is always in the top
left of the image data. The skip\_x field controls how many pixels are ignored
in the x direction before the pixel masking region is repeated. The skip\_y
...
necessary to set the skip\_x and skip\_y fields in a way that resulted in all
...
header.type should be set to one of either MALI\_C55\_PARAM\_BLOCK\_AEXP\_HIST or
MALI\_C55\_PARAM\_BLOCK\_AEXP\_IHIST from [`mali_c55_param_block_type`](#c.mali_c55_param_block_type "mali_c55_param_block_type").

struct mali\_c55\_params\_aexp\_weights
...
header.type should be set to one of either MALI\_C55\_PARAM\_BLOCK\_AEXP\_HIST\_WEIGHTS
or MALI\_C55\_PARAM\_BLOCK\_AEXP\_IHIST\_WEIGHTS from [`mali_c55_param_block_type`](#c.mali_c55_param_block_type "mali_c55_param_block_type").

struct mali\_c55\_params\_digital\_gain
:   Digital gain value
...
header.type should be set to MALI\_C55\_PARAM\_BLOCK\_DIGITAL\_GAIN from
[`mali_c55_param_block_type`](#c.mali_c55_param_block_type "mali_c55_param_block_type") for this block.

enum mali\_c55\_awb\_stats\_mode
...
struct mali\_c55\_params\_awb\_gains
...
header.type should be set to one of either MALI\_C55\_PARAM\_BLOCK\_AWB\_GAINS or
MALI\_C55\_PARAM\_BLOCK\_AWB\_GAINS\_AEXP from [`mali_c55_param_block_type`](#c.mali_c55_param_block_type "mali_c55_param_block_type").

enum mali\_c55\_params\_awb\_tap\_points
...
struct mali\_c55\_params\_awb\_config
...
`tap_point`
:   The tap point from [`enum mali_c55_params_awb_tap_points`](#c.mali_c55_params_awb_tap_points "mali_c55_params_awb_tap_points")

`stats_mode`
:   AWB statistics collection mode, see [`mali_c55_awb_stats_mode`](#c.mali_c55_awb_stats_mode "mali_c55_awb_stats_mode")
...
header.type should be set to MALI\_C55\_PARAM\_BLOCK\_AWB\_CONFIG from
[`mali_c55_param_block_type`](#c.mali_c55_param_block_type "mali_c55_param_block_type") for this block.

struct mali\_c55\_params\_mesh\_shading\_config
...
`mesh_scale`
:   Set the precision and maximum gain range of mesh shading
    - 0 = 0-2x gain
    - 1 = 0-4x gain
    - 2 = 0-8x gain
    - 3 = 0-16x gain
...
The **mesh\_scale** member determines the precision and minimum and maximum gain.
For example if **mesh\_scale** is 0 and therefore selects 0 - 2x gain, a value of
0 in a coefficient means 0.0 gain, a value of 128 means 1.0 gain and 255
means 2.0 gain.

header.type should be set to MALI\_C55\_PARAM\_MESH\_SHADING\_CONFIG from
[`mali_c55_param_block_type`](#c.mali_c55_param_block_type "mali_c55_param_block_type") for this block.

struct mali\_c55\_params\_mesh\_shading\_selection
...
colour channel) by the **mesh\_alpha\_bank\_r**/g/b fields.
...
Where a is the **mesh\_alpha\_r**/g/b value, and LSaand LSbare the node cofficients for the two tables selected by the
**mesh\_alpha\_bank\_r**/g/b value.

The scale of the applied correction may also be controlled by tuning the
**mesh\_strength** member. This is a modifier to the final coefficients which can
be used to globally reduce the gains applied.

header.type should be set to MALI\_C55\_PARAM\_MESH\_SHADING\_SELECTION from
[`mali_c55_param_block_type`](#c.mali_c55_param_block_type "mali_c55_param_block_type") for this block.

MALI\_C55\_PARAMS\_MAX\_SIZE
...
> header types from [`mali_c55_param_block_type`](#c.mali_c55_param_block_type "mali_c55_param_block_type").
