...
> 출처(원문): https://docs.kernel.org/userspace-api/media/v4l/dev-sliced-vbi.html
...
[raw VBI interface](dev-raw-vbi.html#raw-vbi). The data is passed as short
...
calling the [VIDIOC\_S\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) ioctl as defined
...
[ioctl VIDIOC\_QUERYCAP](vidioc-querycap.html#vidioc-querycap) ioctl. At least one of the
read/write or streaming [I/O methods](io.html#io) must be
supported. Sliced VBI devices may have a tuner or modulator.

## 4.7.2. Supplemental Functions

Sliced VBI devices shall support [video input or output](video.html#video)
and [tuner or modulator](tuner.html#tuner) ioctls if they have these
capabilities, and they may support [User Controls](control.html#control) ioctls.
The [video standard](standard.html#standard) ioctls provide information vital
...
[VIDIOC\_S\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) ioctl when the number of VBI
...
`type` field of struct [`v4l2_format`](vidioc-g-fmt.html#c.V4L.v4l2_format "v4l2_format") to
`V4L2_BUF_TYPE_SLICED_VBI_CAPTURE` or
`V4L2_BUF_TYPE_SLICED_VBI_OUTPUT`, and the
[VIDIOC\_G\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) ioctl fills the `fmt.sliced`
member, a struct
[`v4l2_sliced_vbi_format`](#c.V4L.v4l2_sliced_vbi_format "v4l2_sliced_vbi_format").

Applications can request different parameters by initializing or
modifying the `fmt.sliced` member and calling the
[VIDIOC\_S\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) ioctl with a pointer to the
struct [`v4l2_format`](vidioc-g-fmt.html#c.V4L.v4l2_format "v4l2_format") structure.
...
The [VIDIOC\_S\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) ioctl modifies the parameters
...
[ioctl VIDIOC\_STREAMON, VIDIOC\_STREAMOFF](vidioc-streamon.html#vidioc-streamon) ioctl and the first
[`read()`](func-read.html#c.V4L.read "read"), [`write()`](func-write.html#c.V4L.write "write") and
[`select()`](func-select.html#c.V4L.select "select") call.

type v4l2\_sliced\_vbi\_format

### 4.7.3.1. struct v4l2\_sliced\_vbi\_format

|  |  |  |  |  |
| --- | --- | --- | --- | --- |
| \_\_u16 | `service_set` | If `service_set` is non-zero when passed with [VIDIOC\_S\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) or [VIDIOC\_TRY\_FMT](vidioc-g-fmt.html#vidioc-g-fmt), the `service_lines` array will be filled by the driver according to the services specified in this field. For example, if `service_set` is initialized with `V4L2_SLICED_TELETEXT_B | V4L2_SLICED_WSS_625`, a driver for the cx25840 video decoder sets lines 7-22 of both fields [[1]](#f1) to `V4L2_SLICED_TELETEXT_B` and line 23 of the first field to `V4L2_SLICED_WSS_625`. If `service_set` is set to zero, then the values of `service_lines` will be used instead.  On return the driver sets this field to the `union of` all elements of the returned `service_lines` array. It may contain less services than requested, perhaps just one, if the hardware cannot handle more services simultaneously. It may be empty (zero) if none of the requested services are supported by the hardware. | | |
| \_\_u16 | `service_lines`[2][24] | Applications initialize this array with sets of data services the driver shall look for or insert on the respective scan line. Subject to hardware capabilities drivers return the requested set, a subset, which may be just a single service, or an empty set. When the hardware cannot handle multiple services on the same line the driver shall choose one. No assumptions can be made on which service the driver chooses.  Data services are defined in [Sliced VBI services](#vbi-services2). Array indices map to ITU-R line numbers[[2]](#f2) as follows: | | |
...
| \_\_u32 | `io_size` | Maximum number of bytes passed by one [`read()`](func-read.html#c.V4L.read "read") or [`write()`](func-write.html#c.V4L.write "write") call, and the buffer size in bytes for the [ioctl VIDIOC\_QBUF, VIDIOC\_DQBUF](vidioc-qbuf.html#vidioc-qbuf) and [VIDIOC\_DQBUF](vidioc-qbuf.html#vidioc-qbuf) ioctl. Drivers set this field to the size of struct [`v4l2_sliced_vbi_data`](#c.V4L.v4l2_sliced_vbi_data "v4l2_sliced_vbi_data") times the number of non-zero elements in the returned `service_lines` array (that is the number of lines potentially carrying data). | | |
| \_\_u32 | `reserved`[2] | This array is reserved for future extensions.  Applications and drivers must set it to zero. | | |
...
| `V4L2_SLICED_WSS_625` | 0x4000 | [ITU BT.1119](biblio.html#itu1119),  [EN 300 294](biblio.html#en300294) | PAL/SECAM line 23 | See [V4L2\_SLICED\_WSS\_625 payload](#v4l2-sliced-wss-625-payload) below. |
...
a side effect). The [VIDIOC\_S\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) ioctl may
return an `EBUSY` error code when applications attempt to change the
format while i/o is in progress (between a
[ioctl VIDIOC\_STREAMON, VIDIOC\_STREAMOFF](vidioc-streamon.html#vidioc-streamon) and
[VIDIOC\_STREAMOFF](vidioc-streamon.html#vidioc-streamon) call, and after the first
[`read()`](func-read.html#c.V4L.read "read") or [`write()`](func-write.html#c.V4L.write "write") call).

#### 4.7.3.2.1. V4L2\_SLICED\_WSS\_625 payload
...
A single [`read()`](func-read.html#c.V4L.read "read") or [`write()`](func-write.html#c.V4L.write "write")
call must pass all data belonging to one video frame. That is an array
of struct [`v4l2_sliced_vbi_data`](#c.V4L.v4l2_sliced_vbi_data "v4l2_sliced_vbi_data") structures with one or
...
struct [`v4l2_sliced_vbi_data`](#c.V4L.v4l2_sliced_vbi_data "v4l2_sliced_vbi_data") elements must be zero.

type v4l2\_sliced\_vbi\_data

### 4.7.4.1. struct v4l2\_sliced\_vbi\_data

|  |  |  |
| --- | --- | --- |
| \_\_u32 | `id` | A flag from [Sliced VBI services](vidioc-g-sliced-vbi-cap.html#vbi-services) identifying the type of data in this packet. Only a single bit must be set. When the `id` of a captured packet is zero, the packet is empty and the contents of other fields are undefined. Applications shall ignore empty packets. When the `id` of a packet for output is zero the contents of the `data` field are undefined and the driver must no longer insert data on the requested `field` and `line`. |
...
| \_\_u32 | `line` | The field (as opposed to frame) line number this data has been captured from, or shall be inserted at. See [ITU-R 525 line numbering (M/NTSC and M/PAL)](dev-raw-vbi.html#vbi-525) and [ITU-R 625 line numbering](dev-raw-vbi.html#vbi-625) for valid values. Sliced VBI capture devices can set the line number of all packets to `0` if the hardware cannot reliably identify scan lines. The field number must always be valid. |
| \_\_u32 | `reserved` | This field is reserved for future extensions. Applications and drivers must set it to zero. |
| \_\_u8 | `data`[48] | The packet payload. See [Sliced VBI services](vidioc-g-sliced-vbi-cap.html#vbi-services) for the contents and number of bytes passed for each data type. The contents of padding bytes at the end of this array are undefined, drivers and applications shall ignore them. |

Packets are always passed in ascending line number order, without
duplicate line numbers. The [`write()`](func-write.html#c.V4L.write "write") function and
the [ioctl VIDIOC\_QBUF, VIDIOC\_DQBUF](vidioc-qbuf.html#vidioc-qbuf) ioctl must return an `EINVAL`
...
been negotiated with the [VIDIOC\_G\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) or
[VIDIOC\_S\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) ioctl. When the line numbers are
...
A sliced VBI device may support [read/write](rw.html#rw) and/or
streaming ([memory mapping](mmap.html#mmap) and/or
[user pointer](userp.html#userp)) I/O. The latter bears the possibility of
...
providing
[negotiated sliced VBI services](#sliced-vbi-format-negotiation)
as data embedded in the MPEG stream. Users or applications control this
sliced VBI data insertion with the
[V4L2\_CID\_MPEG\_STREAM\_VBI\_FMT](ext-ctrls-codec.html#v4l2-mpeg-stream-vbi-fmt)
control.

If the driver does not provide the
[V4L2\_CID\_MPEG\_STREAM\_VBI\_FMT](ext-ctrls-codec.html#v4l2-mpeg-stream-vbi-fmt)
control, or only allows that control to be set to
[V4L2\_MPEG\_STREAM\_VBI\_FMT\_NONE](ext-ctrls-codec.html#v4l2-mpeg-stream-vbi-fmt),
...
The
[V4L2\_MPEG\_STREAM\_VBI\_FMT\_IVTV](ext-ctrls-codec.html#v4l2-mpeg-stream-vbi-fmt)
...
[`v4l2_mpeg_vbi_fmt_ivtv`](#c.V4L.v4l2_mpeg_vbi_fmt_ivtv "v4l2_mpeg_vbi_fmt_ivtv"). The
...
type v4l2\_mpeg\_vbi\_fmt\_ivtv

### 4.7.5.3. struct v4l2\_mpeg\_vbi\_fmt\_ivtv

|  |  |  |
| --- | --- | --- |
| \_\_u8 | `magic`[4] | A “magic” constant from [Magic Constants for struct v4l2\_mpeg\_vbi\_fmt\_ivtv magic field](#v4l2-mpeg-vbi-fmt-ivtv-magic) that indicates this is a valid sliced VBI data payload and also indicates which member of the anonymous union, `itv0` or `ITV0`, to use for the payload data. |
| union { | (anonymous) | |
| struct [`v4l2_mpeg_vbi_itv0`](#c.V4L.v4l2_mpeg_vbi_itv0 "v4l2_mpeg_vbi_itv0") | `itv0` | The primary form of the sliced VBI data payload that contains anywhere from 1 to 35 lines of sliced VBI data. Line masks are provided in this form of the payload indicating which VBI lines are provided. |
| struct [v4l2\_mpeg\_vbi\_ITV0](#v4l2-mpeg-vbi-itv0-1) | `ITV0` | An alternate form of the sliced VBI data payload used when 36 lines of sliced VBI data are present. No line masks are provided in this form of the payload; all valid line mask bits are implicitly set. |
| } |  | |

### 4.7.5.4. Magic Constants for struct v4l2\_mpeg\_vbi\_fmt\_ivtv magic field

| Defined Symbol | Value | Description |
| --- | --- | --- |
| `V4L2_MPEG_VBI_IVTV_MAGIC0` | “itv0” | Indicates the `itv0` member of the `union in` struct [`v4l2_mpeg_vbi_fmt_ivtv`](#c.V4L.v4l2_mpeg_vbi_fmt_ivtv "v4l2_mpeg_vbi_fmt_ivtv") is valid. |
| `V4L2_MPEG_VBI_IVTV_MAGIC1` | “ITV0” | Indicates the `ITV0` member of the `union in` struct [`v4l2_mpeg_vbi_fmt_ivtv`](#c.V4L.v4l2_mpeg_vbi_fmt_ivtv "v4l2_mpeg_vbi_fmt_ivtv") is valid and that 36 lines of sliced VBI data are present. |

type v4l2\_mpeg\_vbi\_itv0

type v4l2\_mpeg\_vbi\_ITV0

### 4.7.5.5. structs v4l2\_mpeg\_vbi\_itv0 and v4l2\_mpeg\_vbi\_ITV0
...
| struct [`v4l2_mpeg_vbi_itv0_line`](#c.V4L.v4l2_mpeg_vbi_itv0_line "v4l2_mpeg_vbi_itv0_line") | `line`[35] | This is a variable length array that holds from 1 to 35 lines of sliced VBI data. The sliced VBI data lines present correspond to the bits set in the `linemask` array, starting from b0 of `linemask`[0] up through b31 of `linemask`[0], and from b0 of `linemask`[1] up through b3 of `linemask`[1]. `line`[0] corresponds to the first bit found set in the `linemask` array, `line`[1] corresponds to the second bit found set in the `linemask` array, etc. If no `linemask` array bits are set, then `line`[0] may contain one line of unspecified data that should be ignored by applications. |

### 4.7.5.6. struct v4l2\_mpeg\_vbi\_ITV0

|  |  |  |
| --- | --- | --- |
| struct [`v4l2_mpeg_vbi_itv0_line`](#c.V4L.v4l2_mpeg_vbi_itv0_line "v4l2_mpeg_vbi_itv0_line") | `line`[36] | A fixed length array of 36 lines of sliced VBI data. `line`[0] through `line`[17] correspond to lines 6 through 23 of the first field. `line`[18] through `line`[35] corresponds to lines 6 through 23 of the second field. |

type v4l2\_mpeg\_vbi\_itv0\_line

### 4.7.5.7. struct v4l2\_mpeg\_vbi\_itv0\_line

|  |  |  |
| --- | --- | --- |
| \_\_u8 | `id` | A line identifier value from [Line Identifiers for struct v4l2\_mpeg\_vbi\_itv0\_line id field](#itv0-line-identifier-constants) that indicates the type of sliced VBI data stored on this line. |
| \_\_u8 | `data`[42] | The sliced VBI data for the line. |

### 4.7.5.8. Line Identifiers for struct v4l2\_mpeg\_vbi\_itv0\_line id field

| Defined Symbol | Value | Description |
| --- | --- | --- |
| `V4L2_MPEG_VBI_IVTV_TELETEXT_B` | 1 | Refer to [Sliced VBI services](#vbi-services2) for a description of the line payload. |
| `V4L2_MPEG_VBI_IVTV_CAPTION_525` | 4 | Refer to [Sliced VBI services](#vbi-services2) for a description of the line payload. |
| `V4L2_MPEG_VBI_IVTV_WSS_625` | 5 | Refer to [Sliced VBI services](#vbi-services2) for a description of the line payload. |
| `V4L2_MPEG_VBI_IVTV_VPS` | 7 | Refer to [Sliced VBI services](#vbi-services2) for a description of the line payload. |

[[1](#id2)]

According to [ETS 300 706](biblio.html#ets300706) lines 6-22 of the first
