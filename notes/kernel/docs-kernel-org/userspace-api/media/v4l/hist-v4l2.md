...
> 출처(원문): https://docs.kernel.org/userspace-api/media/v4l/hist-v4l2.html
...
1998-08-27: The [`select()`](func-select.html#c.V4L.select "select") function was introduced.

1998-09-10: New video standard interface.

1998-09-18: The `VIDIOC_NONCAP` ioctl was replaced by the otherwise
meaningless `O_TRUNC` [`open()`](func-open.html#c.V4L.open "open") flag, and the
...
renamed. The [ioctl VIDIOC\_QUERYSTD, VIDIOC\_SUBDEV\_QUERYSTD](vidioc-querystd.html#vidioc-querystd) ioctl was
renamed to [ioctl VIDIOC\_ENUMSTD, VIDIOC\_SUBDEV\_ENUMSTD](vidioc-enumstd.html#vidioc-enumstd),
[VIDIOC\_G\_INPUT](vidioc-g-input.html#vidioc-g-input) to
[ioctl VIDIOC\_ENUMINPUT](vidioc-enuminput.html#vidioc-enuminput). A first draft of the
Codec API was released.

1998-11-08: Many minor changes. Most symbols have been renamed. Some
material changes to [`struct v4l2_capability`](vidioc-querycap.html#c.V4L.v4l2_capability "V4L.v4l2_capability").
...
[VIDIOC\_S\_CTRL](vidioc-g-ctrl.html#vidioc-g-ctrl) ioctls under names starting
...
1999-01-27: There is now one QBUF ioctl, VIDIOC\_QWBUF and VIDIOC\_QRBUF
are gone. VIDIOC\_QBUF takes a v4l2\_buffer as a parameter. Added
...
1999-03-18: Fill in the category and catname fields of v4l2\_queryctrl
objects before passing them to the driver. Required a minor change to
the VIDIOC\_QUERYCTRL handlers in the sample drivers.
...
V4L2\_CID\_HFLIP, \_VFLIP. Changed V4L2\_PIX\_FMT\_YUV422P to \_YUV422P,
...
V4L2\_CTRL\_FLAG\_GRAYED (later V4L2\_CTRL\_FLAG\_GRABBED) flag.

1999-05-20: Definition of VIDIOC\_G\_CTRL was wrong causing a
...
1. Some typos in `V4L2_FMT_FLAG` symbols were fixed. [`struct v4l2_clip`](dev-overlay.html#c.V4L.v4l2_clip "V4L.v4l2_clip")
...
3. All `ioctl()` commands that used an integer argument now take a pointer
   ...
   convention in the V4L2 API. The affected ioctls are: VIDIOC\_PREVIEW,
   VIDIOC\_STREAMON, VIDIOC\_STREAMOFF, VIDIOC\_S\_FREQ,
   VIDIOC\_S\_INPUT, VIDIOC\_S\_OUTPUT, VIDIOC\_S\_EFFECT. For example
...
4. All the different get- and set-format commands were swept into one
   ...
   [VIDIOC\_S\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) ioctl taking a `union and` a
   ...
   [`struct v4l2_format`](vidioc-g-fmt.html#c.V4L.v4l2_format "V4L.v4l2_format") was renamed to [`struct v4l2_pix_format`](pixfmt-v4l2.html#c.V4L.v4l2_pix_format "V4L.v4l2_pix_format"), while
   [`struct v4l2_format`](vidioc-g-fmt.html#c.V4L.v4l2_format "V4L.v4l2_format") is now the enveloping structure
...
5. Similar to the changes above, the `VIDIOC_G_PARM` and
   ...
   `VIDIOC_S_OUTPARM`. A `type` field in the new [`struct v4l2_streamparm`](vidioc-g-parm.html#c.V4L.v4l2_streamparm "V4L.v4l2_streamparm")
...
6. Control enumeration was simplified, and two new control flags were
   ...
   separate windows.
7. The [`struct v4l2_buffer`](buffer.html#c.V4L.v4l2_buffer "V4L.v4l2_buffer") `timestamp` was
   ...
   stream. The data type name for timestamps is stamp\_t, defined as a
   ...
   /cpirazzi\_engr/lg/time/intro.html. UST uses timestamps that are
...
8. A `sequence` field was added to [`struct v4l2_buffer`](buffer.html#c.V4L.v4l2_buffer "V4L.v4l2_buffer"). The `sequence`
...
1999-12-23: In [`struct v4l2_vbi_format`](dev-raw-vbi.html#c.V4L.v4l2_vbi_format "V4L.v4l2_vbi_format") the
...
[VIDIOC\_S\_FREQUENCY](vidioc-g-frequency.html#vidioc-g-frequency) ioctls were added.
...
compatibility* as the [VIDIOC\_G\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) and
[VIDIOC\_S\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) ioctls may fail now if the
struct `v4l2_fmt` `type` field does not contain
`V4L2_BUF_TYPE_VBI`. In the documentation of the [`struct v4l2_vbi_format`](dev-raw-vbi.html#c.V4L.v4l2_vbi_format "V4L.v4l2_vbi_format")`,
...
3. Drivers are again permitted to return negative (unknown) start values
   ...
   error codes with the [VIDIOC\_S\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) ioctl was
...
1. As specified in [Related Devices](open.html#related), drivers must make related device
   functions available under all minor device numbers.
2. The [`open()`](func-open.html#c.V4L.open "open") function requires access mode
...
3. The [`struct v4l2_capability`](vidioc-querycap.html#c.V4L.v4l2_capability "V4L.v4l2_capability") changed
   ...
   capability V4L2\_CAP\_ASYNCIO can be flagged. In response to these
   ...
   [Video Inputs and Outputs](video.html#video) and [Audio Inputs and Outputs](audio.html#audio).
   ...
   described in [Data Formats](format.html#format) and [Video Standards](standard.html#standard).

   `V4L2_FLAG_SELECT` was removed. We believe the [`select()`](func-select.html#c.V4L.select "V4L.select") function
   ...
   available as described in [Data Formats](format.html#format).
4. In [`struct v4l2_input`](vidioc-enuminput.html#c.V4L.v4l2_input "V4L.v4l2_input") the `assoc_audio`
   ...
   Accordingly [`struct v4l2_output`](vidioc-enumoutput.html#c.V4L.v4l2_output "V4L.v4l2_output") lost its
   `capability` and `assoc_audio` fields. `audioset`,
   `modulator` and `std` where added instead.
5. The [`struct v4l2_audio`](vidioc-g-audio.html#c.V4L.v4l2_audio "V4L.v4l2_audio") field `audio` was
   ...
   Again for consistency the [`struct v4l2_audioout`](vidioc-g-audioout.html#c.V4L.v4l2_audioout "V4L.v4l2_audioout") field `audio` was renamed
   to `index`.
6. The [`struct v4l2_tuner`](vidioc-g-tuner.html#c.V4L.v4l2_tuner "V4L.v4l2_tuner") `input` field was
   ...
   simple set (more about this below) and moved into [`struct v4l2_input`](vidioc-enuminput.html#c.V4L.v4l2_input "V4L.v4l2_input").
   A `type` field was added.

   Accordingly in [`struct v4l2_modulator`](vidioc-g-modulator.html#c.V4L.v4l2_modulator "V4L.v4l2_modulator") the
   `output` was replaced by an `index` field.

   In [`struct v4l2_frequency`](vidioc-g-frequency.html#c.V4L.v4l2_frequency "V4L.v4l2_frequency") the `port`
...
7. The idea of completely transparent video standards was dropped.
   ...
   standards by [v4l2\_std\_id](vidioc-enumstd.html#v4l2-std-id) and symbols
   defined in the `videodev2.h` header file. For details see
   [Video Standards](standard.html#standard). The [VIDIOC\_G\_STD](vidioc-g-std.html#vidioc-g-std) and
   [VIDIOC\_S\_STD](vidioc-g-std.html#vidioc-g-std) now take a pointer to this
   type as argument. [ioctl VIDIOC\_QUERYSTD, VIDIOC\_SUBDEV\_QUERYSTD](vidioc-querystd.html#vidioc-querystd) was
   added to autodetect the received standard, if the hardware has this
   capability. In [`struct v4l2_standard`](vidioc-enumstd.html#c.V4L.v4l2_standard "V4L.v4l2_standard") an
   `index` field was added for
   [ioctl VIDIOC\_ENUMSTD, VIDIOC\_SUBDEV\_ENUMSTD](vidioc-enumstd.html#vidioc-enumstd). A
   [v4l2\_std\_id](vidioc-enumstd.html#v4l2-std-id) field named `id` was added as
   ...
   [ioctl VIDIOC\_ENUMSTD, VIDIOC\_SUBDEV\_ENUMSTD](vidioc-enumstd.html#vidioc-enumstd) now takes a pointer to a
   [`struct v4l2_standard`](vidioc-enumstd.html#c.V4L.v4l2_standard "V4L.v4l2_standard") directly. The
   information which standards are supported by a particular video
   input or output moved into [`struct v4l2_input`](vidioc-enuminput.html#c.V4L.v4l2_input "V4L.v4l2_input")
   and [`struct v4l2_output`](vidioc-enumoutput.html#c.V4L.v4l2_output "V4L.v4l2_output") fields named `std`,
   respectively.
8. The struct [v4l2\_queryctrl](vidioc-queryctrl.html#v4l2-queryctrl) fields
   `category` and `group` did not catch on and/or were not
   implemented as expected and therefore removed.
9. The [VIDIOC\_TRY\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) ioctl was added to
   negotiate data formats as with
   [VIDIOC\_S\_FMT](vidioc-g-fmt.html#vidioc-g-fmt), but without the overhead of
   programming the hardware and regardless of I/O in progress.

   In [`struct v4l2_format`](vidioc-g-fmt.html#c.V4L.v4l2_format "V4L.v4l2_format") the `fmt` `union was`
   extended to contain [`struct v4l2_window`](dev-overlay.html#c.V4L.v4l2_window "V4L.v4l2_window"). All
   ...
   removed. The `type` field changed to type [`enum v4l2_buf_type`](buffer.html#c.V4L.v4l2_buf_type "V4L.v4l2_buf_type") and
   the buffer type names changed as follows.

   | Old defines | [`enum v4l2_buf_type`](buffer.html#c.V4L.v4l2_buf_type "V4L.v4l2_buf_type") |
   | --- | --- |
...
10. In [`struct v4l2_fmtdesc`](vidioc-enum-fmt.html#c.V4L.v4l2_fmtdesc "V4L.v4l2_fmtdesc") a [`enum v4l2_buf_type`](buffer.html#c.V4L.v4l2_buf_type "V4L.v4l2_buf_type") field named `type` was
    added as in [`struct v4l2_format`](vidioc-g-fmt.html#c.V4L.v4l2_format "V4L.v4l2_format"). The `VIDIOC_ENUM_FBUFFMT` ioctl is no
    longer needed and was removed. These calls can be replaced by
    [ioctl VIDIOC\_ENUM\_FMT](vidioc-enum-fmt.html#vidioc-enum-fmt) with type `V4L2_BUF_TYPE_VIDEO_OVERLAY`.
11. In [`struct v4l2_pix_format`](pixfmt-v4l2.html#c.V4L.v4l2_pix_format "V4L.v4l2_pix_format") the `depth`
    ...
    The interlace flags were replaced by a [`enum v4l2_field`](field-order.html#c.V4L.v4l2_field "V4L.v4l2_field") value in a
    newly added `field` field.

    | Old flag | [`enum v4l2_field`](field-order.html#c.V4L.v4l2_field "V4L.v4l2_field") |
    | --- | --- |
    ...
    The color space flags were replaced by a [`enum v4l2_colorspace`](colorspaces-defs.html#c.V4L.v4l2_colorspace "V4L.v4l2_colorspace") value in
...
12. In [`struct v4l2_requestbuffers`](vidioc-reqbufs.html#c.V4L.v4l2_requestbuffers "V4L.v4l2_requestbuffers") the
    `type` field was properly defined as [`enum v4l2_buf_type`](buffer.html#c.V4L.v4l2_buf_type "V4L.v4l2_buf_type"). Buffer types
    changed as mentioned above. A new `memory` field of type
    [`enum v4l2_memory`](buffer.html#c.V4L.v4l2_memory "V4L.v4l2_memory") was added to distinguish between
    I/O methods using buffers allocated by the driver or the
    application. See [Input/Output](io.html#io) for details.
13. In [`struct v4l2_buffer`](buffer.html#c.V4L.v4l2_buffer "V4L.v4l2_buffer") the `type` field was
    properly defined as [`enum v4l2_buf_type`](buffer.html#c.V4L.v4l2_buf_type "V4L.v4l2_buf_type").
    Buffer types changed as mentioned above. A `field` field of type
    [`enum v4l2_field`](field-order.html#c.V4L.v4l2_field "V4L.v4l2_field") was added to indicate if a
    ...
    stamp\_t, an unsigned 64 bit integer expressing the sample time in
    nanoseconds, to `struct timeval`. With the addition
    of a second memory mapping method the `offset` field moved into
    union `m`, and a new `memory` field of type [`enum v4l2_memory`](buffer.html#c.V4L.v4l2_memory "V4L.v4l2_memory")
    was added to distinguish between
    I/O methods. See [Input/Output](io.html#io) for details.
...
14. In [`struct v4l2_framebuffer`](vidioc-g-fbuf.html#c.V4L.v4l2_framebuffer "V4L.v4l2_framebuffer") the
...
15. In [`struct v4l2_clip`](dev-overlay.html#c.V4L.v4l2_clip "V4L.v4l2_clip") the `x`, `y`,
    `width` and `height` field moved into a `c` substructure of
    type [`struct v4l2_rect`](dev-overlay.html#c.V4L.v4l2_rect "V4L.v4l2_rect"). The `x` and `y`
    fields were renamed to `left` and `top`, i. e. offsets to a
    context dependent origin.
16. In [`struct v4l2_window`](dev-overlay.html#c.V4L.v4l2_window "V4L.v4l2_window") the `x`, `y`,
    `width` and `height` field moved into a `w` substructure as
    above. A `field` field of type [`enum v4l2_field`](field-order.html#c.V4L.v4l2_field "V4L.v4l2_field") was added to
...
17. The digital zoom interface, including struct `v4l2_zoomcap`,
    ...
    [`struct v4l2_cropcap`](vidioc-cropcap.html#c.V4L.v4l2_cropcap "V4L.v4l2_cropcap") and [`struct v4l2_crop`](vidioc-g-crop.html#c.V4L.v4l2_crop "V4L.v4l2_crop")
    where redefined for this purpose. See [Image Cropping, Insertion and Scaling -- the CROP API](crop.html#crop) for details.
18. In [`struct v4l2_vbi_format`](dev-raw-vbi.html#c.V4L.v4l2_vbi_format "V4L.v4l2_vbi_format") the
    ...
    extended.
19. In [`struct v4l2_captureparm`](vidioc-g-parm.html#c.V4L.v4l2_captureparm "V4L.v4l2_captureparm") the type of
    the `timeperframe` field changed from unsigned long to
    [`struct v4l2_fract`](vidioc-enumstd.html#c.V4L.v4l2_fract "V4L.v4l2_fract"). This allows the accurate
    ...
    Similar changes were made to [`struct v4l2_outputparm`](vidioc-g-parm.html#c.V4L.v4l2_outputparm "V4L.v4l2_outputparm").
20. The struct `v4l2_performance` and
    `VIDIOC_G_PERF` ioctl were dropped. Except when using the
    [read/write I/O method](rw.html#rw), which is limited anyway, this
...
21. The example transformation from RGB to YCbCr color space in the old
    ...
    [Image Formats](pixfmt.html#pixfmt).
...
2. An optional driver access priority mechanism was added, see
   [Application Priority](app-pri.html#app-pri) for details.
3. The audio input and output interface was found to be incomplete.

   Previously the [VIDIOC\_G\_AUDIO](vidioc-g-audio.html#vidioc-g-audio) ioctl would
   ...
   [ioctl VIDIOC\_ENUMAUDIO](vidioc-enumaudio.html#vidioc-enumaudio) ioctl was added to
   enumerate audio inputs, while
   [VIDIOC\_G\_AUDIO](vidioc-g-audio.html#vidioc-g-audio) now reports the current
   audio input.

   The same changes were made to
   [VIDIOC\_G\_AUDOUT](vidioc-g-audioout.html#vidioc-g-audout) and
   [VIDIOC\_ENUMAUDOUT](vidioc-enumaudioout.html#vidioc-enumaudout).
...
4. The [ioctl VIDIOC\_OVERLAY](vidioc-overlay.html#vidioc-overlay) ioctl was incorrectly
...
5. [Video Overlay Interface](dev-overlay.html#overlay) incorrectly stated that clipping rectangles define
...
6. The [VIDIOC\_S\_PARM](vidioc-g-parm.html#vidioc-g-parm) and
   [VIDIOC\_S\_CTRL](vidioc-g-ctrl.html#vidioc-g-ctrl) ioctls were defined with
...
1. In [RGB Formats](pixfmt-rgb.html#pixfmt-rgb) the following pixel formats were incorrectly
   ...
   In [Image Properties](diff-v4l.html#v4l-image-properties) the mapping of the V4L
...
2. Unrelated to the fixes above, drivers may still interpret some V4L2
   ...
   for details see [RGB Formats](pixfmt-rgb.html#pixfmt-rgb).

## 6.2.12. V4L2 in Linux 2.6.6, 2004-05-09

1. The [ioctl VIDIOC\_CROPCAP](vidioc-cropcap.html#vidioc-cropcap) ioctl was incorrectly
...
1. A new field `input` (former `reserved[0]`) was added to the
   [`struct v4l2_buffer`](buffer.html#c.V4L.v4l2_buffer "V4L.v4l2_buffer"). Purpose of this
...
1. The return value of the [V4L2 open()](func-open.html#func-open) function was incorrectly
   documented.
2. Audio output ioctls end in -AUDOUT, not -AUDIOOUT.
3. In the Current Audio Input example the `VIDIOC_G_AUDIO` ioctl took
   the wrong argument.
4. The documentation of the [ioctl VIDIOC\_QBUF, VIDIOC\_DQBUF](vidioc-qbuf.html#vidioc-qbuf) and
   [VIDIOC\_DQBUF](vidioc-qbuf.html#vidioc-qbuf) ioctls did not mention the
   [`struct v4l2_buffer`](buffer.html#c.V4L.v4l2_buffer "V4L.v4l2_buffer") `memory` field. It was
...
1. A new sliced VBI interface was added. It is documented in
   [Sliced VBI Data Interface](dev-sliced-vbi.html#sliced) and replaces the interface first proposed in V4L2
   specification 0.8.

## 6.2.16. V4L2 in Linux 2.6.15

1. The [ioctl VIDIOC\_LOG\_STATUS](vidioc-log-status.html#vidioc-log-status) ioctl was added.
2. New video standards `V4L2_STD_NTSC_443`, `V4L2_STD_SECAM_LC`,
   ...
   [typedef v4l2\_std\_id](vidioc-enumstd.html#v4l2-std-id).
...
The capture example in [Video Capture Example](capture-example.html#capture-example) called the
[VIDIOC\_S\_CROP](vidioc-g-crop.html#vidioc-g-crop) ioctl without checking if
cropping is supported. In the video standard selection example in
[Video Standards](standard.html#standard) the [VIDIOC\_S\_STD](vidioc-g-std.html#vidioc-g-std) call used
the wrong argument type.

## 6.2.18. V4L2 spec erratum 2006-01-10

1. The `V4L2_IN_ST_COLOR_KILL` flag in [`struct v4l2_input`](vidioc-enuminput.html#c.V4L.v4l2_input "V4L.v4l2_input") not only
...
2. [VIDIOC\_S\_PARM](vidioc-g-parm.html#vidioc-g-parm) is a write-read ioctl, not
   write-only as stated on its reference page. The ioctl changed in 2003
   as noted above.

## 6.2.19. V4L2 spec erratum 2006-02-03

1. In [`struct v4l2_captureparm`](vidioc-g-parm.html#c.V4L.v4l2_captureparm "V4L.v4l2_captureparm") and [`struct v4l2_outputparm`](vidioc-g-parm.html#c.V4L.v4l2_outputparm "V4L.v4l2_outputparm") the `timeperframe`
   field gives the time in seconds, not microseconds.

## 6.2.20. V4L2 spec erratum 2006-02-04

1. The `clips` field in [`struct v4l2_window`](dev-overlay.html#c.V4L.v4l2_window "V4L.v4l2_window")
   must point to an array of [`struct v4l2_clip`](dev-overlay.html#c.V4L.v4l2_clip "V4L.v4l2_clip"), not
   a linked list, because drivers ignore the
   [`struct v4l2_clip`](dev-overlay.html#c.V4L.v4l2_clip "V4L.v4l2_clip"). `next` pointer.
...
2. A new `V4L2_TUNER_MODE_LANG1_LANG2` was defined to record both
   ...
   the [VIDIOC\_G\_TUNER](vidioc-g-tuner.html#vidioc-g-tuner) section for details.
...
2. In [VIDIOC\_G\_AUDIO](vidioc-g-audio.html#vidioc-g-audio) it was clarified that the
   [`struct v4l2_audio`](vidioc-g-audio.html#c.V4L.v4l2_audio "V4L.v4l2_audio") `mode` field is a flags field.
3. [ioctl VIDIOC\_QUERYCAP](vidioc-querycap.html#vidioc-querycap) did not mention the sliced VBI and radio
   capability flags.
4. In [VIDIOC\_G\_FREQUENCY](vidioc-g-frequency.html#vidioc-g-frequency) it was clarified that
   applications must initialize the tuner `type` field of
   [`struct v4l2_frequency`](vidioc-g-frequency.html#c.V4L.v4l2_frequency "V4L.v4l2_frequency") before calling
   [VIDIOC\_S\_FREQUENCY](vidioc-g-frequency.html#vidioc-g-frequency).
5. The `reserved` array in [`struct v4l2_requestbuffers`](vidioc-reqbufs.html#c.V4L.v4l2_requestbuffers "V4L.v4l2_requestbuffers") has 2 elements,
   not 32.
6. In [Video Output Interface](dev-output.html#output) and [Raw VBI Data Interface](dev-raw-vbi.html#raw-vbi) the device file names
...
1. New ioctls [VIDIOC\_G\_EXT\_CTRLS](vidioc-g-ext-ctrls.html#vidioc-g-ext-ctrls),
   [VIDIOC\_S\_EXT\_CTRLS](vidioc-g-ext-ctrls.html#vidioc-g-ext-ctrls) and
   [VIDIOC\_TRY\_EXT\_CTRLS](vidioc-g-ext-ctrls.html#vidioc-g-ext-ctrls) were added, a
   flag to skip unsupported controls with
   [ioctls VIDIOC\_QUERYCTRL, VIDIOC\_QUERY\_EXT\_CTRL and VIDIOC\_QUERYMENU](vidioc-queryctrl.html#vidioc-queryctrl), new control types
   `V4L2_CTRL_TYPE_INTEGER64` and `V4L2_CTRL_TYPE_CTRL_CLASS`
   ([`enum v4l2_ctrl_type`](vidioc-queryctrl.html#c.V4L.v4l2_ctrl_type "V4L.v4l2_ctrl_type")), and new control flags
   `V4L2_CTRL_FLAG_READ_ONLY`, `V4L2_CTRL_FLAG_UPDATE`,
   `V4L2_CTRL_FLAG_INACTIVE` and `V4L2_CTRL_FLAG_SLIDER`
   ([Control Flags](vidioc-queryctrl.html#control-flags)). See [Extended Controls API](extended-controls.html#extended-controls) for details.

## 6.2.24. V4L2 in Linux 2.6.19

1. In [`struct v4l2_sliced_vbi_cap`](vidioc-g-sliced-vbi-cap.html#c.V4L.v4l2_sliced_vbi_cap "V4L.v4l2_sliced_vbi_cap") a
   ...
   [VIDIOC\_G\_SLICED\_VBI\_CAP](vidioc-g-sliced-vbi-cap.html#vidioc-g-sliced-vbi-cap) ioctl
   ...
   applications.
2. The ioctls [ioctl VIDIOC\_ENUM\_FRAMESIZES](vidioc-enum-framesizes.html#vidioc-enum-framesizes)
   and
   [ioctl VIDIOC\_ENUM\_FRAMEINTERVALS](vidioc-enum-frameintervals.html#vidioc-enum-frameintervals)
   were added.
3. A new pixel format `V4L2_PIX_FMT_RGB444` ([RGB Formats](pixfmt-rgb.html#pixfmt-rgb)) was
   added.

## 6.2.25. V4L2 spec erratum 2006-10-12 (Draft 0.17)

1. `V4L2_PIX_FMT_HM12` ([Reserved Image Formats](pixfmt-reserved.html#reserved-formats)) is a YUV 4:2:0, not
   4:2:2 format.
...
1. Two new field orders `V4L2_FIELD_INTERLACED_TB` and
   `V4L2_FIELD_INTERLACED_BT` were added. See [`enum v4l2_field`](field-order.html#c.V4L.v4l2_field "V4L.v4l2_field") for
   details.
2. Three new clipping/blending methods with a global or straight or
   ...
   See the description of the [VIDIOC\_G\_FBUF](vidioc-g-fbuf.html#vidioc-g-fbuf)
   and [VIDIOC\_S\_FBUF](vidioc-g-fbuf.html#vidioc-g-fbuf) ioctls for details.

   A new `global_alpha` field was added to [`struct v4l2_window`](dev-overlay.html#c.V4L.v4l2_window "V4L.v4l2_window"),
   extending the structure. This may **break compatibility** with
   applications using a [`struct v4l2_window`](dev-overlay.html#c.V4L.v4l2_window "V4L.v4l2_window") directly. However the
   [VIDIOC\_G/S/TRY\_FMT](vidioc-g-fmt.html#vidioc-g-fmt) ioctls, which take a
   pointer to a [`struct v4l2_format`](vidioc-g-fmt.html#c.V4L.v4l2_format "V4L.v4l2_format") parent structure
   with padding bytes at the end, are not affected.
3. The format of the `chromakey` field in [`struct v4l2_window`](dev-overlay.html#c.V4L.v4l2_window "V4L.v4l2_window") changed from
...
1. The pixel formats [V4L2\_PIX\_FMT\_Y16](pixfmt-yuv-luma.html#v4l2-pix-fmt-y16) and
   [V4L2\_PIX\_FMT\_SBGGR16](pixfmt-srggb16.html#v4l2-pix-fmt-sbggr16) were added.
2. New [controls](control.html#control) `V4L2_CID_POWER_LINE_FREQUENCY`,
...
3. A [Camera controls class](ext-ctrls-camera.html#camera-controls) was added, with
...
4. The `VIDIOC_G_MPEGCOMP` and `VIDIOC_S_MPEGCOMP` ioctls, which
   were superseded by the [extended controls](extended-controls.html#extended-controls)
...
1. The [ioctl VIDIOC\_S\_HW\_FREQ\_SEEK](vidioc-s-hw-freq-seek.html#vidioc-s-hw-freq-seek) ioctl
...
2. Finalized the RDS capture API. See [RDS Interface](dev-rds.html#rds) for more information.
...
1. Added `V4L2_CID_IRIS_ABSOLUTE` and `V4L2_CID_IRIS_RELATIVE`
   controls to the [Camera controls class](ext-ctrls-camera.html#camera-controls).
...
1. The old VIDIOC\_\*\_OLD symbols and V4L1 support were removed.
2. Multi-planar API added. Does not affect the compatibility of current
   drivers and applications. See [multi-planar API](planar-apis.html#planar-apis)
   for details.

## 6.2.40. V4L2 in Linux 3.1

1. VIDIOC\_QUERYCAP now returns a per-subsystem version instead of a
   per-driver one.

   Standardize an error code for invalid ioctl.

   Added V4L2\_CTRL\_TYPE\_BITMASK.

## 6.2.41. V4L2 in Linux 3.2

1. V4L2\_CTRL\_FLAG\_VOLATILE was added to signal volatile controls to
   userspace.
2. Add selection API for extended control over cropping and composing.
   ...
   applications. See [selection API](selection-api.html#selection-api) for details.

## 6.2.42. V4L2 in Linux 3.3

1. Added `V4L2_CID_ALPHA_COMPONENT` control to the
   [User controls class](control.html#control).
2. Added the device\_caps field to `struct v4l2_capabilities` and added
   the new V4L2\_CAP\_DEVICE\_CAPS capability.

## 6.2.43. V4L2 in Linux 3.4

1. Added [JPEG compression control class](ext-ctrls-jpeg.html#jpeg-controls).
2. Extended the DV Timings API:
   [ioctl VIDIOC\_ENUM\_DV\_TIMINGS, VIDIOC\_SUBDEV\_ENUM\_DV\_TIMINGS](vidioc-enum-dv-timings.html#vidioc-enum-dv-timings),
   [ioctl VIDIOC\_QUERY\_DV\_TIMINGS](vidioc-query-dv-timings.html#vidioc-query-dv-timings) and
   [ioctl VIDIOC\_DV\_TIMINGS\_CAP, VIDIOC\_SUBDEV\_DV\_TIMINGS\_CAP](vidioc-dv-timings-cap.html#vidioc-dv-timings-cap).
...
2. Added selection API for V4L2 subdev interface:
   [ioctl VIDIOC\_SUBDEV\_G\_SELECTION, VIDIOC\_SUBDEV\_S\_SELECTION](vidioc-subdev-g-selection.html#vidioc-subdev-g-selection) and
   [VIDIOC\_SUBDEV\_S\_SELECTION](vidioc-subdev-g-selection.html#vidioc-subdev-g-selection).
...
1. Replaced `input` in [`struct v4l2_buffer`](buffer.html#c.V4L.v4l2_buffer "V4L.v4l2_buffer") by
   `reserved2` and removed `V4L2_BUF_FLAG_INPUT`.
2. Added V4L2\_CAP\_VIDEO\_M2M and V4L2\_CAP\_VIDEO\_M2M\_MPLANE
   capabilities.
3. Added support for frequency band enumerations:
   [ioctl VIDIOC\_ENUM\_FREQ\_BANDS](vidioc-enum-freq-bands.html#vidioc-enum-freq-bands).

## 6.2.46. V4L2 in Linux 3.9

1. Added timestamp types to `flags` field in
   [`struct v4l2_buffer`](buffer.html#c.V4L.v4l2_buffer "V4L.v4l2_buffer"). See [Buffer Flags](buffer.html#buffer-flags).
2. Added `V4L2_EVENT_CTRL_CH_RANGE` control event changes flag. See
   [Control Changes](vidioc-dqevent.html#ctrl-changes-flags).

## 6.2.47. V4L2 in Linux 3.10

1. Removed obsolete and unused DV\_PRESET ioctls VIDIOC\_G\_DV\_PRESET,
...
2. Added new debugging ioctl
   [ioctl VIDIOC\_DBG\_G\_CHIP\_INFO](vidioc-dbg-g-chip-info.html#vidioc-dbg-g-chip-info).
...
1. In [`struct v4l2_rect`](dev-overlay.html#c.V4L.v4l2_rect "V4L.v4l2_rect"), the type of `width` and
...
1. Added event V4L2\_EVENT\_SOURCE\_CHANGE.

## 6.2.52. V4L2 in Linux 3.17

1. Extended [`struct v4l2_pix_format`](pixfmt-v4l2.html#c.V4L.v4l2_pix_format "V4L.v4l2_pix_format"). Added
   format flags.
2. Added compound control types and
   [VIDIOC\_QUERY\_EXT\_CTRL](vidioc-queryctrl.html#vidioc-queryctrl).
...
1. Rewrote Colorspace chapter, added new [`enum v4l2_ycbcr_encoding`](colorspaces-defs.html#c.V4L.v4l2_ycbcr_encoding "V4L.v4l2_ycbcr_encoding")
   and [`enum v4l2_quantization`](colorspaces-defs.html#c.V4L.v4l2_quantization "V4L.v4l2_quantization") fields to [`struct v4l2_pix_format`](pixfmt-v4l2.html#c.V4L.v4l2_pix_format "V4L.v4l2_pix_format"),
   [`struct v4l2_pix_format_mplane`](pixfmt-v4l2-mplane.html#c.V4L.v4l2_pix_format_mplane "V4L.v4l2_pix_format_mplane") and [`struct v4l2_mbus_framefmt`](subdev-formats.html#c.V4L.v4l2_mbus_framefmt "V4L.v4l2_mbus_framefmt").
...
in XPixmaps [[1]](#f1). With their implementation XFree86 makes the extension
...
advantages over the V4L2 [video overlay interface](dev-overlay.html#overlay). The
...
and overlay are discussed in [Video Overlay Interface](dev-overlay.html#overlay) apply.
...
* [ioctl VIDIOC\_DBG\_G\_REGISTER, VIDIOC\_DBG\_S\_REGISTER](vidioc-dbg-g-register.html#vidioc-dbg-g-register) and
  [VIDIOC\_DBG\_S\_REGISTER](vidioc-dbg-g-register.html#vidioc-dbg-g-register) ioctls.
...
* `VIDIOC_G_MPEGCOMP` and `VIDIOC_S_MPEGCOMP` ioctls. Use Extended
  Controls, [Extended Controls API](extended-controls.html#extended-controls).
* VIDIOC\_G\_DV\_PRESET, VIDIOC\_S\_DV\_PRESET,
  VIDIOC\_ENUM\_DV\_PRESETS and VIDIOC\_QUERY\_DV\_PRESET ioctls. Use
  the DV Timings API ([Digital Video (DV) Timings](dv-timings.html#dv-timings)).
* `VIDIOC_SUBDEV_G_CROP` and `VIDIOC_SUBDEV_S_CROP` ioctls. Use
  `VIDIOC_SUBDEV_G_SELECTION` and `VIDIOC_SUBDEV_S_SELECTION`,
  [ioctl VIDIOC\_SUBDEV\_G\_SELECTION, VIDIOC\_SUBDEV\_S\_SELECTION](vidioc-subdev-g-selection.html#vidioc-subdev-g-selection).
