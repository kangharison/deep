...
> 출처(원문): https://docs.kernel.org/userspace-api/fwctl/fwctl-cxl.html
...
under the fwctl policy of FWCTL\_RPC\_CONFIGURATION.
...
must be FWCTL\_RPC\_DEBUG\_WRITE\_FULL. The effects for this level are
...
fwctl access policy must be FWCTL\_RPC\_DEBUG\_WRITE or higher.

## fwctl cxl User API

struct fwctl\_rpc\_cxl
:   ioctl(FWCTL\_RPC) input for CXL
...
struct fwctl\_rpc\_cxl\_out
:   ioctl(FWCTL\_RPC) output for CXL
...
First step for the app is to issue the ioctl(FWCTL\_CMD\_INFO). Successful
...
user space via ioctl(FWCTL\_RPC). A `struct fwctl_rpc_cxl` is pointed to
...
int devm\_cxl\_setup\_features(struct cxl\_dev\_state \*cxlds)
