...
> 출처(원문): https://docs.kernel.org/userspace-api/fwctl/pds_fwctl.html
...
auxiliary\_device named pds\_core.fwctl.N. The pds\_fwctl driver binds to
...
The pds\_fwctl driver has little knowledge of the firmware’s internals.
It only knows how to send commands through pds\_core’s message queue to the
...
When a connection is created the pds\_fwctl driver requests from the
...
together with the request data and sent through pds\_core’s message queue
