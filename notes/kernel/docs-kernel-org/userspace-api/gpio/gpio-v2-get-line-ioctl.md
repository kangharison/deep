...
> 출처(원문): https://docs.kernel.org/userspace-api/gpio/gpio-v2-get-line-ioctl.html
> 자동 미러링: docs.kernel.org · 정본은 출처 URL (영문 원문 자동 변환본)

---

# GPIO\_V2\_GET\_LINE\_IOCTL

## Name

GPIO\_V2\_GET\_LINE\_IOCTL - Request a line or lines from the kernel.
...
`request`
:   The [`line_request`](chardev.html#c.gpio_v2_line_request "gpio_v2_line_request") specifying the lines
...
[Line Request](chardev.html#gpio-v2-line-request).
...
as possible. e.g. [GPIO\_V2\_LINE\_GET\_VALUES\_IOCTL](gpio-v2-line-get-values-ioctl.html) will read all the
...
The [`debounce_period_us`](chardev.html#c.gpio_v2_line_attribute "gpio_v2_line_attribute") attribute may only
be applied to lines with `GPIO_V2_LINE_FLAG_INPUT` set. When set, debounce
applies to both the values returned by [GPIO\_V2\_LINE\_GET\_VALUES\_IOCTL](gpio-v2-line-get-values-ioctl.html) and
the edges returned by [GPIO\_V2\_LINE\_EVENT\_READ](gpio-v2-line-event-read.html). If not
...
In all cases, the configuration reported by [GPIO\_V2\_GET\_LINEINFO\_IOCTL](gpio-v2-get-lineinfo-ioctl.html)
...
On success 0 and the [`request.fd`](chardev.html#c.gpio_v2_line_request "gpio_v2_line_request") contains the
