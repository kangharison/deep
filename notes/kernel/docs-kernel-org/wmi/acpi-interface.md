...
> 출처(원문): https://docs.kernel.org/wmi/acpi-interface.html
...
| String | Converted into a `string` with a terminating NULL character to match the behavior the of the Windows driver. |
| Package | Each element inside the package is converted with alignment of the resulting data types being respected. Nested packages are not allowed. |
