...
> 출처(원문): https://docs.kernel.org/wmi/driver-development-guide.html
...
documented at [WMI Driver API](../driver-api/wmi.html). This document will serve
as an introductory guide for WMI driver writers using this API. It is supposed
to be a successor to the original LWN article [[1]](#id2) which deals with WMI drivers
...
[WMI embedded Binary MOF driver](devices/wmi-bmof.html).
...
The basic WMI driver is build around the [`struct wmi_driver`](../driver-api/wmi.html#c.wmi_driver "wmi_driver"), which is then bound
to matching WMI devices using a [`struct wmi_device_id`](../driver-api/basics.html#c.wmi_device_id "wmi_device_id") table:
...
[Devres - Managed Device Resource](../driver-api/driver-model/devres.html) for details.
...
[Device Driver Design Patterns](../driver-api/driver-model/design-patterns.html).
...
WMI drivers can call WMI device methods using [`wmidev_invoke_method()`](../driver-api/wmi.html#c.wmidev_invoke_method "wmidev_invoke_method"). For each WMI method
...
methods that do not return any values, [`wmidev_invoke_procedure()`](../driver-api/wmi.html#c.wmidev_invoke_procedure "wmidev_invoke_procedure") should be used instead.
...
the number of instances can be retrieved using [`wmidev_instance_count()`](../driver-api/wmi.html#c.wmidev_instance_count "wmidev_instance_count").

Take a look at drivers/platform/x86/intel/wmi/thunderbolt.c for an example WMI method driver.

## WMI data block drivers

WMI drivers can query WMI data blocks using [`wmidev_query_block()`](../driver-api/wmi.html#c.wmidev_query_block "wmidev_query_block"), the layout of the returned
buffer is again device-specific and described by the Binary MOF data. Some WMI data blocks are
also writeable and can be set using [`wmidev_set_block()`](../driver-api/wmi.html#c.wmidev_set_block "wmidev_set_block"). The number of data block instances can
again be retrieved using [`wmidev_instance_count()`](../driver-api/wmi.html#c.wmidev_instance_count "wmidev_instance_count").

Take a look at drivers/platform/x86/intel/wmi/sbl-fw-update.c for an example WMI data block driver.

## WMI event drivers

WMI drivers can receive WMI events via the `notify_new()` callback inside the [`struct wmi_driver`](../driver-api/wmi.html#c.wmi_driver "wmi_driver").
...
[`struct wmi_driver`](../driver-api/wmi.html#c.wmi_driver "wmi_driver"). Setting this field to 0 will thus enable the WMI driver to receive WMI events
...
WMI drivers can exchange data with the WMI driver core using [`struct wmi_buffer`](../driver-api/wmi.html#c.wmi_buffer "wmi_buffer"). The internal
...
such using `__le64` and friends. When parsing WMI string data items the [`struct wmi_string`](../driver-api/wmi.html#c.wmi_string "wmi_string") should
be used as WMI strings have a different layout than C strings.

See [ACPI WMI interface](acpi-interface.html) for more information regarding the binary format
of WMI data items.
...
[Linux kernel coding style](../process/coding-style.html). The checkpatch utility can catch many common coding style
