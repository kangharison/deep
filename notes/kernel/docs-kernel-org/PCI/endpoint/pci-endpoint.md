...
> 출처(원문): https://docs.kernel.org/PCI/endpoint/pci-endpoint.html
...
* `devm_pci_epc_create()`/`pci_epc_create()`
  ...
  >
  > > + write\_header: ops to populate configuration space header
  > > + set\_bar: ops to configure the BAR
  > > + clear\_bar: ops to reset the BAR
  > > + alloc\_addr\_space: ops to allocate in PCI controller address space
  > > + free\_addr\_space: ops to free the allocated address space
  > > + raise\_irq: ops to raise a legacy, MSI or MSI-X interrupt
...
* `pci_epc_mem_init()`
  ...
  > Initialize the pci\_epc\_mem structure used for allocating EPC addr space.
* `pci_epc_mem_exit()`

  > Cleanup the pci\_epc\_mem structure allocated during `pci_epc_mem_init()`.
...
* `pci_epc_set_bar()`
  ...
  > dynamic\_inbound\_mapping EPC feature bit.
  >
  > When pci\_epf\_bar.num\_submap is non-zero, the endpoint function driver is
  > requesting BAR subrange mapping using pci\_epf\_bar.submap. This requires
  > the EPC to advertise support via the subrange\_mapping EPC feature bit.
  >
  ...
  > twice for the same BAR (requires dynamic\_inbound\_mapping): first with
  > num\_submap set to zero and configuring the BAR size, then after the PCIe
  > link is up and the host enumerates the endpoint and programs the BAR
  > base address, again with num\_submap set to non-zero value.
  >
...
* `pci_epf_register_driver()`
  ...
  >     + add\_cfs: optional ops to create function specific configfs
  >       attributes
...
* `pci_epf_free_space()`
  ...
  (using pci\_epf\_alloc\_space) by invoking `pci_epf_free_space()`.
