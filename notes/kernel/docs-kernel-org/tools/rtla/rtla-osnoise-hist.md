...
> 출처(원문): https://docs.kernel.org/tools/rtla/rtla-osnoise-hist.html
...
The **rtla osnoise hist** tool collects all **osnoise:sample\_threshold**
...
> Set the osnoise hist tracer to run the sample threads in the cpu-list.
>
> By default, the osnoise hist tracer runs the sample threads on all CPUs.
...
> Set scheduling parameters to the osnoise hist tracer threads, the format to set the priority are:
>
...
> Example:
>
> $ rtla osnoise hist -s 20 --on-threshold trace
...
> Example:
>
> $ rtla osnoise hist -d 5s --on-end trace
>
