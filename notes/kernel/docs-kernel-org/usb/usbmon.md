...
> 출처(원문): https://docs.kernel.org/usb/usbmon.html
...
An output bulk transfer to send a SCSI command 0x28 (READ\_10) in a 31-byte
...
The following ioctl calls are available, with MON\_IOC\_MAGIC 0x92:

> MON\_IOCQ\_URB\_LEN, defined as \_IO(MON\_IOC\_MAGIC, 1)

This call returns the length of data in the next event. Note that majority of
events contain no data, so if this call returns zero, it does not mean that
no events are available.

> MON\_IOCG\_STATS, defined as \_IOR(MON\_IOC\_MAGIC, 3, `struct mon_bin_stats`)
...
> MON\_IOCT\_RING\_SIZE, defined as \_IO(MON\_IOC\_MAGIC, 4)
...
> MON\_IOCX\_GET, defined as \_IOW(MON\_IOC\_MAGIC, 6, `struct mon_get_arg`)
> MON\_IOCX\_GETX, defined as \_IOW(MON\_IOC\_MAGIC, 10, `struct mon_get_arg`)
...
The MON\_IOCX\_GET copies 48 bytes to hdr area, MON\_IOCX\_GETX copies 64 bytes.

> MON\_IOCX\_MFETCH, defined as \_IOWR(MON\_IOC\_MAGIC, 7, `struct mon_mfetch_arg`)
...
device is open with O\_NONBLOCK.
