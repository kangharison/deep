...
> 출처(원문): https://docs.kernel.org/usb/gadget_configfs.html
...
Configfs (please see [Configfs - Userspace-driven Kernel Object Configuration](../filesystems/configfs.html)) lends itself nicely
...
In order for this to work configfs must be available, so CONFIGFS\_FS must be
‘y’ or ‘m’ in .config. As of this writing USB\_LIBCOMPOSITE selects CONFIGFS\_FS.
...
appropriate.
Please refer to [ABI file testing/configfs-usb-gadget](../admin-guide/abi-testing-files.html#abi-file-testing-configfs-usb-gadget) for more information.
...
The filesystem part of configfs operates on config\_items/groups and
configfs\_attributes which are generic and of the same type for all
configured elements. However, they are embedded in usage-specific
larger structures. In the picture below there is a “cs” which contains
a config\_item and an “sa” which contains a configfs\_attribute.
...
known container\_of technique and an appropriate sa’s function (show or
...
[Configfs - Userspace-driven Kernel Object Configuration](../filesystems/configfs.html).
...
3. The file drivers/usb/gadget/configfs.c contains code for
   ...
   > * gadget’s config\_group
   > * gadget’s default groups (configs, functions, strings)
   > * associating functions with configurations (symlinks)
4. Each USB function naturally has its own view of what it wants configured, so
   config\_groups for particular functions are defined in the functions
   implementation files drivers/usb/gadget/f\_\*.c.
5. Function’s code is written in such a way that it uses
   `usb_get_function_instance()`, which, in turn, calls request\_module. So,
