...
> 출처(원문): https://docs.kernel.org/usb/gadget_uvc.html
...
The Kconfig options USB\_CONFIGFS, USB\_LIBCOMPOSITE, USB\_CONFIGFS\_F\_UVC and
USB\_F\_UVC must be selected to enable support for the UVC gadget.
...
see [ABI file testing/configfs-usb-gadget-uvc](../admin-guide/abi-testing-files.html#abi-file-testing-configfs-usb-gadget-uvc)

### Assumptions

This section assumes that you have mounted configfs at /sys/kernel/config and
created a gadget as /sys/kernel/config/usb\_gadget/g1.
...
[Packed YUV formats](../userspace-api/media/v4l/pixfmt-packed-yuv.html).
...
but a rule of thumb is that increasing the streaming\_maxpacket setting will
improve bandwidth (and thus the maximum possible framerate), whilst the same is
true for streaming\_maxburst provided the USB connection is running at SuperSpeed.
Increasing streaming\_interval will reduce bandwidth and framerate.
