...
> 출처(원문): https://docs.kernel.org/security/credentials.html
...
> 2. the reference count on the group\_info `struct it` points to may be changed;
...
has \_const\_ pointers to its credential sets, as does [`struct file`](../filesystems/api-summary.html#c.file "file"). Furthermore,
...
### [Accessing Task Credentials](#id6)
...
### [Accessing Another Task’s Credentials](#id7)
...
this locks current->cred\_replace\_mutex and then allocates and constructs a
...
The caller should \_not\_ call `put_cred()` on the new credentials afterwards.
...
## [Overriding the VFS’s Use of Credentials](#id11)
