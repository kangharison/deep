...
> 출처(원문): https://docs.kernel.org/security/secrets/coco.html
...
from the firmware to the operating system, in the EFI driver and the efi\_secret
kernel module.
...
The efi\_secret kernel module allows userspace applications to access these
...
`KVM_SEV_LAUNCH_SECRET` command (see [[sev]](#sev)). The structure of the injected
...
the EFI driver will autoload the efi\_secret kernel module, which exposes the
secrets to userspace applications via securityfs. The details of the
efi\_secret filesystem interface are in [[secrets-coco-abi]](#secrets-coco-abi).
...
The guest application reads the secret from the efi\_secret filesystem and
...
Here is a simple example for usage of the efi\_secret module in a guest
...
See [[sev-api-spec]](#sev-api-spec) for more info regarding SEV `LAUNCH_SECRET` operation.

[[sev](#id1)]

[Secure Encrypted Virtualization (SEV)](../../virt/kvm/x86/amd-memory-encryption.html)

[[secrets-coco-abi](#id2)]

[ABI file testing/securityfs-secrets-coco](../../admin-guide/abi-testing-files.html#abi-file-testing-securityfs-secrets-coco)
