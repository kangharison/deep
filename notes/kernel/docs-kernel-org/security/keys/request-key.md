...
> 출처(원문): https://docs.kernel.org/security/keys/request-key.html
...
[Kernel Key Retention Service](core.html)). This document explains more fully how
...
> 1. When the key management code searches for a key (keyring\_search\_rcu) it
...
When `request_key()` is invoked, if CONFIG\_KEYS\_REQUEST\_CACHE=y, a per-task
...
returned. If CONFIG\_KEYS\_REQUEST\_CACHE=y, then that key is placed in the
