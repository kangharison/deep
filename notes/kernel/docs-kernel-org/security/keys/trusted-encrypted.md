...
> 출처(원문): https://docs.kernel.org/security/keys/trusted-encrypted.html
...
>      the OTP key can be done via a module parameter (dcp\_use\_otp\_key).
...
>   CAAM HWRNG, enable CRYPTO\_DEV\_FSL\_CAAM\_RNG\_API and ensure the device
>   is probed.
...
time with the TrouSerS utility: “tpm\_takeownership -u -z”.
...
pubkey is the binary representation of TPM2B\_PRIVATE excluding the
initial TPM2B header, which can be reconstructed from the ASN.1 octet
string length.

privkey is the binary representation of TPM2B\_PUBLIC excluding the
...
:   Random AES 128 key which is used to encrypt **payload**,
    **blob\_key** itself is encrypted with OTP or UNIQUE device key in
...
`payload`
:   The payload itself, encrypted using AES-128-GCM and **blob\_key**,
    GCM auth tag of size DCP\_BLOB\_AUTHLEN is attached at the end of it.

**Description**

The total size of a DCP BLOB is sizeof([`struct dcp_blob_fmt`](#c.dcp_blob_fmt "dcp_blob_fmt")) + **payload\_len** +
DCP\_BLOB\_AUTHLEN.
