...
> 출처(원문): https://docs.kernel.org/security/keys/core.html
...
## [Userspace System Call Interface](#id6)
...
>   permission.
>
>   See also [Key Request Service](request-key.html).
...
>   “from\_ring\_id” to the keyring specified by “to\_ring\_id”. If the two
>   keyrings are the same, nothing is done.
>
>   “flags” can have KEYCTL\_MOVE\_EXCL set in it to cause the operation to fail
...
>   occur from KEYCTL\_LINK also apply on the destination keyring here.
...
>   See [Asymmetric / Public-key Cryptography Key Type](../../crypto/asymmetric-keys.html) for specific restrictions
...
>   is returned in the keyctl\_pkey\_query struct:
>
>   ```
>   __u32   supported_ops;
>   __u32   key_size;
...
>
>   “queue\_fd” is a file descriptor referring to an open pipe which
>   manages the buffer into which notifications will be delivered.
>
>   “filter” is either NULL to remove a watch or a filter specification to
>   indicate what events are required from the key.
>
>   See [General notification mechanism](../../core-api/watch_queue.html) for more information.
>
>   Note that only one watch may be emplaced for any particular { key,
>   queue\_fd } combination.
>
...
>   ```
>
>   In this, watch::type will be “WATCH\_TYPE\_KEY\_NOTIFY” and subtype will be
>   one of:
>
>   ```
...
>   restriction).
>
>   If a watched key is deleted, a basic watch\_notification will be issued
>   with “type” set to WATCH\_TYPE\_META and “subtype” set to
>   watch\_meta\_removal\_notification. The watchpoint ID will be set in the
>   “info” field.
>
...
>   tag. The domain\_tag may be NULL, specifying a global domain that is
...
>   passed to the key\_type->`request_key()` op if it exists, and the
>   callout\_info is a blob of length callout\_len, if given (the length may be
>   0).
...
>
>   In the case of `key_get()`, if the pointer is NULL or CONFIG\_KEYS is not set
...
>   upon failure (use IS\_ERR/PTR\_ERR to determine). If successful, the returned
...
## [Notes On Accessing Payload Contents](#id8)
...
>    use of [`call_rcu()`](../../core-api/kernel-api.html#c.call_rcu "call_rcu") and, if the payload is of variable size, the length of
>    the payload. key->datalen cannot be relied upon to be consistent with the
>    payload just dereferenced if the key’s semaphore is not held.
>
>    Note that key->payload.data[0] has a shadow that is marked for \_\_rcu
>    usage. This is called key->payload.rcu\_data0. The following accessors
...
>   keytype->def\_datalen, then `key_payload_reserve()` should be called.
>
>   This method does not have to lock the key in order to attach a payload.
>   The fact that KEY\_FLAG\_INSTANTIATED is not set in key->flags prevents
...
>   be made under RCU conditions, and [`call_rcu()`](../../core-api/kernel-api.html#c.call_rcu "call_rcu") must be used to dispose of
>   the old payload.
>
...
>   match\_data->preparsed after a successful call to `match_preparse()`.
...
>
>   This method will be called with the RCU read lock held. [`rcu_dereference()`](../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference")
...
>   caller of request\_key\*(). `complete_request_key()` must be called prior to
>   returning.
>
...
>   digest algorithm - the name of which should be supplied in hash\_algo.
>
...
>   to generate the raw data may need to be indicated in hash\_algo.
>
...
>
>   The parameter block is as for `asym_eds_op()` and co. but in\_len and out\_len
>   are unused. The encoding and hash\_algo fields should be used to reduce
...
>   ```
>
>   The supported\_ops field will contain a bitmask indicating what operations
...
>   ```
>
>   The key\_size field is the size of the key in bits. max\_data\_size and
>   max\_sig\_size are the maximum raw data and signature sizes for creation and
>   verification of a signature; max\_enc\_size and max\_dec\_size are the maximum
>   raw data and signature sizes for encryption and decryption. The
>   max\_\*\_size fields are measured in bytes.
>
>   If successful, 0 will be returned. If the key doesn’t support this,
>   EOPNOTSUPP will be returned.

## [Request-Key Callback Service](#id10)
...
service. This will be passed as the <callout\_info> parameter. If no such
