...
> 출처(원문): https://docs.kernel.org/security/SCTP.html
...
described in the [SCTP SELinux Support](#sctp-selinux-support) chapter.
...
then the following *\_PARAM\_*’s are sent to the peer in an
...
The [SCTP LSM Support](#sctp-lsm-support) chapter above describes the following SCTP security
...
**sctp\_peeloff**(3). `security_sctp_sk_clone()` will set the new
...
:   1. If peer labeling is not enabled, then the peer context will always be
    ...
    7. IPSEC is not supported as RFC 3554 - sctp/ipsec support has not been
       implemented in userspace (**racoon**(8) or **ipsec\_pluto**(8)),
