...
> 출처(원문): https://docs.kernel.org/security/lsm.html
...
optional, requiring CONFIG\_SECURITY to be enabled. The capabilities
logic is implemented as a security module.
This capabilities module is discussed further in
[LSM Capabilities Module](#lsm-capabilities-module).
...
information, security fields are included in `struct inode` and [`struct file`](../filesystems/api-summary.html#c.file "file").
...
[`struct sk_buff`](../networking/kapi.html#c.sk_buff "sk_buff") and
`struct scm_cookie`.
...
hook, and the hooks are called in the order specified by CONFIG\_LSM.
...
of hooks include the [`security_inode_alloc()`](../core-api/kernel-api.html#c.security_inode_alloc "security_inode_alloc") and [`security_inode_free()`](../core-api/kernel-api.html#c.security_inode_free "security_inode_free")
These hooks are used to allocate
and free security structures for inode objects.
An example of the second category of hooks
is the [`security_inode_permission()`](../core-api/kernel-api.html#c.security_inode_permission "security_inode_permission") hook.
