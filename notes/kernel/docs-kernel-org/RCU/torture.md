...
> 출처(원문): https://docs.kernel.org/RCU/torture.html
...
The CONFIG\_RCU\_TORTURE\_TEST config option is available for all RCU
implementations. It creates an rcutorture kernel module that can
be loaded to run a torture test. The test periodically outputs
status messages via [`printk()`](../core-api/printk-basics.html#c.printk "printk"), which can be examined via the dmesg
...
use other commands to access the output of the [`printk()`](../core-api/printk-basics.html#c.printk "printk")s used by
the RCU torture test. The [`printk()`](../core-api/printk-basics.html#c.printk "printk")s use KERN\_ALERT, so they should
be evident. ;-)
...
* “rtmbe”: A non-zero value indicates that rcutorture believes that
  [`rcu_assign_pointer()`](../core-api/kernel-api.html#c.rcu_assign_pointer "rcu_assign_pointer") and [`rcu_dereference()`](../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") are not working
  correctly. This value should be zero.
* “rtbe”: A non-zero value indicates that one of the [`rcu_barrier()`](../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier")
...
* “rtb”: The number of times that rcutorture attempted to force
  ...
  priority boosting via the “test\_boost” module parameter, this
...
* “Reader Pipe”: Histogram of “ages” of structures seen by readers.
  ...
  after passing through (RCU\_TORTURE\_PIPE\_LEN-2) grace periods.
...
using a dynamically allocated srcu\_struct (hence “srcud-” rather than
...
In that case, the kernel should be built with CONFIG\_RCU\_TORTURE\_TEST=m
...
“FAILURE”, or “RCU\_HOTPLUG” indication to be [`printk()`](../core-api/printk-basics.html#c.printk "printk")ed. The first
...
CPU stall-warning code, use “--bootargs ‘rcutorture.stall\_cpu=30’”.
