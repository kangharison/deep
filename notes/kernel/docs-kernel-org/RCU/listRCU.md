...
> 출처(원문): https://docs.kernel.org/RCU/listRCU.html
...
When iterating a list while holding the [`rcu_read_lock()`](../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock"), writers may
modify the list. The reader is guaranteed to see all of the elements
which were added to the list before they acquired the [`rcu_read_lock()`](../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock")
and are still on the list when they drop the [`rcu_read_unlock()`](../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock").
Elements which are added to, or removed from the list may or may not
be seen. If the writer calls [`list_replace_rcu()`](../core-api/kernel-api.html#c.list_replace_rcu "list_replace_rcu"), the reader may see
...
writer lock protection. The [`list_del_rcu()`](../core-api/kernel-api.html#c.list_del_rcu "list_del_rcu") invocation removes
...
grace periods elapse, with the help of [`call_rcu()`](../core-api/kernel-api.html#c.call_rcu "call_rcu"), which is invoked via
...
The `read_lock()` and `read_unlock()` calls have become [`rcu_read_lock()`](../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock")
and [`rcu_read_unlock()`](../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock"), respectively, and the [`list_for_each_entry()`](../core-api/list.html#c.list_for_each_entry "list_for_each_entry")
has become [`list_for_each_entry_rcu()`](../core-api/kernel-api.html#c.list_for_each_entry_rcu "list_for_each_entry_rcu"). The **`_rcu()`** list-traversal
...
auditsc\_lock can therefore be eliminated, since use of RCU eliminates the
need for writers to exclude readers.

The [`list_del()`](../core-api/list.html#c.list_del "list_del"), [`list_add()`](../core-api/list.html#c.list_add "list_add"), and [`list_add_tail()`](../core-api/list.html#c.list_add_tail "list_add_tail") primitives have been
replaced by [`list_del_rcu()`](../core-api/kernel-api.html#c.list_del_rcu "list_del_rcu"), [`list_add_rcu()`](../core-api/kernel-api.html#c.list_add_rcu "list_add_rcu"), and [`list_add_tail_rcu()`](../core-api/kernel-api.html#c.list_add_tail_rcu "list_add_tail_rcu").
The **`_rcu()`** list-manipulation primitives add memory barriers that are
needed on weakly ordered CPUs. The [`list_del_rcu()`](../core-api/kernel-api.html#c.list_del_rcu "list_del_rcu") primitive omits the
...
to replace the old limit object with the new one using [`list_replace_rcu()`](../core-api/kernel-api.html#c.list_replace_rcu "list_replace_rcu").
The old limit object is then freed after a grace period using [`kfree_rcu()`](../core-api/kernel-api.html#c.kfree_rcu "kfree_rcu").
...
[Answer to Quick Quiz](#quick-quiz-answer)
...
while executing the [`list_replace_rcu()`](../core-api/kernel-api.html#c.list_replace_rcu "list_replace_rcu").
