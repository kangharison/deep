...
> 출처(원문): https://docs.kernel.org/RCU/Design/Requirements/Requirements.html
...
1. [Fundamental Requirements](#fundamental-requirements)
2. [Fundamental Non-Requirements](#fundamental-non-requirements)
3. [Parallelism Facts of Life](#parallelism-facts-of-life)
4. [Quality-of-Implementation Requirements](#quality-of-implementation-requirements)
5. [Linux Kernel Complications](#linux-kernel-complications)
6. [Software-Engineering Requirements](#software-engineering-requirements)
7. [Other RCU Flavors](#other-rcu-flavors)
8. [Possible Future Changes](#possible-future-changes)

This is followed by a [summary](#summary), however, the answers to
...
1. [Grace-Period Guarantee](#grace-period-guarantee)
2. [Publish/Subscribe Guarantee](#publish-subscribe-guarantee)
3. [Memory-Barrier Guarantees](#memory-barrier-guarantees)
4. [RCU Primitives Guaranteed to Execute Unconditionally](#rcu-primitives-guaranteed-to-execute-unconditionally)
5. [Guaranteed Read-to-Write Upgrade](#guaranteed-read-to-write-upgrade)
...
critical section begins with the marker [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") and ends
with the marker [`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock"). These markers may be nested, and
RCU treats a nested set as one big RCU read-side critical section.
Production-quality implementations of [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") and
[`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock") are extremely lightweight, and in fact have
...
Because the [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") on line 14 waits for all pre-existing
...
started after the [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") started, and must therefore also
...
| --- |
| **Quick Quiz**: |
| Wait a minute! You said that updaters can make useful forward progress concurrently with readers, but pre-existing readers will block [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu")!!! Just who are you trying to fool??? |
| **Answer**: |
| First, if updaters do not wish to be blocked by readers, they can use [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") or [`kfree_rcu()`](../../../core-api/kernel-api.html#c.kfree_rcu "kfree_rcu"), which will be discussed later. Second, even when using [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu"), the other update-side code does run concurrently with readers, whether pre-existing or not. |
...
the [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") in `start_recovery()` to guarantee that
...
| --- |
| **Quick Quiz**: |
| Why is the [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") on line 28 needed? |
| **Answer**: |
...
critical section must not contain calls to [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu").
...
[`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu").
...
[`rcu_assign_pointer()`](../../../core-api/kernel-api.html#c.rcu_assign_pointer "rcu_assign_pointer") to insert the new data, and readers use
[`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") to access data, whether new or old. The following
...
The [`rcu_assign_pointer()`](../../../core-api/kernel-api.html#c.rcu_assign_pointer "rcu_assign_pointer") on line 13 is conceptually equivalent to a
...
| --- |
| **Quick Quiz**: |
| But [`rcu_assign_pointer()`](../../../core-api/kernel-api.html#c.rcu_assign_pointer "rcu_assign_pointer") does nothing to prevent the two assignments to `p->a` and `p->b` from being reordered. Can’t that also cause problems? |
| **Answer**: |
...
`do_something_gp()` uses [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") to fetch from `gp`:
...
The [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") uses volatile casts and (for DEC Alpha) memory
...
ever appear, then [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") could be implemented as a
`memory_order_consume` load. Regardless of the exact implementation, a
pointer fetched by [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") may not be used outside of the
outermost RCU read-side critical section containing that
[`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference"), unless protection of the corresponding data
element has been passed from RCU to some other synchronization
mechanism, most commonly locking or reference counting
(see [Reference-count design for elements of lists/arrays protected by RCU](../../rcuref.html)).

In short, updaters use [`rcu_assign_pointer()`](../../../core-api/kernel-api.html#c.rcu_assign_pointer "rcu_assign_pointer") and readers use
[`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference"), and these two RCU API elements work together to
...
3. At this point, only the updater has a reference to the newly removed
   ...
   by passing it to [`kfree()`](../../../core-api/mm-api.html#c.kfree "kfree").
...
element referenced by `p` is freed. The [`rcu_access_pointer()`](../../../core-api/kernel-api.html#c.rcu_access_pointer "rcu_access_pointer") on
line 6 is similar to [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference"), except that:

1. The value returned by [`rcu_access_pointer()`](../../../core-api/kernel-api.html#c.rcu_access_pointer "rcu_access_pointer") cannot be
   dereferenced. If you want to access the value pointed to as well as
   the pointer itself, use [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") instead of
   [`rcu_access_pointer()`](../../../core-api/kernel-api.html#c.rcu_access_pointer "rcu_access_pointer").
2. The call to [`rcu_access_pointer()`](../../../core-api/kernel-api.html#c.rcu_access_pointer "rcu_access_pointer") need not be protected. In
   contrast, [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") must either be within an RCU
...
[`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") for subscription, nor did it have anything
resembling the dependency-ordering barrier that was later subsumed
into [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") and later still into `READ_ONCE()`. The
...
[`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference")!
...
1. Each CPU that has an RCU read-side critical section that begins
   before [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") starts is guaranteed to execute a full
   memory barrier between the time that the RCU read-side critical
   section ends and the time that [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") returns. Without
   ...
   [`kfree()`](../../../core-api/mm-api.html#c.kfree "kfree") on line 14 of `remove_gp_synchronous()`.
2. Each CPU that has an RCU read-side critical section that ends after
   [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") returns is guaranteed to execute a full memory
   barrier between the time that [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") begins and the
   ...
   [`kfree()`](../../../core-api/mm-api.html#c.kfree "kfree") on line 14 of `remove_gp_synchronous()` might later run
   `do_something_gp()` and find the newly deleted `struct foo`.
3. If the task invoking [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") remains on a given CPU,
   then that CPU is guaranteed to execute a full memory barrier sometime
   during the execution of [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu"). This guarantee ensures
   that the [`kfree()`](../../../core-api/mm-api.html#c.kfree "kfree") on line 14 of `remove_gp_synchronous()` really
   does execute after the removal on line 11.
4. If the task invoking [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") migrates among a group of
   ...
   execution of [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu"). This guarantee also ensures that
   the [`kfree()`](../../../core-api/mm-api.html#c.kfree "kfree") on line 14 of `remove_gp_synchronous()` really does
   execute after the removal on line 11, but also in the case where the
   thread executing the [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") migrates in the meantime.

|  |
| --- |
| **Quick Quiz**: |
| Given that multiple CPUs can start RCU read-side critical sections at any time without any ordering whatsoever, how can RCU possibly tell whether or not a given RCU read-side critical section starts before a given instance of [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu")? |
| **Answer**: |
| If RCU cannot tell whether or not a given RCU read-side critical section starts before a given instance of [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu"), then it must assume that the RCU read-side critical section started first. In other words, a given instance of [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") can avoid waiting on a given RCU read-side critical section only if it can prove that [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") started first. A related question is “When [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") doesn’t generate any code, why does it matter how it relates to a grace period?” The answer is that it is not the relationship of [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") itself that is important, but rather the relationship of the code within the enclosed RCU read-side critical section to the code preceding and following the grace period. If we take this viewpoint, then a given RCU read-side critical section begins before a given grace period when some access preceding the grace period observes the effect of some access within the critical section, in which case none of the accesses within the critical section may observe the effects of any access following the grace period.  As of late 2016, mathematical models of RCU take this viewpoint, for example, see slides 62 and 63 of the [2016 LinuxCon EU](http://www2.rdrop.com/users/paulmck/scalability/paper/LinuxMM.2016.10.04c.LCE.pdf) presentation. |
...
| **Answer**: |
| Yes, they really are required. To see why the first guarantee is required, consider the following sequence of events:   1. CPU 1: [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") 2. CPU 1: `q = rcu_dereference(gp); /* Very likely to return p. */` 3. CPU 0: `list_del_rcu(p);` 4. CPU 0: [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") starts. 5. CPU 1: `do_something_with(q->a);`    `/* No smp_mb(), so might happen after kfree(). */` 6. CPU 1: [`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock") 7. CPU 0: [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") returns. 8. CPU 0: `kfree(p);`   Therefore, there absolutely must be a full memory barrier between the end of the RCU read-side critical section and the end of the grace period.  The sequence of events demonstrating the necessity of the second rule is roughly similar:   1. CPU 0: `list_del_rcu(p);` 2. CPU 0: [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") starts. 3. CPU 1: [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") 4. CPU 1: `q = rcu_dereference(gp);`    `/* Might return p if no memory barrier. */` 5. CPU 0: [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") returns. 6. CPU 0: `kfree(p);` 7. CPU 1: `do_something_with(q->a); /* Boom!!! */` 8. CPU 1: [`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock")   And similarly, without a memory barrier between the beginning of the grace period and the beginning of the RCU read-side critical section, CPU 1 might end up accessing the freelist.  The “as if” rule of course applies, so that any implementation that acts as if the appropriate memory barriers were in place is a correct implementation. That said, it is much easier to fool yourself into believing that you have adhered to the as-if rule than it is to actually adhere to it! |

|  |
| --- |
| **Quick Quiz**: |
| You claim that [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") and [`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock") generate absolutely no code in some kernel builds. This means that the compiler might arbitrarily rearrange consecutive RCU read-side critical sections. Given such rearrangement, if a given RCU read-side critical section is done, how can you be sure that all prior RCU read-side critical sections are done? Won’t the compiler rearrangements make that impossible to determine? |
| **Answer**: |
| In cases where [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") and [`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock") generate absolutely no code, RCU infers quiescent states only at special locations, for example, within the scheduler. Because calls to `schedule()` had better prevent calling-code accesses to shared variables from being rearranged across the call to `schedule()`, if RCU detects the end of a given RCU read-side critical section, it will necessarily detect the end of all prior RCU read-side critical sections, no matter how aggressively the compiler scrambles the code. Again, this all assumes that the compiler cannot scramble code across calls to the scheduler, out of interrupt handlers, into the idle loop, into user-mode code, and so on. But if your kernel build allows that sort of scrambling, you have broken far more than just RCU! |
...
before invoking [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu"), however, this inconvenience can
be avoided through use of the [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") and [`kfree_rcu()`](../../../core-api/kernel-api.html#c.kfree_rcu "kfree_rcu") API
...
1. [Readers Impose Minimal Ordering](#readers-impose-minimal-ordering)
2. [Readers Do Not Exclude Updaters](#readers-do-not-exclude-updaters)
3. [Updaters Only Wait For Old Readers](#updaters-only-wait-for-old-readers)
4. [Grace Periods Don’t Partition Read-Side Critical Sections](#grace-periods-don-t-partition-read-side-critical-sections)
5. [Read-Side Critical Sections Don’t Partition Grace Periods](#read-side-critical-sections-don-t-partition-grace-periods)

### Readers Impose Minimal Ordering

Reader-side markers such as [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") and
[`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock") provide absolutely no ordering guarantees except
through their interaction with the grace-period APIs such as
[`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu"). To see this, consider the following pair of
threads:
...
not be possible if [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") and [`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock") had
...
Neither [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") nor [`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock") exclude updates.
...
If the `thread0()` function’s [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") excluded the
`thread1()` function’s update, the `WARN_ON()` could never fire. But
the fact is that [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") does not exclude much of anything
aside from subsequent grace periods, of which `thread1()` has none, so
the `WARN_ON()` can and does fire.

### Updaters Only Wait For Old Readers

It might be tempting to assume that after [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu")
completes, there are no readers executing. This temptation must be
avoided because new readers can start immediately after
[`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") starts, and [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") is under no
obligation to wait for these new readers.

|  |
| --- |
| **Quick Quiz**: |
| Suppose that [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") did wait until *all* readers had completed instead of waiting only on pre-existing readers. For how long would the updater be able to rely on there being no readers? |
| **Answer**: |
| For no time at all. Even if [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") were to wait until all readers had completed, a new reader might start immediately after [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") completed. Therefore, the code following [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") can *never* rely on there being no readers. |
...
3. [Forward Progress](#forward-progress)
...
| **Answer**: |
| These are forbidden within Linux-kernel RCU read-side critical sections because it is not legal to place a quiescent state (in this case, voluntary context switch) within an RCU read-side critical section. However, sleeping locks may be used within userspace RCU read-side critical sections, and also within Linux-kernel sleepable RCU [(SRCU)](#sleepable-rcu) read-side critical sections. In addition, the -rt patchset turns spinlocks into a sleeping locks so that the corresponding critical sections can be preempted, which also means that these sleeplockified spinlocks (but not other sleeping locks!) may be acquire within -rt-Linux-kernel RCU read-side critical sections. Note that it *is* legal for a normal RCU read-side critical section to conditionally acquire a sleeping locks (as in [`mutex_trylock()`](../../../kernel-hacking/locking.html#c.mutex_trylock "mutex_trylock")), but only as long as it does not loop indefinitely attempting to conditionally acquire that sleeping locks. The key point is that things like [`mutex_trylock()`](../../../kernel-hacking/locking.html#c.mutex_trylock "mutex_trylock") either return with the mutex held, or return an error indication if the mutex was not immediately available. Either way, [`mutex_trylock()`](../../../kernel-hacking/locking.html#c.mutex_trylock "mutex_trylock") returns immediately without sleeping. |
...
project, which resulted in [SRCU](#sleepable-rcu) becoming optional
for those kernels not needing it.

The remaining performance requirements are, for the most part,
unsurprising. For example, in keeping with RCU’s read-side
specialization, [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") should have negligible overhead
(for example, suppression of a few minor compiler optimizations).
Similarly, in non-preemptible environments, [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") and
[`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock") should have exactly zero overhead.

In preemptible environments, in the case where the RCU read-side
critical section was not preempted (as will be the case for the
highest-priority real-time process), [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") and
[`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock") should have minimal overhead. In particular, they
...
was preempted, [`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock") may acquire spinlocks and disable
...
The [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") grace-period-wait primitive is optimized for
...
[`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") are required to use batching optimizations so that
...
of [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu"), thus amortizing the per-invocation overhead
...
In some cases, the multi-millisecond [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") latencies are
unacceptable. In these cases, [`synchronize_rcu_expedited()`](../../../core-api/kernel-api.html#c.synchronize_rcu_expedited "synchronize_rcu_expedited") may be
...
requirements for [`synchronize_rcu_expedited()`](../../../core-api/kernel-api.html#c.synchronize_rcu_expedited "synchronize_rcu_expedited") on large systems, but,
...
requirements: A storm of [`synchronize_rcu_expedited()`](../../../core-api/kernel-api.html#c.synchronize_rcu_expedited "synchronize_rcu_expedited") invocations on
4096 CPUs should at least make reasonable forward progress. In return
for its shorter latencies, [`synchronize_rcu_expedited()`](../../../core-api/kernel-api.html#c.synchronize_rcu_expedited "synchronize_rcu_expedited") is permitted
...
[`synchronize_rcu_expedited()`](../../../core-api/kernel-api.html#c.synchronize_rcu_expedited "synchronize_rcu_expedited")’s reduced grace-period latency is
unacceptable. In these situations, the asynchronous [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") can
be used in place of [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") as follows:
...
lines 1-5. The function `remove_gp_cb()` is passed to [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu")
...
[`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") function may be used in a number of situations where
neither [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") nor [`synchronize_rcu_expedited()`](../../../core-api/kernel-api.html#c.synchronize_rcu_expedited "synchronize_rcu_expedited") would
...
[`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") is illegal within NMI handlers and from idle and offline
...
| --- |
| **Quick Quiz**: |
| Why does line 19 use [`rcu_access_pointer()`](../../../core-api/kernel-api.html#c.rcu_access_pointer "rcu_access_pointer")? After all, [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") on line 25 stores into the structure, which would interact badly with concurrent insertions. Doesn’t this mean that [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") is required? |
| **Answer**: |
| Presumably the `->gp_lock` acquired on line 18 excludes any changes, including any insertions that [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") would protect against. Therefore, any insertions will be delayed until after `->gp_lock` is released on line 25, which in turn means that [`rcu_access_pointer()`](../../../core-api/kernel-api.html#c.rcu_access_pointer "rcu_access_pointer") suffices. |

However, all that `remove_gp_cb()` is doing is invoking [`kfree()`](../../../core-api/mm-api.html#c.kfree "kfree") on
the data element. This is a common idiom, and is supported by
[`kfree_rcu()`](../../../core-api/kernel-api.html#c.kfree_rcu "kfree_rcu"), which allows “fire and forget” operation as shown
below:
...
Note that `remove_gp_faf()` simply invokes [`kfree_rcu()`](../../../core-api/kernel-api.html#c.kfree_rcu "kfree_rcu") and
proceeds, without any need to pay any further attention to the
subsequent grace period and [`kfree()`](../../../core-api/mm-api.html#c.kfree "kfree"). It is permissible to invoke
[`kfree_rcu()`](../../../core-api/kernel-api.html#c.kfree_rcu "kfree_rcu") from the same environments as for [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu").
Interestingly enough, DYNIX/ptx had the equivalents of [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu")
and [`kfree_rcu()`](../../../core-api/kernel-api.html#c.kfree_rcu "kfree_rcu"), but not [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu"). This was due to the
fact that RCU was not heavily used within DYNIX/ptx, so the very few
places that needed something like [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") simply
open-coded it.

|  |
| --- |
| **Quick Quiz**: |
| Earlier it was claimed that [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") and [`kfree_rcu()`](../../../core-api/kernel-api.html#c.kfree_rcu "kfree_rcu") allowed updaters to avoid being blocked by readers. But how can that be correct, given that the invocation of the callback and the freeing of the memory (respectively) must still wait for a grace period to elapse? |
| **Answer**: |
| We could define things this way, but keep in mind that this sort of definition would say that updates in garbage-collected languages cannot complete until the next time the garbage collector runs, which does not seem at all reasonable. The key point is that in most cases, an updater using either [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") or [`kfree_rcu()`](../../../core-api/kernel-api.html#c.kfree_rcu "kfree_rcu") can proceed to the next update as soon as it has invoked [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") or [`kfree_rcu()`](../../../core-api/kernel-api.html#c.kfree_rcu "kfree_rcu"), without having to wait for a subsequent grace period. |
...
[`get_state_synchronize_rcu()`](../../../core-api/kernel-api.html#c.get_state_synchronize_rcu "get_state_synchronize_rcu") and [`cond_synchronize_rcu()`](../../../core-api/kernel-api.html#c.cond_synchronize_rcu "cond_synchronize_rcu") functions
...
On line 14, [`get_state_synchronize_rcu()`](../../../core-api/kernel-api.html#c.get_state_synchronize_rcu "get_state_synchronize_rcu") obtains a “cookie” from RCU,
...
[`cond_synchronize_rcu()`](../../../core-api/kernel-api.html#c.cond_synchronize_rcu "cond_synchronize_rcu") has appeared quite recently, so it is too
...
CPUs 1 through 63 spin in tight loops that invoke [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu"). Even
...
are provided only for RCU, not for [SRCU](#sleepable-rcu) or [Tasks
RCU](#tasks-rcu).

RCU takes the following steps in [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") to encourage timely
...
RCU, not for [SRCU](#sleepable-rcu) or [Tasks
RCU](#tasks-rcu). Even for RCU, callback-invocation forward
progress for `rcu_nocbs` CPUs is much less well-developed, in part
because workloads benefiting from `rcu_nocbs` CPUs tend to invoke
[`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") relatively infrequently. If workloads emerge that need
both `rcu_nocbs` CPUs and high [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") invocation rates, then
...
Implementations of RCU for which [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") and
[`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock") generate no code, such as Linux-kernel RCU when
`CONFIG_PREEMPTION=n`, can be nested arbitrarily deeply. After all, there
is no overhead. Except that if all these instances of
[`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") and [`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock") are visible to the
...
RCU callbacks in the [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") code path. Finally, high update
rates should not delay RCU read-side critical sections, although some
small read-side delays can occur when using
[`synchronize_rcu_expedited()`](../../../core-api/kernel-api.html#c.synchronize_rcu_expedited "synchronize_rcu_expedited"), courtesy of this function’s use of
...
1. It is all too easy to forget to use [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") everywhere
   that it is needed, so kernels built with `CONFIG_PROVE_RCU=y` will
   splat if [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference") is used outside of an RCU read-side
   critical section. Update-side code can use
   [`rcu_dereference_protected()`](../../../core-api/kernel-api.html#c.rcu_dereference_protected "rcu_dereference_protected"), which takes a [lockdep
   ...
   [`rcu_dereference_check()`](../../../core-api/kernel-api.html#c.rcu_dereference_check "rcu_dereference_check"), which also takes a lockdep expression,
   and emits a lockdep splat if neither [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") nor the
   ...
   [`rcu_read_lock_held()`](../../../core-api/kernel-api.html#c.rcu_read_lock_held "rcu_read_lock_held") is provided to allow a function to verify
...
3. It is also easy to forget to use [`rcu_assign_pointer()`](../../../core-api/kernel-api.html#c.rcu_assign_pointer "rcu_assign_pointer") and
   [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference"), perhaps (incorrectly) substituting a simple
...
4. Kernels built with `CONFIG_DEBUG_OBJECTS_RCU_HEAD=y` will splat if
   a data element is passed to [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") twice in a row, without a
   ...
   [`init_rcu_head_on_stack()`](../../../core-api/kernel-api.html#c.init_rcu_head_on_stack "init_rcu_head_on_stack") and cleaned up with
   [`destroy_rcu_head_on_stack()`](../../../core-api/kernel-api.html#c.destroy_rcu_head_on_stack "destroy_rcu_head_on_stack"). Similarly, statically allocated
...
8. Open-coded use of [`rcu_assign_pointer()`](../../../core-api/kernel-api.html#c.rcu_assign_pointer "rcu_assign_pointer") and [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference")
...
9. Some linked structures are created at compile time, but still require
   `__rcu` checking. The [`RCU_POINTER_INITIALIZER()`](../../../core-api/kernel-api.html#c.RCU_POINTER_INITIALIZER "RCU_POINTER_INITIALIZER") macro serves
   this purpose.
10. It is not necessary to use [`rcu_assign_pointer()`](../../../core-api/kernel-api.html#c.rcu_assign_pointer "rcu_assign_pointer") when creating
    linked structures that are to be published via a single external
    pointer. The [`RCU_INIT_POINTER()`](../../../core-api/kernel-api.html#c.RCU_INIT_POINTER "RCU_INIT_POINTER") macro is provided for this task.
...
2. [Firmware Interface](#firmware-interface)
3. [Early Boot](#early-boot)
4. [Interrupts and NMIs](#interrupts-and-nmis)
5. [Loadable Modules](#loadable-modules)
6. [Hotplug CPU](#hotplug-cpu)
7. [Scheduler and RCU](#scheduler-and-rcu)
8. [Tracing and RCU](#tracing-and-rcu)
9. [Accesses to User Memory and RCU](#accesses-to-user-memory-and-rcu)
10. [Energy Efficiency](#energy-efficiency)
11. [Scheduling-Clock Interrupts and RCU](#scheduling-clock-interrupts-and-rcu)
12. [Memory Efficiency](#memory-efficiency)
13. [Performance, Scalability, Response Time, and Reliability](#performance-scalability-response-time-and-reliability)
...
set up. The read-side primitives ([`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock"),
[`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock"), [`rcu_dereference()`](../../../core-api/kernel-api.html#c.rcu_dereference "rcu_dereference"), and
[`rcu_access_pointer()`](../../../core-api/kernel-api.html#c.rcu_access_pointer "rcu_access_pointer")) will operate normally very early on, as will
[`rcu_assign_pointer()`](../../../core-api/kernel-api.html#c.rcu_assign_pointer "rcu_assign_pointer").

Although [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") may be invoked at any time during boot,
...
Perhaps surprisingly, [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") and
[`synchronize_rcu_expedited()`](../../../core-api/kernel-api.html#c.synchronize_rcu_expedited "synchronize_rcu_expedited"), will operate normally during very early
boot, the reason being that there is only one CPU and preemption is
disabled. This means that the call [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") (or friends)
itself is a quiescent state and thus a grace period, so the early-boot
implementation can be a no-op.

However, once the scheduler has spawned its first kthread, this early
boot trick fails for [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") (as well as for
[`synchronize_rcu_expedited()`](../../../core-api/kernel-api.html#c.synchronize_rcu_expedited "synchronize_rcu_expedited")) in `CONFIG_PREEMPTION=y` kernels. The
reason is that an RCU read-side critical section might be preempted,
which means that a subsequent [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") really does have to
wait for something, as opposed to simply returning immediately.
Unfortunately, [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") can’t do this until all of its
...
code, as are invocations of [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu").
...
update-side primitives, including [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu"), are prohibited within
NMI handlers.
...
functions, for example, any outstanding [`mod_timer()`](../../../driver-api/basics.html#c.mod_timer "mod_timer") must be dealt
with via [`timer_shutdown_sync()`](../../../driver-api/basics.html#c.timer_shutdown_sync "timer_shutdown_sync") or similar.

Unfortunately, there is no way to cancel an RCU callback; once you
invoke [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu"), the callback function is eventually going to be
...
RCU therefore provides [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier"), which waits until all
in-flight RCU callbacks have been invoked. If a module uses
[`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu"), its exit function should therefore prevent any future
invocation of [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu"), then invoke [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier"). In theory,
the underlying module-unload code could invoke [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier")
...
[`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") into RCU. The need for [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") for module
unloading became apparent later.

Important

The [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") function is not, repeat,
...
[`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") is within its rights to return immediately. Even if
there are callbacks posted, [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") does not necessarily need
to wait for a grace period.

|  |
| --- |
| **Quick Quiz**: |
| Wait a minute! Each RCU callbacks must wait for a grace period to complete, and [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") must wait for each pre-existing callback to be invoked. Doesn’t [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") therefore need to wait for a full grace period if there is even one callback posted anywhere in the system? |
| **Answer**: |
| Absolutely not!!! Yes, each RCU callbacks must wait for a grace period to complete, but it might well be partly (or even completely) finished waiting by the time [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") is invoked. In that case, [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") need only wait for the remaining portion of the grace period to elapse. So even if there are quite a few callbacks posted, [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") might well return quite quickly.  So if you need to wait for a grace period as well as for all pre-existing callbacks, you will need to invoke both [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") and [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier"). If latency is a concern, you can always use workqueues to invoke them concurrently. |

### Hotplug CPU

The Linux kernel supports CPU hotplug, which means that CPUs can come
and go. It is of course illegal to use any RCU API member from an
offline CPU, with the exception of [SRCU](#sleepable-rcu) read-side
...
grace-period operations such as ([`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") and
[`synchronize_rcu_expedited()`](../../../core-api/kernel-api.html#c.synchronize_rcu_expedited "synchronize_rcu_expedited")). However, these synchronous operations
do block and therefore cannot be invoked from notifiers that execute via
`stop_machine()`, specifically those between the `CPUHP_AP_OFFLINE`
and `CPUHP_AP_ONLINE` states.

In addition, all-callback-wait operations such as [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") may
not be invoked from any CPU-hotplug notifier. This restriction is due
to the fact that there are phases of CPU-hotplug operations where the
outgoing CPU’s callbacks will not be invoked until after the CPU-hotplug
operation ends, which could also result in deadlock. Furthermore,
[`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") blocks CPU-hotplug operations during its execution,
...
#### Another race causing GP hangs in rcu\_gpu\_init(): Reporting QS for Now-offline CPUs
...
[`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock"), even if interrupts and preemption were enabled
somewhere within the corresponding RCU read-side critical section.
Therefore, it is now perfectly legal to execute [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock")
with preemption enabled, acquire one of the scheduler locks, and hold
that lock across the matching [`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock").
...
referenced by system-call parameters. The [`get_user()`](../../../core-api/mm-api.html#c.get_user "get_user") macro does this job.

However, user-space memory might well be paged out, which means that
[`get_user()`](../../../core-api/mm-api.html#c.get_user "get_user") might well page-fault and thus block while waiting for the
resulting I/O to complete. It would be a very bad thing for the compiler to
reorder a [`get_user()`](../../../core-api/mm-api.html#c.get_user "get_user") invocation into an RCU read-side critical section.
...
build, and if [`get_user()`](../../../core-api/mm-api.html#c.get_user "get_user") did page fault, the result would be a quiescent
state in the middle of an RCU read-side critical section. This misplaced
quiescent state could result in line 4 being a use-after-free access,
which could be bad for your kernel’s actuarial statistics. Similar examples
can be constructed with the call to [`get_user()`](../../../core-api/mm-api.html#c.get_user "get_user") preceding the
[`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock").

Unfortunately, [`get_user()`](../../../core-api/mm-api.html#c.get_user "get_user") doesn’t have any particular ordering properties,
...
Therefore, the Linux-kernel definitions of [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") and
[`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock") must act as compiler barriers, at least for outermost
instances of [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") and [`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock") within a nested set
...
[earlier](#energy-efficiency). It also requires that the
...
of the `rcu_head` structure used by [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") and
[`kfree_rcu()`](../../../core-api/kernel-api.html#c.kfree_rcu "kfree_rcu"). Although this structure contains nothing more than a
...
tracking the file and line of the [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") or [`kfree_rcu()`](../../../core-api/kernel-api.html#c.kfree_rcu "kfree_rcu") that
...
`->next` field. RCU makes this guarantee as long as [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") is
used to post the callback, as opposed to [`kfree_rcu()`](../../../core-api/kernel-api.html#c.kfree_rcu "kfree_rcu") or some future
“lazy” variant of [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") that might one day be created for
...
`rcu_head` structure to one of the [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") family of functions
...
discussion](#performance-and-scalability), RCU is used heavily by
hot code paths in performance-critical portions of the Linux kernel’s
networking, security, virtualization, and scheduling code paths. RCU
must therefore use efficient implementations, especially in its
read-side primitives. To that end, it would be good if preemptible RCU’s
implementation of [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") could be inlined, however, doing
...
optimizations for [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu"), [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu"),
[`synchronize_rcu_expedited()`](../../../core-api/kernel-api.html#c.synchronize_rcu_expedited "synchronize_rcu_expedited"), and [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier"). As a general
...
1. [Bottom-Half Flavor (Historical)](#bottom-half-flavor-historical)
2. [Sched Flavor (Historical)](#sched-flavor-historical)
3. [Sleepable RCU](#sleepable-rcu)
4. [Tasks RCU](#tasks-rcu)
5. [Tasks Trace RCU](#tasks-trace-rcu)
...
Because [`rcu_read_lock_bh()`](../../../core-api/kernel-api.html#c.rcu_read_lock_bh "rcu_read_lock_bh") and [`rcu_read_unlock_bh()`](../../../core-api/kernel-api.html#c.rcu_read_unlock_bh "rcu_read_unlock_bh") disable and
...
case, [`rcu_read_unlock_bh()`](../../../core-api/kernel-api.html#c.rcu_read_unlock_bh "rcu_read_unlock_bh") will invoke softirq processing, which can
...
read-side critical section rather than [`rcu_read_unlock_bh()`](../../../core-api/kernel-api.html#c.rcu_read_unlock_bh "rcu_read_unlock_bh"), but the
...
[`rcu_read_unlock_bh()`](../../../core-api/kernel-api.html#c.rcu_read_unlock_bh "rcu_read_unlock_bh"). This can of course make it appear at first
glance as if [`rcu_read_unlock_bh()`](../../../core-api/kernel-api.html#c.rcu_read_unlock_bh "rcu_read_unlock_bh") was executing very slowly.

The [RCU-bh
API](https://lwn.net/Articles/609973/#RCU%20Per-Flavor%20API%20Table)
includes [`rcu_read_lock_bh()`](../../../core-api/kernel-api.html#c.rcu_read_lock_bh "rcu_read_lock_bh"), [`rcu_read_unlock_bh()`](../../../core-api/kernel-api.html#c.rcu_read_unlock_bh "rcu_read_unlock_bh"), [`rcu_dereference_bh()`](../../../core-api/kernel-api.html#c.rcu_dereference_bh "rcu_dereference_bh"),
[`rcu_dereference_bh_check()`](../../../core-api/kernel-api.html#c.rcu_dereference_bh_check "rcu_dereference_bh_check"), and [`rcu_read_lock_bh_held()`](../../../core-api/kernel-api.html#c.rcu_read_lock_bh_held "rcu_read_lock_bh_held"). However, the
old RCU-bh update-side APIs are now gone, replaced by [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu"),
[`synchronize_rcu_expedited()`](../../../core-api/kernel-api.html#c.synchronize_rcu_expedited "synchronize_rcu_expedited"), [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu"), and [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier"). In addition,
...
[`rcu_read_lock_sched()`](../../../core-api/kernel-api.html#c.rcu_read_lock_sched "rcu_read_lock_sched") and [`rcu_read_unlock_sched()`](../../../core-api/kernel-api.html#c.rcu_read_unlock_sched "rcu_read_unlock_sched") disable and
re-enable preemption, respectively. This means that if there was a
preemption attempt during the RCU-sched read-side critical section,
[`rcu_read_unlock_sched()`](../../../core-api/kernel-api.html#c.rcu_read_unlock_sched "rcu_read_unlock_sched") will enter the scheduler, with all the
latency and overhead entailed. Just as with [`rcu_read_unlock_bh()`](../../../core-api/kernel-api.html#c.rcu_read_unlock_bh "rcu_read_unlock_bh"),
this can make it look as if [`rcu_read_unlock_sched()`](../../../core-api/kernel-api.html#c.rcu_read_unlock_sched "rcu_read_unlock_sched") was executing
very slowly. However, the highest-priority task won’t be preempted, so
that task will enjoy low-overhead [`rcu_read_unlock_sched()`](../../../core-api/kernel-api.html#c.rcu_read_unlock_sched "rcu_read_unlock_sched")
invocations.

The [RCU-sched
API](https://lwn.net/Articles/609973/#RCU%20Per-Flavor%20API%20Table)
includes [`rcu_read_lock_sched()`](../../../core-api/kernel-api.html#c.rcu_read_lock_sched "rcu_read_lock_sched"), [`rcu_read_unlock_sched()`](../../../core-api/kernel-api.html#c.rcu_read_unlock_sched "rcu_read_unlock_sched"),
`rcu_read_lock_sched_notrace()`, `rcu_read_unlock_sched_notrace()`,
[`rcu_dereference_sched()`](../../../core-api/kernel-api.html#c.rcu_dereference_sched "rcu_dereference_sched"), [`rcu_dereference_sched_check()`](../../../core-api/kernel-api.html#c.rcu_dereference_sched_check "rcu_dereference_sched_check"), and
`rcu_read_lock_sched_held()`. However, the old RCU-sched update-side APIs
are now gone, replaced by [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu"), [`synchronize_rcu_expedited()`](../../../core-api/kernel-api.html#c.synchronize_rcu_expedited "synchronize_rcu_expedited"),
[`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu"), and [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier"). In addition, anything that disables
...
pass a “cookie” from [`srcu_read_lock()`](../../../core-api/kernel-api.html#c.srcu_read_lock "srcu_read_lock") to [`srcu_read_unlock()`](../../../core-api/kernel-api.html#c.srcu_read_unlock "srcu_read_unlock"), for
example, as follows:
...
[`synchronize_srcu()`](../../../core-api/kernel-api.html#c.synchronize_srcu "synchronize_srcu") for domain `ss`, deadlock would still be
possible. Furthermore, if line 5 acquired a mutex that was held across a
[`synchronize_srcu()`](../../../core-api/kernel-api.html#c.synchronize_srcu "synchronize_srcu") for some other domain `ss1`, and if an
`ss1`-domain SRCU read-side critical section acquired another mutex
that was held across as `ss`-domain [`synchronize_srcu()`](../../../core-api/kernel-api.html#c.synchronize_srcu "synchronize_srcu"), deadlock
...
idle and even offline CPUs, with the exception of [`srcu_read_lock_fast()`](../../../core-api/kernel-api.html#c.srcu_read_lock_fast "srcu_read_lock_fast")
and friends. This ability requires that [`srcu_read_lock()`](../../../core-api/kernel-api.html#c.srcu_read_lock "srcu_read_lock") and
[`srcu_read_unlock()`](../../../core-api/kernel-api.html#c.srcu_read_unlock "srcu_read_unlock") contain memory barriers, which means that SRCU
readers will run a bit slower than would RCU readers. It also motivates
the [`smp_mb__after_srcu_read_unlock()`](../../../core-api/kernel-api.html#c.smp_mb__after_srcu_read_unlock "smp_mb__after_srcu_read_unlock") API, which, in combination with
[`srcu_read_unlock()`](../../../core-api/kernel-api.html#c.srcu_read_unlock "srcu_read_unlock"), guarantees a full memory barrier.

Also unlike other RCU flavors, [`synchronize_srcu()`](../../../core-api/kernel-api.html#c.synchronize_srcu "synchronize_srcu") may **not** be
...
course also prohibits [`srcu_barrier()`](../../../core-api/kernel-api.html#c.srcu_barrier "srcu_barrier") from being invoked from
...
by default), and if a [`synchronize_srcu()`](../../../core-api/kernel-api.html#c.synchronize_srcu "synchronize_srcu") invocation ends this idle
...
allow users to put much heavier stress on [`call_srcu()`](../../../core-api/kernel-api.html#c.call_srcu "call_srcu"), it is
...
The [SRCU API](https://lwn.net/Articles/609973/#RCU%20Per-Flavor%20API%20Table)
includes [`srcu_read_lock()`](../../../core-api/kernel-api.html#c.srcu_read_lock "srcu_read_lock"), [`srcu_read_unlock()`](../../../core-api/kernel-api.html#c.srcu_read_unlock "srcu_read_unlock"),
[`srcu_dereference()`](../../../core-api/kernel-api.html#c.srcu_dereference "srcu_dereference"), [`srcu_dereference_check()`](../../../core-api/kernel-api.html#c.srcu_dereference_check "srcu_dereference_check"), [`synchronize_srcu()`](../../../core-api/kernel-api.html#c.synchronize_srcu "synchronize_srcu"),
[`synchronize_srcu_expedited()`](../../../core-api/kernel-api.html#c.synchronize_srcu_expedited "synchronize_srcu_expedited"), [`call_srcu()`](../../../core-api/kernel-api.html#c.call_srcu "call_srcu"), [`srcu_barrier()`](../../../core-api/kernel-api.html#c.srcu_barrier "srcu_barrier"),
and [`srcu_read_lock_held()`](../../../core-api/kernel-api.html#c.srcu_read_lock_held "srcu_read_lock_held"). It also includes `DEFINE_SRCU()`,
`DEFINE_STATIC_SRCU()`, `DEFINE_SRCU_FAST()`, `DEFINE_STATIC_SRCU_FAST()`,
[`init_srcu_struct()`](../../../core-api/kernel-api.html#c.init_srcu_struct "init_srcu_struct"), and [`init_srcu_struct_fast()`](../../../core-api/kernel-api.html#c.init_srcu_struct_fast "init_srcu_struct_fast") APIs for defining and
initializing `srcu_struct` structures.

More recently, the SRCU API has added polling interfaces:

1. [`start_poll_synchronize_srcu()`](../../../core-api/kernel-api.html#c.start_poll_synchronize_srcu "start_poll_synchronize_srcu") returns a cookie identifying
   the completion of a future SRCU grace period and ensures
   that this grace period will be started.
2. [`poll_state_synchronize_srcu()`](../../../core-api/kernel-api.html#c.poll_state_synchronize_srcu "poll_state_synchronize_srcu") returns `true` iff the
   specified cookie corresponds to an already-completed
   SRCU grace period.
3. [`get_state_synchronize_srcu()`](../../../core-api/kernel-api.html#c.get_state_synchronize_srcu "get_state_synchronize_srcu") returns a cookie just like
   [`start_poll_synchronize_srcu()`](../../../core-api/kernel-api.html#c.start_poll_synchronize_srcu "start_poll_synchronize_srcu") does, but differs in that
...
as [`rcu_read_lock()`](../../../core-api/kernel-api.html#c.rcu_read_lock "rcu_read_lock") and [`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock"). In addition, it does
not work to have these markers in the trampoline itself, because there
would need to be instructions following [`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock"). Although
[`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") would guarantee that execution reached the
[`rcu_read_unlock()`](../../../core-api/kernel-api.html#c.rcu_read_unlock "rcu_read_unlock"), it would not be able to guarantee that execution
...
[`synchronize_rcu_tasks()`](../../../core-api/kernel-api.html#c.synchronize_rcu_tasks "synchronize_rcu_tasks"). In addition, transitions to and from
...
not provide a Tasks RCU quiescent state. (Instead, use [`rcu_softirq_qs()`](../../../core-api/kernel-api.html#c.rcu_softirq_qs "rcu_softirq_qs")
from softirq or `rcu_tasks_classic_qs()` otherwise.)

The tasks-RCU API is quite compact, consisting only of
[`call_rcu_tasks()`](../../../core-api/kernel-api.html#c.call_rcu_tasks "call_rcu_tasks"), [`synchronize_rcu_tasks()`](../../../core-api/kernel-api.html#c.synchronize_rcu_tasks "synchronize_rcu_tasks"), and
[`rcu_barrier_tasks()`](../../../core-api/kernel-api.html#c.rcu_barrier_tasks "rcu_barrier_tasks"). In `CONFIG_PREEMPTION=n` kernels, trampolines
cannot be preempted, so these APIs map to [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu"),
[`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu"), and [`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier"), respectively. In
...
not watching. This means that [`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu") is insufficient, and
...
consisting solely of [`synchronize_rcu_tasks_rude()`](../../../core-api/kernel-api.html#c.synchronize_rcu_tasks_rude "synchronize_rcu_tasks_rude").

### Tasks Trace RCU

Some forms of tracing need to sleep in readers, but cannot tolerate
SRCU’s read-side overhead, which includes a full memory barrier in
both [`srcu_read_lock()`](../../../core-api/kernel-api.html#c.srcu_read_lock "srcu_read_lock") and [`srcu_read_unlock()`](../../../core-api/kernel-api.html#c.srcu_read_unlock "srcu_read_unlock"). This need is handled by
...
to [`synchronize_rcu_tasks_trace()`](../../../core-api/kernel-api.html#c.synchronize_rcu_tasks_trace "synchronize_rcu_tasks_trace") implies at least one call to
[`synchronize_rcu()`](../../../core-api/kernel-api.html#c.synchronize_rcu "synchronize_rcu"), that is, every Tasks Trace RCU grace period contains
...
consisting of [`rcu_read_lock_trace()`](../../../core-api/kernel-api.html#c.rcu_read_lock_trace "rcu_read_lock_trace"), [`rcu_read_unlock_trace()`](../../../core-api/kernel-api.html#c.rcu_read_unlock_trace "rcu_read_unlock_trace"),
`rcu_read_lock_trace_held()`, [`call_rcu_tasks_trace()`](../../../core-api/kernel-api.html#c.call_rcu_tasks_trace "call_rcu_tasks_trace"),
[`synchronize_rcu_tasks_trace()`](../../../core-api/kernel-api.html#c.synchronize_rcu_tasks_trace "synchronize_rcu_tasks_trace"), and [`rcu_barrier_tasks_trace()`](../../../core-api/kernel-api.html#c.rcu_barrier_tasks_trace "rcu_barrier_tasks_trace").
...
[`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") operations. If there is a strong reason to use
[`rcu_barrier()`](../../../core-api/kernel-api.html#c.rcu_barrier "rcu_barrier") in CPU-hotplug notifiers, it will be necessary to
...
the combining tree, nor does [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") in the common case. If you
...
might be charged back to the originating [`call_rcu()`](../../../core-api/kernel-api.html#c.call_rcu "call_rcu") instance, though
