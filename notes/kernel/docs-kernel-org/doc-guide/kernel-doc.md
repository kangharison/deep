...
> 출처(원문): https://docs.kernel.org/doc-guide/kernel-doc.html
...
[General Information](../rust/general-information.html) instead.
...
1. When documenting nested structs or unions, if the `struct`/`union`
   `foo` is named, the member `bar` inside it should be documented as
   `@foo.bar:`
2. When the nested `struct`/`union` is anonymous, the member `bar` in
