...
> 출처(원문): https://docs.kernel.org/doc-guide/parse-headers.html
...
The [parse\_headers.py](#parse-headers) provides a way to generate such
cross-references. It has to be called via Makefile, while building the
documentation. Please see `Documentation/userspace-api/media/Makefile` for an example
about how to use it inside the Kernel tree.

## tools/docs/parse\_headers.py

### NAME

parse\_headers.py - parse a C file, in order to identify functions, structs,
...
2. Replace rules:
   ...
   > replace *type* *old\_symbol* *new\_reference*
   >
   > Replaces *old\_symbol* with a *new\_reference*.
   > The *new\_reference* can be:
   >
...
Copyright (c) 2016, 2025 by Mauro Carvalho Chehab <[mchehab+huawei@kernel.org](mailto:mchehab+huawei%40kernel.org)>.
