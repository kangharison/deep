...
> 출처(원문): https://docs.kernel.org/doc-guide/sphinx.html
...
[Checking for Sphinx dependencies](#sphinx-pre-install) for further details.
...
handles images in both GraphViz and SVG formats (see [Figures & Images](#sphinx-kfigure)).
...
without installing any texlive packages. See [Choice of Math Renderer](#choice-of-math-renderer) for
further info.
...
[Minimal requirements to compile the Kernel](../process/changes.html) can be done by creating
...
> tools/docs/test\_doc\_build.py
...
from ImageMagick (<https://www.imagemagick.org>).[[1]](#ink) All of these are
...
It is also possible to pass an extra DOCS\_CSS overlay file, in order to customize
...
and *targets* (e.g. a ref to `` :ref:`last row <last row>` `` / [last row](#last-row)).
...
[Writing kernel-doc comments](kernel-doc.html).
...
image format, use SVG ([SVG image example](#svg-image-example)):
...
A simple example ([DOT’s hello world example](#hello-dot-file)):
...
*literal-block* ([Embedded DOT (Graphviz) code](#hello-dot-render)).
...
you want to refer to it ([Embedded SVG markup](#hello-svg-render)).
