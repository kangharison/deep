...
> 출처(원문): https://docs.kernel.org/trace/tracepoint-analysis.html
...
Tracepoints (see [Using the Linux Kernel Tracepoints](tracepoints.html)) can be used without
...
See [Event Tracing](events.html) for a proper description on how events
...
[ftrace - Function Tracer](ftrace.html) describes how to enable events on a per-thread
basis using set\_ftrace\_pid.
...
[ftrace - Function Tracer](ftrace.html) covers in-depth how to filter events in
ftrace. Obviously using grep and awk of trace\_pipe is an option as well
as any script reading trace\_pipe.
...
/sys/kernel/tracing/trace\_pipe in human-readable format although binary
...
script that can read trace\_pipe from STDIN or a copy of a trace. When used
