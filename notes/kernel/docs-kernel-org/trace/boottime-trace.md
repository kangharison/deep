...
> 출처(원문): https://docs.kernel.org/trace/boottime-trace.html
...
boot config file [[1]](#id3). All options are under “ftrace.” or “kernel.”
prefix. See kernel parameters for the options which starts
with “kernel.” prefix [[2]](#id4).

[[1](#id1)]

See [Documentation/admin-guide/bootconfig.rst](../admin-guide/bootconfig.html#bootconfig)


[[2](#id2)]

See [Documentation/admin-guide/kernel-parameters.rst](../admin-guide/kernel-parameters.html#kernelparameters)
...
ftrace.[instance.INSTANCE.]tracing\_on = 0|1
...
ftrace.[instance.INSTANCE.]trace\_clock = CLOCK
:   Set given CLOCK to ftrace’s trace\_clock.

ftrace.[instance.INSTANCE.]buffer\_size = SIZE
...
please read the event histogram document ([Event Histograms](histogram.html))
...
ftrace.[instance.INSTANCE.]event.GROUP.EVENT.hist.[N.]size = NR\_ENTRIES
...
ftrace.[instance.INSTANCE.]event.GROUP.EVENT.hist.filter = FILTER\_EXPR
:   Set histogram filter expression. You don’t need ‘if’ in the FILTER\_EXPR.
...
end of core\_initcall. This means you can trace the events from postcore\_initcall.
Most of the subsystems and architecture dependent drivers will be initialized
after that (arch\_initcall or subsys\_initcall). Thus, you can trace those with
boot-time tracing.
If you want to trace events before core\_initcall, you can use the options
