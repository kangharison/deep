...
> 출처(원문): https://docs.kernel.org/trace/fprobetrace.html
...
events are defined via dynamic\_events interface file on tracefs.
...
For the details of TYPE, see [kprobetrace documentation](kprobetrace.html#kprobetrace-types).
...
See the [sample](#fprobetrace-exit-args-sample) below for how it works.

## BTF arguments

BTF (BPF Type Format) argument allows user to trace function and tracepoint
parameters by its name instead of `$argN`. This feature is available if the
kernel is configured with CONFIG\_BPF\_SYSCALL and CONFIG\_DEBUG\_INFO\_BTF.
...
You can see the file::f\_mode and file::f\_inode are updated in `vfs_open()`.
