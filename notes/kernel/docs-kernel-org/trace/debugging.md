...
> 출처(원문): https://docs.kernel.org/trace/debugging.html
...
this. The first is “ftrace\_dump\_on\_oops”. This will dump the tracing ring
...
buffers is broken up into sub-buffers that are by default PAGE\_SIZE. The
above trace\_buf\_size option above sets each of the per CPU buffers to 50K,
...
command line option and then use that memory for the trace\_instance. This
...
location 0x284500000. Then the trace\_instance option will create a trace
instance “boot\_map” at that same location with the same amount of memory
...
at boot is with the “reserve\_mem” option:

```
reserve_mem=12M:4096:trace trace_instance=boot_map@trace
```

The reserve\_mem option above will find 12 megabytes that are available at
boot up, and align it by 4096 bytes. It will label this memory as “trace”
that can be used by later command line options.

The trace\_instance option creates a “boot\_map” instance and will use the
memory reserved by reserve\_mem that was labeled as “trace”. This method is
more generic but may not be as reliable. Due to KASLR, the memory reserved
by reserve\_mem may not be located at the same location. If this happens,
...
around in such a way that it will move the location of the reserve\_mem. By
...
## Using trace\_printk() in the boot instance
...
On boot up, the previous data in the “boot\_map” is copied to the “backup”
instance, and the “sched:*” and “irq:*” events for the current boot are traced
in the “boot\_map”. Thus the user can read the previous boot data from the “backup”
instance without stopping the trace.

Note that this “backup” instance is readonly, and will be removed automatically
if you clear the trace data or read out all trace data from the “trace\_pipe”
or the “trace\_pipe\_raw” files.
