...
> 출처(원문): https://docs.kernel.org/trace/eprobetrace.html
...
with CONFIG\_EPROBE\_EVENTS=y.

Eprobes are created via the /sys/kernel/tracing/dynamic\_events file.

### Synopsis of eprobe\_events
...
the tracing buffer. For example, a common event to trace is the sched\_switch
...
Note, without adding the “u32” after the prev\_pid and next\_pid, the values
would default showing in hexadecimal.

### Example 2

If a specific system call is to be recorded but the syscalls events are not
enabled, the raw\_syscalls can still be used (syscalls are system call
events are not normal events, but are created from the raw\_syscalls events
within the kernel). In order to trace the openat system call, one can create
an event probe on top of the raw\_syscalls event:
...
This is being run on x86\_64 where the word size is 8 bytes and the openat
system call \_\_NR\_openat is set at 257.
