...
> 출처(원문): https://docs.kernel.org/trace/intel_th.html
...
their own (“intel\_th”), where they can be discovered and configured
...
[ABI file testing/sysfs-bus-intel\_th-output-devices](../admin-guide/abi-testing-files.html#abi-file-testing-sysfs-bus-intel-th-output-devices), the most
...
description is at [ABI file testing/sysfs-bus-intel\_th-devices-gth](../admin-guide/abi-testing-files.html#abi-file-testing-sysfs-bus-intel-th-devices-gth).

STH registers an stm class device, through which it provides interface
to userspace and kernelspace software trace sources. See
[System Trace Module](stm.html) for more information on that.
...
devices were enumerated. All TH subdevices (devices on intel\_th bus)
begin with this id: 0-gth, 0-msc0, 0-msc1, 0-pti, 0-sth, which is
followed by device’s name and an optional index.

Output devices also get a device node in /dev/intel\_thN, where N is
the Intel TH device id. For example, MSU’s memory buffers, when
allocated, are accessible via /dev/intel\_th0/msc{0,1}.
...
# .. send data to master 33, see [System Trace Module](stm.html) for more details ..
...
In order to enable the host mode, set the ‘host\_mode’ parameter of the
‘intel\_th’ kernel module to ‘y’. None of the virtual output devices
will show up on the intel\_th bus. Also, trace configuration and
