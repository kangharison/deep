...
> 출처(원문): https://docs.kernel.org/trace/ftrace.html
...
See [Event Tracing](events.html) for more information.

## Implementation Details

See [Function Tracer Design](ftrace-design.html) for details for arch porters and such.
...
> > > echoing their name into current\_tracer.
> >
> > tracing\_on:
> >
...
> > > writing with the O\_TRUNC flag clears the ring buffer content.
> > > Note, this file is not a consumer. If tracing is off
> > > (no tracer running, or tracing\_on is zero), it will produce
...
> > > trace option (also in trace\_options). Options may also be set
...
> > > blocking read syscall on one of the per\_cpu trace\_pipe\_raw files, it
> > > will block until the given amount of data specified by buffer\_percent
...
> > > (see “per\_cpu/cpu0/buffer\_size\_kb” below), and if they do
...
> > >
> > > Note, the buffer\_subbuf\_size\_kb is a way for the user to specify the
...
> > >
> > > It may also stop tracing if disable\_on\_free option is set.
> >
> > tracing\_cpumask:
> >
...
> > > This influences the tracers “function” and “function\_graph”
> > > and thus also function profiling (see “function\_profile\_enabled”).
> > >
> > > The functions listed in “available\_filter\_functions” are what
...
> > > of the “available\_filter\_functions” file.
> >
> > set\_ftrace\_notrace:
> >
> > > This has an effect opposite to that of
> > > set\_ftrace\_filter. Any function that is added here will not
> > > be traced. If a function exists in both set\_ftrace\_filter
> > > and set\_ftrace\_notrace, the function will \_not\_ be traced.
> >
> > set\_ftrace\_pid:
> >
...
> > > cause PIDs of tasks that exit to be removed from the file.
> > >
> > > If a PID is in both this file and “set\_ftrace\_pid”, then this
> > > file takes precedence, and the thread will not be traced.
> >
> > set\_event\_pid:
> >
> > > Have the events only trace a task with a PID listed in this file.
> > > Note, sched\_switch and sched\_wake\_up will also trace events
> > > listed in this file.
> > >
...
> > > exits.
> >
> > set\_event\_notrace\_pid:
> >
> > > Have the events not trace a task with a PID listed in this file.
> > > Note, sched\_switch and sched\_wakeup will trace threads not listed
> > > in this file, even if a thread’s PID is in the file if the
> > > sched\_switch or sched\_wakeup events also trace a thread that should
> > > be traced.
> > >
> > > To have the PIDs of children of tasks with their PID in this file
> > > added on fork, enable the “event-fork” option. That option will also
...
> > > Note, set\_ftrace\_filter and set\_ftrace\_notrace still affects
> > > what functions are being traced.
> >
> > set\_graph\_notrace:
> >
> > > Similar to set\_graph\_function, but will disable function graph
...
> > > “set\_ftrace\_filter”, “set\_ftrace\_notrace”,
> > > “set\_graph\_function”, or “set\_graph\_notrace”.
> > > (See the section “dynamic ftrace” below for more details.)
> >
> > available\_filter\_functions\_addrs:
> >
> > > Similar to available\_filter\_functions, but with address displayed
> > > for each function. The displayed address is the patch-site address
> > > and can differ from /proc/kallsyms address.
> >
> > syscall\_user\_buf\_size:
> >
...
> > > enabled\_functions but shows all functions that have ever been
> > > traced.
> > >
...
> > >
> > > grep ‘ M ‘ /sys/kernel/tracing/touched\_functions
> >
...
> > >
> > > trace\_stat/function<cpu> ( function0, function1, etc).
> >
> > trace\_stat:
> >
...
> > kprobe\_events:
> >
> > > Enable dynamic trace points. See [Kprobe-based Event Tracing](kprobetrace.html).
> >
> > kprobe\_profile:
> >
> > > Dynamic trace points stats. See [Kprobe-based Event Tracing](kprobetrace.html).
> >
> > max\_graph\_depth:
> >
...
> >
> > > By default, 128 comms are saved (see “saved\_cmdlines” above). To
...
> >
> > > This is similar to “set\_ftrace\_filter” but it limits what
...
> > >
> > > mono:
> > > :   This uses the fast monotonic clock (CLOCK\_MONOTONIC)
> > >     which is monotonic and is subject to NTP rate adjustments.
> > >
> > > mono\_raw:
> > > :   This is the raw monotonic clock (CLOCK\_MONOTONIC\_RAW)
> > >     which is monotonic but is not subject to any rate adjustments
> > >     and ticks at the same rate as the hardware clocksource.
> > >
> > > boot:
> > > :   This is the boot clock (CLOCK\_BOOTTIME) and is based on the
...
> > >     [`ktime_get_boot_fast_ns()`](../core-api/timekeeping.html#c.ktime_get_boot_fast_ns "ktime_get_boot_fast_ns") function for more information.
> > >
> > > tai:
> > > :   This is the tai clock (CLOCK\_TAI) and is derived from the wall-
...
> > >     handle them. See comments in the [`ktime_get_tai_fast_ns()`](../core-api/timekeeping.html#c.ktime_get_tai_fast_ns "ktime_get_tai_fast_ns")
...
> > > ```
> > >
> > > Note: Writing into the trace\_marker file can also initiate triggers
> > > :   that are written into /sys/kernel/tracing/events/ftrace/print/trigger
> > >     See “Event triggers” in [Event Tracing](events.html) and an
> > >     example in [Event Histograms](histogram.html) (Section 3.)
> >
> > trace\_marker\_raw:
> >
> > > This is similar to trace\_marker above, but is meant for binary data
> > > to be written to it, where a tool can be used to parse the data
> > > from trace\_pipe\_raw.
> >
> > uprobe\_events:
> >
> > > Add dynamic tracepoints in programs.
> > > See [Uprobe-tracer: Uprobe-based Event Tracing](uprobetracer.html)
> >
> > uprobe\_profile:
> >
...
> > > size buffers. This file is similar to the buffer\_size\_kb
...
> >
> > > This is similar to the “trace\_pipe” file, and is a consuming
...
> > > the trace\_pipe\_raw file can be used to extract the data
...
> > > data.
> > >
> > > Like trace\_pipe, this is a consuming reader, where multiple
...
> >
> > > Similar to the trace\_pipe\_raw, but will read the binary format
...
> > See tracing\_max\_latency. When a new max is recorded,
...
>
> > Traces and records the max latency that it takes for
> > a SCHED\_DEADLINE task to be woken (as the “wakeup” and
> > “wakeup\_rt” does).
>
> “mmiotrace”
>
...
> available via the tracing/error\_log file. For the commands that
> support it, reading the tracing/error\_log file after an error will
> display more detailed information about what went wrong, if
> information is available. The tracing/error\_log file is a circular
...
function name that was traced “sys\_close” and the parent function that
called this function “system\_call\_fastpath”. The timestamp is the time
...
> * \_\_lock\_task\_sighand is where the interrupts were disabled.
> * \_raw\_spin\_unlock\_irqrestore is where they were enabled again.
...
>
> need-resched:
> :   * ‘B’ all, TIF\_NEED\_RESCHED, PREEMPT\_NEED\_RESCHED and TIF\_RESCHED\_LAZY is set,
...
>     * ‘L’ both PREEMPT\_NEED\_RESCHED and TIF\_RESCHED\_LAZY is set,
>     * ‘b’ both TIF\_NEED\_RESCHED and TIF\_RESCHED\_LAZY is set,
>     * ‘l’ only TIF\_RESCHED\_LAZY is set
>     * ‘.’ otherwise.
>
> hardirq/softirq:
...
The trace\_options file (or the options directory) is used to control
...
>     seeing just “ktime\_get”, you will see
>     “ktime\_get+0xb/0x20”.
>
>     ```
>     sym-offset:
...
>     this set. By setting this flag, it clears the trace\_printk\_dest flag
...
>     trace\_marker file (/sys/kernel/tracing/trace\_marker or trace\_marker\_raw),
...
>     into the top level trace\_marker file will also be redirected into this
>     instance.
>
>     Note, by default this option is set for the top level instance. If it
>     is disabled, then writes to the trace\_marker or trace\_marker\_raw files
...
>     writing to the ring buffer (as if tracing\_on was set to zero).
...
>     in the sched\_switch trace point to fill comm cache
>     with mapped pids and comms. But this may cause some
>     overhead, and if you only care about pids, and not the
>     name of the task, disabling this option can lower the
>     impact of tracing. See “saved\_cmdlines”.
>
> record-tgid
> :   When any event or tracer is enabled, a hook is enabled
>     in the sched\_switch trace point to fill the cache of
...
>
> disable\_on\_free
> :   When the free\_buffer is closed, tracing will
>     stop (tracing\_on set to 0).
>
> irq-info
...
>
> markers
> :   When set, the trace\_marker is writable (only by root).
>     When disabled, the trace\_marker will error with EINVAL
>     on write.
>
> event-fork
> :   When set, tasks with PIDs listed in set\_event\_pid will have
>     the PIDs of their children added to set\_event\_pid when those
>     tasks fork. Also, when tasks with PIDs in set\_event\_pid exit,
...
>
> function-fork
> :   When set, tasks with PIDs listed in set\_ftrace\_pid will
>     have the PIDs of their children added to set\_ftrace\_pid
>     when those tasks fork. Also, when tasks with PIDs in
>     set\_ftrace\_pid exit, their PIDs will be removed from the
>     file.
>
...
>     “set\_ftrace\_filter” otherwise the system performance
...
> Since the function\_graph tracer has a slightly different output
...
To reset the maximum, echo 0 into tracing\_max\_latency. Here is
an example:
...
very good). The \_raw\_spin\_lock\_irq in run\_timer\_softirq disabled
...
the entire time. The irq\_enter code lets us know that we entered
...
The trace\_hardirqs\_off\_thunk is called from assembly on x86 when
...
work well with Real-Time tasks. The wakeup\_rt tracer was designed
...
and it has an rt\_prio of 5. This priority is user-space priority
and not the internal kernel priority. The policy is 1 for
SCHED\_FIFO and 2 for SCHED\_RR.
...
before `task_woken_rt()` was called, the NEED\_RESCHED flag was set,
...
>     written into “current\_tracer”), the original value for
>     tracing\_threshold is placed back into this file.
>
...
>     listed in the tracing\_cpumask between each period
...
ftrace\_enabled is set; otherwise this tracer is a nop.
See the “ftrace\_enabled” section below.
...
By writing into set\_ftrace\_pid you can trace a
...
task\_struct. On function entry the tracer overwrites the return
...
* The cpu number on which the function executed is default
  ...
  tracing\_cpumask file) or you might sometimes see unordered
...
> > * hide: echo nofuncgraph-tail > trace\_options
> > * show: echo funcgraph-tail > trace\_options
>
...
> > * hide: echo nofuncgraph-retval > trace\_options
> > * show: echo funcgraph-retval > trace\_options
>
...
The above example shows that the function cpu\_cgroup\_can\_attach
...
> > * smart: echo nofuncgraph-retval-hex > trace\_options
> > * hexadecimal: echo funcgraph-retval-hex > trace\_options
>
...
* In certain procedure call standards, such as arm64’s AAPCS64, when a
  ...
  **Case One**:
  ...
  The function narrow\_to\_u8 is defined as follows:
  ...
  **Case Two**:
  ...
  The function error\_if\_not\_4g\_aligned is defined as follows:
  ...
  When passing 0x2\_0000\_0000 to it, the return value may be recorded as
  0x2\_0000\_0000 instead of 0.
...
If CONFIG\_DYNAMIC\_FTRACE is set, the system will run with
...
calls “\_\_fentry\_\_” instead of “mcount”. Which is called before
...
traced. Check the “available\_filter\_functions” file to see what functions
can be traced.

A section called “\_\_mcount\_loc” is created that holds
...
available\_filter\_functions list. Modules are processed as they
...
kstop\_machine to prevent races with the CPUs executing code being
...
If I am only interested in sys\_nanosleep and hrtimer\_interrupt:
...
Notice that we lost the sys\_nanosleep.
...
The set\_ftrace\_notrace prevents those functions from being
traced.
...
functions in the “available\_filter\_functions” file), is selected.
...
you just have to echo its name into set\_graph\_function:
...
Note, the proc sysctl ftrace\_enable is a big on/off switch for the
function tracer. By default it is enabled (when function tracing is
enabled in the kernel). If it is disabled, all function tracing is
disabled. This includes not only the function tracers for ftrace, but
also for any other uses (perf, kprobes, stack tracing, profiling, etc). It
cannot be disabled if there is a callback with FTRACE\_OPS\_FL\_PERMANENT set
registered.
...
A few commands are supported by the set\_ftrace\_filter interface.
...
* mod:
  ...
  > echo ‘write\*:mod:ext3’ > set\_ftrace\_filter
...
* traceon/traceoff:
  ...
  To always disable tracing when \_\_schedule\_bug is hit:
  ...
  to set\_ftrace\_filter. To remove a command, prepend it by ‘!’
  ...
  The above removes the traceoff command for \_\_schedule\_bug
...
The trace\_pipe outputs the same content as the trace file, but
the effect on the tracing is different. Every read from
trace\_pipe is consumed. This means that subsequent reads will be
...
Note, reading the trace\_pipe file will block until more input is
...
prevent new entries from being added. The trace\_pipe file does
not have this limitation.

## trace entries

Having too much or not enough data can be troublesome in
diagnosing an issue in the kernel. The file buffer\_size\_kb is
...
Or simply read buffer\_total\_size\_kb
...
When the per\_cpu buffers are not the same, the buffer\_size\_kb
...
Writing to the top level buffer\_size\_kb will reset all the buffers
to be the same again.

## Snapshot

CONFIG\_TRACER\_SNAPSHOT makes a generic snapshot feature
...
affect the main buffer with the exception of trace\_options. Currently,
the trace\_options affect all instances and the top level buffer
...
current\_tracer and available\_tracers. This is because the buffers
...
CONFIG\_STACK\_TRACER enables the ftrace stack tracing functionality.
To enable it, write a ‘1’ into /proc/sys/kernel/stack\_tracer\_enabled.
