...
> 출처(원문): https://docs.kernel.org/trace/rv/da_monitor_instrumentation.html
...
The RV monitor file created by dot2k, with the name “$MODEL\_NAME.c”
...
The preempt\_disable event from the model connects directly to the
preemptirq:preempt\_disable. The preemptirq:preempt\_disable event
...
preempt\_enable event will be used to synchronize the system to the model.
...
preempt\_enable execution. Hence, it can be used to synchronize the
...
“da\_handle\_start\_event\_$(MONITOR\_NAME)(event)”, in this case:
...
*detach* the tracepoints\_to\_attach, which was enough for this case.
