...
> 출처(원문): https://docs.kernel.org/trace/ring-buffer-map.html
...
then a consumer for that ring-buffer, in a similar fashion to trace\_pipe.

## Memory mapping setup

The mapping works with a mmap() of the trace\_pipe\_raw interface.
...
be safely read by the mapper (see [Lockless Ring Buffer Design](ring-buffer-design.html)).
...
the trace\_pipe\_raw ioctl() TRACE\_MMAP\_IOCTL\_GET\_READER. This ioctl also updates
...
kernel with trace\_pipe) are allowed but not recommended. They will compete for
the ring-buffer and the output is unpredictable, just like concurrent readers on
trace\_pipe would be.
