...
> 출처(원문): https://docs.kernel.org/trace/events.html
...
Tracepoints (see [Using the Linux Kernel Tracepoints](tracepoints.html)) can be used
...
/sys/kernel/tracing/available\_events.

To enable a particular event, such as ‘sched\_wakeup’, simply echo it
to /sys/kernel/tracing/set\_event. For example:
...
To disable an event, echo the event name to the set\_event file prefixed
...
subsystem name is optional, but it is displayed in the available\_events
...
The set\_event file may also be used to enable events associated to only
a specific module:
...
events and correspond to the fields defined in the TRACE\_EVENT
...
For example, here’s the information displayed for the ‘sched\_wakeup’
event:
...
“filename” from sys\_enter\_openat), then you have to append “.ustring” to the
field name:
...
The above will filter when the field “call\_site” falls on the address within
“security\_prepare\_creds”. That is, it will compare the value of “call\_site” and
the filter will return true if it is greater than or equal to the start of
the function “security\_prepare\_creds” and less than the end of that function.
...
For example, this will filter events that have their .target\_cpu field present
...
sched subsystem (all events but those that have a prev\_pid field retain
their old filters):
...
The set\_event\_pid file in the same directory as the top events directory
exists, will filter all events from tracing any task that does not have the
PID listed in the set\_event\_pid file.
...
set\_ftrace\_filter ‘ftrace filter commands’ (see the ‘Filter commands’
section of [ftrace - Function Tracer](ftrace.html)), but there are major
differences and the implementation isn’t currently tied to it in any
way, so beware about making generalizations between the two.

Note

Writing into trace\_marker (See [ftrace - Function Tracer](ftrace.html))
...
* enable\_event/disable\_event
  ...
  Note that there can be any number of enable/disable\_event triggers
  per triggering event, but there can only be one trigger per
  triggered event. e.g. sys\_enter\_read can have triggers enabling both
  kmem:kmalloc and sched:sched\_switch, but can’t have two kmem:kmalloc
  versions such as kmem:kmalloc and kmem:kmalloc:1 or ‘kmem:kmalloc if
  bytes\_req == 256’ and ‘kmem:kmalloc if bytes\_alloc == 256’ (they
...
* hist
  ...
  See [Event Histograms](histogram.html) for details and examples.
...
of a lower-level “dynevent\_cmd” event command API, which is also
...
The sched\_fields param in this example points to an array of `struct
...
representing a ‘type field\_name;’ field specification should be
...
initialize a dynevent\_cmd object using `synth_event_cmd_init()`.
...
`synth_event_add_field()`, supplying the dynevent\_cmd object, a field
...
See `synth_field_size()` for available types. If field\_name contains [n]
the field is considered to be an array.

A group of fields can also be added all at once using an array of
synth\_field\_desc with `add_synth_fields()`. For example, this would add
just the first four sched\_fields:

```
ret = synth_event_add_fields(&cmd, sched_fields, 4);
```

If you already have a string of the form ‘type field\_name’,
...
The `synth_event_trace()` function is passed the trace\_event\_file
...
accomplish the same thing. It is passed the trace\_event\_file
representing the synthetic event (which can be retrieved using
`trace_get_event_file()` using the synthetic event name, “synthetic” as
the system name, and the trace instance name (NULL if using the global
...
It’s passed the trace\_event\_file representing the synthetic event
...
same synth\_event\_trace\_state object used in the `synth_event_trace_start()`,
...
used. Each call is passed the same synth\_event\_trace\_state object used in
...
should create and initialize a dynevent\_cmd object using
...
`kprobe_event_add_fields()`, supplying the dynevent\_cmd object along
...
top of a lower-level “dynevent\_cmd” interface. This interface is
...
initialize an instance of a dynevent\_cmd. Here, for instance, we
create a dynevent\_cmd on the stack and initialize it:
...
The dynevent\_cmd initialization needs to be given a user-specified
buffer and the length of the buffer (MAX\_DYNEVENT\_CMD\_LEN can be used
...
together as a unit, for example, a ‘type field\_name;’ arg or a simple
...
Again, the arg\_pair is first initialized, in this case with a callback
...
Any number of dynevent\_\*`_add()` calls can be made to build up the string
...
See the dynevent\_cmd function definitions themselves for the details
of the API.
