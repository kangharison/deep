...
> 출처(원문): https://docs.kernel.org/trace/uprobetracer.html
...
To enable this feature, build your kernel with CONFIG\_UPROBE\_EVENTS=y.

Similar to the kprobe-event tracer, this doesn’t need to be activated via
current\_tracer. Instead of that, add probe points via
/sys/kernel/tracing/uprobe\_events, and enable it via
...
uprobe\_events. That interface will provide unified access to other
dynamic events too.

## Synopsis of uprobe\_tracer
...
/sys/kernel/tracing/uprobe\_profile. The first column is the filename,
the second is the event name, the third is the number of probe hits.

## Usage examples

> * Add a probe as a new uprobe event, write a new definition to uprobe\_events
...
We can see the events that are registered by looking at the uprobe\_events file.
...
Format of events can be seen by viewing the file events/uprobes/zfree\_entry/format.
