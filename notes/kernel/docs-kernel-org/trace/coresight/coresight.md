...
> 출처(원문): https://docs.kernel.org/trace/coresight/coresight.html
...
struct coresight\_device \*coresight\_register(struct coresight\_desc \*desc);

void coresight\_unregister(struct coresight\_device \*csdev);
...
The “coresight\_dev\_type” identifies what the device is, i.e, source link or
sink while the “coresight\_dev\_subtype” will characterise that type further.
...
via the CTM. See ([CoreSight Embedded Cross Trigger (CTI & CTM).](coresight-ect.html))
[[4]](#fourth) for further details.
...
> >
> > cs\_etm// [Kernel PMU event]

Regardless of the number of tracers available in a system (usually equal to the
amount of processor cores), the “cs\_etm” PMU will be listed only once.

A Coresight PMU works the same way as any other PMU, i.e the name of the PMU is
provided along with configuration options within forward slashes ‘/’ (see
[Config option formats](#config-option-formats)).
...
($SYSFS)/bus/event\_source/devices/cs\_etm/sinks/:
...
Execution can be recorded using ‘perf record’ with the cs\_etm event,
...
For a kernel running at EL1, the PID is stored in CONTEXTIDR\_EL1. A PE may
...
> perf record -e cs\_etm/contextid1,contextid2/u -- vm
...
They are also listed in the folder /sys/bus/event\_source/devices/cs\_etm/format/

| Option | Description |
| --- | --- |
| branch\_broadcast | Session local version of the system wide setting: [ETM\_MODE\_BB](coresight-etm4x-reference.html#coresight-branch-broadcast) |
| contextid | See [Tracing PID](#tracing-pid) |
| contextid1 | See [Tracing PID](#tracing-pid) |
| contextid2 | See [Tracing PID](#tracing-pid) |
| configid | Selection for a custom configuration. This is an implementation detail and not used directly, see [Using Configurations in perf](coresight-config.html#using-configurations-in-perf) |
| preset | Override for parameters in a custom configuration, see [Using Configurations in perf](coresight-config.html#using-configurations-in-perf) |
| sinkid | Hashed version of the string to select a sink, automatically set when using the @ notation. This is an internal implementation detail and is not used directly, see [Using perf framework](#using-perf-framework). |
| cycacc | Session local version of the system wide setting: [ETMv4\_MODE\_CYCACC](coresight-etm4x-reference.html#coresight-cycle-accurate) |
| retstack | Session local version of the system wide setting: [ETM\_MODE\_RETURNSTACK](coresight-etm4x-reference.html#coresight-return-stack) |
| timestamp | Controls generation and interval of timestamps.  0 = off, 1 = minimum interval .. 15 = maximum interval.  Values 1 - 14 use a counter that decrements every cycle to generate a timestamp on underflow. The reload value for the counter is 2 ^ (interval - 1). If the value is 1 then the reload value is 1, if the value is 11 then the reload value is 1024 etc.  Setting the maximum interval (15) will disable the counter generated timestamps, freeing the counter resource, leaving only ones emitted when a SYNC packet is generated. The sync interval is controlled with TRCSYNCPR.PERIOD which is every 4096 bytes of trace by default. |
| cc\_threshold | Cycle count threshold value. If nothing is provided here or the provided value is 0, then the default value i.e 0x100 will be used. If provided value is less than minimum cycles threshold value, as indicated via TRCIDR3.CCITMIN, then the minimum value will be used instead. |
...
found in sysfs with more information on each entry being found in [[1]](#first):
...
- [System Trace Module](../stm.html) [[2]](#second).
...
([CoreSight Embedded Cross Trigger (CTI & CTM).](coresight-ect.html)) [[4]](#fourth).
...
([CoreSight System Configuration Manager](coresight-config.html)) [[5]](#fifth).

[[1](#id3)]

[ABI file testing/sysfs-bus-coresight-devices-stm](../../admin-guide/abi-testing-files.html#abi-file-testing-sysfs-bus-coresight-devices-stm)
