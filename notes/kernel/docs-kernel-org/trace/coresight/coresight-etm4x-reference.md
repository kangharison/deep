...
> 출처(원문): https://docs.kernel.org/trace/coresight/coresight-etm4x-reference.html
...
Notes:
:   When FEAT\_TRF is implemented, value of TRFCR\_ELx.TS used for trace session. Otherwise -1
...
Notes:
:   Pair of addresses for a range selected by addr\_idx. Include
...
Notes:
:   Set a single address comparator according to addr\_idx. This
...
Notes:
:   Set a trace start address comparator according to addr\_idx.
...
Notes:
:   Set a trace stop address comparator according to addr\_idx.
    Select comparator in control register.
...
Notes:
:   Link context ID comparator to address comparator addr\_idx
...
:   `echo ctxt_idx > addr_context`
    ...
    Where ctxt\_idx is the index of the linked context id / vmid
    comparator.
...
:   Set to enable branch broadcast if supported in hardware [IDR0]. The primary use for this feature
    ...
    overrides any setting of [ETM\_MODE\_RETURNSTACK](#coresight-return-stack), so where a branch
