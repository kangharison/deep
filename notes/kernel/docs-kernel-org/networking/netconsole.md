...
> 출처(원문): https://docs.kernel.org/networking/netconsole.html
...
To include this feature, select CONFIG\_NETCONSOLE\_DYNAMIC when building the
...
> | dev\_name | Local network interface name | (read-write) |
> | local\_port | Source UDP port to use | (read-write) |
> | remote\_port | Remote agent’s UDP port | (read-write) |
> | local\_ip | Source IP address to use | (read-write) |
> | remote\_ip | Remote agent’s IP address | (read-write) |
> | local\_mac | Local interface’s MAC address | (read-only) |
> | remote\_mac | Remote agent’s MAC address | (read-write) |
> | transmit\_errors | Number of packet send errors | (read-only) |
...
taskname\_enabled under the userdata directory. This file is used to enable
...
Within the netconsole configfs hierarchy, there is a file named release\_enabled
...
Inside the netconsole configfs hierarchy, there is a file called
cpu\_nr under the userdata directory. This file is used to enable or disable
...
When this option is enabled, the netconsole messages will include an additional
line in the userdata field with the format cpu=<cpu\_number>. This allows the
...
Within the netconsole configfs hierarchy, there is a file named msgid\_enabled
...
eventually wrap around after reaching uint32\_t max value, so the message ID is
...
the “console\_loglevel” parameter (on the sender) to only send high
...
dmesg(8) man page and [The kernel’s command-line parameters](../admin-guide/kernel-parameters.html)
for details.
