...
> 출처(원문): https://docs.kernel.org/networking/bonding.html
...
It is critical that either the miimon or arp\_interval and
arp\_ip\_target parameters be specified, otherwise serious network
...
>
> actor\_port\_prio or 3
>
> > The active aggregator is chosen by the highest total sum of
> > actor port priorities across its active ports. Note this
> > priority is actor\_port\_prio, not per port prio, which is
> > used for primary reselect.
>
> The bandwidth, count and actor\_port\_prio selection policies permit
...
> the arp\_ip\_target option.
>
> This behavior can be modified by the arp\_validate option,
> below.
>
...
> arp\_interval is > 0. These are the targets of the NS request
> sent to determine the health of the link to the targets.
...
> that they were generated by an arp\_ip\_target. Since backup slaves
...
> Specifies the quantity of arp\_ip\_targets that must be reachable
> in order for the ARP monitor to consider a slave as being up.
> This option affects only active-backup mode for slaves with
> arp\_validation enabled.
>
> Possible values are:
>
> any or 0
>
> > consider the slave up only when any of the arp\_ip\_targets
> > is reachable
>
> all or 1
>
> > consider the slave up only when all of the arp\_ip\_targets
> > are reachable

arp\_missed\_max

> Specifies the number of arp\_interval monitor checks that must
...
> arp\_missed\_max + 1 times before being marked down).
>
...
> none or 0
>
> > This setting disables fail\_over\_mac, and causes
...
> active or 1
>
> > The “active” fail\_over\_mac policy indicates that the
...
> follow or 2
>
> > The “follow” fail\_over\_mac policy causes the MAC
...
:   Option specifying whether to send LACPDU frames periodically.
    ...
    :   LACPDU frames are sent along the configured links
        periodically. See lacp\_rate for more details.
...
> instance of the bonding driver. E.g., if max\_bonds is 3, and
...
>
> The default value is 100 if arp\_interval is not set.
...
> > policies may be selected via the xmit\_hash\_policy option,
> > described below.
> >
...
> >
> > In tlb\_dynamic\_lb=1 mode; the outgoing traffic is
> > distributed according to the current load (computed
> > relative to the speed) on each slave.
> >
> > In tlb\_dynamic\_lb=0 mode; the load balancing based on
...
> > curr\_active\_slave) while having a unique hardware
> > address for each slave in the bond. If the
> > curr\_active\_slave fails its hardware address is
> > swapped with the new curr\_active\_slave that was
> > chosen.
...
> the rate specified by peer\_notif\_delay if the number is
> greater than 1.
>
> The valid range is 0 - 255; the default value is 1. These options
> affect the active-backup or 802.3ad (broadcast\_neighbor enabled) mode.
...
:   Slave priority. A higher number means higher priority.
    ...
    follows the primary\_reselect rules.
...
> > the active slave.
>
> Changing the primary\_reselect policy via sysfs will cause an
...
> ETHTOOL ioctls and [`netif_carrier_ok()`](kapi.html#c.netif_carrier_ok "netif_carrier_ok") to determine link
> state.
>
> All link state checks are now done with [`netif_carrier_ok()`](kapi.html#c.netif_carrier_ok "netif_carrier_ok").
>
...
> > addresses are first hashed using ipv6\_addr\_hash.
> >
> > This algorithm will place all traffic to a particular
...
> > relies on skb\_flow\_dissect to obtain the header fields
...
Do not alter the UNIQUE or \_nm\_name lines. Remove any other
lines (USERCTL, etc).
...
The line BONDING\_MASTER=’yes’ indicates that the device is a
bonding master device. The only useful value is “yes.”

The contents of BONDING\_MODULE\_OPTS are supplied to the
...
specified in BONDING\_OPTS are identical to the bonding module parameters
except for the arp\_ip\_target field when using versions of initscripts older
...
options via BONDING\_OPTS, it is not necessary to edit
`/etc/modprobe.d/*.conf`.

For even older versions of initscripts that do not support
BONDING\_OPTS, it is necessary to edit /etc/modprobe.d/*.conf, depending upon
...
specifying the appropriate BONDING\_OPTS= in ifcfg-bondX where X is the
...
are the same as for the bonding\_masters file.
...
/sys/class/net/bond0/slave\_eth0 pointing to /sys/class/net/eth0, and
...
exception of arp\_ip\_target, they accept the same values. To see the
...
the lp\_interval is the number of seconds between instances where
...
when the driver initializes (see [HOWTO for multiqueue network device support](multiqueue.html)
for details). If more or less queues are desired the module parameter
tx\_queues can be used to change this value. There is no sysfs parameter
...
The queue\_id for a slave can be set using the command:

```
# echo "eth1:2" > /sys/class/net/bond0/bonding/queue_id
```

Any interface that needs a queue\_id set should set it with multiple calls
like the one above until proper priorities are set for all interfaces. On
distributions that allow configuration via initscripts, multiple ‘queue\_id’
arguments can be added to BONDING\_OPTS to set all needed slave queues.
...
> 1. ad\_actor\_system : You can set a random mac-address that can be used for
...
>    ```
> 2. ad\_actor\_sys\_prio : Randomize the system priority. The default value
...
>    ```
> 3. ad\_user\_port\_key : Use the user portion of the port-key. The default
...
and SNMP functions such as Interface\_Scan\_Next will report that
association.
...
:   This mode is the only mode that will permit a single
    ...
    altering the net.ipv4.tcp\_reordering sysctl parameter. The
...
arp\_interval parameters (described in the module parameters section,
above). In general, miimon monitors the carrier state as sensed by
the underlying network device, and the arp monitor (arp\_interval)
...
the fail\_over\_mac option is enabled, the bonding device’s MAC address is
...
source (named [Linux Ethernet Bonding Driver HOWTO](#)).
