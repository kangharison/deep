...
> 출처(원문): https://docs.kernel.org/networking/net_failover.html
...
The net\_failover driver provides an automated failover mechanism via APIs
...
net\_failover enables hypervisor controlled accelerated datapath to virtio-net
enabled VMs in a transparent manner with no/minimal guest userspace changes.

To support this, the hypervisor needs to enable VIRTIO\_NET\_F\_STANDBY
...
like systemd-networkd, ifupdown, etc, do not understand the ‘net\_failover’
...
net\_failover also enables hypervisor controlled live migration to be supported
