...
> 출처(원문): https://docs.kernel.org/networking/device_drivers/wifi/intel/ipw2100.html
...
>     Note
>
>     ‘debug\_level’ is only enabled if CONFIG\_IPW2100\_DEBUG is turn on.
...
See the Sysfs helper file ‘rf\_kill’ for determining the state of the RF switch
on your system.
...
> | iwpriv eth1 set\_power 0 | Sets power level to AUTO and enables power management if not previously enabled. |
> | iwpriv eth1 set\_power 1-5 | Set the power level as specified, enabling power management if not previously enabled. |
