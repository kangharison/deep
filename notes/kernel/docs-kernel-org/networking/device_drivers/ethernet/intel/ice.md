...
> 출처(원문): https://docs.kernel.org/networking/device_drivers/ethernet/intel/ice.html
...
This driver supports XDP (Express Data Path) and AF\_XDP zero-copy. Note that
...
NOTE: The rx\_bytes value of ethtool does not match the rx\_bytes value of
Netdev, due to the 4-byte CRC being stripped by the device. The difference
between the two rx\_bytes values will be 4 x the number of Rx packets. For
example, if Rx packets are 10 and Netdev (software statistics) displays
rx\_bytes as “X”, then ethtool (hardware statistics) will display rx\_bytes as
...
supported. There are also specific differential input/output pins (TIME\_SYNC,
...
Requires kernel compiled with CONFIG\_GNSS=y or CONFIG\_GNSS=m.
...
* log\_level is a name as described below. Each level includes the
...
specified log\_level or to read the log\_level of all of the modules.
...
To read the log\_level of a specific module (e.g. module ‘general’):

```
# cat /sys/kernel/debug/ice/0000\:18\:00.0/fwlog/modules/general
```

To read the log\_level of all the modules:
...
‘fwlog/nr\_messages’. The range is 1-128 (1 means push every log message, 128
...
‘fwlog/nr\_messages’. An example to set the value is:
...
To change the amount of memory then write to ‘fwlog/log\_size’. The value must be
...
> (rx\_dropped.nic can mean that there is no PCIe bandwidth):
>
> ```
...
> device’s local\_cpulist: `/sys/class/net/<ethX>/device/local_cpulist`.
>
