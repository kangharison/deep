...
> 출처(원문): https://docs.kernel.org/networking/device_drivers/ethernet/ti/icssg_prueth.html
...
(TX\_PRUs). Each one of these runs its own firmware. The firmwares combnined are
...
> * `FW_RX_EOF_SHORT_FRMERR`: Incremented if End Of Frame (EOF) task is scheduled without seeing RX\_B1
