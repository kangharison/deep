...
> 출처(원문): https://docs.kernel.org/networking/device_drivers/ethernet/ti/cpsw_switchdev.html
...
* The new (cpsw\_new.c) driver is operating in dual-emac mode by default, thus
  working as 2 individual network interfaces. Main differences from legacy CPSW
  driver are:

> * optimized promiscuous mode: The P0\_UNI\_FLOOD (both ports) is enabled in
>   addition to ALLMULTI (current port) instead of ALE\_BYPASS.
...
See [ti-cpsw-switch devlink support](../../../devlink/ti-cpsw-switch.html)

## Bridging in dual mac mode

The dual\_mac mode requires two vids to be reserved for internal purposes,
which, by default, equal CPSW Port numbers. As result, bridge has to be
configured in vlan unaware mode or default\_pvid has to be adjusted:
...
marking packets with offload\_fwd\_mark flag unless “ale\_bypass=0”
...
Note. This step is mandatory for bridge/default\_pvid.
...
CPU port mcast\_flooding is always on

Turning flooding on/off on switch ports:
bridge link set dev sw0p1 mcast\_flood on/off
