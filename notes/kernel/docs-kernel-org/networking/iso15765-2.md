...
> 출처(원문): https://docs.kernel.org/networking/iso15765-2.html
...
Unlike the CAN\_RAW socket API, only the ISO-TP data field (the actual payload)
...
* `flags`: modifiers to be applied to the default behaviour of the ISO-TP
  ...
  + `CAN_ISOTP_DYN_FC_PARMS`: enable dynamic update of flow control
    parameters.
* `frame_txtime`: frame transmission time (defined as N\_As/N\_Ar inside the
