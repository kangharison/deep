...
> 출처(원문): https://docs.kernel.org/networking/snmp_counter.html
...
beginning of ip\_rcv function, always be updated together with
...
setting the SO\_KEEPALIVE socket option.
...
counter is using the SO\_LINGER option. Please refer to the SO\_LINGER
...
async. If you use the SO\_LINGER option, set l\_onoff to 1, and l\_linger
...
time is l\_linger seconds. If set l\_onoff to 1 and set l\_linger to 0,
...
and would come to the TIME\_WAIT state finally. When kernel has no
...
the tcp\_mem. Please refer the tcp\_mem section in the [TCP man page](http://man7.org/linux/man-pages/man7/tcp.7.html):

2. the orphan socket count is higher than net.ipv4.tcp\_max\_orphans
...
When a TCP connection comes into FIN\_WAIT\_2 state, instead of waiting
...
Linux kernel TCP stack. By configuring the TCP\_LINGER2 socket option,
...
of the function tcp\_is\_sackblock\_valid in the kernel source code. A
...
updated. Which counter will be updated depends on the undo\_marker flag
of the TCP socket. If the undo\_marker is not set, the TCP stack isn’t
...
will be updated. If the undo\_marker is set, TcpExtTCPDSACKIgnoredOld
will be updated. As implied in its name, it might be an old packet.

## SACK shift

The linux networking stack stores data in sk\_buff struct (skb for
...
frequently. Please find more details in the tcp\_invalid\_ratelimit
...
due to tcp\_invalid\_ratelimit, kernel would update one of below
...
tcp\_invalid\_ratelimit allows, the TCP stack will skip sending ACK and
...
net.ipv4.tcp\_retries1 times, because some middle-boxes may black-hole
...
fastopenq->max\_qlen, the TCP stack will reject the fast open request
and update this counter. When this counter is updated, the TCP stack
won’t update TcpExtTCPFastOpenPassive or
TcpExtTCPFastOpenPassiveFail. The fastopenq->max\_qlen is set by the
TCP\_FASTOPEN socket operation and it could not be larger than
net.core.somaxconn. For example:

setsockopt(sfd, SOL\_TCP, TCP\_FASTOPEN, &qlen, sizeof(qlen));
...
Change tcp\_max\_orphans to a smaller value on client:
...
Type Ctrl-C on client, stop client\_orphan.py.
...
The explanation of the test: after run server\_orphan.py and
client\_orphan.py, we set up 64 connections between server and
client. Run the iptables command, the server will drop all packets from
the client, type Ctrl-C on client\_orphan.py, the system of the client
...
from the client, so all connection on clients would be stuck on FIN\_WAIT\_1
stage, so they will keep as orphan sockets until timeout. We have echo
10 to /proc/sys/net/ipv4/tcp\_max\_orphans, so the client system would
...
approximate count is more than tcp\_max\_orphans, kernel checks the
exact count again. So if the approximate count is less than
tcp\_max\_orphans, but exactly count is more than tcp\_max\_orphans, you
would find TcpExtTCPAbortOnMemory is not increased at all. If
tcp\_max\_orphans is large enough, it won’t occur, but if you decrease
tcp\_max\_orphans to a small value like our test, you might find this
issue. So in our test, the client set up 64 connections although the
tcp\_max\_orphans is 10. If the client only set up 11 connections, we
...
FIN\_WAIT\_1 state finally. So we wait for a few minutes, we could find
...
Run server\_linger.py on server:

```
nstatuser@nstat-b:~$ python3 server_linger.py
```

Run client\_linger.py on client:

```
nstatuser@nstat-a:~$ python3 client_linger.py
```

After run client\_linger.py, check the output of nstat:
...
Save the above code as server\_coalesce.py, and run:

```
python3 server_coalesce.py
```

On the client, save below code as client\_coalesce.py:
...
Prepare on server B, disable send\_redirects for all interfaces:
...
Redirect message to server A, set send\_redirects to 0 will disable
this behavior.
