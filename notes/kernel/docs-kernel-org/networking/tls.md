...
> 출처(원문): https://docs.kernel.org/networking/tls.html
...
After setting the TLS\_TX socket option all application data sent over this
...
MSG\_MORE is passed. MSG\_MORE will delay creation of a record until
MSG\_MORE is not passed, or the maximum record size is reached.
...
call using MSG\_MORE, the MSG\_MORE data is left on the socket buffer.

### Receiving TLS application data

After setting the TLS\_RX socket option, all recv family socket calls
...
using a record of type @record\_type.
...
key to the kernel using the TLS\_TX and TLS\_RX socket options, as for
...
kernel, until the new key has been provided using the TLS\_RX socket
...
Userspace should make sure that the crypto\_info provided has been set
...
* For TLS 1.3, the value should be set to record\_size\_limit - 1, since
