...
> 출처(원문): https://docs.kernel.org/networking/can_ucan_protocol.html
...
UCAN\_COMMAND\_GET\_PROTOCOL\_VERSION
...
| --- | --- | --- |
| stopped | SET\_BITTIMING | stopped |
...
UCAN\_OUT\_TX message). The flag indicates the result of the
...
[CAN Error Handling](#can-ucan-error-handling))
...
ucan\_message\_out value. Each ucan\_message\_out must be aligned to 4
bytes (relative to the start of the data buffer). The mechanism is
same as described in [len field](#can-ucan-in-message-len).
...
`subtype`
:   echo id to be replied within a CAN\_IN\_TX\_COMPLETE message
