...
> 출처(원문): https://docs.kernel.org/networking/oa-tc6-framework.html
...
The include/linux/oa\_tc6.h defines the following functions:

struct oa\_tc6 \*oa\_tc6\_init(struct [spi\_device](../driver-api/spi.html#c.spi_device "spi_device") \*spi, struct [net\_device](kapi.html#c.net_device "net_device") \*netdev)

Initialize OA TC6 lib.

void oa\_tc6\_exit(struct oa\_tc6 \*tc6)

Free allocated OA TC6 lib.

int oa\_tc6\_write\_register(struct oa\_tc6 \*tc6, u32 address, u32 value)

Write a single register in the MAC-PHY.

int oa\_tc6\_write\_registers(struct oa\_tc6 \*tc6, u32 address, u32 value[], u8 length)

Writing multiple consecutive registers starting from @address in the MAC-PHY.
Maximum of 128 consecutive registers can be written starting at @address.

int oa\_tc6\_read\_register(struct oa\_tc6 \*tc6, u32 address, u32 \*value)

Read a single register in the MAC-PHY.

int oa\_tc6\_read\_registers(struct oa\_tc6 \*tc6, u32 address, u32 value[], u8 length)

Reading multiple consecutive registers starting from @address in the MAC-PHY.
Maximum of 128 consecutive registers can be read starting at @address.

netdev\_tx\_t oa\_tc6\_start\_xmit(struct oa\_tc6 \*tc6, struct [sk\_buff](kapi.html#c.sk_buff "sk_buff") \*skb);

The transmit Ethernet frame in the skb is or going to be transmitted through
the MAC-PHY.

int oa\_tc6\_zero\_align\_receive\_frame\_enable(struct oa\_tc6 \*tc6);
