...
> 출처(원문): https://docs.kernel.org/networking/checksum-offloads.html
...
* RX Checksum Offload
* CHECKSUM\_UNNECESSARY conversion
...
checksum defined by the sk\_buff fields skb->csum\_start and skb->csum\_offset.
The device should compute the 16-bit ones-complement checksum (i.e. the
‘IP-style’ checksum) from csum\_start to the end of the packet, and fill in the
result at (csum\_start + csum\_offset).

Because csum\_offset cannot be negative, this ensures that the previous value of
...
skb->csum\_start and skb->csum\_offset as described above, and setting
skb->csum\_not\_inet: see skbuff.h comment (section ‘D’) for more details.
...
A driver declares its offload capabilities in netdev->hw\_features; see
[Netdev features mess and how to get out from it alive](netdev-features.html) for more. Note that a device
which only advertises NETIF\_F\_IP[V6]\_CSUM must still obey the csum\_start and
csum\_offset given in the SKB; if it tries to deduce these itself in hardware
...
`skb_checksum_help()` / skb\_crc32c\_csum\_help functions, as mentioned in
...
means calling skb\_csum\_hwoffload\_help(skb, features).
...
from csum\_start to the end of the packet will be equal to the complement of
...
summing when we get to csum\_start, then add the complement of the 16-bit word
at (csum\_start + csum\_offset).
...
net/ipv4/ip\_gre.c:`build_header()`. It is *not* currently performed when
constructing an IPv6 GRE header; the GRE checksum is computed over the whole
packet in net/ipv6/ip6\_gre.c:`ip6gre_xmit2()`, but it should be possible to use
...
flag VXLAN\_F\_REMCSUM\_TX (per `struct vxlan_rdst`) to indicate that RCO should be
