...
> 출처(원문): https://docs.kernel.org/networking/mptcp.html
...
see [MPTCP Sysfs variables](mptcp-sysctl.html). There are two types: the in-kernel one (`kernel`) where
...
[Family mptcp\_pm netlink specification](../netlink/specs/mptcp_pm.html).
...
see [MPTCP Sysfs variables](mptcp-sysctl.html).
...
* `ENOPROTOOPT` (*Protocol not available*): MPTCP has been disabled using
  `net.mptcp.enabled` sysctl knob; see [MPTCP Sysfs variables](mptcp-sysctl.html).
