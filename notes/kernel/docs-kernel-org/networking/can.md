...
> 출처(원문): https://docs.kernel.org/networking/can.html
...
As described in [Motivation / Why Using the Socket API](#socketcan-motivation) the main goal of SocketCAN is to
...
CAN protocol modules by the SocketCAN core (see [SocketCAN Core Module](#socketcan-core-module)).
...
reflect the correct [[1]](#f1) traffic on the node the loopback of the sent
data has to be performed right after a successful transmission. If
the CAN network interface is not capable of performing the loopback for
some reason the SocketCAN core can do this task as a fallback solution.
See [Local Loopback of Sent Frames](#socketcan-local-loopback2) for details (recommended).

The loopback functionality is enabled by default to reflect standard
networking behaviour for CAN applications. Due to some requests from
the RT-SocketCAN group the loopback optionally may be disabled for each
separate socket. See sockopts from the CAN RAW sockets in [RAW Protocol Sockets with can\_filters (SOCK\_RAW)](#socketcan-raw-sockets).
...
- see [SocketCAN Concept](#socketcan-concept)). After binding (CAN\_RAW) or connecting (CAN\_BCM)
...
used instead of can\_dlc. The deprecated can\_dlc was misleadingly named as
it always contained the plain payload length in bytes and not the so called
‘data length code’ (DLC).

To pass the raw DLC from/to a Classical CAN network device the len8\_dlc
...
bytes of payload (`struct can_frame`) like the CAN\_RAW socket. Therefore e.g.
the CAN\_RAW socket supports a new socket option CAN\_RAW\_FD\_FRAMES that
switches the socket into a mode that allows the handling of CAN FD frames
and Classical CAN frames simultaneously (see [RAW Socket Option CAN\_RAW\_FD\_FRAMES](#socketcan-rawfd)).
...
The `struct canfd_frame` and the existing `struct can_frame` have the can\_id,
...
can\_frame.len are equal and contain a length information and no DLC.
For details about the distinction of CAN and CAN FD capable devices and
the mapping to the bus-relevant data length code (DLC), see [CAN FD (Flexible Data Rate) Driver Support](#socketcan-can-fd-driver).
...
msg->msg\_flags field may contain the following flags:
...
:   set when the frame was sent via the socket it is received on.
    ...
    [Local Loopback of Sent Frames](#socketcan-local-loopback1) and [Local Loopback of Sent Frames](#socketcan-local-loopback2).
    (Note: In order to receive such messages on a RAW socket,
    CAN\_RAW\_RECV\_OWN\_MSGS must be set.)

### RAW Protocol Sockets with can\_filters (SOCK\_RAW)
...
* The loopback of sent CAN frames is enabled (see [Local Loopback of Sent Frames](#socketcan-local-loopback2))
...
#### RAW socket option CAN\_RAW\_FILTER
...
#### RAW Socket Option CAN\_RAW\_ERR\_FILTER

As described in [Network Problem Notifications](#socketcan-network-problem-notifications) the CAN interface driver can generate so
...
#### RAW Socket Option CAN\_RAW\_LOOPBACK

To meet multi user needs the local loopback is enabled by default
(see [Local Loopback of Sent Frames](#socketcan-local-loopback1) for details). But in some embedded use-cases
...
#### RAW socket option CAN\_RAW\_RECV\_OWN\_MSGS
...
filtering as other CAN frames (see [RAW socket option CAN\_RAW\_FILTER](#socketcan-rawfilter)).

#### RAW Socket Option CAN\_RAW\_FD\_FRAMES
...
#### RAW socket option CAN\_RAW\_JOIN\_FILTERS
...
applied (see [RAW socket option CAN\_RAW\_FILTER](#socketcan-rawfilter)).
...
### Broadcast Manager Protocol Sockets (SOCK\_DGRAM)
...
`struct can_frame` as known from the CAN\_RAW socket. Instead a special BCM
...
at the beginning of [RAW Socket Option CAN\_RAW\_FD\_FRAMES](#socketcan-rawfd) and in the include/linux/can.h include. All
...
:   Copies the can\_id from the message header to each
    ...
    can\_id(s) stored for transmission in the subsequent `struct can_frame`(s).
...
CAN\_FD\_FRAME:
:   The CAN frames following the bcm\_msg\_head are `struct canfd_frame`’s
...
The programming API of the CAN\_BCM depends on `struct can_frame` which is
...
bcm\_msg\_head are defined as `struct canfd_frame`:
...
### Connected Transport Protocols (SOCK\_SEQPACKET)

(to be written)

### Unconnected Transport Protocols (SOCK\_DGRAM)
...
modules to subscribe needed CAN IDs (see [Receive Lists](#socketcan-receive-lists)).
...
As described in [Receive Lists](#socketcan-receive-lists) the SocketCAN core uses several filter
...
See e.g. at [Network Devices, the Kernel, and You!](netdevices.html) . The differences
for writing CAN network device driver are described below:

### General Settings

CAN network device drivers can use `alloc_candev_mqs()` and friends instead of
[`alloc_netdev_mqs()`](kapi.html#c.alloc_netdev_mqs "alloc_netdev_mqs"), to automatically take care of CAN-specific setup:
...
As described in [Local Loopback of Sent Frames](#socketcan-local-loopback1) the CAN network device driver should
...
also [Network Problem Notifications](#socketcan-network-problem-notifications)).
...
payload. The representation of this length in can\_frame.len and
...
2. non-ISO compliant (fixed, like the M\_CAN IP core v3.0.1 in m\_can.c)
...
(see [SocketCAN Resources](#socketcan-resources)) there might be further drivers available, also for
