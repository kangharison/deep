...
> 출처(원문): https://docs.kernel.org/networking/pktgen.html
...
Enable CONFIG\_NET\_PKTGEN to compile and build pktgen either in-kernel
...
Which is controlled through procfile /proc/net/pktgen/kpktgend\_X.

Example: /proc/net/pktgen/kpktgend\_0:
...
> * add\_device [DEVICE@NAME](mailto:DEVICE%40NAME) -- adds a single device
> * rem\_device\_all -- remove all associated devices
...
pktgen\_sampleXX scripts and modify them to fit your own needs.

## Interrupt affinity

Note that when adding devices to a specific CPU it is a good idea to
also assign /proc/irq/XX/smp\_affinity so that the TX interrupts are bound
to the same CPU. This reduces cache bouncing when freeing skbs.

Plus using the device flag QUEUE\_MAP\_CPU, which maps the SKBs TX queue
...
you can use “pgset spi SPI\_VALUE” to specify which transformation mode
to employ.
...
However, if the “clone\_skb” or “burst” parameters are configured, the skb
