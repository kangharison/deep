...
> 출처(원문): https://docs.kernel.org/networking/j1939.html
...
* **Dynamic addressing:** Address Claiming in J1939 is time critical.
  ...
  requirement for \_every\_ user space process that communicates via J1939. This
...
* **Transport:** both TP & ETP reuse some PGNs to relay big packets over them.
  ...
  knowing it. The individual TP & ETP sessions \_must\_ be serialized
  ...
  for \_every\_ user space process that communicates via J1939.
...
**Non-Blocking Operation with `MSG\_DONTWAIT`:**

The J1939 stack supports non-blocking operation when used in combination with
the MSG\_DONTWAIT flag. In this mode, the stack attempts to take as much data
...
* **Error handling:** When using MSG\_DONTWAIT, the user must rely on the
  error queue to detect transfer errors. See the **SO\_J1939\_ERRQUEUE** section
...
Destination Address, which is \_not\_ part of the PGN. When communicating a PGN
...
However, **`SO\_J1939\_FILTER` has a higher priority** compared to
...
SO\_J1939\_FILTER. The filters will limit which packets are passed to the
...
the CAP\_NET\_ADMIN capability. These are reserved for high-priority traffic
...
* **``J1939\_EE\_INFO\_TX\_ABORT``**: Transmission abort errors.
* **``J1939\_EE\_INFO\_RX\_RTS``**: Reception of RTS (Request to Send) control
  frames.
* **``J1939\_EE\_INFO\_RX\_DPO``**: Reception of data packets with Data Page Offset
  (DPO).
* **``J1939\_EE\_INFO\_RX\_ABORT``**: Reception abort errors.
...
1. `serr->ee_origin == SO_EE_ORIGIN_TIMESTAMPING`:
   ...
   In this case, the serr->ee\_info field will contain one of the following
   timestamp types:
...
When the J1939\_EE\_INFO\_RX\_RTS message is provided, it will include the
