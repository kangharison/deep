...
> 출처(원문): https://docs.kernel.org/networking/xdp-rx-metadata.html
...
\_\_bpf\_kfunc int bpf\_xdp\_metadata\_rx\_timestamp(const struct xdp\_md \*ctx, u64 \*timestamp)
...
\_\_bpf\_kfunc int bpf\_xdp\_metadata\_rx\_hash(const struct xdp\_md \*ctx, u32 \*hash, enum xdp\_rss\_hash\_type \*rss\_type)
...
The RSS hash type (**rss\_type**) specifies what portion of packet headers NIC
...
\_\_bpf\_kfunc int bpf\_xdp\_metadata\_rx\_vlan\_tag(const struct xdp\_md \*ctx, \_\_be16 \*vlan\_proto, u16 \*vlan\_tci)
...
the AF\_XDP use case, see below).

## AF\_XDP

[AF\_XDP](af_xdp.html) use-case implies that there is a contract between the BPF
...
descriptor does \_not\_ explicitly carry the size of the metadata).
...
xdp\_frame. If these conditions are not met, it returns a negative error. In this
