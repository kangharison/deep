...
> 출처(원문): https://docs.kernel.org/networking/sfp-phylink.html
...
exactly the settings given to it in its [`mac_config()`](kapi.html#c.mac_config "mac_config") function.
The link should be forced up or down appropriately in the
[`mac_link_up()`](kapi.html#c.mac_link_up "mac_link_up") and [`mac_link_down()`](kapi.html#c.mac_link_down "mac_link_down") functions.
...
acknowledge the control word. The [`mac_link_up()`](kapi.html#c.mac_link_up "mac_link_up") and
[`mac_link_down()`](kapi.html#c.mac_link_down "mac_link_down") functions must not force the MAC side link
up and down.
...
1. Optionally split the network driver’s phylib update function into
   ...
   Please see [`mac_link_up()`](kapi.html#c.mac_link_up "mac_link_up") for more information on this.
2. Replace:
...
5. Replace the following functions:
   ...
   | --- | --- |
   | phy\_start(phydev) | phylink\_start(priv->phylink) |
   | phy\_stop(phydev) | phylink\_stop(priv->phylink) |
   | phy\_mii\_ioctl(phydev, ifr, cmd) | phylink\_mii\_ioctl(priv->phylink, ifr, cmd) |
   | phy\_ethtool\_get\_wol(phydev, wol) | phylink\_ethtool\_get\_wol(priv->phylink, wol) |
   | phy\_ethtool\_set\_wol(phydev, wol) | phylink\_ethtool\_set\_wol(priv->phylink, wol) |
   | phy\_disconnect(phydev) | phylink\_disconnect\_phy(priv->phylink) |
...
7. Replace the call to:
   ...
   the [`phy_attach_direct()`](kapi.html#c.phy_attach_direct "phy_attach_direct") inside this function call if a PHY is specified
   in the DT node `node`.
   ...
   dynamic.
8. Add a [`struct phylink_mac_ops`](kapi.html#c.phylink_mac_ops "phylink_mac_ops") instance to
   the driver, which is a table of function pointers, and implement
   these functions. The old link update function for
   `of_phy_connect()` becomes three methods: [`mac_link_up()`](kapi.html#c.mac_link_up "mac_link_up"),
   [`mac_link_down()`](kapi.html#c.mac_link_down "mac_link_down"), and [`mac_config()`](kapi.html#c.mac_config "mac_config"). If step 1 was
   performed, then the functionality will have been split there.

   It is important that if in-band negotiation is used,
   [`mac_link_up()`](kapi.html#c.mac_link_up "mac_link_up") and [`mac_link_down()`](kapi.html#c.mac_link_down "mac_link_down") do not prevent the
   ...
   The [`mac_get_caps()`](kapi.html#c.mac_get_caps "mac_get_caps") method is optional, and if provided should
   ...
   The [`mac_link_up()`](kapi.html#c.mac_link_up "mac_link_up") method is used to inform the MAC that the
   ...
   The [`mac_config()`](kapi.html#c.mac_config "mac_config") method is used to update the MAC with the
   ...
   documentation in [`struct phylink_mac_ops`](kapi.html#c.phylink_mac_ops "phylink_mac_ops").
9. Fill-in the [`struct phylink_config`](kapi.html#c.phylink_config "phylink_config") fields with
   a reference to the [`struct device`](../driver-api/infrastructure.html#c.device "device") associated to your
   [`struct net_device`](kapi.html#c.net_device "net_device"):
...
10. Some Ethernet controllers work in pair with a PCS (Physical Coding Sublayer)
    ...
    provides a PCS abstraction through [`struct phylink_pcs`](kapi.html#c.phylink_pcs "phylink_pcs").
    ...
    one [`struct phylink_pcs`](kapi.html#c.phylink_pcs "phylink_pcs") instance per PCS block within
    ...
    Populate the relevant [`struct phylink_pcs_ops`](kapi.html#c.phylink_pcs_ops "phylink_pcs_ops") to
    configure your PCS. Create a [`pcs_get_state()`](kapi.html#c.pcs_get_state "pcs_get_state") function that reports
    the inband link state, a [`pcs_config()`](kapi.html#c.pcs_config "pcs_config") function to configure your
    PCS according to phylink-provided parameters, and a [`pcs_validate()`](kapi.html#c.pcs_validate "pcs_validate")
...
11. If your controller relies on, or accepts the presence of an external PCS
    controlled through its own driver, add a pointer to a phylink\_pcs instance
    ...
    corresponding [`struct mii_bus`](kapi.html#c.mii_bus "mii_bus") and the PCS’s address on
    ...
    ```
12. Populate the [`mac_select_pcs()`](kapi.html#c.mac_select_pcs "mac_select_pcs") callback and add it to your
    [`struct phylink_mac_ops`](kapi.html#c.phylink_mac_ops "phylink_mac_ops") set of ops. This function
    must return a pointer to the relevant [`struct phylink_pcs`](kapi.html#c.phylink_pcs "phylink_pcs")
    ...
    internal PCS.
13. Fill-in all the [`phy_interface_t`](kapi.html#c.phy_interface_t "phy_interface_t") (i.e. all MAC to
    ...
    phy_interface_set_rgmii(priv->phylink_config.supported_interfaces);
     ...
     __set_bit(PHY_INTERFACE_MODE_1000BASEX,
               priv->phylink_config.supported_interfaces);
    ```
14. Remove calls to [`of_parse_phandle()`](../devicetree/kernel-api.html#c.of_parse_phandle "of_parse_phandle") for the PHY,
...
16. Verify that the driver does not call:
    ...
    and cause phylink to omit calls via the [`mac_link_up()`](kapi.html#c.mac_link_up "mac_link_up") and
    [`mac_link_down()`](kapi.html#c.mac_link_down "mac_link_down") methods.

Network drivers should call [`phylink_stop()`](kapi.html#c.phylink_stop "phylink_stop") and [`phylink_start()`](kapi.html#c.phylink_start "phylink_start") via their
suspend/resume paths, which ensures that the appropriate
[`struct phylink_mac_ops`](kapi.html#c.phylink_mac_ops "phylink_mac_ops") methods are called
as necessary.
