...
> 출처(원문): https://docs.kernel.org/networking/tcp_ao.html
...
always have one current\_key [3.3]:
...
peer and/or sndid/rcvid, VRF L3 interface or get current\_key/rnext\_key.
...
A tip here for scaled TCP\_LISTEN sockets, that may have some thousands TCP-AO
...
may be added to TCP\_CLOSED or TCP\_LISTEN sockets. It’s not allowed to add
...
sne\_flag when the current SEQ number rolls over. The flag is cleared when
both current and previous SEQ numbers cross 0x7fff, which is 32Kb.

In times when sne\_flag is set, the algorithm compares SEQ for each packet with
...
the first SEQ byte that ACK is wanted for (snd\_una) and the next SEQ byte that
is wanted (rcv\_nxt) - that’s enough information for a rough estimation
...
from the segment with snd\_una or rcv\_nxt and fits the result into a 2GB window around them,
