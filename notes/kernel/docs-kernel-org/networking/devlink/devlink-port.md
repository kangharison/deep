...
> 출처(원문): https://docs.kernel.org/networking/devlink/devlink-port.html
...
devlink port function set hw\_addr command. For Ethernet port function
...
devlink port function set ipsec\_crypto command.

Users may also set the IPsec packet capability of the function using
devlink port function set ipsec\_packet command.

Users may also set the maximum IO event queues of the function
using devlink port function set max\_io\_eqs command.
...
A subfunction device is created on the [Documentation/driver-api/auxiliary\_bus.rst](../../driver-api/auxiliary_bus.html#auxiliary-bus).
...
:   Allow users to set the bandwidth allocation per traffic class on rate
    ...
    When applied to a non-leaf node, tc\_bw determines how bandwidth is shared
