...
> 출처(원문): https://docs.kernel.org/networking/kcm.html
...
KCM supports SOCK\_DGRAM and SOCK\_SEQPACKET socket types.
...
SO\_RCVBUF). If the length of a new message reported by the BPF program is
...
by SO\_RCVTIMEO). If the timer expires before assembly is complete an error
...
* type is either SOCK\_DGRAM or SOCK\_SEQPACKET
* protocol is KCMPROTO\_CONNECTED
...
The kcm\_attach structure contains:

> * fd: file descriptor for TCP socket being attached
> * bpf\_prog\_fd: file descriptor for compiled BPF program downloaded

### Unattach transport sockets

Unattaching a transport socket from a multiplexor is straightforward. An
“unattach” ioctl is done with the kcm\_unattach structure as the argument:
...
processing thread and inserted into the epoll (similar to how SO\_REUSEPORT
...
>    except the last have MSG\_BATCH in the flags of sendmsg call.
