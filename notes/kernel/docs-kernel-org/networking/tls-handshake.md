...
> 출처(원문): https://docs.kernel.org/networking/tls-handshake.html
...
SOL\_TLS socket options. The handshake agent returns the socket to the
...
The @ta\_sock field references an open and connected socket. The consumer
...
instantiated a [`struct file`](../filesystems/api-summary.html#c.file "file") in sock->file.

@ta\_done contains a callback function that is invoked when the handshake
has completed. Further explanation of this function is in the “Handshake
Completion” sesction below.

The consumer can provide a NUL-terminated hostname in the @ta\_peername
...
The consumer can fill in the @ta\_timeout\_ms field to force the servicing
...
the handshake agent’s process keyring in the @ta\_keyring field to prevent
access of those keys by other subsystems.

To request an x.509-authenticated TLS session, the consumer fills in
the @ta\_my\_cert and @ta\_my\_privkey fields with the serial numbers of
...
zero return guarantees the callback function @ta\_done will be invoked
...
@ta\_done will not be invoked on this socket.
...
However, in this case, the consumer fills in the @ta\_my\_peerids array
with serial numbers of keys containing the peer identities it wishes
to offer, and the @ta\_num\_peerids field with the number of array
...
the consumer’s handshake completion callback, provided in the @ta\_done
field in the tls\_handshake\_args structure, is invoked.
...
The consumer provides a cookie in the @ta\_data field of the
tls\_handshake\_args structure that is returned in the @data parameter of
...
remote peer’s identity or the value TLS\_NO\_PEERID if the session is not
authenticated.
...
socket’s sk\_data\_ready callback function to ignore all incoming data.
...
subsequent [`sock_recvmsg()`](kapi.html#c.sock_recvmsg "sock_recvmsg"). Each control message indicates whether the
received message data is TLS record data or session metadata.

See [Kernel TLS](tls.html) for details on how a kTLS consumer recognizes incoming
