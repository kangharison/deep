...
> 출처(원문): https://docs.kernel.org/networking/dsa/bcm_sf2.html
...
* `SWITCH_MDIO`: external MDIO bus controller (there is another one in SWITCH\_CORE,
...
device\_node pointers which are then accessible by the switch driver setup
function to setup resources such as register ranges and interrupts. This
currently works very well because none of the of\_\* functions utilized by the
driver require a [`struct device`](../../driver-api/infrastructure.html#c.device "device") to be bound to a `struct device_node`, but things
