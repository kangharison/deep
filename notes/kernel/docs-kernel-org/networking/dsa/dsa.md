...
> 출처(원문): https://docs.kernel.org/networking/dsa/dsa.html
...
driver declares NETIF\_F\_HW\_CSUM in vlan\_features and looks at csum\_start and
csum\_offset. For those cases, DSA will shift the checksum start and offset by
the tag size. If the DSA conduit driver still uses the legacy NETIF\_F\_IP\_CSUM
or NETIF\_F\_IPV6\_CSUM in vlan\_features, the offload might only work if the
...
These user network devices have custom net\_device\_ops and ethtool\_ops function
...
* `dsa_switch_tree`: structure assigned to the conduit network device under
  `dsa_ptr`, this structure references a dsa\_platform\_data structure as well as
  the tagging protocol supported by the switch tree, and which receive/transmit
  function hooks should be invoked, information about the directly attached
  switch is also provided: CPU port. Finally, a collection of dsa\_switch are
...
* `dsa_switch`: structure describing a switch device in the tree, referencing
  ...
  device, and a reference to the backing``dsa\_switch\_ops``
...
Once a conduit network device is configured to use DSA (dev->dsa\_ptr becomes
...
* `set_wol`: ethtool function used to configure Wake-on-LAN settings per-port,
  direct counterpart to set\_wol with similar restrictions
...
* `port_enable`: function invoked by the DSA user network device ndo\_open
...
* `port_disable`: function invoked by the DSA user network device ndo\_close
