...
> 출처(원문): https://docs.kernel.org/networking/tls-offload.html
...
documentation in [Documentation/networking/tls.rst](tls.html#kernel-tls).
...
[`struct sk_buff`](kapi.html#c.sk_buff "sk_buff"). The packets reach the TCP stack and
...
packets requiring HW offload, see the [5-tuple matching limitations](#tuple-problems) section)
...
the `decrypted` mark in [`struct sk_buff`](kapi.html#c.sk_buff "sk_buff")
...
added to the device table and are in TLS\_HW mode. For example,
...
future logged ones. It also clears the RESYNC\_REQ\_ASYNC flag from the resync
