...
> 출처(원문): https://docs.kernel.org/networking/cdc_mbim.html
> 자동 미러링: docs.kernel.org · 정본은 출처 URL (영문 원문 자동 변환본)

---

# cdc\_mbim - Driver for CDC MBIM Mobile Broadband modems

The cdc\_mbim driver supports USB devices conforming to the “Universal
...
functions will be handled by either the cdc\_ncm driver or the cdc\_mbim
driver depending on the prefer\_mbim setting. Setting prefer\_mbim=N
makes the cdc\_mbim driver ignore these functions and lets the cdc\_ncm
...
MBIM functions are inactive when unmanaged. The cdc\_mbim driver only
...
The cdc\_mbim driver does not process or police messages on the control
...
IOCTL\_WDM\_MAX\_COMMAND: Get Maximum Command Size
...
services. This is fully supported by the cdc\_mbim driver.
...
The cdc\_mbim driver represents the MBIM data channel as a single
...
channel. The cdc\_mbim driver models such IP sessions as 802.1q VLAN
subdevices of the master wwanY device, mapping MBIM IP session Z to
VLAN ID Z for all values of Z greater than 0.

The device maximum Z is given in the MBIM\_DEVICE\_CAPS\_INFO structure
...
the same shared USB data channel. The cdc\_mbim driver models these
...
The device maximum A is given in the MBIM\_DEVICE\_SERVICES\_INFO
...
> * RX frames will have the protocol field set to ETH\_P\_802\_3 (but will
...
Summarizing the cdc\_mbim driver mapping described above, we have this
...
>
>    > * [The Linux-USB Host Side API](../driver-api/usb/usb.html)
> 7. “/sys/bus/usb/devices/.../descriptors”
>
>    > * [ABI file stable/sysfs-bus-usb](../admin-guide/abi-stable-files.html#abi-file-stable-sysfs-bus-usb)
