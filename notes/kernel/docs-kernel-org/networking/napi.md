...
> 출처(원문): https://docs.kernel.org/networking/napi.html
...
The name NAPI no longer stands for anything in particular [[1]](#id3).
...
interrupts first ([busy polling](#poll)).

NAPI processing usually happens in the software interrupt context,
but there is an option to use [separate kernel threads](#threaded)
for NAPI processing.
...
[`netif_napi_add()`](kapi.html#c.netif_napi_add "netif_napi_add") and [`netif_napi_del()`](kapi.html#c.netif_napi_del "netif_napi_del") add/remove a NAPI instance
...
[`napi_enable()`](kapi.html#c.napi_enable "napi_enable") and [`napi_disable()`](kapi.html#c.napi_disable "napi_disable") manage the disabled state.
A disabled NAPI can’t be scheduled and its poll method is guaranteed
to not be invoked. [`napi_disable()`](kapi.html#c.napi_disable "napi_disable") waits for ownership of the NAPI
...
calling [`napi_disable()`](kapi.html#c.napi_disable "napi_disable") multiple times in a row will deadlock.

### Datapath API

[`napi_schedule()`](kapi.html#c.napi_schedule "napi_schedule") is the basic method of scheduling a NAPI poll.
Drivers should call this function in their interrupt handler
(see [Scheduling and IRQ masking](#drv-sched) for more info). A successful call to [`napi_schedule()`](kapi.html#c.napi_schedule "napi_schedule")
...
processed) the poll method should call [`napi_complete_done()`](kapi.html#c.napi_complete_done "napi_complete_done")
before returning. [`napi_complete_done()`](kapi.html#c.napi_complete_done "napi_complete_done") releases the ownership
of the instance.
...
not call [`napi_complete_done()`](kapi.html#c.napi_complete_done "napi_complete_done") and wait to be called again,
or return `budget - 1`.

If the `budget` is 0 [`napi_complete_done()`](kapi.html#c.napi_complete_done "napi_complete_done") should never be called.
...
if [`napi_schedule()`](kapi.html#c.napi_schedule "napi_schedule") succeeded (e.g. if the instance gets disabled).

As mentioned in the [Control API](#drv-ctrl) section - [`napi_disable()`](kapi.html#c.napi_disable "napi_disable") and subsequent
...
[`napi_complete_done()`](kapi.html#c.napi_complete_done "napi_complete_done").
...
to IRQ being auto-masked by the device) should use the [`napi_schedule_prep()`](kapi.html#c.napi_schedule_prep "napi_schedule_prep")
and [`__napi_schedule()`](kapi.html#c.__napi_schedule "__napi_schedule") calls:
...
IRQ should only be unmasked after a successful call to [`napi_complete_done()`](kapi.html#c.napi_complete_done "napi_complete_done"):
...
[`napi_schedule_irqoff()`](kapi.html#c.napi_schedule_irqoff "napi_schedule_irqoff") is a variant of [`napi_schedule()`](kapi.html#c.napi_schedule "napi_schedule") which takes advantage
of guarantees given by being invoked in IRQ context (no need to
mask interrupts). [`napi_schedule_irqoff()`](kapi.html#c.napi_schedule_irqoff "napi_schedule_irqoff") will fall back to [`napi_schedule()`](kapi.html#c.napi_schedule "napi_schedule") if
...
The [`netif_napi_add_config()`](kapi.html#c.netif_napi_add_config "netif_napi_add_config") API prevents this loss of configuration by
...
Drivers should try to use [`netif_napi_add_config()`](kapi.html#c.netif_napi_add_config "netif_napi_add_config") whenever possible.
...
completely suspend IRQs. It is used in combination with SO\_PREFER\_BUSY\_POLL,
...
`net.core.busy_read` sysctls. An io\_uring API for NAPI busy polling
also exists. Threaded polling of NAPI also has a mode to busy poll for
packets ([threaded busy polling](#threaded-busy-poll)) using the NAPI
processing kthread.
...
the NAPI ID of the incoming connection using SO\_INCOMING\_NAPI\_ID and then
...
Alternatively, if the application uses SO\_REUSEPORT, a bpf or ebpf program can
...
1. `/proc/sys/net/core/busy_poll` can be set with a time in useconds to busy
   ...
   epoll-based applications to busy poll when they call epoll\_wait. This may
...
applications and especially applications using AF\_XDP sockets) may not
...
While application calls to epoll\_wait successfully retrieve events, the kernel will
...
>    call to epoll\_wait, noting that applications can control how much data
>    they retrieve by setting `max_events` when calling epoll\_wait.
...
As mentioned above, as long as subsequent calls to epoll\_wait return events to
...
Once a call to epoll\_wait results in no events being found, IRQ suspension is
