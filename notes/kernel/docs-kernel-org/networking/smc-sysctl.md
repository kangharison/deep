...
> 출처(원문): https://docs.kernel.org/networking/smc-sysctl.html
...
:   Setting SMC auto corking size:
    ...
    sent.
    autocorking\_size limits the maximum corked bytes that can be sent to
    the under device in 1 single sending. If set to 0, the SMC auto corking
    is disabled.
    Applications can still use TCP\_CORK for optimal behavior when they
    know how/when to uncork their sockets.

    Default: 64K

smcr\_buf\_type - INTEGER
...
smcr\_testlink\_time - INTEGER
:   How frequently SMC-R link sends out TEST\_LINK LLC messages to confirm
    viability, after the last activity of connections on it. Value 0 means
    disabling TEST\_LINK.
...
smcr\_max\_links\_per\_lgr - INTEGER
...
:   So-called work request buffers are SMCR link (and RDMA queue pair) level
    ...
    of SMC\_WR\_BUF\_SIZE (48) bytes. If the allocation fails, we keep retrying
...
:   Select the SMC handshake control profile by name.
    ...
    BPF struct\_ops instance of type smc\_hs\_ctrl.

    The selected profile controls whether SMC options are advertised
    during TCP SYN/SYN-ACK handshake.

    Only available when CONFIG\_SMC\_HS\_CTRL\_BPF is enabled.
