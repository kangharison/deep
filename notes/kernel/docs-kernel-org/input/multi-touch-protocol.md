...
> 출처(원문): https://docs.kernel.org/input/multi-touch-protocol.html
...
`input_mt_sync()` at the end of each packet. This generates a SYN\_MT\_REPORT
event, which instructs the receiver to accept the data for the current
contact and prepare to receive another.

Drivers for type B devices separate contact packets by calling
[`input_mt_slot()`](../driver-api/input.html#c.input_mt_slot "input_mt_slot"), with a slot as argument, at the beginning of each packet.
...
important. Event filtering and finger tracking is left to user space [[3]](#f3).
...
use\_count to false when calling [`input_mt_report_pointer_emulation()`](../driver-api/input.html#c.input_mt_report_pointer_emulation "input_mt_report_pointer_emulation").
...
may be used to track identified contacts over time [[5]](#f5).

In the type B protocol, ABS\_MT\_TOOL\_TYPE and ABS\_MT\_TRACKING\_ID are
implicitly handled by input core; drivers should instead call
[`input_mt_report_slot_state()`](../driver-api/input.html#c.input_mt_report_slot_state "input_mt_report_slot_state").
...
:   The length of the major axis of the contact. The length should be given in
    ...
    possible value of ABS\_MT\_TOUCH\_MAJOR is sqrt(X^2 + Y^2), the diagonal [[4]](#f4).

ABS\_MT\_TOUCH\_MINOR
:   The length, in surface units, of the minor axis of the contact. If the
    contact is circular, this event can be omitted [[4]](#f4).

ABS\_MT\_WIDTH\_MAJOR
:   The length, in surface units, of the major axis of the approaching
    tool. This should be understood as the size of the tool itself. The
    orientation of the contact and the approaching tool are assumed to be the
    same [[4]](#f4).

ABS\_MT\_WIDTH\_MINOR
:   The length, in surface units, of the minor axis of the approaching
    tool. Omit if circular [[4]](#f4).
...
:   The pressure, in arbitrary units, on the contact area. May be used instead
    ...
    [Input event codes](event-codes.html#input-event-codes) for details.
...
:   The orientation of the touching ellipse. The value should describe a signed
    ...
    ABS\_MT\_ORIENTATION should be [0, 1] [[4]](#f4).
...
:   The surface Y coordinate of the center of the approaching tool. Omit if the
    ...
    tool axis points towards the touch point [[1]](#f1). Otherwise, the tool axes are
...
:   The type of approaching tool. A lot of kernel drivers cannot distinguish
    ...
    MT\_TOOL\_FINGER, MT\_TOOL\_PEN, and MT\_TOOL\_PALM [[2]](#f2).
    For type B devices, this event is handled by input core; drivers should
    instead use [`input_mt_report_slot_state()`](../driver-api/input.html#c.input_mt_report_slot_state "input_mt_report_slot_state"). A contact’s ABS\_MT\_TOOL\_TYPE may
...
:   The BLOB\_ID groups several packets together into one arbitrarily shaped
    ...
    should not be confused with the high-level trackingID [[5]](#f5). Most type A
    devices do not have blob capability, so drivers can safely omit this event.

ABS\_MT\_TRACKING\_ID
:   The TRACKING\_ID identifies an initiated contact throughout its life cycle
    [[5]](#f5). The value range of the TRACKING\_ID should be large enough to ensure
    unique identification of a contact maintained over an extended period of
    time. For type B devices, this event is handled by input core; drivers
    should instead use [`input_mt_report_slot_state()`](../driver-api/input.html#c.input_mt_report_slot_state "input_mt_report_slot_state").
...
implementation can be found in [[3]](#f3).
...
[4]
([1](#id4),[2](#id5),[3](#id6),[4](#id7),[5](#id8))

See the section on event computation.


[5]
([1](#id1),[2](#id3),[3](#id11),[4](#id12))
