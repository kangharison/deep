...
> 출처(원문): https://docs.kernel.org/input/input-programming.html
...
Then it allocates a new input device structure with [`input_allocate_device()`](../driver-api/input.html#c.input_allocate_device "input_allocate_device")
...
device has appeared. [`input_register_device()`](../driver-api/input.html#c.input_register_device "input_register_device") may sleep and therefore must
...
KEY\_MAX). Value is interpreted as a truth value, i.e. any non-zero value means
...
## 1.6. BITS\_TO\_LONGS(), BIT\_WORD(), BIT\_MASK()
...
autorepeat for your device, just set EV\_REP in dev->evbit. All will be
...
Within the callback, devices should use the regular input\_report\_\* functions
and input\_sync as is used by other devices.
