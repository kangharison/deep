...
> 출처(원문): https://docs.kernel.org/input/joydev/joystick-api.html
...
driver now reports only any changes of its state. See [Programming Interface](#),
...
>    man page.
> 2. open the device in non-blocking mode (O\_NONBLOCK)
...
information for all axes. That is, `struct js_corr` corr[MAX\_AXIS];
