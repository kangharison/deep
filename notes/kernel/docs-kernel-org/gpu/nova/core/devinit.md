...
> 출처(원문): https://docs.kernel.org/gpu/nova/core/devinit.html
...
FWSEC (Firmware Security) image (see [FWSEC (Firmware Security)](fwsec.html) and [VBIOS](vbios.html)) and it runs before the
...
1. Located in the VBIOS ROM in the same ROM partition (see [VBIOS](vbios.html) and [FWSEC (Firmware Security)](fwsec.html)).
...
indicating that core initialization is complete (known as GFW\_BOOT). This signal is
...
when receiving the GFW\_BOOT signal.
