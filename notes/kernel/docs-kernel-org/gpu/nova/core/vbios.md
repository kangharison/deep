...
> 출처(원문): https://docs.kernel.org/gpu/nova/core/vbios.html
...
* Second FwSec Image (Type 0xE0) - The second FwSec image (Secure Firmware)
  ...
  loads (see [Device Initialization (devinit)](devinit.html)). The DEVINIT ucode is itself another ucode that is
...
tokens for a token with the id BIT\_TOKEN\_ID\_FALCON\_DATA (0x70) which indicates the
