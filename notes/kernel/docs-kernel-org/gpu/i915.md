...
> 출처(원문): https://docs.kernel.org/gpu/i915.html
...
intel\_wakeref\_t intel\_runtime\_pm\_get\_raw(struct intel\_runtime\_pm \*rpm)
:   grab a raw runtime pm reference

**Parameters**

`struct intel_runtime_pm *rpm`
:   the intel\_runtime\_pm structure
...
call to [`intel_runtime_pm_put_raw()`](#c.intel_runtime_pm_put_raw "intel_runtime_pm_put_raw") to release the reference again.

**Return**

the wakeref cookie to pass to [`intel_runtime_pm_put_raw()`](#c.intel_runtime_pm_put_raw "intel_runtime_pm_put_raw"), evaluates
as True if the wakeref was acquired, or False otherwise.

intel\_wakeref\_t intel\_runtime\_pm\_get(struct intel\_runtime\_pm \*rpm)
...
Any runtime pm reference obtained by this function must have a symmetric
call to [`intel_runtime_pm_put()`](#c.intel_runtime_pm_put "intel_runtime_pm_put") to release the reference again.

**Return**

the wakeref cookie to pass to [`intel_runtime_pm_put()`](#c.intel_runtime_pm_put "intel_runtime_pm_put")

intel\_wakeref\_t \_\_intel\_runtime\_pm\_get\_if\_active(struct intel\_runtime\_pm \*rpm, bool ignore\_usecount)
...
:   get a ref even if dev->power.usage\_count is 0
...
If **ignore\_usecount** is true, a reference will be acquired even if there is no
user requiring the device to be powered up (dev->power.usage\_count == 0).
...
the wakeref cookie to pass to [`intel_runtime_pm_put()`](#c.intel_runtime_pm_put "intel_runtime_pm_put"), evaluates
as True if the wakeref was acquired, or False otherwise.

intel\_wakeref\_t intel\_runtime\_pm\_get\_noresume(struct intel\_runtime\_pm \*rpm)
:   grab a runtime pm reference
...
It will \_not\_ resume the device but instead only get an extra wakeref.
...
void intel\_runtime\_pm\_put\_raw(struct intel\_runtime\_pm \*rpm, intel\_wakeref\_t wref)
...
[`intel_runtime_pm_get_raw()`](#c.intel_runtime_pm_get_raw "intel_runtime_pm_get_raw") and might power down the corresponding
hardware block right away if this is the last reference.

void intel\_runtime\_pm\_put\_unchecked(struct intel\_runtime\_pm \*rpm)
...
This function drops the device-level runtime pm reference obtained by
[`intel_runtime_pm_get()`](#c.intel_runtime_pm_get "intel_runtime_pm_get") and might power down the corresponding
hardware block right away if this is the last reference.

This function exists only for historical reasons and should be avoided in
new code, as the correctness of its use cannot be checked. Always use
[`intel_runtime_pm_put()`](#c.intel_runtime_pm_put "intel_runtime_pm_put") instead.
...
to be kept awake so the **fw\_domains** would be then FORCEWAKE\_ALL.
...
This function is a wrapper around [`intel_uncore_forcewake_get()`](#c.intel_uncore_forcewake_get "intel_uncore_forcewake_get") to acquire
...
This function complements [`intel_uncore_forcewake_user_get()`](#c.intel_uncore_forcewake_user_get "intel_uncore_forcewake_user_get") and releases
...
See [`intel_uncore_forcewake_get()`](#c.intel_uncore_forcewake_get "intel_uncore_forcewake_get"). This variant places the onus
on the caller to explicitly handle the dev\_priv->uncore.lock spinlock.
...
domains obtained by [`intel_uncore_forcewake_get()`](#c.intel_uncore_forcewake_get "intel_uncore_forcewake_get").
...
See [`intel_uncore_forcewake_put()`](#c.intel_uncore_forcewake_put "intel_uncore_forcewake_put"). This variant places the onus
on the caller to explicitly handle the dev\_priv->uncore.lock spinlock.

int \_\_intel\_wait\_for\_register\_fw(struct intel\_uncore \*uncore, i915\_reg\_t reg, u32 mask, u32 value, unsigned int fast\_timeout\_us, unsigned int slow\_timeout\_ms, u32 \*out\_value)
...
Otherwise, the wait will timeout after **slow\_timeout\_ms** milliseconds.
For atomic context **slow\_timeout\_ms** must be zero and **fast\_timeout\_us**
...
int \_\_intel\_wait\_for\_register(struct intel\_uncore \*uncore, i915\_reg\_t reg, u32 mask, u32 value, unsigned int fast\_timeout\_us, unsigned int slow\_timeout\_ms, u32 \*out\_value)
:   wait until register matches expected state
...
enum forcewake\_domains intel\_uncore\_forcewake\_for\_reg(struct intel\_uncore \*uncore, i915\_reg\_t reg, unsigned int op)
...
void intel\_irq\_suspend(struct drm\_i915\_private \*i915)
:   Suspend interrupts
...
void intel\_irq\_resume(struct drm\_i915\_private \*i915)
:   Resume interrupts
...
void intel\_vgpu\_detect(struct drm\_i915\_private \*dev\_priv)
:   detect virtual GPU
...
void intel\_vgt\_deballoon(struct i915\_ggtt \*ggtt)
...
int intel\_vgt\_balloon(struct i915\_ggtt \*ggtt)
...
* Workaround batchbuffers: buffers that get executed automatically by the
  ...
  1. INDIRECT\_CTX: A batchbuffer and an offset are provided in the default
     context, pointing the hardware to jump to that location when that offset
     is reached in the context restore. Workaround batchbuffer in the driver
     currently uses this mechanism for all platforms.
  2. BB\_PER\_CTX\_PTR: A batchbuffer is provided in the default context,
...
frontbuffer slots through [`intel_frontbuffer_track()`](#c.intel_frontbuffer_track "intel_frontbuffer_track"). The functions in this
...
bool intel\_frontbuffer\_invalidate(struct intel\_frontbuffer \*front, enum fb\_op\_origin origin)
...
void intel\_frontbuffer\_flush(struct intel\_frontbuffer \*front, enum fb\_op\_origin origin)
...
void frontbuffer\_flush(struct intel\_display \*display, unsigned int frontbuffer\_bits, enum fb\_op\_origin origin)
:   flush frontbuffer
...
void intel\_frontbuffer\_flip(struct intel\_display \*display, unsigned frontbuffer\_bits)
...
void intel\_frontbuffer\_queue\_flush(struct intel\_frontbuffer \*front)
...
void intel\_frontbuffer\_track(struct intel\_frontbuffer \*old, struct intel\_frontbuffer \*new, unsigned int frontbuffer\_bits)
...
This updates the frontbuffer tracking bits **frontbuffer\_bits** by clearing them
...
bool intel\_set\_cpu\_fifo\_underrun\_reporting(struct intel\_display \*display, enum [pipe](#c.intel_set_cpu_fifo_underrun_reporting "pipe") pipe, bool enable)
...
bool intel\_set\_pch\_fifo\_underrun\_reporting(struct intel\_display \*display, enum pipe pch\_transcoder, bool enable)
...
void intel\_cpu\_fifo\_underrun\_irq\_handler(struct intel\_display \*display, enum [pipe](#c.intel_cpu_fifo_underrun_irq_handler "pipe") pipe)
...
void intel\_pch\_fifo\_underrun\_irq\_handler(struct intel\_display \*display, enum pipe pch\_transcoder)
...
void intel\_check\_cpu\_fifo\_underruns(struct intel\_display \*display)
...
void intel\_check\_pch\_fifo\_underruns(struct intel\_display \*display)
...
implement legacy plane updates (i.e., drm\_plane->`update_plane()` and
drm\_plane->`disable_plane()`). This allows plane updates to use the
atomic state infrastructure and perform plane updates as separate
prepare/check/commit/cleanup steps.

void intel\_plane\_destroy(struct [drm\_plane](drm-kms.html#c.drm_plane "drm_plane") \*plane)
:   destroy a plane
...
struct [drm\_plane\_state](drm-kms.html#c.drm_plane_state "drm_plane_state") \*intel\_plane\_duplicate\_state(struct [drm\_plane](drm-kms.html#c.drm_plane "drm_plane") \*plane)
...
void intel\_plane\_destroy\_state(struct [drm\_plane](drm-kms.html#c.drm_plane "drm_plane") \*plane, struct [drm\_plane\_state](drm-kms.html#c.drm_plane_state "drm_plane_state") \*state)
:   destroy plane state
...
int intel\_prepare\_plane\_fb(struct [drm\_plane](drm-kms.html#c.drm_plane "drm_plane") \*\_plane, struct [drm\_plane\_state](drm-kms.html#c.drm_plane_state "drm_plane_state") \*\_new\_plane\_state)
...
void intel\_cleanup\_plane\_fb(struct [drm\_plane](drm-kms.html#c.drm_plane "drm_plane") \*plane, struct [drm\_plane\_state](drm-kms.html#c.drm_plane_state "drm_plane_state") \*\_old\_plane\_state)
...
Asynchronous page flip is the implementation for the DRM\_MODE\_PAGE\_FLIP\_ASYNC
...
The platform independent interrupt handler [`intel_hpd_irq_handler()`](#c.intel_hpd_irq_handler "intel_hpd_irq_handler") in
...
uevent to userspace via [`drm_kms_helper_hotplug_event()`](drm-kms-helpers.html#c.drm_kms_helper_hotplug_event "drm_kms_helper_hotplug_event").
...
enum hpd\_pin intel\_hpd\_pin\_default(enum [port](#c.intel_hpd_pin_default "port") port)
...
bool intel\_hpd\_irq\_storm\_detect(struct intel\_display \*display, enum hpd\_pin pin, bool long\_hpd)
...
The number of IRQs that are allowed within **HPD\_STORM\_DETECT\_PERIOD** is
stored in **display->hotplug.hpd\_storm\_threshold** which defaults to
**HPD\_STORM\_DEFAULT\_THRESHOLD**. Long IRQs count as +10 to this threshold, and
short IRQs count as +1. If this threshold is exceeded, it’s considered an
IRQ storm and the IRQ state is set to **HPD\_MARK\_DISABLED**.

By default, most systems will only count long IRQs towards
`display->hotplug`.hpd\_storm\_threshold. However, some older systems also
...
void intel\_hpd\_irq\_handler(struct intel\_display \*display, u32 pin\_mask, u32 long\_mask)
...
triggered (**pin\_mask**), and which of those pins may be long pulses
(**long\_mask**). The **long\_mask** is ignored if the port corresponding to the pin
is not a digital port.

Here, we do hotplug irq storm detection and mitigation, and pass further
processing to appropriate bottom halves.

void intel\_hpd\_init(struct intel\_display \*display)
...
Also see: [`intel_hpd_poll_enable()`](#c.intel_hpd_poll_enable "intel_hpd_poll_enable") and [`intel_hpd_poll_disable()`](#c.intel_hpd_poll_disable "intel_hpd_poll_disable").

void intel\_hpd\_poll\_enable(struct intel\_display \*display)
...
dev->mode\_config.mutex, we do the actual hotplug enabling in a separate
worker.

Also see: [`intel_hpd_init()`](#c.intel_hpd_init "intel_hpd_init") and [`intel_hpd_poll_disable()`](#c.intel_hpd_poll_disable "intel_hpd_poll_disable").

void intel\_hpd\_poll\_disable(struct intel\_display \*display)
...
Also see: [`intel_hpd_init()`](#c.intel_hpd_init "intel_hpd_init") and [`intel_hpd_poll_enable()`](#c.intel_hpd_poll_enable "intel_hpd_poll_enable").

void intel\_hpd\_block(struct intel\_encoder \*encoder)
...
* It’s guaranteed that the blocked encoders’ HPD pulse handler
  (via intel\_digital\_port::`hpd_pulse()`) is not running.
* The hotplug event handling (via intel\_encoder::`hotplug()`) of an
  HPD IRQ pending at the time this function is called may be still
  running.
* Detection on the encoder’s connector (via
  drm\_connector\_helper\_funcs::`detect_ctx()`,
  drm\_connector\_funcs::`detect()`) remains allowed, for instance as part of
  userspace connector probing, or DRM core’s connector polling.

The call must be followed by calling [`intel_hpd_unblock()`](#c.intel_hpd_unblock "intel_hpd_unblock"), or
[`intel_hpd_clear_and_unblock()`](#c.intel_hpd_clear_and_unblock "intel_hpd_clear_and_unblock").

Note that the handling of HPD IRQs for another encoder using the same HPD
pin as that of **encoder** will be also blocked.

void intel\_hpd\_unblock(struct intel\_encoder \*encoder)
...
previously blocked by [`intel_hpd_block()`](#c.intel_hpd_block "intel_hpd_block"). Any HPD IRQ raised on the
HPD pin while it was blocked will be handled for **encoder** and for any
other encoder sharing the same HPD pin.

void intel\_hpd\_clear\_and\_unblock(struct intel\_encoder \*encoder)
...
The struct [`i915_audio_component`](#c.i915_audio_component "i915_audio_component") is used to interact between the graphics
and audio drivers. The struct `i915_audio_component_ops` **ops** in it is
defined in graphics driver and called in audio driver. The
struct `i915_audio_component_audio_ops` **audio\_ops** is called from i915 driver.

void intel\_audio\_codec\_enable(struct intel\_encoder \*encoder, const struct intel\_crtc\_state \*crtc\_state, const struct [drm\_connector\_state](drm-kms.html#c.drm_connector_state "drm_connector_state") \*conn\_state)
...
void intel\_audio\_codec\_disable(struct intel\_encoder \*encoder, const struct intel\_crtc\_state \*old\_crtc\_state, const struct [drm\_connector\_state](drm-kms.html#c.drm_connector_state "drm_connector_state") \*old\_conn\_state)
...
void intel\_audio\_hooks\_init(struct intel\_display \*display)
...
void intel\_audio\_component\_init(struct intel\_display \*display)
...
initializes an instance of [`struct i915_audio_component`](#c.i915_audio_component "i915_audio_component") which it receives
...
void intel\_audio\_component\_cleanup(struct intel\_display \*display)
...
void intel\_audio\_init(struct intel\_display \*display)
...
void intel\_audio\_deinit(struct intel\_display \*display)
...
`base`
:   the drm\_audio\_component base class
...
void intel\_lpe\_audio\_irq\_handler(struct intel\_display \*display)
...
int intel\_lpe\_audio\_init(struct intel\_display \*display)
...
void intel\_lpe\_audio\_teardown(struct intel\_display \*display)
...
void intel\_lpe\_audio\_notify(struct intel\_display \*display, enum transcoder cpu\_transcoder, enum [port](#c.intel_lpe_audio_notify "port") port, const void \*eld, int ls\_clock, bool dp\_output)
...
update. For this integration [`intel_psr_invalidate()`](#c.intel_psr_invalidate "intel_psr_invalidate") and [`intel_psr_flush()`](#c.intel_psr_flush "intel_psr_flush")
...
so DC3CO is enabled and tgl\_dc3co\_disable\_work is schedule to run after 6
...
void intel\_psr\_disable(struct [intel\_dp](#c.intel_psr_disable "intel_dp") \*intel\_dp, const struct intel\_crtc\_state \*old\_crtc\_state)
:   Disable PSR
...
void intel\_psr\_pause(struct [intel\_dp](#c.intel_psr_pause "intel_dp") \*intel\_dp)
:   Pause PSR
...
void intel\_psr\_resume(struct [intel\_dp](#c.intel_psr_resume "intel_dp") \*intel\_dp)
:   Resume PSR
...
bool intel\_psr\_needs\_vblank\_notification(const struct intel\_crtc\_state \*crtc\_state)
...
user-space is polling for vblank events. Also Wa\_16025596647 needs
information when vblank is enabled/disabled.

void intel\_psr\_trigger\_frame\_change\_event(struct intel\_dsb \*dsb, struct intel\_atomic\_state \*state, struct intel\_crtc \*crtc)
...
int intel\_psr\_min\_set\_context\_latency(const struct intel\_crtc\_state \*crtc\_state)
...
void intel\_psr\_invalidate(struct intel\_display \*display, unsigned frontbuffer\_bits, enum fb\_op\_origin origin)
:   Invalidate PSR
...
void intel\_psr\_flush(struct intel\_display \*display, unsigned frontbuffer\_bits, enum fb\_op\_origin origin)
:   Flush PSR
...
void intel\_psr\_init(struct [intel\_dp](#c.intel_psr_init "intel_dp") \*intel\_dp)
...
bool intel\_psr\_link\_ok(struct [intel\_dp](#c.intel_psr_link_ok "intel_dp") \*intel\_dp)
:   return psr->link\_ok
...
panel I.e. if panel is stating bad link and intel\_psr\_link\_ok is stating link
is ok caller should rely on latter.

Return value of link\_ok
...
void intel\_psr\_notify\_dc5\_dc6(struct intel\_display \*display)
...
This is targeted for underrun on idle PSR HW bug (Wa\_16025596647) to schedule
psr\_dc5\_dc6\_wa\_work used for applying/removing the workaround.

void intel\_psr\_dc5\_dc6\_wa\_init(struct intel\_display \*display)
...
This is targeted for underrun on idle PSR HW bug (Wa\_16025596647) to init
psr\_dc5\_dc6\_wa\_work used for applying the workaround.

void intel\_psr\_notify\_pipe\_change(struct intel\_atomic\_state \*state, struct intel\_crtc \*crtc, bool enable)
...
This is targeted for underrun on idle PSR HW bug (Wa\_16025596647) to apply
remove the workaround when pipe is getting enabled/disabled

void intel\_psr\_notify\_vblank\_enable\_disable(struct intel\_display \*display, bool enable)
...
void intel\_fbc\_handle\_fifo\_underrun\_irq(struct intel\_display \*display)
...
void intel\_fbc\_read\_underrun\_dbg\_info(struct intel\_display \*display, enum [pipe](#c.intel_fbc_read_underrun_dbg_info "pipe") pipe, bool log)
...
void intel\_fbc\_init(struct intel\_display \*display)
:   Initialize FBC
...
void intel\_fbc\_sanitize(struct intel\_display \*display)
:   Sanitize FBC
...
For integration with frontbuffer tracking code, [`intel_drrs_invalidate()`](#c.intel_drrs_invalidate "intel_drrs_invalidate")
and [`intel_drrs_flush()`](#c.intel_drrs_flush "intel_drrs_flush") are called.

DRRS can be further extended to support other internal panels and also
the scenario of video playback wherein RR is set based on the rate
requested by userspace.

void intel\_drrs\_activate(const struct intel\_crtc\_state \*crtc\_state)
:   activate DRRS
...
void intel\_drrs\_deactivate(const struct intel\_crtc\_state \*old\_crtc\_state)
:   deactivate DRRS
...
void intel\_drrs\_invalidate(struct intel\_display \*display, unsigned int frontbuffer\_bits)
...
Hence DRRS needs to be Upclocked, i.e. (LOW\_RR -> HIGH\_RR).

Dirty frontbuffers relevant to DRRS are tracked in busy\_frontbuffer\_bits.

void intel\_drrs\_flush(struct intel\_display \*display, unsigned int frontbuffer\_bits)
...
(LOW\_RR -> HIGH\_RR). And also Idleness detection should be started again,
if no other planes are dirty.

Dirty frontbuffers relevant to DRRS are tracked in busy\_frontbuffer\_bits.

void intel\_drrs\_crtc\_init(struct intel\_crtc \*crtc)
:   Init DRRS for CRTC
...
void intel\_dmc\_block\_pkgc(struct intel\_display \*display, enum [pipe](#c.intel_dmc_block_pkgc "pipe") pipe, bool block)
:   block PKG C-state
...
This interface is target for Wa\_16025596647 usage. I.e. to set/clear
PIPEDMC\_BLOCK\_PKGC\_SW\_BLOCK\_PKGC\_ALWAYS bit in PIPEDMC\_BLOCK\_PKGC\_SW register.

void intel\_dmc\_start\_pkgc\_exit\_at\_start\_of\_undelayed\_vblank(struct intel\_display \*display, enum [pipe](#c.intel_dmc_start_pkgc_exit_at_start_of_undelayed_vblank "pipe") pipe, bool enable)
...
This interface is target for Wa\_16025596647 usage. I.e. start the package C
exit at the start of the undelayed vblank

void intel\_dmc\_load\_program(struct intel\_display \*display)
...
void intel\_dmc\_disable\_program(struct intel\_display \*display)
...
void intel\_dmc\_init(struct intel\_display \*display)
...
void intel\_dmc\_suspend(struct intel\_display \*display)
...
void intel\_dmc\_resume(struct intel\_display \*display)
...
resources released in [`intel_dmc_suspend()`](#c.intel_dmc_suspend "intel_dmc_suspend").

void intel\_dmc\_fini(struct intel\_display \*display)
...
been enabled in DC\_STATE\_EN and the wake lock mode of operation has
been enabled.
...
DMC\_WAKELOCK\_CFG register. There are also 13 control registers
that can be used to hold and release different wakelocks. In the
current implementation, we only need one wakelock, so only
DMC\_WAKELOCK1\_CTL is used. The other definitions are here for
...
The VBT consists of a VBT Header (defined as [`struct vbt_header`](#c.vbt_header "vbt_header")), a BDB
Header ([`struct bdb_header`](#c.bdb_header "bdb_header")), and a number of BIOS Data Blocks (BDB) that
...
bool intel\_bios\_is\_valid\_vbt(struct intel\_display \*display, const void \*buf, size\_t size)
...
void intel\_bios\_init(struct intel\_display \*display)
...
void intel\_bios\_driver\_remove(struct intel\_display \*display)
:   Free any resources allocated by [`intel_bios_init()`](#c.intel_bios_init "intel_bios_init")
...
bool intel\_bios\_is\_tv\_present(struct intel\_display \*display)
...
bool intel\_bios\_is\_lvds\_present(struct intel\_display \*display, u8 \*i2c\_pin)
...
bool intel\_bios\_is\_port\_present(struct intel\_display \*display, enum [port](#c.intel_bios_is_port_present "port") port)
...
bool intel\_bios\_is\_dsi\_present(struct intel\_display \*display, enum [port](#c.intel_bios_is_dsi_present "port") \*port)
...
`bdb_offset`
:   Offset of [`struct bdb_header`](#c.bdb_header "bdb_header") from beginning of VBT
...
> cdclk = vco / cd2x\_div / (sq\_len / sq\_div) / 2

, where vco is the frequency generated by the PLL; cd2x\_div
represents the CD2X Divider; sq\_len and sq\_div are the bit length
...
void intel\_cdclk\_init\_hw(struct intel\_display \*display)
...
void intel\_cdclk\_uninit\_hw(struct intel\_display \*display)
...
bool intel\_cdclk\_clock\_changed(const struct intel\_cdclk\_config \*a, const struct intel\_cdclk\_config \*b)
...
bool intel\_cdclk\_can\_cd2x\_update(struct intel\_display \*display, const struct intel\_cdclk\_config \*a, const struct intel\_cdclk\_config \*b)
...
void intel\_update\_max\_cdclk(struct intel\_display \*display)
...
void intel\_update\_cdclk(struct intel\_display \*display)
...
u32 intel\_read\_rawclk(struct intel\_display \*display)
...
void intel\_init\_cdclk\_hooks(struct intel\_display \*display)
...
[`intel_dpll_init()`](#c.intel_dpll_init "intel_dpll_init") initializes the PLLs for the given platform. The
users of a PLL are tracked and that tracking is integrated with the atomic
modset interface. During an atomic operation, required PLLs can be reserved
for a given CRTC and encoder configuration by calling
[`intel_dpll_reserve()`](#c.intel_dpll_reserve "intel_dpll_reserve") and previously reserved PLLs can be released
with [`intel_dpll_release()`](#c.intel_dpll_release "intel_dpll_release").
Changes to the users are first staged in the atomic state, and then made
effective by calling [`intel_dpll_swap_state()`](#c.intel_dpll_swap_state "intel_dpll_swap_state") during the atomic
commit phase.

struct [intel\_dpll](#c.intel_dpll "intel_dpll") \*intel\_get\_dpll\_by\_id(struct intel\_display \*display, enum [intel\_dpll\_id](#c.intel_dpll_id "intel_dpll_id") id)
:   get a DPLL given its id

**Parameters**

`struct intel_display *display`
:   intel\_display device instance
...
void intel\_dpll\_enable(const struct intel\_crtc\_state \*crtc\_state)
...
void intel\_dpll\_disable(const struct intel\_crtc\_state \*crtc\_state)
...
void intel\_dpll\_crtc\_get(const struct intel\_crtc \*crtc, const struct [intel\_dpll](#c.intel_dpll "intel_dpll") \*pll, struct [intel\_dpll\_state](#c.intel_dpll_state "intel_dpll_state") \*dpll\_state)
...
void intel\_dpll\_crtc\_put(const struct intel\_crtc \*crtc, const struct [intel\_dpll](#c.intel_dpll "intel_dpll") \*pll, struct [intel\_dpll\_state](#c.intel_dpll_state "intel_dpll_state") \*dpll\_state)
...
void intel\_dpll\_swap\_state(struct intel\_atomic\_state \*state)
...
This is the dpll version of [`drm_atomic_helper_swap_state()`](drm-kms-helpers.html#c.drm_atomic_helper_swap_state "drm_atomic_helper_swap_state") since the
...
:   the active **port\_dpll\_id** to select

**Description**

Select the given **port\_dpll\_id** instance from the DPLLs reserved for the
CRTC.

void intel\_dpll\_init(struct intel\_display \*display)
:   Initialize DPLLs

**Parameters**

`struct intel_display *display`
:   intel\_display device

**Description**

Initialize DPLLs for **display**.

int intel\_dpll\_compute(struct intel\_atomic\_state \*state, struct intel\_crtc \*crtc, struct intel\_encoder \*encoder)
...
calling [`intel_dpll_swap_state()`](#c.intel_dpll_swap_state "intel_dpll_swap_state").

**Return**

0 on success, negative error code on failure.

int intel\_dpll\_reserve(struct intel\_atomic\_state \*state, struct intel\_crtc \*crtc, struct intel\_encoder \*encoder)
...
[`intel_dpll_release()`](#c.intel_dpll_release "intel_dpll_release").
...
void intel\_dpll\_release(struct intel\_atomic\_state \*state, struct intel\_crtc \*crtc)
...
This function releases all DPLLs reserved by [`intel_dpll_reserve()`](#c.intel_dpll_reserve "intel_dpll_reserve")
from the current atomic commit **state** and the old **crtc** atomic state.

The new configuration in the atomic commit **state** is made effective by
calling [`intel_dpll_swap_state()`](#c.intel_dpll_swap_state "intel_dpll_swap_state").

void intel\_dpll\_update\_active(struct intel\_atomic\_state \*state, struct intel\_crtc \*crtc, struct intel\_encoder \*encoder)
...
from the port DPLLs reserved previously by [`intel_dpll_reserve()`](#c.intel_dpll_reserve "intel_dpll_reserve"). The
DPLL selected will be based on the current mode of the encoder’s port.

int intel\_dpll\_get\_freq(struct intel\_display \*display, const struct [intel\_dpll](#c.intel_dpll "intel_dpll") \*pll, const struct intel\_dpll\_hw\_state \*dpll\_hw\_state)
...
Return the output frequency corresponding to **pll**’s passed in **dpll\_hw\_state**.

bool intel\_dpll\_get\_hw\_state(struct intel\_display \*display, struct [intel\_dpll](#c.intel_dpll "intel_dpll") \*pll, struct intel\_dpll\_hw\_state \*dpll\_hw\_state)
...
Read out **pll**’s hardware state into **dpll\_hw\_state**.

void intel\_dpll\_dump\_hw\_state(struct intel\_display \*display, struct [drm\_printer](drm-internals.html#c.drm_printer "drm_printer") \*p, const struct intel\_dpll\_hw\_state \*dpll\_hw\_state)
:   dump hw\_state

**Parameters**

`struct intel_display *display`
:   intel\_display structure
...
Dumo out the relevant values in **dpll\_hw\_state**.

bool intel\_dpll\_compare\_hw\_state(struct intel\_display \*display, const struct intel\_dpll\_hw\_state \*a, const struct intel\_dpll\_hw\_state \*b)
...
void intel\_dsb\_reg\_write\_indexed(struct intel\_dsb \*dsb, i915\_reg\_t reg, u32 val)
...
void intel\_dsb\_commit(struct intel\_dsb \*dsb)
...
struct intel\_dsb \*intel\_dsb\_prepare(struct intel\_atomic\_state \*state, struct intel\_crtc \*crtc, enum intel\_dsb\_id dsb\_id, unsigned int max\_cmds)
...
void intel\_dsb\_cleanup(struct intel\_dsb \*dsb)
...
manner regardless of how the multicast/unicast bit is set in MCR\_SELECTOR.
...
void intel\_gt\_mcr\_lock(struct intel\_gt \*gt, unsigned long \*flags)
...
Takes gt->mcr\_lock. uncore->lock should *not* be held when this
function is called, although it may be acquired after this
function call.

void intel\_gt\_mcr\_unlock(struct intel\_gt \*gt, unsigned long flags)
...
Releases the lock acquired by [`intel_gt_mcr_lock()`](#c.intel_gt_mcr_lock "intel_gt_mcr_lock").

**Context**

Releases gt->mcr\_lock

void intel\_gt\_mcr\_lock\_sanitize(struct intel\_gt \*gt)
...
u32 intel\_gt\_mcr\_read(struct intel\_gt \*gt, i915\_mcr\_reg\_t reg, int group, int instance)
...
Takes and releases gt->mcr\_lock
...
void intel\_gt\_mcr\_unicast\_write(struct intel\_gt \*gt, i915\_mcr\_reg\_t reg, u32 value, int group, int instance)
...
Calls a function that takes and releases gt->mcr\_lock

void intel\_gt\_mcr\_multicast\_write(struct intel\_gt \*gt, i915\_mcr\_reg\_t reg, u32 value)
...
void intel\_gt\_mcr\_multicast\_write\_fw(struct intel\_gt \*gt, i915\_mcr\_reg\_t reg, u32 value)
:   write a value to all instances of an MCR register
...
domains; use [`intel_gt_mcr_multicast_write()`](#c.intel_gt_mcr_multicast_write "intel_gt_mcr_multicast_write") in cases where forcewake should
be obtained automatically.

**Context**

The caller must hold gt->mcr\_lock.

u32 intel\_gt\_mcr\_multicast\_rmw(struct intel\_gt \*gt, i915\_mcr\_reg\_t reg, u32 clear, u32 set)
...
domains; use [`intel_gt_mcr_multicast_rmw()`](#c.intel_gt_mcr_multicast_rmw "intel_gt_mcr_multicast_rmw") in cases where forcewake should
be obtained automatically.
...
Calls functions that take and release gt->mcr\_lock

void intel\_gt\_mcr\_get\_nonterminated\_steering(struct intel\_gt \*gt, i915\_mcr\_reg\_t reg, u8 \*group, u8 \*instance)
...
u32 intel\_gt\_mcr\_read\_any\_fw(struct intel\_gt \*gt, i915\_mcr\_reg\_t reg)
...
This function assumes the caller is already holding any necessary forcewake
domains; use [`intel_gt_mcr_read_any()`](#c.intel_gt_mcr_read_any "intel_gt_mcr_read_any") in cases where forcewake should be
...
u32 intel\_gt\_mcr\_read\_any(struct intel\_gt \*gt, i915\_mcr\_reg\_t reg)
:   reads one instance of an MCR register
...
void intel\_gt\_mcr\_get\_ss\_steering(struct intel\_gt \*gt, unsigned int dss, unsigned int \*group, unsigned int \*instance)
...
int intel\_gt\_mcr\_wait\_for\_reg(struct intel\_gt \*gt, i915\_mcr\_reg\_t reg, u32 mask, u32 value, unsigned int fast\_timeout\_us, unsigned int slow\_timeout\_ms)
...
[`__intel_wait_for_register_fw()`](#c.__intel_wait_for_register_fw "__intel_wait_for_register_fw"). Generally this function will only be used
...
GPU batch buffers via one of the ioctls DRM\_IOCTL\_I915\_GEM\_EXECBUFFER2
or DRM\_IOCTL\_I915\_GEM\_EXECBUFFER2\_WR. Most such batchbuffers will
instruct the GPU to perform work (for example rendering) and that work
needs memory from which to read and memory to which to write. All memory
is encapsulated within GEM buffer objects (usually created with the ioctl
DRM\_IOCTL\_I915\_GEM\_CREATE). An ioctl providing a batchbuffer for the GPU
...
DRM\_IOCTL\_I915\_GEM\_CONTEXT\_CREATE which is identified by a 32-bit
...
work can pass that ID (in the lower bits of drm\_i915\_gem\_execbuffer2::rsvd1)
...
1. All locking rules and interface contracts with cross-driver interfaces
   (dma-buf, dma\_fence) need to be followed.
2. dma\_resv will be the outermost lock (when needed) and ww\_acquire\_ctx
   is to be hoisted at highest level and passed down within i915\_gem\_ctx
   in the call chain
3. While holding lru/memory manager (buddy, drm\_mm, whatever) locks
   system memory allocations are not allowed
4. Do not nest different lru/memory manager locks within each other.
   Take them in turn to update memory allocations, relying on the object’s
   dma\_resv ww\_mutex to serialize against other operations.
5. The suggestion for lru/memory managers locks is that they are small
   enough to be spinlocks.
6. All features need to come with exhaustive kernel selftests and/or
   IGT tests when appropriate
7. All LMEM uAPI paths need to be fully restartable (`_interruptible()`
   ...
   > * -EDEADLK handling with ww\_mutex
...
int i915\_gem\_evict\_something(struct i915\_address\_space \*vm, struct i915\_gem\_ww\_ctx \*ww, u64 min\_size, u64 alignment, unsigned long color, u64 start, u64 end, unsigned flags)
...
pinned. Hence obj->pages\_pin\_count does not protect against eviction.

To clarify: This is for freeing up virtual address space, not for freeing
memory in e.g. the shrinker.

int i915\_gem\_evict\_for\_node(struct i915\_address\_space \*vm, struct i915\_gem\_ww\_ctx \*ww, struct [drm\_mm\_node](drm-mm.html#c.drm_mm_node "drm_mm_node") \*target, unsigned int flags)
:   Evict vmas to make room for binding a new one
...
int i915\_gem\_evict\_vm(struct i915\_address\_space \*vm, struct i915\_gem\_ww\_ctx \*ww, struct drm\_i915\_gem\_object \*\*busy\_bo)
...
`struct i915_gem_ww_ctx *ww`
:   An optional `struct i915_gem_ww_ctx`. If not NULL, i915\_gem\_evict\_vm
    will be able to evict vma’s locked by the ww as well.

`struct drm_i915_gem_object **busy_bo`
:   Optional pointer to `struct drm_i915_gem_object`. If not NULL, then
    in the event [`i915_gem_evict_vm()`](#c.i915_gem_evict_vm "i915_gem_evict_vm") is unable to trylock an object for eviction,
    then **busy\_bo** will point to it. -EBUSY is also returned. The caller must drop
...
unsigned long i915\_gem\_shrink(struct i915\_gem\_ww\_ctx \*ww, struct drm\_i915\_private \*i915, unsigned long target, unsigned long \*nr\_scanned, unsigned int shrink)
...
avoid deadlocks in memory reclaim) must fall back to [`i915_gem_shrink_all()`](#c.i915_gem_shrink_all "i915_gem_shrink_all").
...
This is a simple wrapper around [`i915_gem_shrink()`](#c.i915_gem_shrink "i915_gem_shrink") to aggressively shrink all
...
:   Hide the object from the shrinker. By default all object types that support shrinking(see IS\_SHRINKABLE), will also make the object visible to the shrinker after allocating the system memory pages.
...
MUST be balanced with previous call to [`i915_gem_object_make_unshrinkable()`](#c.i915_gem_object_make_unshrinkable "i915_gem_object_make_unshrinkable").
...
int intel\_engine\_init\_cmd\_parser(struct intel\_engine\_cs \*engine)
...
int intel\_engine\_cmd\_parser(struct intel\_engine\_cs \*engine, struct i915\_vma \*batch, unsigned long batch\_offset, unsigned long batch\_length, struct i915\_vma \*shadow, bool trampoline)
...
`link`
:   Link in i915\_gem\_context::stale::engines
...
`ctx`
:   i915\_gem\_context backpointer
...
`idx`
:   Index into i915\_gem\_engines::engines
...
`siblings`
:   Balanced siblings or num\_siblings \* width for parallel
...
> * I915\_GEM\_ENGINE\_TYPE\_INVALID: Invalid engines can be created but they
>   show up as a NULL in i915\_gem\_engines::engines[i] and any attempt to
...
> * I915\_GEM\_ENGINE\_TYPE\_PHYSICAL: A single physical engine, described by
>   i915\_gem\_proto\_engine::engine.
> * I915\_GEM\_ENGINE\_TYPE\_BALANCED: A load-balanced engine set, described
>   i915\_gem\_proto\_engine::num\_siblings and i915\_gem\_proto\_engine::siblings.
> * I915\_GEM\_ENGINE\_TYPE\_PARALLEL: A parallel submission engine set, described
>   i915\_gem\_proto\_engine::width, i915\_gem\_proto\_engine::num\_siblings, and
>   i915\_gem\_proto\_engine::siblings.

struct i915\_gem\_proto\_context
:   prototype context
...
`vm`
:   See [`i915_gem_context.vm`](#c.i915_gem_context "i915_gem_context")

`user_flags`
:   See [`i915_gem_context.user_flags`](#c.i915_gem_context "i915_gem_context")

`sched`
:   See [`i915_gem_context.sched`](#c.i915_gem_context "i915_gem_context")
...
`single_timeline`
:   See See [`i915_gem_context.syncobj`](#c.i915_gem_context "i915_gem_context")

`uses_protected_content`
:   See [`i915_gem_context.uses_protected_content`](#c.i915_gem_context "i915_gem_context")

`pxp_wakeref`
:   See [`i915_gem_context.pxp_wakeref`](#c.i915_gem_context "i915_gem_context")

**Description**

The [`struct i915_gem_proto_context`](#c.i915_gem_proto_context "i915_gem_proto_context") represents the creation parameters for
a [`struct i915_gem_context`](#c.i915_gem_context "i915_gem_context"). This is used to gather parameters provided
either through creation flags or via SET\_CONTEXT\_PARAM so that, when we
create the final i915\_gem\_context, those parameters can be immutable.

The context uAPI allows for two methods of setting context parameters:
SET\_CONTEXT\_PARAM and CONTEXT\_CREATE\_EXT\_SETPARAM. The former is
allowed to be called at any time while the later happens as part of
GEM\_CONTEXT\_CREATE. When these were initially added, Currently,
...
the VM or engine set via SET\_CONTEXT\_PARAM is to delay the creation of
the actual context until after the client is done configuring it with
SET\_CONTEXT\_PARAM. From the perspective of the client, it has the same
u32 context ID the whole time. From the perspective of i915, however,
it’s an i915\_gem\_proto\_context right up until the point where we attempt
to do something which the proto-context can’t handle at which point the
real context gets created.

This is accomplished via a little xarray dance. When GEM\_CONTEXT\_CREATE
is called, we create a proto-context, reserve a slot in context\_xa but
leave it NULL, the proto-context in the corresponding slot in
proto\_context\_xa. Then, whenever we go to look up a context, we first
check context\_xa. If it’s there, we return the i915\_gem\_context and
we’re done. If it’s not, we look in proto\_context\_xa and, if we find it
...
> * Mesa/ANV used the engines API but via CONTEXT\_CREATE\_EXT\_SETPARAM and
...
>   did use the VM API via SET\_CONTEXT\_PARAM. However, CONTEXT\_SETPARAM
>   was always the second ioctl on that context, immediately following
>   GEM\_CONTEXT\_CREATE.
> * The media driver sets engines and bonding/balancing via
>   SET\_CONTEXT\_PARAM. However, CONTEXT\_SETPARAM to set the VM was
>   always the second ioctl on that context, immediately following
>   GEM\_CONTEXT\_CREATE and setting engines immediately followed that.

In order for this dance to work properly, any modification to an
i915\_gem\_proto\_context that is exposed to the client via
drm\_i915\_file\_private::proto\_context\_xa must be guarded by
drm\_i915\_file\_private::proto\_context\_lock. The exception is when a
proto-context has not yet been exposed such as when handling
CONTEXT\_CREATE\_SET\_PARAM during GEM\_CONTEXT\_CREATE.
...
:   User defined engines for this context
    ...
    Execbuf uses the I915\_EXEC\_RING\_MASK as an index into this
    array to select which HW context + engine to execute on. For
    the default array, the user\_ring\_map[] is used to translate
    the legacy uABI onto the appropriate index (e.g. both
    I915\_EXEC\_DEFAULT and I915\_EXEC\_RENDER select the same
    context, and I915\_EXEC\_BSD is weird). For a user defined
    array, execbuf uses I915\_EXEC\_RING\_MASK as a plain index.

    User defined by I915\_CONTEXT\_PARAM\_ENGINE (when the
    CONTEXT\_USER\_ENGINES flag is set).
...
:   Shared timeline syncobj
    ...
    When the SHARED\_TIMELINE flag is set on context creation, we
    emulate a single timeline across all engines using this syncobj.
    For every execbuffer2 call, this syncobj is used as both an in-
    and out-fence. Unlike the real intel\_timeline, this doesn’t
...
:   Work item for deferred cleanup, since `i915_gem_context_put()` tends to
    ...
    FIXME: The only real reason for this is [`i915_gem_engines.fence`](#c.i915_gem_engines "i915_gem_engines"), all
    other callers are from process context and need at most some mild
    shuffling to pull the `i915_gem_context_put()` call out of a spinlock.

`rcu`
:   rcu\_head for deferred freeing.
...
:   context uses PXP-encrypted objects.
    ...
    the lifetime of the context. See I915\_CONTEXT\_PARAM\_PROTECTED\_CONTENT
    in uapi/drm/i915\_drm.h for more info on setting restrictions and
...
`mutex`
:   guards everything that isn’t engines or handles\_vma
...
`stale.engines`
:   list of stale engines

**Description**

The [`struct i915_gem_context`](#c.i915_gem_context "i915_gem_context") represents the combined view of the driver and
...
the addresses. Userspace can set the I915\_EXEC\_NO\_RELOC flag to hint that
all the target addresses in all of its objects match the value in the
relocation entries and that they all match the presumed offsets given by the
list of execbuffer objects. Using this knowledge, we know that if we haven’t
moved any buffers, all the relocation entries are valid and we can skip
the update. (If userspace is wrong, the likely outcome is an impromptu GPU
hang.) The requirement for using I915\_EXEC\_NO\_RELOC are:

> The addresses written in the objects must match the corresponding
> reloc.presumed\_offset which in turn must match the corresponding
> execobject.offset.
>
...
`default_priolist`
:   priority list for I915\_PRIORITY\_NORMAL
...
container for the drm\_gpu\_scheduler plus a few other variables once the i915
...
contained there mean you don’t need to do a ppgtt->switch\_mm yourself,
...
globally const i915\_gtt\_view\_normal singleton instance exists. All old core
...
2. Extend the metadata in the i915\_gtt\_view structure if required.
3. Add support to `i915_get_vma_pages()`.

New views are required to build a scatter-gather table from within the
i915\_get\_vma\_pages function. This table is stored in the vma.gtt\_view and
...
int i915\_gem\_gtt\_reserve(struct i915\_address\_space \*vm, struct i915\_gem\_ww\_ctx \*ww, struct [drm\_mm\_node](drm-mm.html#c.drm_mm_node "drm_mm_node") \*node, u64 size, u64 offset, unsigned long color, unsigned int flags)
...
:   the [`struct drm_mm_node`](drm-mm.html#c.drm_mm_node "drm_mm_node") (typically i915\_vma.node)
...
[`i915_gem_gtt_reserve()`](#c.i915_gem_gtt_reserve "i915_gem_gtt_reserve") tries to insert the **node** at the exact **offset** inside
the address space (using **size** and **color**). If the **node** does not fit, it
tries to evict any overlapping nodes from the GTT, including any
neighbouring nodes if the colors do not match (to ensure guard pages between
differing domains). See [`i915_gem_evict_for_node()`](#c.i915_gem_evict_for_node "i915_gem_evict_for_node") for the gory details
...
int i915\_gem\_gtt\_insert(struct i915\_address\_space \*vm, struct i915\_gem\_ww\_ctx \*ww, struct [drm\_mm\_node](drm-mm.html#c.drm_mm_node "drm_mm_node") \*node, u64 size, u64 alignment, unsigned long color, u64 start, u64 end, unsigned int flags)
...
[`i915_gem_gtt_insert()`](#c.i915_gem_gtt_insert "i915_gem_gtt_insert") first searches for an available hole into which
...
searching for an eviction candidate. See [`i915_gem_evict_something()`](#c.i915_gem_evict_something "i915_gem_evict_something") for
...
void intel\_ggtt\_restore\_fences(struct i915\_ggtt \*ggtt)
:   restore fence state
...
void detect\_bit\_6\_swizzle(struct i915\_ggtt \*ggtt)
...
[`i915_gem_object_save_bit_17_swizzle()`](#c.i915_gem_object_save_bit_17_swizzle "i915_gem_object_save_bit_17_swizzle").
...
can be fixed up later on with [`i915_gem_object_do_bit_17_swizzle()`](#c.i915_gem_object_do_bit_17_swizzle "i915_gem_object_do_bit_17_swizzle"). This must
...
u32 i915\_gem\_fence\_size(struct drm\_i915\_private \*i915, u32 size, unsigned int tiling, unsigned int stride)
...
int i915\_gem\_set\_tiling\_ioctl(struct [drm\_device](drm-internals.html#c.drm_device "drm_device") \*dev, void \*data, struct [drm\_file](drm-internals.html#c.drm_file "drm_file") \*file)
...
int i915\_gem\_get\_tiling\_ioctl(struct [drm\_device](drm-internals.html#c.drm_device "drm_device") \*dev, void \*data, struct [drm\_file](drm-internals.html#c.drm_file "drm_file") \*file)
...
[`i915_gem_set_tiling_ioctl()`](#c.i915_gem_set_tiling_ioctl "i915_gem_set_tiling_ioctl") and [`i915_gem_get_tiling_ioctl()`](#c.i915_gem_get_tiling_ioctl "i915_gem_get_tiling_ioctl") is the userspace
...
objects. The SW support is enabled via the CONFIG\_DRM\_I915\_PXP kconfig.

Objects can opt-in to PXP encryption at creation time via the
I915\_GEM\_CREATE\_EXT\_PROTECTED\_CONTENT create\_ext flag. For objects to be
correctly protected they must be used in conjunction with a context created
with the I915\_CONTEXT\_PARAM\_PROTECTED\_CONTENT flag. See the documentation
...
(I915\_PROTECTED\_CONTENT\_DEFAULT\_SESSION) for use in instructions targeting
protected objects.
...
performed via the mei\_pxp component module.
...
:   used to track if any prior arb session creation resulted
    ...
    GET\_PARAM:I915\_PARAM\_PXP\_STATUS is called.
...
`pxp_component`
:   i915\_pxp\_component `struct of` the bound mei\_pxp
...
:   tracks the status of a pending termination. Only
    re-initialized under gt->irq\_lock and completed in `session_work`.

`session_work`
:   worker that manages session events.

`session_events`
:   pending session events, protected with gt->irq\_lock.
...
The enable\_guc module parameter can be used to select which of those
...
:   mask of events that are processed when receiving
    an INTEL\_GUC\_ACTION\_DEFAULT G2H message.
...
`serial_slot`
:   id to the initial waiter created in tlb\_lookup,
...
:   protects everything in
    submission\_state, ce->guc\_id.id, and ce->guc\_id.ref
...
:   Number of guc\_ids, selftest
    feature to be able to reduce this number while testing.

`submission_state.guc_ids_bitmap`
:   used to allocate
    new guc\_ids, multi-lrc

`submission_state.guc_id_list`
:   list of intel\_context
    with valid guc\_ids but no refs
...
:   threshold of min remaining available guc\_ids before
...
`context_lookup`
:   used to resolve intel\_context from guc\_id, if a
...
`send_mutex`
:   used to serialize the intel\_guc\_send actions
...
:   The number of guc\_ids that have been stolen
...
It handles firmware loading and manages client pool. intel\_guc owns an
i915\_sched\_engine for submission.

u32 intel\_guc\_ggtt\_offset(struct [intel\_guc](#c.intel_guc "intel_guc") \*guc, struct i915\_vma \*vma)
...
[0, ggtt.pin\_bias), which is reserved for Boot ROM, SRAM and WOPCM.
Currently, in order to exclude [0, ggtt.pin\_bias) address space from
GGTT, all gfx objects used by GuC are allocated with [`intel_guc_allocate_vma()`](#c.intel_guc_allocate_vma "intel_guc_allocate_vma")
and pinned with PIN\_OFFSET\_BIAS along with the value of ggtt.pin\_bias.
...
The lower part of GuC Address Space [0, ggtt\_pin\_bias) is mapped to GuC WOPCM
while upper part of GuC Address Space [ggtt\_pin\_bias, GUC\_GGTT\_TOP) is mapped
to DRAM. The value of the GuC ggtt\_pin\_bias is the GuC WOPCM size.

struct i915\_vma \*intel\_guc\_allocate\_vma(struct [intel\_guc](#c.intel_guc "intel_guc") \*guc, u32 size)
...
it in the GGTT somewhere other than than [0, GUC ggtt\_pin\_bias) because that
range is reserved inside GuC.

**Return**

A i915\_vma if successful, otherwise an ERR\_PTR.

#### GuC-specific firmware loader

int intel\_guc\_fw\_upload(struct [intel\_guc](#c.intel_guc "intel_guc") \*guc)
...
H2G. A unique guc\_id is associated with each context. The context is either
...
(see SCHED\_DISABLE\_DELAY\_MS). The thought is this gives the user a window of
time to resubmit something on the context before doing this costly operation.
This delay is only done if the context isn’t closed and the guc\_id usage is
less than a threshold (see NUM\_SCHED\_DISABLE\_GUC\_IDS\_THRESHOLD).

Context deregistration:
Before a context can be destroyed or if we steal its guc\_id we must
deregister the context with the GuC via H2G. If stealing the guc\_id it isn’t
safe to submit anything to this guc\_id until the deregister completes so a
fence is used to stall all requests associated with this guc\_id until the
corresponding G2H returns indicating the guc\_id has been deregistered.

submission\_state.guc\_ids:
Unique number associated with private GuC context data passed in during
context registration / submission / deregistration. 64k available. Simple ida
is used for allocation.

Stealing guc\_ids:
If no guc\_ids are available they can be stolen from another context at
request creation time if that context is unpinned. If a guc\_id can’t be found
...
engine (sched\_engine), thus only one of the contexts which share a
sched\_engine can be submitting at a time. Currently only one sched\_engine is
used for all of GuC submission but that could change in the future.

guc->submission\_state.lock
Global lock for GuC submission state. Protects guc\_ids and destroyed contexts
list.

ce->guc\_state.lock
Protects everything under ce->guc\_state. Ensures that a context is in the
...
Lock ordering rules:
sched\_engine->lock -> ce->guc\_state.lock
guc->submission\_state.lock -> ce->guc\_state.lock

Reset races:
When a full GT reset is triggered it is assumed that some G2H responses to
H2Gs can be lost as the GuC is also reset. Losing these G2H can prove to be
fatal as we do certain operations upon receiving a G2H (e.g. destroy
contexts, release guc\_ids, etc...). When this occurs we can scrub the
...
> |  | Bits | Description |
> | --- | --- | --- |
> | 0 | 31 | **ORIGIN** - originator of the message  * GUC\_HXG\_ORIGIN\_HOST = 0 * GUC\_HXG\_ORIGIN\_GUC = 1 |
> | 30:28 | **TYPE** - message type  * GUC\_HXG\_TYPE\_REQUEST = 0 * GUC\_HXG\_TYPE\_EVENT = 1 * GUC\_HXG\_TYPE\_FAST\_REQUEST = 2 * GUC\_HXG\_TYPE\_NO\_RESPONSE\_BUSY = 3 * GUC\_HXG\_TYPE\_NO\_RESPONSE\_RETRY = 5 * GUC\_HXG\_TYPE\_RESPONSE\_FAILURE = 6 * GUC\_HXG\_TYPE\_RESPONSE\_SUCCESS = 7 |
...
The [HXG Request](#hxg-request) message should be used to initiate synchronous activity
for which confirmation or return data is expected.

The recipient of this message shall use [HXG Response](#hxg-response), [HXG Failure](#hxg-failure)
or [HXG Retry](#hxg-retry) message as a definite reply, and may use [HXG Busy](#hxg-busy)
...
> | --- | --- | --- |
> | 0 | 31 | ORIGIN |
> | 30:28 | TYPE = [GUC\_HXG\_TYPE\_REQUEST](#guc-hxg-type-request) |
...
The [HXG Request](#hxg-request) message should be used to initiate asynchronous activity
for which confirmation or return data is not expected.

If confirmation is required then [HXG Request](#hxg-request) shall be used instead.

The recipient of this message may only use [HXG Failure](#hxg-failure) message if it was
unable to accept this request (like invalid data).

Format of [HXG Fast Request](#hxg-fast-request) message is same as [HXG Request](#hxg-request) except **TYPE**.

> |  | Bits | Description |
> | --- | --- | --- |
> | 0 | 31 | ORIGIN - see [HXG Message](#hxg-message) |
> | 30:28 | TYPE = [GUC\_HXG\_TYPE\_FAST\_REQUEST](#guc-hxg-type-fast-request) |
> | 27:16 | DATA0 - see [HXG Request](#hxg-request) |
> | 15:0 | ACTION - see [HXG Request](#hxg-request) |
> | ... |  | DATAn - see [HXG Request](#hxg-request) |

**HXG Event**

The [HXG Event](#hxg-event) message should be used to initiate asynchronous activity
...
> | --- | --- | --- |
> | 0 | 31 | ORIGIN |
> | 30:28 | TYPE = [GUC\_HXG\_TYPE\_EVENT](#guc-hxg-type-event) |
...
The [HXG Busy](#hxg-busy) message may be used to acknowledge reception of the [HXG Request](#hxg-request)
...
> | --- | --- | --- |
> | 0 | 31 | ORIGIN |
> | 30:28 | TYPE = [GUC\_HXG\_TYPE\_NO\_RESPONSE\_BUSY](#guc-hxg-type-no-response-busy) |
> | 27:0 | **COUNTER** - progress indicator |

**HXG Retry**

The [HXG Retry](#hxg-retry) message should be used by recipient to indicate that the
[HXG Request](#hxg-request) message was dropped and it should be resent again.
...
> | --- | --- | --- |
> | 0 | 31 | ORIGIN |
> | 30:28 | TYPE = [GUC\_HXG\_TYPE\_NO\_RESPONSE\_RETRY](#guc-hxg-type-no-response-retry) |
> | 27:0 | **REASON** - reason for retry  * GUC\_HXG\_RETRY\_REASON\_UNSPECIFIED = 0 |

**HXG Failure**

The [HXG Failure](#hxg-failure) message shall be used as a reply to the [HXG Request](#hxg-request)
message that could not be processed due to an error.

> |  | Bits | Description |
> | --- | --- | --- |
> | 0 | 31 | ORIGIN |
> | 30:28 | TYPE = [GUC\_HXG\_TYPE\_RESPONSE\_FAILURE](#guc-hxg-type-response-failure) |
> | 27:16 | **HINT** - additional error hint |
> | 15:0 | **ERROR** - error/result code |

**HXG Response**

The [HXG Response](#hxg-response) message shall be used as a reply to the [HXG Request](#hxg-request)
message that was successfully processed without an error.

> |  | Bits | Description |
> | --- | --- | --- |
> | 0 | 31 | ORIGIN |
> | 30:28 | TYPE = [GUC\_HXG\_TYPE\_RESPONSE\_SUCCESS](#guc-hxg-type-response-success) |
> | 27:0 | **DATA0** - data (depends on ACTION from [HXG Request](#hxg-request)) |
> | 1 | 31:0 | **DATAn** - data (depends on ACTION from [HXG Request](#hxg-request)) |
> | ... |  |
> | n | 31:0 |
...
phase to setup the [CTB based communication](#ctb-based-communication) that will be used afterwards.

**MMIO HXG Message**

Format of the MMIO messages follows definitions of [HXG Message](#hxg-message).

> |  | Bits | Description |
> | --- | --- | --- |
> | 0 | 31:0 | [Embedded [HXG Message](#hxg-message)] |
> | ... |  |
> | n | 31:0 |

**CT Buffer**

Circular buffer used to send [CTB Message](#ctb-message)

**CTB Descriptor**

> |  | Bits | Description |
> | --- | --- | --- |
> | 0 | 31:0 | **HEAD** - offset (in dwords) to the last dword that was read from the [CT Buffer](#ct-buffer). It can only be updated by the receiver. |
> | 1 | 31:0 | **TAIL** - offset (in dwords) to the last dword that was written to the [CT Buffer](#ct-buffer). It can only be updated by the sender. |
> | 2 | 31:0 | **STATUS** - status of the CTB  * GUC\_CTB\_STATUS\_NO\_ERROR = 0 (normal operation) * GUC\_CTB\_STATUS\_OVERFLOW = 1 (head/tail too large) * GUC\_CTB\_STATUS\_UNDERFLOW = 2 (truncated message) * GUC\_CTB\_STATUS\_MISMATCH = 4 (head/tail modified) * GUC\_CTB\_STATUS\_UNUSED = 8 (CTB is not in use) |
> | ... |  | RESERVED = MBZ |
...
> | 15:12 | **FORMAT** - format of the CTB message  * GUC\_CTB\_FORMAT\_HXG = 0 - see [CTB HXG Message](#ctb-hxg-message) |
> | 11:8 | **RESERVED** |
> | 7:0 | **NUM\_DWORDS** - length of the CTB message (w/o header) |
...
> | --- | --- | --- |
> | 0 | 31:16 | FENCE |
> | 15:12 | FORMAT = [GUC\_CTB\_FORMAT\_HXG](#guc-ctb-format-hxg) |
> | 11:8 | RESERVED = MBZ |
> | 7:0 | NUM\_DWORDS = length (in dwords) of the embedded HXG message |
> | 1 | 31:0 | [Embedded [HXG Message](#hxg-message)] |
> | ... |  |
> | n | 31:0 |
...
This message is used by Host KMD to setup of the [GuC Self Config KLVs](#guc-self-config-klvs).

This message must be sent as [MMIO HXG Message](#mmio-hxg-message).

> |  | Bits | Description |
> | --- | --- | --- |
> | 0 | 31 | ORIGIN = [GUC\_HXG\_ORIGIN\_HOST](#guc-hxg-origin-host) |
> | 30:28 | TYPE = [GUC\_HXG\_TYPE\_REQUEST](#guc-hxg-type-request) |
> | 27:16 | DATA0 = MBZ |
> | 15:0 | ACTION = GUC\_ACTION\_HOST2GUC\_SELF\_CFG = 0x0508 |
> | 1 | 31:16 | **KLV\_KEY** - KLV key, see [GuC Self Config KLVs](#guc-self-config-klvs) |
> | 15:0 | **KLV\_LEN** - KLV length  * 32 bit KLV = 1 * 64 bit KLV = 2 |
> | 2 | 31:0 | **VALUE32** - Bits 31-0 of the KLV value |
> | 3 | 31:0 | **VALUE64** - Bits 63-32 of the KLV value (**KLV\_LEN** = 2) |
>
> |  | Bits | Description |
> | --- | --- | --- |
> | 0 | 31 | ORIGIN = [GUC\_HXG\_ORIGIN\_GUC](#guc-hxg-origin-guc) |
> | 30:28 | TYPE = [GUC\_HXG\_TYPE\_RESPONSE\_SUCCESS](#guc-hxg-type-response-success) |
> | 27:0 | DATA0 = **NUM** - 1 if KLV was parsed, 0 if not recognized |

**HOST2GUC\_CONTROL\_CTB**

This H2G action allows Vf Host to enable or disable H2G and G2H [CT Buffer](#ct-buffer).
...
> | 30:28 | TYPE = [GUC\_HXG\_TYPE\_REQUEST](#guc-hxg-type-request) |
> | 27:16 | DATA0 = MBZ |
> | 15:0 | ACTION = GUC\_ACTION\_HOST2GUC\_CONTROL\_CTB = 0x4509 |
> | 1 | 31:0 | **CONTROL** - control [CTB based communication](#ctb-based-communication)  * GUC\_CTB\_CONTROL\_DISABLE = 0 * GUC\_CTB\_CONTROL\_ENABLE = 1 |
>
> |  | Bits | Description |
> | --- | --- | --- |
> | 0 | 31 | ORIGIN = [GUC\_HXG\_ORIGIN\_GUC](#guc-hxg-origin-guc) |
...
> |  | Bits | Description |
> | --- | --- | --- |
> | 0 | 31:16 | **KEY** - KLV key identifier  * [GuC Self Config KLVs](#guc-self-config-klvs) |
...
[GuC KLV](#guc-klv) keys available for use with [HOST2GUC\_SELF\_CFG](#host2guc-self-cfg).

GUC\_KLV\_SELF\_CFG\_H2G\_CTB\_ADDR0x0902
:   Refers to 64 bit Global Gfx address of H2G [CT Buffer](#ct-buffer).
    Should be above WOPCM address but below APIC base address for native mode.

GUC\_KLV\_SELF\_CFG\_H2G\_CTB\_DESCRIPTOR\_ADDR0x0903
:   Refers to 64 bit Global Gfx address of H2G [CTB Descriptor](#ctb-descriptor).
    Should be above WOPCM address but below APIC base address for native mode.

GUC\_KLV\_SELF\_CFG\_H2G\_CTB\_SIZE0x0904
:   Refers to size of H2G [CT Buffer](#ct-buffer) in bytes.
    Should be a multiple of 4K.

GUC\_KLV\_SELF\_CFG\_G2H\_CTB\_ADDR0x0905
:   Refers to 64 bit Global Gfx address of G2H [CT Buffer](#ct-buffer).
    Should be above WOPCM address but below APIC base address for native mode.

GUC\_KLV\_SELF\_CFG\_G2H\_CTB\_DESCRIPTOR\_ADDR0x0906
:   Refers to 64 bit Global Gfx address of G2H [CTB Descriptor](#ctb-descriptor).
    Should be above WOPCM address but below APIC base address for native mode.

GUC\_KLV\_SELF\_CFG\_G2H\_CTB\_SIZE0x0907
:   Refers to size of G2H [CT Buffer](#ct-buffer) in bytes.
    Should be a multiple of 4K.
...
int intel\_huc\_auth(struct intel\_huc \*huc, enum intel\_huc\_authentication\_type type)
:   Authenticate HuC uCode

**Parameters**

`struct intel_huc *huc`
:   intel\_huc structure
...
The HuC FW layout is the same as the GuC one, see [GuC Firmware Layout](#guc-firmware-layout)

### DMC

See [DMC Firmware Support](#dmc-firmware-support)
...
integrate with drm/i915 and to handle the DRM\_I915\_PERF\_OPEN ioctl.

int i915\_perf\_init(struct drm\_i915\_private \*i915)
...
phase with the [`i915_perf_register()`](#c.i915_perf_register "i915_perf_register") exposing state to userspace.

void i915\_perf\_fini(struct drm\_i915\_private \*i915)
:   Counter part to [`i915_perf_init()`](#c.i915_perf_init "i915_perf_init")
...
void i915\_perf\_register(struct drm\_i915\_private \*i915)
...
void i915\_perf\_unregister(struct drm\_i915\_private \*i915)
...
userspace by [`i915_perf_unregister()`](#c.i915_perf_unregister "i915_perf_unregister") before cleaning up
remaining state in [`i915_perf_fini()`](#c.i915_perf_fini "i915_perf_fini").

int i915\_perf\_open\_ioctl(struct [drm\_device](drm-internals.html#c.drm_device "drm_device") \*dev, void \*data, struct [drm\_file](drm-internals.html#c.drm_file "drm_file") \*file)
...
mutex to avoid an awkward lockdep with mmap\_lock.

Most of the implementation details are handled by
[`i915_perf_open_ioctl_locked()`](#c.i915_perf_open_ioctl_locked "i915_perf_open_ioctl_locked") after taking the `gt->perf`.lock
...
int i915\_perf\_release(struct [inode](#c.i915_perf_release "inode") \*inode, struct [file](#c.i915_perf_release "file") \*file)
...
int i915\_perf\_add\_config\_ioctl(struct [drm\_device](drm-internals.html#c.drm_device "drm_device") \*dev, void \*data, struct [drm\_file](drm-internals.html#c.drm_file "drm_file") \*file)
...
`void *data`
:   ioctl data (pointer to [`struct drm_i915_perf_oa_config`](driver-uapi.html#c.drm_i915_perf_oa_config "drm_i915_perf_oa_config")) copied from
...
int i915\_perf\_remove\_config\_ioctl(struct [drm\_device](drm-internals.html#c.drm_device "drm_device") \*dev, void \*data, struct [drm\_file](drm-internals.html#c.drm_file "drm_file") \*file)
...
`sample_flags`
:   Flags representing the DRM\_I915\_PERF\_PROP\_SAMPLE\_\*
...
:   Whether the stream is currently enabled, considering
    ...
    on I915\_PERF\_IOCTL\_ENABLE and I915\_PERF\_IOCTL\_DISABLE calls.
...
:   The mask used to masking specific\_ctx\_id bits.
...
:   Enables the collection of HW samples, either in response to
    I915\_PERF\_IOCTL\_ENABLE or implicitly called when stream is opened
    without I915\_PERF\_FLAG\_DISABLED.

`disable`
:   Disables the collection of HW samples, either in response
    to I915\_PERF\_IOCTL\_DISABLE or implicitly called before destroying
    the stream.
...
int read\_properties\_unlocked(struct i915\_perf \*perf, u64 \_\_user \*uprops, u32 n\_props, struct [perf\_open\_properties](#c.perf_open_properties "perf_open_properties") \*props)
...
int i915\_perf\_open\_ioctl\_locked(struct i915\_perf \*perf, struct drm\_i915\_perf\_open\_param \*param, struct [perf\_open\_properties](#c.perf_open_properties "perf_open_properties") \*props, struct [drm\_file](drm-internals.html#c.drm_file "drm_file") \*file)
:   DRM ioctl() for userspace to open a stream FD
...
behalf of [`i915_perf_open_ioctl()`](#c.i915_perf_open_ioctl "i915_perf_open_ioctl") with the `gt->perf`.lock mutex
...
[`i915_oa_stream_init()`](#c.i915_oa_stream_init "i915_oa_stream_init"). The code here should only validate config state that
...
void i915\_perf\_destroy\_locked(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream)
...
ssize\_t i915\_perf\_read(struct [file](#c.i915_perf_read "file") \*file, char \_\_user \*buf, size\_t count, loff\_t \*ppos)
...
[`i915_perf_stream_ops->read`](#c.i915_perf_stream_ops "i915_perf_stream_ops") but to save having stream implementations (of
...
long i915\_perf\_ioctl(struct [file](#c.i915_perf_ioctl "file") \*file, unsigned int cmd, unsigned long arg)
...
Implementation deferred to [`i915_perf_ioctl_locked()`](#c.i915_perf_ioctl_locked "i915_perf_ioctl_locked").
...
void i915\_perf\_enable\_locked(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream)
...
void i915\_perf\_disable\_locked(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream)
...
\_\_poll\_t i915\_perf\_poll(struct [file](#c.i915_perf_poll "file") \*file, poll\_table \*wait)
...
Implementation deferred to [`i915_perf_poll_locked()`](#c.i915_perf_poll_locked "i915_perf_poll_locked")

**Return**

any poll events that are ready without sleeping

\_\_poll\_t i915\_perf\_poll\_locked(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream, struct [file](#c.i915_perf_poll_locked "file") \*file, poll\_table \*wait)
...
[`i915_perf_stream_ops->poll_wait`](#c.i915_perf_stream_ops "i915_perf_stream_ops") to call `poll_wait()` with a wait queue that
...
int i915\_oa\_stream\_init(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream, struct drm\_i915\_perf\_open\_param \*param, struct [perf\_open\_properties](#c.perf_open_properties "perf_open_properties") \*props)
...
While [`read_properties_unlocked()`](#c.read_properties_unlocked "read_properties_unlocked") validates properties in isolation it
...
int i915\_oa\_read(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream, char \_\_user \*buf, size\_t count, size\_t \*offset)
:   just calls through to [`i915_oa_ops->read`](#c.i915_oa_ops "i915_oa_ops")
...
void i915\_oa\_stream\_enable(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream)
...
void i915\_oa\_stream\_disable(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream)
...
int i915\_oa\_wait\_unlocked(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream)
...
void i915\_oa\_poll\_wait(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream, struct [file](#c.i915_oa_poll_wait "file") \*file, poll\_table \*wait)
...
`sample_flags`
:   DRM\_I915\_PERF\_PROP\_SAMPLE\_\* properties are tracked as flags
...
`ctx_handle`
:   A gem ctx handle for use with **single\_context**
...
As [`read_properties_unlocked()`](#c.read_properties_unlocked "read_properties_unlocked") enumerates and validates the properties given
to open a stream of metrics the configuration is built up in the structure
which starts out zero initialized.

bool oa\_buffer\_check\_unlocked(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream)
...
also updates the tail in the oa\_buffer object.
...
int append\_oa\_status(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream, char \_\_user \*buf, size\_t count, size\_t \*offset, enum drm\_i915\_perf\_record\_type type)
...
Writes a status record (such as DRM\_I915\_PERF\_RECORD\_OA\_REPORT\_LOST)
...
int append\_oa\_sample(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream, char \_\_user \*buf, size\_t count, size\_t \*offset, const u8 \*report)
...
The contents of a sample are configured through DRM\_I915\_PERF\_PROP\_SAMPLE\_\*
properties when opening a stream, tracked as stream->sample\_flags. This
...
int gen8\_append\_oa\_reports(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream, char \_\_user \*buf, size\_t count, size\_t \*offset)
...
int gen8\_oa\_read(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream, char \_\_user \*buf, size\_t count, size\_t \*offset)
...
int gen7\_append\_oa\_reports(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream, char \_\_user \*buf, size\_t count, size\_t \*offset)
:   Copies all buffered OA reports into userspace read() buffer.
...
int gen7\_oa\_read(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream, char \_\_user \*buf, size\_t count, size\_t \*offset)
:   copy status records then buffered OA reports
...
int oa\_get\_render\_ctx\_id(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream)
...
void oa\_put\_render\_ctx\_id(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream)
...
long i915\_perf\_ioctl\_locked(struct [i915\_perf\_stream](#c.i915_perf_stream "i915_perf_stream") \*stream, unsigned int cmd, unsigned long arg)
:   support ioctl() usage with i915 perf stream FDs
...
int i915\_perf\_ioctl\_version(struct drm\_i915\_private \*i915)
