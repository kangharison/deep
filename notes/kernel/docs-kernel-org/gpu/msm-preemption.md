...
> 출처(원문): https://docs.kernel.org/gpu/msm-preemption.html
...
Additionally the GPU allows to specify a skip\_save\_restore option. This
...
NON\_PRIV records need to be inaccessible from userspace, so they must be mapped
in the kernel address space with the MSM\_BO\_MAP\_PRIV flag.
For example, making the NON\_SECURE record accessible from userspace would allow
