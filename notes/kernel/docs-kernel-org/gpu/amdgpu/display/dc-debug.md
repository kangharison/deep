...
> 출처(원문): https://docs.kernel.org/gpu/amdgpu/display/dc-debug.html
...
* **The DC version (e.g., v3.2.285)**: Display developers release a new DC version
  ...
  version of the display code. Remember from page [Display Core](index.html#amdgpu-display-core),
...
DC has a `struct named` dc\_debug\_options, which is statically initialized by
...
they can set the force\_disable\_subvp field to true in the statically
initialized version of dc\_debug\_options to see if the issue gets fixed. Along
the same lines, users/developers can also try to turn off fams2\_config and
enable\_single\_display\_2to1\_odm\_policy. In summary, the dc\_debug\_options is
