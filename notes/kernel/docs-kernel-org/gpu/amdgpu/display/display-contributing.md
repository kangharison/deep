...
> 출처(원문): https://docs.kernel.org/gpu/amdgpu/display/display-contributing.html
...
The Ftrace debug feature (check ‘[ftrace - Function Tracer](../../../trace/ftrace.html)’) is a
...
‘amdgpu\_dm\_crtc.c` and amdgpu\_dm\_plane.c for some references. However, we
...
bios, core, clk\_mgr, hwss, resource, and irq.
...
### Make atomic\_commit\_[check|tail] more readable

The functions responsible for atomic commit and tail are intricate and
extensive. In particular amdgpu\_dm\_atomic\_commit\_tail is a long function and
...
‘[Multiplane Overlay (MPO)](mpo-overview.html)’) which is intended to
