...
> 출처(원문): https://docs.kernel.org/gpu/automated_testing.html
...
### drivers/gpu/drm/ci/${DRIVER\_NAME}-${HW\_REVISION}-fails.txt

Lists the known failures for a given driver on a specific hardware revision.

### drivers/gpu/drm/ci/${DRIVER\_NAME}-${HW\_REVISION}-flakes.txt
...
### drivers/gpu/drm/ci/${DRIVER\_NAME}-${HW\_REVISION}-skips.txt
...
(see [Runtime locking correctness validator](../locking/lockdep-design.html)) issues have been detected
during the tests.
...
drivers/gpu/drm/ci/${DRIVER\_NAME}\_\*\_fails.txt, for each of the test platforms
...
added more tests to it), update the IGT\_VERSION variable at the top of the
gitlab-ci.yml file.

## How to test your changes to the scripts

For testing changes to the scripts in the drm-ci repo, change the
DRM\_CI\_PROJECT\_PATH and DRM\_CI\_COMMIT\_SHA variables in
