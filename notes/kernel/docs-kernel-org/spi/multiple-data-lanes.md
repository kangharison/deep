...
> 출처(원문): https://docs.kernel.org/spi/multiple-data-lanes.html
...
drivers should set the [`struct spi_transfer.multi_lane_mode`](../driver-api/spi.html#c.spi_transfer "spi_transfer")
...
* `SPI_MULTI_BUS_MODE_SINGLE`: Only use the first lane. Other lanes are
...
* `SPI_MULTI_BUS_MODE_MIRROR`: Send a single data word over all of the
  :   lanes at the same time. This only makes sense for writes and not
      ...
      ```
* `SPI_MULTI_BUS_MODE_STRIPE`: Send or receive two different data words
...
[`struct spi_controller.num_data_lanes`](../driver-api/spi.html#c.spi_controller "spi_controller") to a value
greater than 1.

Then the part of the driver that handles SPI transfers needs to check the
[`struct spi_transfer.multi_lane_mode`](../driver-api/spi.html#c.spi_transfer "spi_transfer") field and implement
