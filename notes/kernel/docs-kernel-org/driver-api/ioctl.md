...
> 출처(원문): https://docs.kernel.org/driver-api/ioctl.html
...
:   An 8-bit number, often a character literal, specific to a subsystem
    or driver, and listed in [Ioctl Numbers](../userspace-api/ioctl/ioctl-number.html)
...
:   The name of the data type pointed to by the argument, the command number
    ...
    Note: do not pass sizeof(data\_type) type into \_IOR/\_IOW/IOWR, as that
    will lead to encoding sizeof(sizeof(data\_type)), i.e. sizeof(size\_t).
    \_IO does not have a data\_type parameter.
...
Prior to Linux 5.5, compat\_ioctl handlers were required to return
...
space timespec exactly. The [`get_timespec64()`](basics.html#c.get_timespec64 "get_timespec64") and [`put_timespec64()`](basics.html#c.put_timespec64 "put_timespec64") helper
...
requires an expensive 64-bit division, a simple \_\_u64 nanosecond value
can be simpler and more efficient.

Timeout values and timestamps should ideally use CLOCK\_MONOTONIC time,
as returned by [`ktime_get_ns()`](../core-api/timekeeping.html#c.ktime_get_ns "ktime_get_ns") or [`ktime_get_ts64()`](../core-api/timekeeping.html#c.ktime_get_ts64 "ktime_get_ts64"). Unlike
CLOCK\_REALTIME, this makes the timestamps immune from jumping backwards
or forwards due to leap second adjustments and `clock_settime()` calls.

[`ktime_get_real_ns()`](../core-api/timekeeping.html#c.ktime_get_real_ns "ktime_get_real_ns") can be used for CLOCK\_REALTIME timestamps that
...
implement the corresponding compat\_ioctl handler.

As long as all the rules for data structures are followed, this is as
easy as setting the .compat\_ioctl pointer to a helper function such as
...
clear the upper bit of a compat\_uptr\_t and turn it into a valid 64-bit
...
compat\_ioctl file operation for drivers that only take arguments that
...
* Pointers have the same problem, in addition to requiring the
  ...
  space, and the use of [`u64_to_user_ptr()`](basics.html#c.u64_to_user_ptr "u64_to_user_ptr") in the kernel to convert
...
* On the x86-32 (i386) architecture, the alignment of 64-bit variables
  ...
  compat\_ioctl conversion handler to translate between the two formats.
...
* `char` members can be either signed or unsigned, depending on
  the architecture, so the \_\_u8 and \_\_s8 types should be used for 8-bit
...
space. This is usually done by calling [`memset()`](../core-api/kernel-api.html#c.memset "memset") before assigning to
individual members.
