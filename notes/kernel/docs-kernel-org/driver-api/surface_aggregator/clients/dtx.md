...
> 출처(원문): https://docs.kernel.org/driver-api/surface_aggregator/clients/dtx.html
> 자동 미러링: docs.kernel.org · 정본은 출처 URL (영문 원문 자동 변환본)

---

# [User-Space DTX (Clipboard Detachment System) Interface](#id13)
...
3. The EC waits for instructions from user-space and acts according to them.
   ...
   * If the latch is locked, the EC will *not* open the latch, meaning the
     ...
     reason `SDTX_DETACH_TIMEDOUT` (see [Events](#events) for details).
...
See [IOCTLs](#ioctls) for more details on these responses.
...
## [User-Space Interface Documentation](#id18)

### [Error Codes and Status Values](#id19)
...
first. All events follow the layout prescribed by [`struct sdtx_event`](#c.sdtx_event "sdtx_event"). Specific
event types can be identified by their event code, described in
[`enum sdtx_event_code`](#c.sdtx_event_code "sdtx_event_code"). Note that other event codes are reserved for future use,
...
#### [`SDTX_EVENT_BASE_CONNECTION`](#id23)
...
provided as payload of type [`struct sdtx_base_info`](#c.sdtx_base_info "sdtx_base_info") with its layout presented
below:
...
#### [`SDTX_IOCTL_EVENTS_DISABLE`](#id28)
...
#### [`SDTX_IOCTL_LATCH_HEARTBEAT`](#id33)
...
#### [`SDTX_IOCTL_GET_DEVICE_MODE`](#id36)
...
See [`struct sdtx_device_mode`](#c.sdtx_device_mode "sdtx_device_mode") for details. Other values are reserved for future
use.

#### [`SDTX_IOCTL_GET_LATCH_STATUS`](#id37)
...
### [Structures and Enums](#id39)

enum sdtx\_device\_mode
...
See [`enum sdtx_event_code`](#c.sdtx_event_code "sdtx_event_code") for currently valid event codes.

enum sdtx\_event\_code
...
Used in [`struct sdtx_event`](#c.sdtx_event "sdtx_event") to describe the type of the event. Further event
