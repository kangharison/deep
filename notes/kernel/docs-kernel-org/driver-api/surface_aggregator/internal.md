...
> 출처(원문): https://docs.kernel.org/driver-api/surface_aggregator/internal.html
...
* [Internal API Documentation](internal-api.html)
  + [Packet Transport Layer](internal-api.html#packet-transport-layer)
  + [Request Transport Layer](internal-api.html#request-transport-layer)
  + [Controller](internal-api.html#controller)
  + [Client Device Bus](internal-api.html#client-device-bus)
  + [Core](internal-api.html#core)
  + [Trace Helpers](internal-api.html#trace-helpers)
...
implemented as platform devices, via [`struct ssam_device`](client-api.html#c.ssam_device "ssam_device") and [`struct ssam_device_driver`](client-api.html#c.ssam_device_driver "ssam_device_driver")
simplify management of client devices and client drivers.

Refer to [Writing Client Drivers](client.html) for
...
that chapter and the [Surface Serial Hub Protocol](ssh.html)
before continuing with the architectural overview below.

## Packet Transport Layer

The packet transport layer is represented via [`struct ssh_ptl`](internal-api.html#c.ssh_ptl "ssh_ptl") and is structured
...
Packets to be transmitted by the SSAM core are represented via [`struct ssh_packet`](client-api.html#c.ssh_packet "ssh_packet")
(in contrast, packets received by the core do not have any specific
structure and are managed entirely via the raw [`struct ssh_frame`](client-api.html#c.ssh_frame "ssh_frame")).
...
be transmitted (i.e. the message wrapped in [`struct ssh_frame`](client-api.html#c.ssh_frame "ssh_frame")). Most notably, it
contains an internal reference count, which is used for managing its
lifetime (accessible via [`ssh_packet_get()`](client-api.html#c.ssh_packet_get "ssh_packet_get") and [`ssh_packet_put()`](client-api.html#c.ssh_packet_put "ssh_packet_put")). When this
counter reaches zero, the `release()` callback provided to the packet via
its [`struct ssh_packet_ops`](client-api.html#c.ssh_packet_ops "ssh_packet_ops") reference is executed, which may then deallocate the
packet or its enclosing structure (e.g. [`struct ssh_request`](client-api.html#c.ssh_request "ssh_request")).

In addition to the `release` callback, the [`struct ssh_packet_ops`](client-api.html#c.ssh_packet_ops "ssh_packet_ops") reference also
...
(minor). See [`SSH_PACKET_PRIORITY()`](client-api.html#c.SSH_PACKET_PRIORITY "SSH_PACKET_PRIORITY") for more details on the priority value.

All packets to be transmitted by the transport layer must be submitted to
this queue via [`ssh_ptl_submit()`](internal-api.html#c.ssh_ptl_submit "ssh_ptl_submit"). Note that this includes control packets
...
[Surface Serial Hub Protocol](ssh.html) for the reasoning behind
...
thread parses and validates the received message into its [`struct ssh_frame`](client-api.html#c.ssh_frame "ssh_frame") and
...
([`ssh_ptl_submit()`](internal-api.html#c.ssh_ptl_submit "ssh_ptl_submit")), packet cancellation ([`ssh_ptl_cancel()`](internal-api.html#c.ssh_ptl_cancel "ssh_ptl_cancel")), data receival
([`ssh_ptl_rx_rcvbuf()`](internal-api.html#c.ssh_ptl_rx_rcvbuf "ssh_ptl_rx_rcvbuf")), and layer shutdown ([`ssh_ptl_shutdown()`](internal-api.html#c.ssh_ptl_shutdown "ssh_ptl_shutdown")) may always be
...
The request transport layer is represented via [`struct ssh_rtl`](internal-api.html#c.ssh_rtl "ssh_rtl") and builds on top
of the packet transport layer. It deals with requests, i.e. SSH packets sent
by the host containing a [`struct ssh_command`](client-api.html#c.ssh_command "ssh_command") as frame payload. This layer
separates responses to requests from events, which are also sent by the EC
via a [`struct ssh_command`](client-api.html#c.ssh_command "ssh_command") payload. While responses are handled in this layer,
...
are represented by [`struct ssh_request`](client-api.html#c.ssh_request "ssh_request"), wrapping the underlying [`struct ssh_packet`](client-api.html#c.ssh_packet "ssh_packet")
storing its message data (i.e. SSH frame with command payload). Note that
all top-level representations, e.g. [`struct ssam_request_sync`](client-api.html#c.ssam_request_sync "ssam_request_sync") are built upon this
struct.

As [`struct ssh_request`](client-api.html#c.ssh_request "ssh_request") extends [`struct ssh_packet`](client-api.html#c.ssh_packet "ssh_packet"), its lifetime is also managed by the
reference counter inside the packet struct (which can be accessed via
[`ssh_request_get()`](client-api.html#c.ssh_request_get "ssh_request_get") and [`ssh_request_put()`](client-api.html#c.ssh_request_put "ssh_request_put")). Once the counter reaches zero, the
`release()` callback of the [`struct ssh_request_ops`](client-api.html#c.ssh_request_ops "ssh_request_ops") reference of the request is
called.
...
flags provided to [`ssh_request_init()`](internal-api.html#c.ssh_request_init "ssh_request_init"), so that the request transport layer
can wait for this response.

Similar to [`struct ssh_packet`](client-api.html#c.ssh_packet "ssh_packet"), [`struct ssh_request`](client-api.html#c.ssh_request "ssh_request") also has a `complete()` callback
...
layer via [`ssh_rtl_submit()`](internal-api.html#c.ssh_rtl_submit "ssh_rtl_submit"). For a request without a response, successful
...
submitted to this queue via [`ssh_rtl_submit()`](internal-api.html#c.ssh_rtl_submit "ssh_rtl_submit"). Once submitted, requests may
...
([`ssh_rtl_submit()`](internal-api.html#c.ssh_rtl_submit "ssh_rtl_submit")), request cancellation ([`ssh_rtl_cancel()`](internal-api.html#c.ssh_rtl_cancel "ssh_rtl_cancel")), and layer
shutdown ([`ssh_rtl_shutdown()`](internal-api.html#c.ssh_rtl_shutdown "ssh_rtl_shutdown")) may always be executed concurrently with
...
[`struct ssam_controller`](internal-api.html#c.ssam_controller "ssam_controller") and the SSH driver. While the lower level transport layers
...
delivery and registration via the (event) completion system ([`struct ssam_cplt`](internal-api.html#c.ssam_cplt "ssam_cplt")).
...
represented by [`struct ssam_event_registry`](client-api.html#c.ssam_event_registry "ssam_event_registry"). As parameters to this request, the
...
ID form the event ID, represented by [`struct ssam_event_id`](client-api.html#c.ssam_event_id "ssam_event_id"). In short, both, event
...
category specified in [`struct ssam_event_id`](client-api.html#c.ssam_event_id "ssam_event_id").
...
event ID combinations). See [`struct ssam_nf`](internal-api.html#c.ssam_nf "ssam_nf"), [`ssam_nf_refcount_inc()`](internal-api.html#c.ssam_nf_refcount_inc "ssam_nf_refcount_inc"), and
[`ssam_nf_refcount_dec()`](internal-api.html#c.ssam_nf_refcount_dec "ssam_nf_refcount_dec") for details.

This management is done together with notifier registration (described in
the next section) via the top-level [`ssam_notifier_register()`](client-api.html#c.ssam_notifier_register "ssam_notifier_register") and
[`ssam_notifier_unregister()`](client-api.html#c.ssam_notifier_unregister "ssam_notifier_unregister") functions.

### Event Delivery

To receive events, a client driver has to register an event notifier via
[`ssam_notifier_register()`](client-api.html#c.ssam_notifier_register "ssam_notifier_register"). This increments the reference counter for that
...
([`struct ssam_event_queue`](internal-api.html#c.ssam_event_queue "ssam_event_queue")). From this event queue the completion work item of that
...
(e.g. [`ssam_notifier_register()`](client-api.html#c.ssam_notifier_register "ssam_notifier_register"), [`ssam_notifier_unregister()`](client-api.html#c.ssam_notifier_unregister "ssam_notifier_unregister"),
[`ssam_request_sync_submit()`](client-api.html#c.ssam_request_sync_submit "ssam_request_sync_submit"), and derivatives) are executed and this guarantee
is not already provided otherwise (e.g. through [`ssam_client_bind()`](client-api.html#c.ssam_client_bind "ssam_client_bind") or
[`ssam_client_link()`](client-api.html#c.ssam_client_link "ssam_client_link")). The writer part guards any transitions that will change
...
smoke-testing against invalid API usage (e.g. in [`ssam_request_sync_submit()`](client-api.html#c.ssam_request_sync_submit "ssam_request_sync_submit")).
...
each other. This includes [`ssam_notifier_register()`](client-api.html#c.ssam_notifier_register "ssam_notifier_register"), [`ssam_notifier_unregister()`](client-api.html#c.ssam_notifier_unregister "ssam_notifier_unregister"),
[`ssam_request_sync_submit()`](client-api.html#c.ssam_request_sync_submit "ssam_request_sync_submit"), as well as all functions building on top of those.
