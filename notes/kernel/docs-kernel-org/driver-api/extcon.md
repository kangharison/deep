...
> 출처(원문): https://docs.kernel.org/driver-api/extcon.html
...
int extcon\_get\_state(struct extcon\_dev \*edev, const unsigned int id)
...
int extcon\_set\_state(struct extcon\_dev \*edev, unsigned int id, bool state)
...
have to use [`extcon_set_state_sync()`](#c.extcon_set_state_sync "extcon_set_state_sync") and `extcon_sync()`.

Returns 0 if success or error number if fail.

int extcon\_set\_state\_sync(struct extcon\_dev \*edev, unsigned int id, bool state)
...
int extcon\_get\_property(struct extcon\_dev \*edev, unsigned int id, unsigned int prop, union extcon\_property\_value \*prop\_val)
