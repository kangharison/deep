...
> 출처(원문): https://docs.kernel.org/driver-api/driver-model/porting.html
...
* Define a [`struct bus_type`](../infrastructure.html#c.bus_type "bus_type") for the bus driver:
...
* Register the bus type.
  ...
  which is usually the [`module_init()`](../basics.html#c.module_init "module_init"), or equivalent, function:
...
[`struct device`](../infrastructure.html#c.device "device") represents a single device. It mainly contains metadata
describing the relationship the device has to other entities.

* Embed a [`struct device`](../infrastructure.html#c.device "device") in the bus-specific device type:
...
* Initialize the device on registration.
  ...
  things to initialize are the bus\_id, parent, and bus fields.

  The bus\_id is an ASCII string that contains the device’s address on
  ...
  belongs to. This should be set to the bus\_type that was declared
...
[`struct device_driver`](../infrastructure.html#c.device_driver "device_driver") is a simple driver structure that contains a set
of operations that the driver model core may call.

* Embed a [`struct device_driver`](../infrastructure.html#c.device_driver "device_driver") in the bus-specific driver.
...
[`struct device_driver`](../infrastructure.html#c.device_driver "device_driver") defines a set of operations that the driver model
...
Instead, a bus may supply a method in [`struct bus_type`](../infrastructure.html#c.bus_type "bus_type") that does the
comparison:
...
-EPROBE\_DEFER) if determining that given driver supports the device is
not possible.
...
[`struct bus_type`](../infrastructure.html#c.bus_type "bus_type"):
...
[`struct bus_type`](../infrastructure.html#c.bus_type "bus_type") contains a list of all devices registered with the bus
...
[`struct bus_type`](../infrastructure.html#c.bus_type "bus_type") also contains a list of all drivers registered with
...
[`struct bus_type`](../infrastructure.html#c.bus_type "bus_type") contains an rwsem that protects all core accesses to
...
Some of the fields in [`struct device`](../infrastructure.html#c.device "device") and [`struct device_driver`](../infrastructure.html#c.device_driver "device_driver") duplicate
