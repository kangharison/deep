...
> 출처(원문): https://docs.kernel.org/driver-api/driver-model/driver.html
...
See the kerneldoc for the [`struct device_driver`](../infrastructure.html#c.device_driver "device_driver").

## Allocation

Device drivers are statically allocated structures. Though there may
be multiple devices in a system that a driver supports, [`struct
device_driver`](../infrastructure.html#c.device_driver "device_driver") represents the driver as a whole (not a particular
device instance).
...
As stated above, [`struct device_driver`](../infrastructure.html#c.device_driver "device_driver") objects are statically
...
Bus-specific drivers should include a generic [`struct device_driver`](../infrastructure.html#c.device_driver "device_driver") in
...
structure), they would use driver\_register and pass a pointer to their
[`struct device_driver`](../infrastructure.html#c.device_driver "device_driver") object.

Most drivers, however, will have a bus-specific structure and will
need to register with the bus using something like pci\_driver\_register.

It is important that drivers register their driver structure as early as
possible. Registration with the core initializes several fields in the
[`struct device_driver`](../infrastructure.html#c.device_driver "device_driver") object, including the reference count and the
...
[`container_of()`](../basics.html#c.container_of "container_of") to convert “dev” to a bus-specific type, both in `probe()`
and other routines. That type often provides device resource data, such
as pci\_dev.resource[] or platform\_device.resources, which is used in
addition to dev->platform\_data to initialize the driver.
...
Optionally, `probe()` may return -EPROBE\_DEFER if the driver depends on
resources that are not yet available (e.g., supplied by a driver that
hasn’t initialized yet). The driver core will put the device onto the
deferred probe list and will try to call it again later. If a driver
must defer, it should return -EPROBE\_DEFER as early as possible to
reduce the amount of time spent on setup work that will need to be
unwound and reexecuted at a later time.

Warning

-EPROBE\_DEFER must not be returned if `probe()` has already created
child devices, even if those child devices are removed again
in a cleanup path. If -EPROBE\_DEFER is returned after a child
...
sync\_state is called only once for a device. It’s called when all the consumer
...
device; i.e. anything in the device’s driver\_data field.
...
Drivers can declare attributes using a DRIVER\_ATTR\_RW and DRIVER\_ATTR\_RO
macro that works identically to the DEVICE\_ATTR\_RW and DEVICE\_ATTR\_RO
macros.
