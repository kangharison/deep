...
> 출처(원문): https://docs.kernel.org/driver-api/uio-howto.html
...
calling [`uio_event_notify()`](infrastructure.html#c.uio_event_notify "uio_event_notify") from the timer’s event
handler.
...
* `unsigned long irq_flags`: Required if you’ve set `irq` to a
  ...
  call to [`request_irq()`](../core-api/genericirq.html#c.request_irq "request_irq").
...
* `int memtype`: Required if the mapping is used. Set this to
  ...
  with `__get_free_pages()` but not [`kmalloc()`](../core-api/mm-api.html#c.kmalloc "kmalloc")). There’s also
...
* `void *internal_addr`: If you have to access this memory region
  ...
  using something like [`ioremap()`](device-io.html#c.ioremap "ioremap"). Addresses returned by
...
### Using uio\_pdrv for platform devices
...
### Using uio\_pdrv\_genirq for platform devices
...
the interrupt line using [`disable_irq_nosync()`](../core-api/genericirq.html#c.disable_irq_nosync "disable_irq_nosync"). After
...
### Using uio\_dmem\_genirq for platform devices
...
### Things to know about uio\_pci\_generic
...
### Writing userspace driver using uio\_pci\_generic
...
### Example code using uio\_pci\_generic
...
### Things to know about uio\_hv\_generic
...
If a subchannel is created by a request to host, then the uio\_hv\_generic
