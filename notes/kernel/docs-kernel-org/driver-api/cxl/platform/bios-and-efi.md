...
> 출처(원문): https://docs.kernel.org/driver-api/cxl/platform/bios-and-efi.html
...
One notable configuration here is the EFI\_MEMORY\_SP (Specific Purpose) bit.
...
If providing cross-host-bridge interleave, a CFMWS entry in the [CEDT](acpi/cedt.html) must be presented with target host-bridges for the interleaved
...
memory regions in the [SRAT](acpi/srat.html) to determine the number of
