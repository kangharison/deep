...
> 출처(원문): https://docs.kernel.org/driver-api/cxl/linux/access-coordinates.html
...
provided via ACPI tables [SRAT](../platform/acpi/srat.html) and
[HMAT](../platform/acpi/hmat.html). However, the platform firmware (BIOS) is
...
The [SRAT](../platform/acpi/srat.html) provides a Generic Port Affinity
...
[HMAT](../platform/acpi/hmat.html) subtable. This piece represents the
performance coordinates between a CPU and a Generic Port (CXL hostbridge).

The [CDAT](../platform/cdat.html) provides the performance coordinates for
...
Window Structure (CFMWS) in the [CEDT](../platform/acpi/cedt.html).
...
[`cxl_endpoint_gather_bandwidth()`](../theory-of-operation.html#c.cxl_endpoint_gather_bandwidth "cxl_endpoint_gather_bandwidth") function. The `min()` of bandwidth from the
...
or walking through the switch(es), [`cxl_rp_gather_bandwidth()`](../theory-of-operation.html#c.cxl_rp_gather_bandwidth "cxl_rp_gather_bandwidth") is called. At
...
via ACPI tables ([SRAT](../platform/acpi/srat.html) and
[HMAT](../platform/acpi/hmat.html)). The minimum bandwidth are aggregated
under the same ACPI0017 device to form a new xarray.

Finally, the [`cxl_region_update_bandwidth()`](../theory-of-operation.html#c.cxl_region_update_bandwidth "cxl_region_update_bandwidth") is called and the aggregated
...
Each [CEDT](../platform/acpi/cedt.html) has a QTG ID field. This field provides
