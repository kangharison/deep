...
> 출처(원문): https://docs.kernel.org/driver-api/cxl/linux/early-boot.html
...
During Linux Early Boot stage (functions in the kernel that have the \_\_init
decorator), the system takes the resources created by EFI/BIOS
([ACPI tables](../platform/acpi.html)) and turns them into resources that the
kernel can consume.
...
* CONFIG\_MHP\_DEFAULT\_ONLINE\_TYPE
  ...
  + Linux Build config that dictates whether and how Specific Purpose memory
    ...
    SystemRAM in ZONE\_NORMAL or ZONE\_MOVABLE).
* nosoftreserve

  + Linux kernel boot option that dictates whether Soft Reserve should be
    supported. Similar to CONFIG\_EFI\_SOFT\_RESERVE.
...
Linux refers to the proximity domains (`PXM`) defined in the [SRAT](../platform/acpi/srat.html) to create NUMA nodes in `acpi_numa_init`.
...
[CEDT](../platform/acpi/cedt.html) adds a description of SPA ranges which
...
See [Example Platform Configurations](../platform/example-configs.html)
for more info.
...
[HMAT](../platform/acpi/hmat.html) and CDAT information for the CXL nodes. All
nodes default to the DRAM tier, unless HMAT/CDAT information is reported to the
memory\_tier component via access\_coordinates.

For more, see [CXL access coordinates documentation](access-coordinates.html).
...
SystemRAM in ZONE\_NORMAL during early boot, CMA reservations per-node can be
