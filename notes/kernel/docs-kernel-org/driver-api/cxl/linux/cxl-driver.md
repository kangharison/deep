...
> 출처(원문): https://docs.kernel.org/driver-api/cxl/linux/cxl-driver.html
...
* cxl\_core - fundamental init interface and core object creation
* cxl\_port - initializes root and provides port enumeration interface.
* cxl\_acpi - initializes root decoders and interacts with ACPI data.
* cxl\_p/mem - initializes memory devices
* cxl\_pci - uses cxl\_port to enumerate the actual fabric hierarchy.
...
The CXL Root is logical object created by the cxl\_acpi driver during
`cxl_acpi_probe` - if the `ACPI0017` Compute Express Link
Root Object Device Class is found.

The Root contains links to:

* Host Bridge Ports defined by CHBS in the [CEDT](../platform/acpi/cedt.html)
* Downstream Ports typically connected to Host Bridge Ports.
* Root Decoders defined by CFMWS the [CEDT](../platform/acpi/cedt.html)
...
will be probed at acpi\_probe time, while similar ports on an actual switch
...
A memdev is probed and added by the cxl\_pci driver in `cxl_pci_probe`
and is managed by the cxl\_mem driver. It primarily provides the IOCTL
...
configurations present in the CFMWS field of the [CEDT](../platform/acpi/cedt.html).
...
is created per CFMWS entry in the [CEDT](../platform/acpi/cedt.html).
...
host bridge UIDs in the CHBS field of the [CEDT](../platform/acpi/cedt.html) and the UID field of CXL Host Bridges defined in
the [DSDT](../platform/acpi/dsdt.html).
...
Root decoder interleave is defined by CFMWS field of the [CEDT](../platform/acpi/cedt.html). The CEDT may actually define multiple CFMWS
...
* [Single Device](example-configurations/single-device.html)
* [Inter-Host-Bridge Interleave](example-configurations/hb-interleave.html)
* [Intra-Host-Bridge Interleave](example-configurations/intra-hb-interleave.html)
* [Multi-Level Interleave](example-configurations/multi-interleave.html)
