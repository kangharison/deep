...
> 출처(원문): https://docs.kernel.org/driver-api/usb/gadget.html
...
[`usb_gadget_driver`](#c.usb_gadget_driver "usb_gadget_driver"), which is responsible for most parts of enumeration
for a [`struct usb_gadget`](#c.usb_gadget "usb_gadget"). The response to a set\_configuration usually
involves enabling one or more of the [`struct usb_ep`](#c.usb_ep "usb_ep") objects exposed by
the gadget, and submitting one or more [`struct usb_request`](#c.usb_request "usb_request") buffers to
...
2. Register a gadget driver that implements some higher level device
   function. That will then `bind()` to a [`usb_gadget`](#c.usb_gadget "usb_gadget"), which activates
...
the [linux/usb/ch9.h](usb.html#usb-chapter9) header file, which is standard in
...
`ep`
:   The associated endpoint set by `usb_ep_alloc_request()`.
...
`is_last`
:   Indicates if this is the last request of a stream\_id before
...
this is analogous to ‘[`struct urb`](usb.html#c.urb "urb")’ on the host side, except that
...
gadget->ep\_list. the control endpoint (gadget->ep0) is not in that list,
and is accessed only in response to a driver `setup()` callback.

void free\_usb\_request(struct [usb\_request](#c.usb_request "usb_request") \*req)
:   frees a usb\_request object and its buffer
...
with `DEFINE_FREE()` to enable automatic, scope-based cleanup for usb\_request
pointers.
...
`state_lock`
:   Spinlock protecting the state and teardown members.

`teardown`
:   True if the device is undergoing teardown, used to prevent
...
:   udc/platform wants to avoid [`skb_reserve()`](../../networking/kapi.html#c.skb_reserve "skb_reserve") in
...
`setup()` call corresponding to USB\_REQ\_SET\_CONFIGURATION, and before
driver `suspend()` calls. They are valid only when is\_otg, and when the
device is acting as a B-Peripheral (so is\_a\_peripheral is false).

size\_t usb\_ep\_align(struct [usb\_ep](#c.usb_ep "usb_ep") \*ep, size\_t len)
...
size\_t usb\_ep\_align\_maybe(struct [usb\_gadget](#c.usb_gadget "usb_gadget") \*g, struct [usb\_ep](#c.usb_ep "usb_ep") \*ep, size\_t len)
:   returns **len** aligned to ep’s maxpacketsize if gadget requires quirk\_ep\_out\_aligned\_size, otherwise returns len.
...
int gadget\_is\_altset\_supported(struct [usb\_gadget](#c.usb_gadget "usb_gadget") \*g)
...
int gadget\_is\_stall\_supported(struct [usb\_gadget](#c.usb_gadget "usb_gadget") \*g)
...
int gadget\_is\_zlp\_supported(struct [usb\_gadget](#c.usb_gadget "usb_gadget") \*g)
...
int gadget\_avoids\_skb\_reserve(struct [usb\_gadget](#c.usb_gadget "usb_gadget") \*g)
...
int gadget\_is\_dualspeed(struct [usb\_gadget](#c.usb_gadget "usb_gadget") \*g)
...
int gadget\_is\_superspeed(struct [usb\_gadget](#c.usb_gadget "usb_gadget") \*g)
...
int gadget\_is\_superspeed\_plus(struct [usb\_gadget](#c.usb_gadget "usb_gadget") \*g)
...
int gadget\_is\_otg(struct [usb\_gadget](#c.usb_gadget "usb_gadget") \*g)
...
If gadget->is\_otg is true, the gadget driver must provide an OTG
...
The driver’s `setup()` callback must also implement set\_configuration,
...
Currently, a number of UDC drivers rely on USB\_GADGET\_DELAYED\_STATUS
being returned from the `setup()` callback, which is a bug. See the comment
next to USB\_GADGET\_DELAYED\_STATUS for details.
...
int usb\_gadget\_register\_driver\_owner(struct [usb\_gadget\_driver](#c.usb_gadget_driver "usb_gadget_driver") \*driver, struct module \*owner, const char \*mod\_name)
...
int usb\_gadget\_unregister\_driver(struct [usb\_gadget\_driver](#c.usb_gadget_driver "usb_gadget_driver") \*driver)
...
If you’re using [`usb_gadget_get_string()`](#c.usb_gadget_get_string "usb_gadget_get_string"), use this to wrap a string
...
If you’re using [`usb_gadget_get_string()`](#c.usb_gadget_get_string "usb_gadget_get_string"), use this to wrap all the
strings for a given language.

void usb\_free\_descriptors(struct usb\_descriptor\_header \*\*v)
:   free descriptors returned by [`usb_copy_descriptors()`](#c.usb_copy_descriptors "usb_copy_descriptors")
...
int usb\_gadget\_get\_string(const struct [usb\_gadget\_strings](#c.usb_gadget_strings "usb_gadget_strings") \*table, int id, u8 \*buf)
...
int usb\_descriptor\_fillbuf(void \*buf, unsigned buflen, const struct usb\_descriptor\_header \*\*src)
...
struct usb\_descriptor\_header \*\*usb\_copy\_descriptors(struct usb\_descriptor\_header \*\*src)
...
Devices using this framework provide a [`struct usb_composite_driver`](#c.usb_composite_driver "usb_composite_driver"),
which in turn provides one or more [`struct usb_configuration`](#c.usb_configuration "usb_configuration")
instances. Each such configuration includes at least one struct
[`usb_function`](#c.usb_function "usb_function"), which packages a user visible role such as “network
...
`config`
:   assigned when **[`usb_add_function()`](#c.usb_add_function "usb_add_function")** is called; this is the
...
`free_func`
:   free the [`struct usb_function`](#c.usb_function "usb_function").
...
associated by **[`usb_add_function()`](#c.usb_add_function "usb_add_function")** with a one configuration; that function
...
A more complex strategy might encapsulate a **usb\_function** structure inside
...
`cdev`
:   assigned by **[`usb_add_config()`](#c.usb_add_config "usb_add_config")** before calling **`bind()`**; this is
...
of the fields described above, and calling **[`usb_add_config()`](#c.usb_add_config "usb_add_config")** to set up
internal data and bind it to a specific device. The configuration’s
**`bind()`** method is then used to initialize all the functions and then
call **[`usb_add_function()`](#c.usb_add_function "usb_add_function")** for them.
...
:   (REQUIRED) Used to allocate resources that are shared across the
    ...
    **[`usb_add_config()`](#c.usb_add_config "usb_add_config")**. This may fail by returning a negative errno
...
module\_usb\_composite\_driver
...
module may only use this macro once, and calling it replaces [`module_init()`](../basics.html#c.module_init "module_init")
and [`module_exit()`](../basics.html#c.module_exit "module_exit")
...
int config\_ep\_by\_speed\_and\_alt(struct [usb\_gadget](#c.usb_gadget "usb_gadget") \*g, struct [usb\_function](#c.usb_function "usb_function") \*f, struct [usb\_ep](#c.usb_ep "usb_ep") \*\_ep, u8 alt)
...
int config\_ep\_by\_speed(struct [usb\_gadget](#c.usb_gadget "usb_gadget") \*g, struct [usb\_function](#c.usb_function "usb_function") \*f, struct [usb\_ep](#c.usb_ep "usb_ep") \*\_ep)
:   configures the given endpoint according to gadget speed.
...
int usb\_add\_function(struct [usb\_configuration](#c.usb_configuration "usb_configuration") \*config, struct [usb\_function](#c.usb_function "usb_function") \*function)
...
int usb\_function\_deactivate(struct [usb\_function](#c.usb_function "usb_function") \*function)
...
int usb\_function\_activate(struct [usb\_function](#c.usb_function "usb_function") \*function)
:   allow function and gadget enumeration

**Parameters**

`struct usb_function *function`
:   function on which [`usb_function_activate()`](#c.usb_function_activate "usb_function_activate") was called

**Description**

Reverses effect of [`usb_function_deactivate()`](#c.usb_function_deactivate "usb_function_deactivate"). If no more functions
are delaying their activation, the gadget driver will respond to
host enumeration procedures.

Returns zero on success, else negative errno.

int usb\_interface\_id(struct [usb\_configuration](#c.usb_configuration "usb_configuration") \*config, struct [usb\_function](#c.usb_function "usb_function") \*function)
...
[`usb_interface_id()`](#c.usb_interface_id "usb_interface_id") is called from usb\_function.`bind()` callbacks to
...
int usb\_func\_wakeup(struct [usb\_function](#c.usb_function "usb_function") \*func)
...
int usb\_add\_config(struct [usb\_composite\_dev](#c.usb_composite_dev "usb_composite_dev") \*cdev, struct [usb\_configuration](#c.usb_configuration "usb_configuration") \*config, int (\*bind)(struct [usb\_configuration](#c.usb_configuration "usb_configuration")\*))
...
int usb\_string\_id(struct [usb\_composite\_dev](#c.usb_composite_dev "usb_composite_dev") \*cdev)
...
**[`usb_string_id()`](#c.usb_string_id "usb_string_id")** is called from `bind()` callbacks to allocate
string IDs. Drivers for functions, configurations, or gadgets will
then store that ID in the appropriate descriptors and string table.

All string identifier should be allocated using this,
**[`usb_string_ids_tab()`](#c.usb_string_ids_tab "usb_string_ids_tab")** or **[`usb_string_ids_n()`](#c.usb_string_ids_n "usb_string_ids_n")** routine, to ensure
that for example different functions don’t wrongly assign different
meanings to the same identifier.

int usb\_string\_ids\_tab(struct [usb\_composite\_dev](#c.usb_composite_dev "usb_composite_dev") \*cdev, struct [usb\_string](#c.usb_string "usb_string") \*str)
...
All string identifier should be allocated using this,
**[`usb_string_id()`](#c.usb_string_id "usb_string_id")** or **[`usb_string_ids_n()`](#c.usb_string_ids_n "usb_string_ids_n")** routine, to ensure that for
example different functions don’t wrongly assign different meanings
to the same identifier.

struct [usb\_string](#c.usb_string "usb_string") \*usb\_gstrings\_attach(struct [usb\_composite\_dev](#c.usb_composite_dev "usb_composite_dev") \*cdev, struct [usb\_gadget\_strings](#c.usb_gadget_strings "usb_gadget_strings") \*\*sp, unsigned n\_strings)
...
`unsigned n_strings`
:   number of entries in each usb\_strings array (sp[]->strings)
...
copied but only a referenced will be made. The [`struct usb_gadget_strings`](#c.usb_gadget_strings "usb_gadget_strings")
array may contain multiple languages and should be NULL terminated.
The ->language pointer of each [`struct usb_gadget_strings`](#c.usb_gadget_strings "usb_gadget_strings") has to contain the
same amount of entries.
For instance: sp[0] is en-US, sp[1] is es-ES. It is expected that the first
usb\_string entry of es-ES contains the translation of the first usb\_string
entry of en-US. Therefore both entries become the same id assign.

int usb\_string\_ids\_n(struct [usb\_composite\_dev](#c.usb_composite_dev "usb_composite_dev") \*c, unsigned n)
:   allocate unused string IDs in batch
...
**[`usb_string_ids_n()`](#c.usb_string_ids_n "usb_string_ids_n")** is called from `bind()` callbacks to allocate
string IDs. Drivers for functions, configurations, or gadgets will
then store that ID in the appropriate descriptors and string table.
...
int usb\_composite\_probe(struct [usb\_composite\_driver](#c.usb_composite_driver "usb_composite_driver") \*driver)
...
void usb\_composite\_unregister(struct [usb\_composite\_driver](#c.usb_composite_driver "usb_composite_driver") \*driver)
...
void usb\_composite\_setup\_continue(struct [usb\_composite\_dev](#c.usb_composite_dev "usb_composite_dev") \*cdev)
...
controller driver interfaces (`usb_bus` or [`usb_gadget`](#c.usb_gadget "usb_gadget")).
...
and [`usb_gadget`](#c.usb_gadget "usb_gadget") driver interfaces; those aren’t discussed here in any
