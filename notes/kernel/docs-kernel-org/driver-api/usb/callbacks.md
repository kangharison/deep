...
> 출처(원문): https://docs.kernel.org/driver-api/usb/callbacks.html
...
completion callback can be found in [USB Request Block (URB)](URB.html#usb-urb).
...
separately in [Power Management for USB](power-management.html#usb-power-management).
...
[`usb_set_intfdata()`](usb.html#c.usb_set_intfdata "usb_set_intfdata"), to associate a data structure with an interface, so
