...
> 출처(원문): https://docs.kernel.org/driver-api/usb/power-management.html
...
methods in its [`usb_driver`](usb.html#c.usb_driver "usb_driver") structure, and the `reset_resume` method
...
>   outstanding URBs ([`usb_kill_urb()`](usb.html#c.usb_kill_urb "usb_kill_urb")) and not submit any more.
...
[USB device persistence during system suspend](persist.html#usb-persist)) and it can also be used under certain
...
usb\_interface’s embedded device structure. When the counter is > 0
...
Linux Plumbers Conference 2012 slides [[1]](#f1) and video [[2]](#f2):

Upon receiving a `ClearPortFeature(PORT_POWER)` request a USB port is
logically off, and may trigger the actual loss of VBUS to the port [[3]](#f3).
...
need to unbind the interface drivers before the [`usb_device`](usb.html#c.usb_device "usb_device") will
