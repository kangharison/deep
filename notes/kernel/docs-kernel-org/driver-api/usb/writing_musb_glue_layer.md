...
> 출처(원문): https://docs.kernel.org/driver-api/usb/writing_musb_glue_layer.html
...
device namespace collisions with explicit IDs. With [`devm_clk_get()`](../../core-api/kernel-api.html#c.devm_clk_get "devm_clk_get")
(line 18) the glue layer allocates the clock -- the `devm_` prefix
indicates that [`clk_get()`](../../core-api/kernel-api.html#c.clk_get "clk_get") is managed: it automatically frees the
...
held at that point, through [`platform_device_add_resources()`](../infrastructure.html#c.platform_device_add_resources "platform_device_add_resources") (line 9).

Finally comes passing on the platform specific data to the controller
driver (line 16). Platform data will be discussed in
[Device Platform Data](#musb-dev-platform-data), but here we are looking at the
...
will be discussed later in [Device Quirks](#musb-dev-quirks) and
[Handling IRQs](#musb-handling-irqs)
...
device controller, which will be discussed later in [Device Quirks](#musb-dev-quirks).
...
`jz4740_driver` platform driver structure in [Linux MUSB Basics](#musb-basics).
...
already set to allocate an automatic id in [Linux MUSB Basics](#musb-basics). In the dev field
...
[Device Quirks](#musb-dev-quirks). Last two fields (line 8 and 9) are also
...
[Writing USB Device Drivers](writing_usb_driver.html#writing-usb-driver)
