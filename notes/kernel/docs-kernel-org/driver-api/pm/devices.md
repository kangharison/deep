...
> 출처(원문): https://docs.kernel.org/driver-api/pm/devices.html
...
[`struct dev_pm_ops`](types.html#c.dev_pm_ops "dev_pm_ops") defined in `include/linux/pm.h`. The roles of the
...
[`struct dev_pm_ops`](types.html#c.dev_pm_ops "dev_pm_ops") objects and it is suitable only for implementing system
...
[`struct dev_pm_ops`](types.html#c.dev_pm_ops "dev_pm_ops") pointed to by the `ops` member of
[`struct dev_pm_domain`](types.html#c.dev_pm_domain "dev_pm_domain"), or by the `pm` member of [`struct bus_type`](../infrastructure.html#c.bus_type "bus_type"),
`struct device_type` and [`struct class`](../infrastructure.html#c.class "class"). They are mostly of interest to the
...
[Runtime Power Management Framework for I/O Devices](../../power/runtime_pm.html).
...
and [`pci_enable_wake()`](../pci/pci.html#c.pci_enable_wake "pci_enable_wake") does something similar for the PCI PME signal.
...
For details refer to [Suspend/Hibernation Notifiers](notifiers.html).
...
[`struct device`](../infrastructure.html#c.device "device"). This field is a pointer to an object of type
[`struct dev_pm_domain`](types.html#c.dev_pm_domain "dev_pm_domain"), defined in `include/linux/pm.h`, providing a set
...
[Runtime Power Management Framework for I/O Devices](../../power/runtime_pm.html) for more information). If an
...
the full-power state, as explained in [Runtime Power Management Framework for I/O Devices](../../power/runtime_pm.html).
...
`DPM_FLAG_SMART_SUSPEND` flag is set (see [above](#smart-suspend-flag)).
