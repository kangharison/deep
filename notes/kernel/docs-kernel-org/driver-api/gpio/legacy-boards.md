...
> 출처(원문): https://docs.kernel.org/driver-api/gpio/legacy-boards.html
...
such as [`gpiod_get()`](index.html#c.gpiod_get "gpiod_get"), [`gpiod_get_optional()`](index.html#c.gpiod_get_optional "gpiod_get_optional"), and others will work.
...
1. **The GPIO controller’s software node must be registered and attached to
   the controller’s ``[`struct device`](../infrastructure.html#c.device "device")`` either as its primary or secondary
...
3. **The reference must have exactly two arguments:**
   ...
   * The second argument is the flags for the GPIO line (e.g.,
     GPIO\_ACTIVE\_HIGH, GPIO\_ACTIVE\_LOW).
...
software nodes using the `fwnode` field in [`struct platform_device_info`](../infrastructure.html#c.platform_device_info "platform_device_info").
