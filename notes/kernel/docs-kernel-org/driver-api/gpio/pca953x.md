...
> 출처(원문): https://docs.kernel.org/driver-api/gpio/pca953x.html
...
Author:
:   Levente Révész <[levente.revesz@eilabs.com](mailto:levente.revesz%40eilabs.com)>
...
### Chips with additional timeout\_en register

These Maxim chips have a bus timeout function which can be enabled in
the timeout\_en register. This is present in only two chips. Defaults to
timeout disabled.

| compatible | lines | interrupt | input | output | invert | direction | timeout\_en |
| --- | --- | --- | --- | --- | --- | --- | --- |
| max7310 | 8 | no | 00 | 01 | 02 | 03 | 04 |
| max7312 | 16 | yes | 00 | 02 | 04 | 06 | 08 |

### Chips with additional int\_mask register
...
use interrupts with these chips, the driver has to set the int\_mask
register.

| compatible | lines | interrupt | input | output | invert | direction | int\_mask |
| --- | --- | --- | --- | --- | --- | --- | --- |
| pca9505 | 40 | yes | 00 | 08 | 10 | 18 | 20 |
| pca9506 | 40 | yes | 00 | 08 | 10 | 18 | 20 |

### Chips with additional int\_mask and out\_conf registers
...
are present in PCAL chips as well, albeit the out\_conf works
differently.

| compatible | lines | interrupt | input | output | invert | direction | int\_mask | out\_conf |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| compatible | master\_output | mode |
| --- | --- | --- |
| pca9698 | 29 | 2A |
...
master\_intensity register and the intensity registers together define
...
| compatible | lines | interrupt | input | output0 | direction | output1 | master\_intensity | blink | intensity |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
> 1. output drive strength setting (out\_strength)
> 2. input latch (in\_latch)
> 3. pull-up/pull-down (pull\_in, pull\_sel)
> 4. push-pull/open-drain outputs (out\_conf)
> 5. interrupt mask and interrupt status (int\_mask, int\_status)

| compatible | out\_strength | in\_latch | pull\_en | pull\_sel | int\_mask | int\_status | out\_conf |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
and uses int\_mask and int\_status for interrupts.
...
| compatible | int\_edge | int\_clear | input\_status | indiv\_out\_conf | debounce | debounce\_count |
| --- | --- | --- | --- | --- | --- | --- |
...
> 1. pull-up/pull-down (pull\_sel)
> 2. a global pull enable, defaults to disabled (config)
> 3. interrupt mask, interrupt status (int\_mask, int\_status)

| compatible | lines | interrupt | input | invert | config | pull\_sel | direction | output | int\_mask | int\_status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
>    (int\_mask, rising\_mask, falling\_mask)
> 4. pull-up (no pull-down)
> 5. tri-state
> 6. debounce

| compatible | lines | interrupt | input | output | invert | direction | pullup\_en |
| --- | --- | --- | --- | --- | --- | --- | --- |
| xra1202 | 8 | yes | 00 | 01 | 02 | 03 | 04 |

| compatible | int\_mask | tristate | int\_status | rising\_mask | falling\_mask | debounce |
| --- | --- | --- | --- | --- | --- | --- |
...
[Chips with LED blink and intensity control](#chips-with-led-blink-and-intensity-control) are missing the invert
register.
...
[PCAL chips with extended interrupt and output configuration functions](#pcal-chips-with-extended-interrupt-and-output-configuration-functions)
have an int\_edge register. This contains 2 bits per line, one of 4 events
...
>    (out\_conf is 1 byte, not (lines/8) bytes, hence the 0x01 offset)
>
>    > * pcal6534

[XRA1202](#xra1202) chips have a different mechanism for the same thing: they have
...
Only [Basic PCAL chips](#basic-pcal-chips) and
[PCAL chips with extended interrupt and output configuration functions](#pcal-chips-with-extended-interrupt-and-output-configuration-functions)
...
are possible. Refer to the chapter [Interrupt source detection](#interrupt-source-detection) to understand
...
[Basic PCAL chips](#basic-pcal-chips) and
[PCAL chips with extended interrupt and output configuration functions](#pcal-chips-with-extended-interrupt-and-output-configuration-functions)
use the same mechanism: their pull\_en register enables the pull-up or pull-down
function, and their pull\_sel register chooses the direction. They all use one
bit per line.
...
[PCA957X chips with a completely different register layout](#pca957x-chips-with-a-completely-different-register-layout) have a pull\_sel
register with one bit per line, and a global pull\_en bit in their config
register.
...
[XRA1202](#xra1202) chips can only pull-up. They have a pullup\_en register.
...
[Chips with additional int\_mask and out\_conf registers](#chips-with-additional-int-mask-and-out-conf-registers) have this function,
...
[Basic PCAL chips](#basic-pcal-chips) have 1 bit per port in one single out\_conf register.
...
[PCAL chips with extended interrupt and output configuration functions](#pcal-chips-with-extended-interrupt-and-output-configuration-functions)
can set this for each line individually. They have the same per-port out\_conf
register as [Basic PCAL chips](#basic-pcal-chips), but they also have an indiv\_out\_conf register
...
> 1. base offset 0x40 + 7\*bankoffset (out\_conf),
>    base offset 0x60, bank 4 (indiv\_out\_conf) with bank offset of 2^n
>
>    > * pcal6524
> 2. base offset 0x30 + 7\*banksize (out\_conf),
>    base offset 0x54, bank 4 (indiv\_out\_conf), closely packed banks
>
>    > * pcal6534
