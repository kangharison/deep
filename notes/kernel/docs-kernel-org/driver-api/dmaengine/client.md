...
> 출처(원문): https://docs.kernel.org/driver-api/dmaengine/client.html
...
> * peripheral\_dma\_vec: DMA an array of scatter gather buffers from/to a
>   peripheral. Similar to slave\_sg, but uses an array of dma\_vec
...
>   appropriate values to the ‘dma\_interleaved\_template’ members. Cyclic
>   interleaved DMA transfers are also possible if supported by the channel by
>   setting the DMA\_PREP\_REPEAT transfer flag.
>
...
> The scatterlist must be mapped using the DMA [`struct device`](../infrastructure.html#c.device "device").
> If a mapping needs to be synchronized later, dma\_sync\_\*\_for\_\*() must be
> called using the DMA [`struct device`](../infrastructure.html#c.device "device"), too.
...
> > metadata must not be accessed after issue\_pending.
...
> >
> >   1. prepare the descriptor (dmaengine\_prep\_\*)
...
> Note
>
> When DESC\_METADATA\_ENGINE mode is used the metadata area for the descriptor
...
>
> Mixed use of DESC\_METADATA\_CLIENT / DESC\_METADATA\_ENGINE is not allowed,
