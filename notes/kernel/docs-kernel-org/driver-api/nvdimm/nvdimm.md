...
> 출처(원문): https://docs.kernel.org/driver-api/nvdimm/nvdimm.html
...
PMEM (nd\_pmem.ko): Drives a system-physical-address range. This range is
...
the PMEM range. In contrast ND\_NAMESPACE\_IO ranges, once registered,
can be immediately attached to nd\_pmem. This latter mode is called
...
> /sys/devices/platform/nfit\_test.0 when the nfit\_test.ko module from
> tools/testing/nvdimm is loaded. This module is a unit test for
> LIBNVDIMM and the acpi\_nfit.ko driver.
...
identified by an “nfit\_handle” a 32-bit value where:
...
range. Per the example there are 2 PMEM regions on the “nfit\_test.0”
...
devices for the nd\_pmem driver to consume.

In addition to the generic attributes of “mapping”s, “interleave\_ways”
...
at the ‘add’ event, and finally, the optional “spa\_index” is provided in
...
triggers the nd\_pmem driver to load and register a disk/block device.
...
“sector\_size” attributes and then bind the device to the nd\_pmem or
nd\_blk driver depending on the region type:
