...
> 출처(원문): https://docs.kernel.org/driver-api/rapidio/rapidio.html
...
by a rio\_mport data structure. This structure contains master port specific
resources such as mailboxes and doorbells. The rio\_mport also includes a unique
...
independent interface for RapidIO subsystem operations, rio\_mport structure
includes rio\_ops data structure which contains pointers to hardware specific
implementations of RapidIO functions.

### 2.2 Device

A RapidIO device is any endpoint (other than mport) or switch in the network.
All devices are presented in the RapidIO subsystem by corresponding rio\_dev data
...
RapidIO subsystem by rio\_dev data structure expanded by additional rio\_switch
...
Each RapidIO network known to the system is represented by corresponding rio\_net
...
> Configuration option CONFIG\_RAPIDIO\_DISC\_TIMEOUT defines time that discovering
...
parameter CONFIG\_RAPIDIO\_DISC\_TIMEOUT defines time (in seconds) which
...
write mport\_ID (not RapidIO destination ID) into that file. The mport\_ID is a
sequential number (0 ... RIO\_MAX\_MPORTS) assigned during mport device
registration. For example for machine with single RapidIO controller, mport\_ID
for that controller always will be 0.

To initiate RapidIO enumeration/discovery on all available mports a user may
write ‘-1’ (or RIO\_MPORT\_ANY) into the scan attribute file.
...
is successfully acquired, the enumerator allocates a new rio\_dev structure and
...
If the device is a switch, the enumerator allocates an additional rio\_switch
...
devices by clearing device ID locks (calls [`rio_clear_locks()`](../../admin-guide/rapidio.html#c.rio_clear_locks "rio_clear_locks")). For each endpoint
...
method’s module initialization routine calls [`rio_register_scan()`](../../admin-guide/rapidio.html#c.rio_register_scan "rio_register_scan") to attach
