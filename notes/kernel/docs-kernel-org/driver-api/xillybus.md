...
> 출처(원문): https://docs.kernel.org/driver-api/xillybus.html
...
On the host, all interface with Xillybus is done through /dev/xillybus\_\*
...
possibly pressing CTRL-C as some stage, even though the xillybus\_\* pipes have
...
* Supporting non-blocking I/O (by setting O\_NONBLOCK on open() ).
...
The Xillybus driver consists of a core module, xillybus\_core.c, and modules
that depend on the specific bus interface (xillybus\_of.c and xillybus\_pcie.c).
...
xilly\_endpoint\_hardware structure is passed to the core module on
...
in xillybus\_core.c as follows:

* is\_writebuf: The pipe’s direction. A non-zero value means it’s an FPGA to
...
* exclusive\_open: A non-zero value forces exclusive opening of the associated
...
* supports\_nonempty: A non-zero value (which is typical) indicates that the
...
XILLYMSG\_OPCODE\_RELEASEBUF message channel 0 and sending an interrupt if
...
XILLYMSG\_OPCODE\_RELEASEBUF message, through which the FPGA informs not just
...
for XILLY\_RX\_TIMEOUT jiffies (currently 10 ms), after which the host commands
...
an equivalent flush roughly XILLY\_RX\_TIMEOUT jiffies after the last write().
...
bytes need to be moved to the next buffer. The parts in xillybus\_core.c
...
XILLYMSG\_OPCODE\_NONEMPTY messages sent by the FPGA when a channel goes from
