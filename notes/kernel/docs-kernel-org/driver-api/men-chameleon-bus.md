...
> 출처(원문): https://docs.kernel.org/driver-api/men-chameleon-bus.html
...
inside the carrier and is intended to be passed to [`ioremap()`](device-io.html#c.ioremap "ioremap") and friends. It
...
number is suitable to be passed to [`request_irq()`](../core-api/genericirq.html#c.request_irq "request_irq").
...
carrier device’s ‘[`struct device`](infrastructure.html#c.device "device")’. Fortunately ‘`struct mcb_device`’ embeds a
pointer (->dma\_dev) to the carrier’s device for DMA purposes:
