...
> 출처(원문): https://docs.kernel.org/driver-api/firmware/firmware_cache.html
...
* The firmware cache is setup by adding a devres entry for each device that
  uses all synchronous call except [`request_firmware_into_buf()`](request_firmware.html#c.request_firmware_into_buf "request_firmware_into_buf").
* If an asynchronous call is used the firmware cache is only set up for a
  device if the second argument (uevent) to [`request_firmware_nowait()`](request_firmware.html#c.request_firmware_nowait "request_firmware_nowait") is
