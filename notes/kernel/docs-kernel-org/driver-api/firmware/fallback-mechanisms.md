...
> 출처(원문): https://docs.kernel.org/driver-api/firmware/fallback-mechanisms.html
...
>   mechanism is available and for the [`request_firmware_nowait()`](request_firmware.html#c.request_firmware_nowait "request_firmware_nowait") call.
...
>   except [`request_firmware_direct()`](request_firmware.html#c.request_firmware_direct "request_firmware_direct"). Most distributions disable this option
>   today. The call [`request_firmware_nowait()`](request_firmware.html#c.request_firmware_nowait "request_firmware_nowait") allows for one alternative
>   fallback mechanism: if this kconfig option is enabled and your second
>   argument to [`request_firmware_nowait()`](request_firmware.html#c.request_firmware_nowait "request_firmware_nowait"), uevent, is set to false you are
...
for [`request_firmware_nowait()`](request_firmware.html#c.request_firmware_nowait "request_firmware_nowait") when uevent is set to true.
...
* Races upon resume from suspend. This is resolved by the firmware cache, but
  ...
  supported for [`request_firmware_into_buf()`](request_firmware.html#c.request_firmware_into_buf "request_firmware_into_buf").
...
via `fw_create_instance()`. This call creates a new [`struct device`](../infrastructure.html#c.device "device") named after
...
This is actually where the original firmware\_class module name came from,
given that originally the only firmware loading mechanism available was the
mechanism we now use as a fallback mechanism, which registers a [`struct class`](../infrastructure.html#c.class "class")
firmware\_class. Because the attributes exposed are part of the module name, the
module name firmware\_class cannot be renamed in the future, to ensure backward
...
firmware request, this is set up with [`firmware_fallback_sysfs()`](#c.firmware_fallback_sysfs "firmware_fallback_sysfs"). It is
...
int firmware\_fallback\_sysfs(struct firmware \*fw, const char \*name, struct [device](#c.firmware_fallback_sysfs "device") \*device, u32 opt\_flags, int ret)
...
on a system by setting the proc sysctl value ignore\_sysfs\_fallback to true.
If this is false we check if the internal API caller set the
**FW\_OPT\_NOFALLBACK\_SYSFS** flag, if so it would also disable the fallback
mechanism. A system may want to enforce the sysfs fallback mechanism at all
times, it can do this by setting ignore\_sysfs\_fallback to false and
force\_sysfs\_fallback to true.
Enabling force\_sysfs\_fallback is functionally equivalent to build a kernel
with CONFIG\_FW\_LOADER\_USER\_HELPER\_FALLBACK.
...
on the use of `dev_set_uevent_suppress()` and [`kobject_uevent()`](../basics.html#c.kobject_uevent "kobject_uevent").
...
enabled this binary would be called each time [`kobject_uevent_env()`](../basics.html#c.kobject_uevent_env "kobject_uevent_env") gets called
...
Users of the [`request_firmware_nowait()`](request_firmware.html#c.request_firmware_nowait "request_firmware_nowait") call have yet another option available
...
rely on the uevent flag which can be disabled by [`request_firmware_nowait()`](request_firmware.html#c.request_firmware_nowait "request_firmware_nowait") to
...
Although this can disable the firmware cache for [`request_firmware_nowait()`](request_firmware.html#c.request_firmware_nowait "request_firmware_nowait")
...
[`firmware_request_platform()`](request_firmware.html#c.firmware_request_platform "firmware_request_platform") function for this, note that this is a
separate fallback mechanism from the other fallback mechanisms and
this does not use the sysfs interface.

A device driver which needs this can describe the firmware it needs
using an efi\_embedded\_fw\_desc struct:
...
The EFI embedded-fw code works by scanning all EFI\_BOOT\_SERVICES\_CODE memory
segments for an eight byte sequence matching prefix; if the prefix is found it
then does a sha256 over length bytes and if that matches makes a copy of length
bytes and adds that to its list with found firmwares.

To avoid doing this somewhat expensive scan on all systems, dmi matching is
used. Drivers are expected to export a dmi\_system\_id array, with each entries’
driver\_data pointing to an efi\_embedded\_fw\_desc.

To register this array with the efi-embedded-fw code, a driver needs to:

1. Always be builtin to the kernel or store the dmi\_system\_id array in a
   separate object file which always gets builtin.
2. Add an extern declaration for the dmi\_system\_id array to
   include/linux/efi\_embedded\_fw.h.
3. Add the dmi\_system\_id array to the embedded\_fw\_table in
   drivers/firmware/efi/embedded-firmware.c wrapped in a #ifdef testing that
   the driver is being builtin.
4. Add “select EFI\_EMBEDDED\_FIRMWARE if EFI\_STUB” to its Kconfig entry.

The [`firmware_request_platform()`](request_firmware.html#c.firmware_request_platform "firmware_request_platform") function will always first try to load firmware
...
3. At the moment the EFI embedded-fw code only works on x86 because other archs
   free EFI\_BOOT\_SERVICES\_CODE before the EFI embedded-fw code gets a chance to
   scan it.
4. The current brute-force scanning of EFI\_BOOT\_SERVICES\_CODE is an ad-hoc
...
2. cp /sys/kernel/debug/efi/boot\_services\_code? to your home dir
3. Open the boot\_services\_code? files in a hex-editor, search for the
   magic prefix for Silead firmware: F0 00 00 00 02 00 00 00, this gives you
   the beginning address of the firmware inside the boot\_services\_code? file.
4. The firmware has a specific pattern, it starts with a 8 byte page-address,
   ...
   this gives you the end of the firmware inside the boot\_services\_code? file.
5. “dd if=boot\_services\_code? of=firmware bs=1 skip=<begin-addr> count=<len>”
...
7. If the extracted firmware works, you can use the found info to fill an
   efi\_embedded\_fw\_desc `struct to` describe it, run “sha256sum firmware”
