...
> 출처(원문): https://docs.kernel.org/driver-api/soundwire/stream.html
...
`SDW_STREAM_DISABLED` are only relevant when then INFO\_PAUSE flag is
...
> 1. The resources allocated for stream information in SDW\_STREAM\_ALLOCATED
...
> 1. All the values computed in SDW\_STREAM\_PREPARED state are programmed
...
When the INFO\_PAUSE flag is supported, a direct transition to
...
When the INFO\_PAUSE flag is supported, the stream state is linked to
the .`hw_free()` operation - the stream is not deprepared on a
TRIGGER\_STOP.

Other implementations may transition to the `SDW_STREAM_DEPREPARED`
state on TRIGGER\_STOP, should they require a transition through the
