...
> 출처(원문): https://docs.kernel.org/driver-api/pin-control.html
...
mapping table; see section [Board/machine configuration](#board-machine-configuration) below.
...
see the section named [Pin control requests from drivers](#pin-control-requests-from-drivers) and
[Drivers needing both pin control and GPIOs](#drivers-needing-both-pin-control-and-gpios) below for details. But in some
...
gpio_chip`](gpio/index.html#c.gpio_chip "gpio_chip")) any number of GPIO ranges can be added to a pin controller instance
like this:
...
from the Linux kernel using the API in `<linux/gpio/consumer.h>` with [`gpiod_get()`](gpio/index.html#c.gpiod_get "gpiod_get")
and similar functions. There are cases where you may be using something
that your datasheet calls “GPIO mode”, but actually is just an electrical
configuration for a certain device. See the section below named
[GPIO mode pitfalls](#gpio-mode-pitfalls) for more details on this scenario.
...
muxed in, then twist it over to GPIO mode and use [`gpiod_direction_output()`](gpio/index.html#c.gpiod_direction_output "gpiod_direction_output")
to drive it low during sleep, then mux it over to UART TX again when you
wake up and maybe even [`gpiod_get()`](gpio/index.html#c.gpiod_get "gpiod_get") / [`gpiod_put()`](gpio/index.html#c.gpiod_put "gpiod_put") as part of this cycle. This
...
runtime, as described further below under the heading [Runtime pinmuxing](#runtime-pinmuxing).
...
place where you have access to all the affected [`struct device`](infrastructure.html#c.device "device") \* pointers. In
...
described in the section [Pin control interaction with the GPIO subsystem](#pin-control-interaction-with-the-gpio-subsystem)
above. This only involves per-pin multiplexing, and will be completely
hidden behind the gpiod\_\*() function namespace. In this case, the driver
