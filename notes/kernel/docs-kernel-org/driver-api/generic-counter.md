...
> 출처(원문): https://docs.kernel.org/driver-api/generic-counter.html
...
`num_actions`
:   number of action modes specified in **actions\_list**
...
`num_functions`
:   number of function modes specified in **functions\_list**
...
:   optional write callback to configure events. The list of
    [`struct counter_event_node`](#c.counter_event_node "counter_event_node") may be accessed via the
    events\_list member of the counter parameter.
...
void \*counter\_priv(const struct [counter\_device](#c.counter_device "counter_device") \*const counter)
...
struct [counter\_device](#c.counter_device "counter_device") \*counter\_alloc(size\_t sizeof\_priv)
:   allocate a counter\_device
...
dynamically to ensure the right lifetime for the embedded [`struct device`](infrastructure.html#c.device "device").

If this succeeds, call `counter_put()` to get rid of the counter\_device again.

int counter\_add(struct [counter\_device](#c.counter_device "counter_device") \*counter)
...
If this succeeds, call [`counter_unregister()`](#c.counter_unregister "counter_unregister") to get rid of the counter\_device again.

void counter\_unregister(struct [counter\_device](#c.counter_device "counter_device") \*const counter)
...
struct [counter\_device](#c.counter_device "counter_device") \*devm\_counter\_alloc(struct [device](infrastructure.html#c.device "device") \*dev, size\_t sizeof\_priv)
:   allocate a counter\_device
...
This is the device managed version of [`counter_add()`](#c.counter_add "counter_add"). It registers a cleanup
callback to care for calling `counter_put()`.

int devm\_counter\_add(struct [device](infrastructure.html#c.device "device") \*dev, struct [counter\_device](#c.counter_device "counter_device") \*const counter)
:   complete registration of a counter
...
This is the device managed version of [`counter_add()`](#c.counter_add "counter_add"). It registers a cleanup
callback to care for calling [`counter_unregister()`](#c.counter_unregister "counter_unregister").

void counter\_push\_event(struct [counter\_device](#c.counter_device "counter_device") \*const counter, const u8 event, const u8 channel)
...
Counter Signals via counter\_signal structures. These Signals should
be stored as an array and set to the signals array member of an
allocated counter\_device structure before the Counter is registered to
the system.

Counter Counts may be allocated via counter\_count structures, and
respective Counter Signal associations (Synapses) made via
counter\_synapse structures. Associated counter\_synapse structures are
stored as an array and set to the synapses array member of the
respective counter\_count structure. These counter\_count structures are
set to the counts array member of an allocated counter\_device structure
before the Counter is registered to the system.

Driver callbacks must be provided to the counter\_device structure in
order to communicate with the device: to read and write various Signals
and Counts, and to set and get the “action mode” and “function mode” for
various Synapses and Counts respectively.

A counter\_device structure is allocated using [`counter_alloc()`](#c.counter_alloc "counter_alloc") and then
registered to the system by passing it to the [`counter_add()`](#c.counter_add "counter_add") function, and
unregistered by passing it to the counter\_unregister function. There are
device managed variants of these functions: [`devm_counter_alloc()`](#c.devm_counter_alloc "devm_counter_alloc") and
[`devm_counter_add()`](#c.devm_counter_add "devm_counter_add").

The [`struct counter_comp`](#c.counter_comp "counter_comp") structure is used to define counter extensions
for Signals, Synapses, and Counts.

The “type” member specifies the type of high-level data (e.g. BOOL,
COUNT\_DIRECTION, etc.) handled by this extension. The “`*_read`” and
...
counter_comp`](#c.counter_comp "counter_comp") structure as the “`*_read`” and “`*_write`” `union
members`.
...
* Device extensions are attributes that expose information/control
  ...
  chip overheated via a device extension called “error\_overtemp”:
  /sys/bus/counter/devices/counterX/error\_overtemp
...
Please refer to the [ABI file testing/sysfs-bus-counter](../admin-guide/abi-testing-files.html#abi-file-testing-sysfs-bus-counter) file
...
[ABI file testing/sysfs-bus-counter](../admin-guide/abi-testing-files.html#abi-file-testing-sysfs-bus-counter) for detailed information
...
> | COUNTER\_EVENT\_OVERFLOW | COUNTER\_EVENT\_INDEX |
> | --- | --- |
> | Channel 0 | Channel 0 |
