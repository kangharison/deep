...
> 출처(원문): https://docs.kernel.org/driver-api/crypto/iaa/iaa-crypto.html
...
The iaa\_crypto driver is designed to work as a layer underneath
...
In the configuration file the option called CONFIG\_CRYPTO\_DEV\_IAA\_CRYPTO.
...
In the configuration file the option called CONFIG\_CRYPTO\_DEV\_IAA\_CRYPTO\_STATS.
...
When loaded, the iaa\_crypto driver automatically creates a default
...
crypto subsystem by removing and reinserting the iaa\_crypto module.

The [IAA disable script](#iaa-disable-script) in the ‘Use Cases’
section below can be used to disable the default configuration.

See [IAA Default Configuration](#iaa-default-config) below for details of the default
configuration.
...
The [IAA sysfs config interface](#iaa-sysfs-config) section in the appendix below can be
...
In order for the iaa\_crypto module to actually do any
compression/decompression work on behalf of a facility, one or more
IAA workqueues need to be bound to the iaa\_crypto driver.

For instance, here’s an example of configuring an IAA workqueue and
binding it to the iaa\_crypto driver (note that device names are
...
Whenever a new workqueue is bound to or unbound from the iaa\_crypto
...
system, the iaa\_crypto driver will work, albeit most likely not as
efficiently.
...
binding of the first IAA workqueue to the iaa\_crypto driver.
...
the last IAA worqueue to the iaa\_crypto driver.

As a result, the IAA crypto algorithms and thus the IAA hardware are
only available when one or more workques are bound to the iaa\_crypto
driver.
...
are captured in each algorithm’s crypto\_ctx and used for all compresses
...
>
>   The crypto async interface support implemented by iaa\_crypto
...
>
>   The iaa\_crypto driver does however provide true asynchronous
...
>   polling in the iaa\_crypto driver previously mentioned.
>
>   This mode can be enabled by writing ‘async\_irq’ to the sync\_mode
>   iaa\_crypto driver attribute:
>
>   ```
...
>   ```
>
>   The mode that does the polling in the iaa\_crypto driver can be
...
>
>   Caveat: since the only mechanism that iaa\_crypto currently implements
>   for async polling without interrupts is via the ‘sync’ mode as
>   described earlier, writing ‘async’ to
>   ‘/sys/bus/dsa/drivers/crypto/sync\_mode’ will internally enable the
>   ‘sync’ mode. This is to ensure correct iaa\_crypto behavior until true
>   async polling without interrupts is enabled in iaa\_crypto.

### IAA Default Configuration

When the iaa\_crypto driver is loaded, each IAA device has a single
...
the iaa\_crypto module. The [IAA disable script](#iaa-disable-script) in the ‘Use
...
The global\_stats file shows a set of global statistics collected since
...
The wq\_stats file shows per-wq stats, a set for each iaa device and wq
...
Writing to ‘stats\_reset’ resets all the stats, including the
...
This is a simple test that uses iaa\_compress as the compressor for a
swap (zswap) device. It sets up the zswap device and then uses the
memory\_memadvise program listed below to forcibly swap out and in a
...
workqueue driver\_name of “crypto”.

The first step is to make sure the iaa\_crypto module is loaded:
...
further IAA configuration is necessary. See [IAA Default Configuration](#iaa-default-config)
...
example, using the memory\_memadvise code below, the following command
...
Then run the [IAA disable script](#iaa-disable-script) in the ‘Use Cases’ section
...
When the workqueues are bound to the iaa\_crypto driver, you should
see something similar to the following in dmesg output if you’ve
enabled debug output (echo -n ‘module iaa\_crypto +p’ >
/sys/kernel/debug/dynamic\_debug/control):
...
you’ve enabled debug output (echo -n ‘module iaa\_crypto +p’ >
/sys/kernel/debug/dynamic\_debug/control):
...
Once zswap is disabled and no longer using iaa\_crypto, the IAA wqs and
...
Finally, at this point the iaa\_crypto module can be removed, which
...
memory\_madvise.c (gcc -o memory\_memadvise memory\_madvise.c):
