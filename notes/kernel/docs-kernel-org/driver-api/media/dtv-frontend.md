# 4.2.Digital TV Frontend kABI

> 출처(원문): https://docs.kernel.org/driver-api/media/dtv-frontend.html
...
### 4.2.1.1. Demodulator driver

The demodulator driver is responsible for talking with the decoding part of the
hardware. Such driver should implement [`dvb_frontend_ops`](#c.dvb_frontend_ops "dvb_frontend_ops"), which
...
1. For satellite digital TV standards (DVB-S, DVB-S2, ISDB-S), the
   ...
   [`dvb_frontend_ops`](#c.dvb_frontend_ops "dvb_frontend_ops") structures, one for each standard.
...
3. All new drivers should implement the
   [DVBv5 statistics](#dvbv5-stats) via `.read_status`.
...
detect a channel, as defined at enum [`dvbfe_algo()`](#c.dvbfe_algo "dvbfe_algo").

The algorithm to be used is obtained via `.get_frontend_algo`. If the driver
doesn’t fill its field at [`struct dvb_frontend_ops`](#c.dvb_frontend_ops "dvb_frontend_ops"), it will default to
...
function callbacks at [`struct dvb_frontend_ops`](#c.dvb_frontend_ops "dvb_frontend_ops").

### 4.2.1.2. Attaching frontend driver to the bridge driver

Before using the Digital TV frontend core, the bridge driver should attach
the frontend demod, tuner and SEC devices and call
[`dvb_register_frontend()`](#c.dvb_register_frontend "dvb_register_frontend"),
in order to register the new frontend at the subsystem. At device
detach/removal, the bridge driver should call
[`dvb_unregister_frontend()`](#c.dvb_unregister_frontend "dvb_unregister_frontend") to
remove the frontend from the core and then [`dvb_frontend_detach()`](#c.dvb_frontend_detach "dvb_frontend_detach")
to free the memory allocated by the frontend drivers.

The drivers should also call [`dvb_frontend_suspend()`](#c.dvb_frontend_suspend "dvb_frontend_suspend") as part of
their handler for the [`device_driver`](../infrastructure.html#c.device_driver "device_driver").`suspend()`, and
[`dvb_frontend_resume()`](#c.dvb_frontend_resume "dvb_frontend_resume") as
part of their handler for [`device_driver`](../infrastructure.html#c.device_driver "device_driver").`resume()`.

A few other optional functions are provided to handle some special cases.

## 4.2.2. Digital TV Frontend statistics

### 4.2.2.1. Introduction

Digital TV frontends provide a range of
[statistics](../../userspace-api/media/dvb/frontend-stat-properties.html#frontend-stat-properties) meant to help tuning the device
...
that’s usually 1 for most video standards [[1]](#f2).
...
### 4.2.2.2. Groups of statistics

There are several groups of statistics currently supported:

Signal strength ([DTV\_STAT\_SIGNAL\_STRENGTH](../../userspace-api/media/dvb/frontend-stat-properties.html#dtv-stat-signal-strength))
:   * Measures the signal strength level at the analog part of the tuner or
    ...
    * As the gain is visible through the set of registers that adjust the gain,
      typically, this statistics is always available [[2]](#f3).
    * Drivers should try to make it available all the times, as these statistics
    ...
    detected at the tuner (`FE_HAS_CARRIER`, see [`fe_status`](../../userspace-api/media/dvb/frontend-header.html#c.fe_status "fe_status")),
    and otherwise return the lowest possible value.

Carrier Signal to Noise ratio ([DTV\_STAT\_CNR](../../userspace-api/media/dvb/frontend-stat-properties.html#dtv-stat-cnr))
:   * Signal to Noise ratio for the main carrier.
    * Signal to Noise measurement depends on the device. On some hardware, it is
      available when the main carrier is detected. On those hardware, CNR
      measurement usually comes from the tuner (e. g. after `FE_HAS_CARRIER`,
      see [`fe_status`](../../userspace-api/media/dvb/frontend-header.html#c.fe_status "fe_status")).

      On other devices, it requires inner FEC decoding,
      as the frontend measures it indirectly from other parameters (e. g. after
      `FE_HAS_VITERBI`, see [`fe_status`](../../userspace-api/media/dvb/frontend-header.html#c.fe_status "fe_status")).

      Having it available after inner FEC is more common.

Bit counts post-FEC ([DTV\_STAT\_POST\_ERROR\_BIT\_COUNT](../../userspace-api/media/dvb/frontend-stat-properties.html#dtv-stat-post-error-bit-count) and [DTV\_STAT\_POST\_TOTAL\_BIT\_COUNT](../../userspace-api/media/dvb/frontend-stat-properties.html#dtv-stat-post-total-bit-count))
...
Bit counts pre-FEC ([DTV\_STAT\_PRE\_ERROR\_BIT\_COUNT](../../userspace-api/media/dvb/frontend-stat-properties.html#dtv-stat-pre-error-bit-count) and [DTV\_STAT\_PRE\_TOTAL\_BIT\_COUNT](../../userspace-api/media/dvb/frontend-stat-properties.html#dtv-stat-pre-total-bit-count))
:   * Those counters measure the number of bits and bit errors before
    ...
    * Due to its nature, those statistics depend on inner coding lock (e. g.
      after `FE_HAS_VITERBI`, see [`fe_status`](../../userspace-api/media/dvb/frontend-header.html#c.fe_status "fe_status")).

Block counts ([DTV\_STAT\_ERROR\_BLOCK\_COUNT](../../userspace-api/media/dvb/frontend-stat-properties.html#dtv-stat-error-block-count) and [DTV-STAT\_TOTAL\_BLOCK\_COUNT](../../userspace-api/media/dvb/frontend-stat-properties.html#dtv-stat-total-block-count))
:   * Those counters measure the number of blocks and block errors after
      the forward error correction (FEC) on the inner coding block
      (before Viterbi, LDPC or other inner code).
    * Due to its nature, those statistics depend on full coding lock
      (e. g. after `FE_HAS_SYNC` or after
      `FE_HAS_LOCK`, see [`fe_status`](../../userspace-api/media/dvb/frontend-header.html#c.fe_status "fe_status")).
...
#### 4.2.2.3.1. if the driver have a bit that indicates when a collected data is ready
...
#### 4.2.2.3.2. If the driver doesn’t provide a statistics available check bit
...
[`dvb_frontend_ops`](#c.dvb_frontend_ops "dvb_frontend_ops") `.read_status` callback. The rationale is that
...
## 4.2.3. Digital TV Frontend functions and types
...
:   Audio mode as defined for the rxsubchans field at videodev2.h,
    e. g. V4L2\_TUNER\_MODE\_\*

`std`
:   TV standard bitmap as defined at videodev2.h, e. g. V4L2\_STD\_\*
...
`info`
:   embedded [`struct dvb_tuner_info`](#c.dvb_tuner_info "dvb_tuner_info") with tuner properties
...
:   callback function used to inform the tuner to tune
    ...
    are stored at [`struct dvb_frontend`](#c.dvb_frontend "dvb_frontend").dtv\_property\_cache.
...
:   callback function used to tune into an analog TV
    channel on hybrid tuners. It passes **analog\_parameters**
    to the driver.
...
:   returns the RF signal strength. Used mostly to support
    ...
    via DVBv5 API ([`struct dvb_frontend`](#c.dvb_frontend "dvb_frontend").dtv\_property\_cache).
...
frequencies used on **get\_frequency** and **set\_frequency** are in Hz for
...
`info`
:   pointer to [`struct analog_demod_info`](#c.analog_demod_info "analog_demod_info")
...
:   Capabilities supported by the frontend,
    as specified in [`enum fe_caps`](../../userspace-api/media/dvb/frontend-header.html#c.fe_caps "fe_caps").
...
:   callback function called when frontend is detached.
    ...
    dvb_frontend`](#c.dvb_frontend "dvb_frontend") allocation.
...
:   callback function used by demod drivers that use
    **DVBFE\_ALGO\_HW** to tune into a frequency.
...
:   callback function used to inform the demod to set the
    ...
    dvb_frontend`](#c.dvb_frontend "dvb_frontend").dtv\_property\_cache. The demod can change
...
:   callback function used to inform the parameters
    ...
    [`struct dvb_frontend`](#c.dvb_frontend "dvb_frontend").dtv\_property\_cache and update
...
:   legacy callback function to return the bit error rate.
    ...
    e. g. **set\_frontend**;/**get\_frontend**, implementing this
    callback only if DVBv3 API compatibility is wanted.

`read_signal_strength`
:   legacy callback function to return the signal
    strength. Newer drivers should provide such info via
    DVBv5 API, e. g. **set\_frontend**/**get\_frontend**,
...
:   callback function to implement the
    ...
    if **set\_voltage** takes more than 8ms to work), and
...
`tuner_ops`
:   pointer to [`struct dvb_tuner_ops`](#c.dvb_tuner_ops "dvb_tuner_ops")

`analog_ops`
:   pointer to [`struct analog_demod_ops`](#c.analog_demod_ops "analog_demod_ops")
...
For more info, consult the media\_api.html with the documentation of the
Userspace API.
...
`refcount`
:   refcount to keep track of [`struct dvb_frontend`](#c.dvb_frontend "dvb_frontend")
    references

`ops`
:   embedded [`struct dvb_frontend_ops`](#c.dvb_frontend_ops "dvb_frontend_ops")
...
`dtv_property_cache`
:   embedded [`struct dtv_frontend_properties`](#c.dtv_frontend_properties "dtv_frontend_properties")
...
int dvb\_register\_frontend(struct [dvb\_adapter](dtv-common.html#c.dvb_adapter "dvb_adapter") \*dvb, struct [dvb\_frontend](#c.dvb_frontend "dvb_frontend") \*fe)
...
:   pointer to [`struct dvb_frontend`](#c.dvb_frontend "dvb_frontend")

**Description**

Allocate and initialize the private data needed by the frontend core to
manage the frontend and calls [`dvb_register_device()`](dtv-common.html#c.dvb_register_device "dvb_register_device") to register a new
...
int dvb\_unregister\_frontend(struct [dvb\_frontend](#c.dvb_frontend "dvb_frontend") \*fe)
...
Stops the frontend kthread, calls [`dvb_unregister_device()`](dtv-common.html#c.dvb_unregister_device "dvb_unregister_device") and frees the
private frontend data allocated by [`dvb_register_frontend()`](#c.dvb_register_frontend "dvb_register_frontend").

**NOTE**

This function doesn’t frees the memory allocated by the demod,
by the SEC driver and by the tuner. In order to free it, an explicit call to
[`dvb_frontend_detach()`](#c.dvb_frontend_detach "dvb_frontend_detach") is needed, after calling this function.

void dvb\_frontend\_detach(struct [dvb\_frontend](#c.dvb_frontend "dvb_frontend") \*fe)
...
This function should be called after [`dvb_unregister_frontend()`](#c.dvb_unregister_frontend "dvb_unregister_frontend"). It
calls the SEC, tuner and demod release functions:
[`dvb_frontend_ops.release_sec`](#c.dvb_frontend_ops "dvb_frontend_ops"), [`dvb_frontend_ops.tuner_ops`](#c.dvb_frontend_ops "dvb_frontend_ops").release,
[`dvb_frontend_ops.analog_ops`](#c.dvb_frontend_ops "dvb_frontend_ops").release and [`dvb_frontend_ops.release`](#c.dvb_frontend_ops "dvb_frontend_ops").

If the driver is compiled with `CONFIG_MEDIA_ATTACH`, it also decreases
the module reference count, needed to allow userspace to remove the
previously used DVB frontend modules.

int dvb\_frontend\_suspend(struct [dvb\_frontend](#c.dvb_frontend "dvb_frontend") \*fe)
...
[`dvb_frontend_ops.tuner_ops`](#c.dvb_frontend_ops "dvb_frontend_ops").`suspend()` is available, it calls it. Otherwise,
it will call [`dvb_frontend_ops.tuner_ops`](#c.dvb_frontend_ops "dvb_frontend_ops").`sleep()`, if available.

It will also call [`dvb_frontend_ops.suspend`](#c.dvb_frontend_ops "dvb_frontend_ops")() to put the demod to suspend,
if available. Otherwise it will call [`dvb_frontend_ops.sleep`](#c.dvb_frontend_ops "dvb_frontend_ops")().

The drivers should also call [`dvb_frontend_suspend()`](#c.dvb_frontend_suspend "dvb_frontend_suspend") as part of their
handler for the [`device_driver.suspend`](../infrastructure.html#c.device_driver "device_driver")().

int dvb\_frontend\_resume(struct [dvb\_frontend](#c.dvb_frontend "dvb_frontend") \*fe)
...
[`dvb_frontend_ops.resume`](#c.dvb_frontend_ops "dvb_frontend_ops")() if available. Otherwise it calls demod
[`dvb_frontend_ops.init`](#c.dvb_frontend_ops "dvb_frontend_ops")().

If [`dvb_frontend_ops.tuner_ops`](#c.dvb_frontend_ops "dvb_frontend_ops").`resume()` is available, It, it calls it.
Otherwise,t will call [`dvb_frontend_ops.tuner_ops`](#c.dvb_frontend_ops "dvb_frontend_ops").`init()`, if available.
...
The drivers should also call [`dvb_frontend_resume()`](#c.dvb_frontend_resume "dvb_frontend_resume") as part of their
handler for the [`device_driver.resume`](../infrastructure.html#c.device_driver "device_driver")().

void dvb\_frontend\_reinitialise(struct [dvb\_frontend](#c.dvb_frontend "dvb_frontend") \*fe)
...
Calls [`dvb_frontend_ops.init`](#c.dvb_frontend_ops "dvb_frontend_ops")() and [`dvb_frontend_ops.tuner_ops`](#c.dvb_frontend_ops "dvb_frontend_ops").`init()`,
...
`FE_DISHNETWORK_SEND_LEGACY_CMD()` using the [`dvb_frontend_ops.set_voltage`](#c.dvb_frontend_ops "dvb_frontend_ops")()
callback.
...
be writing a [`dvb_frontend_ops.dishnetwork_send_legacy_command`](#c.dvb_frontend_ops "dvb_frontend_ops")() and
