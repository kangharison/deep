...
> 출처(원문): https://docs.kernel.org/infiniband/user_mad.html
...
> and then calling the IB\_USER\_MAD\_REGISTER\_AGENT ioctl on a file
...
> ```
>
> Agents can be unregistered with the IB\_USER\_MAD\_UNREGISTER\_AGENT
...
> The old ib\_umad interface did not allow setting the P\_Key index for
> MADs that are sent and did not provide a way for obtaining the P\_Key
> index of received MADs. A new layout for `struct ib_user_mad_hdr`
> with a pkey\_index member has been defined; however, to preserve binary
> compatibility with older applications, this new layout will not be used
> unless one of IB\_USER\_MAD\_ENABLE\_PKEY or IB\_USER\_MAD\_REGISTER\_AGENT2 ioctl’s
> are called before a file descriptor is used for anything else.
>
> In September 2008, the IB\_USER\_MAD\_ABI\_VERSION will be incremented
> to 6, the new layout of `struct ib_user_mad_hdr` will be used by
> default, and the IB\_USER\_MAD\_ENABLE\_PKEY ioctl will be removed.
...
> immediately with errno set to EAGAIN if the O\_NONBLOCK flag is
