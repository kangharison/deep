...
> 출처(원문): https://docs.kernel.org/core-api/folio_queue.html
...
The folio\_queue `struct forms` a single segment in a segmented list of folios
that can be used to form an I/O buffer. As such, the list can be iterated over
using the ITER\_FOLIOQ iov\_iter type.
...
## Querying information about a folio\_queue
...
used to note information such as if a folio needs [`folio_put()`](mm-api.html#c.folio_put "folio_put") calling upon it.
...
to take locks. The ITER\_FOLIOQ iterator inserts appropriate barriers to aid
with this.
...
the IOV iterators, an iov\_iter `struct will` be left pointing to the last segment
...
void folioq\_init(struct folio\_queue \*folioq, unsigned int rreq\_id)
...
unsigned int folioq\_nr\_slots(const struct folio\_queue \*folioq)
...
unsigned int folioq\_count(struct folio\_queue \*folioq)
...
bool folioq\_full(struct folio\_queue \*folioq)
...
bool folioq\_is\_marked(const struct folio\_queue \*folioq, unsigned int slot)
...
void folioq\_mark(struct folio\_queue \*folioq, unsigned int slot)
...
void folioq\_unmark(struct folio\_queue \*folioq, unsigned int slot)
...
bool folioq\_is\_marked2(const struct folio\_queue \*folioq, unsigned int slot)
...
void folioq\_mark2(struct folio\_queue \*folioq, unsigned int slot)
...
void folioq\_unmark2(struct folio\_queue \*folioq, unsigned int slot)
...
unsigned int folioq\_append(struct folio\_queue \*folioq, struct [folio](#c.folioq_append "folio") \*folio)
...
unsigned int folioq\_append\_mark(struct folio\_queue \*folioq, struct [folio](#c.folioq_append_mark "folio") \*folio)
:   Add a folio to a folio queue segment
...
struct [folio](mm-api.html#c.folio "folio") \*folioq\_folio(const struct folio\_queue \*folioq, unsigned int slot)
...
unsigned int folioq\_folio\_order(const struct folio\_queue \*folioq, unsigned int slot)
...
size\_t folioq\_folio\_size(const struct folio\_queue \*folioq, unsigned int slot)
...
segment. Note that no bounds check is made and if the slot hasn’t been
added into yet, the size returned will be PAGE\_SIZE.

void folioq\_clear(struct folio\_queue \*folioq, unsigned int slot)
