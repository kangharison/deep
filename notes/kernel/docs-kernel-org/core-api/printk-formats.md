...
> 출처(원문): https://docs.kernel.org/core-api/printk-formats.html
...
If <type> is architecture-dependent for its size (e.g., cycles\_t, tcflag\_t) or
is dependent on a config option for its size (e.g., blk\_status\_t), use a format
...
return from [`vsnprintf()`](kernel-api.html#c.vsnprintf "vsnprintf").
...
e.g. procfs or sysfs (using e.g. `seq_printf()`, not [`printk()`](printk-basics.html#c.printk "printk")) read by a
...
For printing error pointers (i.e. a pointer for which [`IS_ERR()`](kernel-api.html#c.IS_ERR "IS_ERR") is true)
as a symbolic error name. Error values for which no symbolic name is
known are printed in decimal, while a non-ERR\_PTR passed as the
...
results in printing a string. For direct use in regular [`vsnprintf()`](kernel-api.html#c.vsnprintf "vsnprintf") the (k)
...
[Documentation for /proc/sys/kernel/](../admin-guide/sysctl/kernel.html) for more details.
...
in [`printk()`](printk-basics.html#c.printk "printk").
...
of flags (see [`string_escape_mem()`](kernel-api.html#c.string_escape_mem "string_escape_mem") kernel documentation for the
details):
...
equivalent of %s dentry->d\_name.name we used to use, %pd<n> prints `n`
last components. %pD does the same thing for [`struct file`](../filesystems/api-summary.html#c.file "file").
...
For printing information on an fwnode\_handle. The default is to print the full
...
`TP_printk()` part of a tracepoint. Instead, use the show\_\*`_flags()`
...
If you add other %p extensions, please extend <lib/tests/printf\_kunit.c>
