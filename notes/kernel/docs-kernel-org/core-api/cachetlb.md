...
> 출처(원문): https://docs.kernel.org/core-api/cachetlb.html
...
invoke one of the following flush methods \_after\_ the page table
changes occur:
...
3. `void flush_tlb_range(struct vm_area_struct *vma,
   ...
   > modifications for the address space ‘vma->vm\_mm’ in the range
   ...
   > call flush\_tlb\_page (see below) for each entry which may be
   > modified.
4. `void flush_tlb_page(struct vm_area_struct *vma, unsigned long addr)`

   > This time we need to remove the PAGE\_SIZE sized translation
   > from the TLB. The ‘vma’ is the backing structure used by
   > Linux to keep track of mmap’d regions for a process, the
   > address space is available via vma->vm\_mm. Also, one may
   > test (vma->vm\_flags & VM\_EXEC) to see if this region is
...
5. `void update_mmu_cache_range(struct vm_fault *vmf,
   ...
   > in the software page tables for address space “vma->vm\_mm”
...
2. `void flush_cache_dup_mm(struct mm_struct *mm)`
   ...
   >
   > This option is separate from flush\_cache\_mm to allow some
...
3. `void flush_cache_range(struct vm_area_struct *vma,
   ...
   > entries in the cache for ‘vma->vm\_mm’ for virtual addresses in
   ...
   > call flush\_cache\_page (see below) for each entry which may be
   > modified.
4. `void flush_cache_page(struct vm_area_struct *vma, unsigned long addr, unsigned long pfn)`

   > This time we need to remove a PAGE\_SIZE sized range
   ...
   > left by PAGE\_SHIFT to get the physical address) that ‘addr’
   > translates to. It is this mapping which should be removed from
   > the cache.
   >
   > After running, there will be no entries in the cache for
   > ‘vma->vm\_mm’ for virtual address ‘addr’ which translates
   > to ‘pfn’.
   >
   > This is used primarily during fault processing.
5. `void flush_cache_kmaps(void)`
   ...
   >
   > After running, there will be no entries in the cache for
   > the kernel virtual address range PKMAP\_ADDR(0) to
   > PKMAP\_ADDR(LAST\_PKMAP).
   >
...
PAGE\_SIZE, and does not prevent multiple cache lines for the same
...
one way to solve this (in particular SPARC\_FLAG\_MMAPSHARED).

Next, you have to solve the D-cache aliasing issue for all
other cases. Please keep in mind that fact that, for a given page
mapped into some user address space, there is always at least one more
mapping, that of the kernel in its linear mapping starting at
PAGE\_OFFSET. So immediately, once the first user maps a given
...
> > >    that {get,pin}\_user\_pages{\_fast} already call flush\_dcache\_folio
...
> >
> > There is a bit set aside in folio->flags (PG\_arch\_1) as “architecture
...
> > page. See sparc64’s flush\_dcache\_folio and update\_mmu\_cache\_range
> > implementations for an example of how to go about doing this.
> >
> > The idea is, first at `flush_dcache_folio()` time, if
> > [`folio_flush_mapping()`](mm-api.html#c.folio_flush_mapping "folio_flush_mapping") returns a mapping, and `mapping_mapped()` on that
...
>
> > All the functionality of flush\_icache\_page can be implemented in
> > flush\_dcache\_folio and update\_mmu\_cache\_range. In the future, the hope
