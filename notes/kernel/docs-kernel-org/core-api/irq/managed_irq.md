...
> 출처(원문): https://docs.kernel.org/core-api/irq/managed_irq.html
...
required affinity settings using [`struct irq_affinity`](../genericirq.html#c.irq_affinity "irq_affinity"). For MSI‑X devices, this
is done via [`pci_alloc_irq_vectors_affinity()`](../../PCI/msi-howto.html#c.pci_alloc_irq_vectors_affinity "pci_alloc_irq_vectors_affinity") with the PCI\_IRQ\_AFFINITY flag
set.
...
[`request_irq()`](../genericirq.html#c.request_irq "request_irq") is invoked.

## Isolated CPUs

The affinity of managed interrupts is handled entirely in the kernel and cannot
be modified from user space through the /proc interfaces. The managed\_irq
...
* A QEMU instance is booted with “-device virtio-scsi-pci”.
  ...
  IRQD\_IRQ\_DISABLED, IRQD\_IRQ\_MASKED and IRQD\_MANAGED\_SHUTDOWN.
* A QEMU instance is booted with “-device virtio-scsi-pci,num\_queues=2”
  and the kernel command line includes:
  “irqaffinity=0,1 isolcpus=domain,2-7 isolcpus=managed\_irq,1-3,5-7”.
  ...
  because the managed\_irq mask avoids CPUs 5–7 when possible.

  Replacing the managed\_irq argument with “isolcpus=managed\_irq,1-3,4-5,7”
  results in:
  ...
  The interrupt is shut down once all CPUs listed in its smp\_affinity mask are
  offline.
