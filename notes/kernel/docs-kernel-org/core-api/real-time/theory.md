...
> 출처(원문): https://docs.kernel.org/core-api/real-time/theory.html
...
PREEMPT\_RT transforms the Linux kernel into a real-time kernel. It achieves
this by replacing locking primitives, such as spinlock\_t, with a preemptible
...
By default, the Linux kernel uses the SCHED\_OTHER scheduling policy. Under
...
the policy does not guarantee immediate preemption when a new SCHED\_OTHER task
...
SCHED\_FIFO. When a task with a real-time policy becomes runnable, the
...
described in detail in [Lock types and their rules](../../locking/locktypes.html).
In a non-PREEMPT\_RT configuration, a spinlock\_t is acquired by first disabling
...
To address this, PREEMPT\_RT replaces spinning locks with sleeping spin locks
that do not disable preemption. On PREEMPT\_RT, spinlock\_t is implemented using
...
Lock types such as spinlock\_t and mutex\_t in a PREEMPT\_RT enabled kernel are
...
For example, if a SCHED\_FIFO task A blocks on a lock currently held by a
SCHED\_OTHER task B, task A’s scheduling policy and priority are temporarily
...
under scheduler control, PREEMPT\_RT enforces threaded interrupt handlers.
...
stage, the threaded handler, is the function passed to [`request_irq()`](../genericirq.html#c.request_irq "request_irq") as the
...
By default, the threaded handler executes with the SCHED\_FIFO scheduling policy
and a priority of 50 (MAX\_RT\_PRIO / 2), which is midway between the minimum and
...
By using sleeping locks and forced-threaded interrupts, PREEMPT\_RT
