...
> 출처(원문): https://docs.kernel.org/core-api/printk-index.html
...
> * flags:
>   :   optional flags: currently only ‘c’ for KERN\_CONT
> * filename:line:
>   :   source filename and line number of the related
>       [`printk()`](printk-basics.html#c.printk "printk") call. Note that there are many wrappers, for example,
>       [`pr_warn()`](printk-basics.html#c.pr_warn "pr_warn"), `pr_warn_once()`, `dev_warn()`.
> * function:
>   :   function name where the [`printk()`](printk-basics.html#c.printk "printk") call is used.
> * format:
>   :   format string
...
details into the user space will transform particular [`printk()`](printk-basics.html#c.printk "printk") calls
into KABI.

But it is exactly the opposite. [`printk()`](printk-basics.html#c.printk "printk") calls must \_not\_ be KABI.
...
a dedicated .elf section “.printk\_index”. It is achieved using macro
wrappers doing `__printk_index_emit()` together with the real [`printk()`](printk-basics.html#c.printk "printk")
...
used [`printk()`](printk-basics.html#c.printk "printk") calls, including, for example, [`pr_warn()`](printk-basics.html#c.pr_warn "pr_warn"), or `pr_once()`.

Additional changes are necessary for various subsystem specific wrappers
that call the original [`printk()`](printk-basics.html#c.printk "printk") via a common helper function. These needs
...
The macro [`pr_fmt()`](printk-basics.html#c.pr_fmt "pr_fmt") macro allows to define a prefix that is printed
before the string generated by the related [`printk()`](printk-basics.html#c.printk "printk") calls.
