...
> 출처(원문): https://docs.kernel.org/core-api/debugging-via-ohci1394.html
...
Pass the remote\_dma=1 parameter to the driver to get unfiltered physical DMA.
...
To activate it, enable CONFIG\_PROVIDE\_OHCI1394\_DMA\_INIT (Kernel hacking menu:
Remote debugging over FireWire early on boot) and pass the parameter
“ohci1394\_dma=early” to the recompiled kernel on boot.

## Tools

firescope - Originally developed by Benjamin Herrenschmidt, Andi Kleen ported
it from PowerPC to x86 and x86\_64 and added functionality, firescope can now
...
connections and fully initialize all FireWire hardware \_\_before\_\_ the
...
4. Prepare for debugging with early OHCI-1394 initialization:
   ...
   Compile the kernel to be debugged with CONFIG\_PROVIDE\_OHCI1394\_DMA\_INIT
...
5. Retrieving the printk buffer contents:
   ...
   CONFIG\_PROVIDE\_OHCI1394\_DMA\_INIT enabled, with the option ohci1394\_dma=early.
