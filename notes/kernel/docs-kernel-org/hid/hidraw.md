...
> 출처(원문): https://docs.kernel.org/hid/hidraw.html
...
the O\_NONBLOCK flag to open(), or by setting the O\_NONBLOCK flag using
fcntl().
...
hidraw\_report\_descriptor struct. Make sure to set the size field of the
hidraw\_report\_descriptor `struct to` the size returned from HIDIOCGRDESCSIZE.

HIDIOCGRAWINFO:
:   Get Raw Info

This ioctl will return a hidraw\_devinfo `struct containing` the bus type, the
