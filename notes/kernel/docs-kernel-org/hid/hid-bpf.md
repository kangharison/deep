...
> 출처(원문): https://docs.kernel.org/hid/hid-bpf.html
...
### [Simple fixup of report descriptor](#id3)
...
### [Add a new feature that requires a new kernel API](#id4)
...
### [Morph a device into something else and control that from userspace](#id5)
...
## [High-level view of HID-BPF](#id8)
...
## [In-tree HID-BPF programs and `udev-hid-bpf`](#id9)
...
## [Available types of programs](#id10)

HID-BPF is built “on top” of BPF, meaning that we use bpf struct\_ops method to
...
### [Available `struct_ops` for HID-BPF:](#id12)

struct hid\_bpf\_ops
:   A BPF struct\_ops of callbacks allowing to attach HID-BPF programs to a HID device
...
`flags`
:   flags used while attaching the struct\_ops to the device. Currently only
...
:   called whenever an event is coming in from the device
    ...
    `ctx`: The HID-BPF context as [`struct hid_bpf_ctx`](#c.hid_bpf_ctx "hid_bpf_ctx")
...
### [User API data structures available in programs:](#id13)

struct hid\_bpf\_ctx
...
a call to [`hid_bpf_get_data()`](#c.hid_bpf_get_data "hid_bpf_get_data") in order to get a pointer to that field.
...
### [Available API that can be used in all HID-BPF struct\_ops programs:](#id14)

\_\_bpf\_kfunc \_\_u8 \*hid\_bpf\_get\_data(struct [hid\_bpf\_ctx](#c.hid_bpf_ctx "hid_bpf_ctx") \*ctx, unsigned int offset, const size\_t rdwr\_buf\_size)
...
### [Available API that can be used in syscall HID-BPF programs or in sleepable HID-BPF struct\_ops programs:](#id15)

\_\_bpf\_kfunc struct [hid\_bpf\_ctx](#c.hid_bpf_ctx "hid_bpf_ctx") \*hid\_bpf\_allocate\_context(unsigned int hid\_id)
...
**returns** A pointer to [`struct hid_bpf_ctx`](#c.hid_bpf_ctx "hid_bpf_ctx") on success, `NULL` on error.

\_\_bpf\_kfunc void hid\_bpf\_release\_context(struct [hid\_bpf\_ctx](#c.hid_bpf_ctx "hid_bpf_ctx") \*ctx)
...
\_\_bpf\_kfunc int hid\_bpf\_hw\_request(struct [hid\_bpf\_ctx](#c.hid_bpf_ctx "hid_bpf_ctx") \*ctx, \_\_u8 \*buf, size\_t buf\_\_sz, enum hid\_report\_type rtype, enum hid\_class\_request reqtype)
:   Communicate with a HID device

**Parameters**

`struct hid_bpf_ctx *ctx`
:   the HID-BPF context previously allocated in [`hid_bpf_allocate_context()`](#c.hid_bpf_allocate_context "hid_bpf_allocate_context")
...
\_\_bpf\_kfunc int hid\_bpf\_hw\_output\_report(struct [hid\_bpf\_ctx](#c.hid_bpf_ctx "hid_bpf_ctx") \*ctx, \_\_u8 \*buf, size\_t buf\_\_sz)
...
\_\_bpf\_kfunc int hid\_bpf\_try\_input\_report(struct [hid\_bpf\_ctx](#c.hid_bpf_ctx "hid_bpf_ctx") \*ctx, enum hid\_report\_type type, u8 \*buf, const size\_t buf\_\_sz)
...
\_\_bpf\_kfunc int hid\_bpf\_input\_report(struct [hid\_bpf\_ctx](#c.hid_bpf_ctx "hid_bpf_ctx") \*ctx, enum hid\_report\_type type, u8 \*buf, const size\_t buf\_\_sz)
:   Inject a HID report in the kernel from a HID device
...
## [General overview of a HID-BPF program](#id16)

### [Accessing the data attached to the context](#id17)

The `struct hid_bpf_ctx` doesn’t export the `data` fields directly and to access
it, a bpf program needs to first call [`hid_bpf_get_data()`](#c.hid_bpf_get_data "hid_bpf_get_data").
...
### [Effect of a HID-BPF program](#id18)

For all HID-BPF attachment types except for `hid_rdesc_fixup()`, several eBPF
programs can be attached to the same device. If a HID-BPF struct\_ops has a
`hid_rdesc_fixup()` while another is already attached to the device, the
kernel will return -EINVAL when attaching the struct\_ops.
...
HID stack will work on the modified data, with the `size` field of the last hid\_bpf\_ctx
...
[`hid_bpf_allocate_context()`](#c.hid_bpf_allocate_context "hid_bpf_allocate_context") and must release it with [`hid_bpf_release_context()`](#c.hid_bpf_release_context "hid_bpf_release_context")
before returning.
Once the context is retrieved, one can also request a pointer to kernel memory with
[`hid_bpf_get_data()`](#c.hid_bpf_get_data "hid_bpf_get_data"). This memory is big enough to support all input/output/feature
...
Whenever a struct\_ops containing a `SEC("struct_ops/hid_rdesc_fixup")` program
is attached (if no program was attached before), the kernel immediately disconnects
the HID device and does a reprobe.

In the same way, when this struct\_ops is detached, the kernel issues a disconnect
on the device.

There is no `detach` facility in HID-BPF. Detaching a program happens when
all the user space file descriptors pointing at a HID-BPF struct\_ops link are closed.
...
The previous owner will likely pin the struct\_ops link in the bpffs, and we can then
replace it through normal bpf operations.

## [Attaching a bpf program to a device](#id19)

We now use standard struct\_ops attachment through `bpf_map__attach_struct_ops()`.
But given that we need to attach a struct\_ops to a dedicated HID device, the caller
must set `hid_id` in the struct\_ops map before loading the program in the kernel.
...
## [An (almost) complete example of a BPF enhanced HID device](#id20)
...
BPF filesystem through a call to `bpf_obj_pin()`, as with any bpf\_link.

### [Controlling the device](#id22)
