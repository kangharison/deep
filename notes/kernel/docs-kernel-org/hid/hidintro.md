...
> 출처(원문): https://docs.kernel.org/hid/hidintro.html
...
* [Manual parsing of HID report descriptors](hidreport-parsing.html)
...
[HID I/O Transport Drivers](hid-transport.html)). Devices may misbehave because the
...
(USB, I2C, Bluetooth, etc.). See [HID I/O Transport Drivers](hid-transport.html).

## [Parsing HID report descriptors](#id5)
...
directly accessing the hidraw driver [[1]](#hidraw).
...
[Manual parsing of HID report descriptors](hidreport-parsing.html); you only need to understand it
...
### [Output, Input and Feature Reports](#id6)
...
## [Collections, Report IDs and Evdev events](#id7)
...
## [When something does not work](#id9)
...
**HID\_QUIRK\_IGNORE**: ignore this device

**HID\_QUIRK\_NOGET**:

**HID\_QUIRK\_HIDDEV\_FORCE**:

**HID\_QUIRK\_BADPAD**:

**HID\_QUIRK\_MULTI\_INPUT**:

**HID\_QUIRK\_HIDINPUT\_FORCE**:

**HID\_QUIRK\_ALWAYS\_POLL**:

**HID\_QUIRK\_INPUT\_PER\_APP**:

**HID\_QUIRK\_X\_INVERT**:

**HID\_QUIRK\_Y\_INVERT**:

**HID\_QUIRK\_IGNORE\_MOUSE**:

**HID\_QUIRK\_SKIP\_OUTPUT\_REPORTS**:

**HID\_QUIRK\_SKIP\_OUTPUT\_REPORT\_ID**:

**HID\_QUIRK\_NO\_OUTPUT\_REPORTS\_ON\_INTR\_EP**:

**HID\_QUIRK\_HAVE\_SPECIAL\_DRIVER**:

**HID\_QUIRK\_INCREMENT\_USAGE\_ON\_DUPLICATE**:

**HID\_QUIRK\_IGNORE\_SPECIAL\_DRIVER**

**HID\_QUIRK\_POWER\_ON\_AFTER\_BACKLIGHT**

**HID\_QUIRK\_FULLSPEED\_INTERVAL**:

**HID\_QUIRK\_NO\_INIT\_REPORTS**:

**HID\_QUIRK\_NO\_IGNORE**:

**HID\_QUIRK\_NO\_INPUT\_SYNC**:
...
See [Submitting patches: the essential guide to getting your code into the kernel](../process/submitting-patches.html) for guidelines on how
to submit a patch. Quirks for other busses need to go into hid-quirks.c.

### [Fixing HID report descriptors](#id11)

Should you need to patch HID report descriptors the easiest way is to
resort to eBPF, as described in [HID-BPF](hid-bpf.html).
...
for your code, see e.g. samples/hid/hid\_mouse.bpf.c:
...
Check [Manual parsing of HID report descriptors](hidreport-parsing.html) if you need any help
...
everyone else. See [Submitting patches: the essential guide to getting your code into the kernel](../process/submitting-patches.html) for
guidelines on how to do this.

### [Modifying the transmitted data on the fly](#id12)
...
### [Writing a specialized driver](#id13)
...
read hidraw: see [HIDRAW - Raw Access to USB and Bluetooth Human Interface Devices](hidraw.html) and
