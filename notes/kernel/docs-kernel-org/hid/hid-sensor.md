...
> 출처(원문): https://docs.kernel.org/hid/hid-sensor.html
...
if there is a field report\_interval, which is parsed by a call to
sensor\_hub\_input\_get\_attribute\_info before, then it can directly set that
individual field:
...
if there is a field report\_interval, which is parsed by a call to
sensor\_hub\_input\_get\_attribute\_info before, then it can directly get that
