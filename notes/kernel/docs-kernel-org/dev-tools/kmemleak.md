...
> 출처(원문): https://docs.kernel.org/dev-tools/kmemleak.html
...
is configured via the CONFIG\_DEBUG\_KMEMLEAK\_MEM\_POOL\_SIZE option.

If CONFIG\_DEBUG\_KMEMLEAK\_DEFAULT\_OFF are enabled, the kmemleak is
disabled by default. Passing `kmemleak=on` on the kernel command
line enables the function.

If you are getting errors like “Error while writing to stdout” or “write\_loop:
Invalid argument”, make sure kmemleak is properly enabled.

## Basic Algorithm

The memory allocations via [`kmalloc()`](../core-api/mm-api.html#c.kmalloc "kmalloc"), [`vmalloc()`](../core-api/mm-api.html#c.vmalloc "vmalloc"),
[`kmem_cache_alloc()`](../core-api/mm-api.html#c.kmem_cache_alloc "kmem_cache_alloc") and
...
* `kmemleak_vmalloc` - notify of a [`vmalloc()`](../core-api/mm-api.html#c.vmalloc "vmalloc") memory allocation
...
* `kmemleak_alloc_recursive` - as kmemleak\_alloc but checks the recursiveness
* `kmemleak_free_recursive` - as kmemleak\_free but checks the recursiveness
...
module, a module that deliberately leaks memory. Set CONFIG\_SAMPLE\_KMEMLEAK
