...
> 출처(원문): https://docs.kernel.org/dev-tools/kfence.html
...
sizes up to PAGE\_SIZE are supported). At this point, the timer is reset, and
...
bool is\_kfence\_address(const void \*addr)
...
corruptions, therefore [`is_kfence_address()`](#c.is_kfence_address "is_kfence_address") must be used to check whether
...
introducing another load and therefore need to keep KFENCE\_POOL\_SIZE a
constant (until immediate patching support is added to the kernel).

void kfence\_shutdown\_cache(struct kmem\_cache \*s)
...
void \*kfence\_alloc(struct kmem\_cache \*s, size\_t size, gfp\_t flags)
...
[`kfence_alloc()`](#c.kfence_alloc "kfence_alloc") should be inserted into the heap allocation fast path,
allowing it to transparently return KFENCE-allocated objects with a low
probability using a static branch (the probability is controlled by the
kfence.sample\_interval boot parameter).

size\_t kfence\_ksize(const void \*addr)
...
[`kfence_ksize()`](#c.kfence_ksize "kfence_ksize") returns the number of bytes requested for a KFENCE object at
allocation time. This number may be less than the object size of the
corresponding `struct kmem_cache`.

void \*kfence\_object\_start(const void \*addr)
...
void \_\_kfence\_free(void \*addr)
...
Requires: is\_kfence\_address(addr)

Release a KFENCE object and mark it as freed.

bool kfence\_free(void \*addr)
...
bool kfence\_handle\_page\_fault(unsigned long addr, bool is\_write, struct pt\_regs \*regs)
