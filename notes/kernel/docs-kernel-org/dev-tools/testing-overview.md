...
> 출처(원문): https://docs.kernel.org/dev-tools/testing-overview.html
...
KUnit ([KUnit - Linux Kernel Unit Testing](kunit/index.html)) is an entirely in-kernel system
...
kselftest ([Linux Kernel Selftests](kselftest.html)), on the other hand, is
...
[Using gcov with the Linux kernel](gcov.html) is GCC’s coverage testing tool, which can be
...
[KCOV: code coverage for fuzzing](kcov.html) is a feature which can be built in to the
...
* KASAN detects invalid memory accesses such as out-of-bounds and
  use-after-free errors. See [Kernel Address Sanitizer (KASAN)](kasan.html)
* UBSAN detects behaviour that is undefined by the C standard, like integer
  overflows. See [Undefined Behavior Sanitizer - UBSAN](ubsan.html)
* KCSAN detects data races. See [Kernel Concurrency Sanitizer (KCSAN)](kcsan.html)
* KFENCE is a low-overhead detector of memory issues, which is much faster than
  KASAN and can be used in production. See [Kernel Electric-Fence (KFENCE)](kfence.html)
* lockdep is a locking correctness validator. See
  [Runtime locking correctness validator](../locking/lockdep-design.html)
* Runtime Verification (RV) supports checking specific behaviours for a given
  subsystem. See [Runtime Verification](../trace/rv/runtime-verification.html)
...
examining the code. See the [Sparse](sparse.html) documentation
...
[Coccinelle](coccinelle.html) documentation page for details.
