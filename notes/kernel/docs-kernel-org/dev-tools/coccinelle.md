...
> 출처(원문): https://docs.kernel.org/dev-tools/coccinelle.html
...
DEBUG\_FILE support is only supported when using coccinelle >= 1.0.2.

Currently, DEBUG\_FILE support is only available to check folders, and
not single files. This is because checking a single file requires spatch
to be called twice leading to DEBUG\_FILE being set both times to the same value,
