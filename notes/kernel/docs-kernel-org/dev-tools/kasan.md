...
> 출처(원문): https://docs.kernel.org/dev-tools/kasan.html
...
Generic KASAN, enabled with CONFIG\_KASAN\_GENERIC, is the mode intended for
...
Software Tag-Based KASAN or SW\_TAGS KASAN, enabled with CONFIG\_KASAN\_SW\_TAGS,
...
Hardware Tag-Based KASAN or HW\_TAGS KASAN, enabled with CONFIG\_KASAN\_HW\_TAGS,
...
Generic KASAN is supported on x86\_64, arm, arm64, powerpc, riscv, s390, xtensa,
...
Generic KASAN supports finding bugs in all of slab, page\_alloc, vmap, vmalloc,
stack, and global memory.

Software Tag-Based KASAN supports slab, page\_alloc, vmalloc, and stack memory.

Hardware Tag-Based KASAN supports slab, page\_alloc, and non-executable vmalloc
memory.
...
* `kasan.mode=sync`, `=async` or `=asymm` controls whether KASAN
  ...
  fault occurs, the information is stored in hardware (in the TFSR\_EL1
...
* `kasan.page_alloc.sample=<sampling interval>` makes KASAN tag only every
  Nth page\_alloc allocation with the order equal or greater than
...
* `kasan.page_alloc.sample.order=<minimum page order>` specifies the minimum
  ...
  than `1`.
  This parameter is intended to allow sampling only large page\_alloc
...
the accessed address (see the [Implementation details](#implementation-details) section).
...
[`call_rcu()`](../core-api/kernel-api.html#c.call_rcu "call_rcu") and workqueue queuing.

### CONFIG\_KASAN\_EXTRA\_INFO

Enabling CONFIG\_KASAN\_EXTRA\_INFO allows KASAN to record and report more
...
Here is the report with CONFIG\_KASAN\_EXTRA\_INFO enabled (only the
...
Note that enabling CONFIG\_KASAN\_HW\_TAGS always results in in-kernel TBI being
