...
> 출처(원문): https://docs.kernel.org/dev-tools/kunit/start.html
...
This page contains an overview of the kunit\_tool and KUnit framework,
...
kunit\_tool is a Python script, which configures and builds a kernel, runs
...
the kernel” section of the [admin-guide](../../admin-guide/README.html),
...
By default, kunit\_tool runs all tests reachable with minimal configuration,
that is, using default values for most of the kconfig options. However,
you can select which tests to run by:

* [Customizing Kconfig](#customizing-kconfig) used to compile the kernel, or
* [Filtering tests by name](#filtering-tests-by-name) to select specifically which compiled tests to run.
...
Before running the tests, kunit\_tool ensures that all config options
...
1. Edit `.kunit/.kunitconfig`. The file should contain the list of kconfig
   ...
   You may want to remove CONFIG\_KUNIT\_ALL\_TESTS from the `.kunitconfig` as
   it will enable a number of additional tests that you may not want.
   If you need to run on an architecture other than UML see [Running tests on QEMU](run_wrapper.html#kunit-on-qemu).
...
(read instructions regarding the pattern in the manpage *[glob(7)](https://man7.org/linux/man-pages/man7/glob.7.html)*).
...
take a look at [Running tests with kunit\_tool](run_wrapper.html)
...
for testing different things), see [Writing Tests](usage.html)
