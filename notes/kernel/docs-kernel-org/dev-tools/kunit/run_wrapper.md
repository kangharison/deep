...
> 출처(원문): https://docs.kernel.org/dev-tools/kunit/run_wrapper.html
...
We can either run KUnit tests using kunit\_tool or can run tests
manually, and then use kunit\_tool to parse the results. To run tests
manually, see: [Run Tests without kunit\_tool](run_manual.html).
...
kunit\_tool will generate a `.kunitconfig` with a default
...
kunit\_tool. This is useful if we have several different groups of
...
To view kunit\_tool flags (optional command-line arguments), run:
...
kunit\_tool ensures that all config options in `.kunitconfig` are
...
When running kunit\_tool, from a `.kunitconfig`, we can generate a
...
The `run` command discussed in section: **Running tests with kunit\_tool**,
is equivalent to running the above three commands in sequence.

## Parsing test results

KUnit tests output displays results in TAP (Test Anything Protocol)
format. When running tests, kunit\_tool parses this output and prints
...
kunit\_tool. This accepts a filename for an argument, or will read from
standard input.
...
kunit\_tool supports running tests on qemu as well as
...
* `--arch`: Selects a configs collection (Kconfig, qemu config options
  ...
  for example: i386, x86\_64, arm and so on; run on qemu.
...
Once we have a `QemuConfig`, we can pass it into kunit\_tool,
...
kunit\_tool has a number of other command-line arguments which can
...
* `--build_dir`: Specifies kunit\_tool build directory. It includes
...
* `--arch`: Runs tests on the specified architecture. The architecture
  ...
  For example, i386, x86\_64, arm, um, etc. Non-UML architectures run on qemu.
  Default is um.
* `--cross_compile`: Specifies the Kbuild toolchain. It passes the
  same argument as passed to the `CROSS_COMPILE` variable used by
...
The kunit\_tool comes with a bash completion script:
