...
> 출처(원문): https://docs.kernel.org/dev-tools/kunit/faq.html
...
(see [Running tests on QEMU](run_wrapper.html#kunit-on-qemu)).

In short, yes, you can run KUnit on other architectures, but it might require
more work than using KUnit on UML.

For more information, see [Writing Tests For Other Architectures](usage.html#kunit-on-non-uml).
...
1. Run `./tools/testing/kunit/kunit.py run` with the `--raw_output`
   parameter. This might show details or error messages hidden by the kunit\_tool
   parser.
...
4. Make sure the kernel .config has `CONFIG_KUNIT=y` and at least one test
   (e.g. `CONFIG_KUNIT_EXAMPLE_TEST=y`). kunit\_tool will keep its .config
...
6. Finally, try running KUnit outside UML. KUnit and KUnit tests can be
   ...
   can be parsed with `kunit.py parse`. For more details, see [Running tests on QEMU](run_wrapper.html#kunit-on-qemu).
