...
> 출처(원문): https://docs.kernel.org/dev-tools/kunit/architecture.html
...
* [kunit\_tool (Command-line Test Harness)](#kunit-tool-command-line-test-harness)
...
`struct called` [`struct kunit_case`](api/test.html#c.kunit_case "kunit_case").
...
The test case includes a [`KUNIT_CASE_PARAM()`](api/test.html#c.KUNIT_CASE_PARAM "KUNIT_CASE_PARAM") macro that accepts a
...
## kunit\_tool (Command-line Test Harness)
...
[Run Tests without kunit\_tool](run_manual.html)) or use `kunit_tool`
(see [Running tests with kunit\_tool](run_wrapper.html)).
...
* `build` runs `make` on the kernel tree with required options
  (depends on the architecture and some options, for example: build\_dir)
