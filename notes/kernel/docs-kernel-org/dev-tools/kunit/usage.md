...
> 출처(원문): https://docs.kernel.org/dev-tools/kunit/usage.html
...
To learn about more KUnit expectations, see [Test API](api/test.html).
...
For more information, see [Test API](api/test.html).
...
[Test API](api/test.html) and the
[Resource API](api/resource.html)).
...
[Resource API](api/resource.html). To see the full version of this
...
Note that, for functions like device\_unregister which only accept a single
...
avoid resource leaks. For more information, see [Resource API](api/resource.html).
...
described in [Devres - Managed Device Resource](../../driver-api/driver-model/devres.html)
