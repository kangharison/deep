...
> 출처(원문): https://docs.kernel.org/dev-tools/kunit/run_manual.html
> 자동 미러링: docs.kernel.org · 정본은 출처 URL (영문 원문 자동 변환본)

---

# Run Tests without kunit\_tool

If we do not want to use kunit\_tool (For example: we want to integrate
...
KUnit tests can run without kunit\_tool. This can be useful, if:

* We have an existing kernel configuration to test.
* Need to run on real hardware (or using an emulator/VM kunit\_tool
  does not support).
...
By default, kunit.enable is set to 1 because KUNIT\_DEFAULT\_ENABLED is
...
information about debugfs at [DebugFS](../../filesystems/debugfs.html)).
...
[`kunit_test_init_section_suites()`](api/test.html#c.kunit_test_init_section_suites "kunit_test_init_section_suites") macro.
