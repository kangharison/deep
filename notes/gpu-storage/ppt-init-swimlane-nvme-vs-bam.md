...
### Phase 한 줄 요약
- **① Module Load** — `nvme.ko` 로드, PCI 드라이버 등록
- **② PCI Probe** — `nvme_dev` 구조체 할당 + `dev->queues[]` 빈 슬롯 확보 (N+1개)
- **③ BAR0 Map** — NVMe 컨트롤러의 MMIO 영역을 커널에 매핑
- **④ Ctrl Enable** — PCI 활성화 + CAP 읽기 + **Admin Q ring 할당** + `CC.EN=1` (컨트롤러 기동)
- **⑤ Admin blk-mq** — Admin용 blk-mq tag_set 구성 (Identify 발행 경로 확보)
- **⑥ Identify** — Admin Q로 Controller/NS 정보 조회 + IO Q 개수 협상
- **⑦ I/O Q Setup** — I/O Queue N개 생성 (DMA ring + CREATE_IO_CQ/SQ)
- **⑧ blk-mq + NS Scan** — IO용 blk-mq + namespace scan → `/dev/nvmeXn1` 등록

```
 시간 ────────────────────────────────────────────────────────────────────────────────▶
         │①Module │②PCI    │③BAR0   │④Ctrl    │⑤Admin  │⑥Identify│⑦I/O Q  │⑧blk-mq │
         │ Load   │ Probe  │ Map    │ Enable  │ blk-mq │         │ Setup  │+NS scan│
 ════════╪════════╪════════╪════════╪═════════╪════════╪═════════╪════════╪════════╡
         │        │        │        │         │        │         │        │        │
 User    │        │        │        │         │        │         │        │┌──────┐│
 space   │        │        │        │         │        │         │        ││/dev/ ││
 ...
 ────────┼────────┼────────┼────────┼─────────┼────────┼─────────┼────────┼────────┤
         │        │        │        │         │        │         │        │        │
 NVMe    │┌──────┐│┌──────┐│┌──────┐│┌──────┐ │        │┌───────┐│┌──────┐│        │
 PCIe    ││pci_  │││nvme_ │││dev-> │││CC.EN=1│ │        ││ctrl   │││IO Q  │││        │
 driver  ││drvr  │││dev + │││bar   │││+admin │ │        ││info   │││×N    │││        │
 (kernel)││등록  │││queues│││ioremap││Q ring │ │        ││stored │││created │       │
         │└──────┘││[] 빈 │││      │││alloc  │ │        ││+ num_ │││      │││        │
         │        ││슬롯  │││      │││(slot 0)        │ ││queues │││(slot  │        │
         │        ││(N+1) │││      │││       │ │        ││정해짐 │││1..N) │││        │
         │        │└──────┘│└──────┘│└──────┘ │        │└───────┘│└──────┘│        │
 ════════╪════════╪════════╪════════╪═════════╪════════╪═════════╪════════╪════════╡
         │        │        │        │┌──────┐ │        │         │┌──────┐│        │
 Host    │        │        │        ││admin │ │        │         ││IO    ││ PRP    │
 DRAM    │        │        │        ││SQ/CQ │ │        │         ││SQ/CQ ││ buf    │
 (DMA-   │        │        │        ││ring  │ │        │         ││rings ││ 적재   │
  coh)   │        │        │        ││(slot0)        │ │         ││×N    ││        │
         │        │        │        │└──┬───┘ │        │         │└──┬───┘│        │
         │        │        │        │   │DMA주소       │         │   │    │        │
 ...
 ────────┼────────┼────────┼────────┼───┼─────┼────────┼─────────┼───┼────┼────────┤
         │        │        │        │   ▼     │        │         │   ▼    │        │
 SSD     │        │        │        │┌──────┐ │        │┌───────┐│┌──────┐│ SQE    │
 Ctrl FW │        │        │        ││admin │ │        ││Identify││IO Q  ││ fetch  │
 (on-SSD)│        │        │        ││Q     │ │        ││명령   │││arb + ││ →FTL   │
         │        │        │        ││proc  │ │        ││처리   │││DMA   ││ →LBA   │
         │        │        │        ││CSTS. │ │        ││(→CQE) │││engine││        │
         │        │        │        ││RDY   │ │        │└───────┘│││+FTL  ││        │
